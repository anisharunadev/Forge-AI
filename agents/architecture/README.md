# Architecture Style Detector (FORA-29, sub-goal 2.2)
...
for the 10 styles in the FORA-29 acceptance criteria plus a human-readable
rationale.

## Inputs

- `forge/2.2/input/codebase-graph.json` — the 2.1 graph artefact
  (schemaVersion 1, produced by `arch-analyzer/m1` per FORA-27).
...
| 5. < 10 s, no model spend | Pure-Python, ~3 ms warm on the FORA-27 graph; `cost_usd = 0.0` | `smoke_test` step "cost bound" |
...
```bash
# (one-time) copy the FORA-27 attachment into the workspace
