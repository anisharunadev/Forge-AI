"""
Audit recorder for the Security Agent (FORA-74 AC #4).

Every tool call the Security Agent makes is recorded so the
...
type here so the Security Agent does not import the broader
...
class ToolCallRecord:
    """One row in the Security Agent's per-process audit log.
...
# token here is one a Developer Agent uses that the Security Agent
# must NOT use.
_FORBIDDEN_TOKENS = (
    "developer_prompt",
    "developer_scratch",
...
def _digest(payload: Any) -> str:
    ...
    The Security Agent does NOT store the payload body — only
...
class SecurityAuditRecorder:
    """In-process audit recorder for the Security Agent.
    ...
    def record(
        ...
        *,
        ...
        rec = ToolCallRecord(
            record_id=f"sec-audit-{uuid.uuid4().hex[:12]}",
            run_id=self._run_id,
