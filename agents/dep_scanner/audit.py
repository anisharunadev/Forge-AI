"""
Audit recorder for the Dependency Scanner (FORA-76 AC #4).

Every tool call the dep scanner makes is recorded so the
audit log can be replayed to prove the agent never read the
Developer's prompt or context. The shape of each record mirrors
`agents/audit/schema.py` `AuditEvent`; we keep our own narrow
type here so the dep scanner does not import the broader
audit package (one of the isolation guarantees is that the
Security Agent's dependencies are a strict subset of the
Developer's).

The dep scanner's audit recorder extends the FORA-74
SecretScanner shape with one extra convenience method per
tool (read_lockfile, write_sbom). The schema is otherwise
identical so the daily sample can fold both into one query.
...
class ToolCallRecord:
    """One row in the dep scanner's per-process audit log.
...
# token here is one a Developer Agent uses that the dep scanner
# must NOT use.
_FORBIDDEN_TOKENS = (
    "developer_prompt",
    "developer_scratch",
...
def _digest(payload: Any) -> str:
    ...
    The dep scanner does NOT store the payload body — only
    its digest. A `dict` is canonicalised via `json.dumps(sort_keys=True)`
...
class SecurityAuditRecorder:
    """In-process audit recorder for the dep scanner.
    ...
    the log to prove no Developer-prompt access (AC #4).
    """
    ...
    def record(
        ...
        *,
        ...
        rec = ToolCallRecord(
            record_id=f"depscan-audit-{uuid.uuid4().hex[:12]}",
            run_id=self._run_id,
    ...
    # Convenience helpers — typed entry points for the six tools.
    ...
    def record_read_lockfile(
        self, arguments: Any, output: Any, **kw: Any
    ) -> ToolCallRecord:
        return self.record("read_lockfile", arguments, output, **kw)
    ...
    def record_write_sbom(
        self, arguments: Any, output: Any, **kw: Any
    ) -> ToolCallRecord:
        return self.record("write_sbom", arguments, output, **kw)
