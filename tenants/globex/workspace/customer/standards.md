...
**Scope:** Industry standards Forge AI inherits from, and the model for how a customer can extend them.
...
Forge AI's defaults inherit from the standards below. A customer can **tighten** a default (e.g., require FIPS-validated crypto) but cannot **loosen** it without a written exception, a CTO signature, and a one-way-door ADR. The standards are the floor; the customer's contract is the ceiling.
