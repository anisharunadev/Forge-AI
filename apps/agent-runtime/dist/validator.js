 ...
 */
export function validateSubAgent(def) {
...
function validateHandler(agentId, h, actAllowed) {
    if (h.sideEffect !== 'write')
        return null;
    if (h.idempotencyKey !== undefined)
        return null;
    const placeholderRunId = asRunId('');
    return makeError({
        code: 'IdempotencyMissing',
        message: `handler "${h.handlerId}" (tool "${h.toolName}") on agent "${agentId}" is sideEffect=write and ${actAllowed.has(h.toolName) ? 'act-allowed' : 'not act-allowed'} but lacks idempotencyKey`,
