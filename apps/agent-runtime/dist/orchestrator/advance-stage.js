 ...
 */
import { classify, nextStage, TERMINAL_STAGE, } from './stage-table.js';
import { asEventId } from './types.js';
import { runStateAfterDecision } from './state-machine.js';
import { InvalidDecisionError, InvalidTransitionError, RunNotFoundError, RunNotRunningError, StageMismatchError, } from './errors.js';
// ---------------------------------------------------------------------------
// Public entry
// ---------------------------------------------------------------------------
export async function advanceStage(request, deps) {
...
async function handleInvalidTransition(request, run, deps, reason) {
    const now = deps.now?.() ?? Date.now();
    const actor = request.requestedBy ?? asActorId(deps.systemActorId ?? 'system');
...
async function applyTransition(request, run, deps) {
    const now = deps.now?.() ?? Date.now();
    const actor = request.requestedBy ?? asActorId(deps.systemActorId ?? 'system');
    const mintEvt = deps.mintEventId ?? defaultEventIdMint;
    const occurredAt = new Date(now).toISOString();
    // Compute the new stage + run state for the store.
    const newStage = request.decision.kind === 'abort' ? request.fromStage : request.toStage;
    ...
    if (request.decision.kind === 'next') {
        events.push(buildGatePassedEvent(request, run.currentStage, request.toStage, actor, mintEvt, occurredAt));
...
function buildGatePassedEvent(request, from, to, actor, mintEvt, occurredAt) {
