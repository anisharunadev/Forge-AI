/**
 * Budget meter — §7 of the design doc.
 *
 * Tracks three axes per run: tokens (input + output), USD, wall-clock ms.
 * Spend is recorded at exactly four sites:
 *
 *   1. Planner LLM result            (recordPlannerLlm)
 *   2. Reflector LLM result          (recordReflectorLlm)
 *   3. Tool handler `costHint`       (recordToolCostHint)
 *   4. Stage-boundary wall-clock     (recordStageBoundaryWallClock)
 *
 * Pre-stage check raises `BudgetExceeded` with `{ ceiling, spent }`
 * snapshots when the run would exceed any axis. After `BudgetExceeded`
 * the stage machine aborts the run; no further stage runs execute.
 *
 * The meter is pure state — no I/O. The stage machine is the only caller.
 */

import { makeError, type RunId, type Stage, type TypedError } from './types.js';

export interface Budget {
  /** Token ceiling (input + output). Optional. */
  tokenCeiling?: number;
  /** USD ceiling. Optional. */
  usdCeiling?: number;
  /** Wall-clock ceiling in ms. Optional. */
  wallClockCeilingMs?: number;
}

export interface SpendSnapshot {
  tokens: number;
  usd: number;
  wallClockMs: number;
}

export interface LlmUsage {
  inputTokens?: number;
  outputTokens?: number;
  /** Total tokens (input + output). Optional; the meter sums what's given. */
  tokens?: number;
  usd?: number;
}

export interface ToolCostHint {
  tokens?: number;
  usd?: number;
}

export class BudgetExceededError extends Error {
  constructor(
    public readonly ceiling: Budget,
    public readonly spent: SpendSnapshot,
    public readonly stage: Stage,
    public readonly runId: RunId,
  ) {
    super(
      `budget exceeded at stage "${stage}": spent ${JSON.stringify(spent)} of ${JSON.stringify(ceiling)}`,
    );
    this.name = 'BudgetExceededError';
  }
}

export function toBudgetExceededTypedError(err: BudgetExceededError): TypedError {
  return makeError({
    code: 'BudgetExceeded',
    message: err.message,
    runId: err.runId,
    stage: err.stage,
    ceiling: err.ceiling,
    spent: err.spent,
  });
}

function addTokens(target: SpendSnapshot, usage: LlmUsage | ToolCostHint): void {
  if (typeof usage.tokens === 'number') {
    target.tokens += usage.tokens;
    return;
  }
  if ('inputTokens' in usage || 'outputTokens' in usage) {
    const u = usage as LlmUsage;
    target.tokens += (u.inputTokens ?? 0) + (u.outputTokens ?? 0);
  }
}

function addUsd(target: SpendSnapshot, usage: LlmUsage | ToolCostHint): void {
  if (typeof usage.usd === 'number') target.usd += usage.usd;
}

export class BudgetMeter {
  private readonly _spent: SpendSnapshot = { tokens: 0, usd: 0, wallClockMs: 0 };
  private readonly ceiling: Budget;
  private readonly startedAtMs: number;
  private readonly now: () => number;

  constructor(args: { runId: RunId; ceiling: Budget; startedAtMs: number; now: () => number }) {
    this.ceiling = args.ceiling;
    this.startedAtMs = args.startedAtMs;
    this.now = args.now;
  }

  spent(): SpendSnapshot {
    return { ...this._spent };
  }

  /**
   * Throw `BudgetExceededError` if any axis would exceed its ceiling if
   * the stage at hand began now. Called *before* each stage.
   */
  checkBeforeStage(stage: Stage, runId: RunId): void {
    const projected = this.project();
    const violations = this.violations(projected);
    if (violations.length === 0) return;
    throw new BudgetExceededError(this.ceiling, projected, stage, runId);
  }

  recordPlannerLlm(usage: LlmUsage): void {
    addTokens(this._spent, usage);
    addUsd(this._spent, usage);
  }

  recordReflectorLlm(usage: LlmUsage): void {
    addTokens(this._spent, usage);
    addUsd(this._spent, usage);
  }

  recordToolCostHint(hint: ToolCostHint): void {
    addTokens(this._spent, hint);
    addUsd(this._spent, hint);
  }

  /** Update the wall-clock axis with the elapsed ms since the run started. */
  recordStageBoundaryWallClock(): void {
    this._spent.wallClockMs = Math.max(0, this.now() - this.startedAtMs);
  }

  private project(): SpendSnapshot {
    return { ...this._spent, wallClockMs: Math.max(0, this.now() - this.startedAtMs) };
  }

  private violations(s: SpendSnapshot): string[] {
    const out: string[] = [];
    if (this.ceiling.tokenCeiling !== undefined && s.tokens > this.ceiling.tokenCeiling) out.push('tokens');
    if (this.ceiling.usdCeiling !== undefined && s.usd > this.ceiling.usdCeiling) out.push('usd');
    if (
      this.ceiling.wallClockCeilingMs !== undefined &&
      s.wallClockMs > this.ceiling.wallClockCeilingMs
    )
      out.push('wallClock');
    return out;
  }
}

/** A meter with no ceiling — the default for runs that don't specify one. */
export class UnlimitedBudgetMeter extends BudgetMeter {
  constructor(args: { runId: RunId; now: () => number }) {
    super({ runId: args.runId, ceiling: {}, startedAtMs: args.now(), now: args.now });
  }
  override checkBeforeStage(): void { /* no ceiling, never exceeds */ }
}
