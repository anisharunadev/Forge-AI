...
/**
 * Architecture Center — the main client component.
 *
 * Extracted from `app/architecture/page.tsx` as part of M19
 * (architecture god-page decomposition). The page.tsx is now a
 * thin wrapper that passes `initialTab`; per-tab URLs route
 * through `app/architecture/[tab]/page.tsx`.
 *
 * Step 30 modernization (10 tabs: overview, adrs, contracts, tasks,
 * risks, trace, versions, radar, diagrams, security). See
 * `components/architecture/inline/` for the small helpers that
 * were also extracted from the original monolith.
 *
...
export interface ArchitectureCenterProps {
  /** Initial tab to render. Defaults to `'overview'`. Per-tab URL
   *  segments (e.g. `/architecture/adrs`) override this. */
  readonly initialTab?: TabId;
}

/**
 * ArchitectureCenter — the main client component for the
 * Architecture Center. Renders the tabs shell, command palette,
 * and the active tab's content.
 *
 * Extracted from `app/architecture/page.tsx` (M19). The page.tsx
 * is now a thin wrapper that passes `initialTab`.
 */
export function ArchitectureCenter({ initialTab = 'overview' }: ArchitectureCenterProps = {}) {
  ...
  // Day 2 mock-removal track H — diagrams count (replaces MOCK_DIAGRAMS).
  ...
  // The `initialTab` prop takes priority over the URL `?tab=` param
  // so per-tab routes (`/architecture/[tab]`) render the right tab
  // without requiring a query string.
  const tabParam =
    (initialTab as TabId | null) ??
