...
import { cloudBrokeredEvent, type AuditSink } from './audit.js';
...
export async function brokerAction(
  ...
  if (!trust || trust.trust_state !== 'active') {
    return reject(
      ...
      cloud,
      'cloud_disabled',
      trust?.disabled_reason ?? `no trust record for tenant ${request.tenant_id}`,
      start,
      deps,
    );
  }
  ...
  const adapter: CloudAdapter | undefined = deps.adapters.get(cloud);
  if (!adapter) {
  ...
  if (assume.expires_at_ms - start > MAX_CREDENTIAL_LIFETIME_MS) {
    return reject(
      ...
      cloud,
      'credential_too_long',
      `assumed credential would exceed ${MAX_CREDENTIAL_LIFETIME_MS}ms cap`,
      start,
      deps,
    );
  }
  ...
  await deps.audit.write(
    cloudBrokeredEvent({ result, actor, metadata: { tenant_account: trust.account } }),
  );
...
async function reject(
  request: BrokeredRequest,
  actor: string,
  cloud: BrokeredResult['cloud'] | 'unknown',
  code: BrokeredResult['response_code'],
  reason: string,
  start: number,
  deps: BrokerDeps,
): Promise<BrokeredResult> {
  const trustAccount =
    cloud !== 'unknown' ? deps.trust_store.get(request.tenant_id, cloud)?.account ?? 'unknown' : 'unknown';
  const actionCloud = (CLOUDS as readonly string[]).includes(cloud as string)
    ? (cloud as BrokeredResult['cloud'])
    : 'aws';
  const result: BrokeredResult = {
    trace_id: request.trace_id,
    tenant_id: request.tenant_id,
  ...
  await deps.audit.write(
    cloudBrokeredEvent({ result, actor, metadata: { reject_reason: reason } }),
  );
