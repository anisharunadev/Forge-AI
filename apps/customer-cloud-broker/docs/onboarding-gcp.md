# GCP onboarding runbook (FORA-126 / FORA-126.3 / 0.7.4)
...
identity pool in the customer's project trusts the FORA
...
FORA-signed OIDC token for a short-lived Google access token at
action time (≤15 min), impersonates the GSA, performs the action,
and discards the credential.

> **Adapter status (FORA-126.3 landed)**: the GCP adapter in
> `src/adapters/gcp.ts` is implemented. `probeTrust` validates
> the `cloud_trust.yaml` shape; `assume` exchanges the FORA JWT
...
- The FORA support team has confirmed the **OIDC issuer URL** for
...
- **Audiences**: `customer-cloud-broker` (the broker's
  client_id at the FORA IdP). The broker's
...
gates which FORA subjects can impersonate the GSA):
...
- **Service account ID**: `fora-deploy-agent` (the same string —
  this is what the broker stores in `cloud_trust.yaml` as
  `service_account`).
- **Description**: `Impersonated by the FORA customer-cloud-broker
  via Workload Identity Federation (FORA-126).`
...
the FORA-126.3 adapter in place, the canary probe (phase 2,
FORA-126.4) actually exercises `assume()` against
`https://sts.googleapis.com/v1/token`; a success proves the
customer's WIF provider accepts the broker's OIDC issuer and
audience. Until FORA-126.4 lands, phase 1 is sufficient to mark
...
- **Attribute condition uses a regex like `^agent:deploy-agent:run-.*$`
  ...
  adversarial FORA agent that emits a non-conforming `sub` would
  also pass.
