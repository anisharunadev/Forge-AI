...
function makeHandle(args, expires_at_ms) {
...
function newHolder(client, access_token, expiration_ms) {
...
export class GcpAdapter {
    ...
    async assume(args, for_jwt) {
        ...
        const audience = `//iam.googleapis.com/projects/${args.project_number}` +
    ...
    async perform(handle, args, ctx = {}) {
        ...
        try {
            ...
            if (err instanceof Error) {
                ...
                if (!reason.startsWith('gcp_rate_limited:') &&
...
// ---------------------------------------------------------------------------
async function defaultAssumeFn(input) {
...
// ---------------------------------------------------------------------------
async function defaultDispatchFn(service, operation, params, holder) {
    ...
    try {
        const method = client[opMap.operations[operation]];
...
function gcpRoleFingerprint(args) {
    return 'gcp:' + createHash('sha256')
        .update(`${args.project_number}|${args.workload_identity_pool}|${args.workload_identity_provider}|${args.service_account}`)
        .digest('hex')
        .slice(0, 16);
}
