 ...
 */
export async function createRun(pool, tenantId, body, defaultCostCeiling, client) {
    ...
    try {
        ...
        const insertRun = await ownClient.query(`INSERT INTO agent_runs
       ...
       RETURNING id, tenant_id, goal_id, project_id, status, current_stage,
        ...
        for (const stage of STAGES_IN_ORDER) {
            await ownClient.query(`INSERT INTO agent_run_stages (run_id, stage, status)
           VALUES ($1, $2, 'pending')
           ON CONFLICT (run_id, stage) DO NOTHING`, [runId, stage]);
        }
        if (client === undefined) {
 ...
 */
export async function findRunById(pool, tenantId, runId) {
    const r = await pool.query(`SELECT id, tenant_id, goal_id, project_id, status, current_stage,
      ...
      WHERE id = $1
        AND tenant_id = $2
        AND deleted_at IS NULL`, [runId, tenantId]);
    const row = r.rows[0];
 ...
 */
export async function listStages(pool, tenantId, runId) {
    // The tenant gate is enforced by joining to agent_runs. A soft-deleted
    // run returns zero stage rows, which we map to `null` here.
    const r = await pool.query(`SELECT s.id, s.run_id, s.stage, s.status, s.decision,
      ...
      ORDER BY array_position($3::text[], s.stage::text)`, [runId, tenantId, STAGES_IN_ORDER]);
    if (r.rows.length === 0) {
        return null;
    }
 ...
 */
export async function transitionRunStatus(pool, tenantId, runId, expected, next, client) {
    const executor = client ?? pool;
    const r = await executor.query(`UPDATE agent_runs
        SET status = $4,
      ...
      RETURNING id, tenant_id, goal_id, project_id, status, current_stage,
                triggered_by, cost_ceiling_usd::text AS cost_ceiling_usd,
                cost_spent_usd::text AS cost_spent_usd,
                started_at, finished_at, deleted_at, archived_at`, [runId, tenantId, expected, next]);
    const row = r.rows[0];
    return row ? rowToRun(row) : null;
}
/**
 ...
 */
export async function listActiveRunsForRecovery(pool, tenantId) {
    const r = await pool.query(`SELECT id, tenant_id, goal_id, project_id, status, current_stage,
            triggered_by, cost_ceiling_usd::text AS cost_ceiling_usd,
            cost_spent_usd::text AS cost_spent_usd,
            started_at, finished_at, deleted_at, archived_at
      ...
      WHERE tenant_id = $1
        AND deleted_at IS NULL
        AND status NOT IN ('done', 'aborted')`, [tenantId]);
 ...
 * the cached response.
 */
export async function findIdempotencyRecord(client, tenantId, key) {
    const r = await client.query(`SELECT key, tenant_id, run_id, request_fingerprint,
    ...
    return {
        ...
        run_id: row.run_id ? asRunId(row.run_id) : null,
 ...
 */
export async function insertIdempotencyRecord(client, record) {
    await client.query(`INSERT INTO agent_run_idempotency_keys
