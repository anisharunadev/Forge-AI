 ...
 */
import { GATES, pagesAt50Percent, } from './gates.js';
/** Tick the sweeper once. Idempotent against the same `clock.now()`. */
export async function tickSweeper(deps, args = {}) {
        ...
        if (halfTtlReached && !row.paged_at_50_percent) {
            ...
            if (tier && pagesAt50Percent(tier)) {
                await deps.pager.pageApprover({
                    ...
                    idempotencyKey: `pager-50:${row.id}`,
                });
 ...
 * did not persist.
 */
async function expireRow(deps, row) {
    ...
    await deps.pager.pageApprover({
        ...
        idempotencyKey: `pager-exp:${row.id}`,
    });
}
/**
