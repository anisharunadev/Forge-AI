...
export class PgApprovalsRepo {
    ...
    async insertPending(args) {
        ...
        try {
            ...
            const r = await client.query(`INSERT INTO agent_run_approvals
    ...
    async markStageWaitingApproval(args) {
        ...
        try {
            ...
            const r = await client.query(`UPDATE agent_run_stages
          ...
          WHERE agent_run_stages.run_id = r.id
            ...
            AND agent_run_stages.status IN ('pending','running')`, [args.runId, args.stage]);
            if (r.rowCount === 0) {
    ...
    async findById(args) {
        const r = await this.pool.query(`SELECT ${ROW_COLUMNS}
         FROM agent_run_approvals
        WHERE id = $1
          AND tenant_id = $2
          AND deleted_at IS NULL`, [args.approvalId, args.tenantId]);
        const row = r.rows[0];
    ...
    async findPendingByStage(args) {
        ...
        LIMIT 1`, [args.runId, args.stage, args.tenantId]);
        const row = r.rows[0];
        return row ? rowToApproval(row) : null;
    }
    /**
    ...
    async applyDecision(args) {
        ...
        try {
            await client.query('BEGIN');
            const nextStatus = args.decision === 'accept' ? 'approved' : 'rejected';
            const r = await client.query(`UPDATE agent_run_approvals
            SET status = $4,
                decision = $3,
                decided_by = $5::jsonb,
                decided_at = now(),
          ...
          ${RETURNING_COLUMNS}`, [
            ...
            if (!row) {
                ...
                const existing = await client.query(`SELECT ${ROW_COLUMNS}
             FROM agent_run_approvals
            WHERE id = $1
              AND tenant_id = $2
              AND deleted_at IS NULL`, [args.approvalId, args.tenantId]);
    ...
    async expire(args) {
        ...
        try {
          ...
          WHERE id = $1
            ...
            AND deleted_at IS NULL
            AND status = 'pending'
          ${RETURNING_COLUMNS}`, [args.approvalId, args.tenantId, args.expiredAt.toISOString()]);
            const row = r.rows[0];
            if (!row) {
    ...
    async extend(args) {
        ...
        try {
          ...
          WHERE id = $1
            ...
            AND deleted_at IS NULL
            AND status = 'pending'
          ${RETURNING_COLUMNS}`, [args.approvalId, args.tenantId, args.newExpiresAt.toISOString()]);
            const row = r.rows[0];
            if (!row) {
                await client.query('COMMIT');
                ...
                throw new Error(`extend: approval ${args.approvalId} not pending`);
            }
            await client.query('COMMIT');
            return rowToApproval(row);
        }
    ...
    async setInteractionId(args) {
        ...
        try {
          ...
          WHERE id = $1
            ...
            AND deleted_at IS NULL
            AND status = 'pending'
          ${RETURNING_COLUMNS}`, [args.approvalId, args.tenantId, args.interactionId]);
            const row = r.rows[0];
            if (!row) {
                await client.query('COMMIT');
                throw new Error(`setInteractionId: approval ${args.approvalId} not pending`);
            }
            await client.query('COMMIT');
            return rowToApproval(row);
        }
    ...
    async markPagedAt50Percent(args) {
        ...
        await this.pool.query(`UPDATE agent_run_approvals
          SET paged_at_50_percent = true
        WHERE id = $1
          AND tenant_id = $2
          AND deleted_at IS NULL
          AND status = 'pending'
          AND paged_at_50_percent = false`, [args.approvalId, args.tenantId]);
    }
    /**
    ...
    async listPendingForSweep(args) {
        const r = args.tenantId
            ? await this.pool.query(`SELECT ${ROW_COLUMNS}
             FROM agent_run_approvals
            ...
            LIMIT $3`, [args.tenantId, args.asOf.toISOString(), args.limit])
            : await this.pool.query(`SELECT ${ROW_COLUMNS}
             FROM agent_run_approvals
            WHERE status = 'pending'
              AND deleted_at IS NULL
              AND expires_at <= $1
            ORDER BY expires_at ASC
            LIMIT $2`, [args.asOf.toISOString(), args.limit]);
...
//# sourceMappingURL=approvals-repo-pg.js.map