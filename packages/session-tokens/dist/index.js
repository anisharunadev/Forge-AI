...
export class TokenIssuer {
    ...
    constructor(cfg) {
        this.cfg = cfg;
        if (!cfg.issuer)
            throw new Error('TokenIssuer requires issuer');
        if (!cfg.audience)
            throw new Error('TokenIssuer requires audience');
    }
    async mint(input) {
    ...
    async verifyWithDpop(args) {
        ...
        const proofPub = await importJWK(protectedHeader.jwk, 'ES256');
