 ...
 */
export function compileIgnore(patterns: readonly string[]): CompiledIgnore {
...
function matchesAnySuffix(normalized: string, re: RegExp): boolean {
...
function hasMatchingAncestorDir(normalized: string, candidate: string, anchored: boolean): boolean {
  ...
  for (let i = 1; i <= parts.length; i += 1) {
    ...
    if (ancestor === prefix) return true;
    if (!anchored && ancestor.endsWith('/' + prefix)) return true;
    if (!anchored && ancestor === prefix) return true;
  }
  return false;
}

/** Walk a directory recursively, returning matching files. */
export function walk(opts: ScanOptions = {}): ScannedFile[] {
  ...
  function recurse(dir: string): void {
    ...
    for (const entry of entries) {
      ...
      if (st.isDirectory()) {
        ...
        if (matchesIgnore(dirRel + '/', allPatterns)) continue;
        recurse(abs);
        continue;
      }
      ...
      if (!includeMigrations && /(?:^|\/)migrations\//.test(rel)) continue;
      if (matchesIgnore(rel, allPatterns)) continue;
