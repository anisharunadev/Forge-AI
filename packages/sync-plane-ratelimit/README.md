...
Plus, added in v0.4 (FORA-487.3 / FORA-517):

5. **Backoff scheduler** — retries failed calls with `Retry-After` /
   exponential-with-jitter, idempotency-keyed via the FORA-401 `sync_op`
...
## Backoff scheduler (v0.4 — FORA-487.3)
...
`BackoffScheduler` is the FORA-401 `sync_op` dedupe spine + a retry
...
| `connector.backoff.retried`         | A platform call was retried after backoff (FORA-487.3) | `{ idempotency_key, attempt, next_attempt, backoff_ms, status, … }` |
| `connector.backoff.exhausted`       | The retry budget was exhausted (FORA-487.3) | `{ idempotency_key, attempts, final_status, backoff_ms, … }`         |

The v0.1 `sync.outbound.*` / `sync.platform.*` namespace was renamed to
`connector.*` per **FORA-391 Plan 3 §6** in FORA-487.1.
...
- Pattern reused from `@fora/customer-cloud-broker/src/adapters/aws.ts`
  (FORA-126.5).
- The Sync Plane service (FORA-252 / 11.1) wires the platform adapter at
  construction time. The smoke test injects a mock.
- The backoff scheduler's `SyncOpStore` is the FORA-401 dedupe spine
...
- **Integration** — `src/__tests__/outbound.test.ts` (FORA-256 5 ACs end-to-end),
  `src/__tests__/backoff_scheduler.test.ts` (orchestrator + FIFO + audit).
- **Contract** — covered by `@fora/sync-plane-service` tests (FORA-252).
- **E2E** — covered by the smoke test in `agents/sync_plane_service/__pycache__/smoke_test.cpython-312.pyc` (FORA-252 acceptance gate).

## Versioning

This package follows semver. The v0.1 → v0.2 bump (FORA-487.1) was a
**typed-enum rename** (no runtime behaviour change). The v0.3 → v0.4
bump (FORA-487.3) is **additive** — two new audit event types and a
