 ...
 */
export async function auditBypassRls(projectRoot, allowList) {
    const findings = [];
    ...
    for await (const file of walk(projectRoot)) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (!BYPASSRLS_RE.test(line))
                continue;
            if (isInAllowList)
                continue; // allow-list path; expected
            findings.push({
                file: abs,
 ...
 */
export function isAllowedRole(text) {
...
export async function assertAllowListDirs(allowList) {
...
//# sourceMappingURL=bypass-audit.js.map