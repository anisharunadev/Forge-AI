...
Postgres migration runner that bootstraps every FORA multi-tenant table with the canonical `tenant_isolation` RLS policy, enforces a `BYPASSRLS` audit, and ships the property-based test that proves cross-tenant reads are impossible.

Implements **FORA-162** (0.7.2a) — the database half of [FORA-124 0.7.2 Per-tenant namespace primitive](../workspace/memory/architecture.md#6-data-model-discipline).
...
- Cross-tenant analytics (impossible by construction per FORA-124 out-of-scope).
...
const pool = new Pool({ connectionString: process.env.FORA_DATABASE_URL });
...
```bash
export FORA_DATABASE_URL=postgres://migrator:...@localhost:5432/fora
pnpm test
```

Without `FORA_DATABASE_URL` the e2e portion is skipped and only the pure-string property runs (the build-time gate).
