...
export async function runMigrations(
  ...
  if (opts.projectRoot) {
    ...
    if (findings.length > 0) {
      const lines = findings.map(
        (f) => `  - ${f.relPath}:${f.line}  ${f.text}`,
      );
      throw new Error(
        `BYPASSRLS audit failed. Findings outside the allow-list:\n${lines.join('\n')}`,
      );
    }
  }
  ...
  try {
    ...
    const result: RunResult = { applied: [], verified: [] };
    ...
    for (const model of getRlsModels()) {
      ...
      if (!present) {
        throw new Error(
          `tenant_isolation policy missing on ${model.name} after migration`,
        );
      }
 ...
 */
export async function applyModelDdlForTest(
  client: PoolClient,
...
function TENANTS_BOOTSTRAP_MODEL(): TenantScopedModel {
  const t = FORA_MODELS.find((m) => m.name === TENANTS_MODEL_NAME);
  if (!t) throw new Error('tenants model missing from registry');
  return t;
}

function migrationVersionFor(model: TenantScopedModel): string {
...
async function isApplied(client: PoolClient, version: string): Promise<boolean> {
  const r = await client.query<{ version: string }>(
    `SELECT version FROM ${SCHEMA_MIGRATIONS_TABLE} WHERE version = $1`,
    [version],
  );
...
async function policyExists(client: PoolClient, table: string): Promise<boolean> {
