...
Project context, phase list, dependencies, and recommendations are resolved inside the workflow using `forge-tools query init.manager`. No upfront context loading needed.
</context>

<process>
If `--analyze-deps` is in $ARGUMENTS:
Read and execute `~/.claude/forge-core/workflows/analyze-dependencies.md` end-to-end.
