...
<objective>
Generate and update up to 9 documentation files for the current project. Each doc type is written by a forge-doc-writer subagent that explores the codebase directly — no hallucinated paths, phantom endpoints, or stale signatures.
...
<execution_context>
@~/.claude/forge-core/workflows/docs-update.md
</execution_context>
...
- `--verify-only` — Check existing docs for accuracy against the codebase. No files are written. Reports VERIFY marker count. Full codebase fact-checking requires the forge-doc-verifier agent (Phase 4).
