...
<objective>
Manage the runtime skill surface without reinstall. Reads/writes `~/.claude/.forge-surface.json`
(sibling to `~/.claude/.forge-profile`) and re-stages the active skills directory in place.
Skill dirs live at `~/.claude/skills/forge-*/`.
...
| `reset` | Delete `.forge-surface.json`, return to install-time profile |
| *(none)* | Treat as `list` |
...
`forge-core/bin/lib/surface.cjs`. The registry is loaded via:
```js
...
```
Base profile:   standard  (from .forge-surface.json)
Install profile: standard  (from .forge-profile)
```
...
1. Check if `.forge-surface.json` exists.
2. Delete it.
...
`~/.claude/skills/forge-*/` because the `claude global` layout has `destSubpath =
...
Surface state is stored at `${RUNTIME_CONFIG_DIR}/.forge-surface.json`
(i.e. `~/.claude/.forge-surface.json`).
...
- Missing `surface.cjs` → prompt: "Run `npm i -g forge-core` to reinstall GSD."

<execution_context>
Surface state file: `~/.claude/.forge-surface.json`
Install profile marker: `~/.claude/.forge-profile`
Skill dirs: `~/.claude/skills/forge-*/`
Engine module: `~/.claude/forge-core/bin/lib/surface.cjs`
Cluster definitions: `~/.claude/forge-core/bin/lib/clusters.cjs`
</execution_context>
