...
```bash
# Clone the repo
git clone https://github.com/forge-ai/forge-core.git
cd forge-core
...
**Process:**
1. Open a [Bug Report issue](https://github.com/forge-ai/forge-core/issues/new?template=bug_report.yml) — fill it out completely.
...
**Process:**
1. Open an [Enhancement issue](https://github.com/forge-ai/forge-core/issues/new?template=enhancement.yml) with the full proposal.  The issue template requires: the problem being solved, the concrete benefit, the scope of changes, and alternatives considered.
...
**Process:**
1. **Discuss first** — check [Discussions](https://github.com/forge-ai/forge-core/discussions) to see if the idea has been raised. If it has and was declined, don't open a new issue.
2. Open a [Feature Request issue](https://github.com/forge-ai/forge-core/issues/new?template=feature_request.yml) with the complete spec. The template requires: the solo-developer problem being solved, what is being added, full scope of affected files and systems, user stories, acceptance criteria, and assessment of maintenance burden.
...
# ... commit, push
gh pr create --base next --repo forge-ai/forge-core
```
...
npm run changeset -- --type Fixed --pr <YOUR_PR_NUMBER> \
  --body "**\`/forge-foo\` no longer drops trailing slashes** — explain the user-visible change."
```
...
Fragments are consolidated into `CHANGELOG.md` at release time by the release workflow. See [`.changeset/README.md`](.changeset/README.md) for the format spec and [#2975](https://github.com/forge-ai/forge-core/issues/2975) for the rationale.

**CI enforcement:** the `Changeset Required` workflow (`scripts/changeset/lint.cjs`) fails any PR that touches `bin/`, `forge-core/`, `agents/`, `commands/`, `hooks/`, or `sdk/src/` without a `.changeset/*.md` fragment. The gate also **validates the content** of every changed fragment: a fragment whose frontmatter does not parse (e.g. a `pr: 0` placeholder that was never backfilled to the real PR number) fails the gate with `fail_invalid_fragment`, naming the offending file. This stops a malformed fragment from merging to `next` and only detonating later in the release job's CHANGELOG render.
...
node scripts/release-notes/format-github-release-notes.cjs \
  --tag vX.Y.Z --repo forge-ai/forge-core --apply
```
...
If your PR adds, changes, deprecates, or removes user-visible behavior, you **must** update the relevant documentation in `docs/`. CI will fail any PR whose changeset fragment is typed `Added`, `Changed`, `Deprecated`, or `Removed` without also modifying at least one file under `docs/` ([#3213](https://github.com/forge-ai/forge-core/issues/3213)).
...
| `runGsdTools(args, cwd, env?)` | Executes forge-tools.cjs | Testing CLI commands |
...
Changes to CLI parsing, command dispatch, query dispatch, command routers, `forge-tools`, or `forge-sdk` must include a negative input matrix for the affected command family.
...
const configSrc = fs.readFileSync(
  path.join(GSD_ROOT, 'forge-core', 'bin', 'lib', 'config-schema.cjs'), 'utf-8'
);
assert.ok(
...
const cmdContent = fs.readFileSync(path.join(tmpDir, 'forge-sdk.cmd'), 'utf8');
...
- Setup guide: [gsd-test-runner getting started](https://github.com/forge-ai/forge-test-runner/blob/main/docs/getting-started.md)
...
if git diff --cached --name-only | grep -Eq "^sdk/src/query/command-manifest\.|^sdk/src/query/command-aliases\.generated\.ts$|^forge-core/bin/lib/command-aliases\.generated\.cjs$|^sdk/scripts/gen-command-aliases\.ts$"; then
  npm run check:alias-drift
fi
EOF
...
- **No external dependencies in core** — `forge-tools.cjs` and all lib files use only Node.js built-ins
- **Conventional commits** — `feat:`, `fix:`, `docs:`, `refactor:`, `test:`, `ci:`. The full grammar is `<type>(<scope>): <subject>` (enforced by `hooks/forge-validate-commit.sh`; subject ≤72 chars, lowercase, imperative mood, no trailing period). When the work resolves a tracked issue, put the issue number in the scope: `fix(#1520): randomize mktemp temp paths on BSD/macOS`. The same convention applies to PR titles — release notes are grouped by the title's type prefix (`feat` → Feature, `fix` → Fix, everything else → Enhancement).
...
forge-core/
  ...
  workflows/            — Workflow definitions (.md)
                          ...
                          (agents/forge-*.md). Full how-to + reference in
...
| `.github/agents/forge-*` | Yes (`.gitignore:37`) | Local CI-surface bundle |
