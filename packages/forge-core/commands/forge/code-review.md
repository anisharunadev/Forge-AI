...
- `--fix` (optional) — after review completes (or if REVIEW.md already exists), auto-apply fixes found. Spawns forge-code-fixer agent. Accepts sub-flags:
...
<execution_context>
@~/.claude/forge-core/workflows/code-review.md
</execution_context>
...
Context files (CLAUDE.md, SUMMARY.md, phase state) are resolved inside the workflow via `forge-tools query init.phase-op` and delegated to agent via `<files_to_read>` blocks.
</context>
...
- Agent spawning (forge-code-reviewer)
