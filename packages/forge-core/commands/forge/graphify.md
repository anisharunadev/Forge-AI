...
**CJS-only (graphify):** `graphify` subcommands are not registered on `forge-tools query`. Use the `forge_run` launcher shim (defined in each bash block below) or invoke the binary directly: `node <runtime-home>/forge-core/bin/forge-tools.cjs graphify …` where `<runtime-home>` is your runtime's config directory (e.g. `~/.claude`, `~/.hermes`, `~/.cursor`). See `docs/CLI-TOOLS.md` for details. Other tooling may still use `forge-tools query` where a handler exists.
...
**DO NOT use the forge-tools config get-value command** -- it hard-exits on missing keys.
...
Knowledge graph is disabled. To activate:
  ...
  node <runtime-home>/forge-core/bin/forge-tools.cjs config-set graphify.enabled true

Then run /forge:graphify build to create the initial graph.
```
...
forge_run graphify query <term>
```

Parse the JSON output and display results:
- If the output contains `"disabled": true`, display the disabled message from Step 1 and **STOP**
- If the output contains `"error"` field, display the error message and **STOP**
- If no nodes found, display: `No graph matches for '<term>'. Try /forge:graphify build to create or rebuild the graph.`
...
graphify update . \
  ...
  && forge_run graphify build snapshot \
  && forge_run graphify status
```
...
**MVP-mode rendering.** When a phase has `**Mode:** mvp` in ROADMAP.md (resolved via `forge-tools query roadmap.get-phase --pick mode`), render its graph node with two distinct visual signals:
...
5. DO NOT use `forge-tools config get-value` for the config gate -- it exits on missing keys.
