...
- @~/.claude/forge-core/workflows/complete-milestone.md (main workflow)
- @~/.claude/forge-core/templates/milestone-archive.md (archive template)
  </execution_context>
...
0. **Check for audit:**
   ...
   - If audit status is `gaps_found`: recommend closing the gaps inline
     ...
     via `/forge:phase --insert <N>` plus the standard
   ...
   a closure phase per gap with `/forge:phase --insert <N>` and run the
   standard `/forge:discuss-phase` → `/forge:plan-phase` → `/forge:execute-phase`
