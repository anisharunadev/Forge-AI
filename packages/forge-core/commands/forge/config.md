...
| --profile &lt;name&gt; | Switch model profile without interactive prompt | forge-tools query config-set-model-profile |

</routing>

<execution_context>
@~/.claude/forge-core/workflows/settings.md
@~/.claude/forge-core/workflows/settings-advanced.md
@~/.claude/forge-core/workflows/settings-integrations.md
</execution_context>
...
- If it starts with `--profile`: extract the profile name (remainder after `--profile`), then:
  1. Verify `forge-tools` is on PATH via `command -v forge-tools`; if absent, emit the install hint `Install GSD via 'npm i -g @forge-ai/forge-core'` and stop.
  2. Run: `forge-tools query config-set-model-profile <profile-name> --raw` and display the output verbatim.
