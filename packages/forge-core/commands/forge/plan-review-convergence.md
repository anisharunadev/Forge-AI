---
name: forge:plan-review-convergence
...
<objective>
Cross-AI plan convergence loop — an outer revision gate around forge-review and forge-planner.
...
**Flow:** Skill("forge-plan-phase") → Agent→Skill("forge-review") → check unresolved HIGH + actionable non-HIGH → Skill("forge-plan-phase --reviews") → Agent→Skill("forge-review") → ... → Converge or escalate

Replaces forge-plan-phase's internal forge-plan-checker with external AI reviewers (codex, gemini, etc.). Plan-phase runs **inline** (bare Skill at depth 0) so it can spawn forge-planner/forge-plan-checker at depth 1. Review runs inside an isolated Agent (forge-review is a Bash leaf — no sub-agents needed). Orchestrator only does loop control.

**Orchestrator role:** Parse arguments, validate phase, run plan-phase inline (Skill at depth 0), spawn an Agent for forge-review, check unresolved HIGH and actionable non-HIGH counts, stall detection, escalation gate.
</objective>

<execution_context>
@$HOME/.claude/forge-core/workflows/plan-review-convergence.md
