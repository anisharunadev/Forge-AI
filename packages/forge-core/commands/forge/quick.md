...
<execution_context>
@~/.claude/forge-core/workflows/quick.md
</execution_context>
...
- Check if SUMMARY.md exists; if so, read `status` from its frontmatter via:
  ```bash
  forge-tools query frontmatter.get .planning/quick/{dir}/SUMMARY.md status
  ```
...
─────────────────────────────────────
Resume with: /forge:quick resume {slug}
```
...
- Status fields read via `forge-tools query frontmatter.get` — never eval'd or shell-expanded
</security_notes>
