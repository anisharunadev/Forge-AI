...
/**
 * Bug #2969: /forge-reapply-patches Step 5 hunk verification gate reports
...
// Script lives at forge-core/bin/ so the installer ships it under
// `${GSD_HOME}/forge-core/bin/` (issue #2994). The top-level scripts/
...
before(() => {
  tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-2969-'));
...
describe('Bug #2969: deterministic Step 5 verification gate', () => {
  ...
  test('treats forge-hook-version install-time substitution as upstream-owned, not missing user content (#229)', () => {
    resetFixture();
    const rel = path.join('hooks', 'forge-statusline.js');
    const pristine = [
    ...
    const backup = [
      '// forge-hook-version: 1.41.0',
      'console.log("statusline hook");',
      '',
    ].join('\n');
    const installed = [
      '// forge-hook-version: 1.42.3',
      'console.log("statusline hook");',
      '',
    ].join('\n');
