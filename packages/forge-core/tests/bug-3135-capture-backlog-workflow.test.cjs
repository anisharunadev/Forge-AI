...
//
// PR #2824 consolidated add-backlog into `forge-capture --backlog` by creating
// a routing wrapper in commands/forge/capture.md that delegates to
...
//
// Fix: create forge-core/workflows/add-backlog.md with the full process
// ported from the deleted commands/forge/add-backlog.md (git ref 87917131^).
...
const WORKFLOW = path.join(ROOT, 'forge-core', 'workflows', 'add-backlog.md');
const COMMANDS_DIR = path.join(ROOT, 'commands', 'forge');

// ─── #3135: add-backlog workflow ─────────────────────────────────────────────

describe('#3135: forge-core/workflows/add-backlog.md', () => {
  test('file exists', () => {
    assert.ok(
      fs.existsSync(WORKFLOW),
      'forge-core/workflows/add-backlog.md does not exist — capture --backlog has no implementation to load',
    );
  });

  test('uses forge-sdk query phase.next-decimal to find next 999.x slot', () => {
    const src = fs.readFileSync(WORKFLOW, 'utf8');
    assert.ok(
      src.includes('phase.next-decimal'),
      'add-backlog.md must use forge-sdk query phase.next-decimal to find the next 999.x number',
    );
  });
  ...
  test('uses generate-slug for the directory name', () => {
    ...
    assert.ok(
      ...
      'add-backlog.md must use forge-sdk query generate-slug to build the phase directory slug',
    );
  });

  test('commits via forge-sdk query commit', () => {
    const src = fs.readFileSync(WORKFLOW, 'utf8');
    assert.ok(
      src.includes('forge-sdk query commit') || src.includes('query commit'),
      'add-backlog.md must commit via forge-sdk query commit',
    );
  });
  ...
  test('documents /forge-review-backlog for promotion', () => {
    const src = fs.readFileSync(WORKFLOW, 'utf8');
    assert.ok(
      src.includes('review-backlog') || src.includes('forge-review-backlog'),
      'add-backlog.md should mention /forge-review-backlog for promoting items to active milestone',
    );
  });
});
