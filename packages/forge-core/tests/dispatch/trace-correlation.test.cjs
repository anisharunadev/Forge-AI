...
const { createHub } = require('../../forge-core/bin/lib/command-routing-hub.cjs');
const { createDefaultLogger } = require('../../forge-core/bin/lib/observability/logger.cjs');
...
describe('trace correlation — end-to-end parentTraceId propagation', () => {
  before(() => {
    ...
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-trace-correlation-test-'));
  ...
  test('invalid parentTraceId in a child dispatch breaks the correlation tree for that child but does not poison sibling traces', () => {
    ...
    const isolatedTmp = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-poison-test-'));
