...
const JS_HOOKS = [
  { name: 'forge-check-update.js',      registrationAnchor: 'hasGsdUpdateHook' },
  { name: 'forge-context-monitor.js',   registrationAnchor: 'hasContextMonitorHook' },
  { name: 'forge-prompt-guard.js',      registrationAnchor: 'hasPromptGuardHook' },
  { name: 'forge-read-guard.js',        registrationAnchor: 'hasReadGuardHook' },
  { name: 'forge-workflow-guard.js',    registrationAnchor: 'hasWorkflowGuardHook' },
  { name: 'forge-worktree-path-guard.js', registrationAnchor: 'hasWorktreePathGuardHook' },
];
...
describe('bug #1754: .js hook registration guards', () => {
  ...
  test('all .js hooks use the same guard pattern as .sh hooks', () => {
    ...
    // Count unique hook file existence checks (pattern: path.join(targetDir, 'hooks', 'forge-*.js'))
    const jsGuards = (registrationSection.match(/forge-[\w-]+\.js.*not found at target/g) || []);
    const shGuards = (registrationSection.match(/forge-[\w-]+\.sh.*not found at target/g) || []);
