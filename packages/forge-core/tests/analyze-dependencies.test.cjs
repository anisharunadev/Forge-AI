...
describe('analyze-dependencies command', () => {
  test('analyze-dependencies command file was consolidated away (deleted in #2790)', () => {
    // The standalone /forge-analyze-dependencies command was removed as a dead skill in #2790.
    // The underlying workflow (workflows/analyze-dependencies.md) remains functional.
    const deleted = path.join(__dirname, '..', 'commands', 'forge', 'analyze-dependencies.md');
  ...
  test('workflow file is sufficient without a standalone command file', () => {
    const p = path.join(__dirname, '..', 'forge-core', 'workflows', 'analyze-dependencies.md');
  ...
  test('docs/COMMANDS.md does not document the consolidated-away /forge-analyze-dependencies entry', () => {
    ...
    // from consolidated entry points (see forge-phase / forge-progress workflow chains).
    ...
    assert.ok(!/^### `\/forge-analyze-dependencies`/m.test(content),
      'COMMANDS.md should not document the removed /forge-analyze-dependencies command');
  });
});
