...
describe('validate health command', () => {
  ...
  // Regression: #3652 — after /forge:complete-milestone, phase dirs move into
...
describe('validate health --repair command', () => {
  ...
  test('creates config.json with defaults when missing', () => {
    ...
    assert.strictEqual(diskConfig.phase_branch_template, 'forge/phase-{phase}-{slug}');
    assert.strictEqual(diskConfig.milestone_branch_template, 'forge/{milestone}-{slug}');
  });
...
// forge-core/bin/lib/verify.cjs. These tests fail on old verify.cjs and
