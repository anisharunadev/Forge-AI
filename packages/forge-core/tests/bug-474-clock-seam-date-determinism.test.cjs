...
describe('bug-474: realClock GSD_NOW_MS invalid-input hardening', () => {
  const realClock = require('../forge-core/bin/lib/clock.cjs').realClock;
...
describe('bug-474: installer-migrations lock-loop timeout is deterministic via clock seam', () => {
  ...
  test('acquireInstallMigrationLock timeout path fires deterministically via makeFakeClock', (t) => {
    // AAA — Arrange
    const os = require('os');
    const { acquireInstallMigrationLock } = require('../forge-core/bin/lib/installer-migrations.cjs');
    const configDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-474-lock-'));

    t.after(() => {
      cleanup(configDir);
    });

    const LOCK_NAME = 'forge-install-migration.lock';
