 ...
 *
 * Seam: forge-core/workflows/{execute-phase,execute-plan,quick}.md,
 *       agents/forge-executor.md, references/git-integration.md
 *
...
const EXECUTE_PHASE_PATH = path.join(REPO_ROOT, 'forge-core', 'workflows', 'execute-phase.md');
const EXECUTE_PLAN_PATH = path.join(REPO_ROOT, 'forge-core', 'workflows', 'execute-plan.md');
const QUICK_PATH = path.join(REPO_ROOT, 'forge-core', 'workflows', 'quick.md');
const EXECUTOR_AGENT_PATH = path.join(REPO_ROOT, 'agents', 'forge-executor.md');
const GIT_INTEGRATION_PATH = path.join(REPO_ROOT, 'forge-core', 'references', 'git-integration.md');
const WORKTREE_BRANCH_CHECK_FRAGMENT = path.join(REPO_ROOT, 'forge-core', 'references', 'worktree-branch-check.md');
...
describe('bug #2924: worktree HEAD attachment + destructive recovery', () => {
  ...
  describe('forge-executor.md prohibits update-ref self-recovery', () => {
    ...
    test('destructive_git_prohibition block exists', () => {
      assert.ok(block, 'forge-executor.md must contain a <destructive_git_prohibition> block');
    });
  ...
  describe('forge-executor.md task_commit_protocol enforces worktree-agent-* allow-list', () => {
    const content = fs.readFileSync(EXECUTOR_AGENT_PATH, 'utf-8');
    const block = extractNamedBlock(content, 'task_commit_protocol');

    test('task_commit_protocol block exists', () => {
      assert.ok(block, 'forge-executor.md must contain a <task_commit_protocol> block');
    });
  ...
  describe('no workflow file performs unconditional update-ref on a protected branch', () => {
    const workflowsDir = path.join(REPO_ROOT, 'forge-core', 'workflows');
...
describe('worktree cleanup after executor completes (#1496)', () => {
  const executePhasePath = path.join(__dirname, '..', 'forge-core', 'workflows', 'execute-phase.md');
  const quickPath = path.join(__dirname, '..', 'forge-core', 'workflows', 'quick.md');
  ...
  test('quick.md includes worktree cleanup after executor returns', () => {
    ...
    assert.ok(
      ...
      'quick cleanup must delegate to forge_run query worktree.cleanup-wave (#3797)',
    );
  });
  ...
  test('quick.md cleanup-wave uses || exit 1 to enforce safety semantics', () => {
    ...
    assert.match(
      content,
      /forge_run query worktree\.cleanup-wave.*\|\| exit 1/,
...
describe('worktree merge: orchestrator file protection (#1756)', () => {
  ...
  test('execute-phase.md delegates wave cleanup to SDK with fail-closed || exit 1', () => {
    ...
    assert.match(
      ...
      'execute-phase.md must delegate to forge_run query worktree.cleanup-wave with || exit 1 (#3797)',
    );
  });
  ...
  test('quick.md delegates wave cleanup to SDK with fail-closed || exit 1', () => {
    ...
    assert.match(
      ...
      'quick.md must delegate to forge_run query worktree.cleanup-wave with || exit 1 (#3797)',
    );
  });
});
...
describe('worktree commit safety hardening (#1977)', () => {
  ...
  test('forge-executor.md task_commit_protocol includes post-commit deletion verification', () => {
    const content = fs.readFileSync(EXECUTOR_AGENT_PATH, 'utf-8');
...
describe('bug #3384: worktree cleanup workflow contracts', () => {
    ...
    assert.match(content, /record the `\{agent_id, worktree_path, branch, expected_base\}` entry with `forge_run query worktree\.record-agent/);
  ...
  test('#1297 forge-executor self-reports authoritative worktree metadata', () => {
  ...
  test('quick contract requires a cleanup manifest instead of global worktree discovery', () => {
    ...
    assert.match(content, /worktree\.cleanup-wave/);
    assert.match(content, /mktemp "\$\{TMPDIR:-\/tmp\}\/forge-quick-worktree-/);
    ...
    // QUICK_WORKTREE_MANIFEST node-e code is removed — the forge_run call with || exit 1 is the
    // only cleanup path, enforcing safety-refusal semantics (#3174/#3384).
    assert.match(content, /forge_run query worktree\.cleanup-wave --manifest "\$QUICK_WORKTREE_MANIFEST" \|\| exit 1/);
...
test('#3425: helper cleanup path pins orchestrator CWD to primary worktree and checks EXPECTED_BRANCH', () => {
  ...
  // After #3797 architectural fix, callsites use forge_run
  assert.match(content, /forge_run query worktree\.cleanup-wave --manifest "\$WAVE_WORKTREE_MANIFEST"/);
});
