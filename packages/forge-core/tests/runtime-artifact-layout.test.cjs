...
const installProfiles = require('../forge-core/bin/lib/install-profiles.cjs');
...
describe('resolveRuntimeArtifactLayout — claude local', () => {
  test('returns correct layout for claude scope=local', () => {
    ...
    assert.strictEqual(layout.kinds[0].destSubpath, 'commands'); // #1367: flat forge-<cmd>.md layout
    assert.strictEqual(layout.kinds[0].prefix, 'forge-');
    ...
    assert.strictEqual(layout.kinds[1].prefix, 'forge-');
...
describe('resolveRuntimeArtifactLayout — hermes', () => {
  test('returns correct layout for hermes', () => {
    ...
    assert.strictEqual(layout.kinds[0].prefix, 'forge-'); // #947: restored canonical prefix
    assert.strictEqual(typeof layout.kinds[0].stage, 'function');
  });
});
...
describe('resolveRuntimeArtifactLayout — opencode', () => {
  test('returns commands + skills layout for opencode (#784)', () => {
    ...
    assert.strictEqual(commands.prefix, 'forge-');
    ...
    assert.strictEqual(skills.prefix, 'forge-');
...
describe('resolveRuntimeArtifactLayout edge-cases', () => {
  test('hermes has destSubpath skills/forge and forge- prefix (#947: restored from bare-stem)', () => {
    const layout = resolveRuntimeArtifactLayout('hermes', '/tmp/x');
    assert.strictEqual(layout.kinds[0].destSubpath, 'skills/gsd');
    assert.strictEqual(layout.kinds[0].prefix, 'forge-'); // #947: bare-stem prefix='' reversed
  });
...
const CORE_AGENTS = new Set(['forge-planner']);
...
describe('stage — skills kind (claude global)', () => {
  test('stage returns a directory containing forge-<stem>/SKILL.md entries', () => {
    ...
    for (const entry of entries) {
      assert.ok(entry.startsWith('forge-'), `entry should start with forge-: ${entry}`);
  ...
  test('stage with skills="*" produces flat layout for claude (#924: reverted from nested)', () => {
    ...
    // #924: Claude is reverted to FLAT. Full profile produces >= 60 top-level forge-* dirs.
    // (Previously nested: exactly 6 forge-ns-* router dirs. That broke Skill-tool discovery.)
...
describe('stage — skills kind (kimi global)', () => {
  test('stage returns Kimi SKILL.md dirs and agent YAML/prompt artifacts', () => {
    ...
    const stagedDir = skillsKind.stage(PROFILE_CORE);
    assert.ok(fs.existsSync(stagedDir), 'stagedDir must exist');
    const skillMd = path.join(stagedDir, 'forge-new-project', 'SKILL.md');
    assert.ok(fs.existsSync(skillMd), 'forge-new-project/SKILL.md must exist');
    ...
    const executorYamlPath = path.join(stagedAgentsDir, 'subagents', 'forge-executor.yaml');
    const executorPromptPath = path.join(stagedAgentsDir, 'subagents', 'forge-executor.md');
    ...
    assert.ok(fs.existsSync(executorYamlPath), 'agents/subagents/forge-executor.yaml must be staged');
    assert.ok(fs.existsSync(executorPromptPath), 'agents/subagents/forge-executor.md must be staged');
    ...
    assert.match(executorYaml, /system_prompt_path: \.\/forge-executor\.md/);
  ...
  test('tracks Kimi agent staging dir before writing artifacts', () => {
    ...
    try {
      fs.writeFileSync = function writeFileSyncWithInjectedFailure(file, ...args) {
        ...
        if (filePath.includes('forge-kimi-agents-') && path.basename(filePath) === 'gsd.yaml') {
      ...
      added = [...installProfiles.STAGED_DIRS]
        .filter(dir => !before.has(dir) && path.basename(dir).startsWith('forge-kimi-agents-'));
...
describe('stage — opencode/kilo skills kind (#784)', () => {
  for (const runtime of ['opencode', 'kilo']) {
    test(`${runtime} skills stage writes forge-<stem>/SKILL.md with name + description`, () => {
      ...
      for (const entry of entries) {
        ...
        assert.ok(!/\/forge:/.test(content), 'body must not contain /forge: colon refs');
      }
    });
  }
});
