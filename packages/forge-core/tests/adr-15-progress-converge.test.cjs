...
const COMMAND_PATH = path.join(REPO_ROOT, 'commands', 'forge', 'progress.md');
const WORKFLOW_PATH = path.join(REPO_ROOT, 'forge-core', 'workflows', 'next.md');
const FULL_MD_PATH = path.join(REPO_ROOT, 'forge-core', 'workflows', 'help', 'modes', 'full.md');
...
describe('ADR-15: /forge:progress --next --auto --converge (#1190)', () => {
  ...
  test('next workflow routes Route 3 through plan-review-convergence when PLAN_STRATEGY=converge', () => {
    ...
    assert.match(
      ...
      'next workflow should reference /forge:plan-review-convergence for the converge route',
    );
    assert.match(
      workflow,
  ...
  test('next workflow preserves --auto re-invocation chaining', () => {
    ...
    assert.match(
      workflow,
      /\/forge:progress --next --auto/,
      'workflow should re-invoke /forge:progress --next --auto for chaining',
    );
  ...
  test('full.md documents --converge, --auto, and --cross-ai for /forge:progress', () => {
    const fullMd = read(FULL_MD_PATH);

    assert.match(
      fullMd,
      /\/forge:progress --next --auto --converge/,
      'full.md should show /forge:progress --next --auto --converge usage',
    );
    assert.match(
      fullMd,
      /--auto/,
  ...
  test('COMMANDS.md documents progress convergence flags and usage', () => {
    ...
    assert.match(
      commandsDoc,
      /\/forge-progress --next --auto --converge/,
      'COMMANDS.md should show /forge-progress --next --auto --converge usage example',
    );
  ...
  test('how-to shows /forge-progress --next --auto --converge usage', () => {
    const howTo = read(HOW_TO_PATH);

    assert.match(
      howTo,
      /\/forge-progress --next --auto --converge/,
      'how-to should show /forge-progress --next --auto --converge usage',
    );
  });
});
