...
// quick.md is the shipped orchestration contract for /forge-quick; this
...
const QUICK_MD = path.join(__dirname, '..', 'forge-core', 'workflows', 'quick.md');
...
describe('bug #3521 — quick.md post-merge cleanup CWD safety (via SDK delegation, #3797)', () => {
  ...
  test('quick.md cleanup delegates CWD-safe worktree cleanup to SDK (worktree.cleanup-wave)', () => {
    const content = readQuickMd();
    // After #3797: quick.md delegates to forge_run query worktree.cleanup-wave
    ...
    assert.ok(
      ...
      'quick.md must delegate cleanup to forge_run query worktree.cleanup-wave (#3797)',
    );
  });
  ...
  test('quick.md cleanup-wave call uses || exit 1 to enforce fail-closed safety (#3521 contract)', () => {
    ...
    // This is the equivalent of the pre-#3797 `forge_run query ... || exit 1` in the
    // `if command -v forge-sdk` branch.
    assert.match(
      content,
