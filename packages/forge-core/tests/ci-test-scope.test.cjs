...
describe('ci-test-scope.cjs', () => {
  ...
  test('product code (forge-core/bin/lib/foo.cjs) — product_changed true', () => {
    const result = scopeFor(['forge-core/bin/lib/foo.cjs']);
    assert.strictEqual(result.code_changed, true);
    assert.strictEqual(result.product_changed, true,
      `expected product_changed=true for forge-core/ change, got: ${JSON.stringify(result)}`);
  });
  ...
  test('mixed docs + code — escalates to product_changed true', () => {
    // Use bin/forge (installer rule, fullMatrix:true) to get a code file that reliably triggers full matrix.
  ...
  test('command changes request command tests without full parity matrix', () => {
    const result = scopeFor(['commands/forge/plan-phase.md']);
    assert.strictEqual(result.code_changed, true);
    assert.strictEqual(result.full_matrix, false);
  ...
  test('bug-408: code change with no rule match falls back to unit suite token', () => {
    // A plain source file that matches no RULES entry but is under forge-core/ (code path)
    const result = scopeFor(['forge-core/src/some-util.js']);
    assert.strictEqual(result.code_changed, true);
