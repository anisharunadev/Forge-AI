...
/**
 * Tests for forge-core/bin/lib/graphify.cjs
 *
...
// containing a full-profile .forge-surface.json with graphify surfaced, and
// clear GSD_RUNTIME / GSD_WORKSTREAM / GSD_PROJECT for hermeticity.
// An EMPTY tmp config dir (no .forge-surface.json) also works — the resolver
...
function makeSurfacedConfigDir() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-graphify-surface-cfg-'));
  fs.writeFileSync(
...
describe('status', () => {
  ...
  describe('tri-state graphify gate (isCapabilityActive cutover)', () => {
    ...
    //   but graphify NOT surfaced (disabled cluster in .forge-surface.json).
    //
    ...
    //     .forge-surface.json  → full profile, disabledClusters:["graphify"]
    //     (no .forge-profile   → defaults to 'full' → installedSkills='*' → installed=true)
    //   tmpProjectDir/
    ...
    describe('hermetic: graphify installed + config-enabled but NOT surfaced → disabled', () => {
      ...
      beforeEach(() => {
        tmpConfigDir = createTempDir('forge-graphify-surface-test-config-');
        tmpProjectDir = createTempDir('forge-graphify-surface-test-project-');

        // Fixture: .forge-surface.json — full profile with graphify cluster disabled.
        ...
        // No .forge-profile written → readActiveProfile returns null → defaults to 'full'
        ...
        fs.writeFileSync(
          path.join(tmpConfigDir, '.forge-surface.json'),
      ...
      test('graphifyStatus is NOT disabled when installed + SURFACED + config-enabled=true (positive control)', () => {
        // Re-write .forge-surface.json with graphify surfaced (disabledClusters empty).
  ...
  describe('disabledResponse', () => {
    test('returns disabled:true with enable instructions', () => {
      ...
      assert.strictEqual(result.disabled, true);
      assert.ok(result.message.includes('forge-tools config-set graphify.enabled true'));
    });
  });
