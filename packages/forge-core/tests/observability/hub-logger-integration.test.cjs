...
const {
  createHub,
} = require('../../forge-core/bin/lib/command-routing-hub.cjs');

const {
  createDefaultLogger,
} = require('../../forge-core/bin/lib/observability/logger.cjs');
...
function makeTmpDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'forge-hub-logger-test-'));
}
