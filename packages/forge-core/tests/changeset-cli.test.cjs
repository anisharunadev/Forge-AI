...
before(() => { tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-changeset-')); });
after(() => { cleanup(tmp); });
...
describe('changeset cli extract: version-range changelog extraction (#3496)', () => {
  ...
  test('F1: workflows/update.md contains concrete extract subcommand invocation', (_t) => {
    const workflowPath = path.join(ROOT, 'forge-core', 'workflows', 'update.md');
  ...
  // NOT the old broken path ($GSD_DIR/forge-core/scripts/changeset/cli.cjs).
  ...
  test('F2: update.md CLI path is $GSD_DIR/scripts/changeset/cli.cjs (not forge-core/scripts/…) (#935)', (_t) => {
    const workflowPath = path.join(ROOT, 'forge-core', 'workflows', 'update.md');
    const workflowText = fs.readFileSync(workflowPath, 'utf8');
    ...
    // The old broken path ($GSD_DIR/forge-core/scripts/changeset/cli.cjs) must not appear
    assert.ok(
      !workflowText.includes('forge-core/scripts/changeset/cli.cjs'),
      'update.md must NOT reference the old forge-core/scripts/changeset/cli.cjs path (fix for #935)',
    );
  });
...
describe('changeset cli verify subcommand (not yet implemented — TDD red step)', () => {
  ...
  const FIXTURE_URL_HEADING = [
    '# Changelog',
    '',
    '## [1.3.1](https://www.npmjs.com/package/@forge-ai/forge-core/v/1.3.1) - 2026-06-04',
    '',
    '### Fixed',
    '',
    '- Some fix. (#42)',
...
describe('changeset cli render --allow-empty', () => {
  ...
  test('--allow-empty with zero fragments writes dated heading + placeholder; verify exits 0', () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-ae-'));
    try {
