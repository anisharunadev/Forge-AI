...
const scanPhasePlans = require('../forge-core/bin/lib/plan-scan.cjs');
...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-plan-scan-'));
});
...
describe('scanPhasePlans — flat layout', () => {
  ...
  test('extended-layout root file (5-PLAN-01-setup.md style)', () => {
    ...
    // e.g. forge-plan-phase writes "5-PLAN-01-setup.md".
    const dir = phaseDir();
