...
function runGsd(args, cwd) {
  return spawnSync(process.execPath, [path.join(ROOT, 'forge-core/bin/forge-tools.cjs'), ...args], {
    cwd,
    encoding: 'utf8',
  });
}
...
describe('bug #3212 execute-phase stall detection and safe resume', () => {
  test('config schemas register executor stall detector keys', () => {
    ...
    const { VALID_CONFIG_KEYS: cjsKeys } = require('../forge-core/bin/lib/config-schema.cjs');
    const manifest = JSON.parse(read('forge-core/bin/shared/config-schema.manifest.json'));
  ...
  test('config-get returns schema defaults for executor stall detector keys', (t) => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-3212-'));
    t.after(() => cleanup(tmp));
  ...
  test('execute-phase verifies partial-plan drift before dispatch', () => {
    const workflow = read('forge-core/workflows/execute-phase.md');
  ...
  test('execute-plan documents atomic close-out invariant', () => {
    const workflow = read('forge-core/workflows/execute-plan.md');
  ...
  test('forensics includes the partial-plan drift detector', () => {
    const workflow = read('forge-core/workflows/forensics.md');
