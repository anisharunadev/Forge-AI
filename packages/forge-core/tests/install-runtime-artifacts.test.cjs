...
} = require('../forge-core/bin/lib/runtime-artifact-layout.cjs');
...
function loadFreshInstallerWithPlanStubs({ installStub, uninstallStub }) {
  ...
  const planPath = require.resolve('../forge-core/bin/lib/runtime-artifact-install-plan.cjs');
...
describe('installRuntimeArtifacts — consumes Runtime Artifact Install Plan Module', () => {
  test('executes returned copy items and cleanup obligations', (t) => {
    const configDir = createTempDir('forge-install-plan-adapter-');
    const sourceDir = createTempDir('forge-install-plan-source-');
    const cleanupDir = createTempDir('forge-install-plan-cleanup-');
    t.after(() => {
      cleanup(configDir);
      cleanup(sourceDir);
      cleanup(cleanupDir);
    ...
    const { installer, restore } = loadFreshInstallerWithInstallPlanStub((args) => {
      ...
      return {
        ...
        plan: {
          ...
          items: [
            { kind: 'commands', sourceDir, destDir: path.join(configDir, 'commands', 'forge') },
          ],
        },
      };
    });
    ...
    assert.ok(fs.existsSync(path.join(configDir, 'commands', 'forge', 'proof.md')));
  ...
  test('cleans returned obligations when planning fails', (t) => {
    const configDir = createTempDir('forge-install-plan-fail-');
    const cleanupDir = createTempDir('forge-install-plan-fail-cleanup-');
    t.after(() => {
      cleanup(configDir);
      cleanup(cleanupDir);
    });
...
describe('installRuntimeArtifacts — skills runtimes write forge-prefixed skill dirs', () => {
  for (const runtime of SKILLS_RUNTIMES_LAYOUT) {
    test(`${runtime}: forge-prefixed skill dirs in skills/`, (t) => {
      const configDir = createTempDir(`forge-ial-${runtime}-`);
      ...
      if (runtime === 'kimi') {
        const newProjectSkill = path.join(destDir, 'forge-new-project', 'SKILL.md');
        assert.ok(fs.existsSync(newProjectSkill), 'kimi: forge-new-project/SKILL.md must exist');
        const content = fs.readFileSync(newProjectSkill, 'utf8');
        assert.match(content, /^name: forge-new-project$/m);
        ...
        const executorYaml = path.join(agentsDir, 'subagents', 'forge-executor.yaml');
        const executorPrompt = path.join(agentsDir, 'subagents', 'forge-executor.md');
        ...
        assert.ok(fs.existsSync(executorYaml), 'kimi: agents/subagents/forge-executor.yaml must exist');
        assert.ok(fs.existsSync(executorPrompt), 'kimi: agents/subagents/forge-executor.md must exist');
        ...
        assert.match(executorYamlContent, /system_prompt_path: \.\/forge-executor\.md/);
        assert.match(executorYamlContent, /kimi_cli\.tools\./);
        assert.doesNotMatch(executorYamlContent, /mcp__/);
      }

      if (RESOLVED_CORE.skills !== '*') {
        const prefixedCount = countPrefixedEntries(destDir, skillsKind.prefix || 'forge-');
...
describe('installRuntimeArtifacts — hermes nested layout', () => {
  test('hermes: skills/forge/forge-<stem>/SKILL.md with forge- prefix in name (#947)', (t) => {
    const configDir = createTempDir('forge-ial-hermes-');
    t.after(() => cleanup(configDir));

    installRuntimeArtifacts('hermes', configDir, 'global', RESOLVED_CORE);

    const nestedDir = path.join(configDir, 'skills', 'forge');
    assert.ok(fs.existsSync(nestedDir));
    // #947: Hermes now uses canonical forge- prefix — skills/forge/forge-<stem>/SKILL.md
    assert.ok(fs.existsSync(path.join(nestedDir, 'forge-help', 'SKILL.md')),
      'skills/forge/forge-help/SKILL.md must exist (canonical forge- prefix, #947)');
    assert.ok(!fs.existsSync(path.join(nestedDir, 'help')),
      'bare-stem skills/forge/help/ must NOT exist (#947 fix)');
  });
});

describe('installRuntimeArtifacts — gemini commands layout', () => {
  test('gemini: commands/forge/ created, no skills/', (t) => {
    const configDir = createTempDir('forge-ial-gemini-');
    t.after(() => cleanup(configDir));

    installRuntimeArtifacts('gemini', configDir, 'global', RESOLVED_CORE);

    assert.ok(fs.existsSync(path.join(configDir, 'commands', 'forge')));
    assert.ok(fs.existsSync(path.join(configDir, 'commands', 'forge', 'help.md')));
    assert.ok(!fs.existsSync(path.join(configDir, 'skills')));
  });
});

describe('installRuntimeArtifacts — cursor commands layout (#785)', () => {
  test('cursor: skills/ AND commands/ both created; commands/forge-help.md is plain markdown', (t) => {
    const configDir = createTempDir('forge-ial-cursor-cmds-');
    t.after(() => cleanup(configDir));
    ...
    assert.ok(fs.existsSync(path.join(skillsDir, 'forge-help', 'SKILL.md')),
      'skills/forge-help/SKILL.md must exist');

    // New commands kind (#785)
    const commandsDir = path.join(configDir, 'commands');
    assert.ok(fs.existsSync(commandsDir), 'commands/ must exist (#785)');
    assert.ok(fs.existsSync(path.join(commandsDir, 'forge-help.md')),
      'commands/forge-help.md must exist (#785)');

    // Cursor commands are plain markdown — no YAML frontmatter
    const helpContent = fs.readFileSync(path.join(commandsDir, 'forge-help.md'), 'utf8');
...
describe('installRuntimeArtifacts — windsurf workflows layout (#1615)', () => {
  test('windsurf: local install writes workflow slash-command files, not skills', (t) => {
    const configDir = createTempDir('forge-ial-windsurf-');
    t.after(() => cleanup(configDir));
    ...
    assert.ok(fs.existsSync(path.join(workflowsDir, 'forge-help.md')),
      'workflows/forge-help.md must exist for /forge-help');
    assert.ok(!fs.existsSync(path.join(configDir, 'skills')),
      'Windsurf must not install dead skills/ artifacts for slash commands');

    const helpContent = fs.readFileSync(path.join(workflowsDir, 'forge-help.md'), 'utf8');
    assert.ok(!helpContent.startsWith('---'), 'Windsurf workflows must be plain markdown, not SKILL.md frontmatter');
    assert.match(helpContent, /# forge-help/, 'workflow should identify the slash command it backs');
    assert.ok(helpContent.includes(`${configDir}/forge-core/commands/forge/help.md`.replace(/\\/g, '/')),
  ...
  test('windsurf: global install is explicit no-op for workflow artifacts', (t) => {
    const configDir = createTempDir('forge-ial-windsurf-global-');
    t.after(() => cleanup(configDir));
...
describe('installRuntimeArtifacts — cline skills (#782)', () => {
  test('cline: global install writes forge-prefixed skill dirs under skills/', (t) => {
    const configDir = createTempDir('forge-ial-cline-');
    t.after(() => cleanup(configDir));
    ...
    assert.ok(
      fs.existsSync(path.join(skillsDir, 'forge-help', 'SKILL.md')),
      'forge-help/SKILL.md must exist'
    );
  });
});
...
describe('installRuntimeArtifacts — opencode / kilo flat commands', () => {
  for (const runtime of ['opencode', 'kilo']) {
    test(`${runtime}: command/forge-help.md exists`, (t) => {
      const configDir = createTempDir(`forge-ial-${runtime}-`);
      t.after(() => cleanup(configDir));
      ...
      assert.ok(fs.existsSync(path.join(commandDir, 'forge-help.md')));
    });
  }
});
...
describe('installOpencodeFamilySkills — emits skills/<name>/SKILL.md (#784)', () => {
  for (const runtime of ['opencode', 'kilo']) {
    test(`${runtime}: writes forge-help/SKILL.md with name + description`, (t) => {
      const configDir = createTempDir(`forge-ocs-${runtime}-`);
      t.after(() => cleanup(configDir));
      ...
      const skillMd = path.join(configDir, 'skills', 'forge-help', 'SKILL.md');
      assert.ok(fs.existsSync(skillMd), 'forge-help/SKILL.md must exist');
      const content = fs.readFileSync(skillMd, 'utf8');
      assert.match(content, /^name: forge-help$/m, 'name matches dir');
      assert.match(content, /^description: /m, 'description present');
      assert.ok(!/\/forge:/.test(content), 'no /forge: colon refs in body');
    });

    test(`${runtime}: rewrites body paths to the actual install target (#784 path fix)`, (t) => {
      const configDir = createTempDir(`forge-ocp-${runtime}-`);
      t.after(() => cleanup(configDir));
      ...
      const help = fs.readFileSync(path.join(configDir, 'skills', 'forge-help', 'SKILL.md'), 'utf8');
      // forge-help references forge-core workflow files via @<configDir>/forge-core/...
      assert.ok(
        help.includes(`${configDir}/forge-core/`),
        'forge-help body must reference the actual install target via pathPrefix',
      );
    ...
    test(`${runtime}: preserves user-owned forge-dev-preferences across reinstall (#784)`, (t) => {
      const configDir = createTempDir(`forge-ocd-${runtime}-`);
      t.after(() => cleanup(configDir));

      const userSkill = path.join(configDir, 'skills', 'forge-dev-preferences');
      ...
      assert.ok(fs.existsSync(path.join(configDir, 'skills', 'forge-help', 'SKILL.md')));
    });
  }
});
...
describe('uninstallRuntimeArtifacts — consumes Runtime Artifact Uninstall Plan Module', () => {
  test('removes returned plan destinations with layout kind metadata', (t) => {
    const configDir = createTempDir('forge-uninstall-plan-adapter-');
    t.after(() => cleanup(configDir));

    const commandsDir = path.join(configDir, 'custom-commands');
    fs.mkdirSync(commandsDir, { recursive: true });
    fs.writeFileSync(path.join(commandsDir, 'forge-help.md'), '# remove\n');
...
describe('uninstallRuntimeArtifacts — removes forge-owned entries, preserves foreign', () => {
  for (const runtime of ALL_RUNTIMES_LAYOUT) {
    test(`${runtime}: forge entries removed, foreign preserved`, (t) => {
      const configDir = createTempDir(`forge-ual-${runtime}-`);
      t.after(() => cleanup(configDir));
      ...
      if (runtime === 'hermes') {
        ...
        // Seed a forge-* prefixed skill (canonical #947 layout) and a bare-stem skill (#3664 era)
        fs.mkdirSync(path.join(destDir, 'forge-help'), { recursive: true });
        fs.writeFileSync(path.join(destDir, 'forge-help', 'SKILL.md'), '# forge-help\n');
        ...
        // skills/forge/ removed (forge-* removed by _removeGsdEntries, bare-stem by legacy cleanup,
        // then DESCRIPTION.md removed, category dir removed as empty)
        assert.ok(!fs.existsSync(destDir), 'skills/forge/ must be removed after uninstall');
        // User skill outside skills/forge/ preserved
      ...
      for (const kind of layout.kinds) {
        ...
        if (kind.kind === 'skills') {
          ...
          fs.writeFileSync(path.join(destDir, 'subagents', 'forge-executor.yaml'), 'version: 1\n');
          fs.writeFileSync(path.join(destDir, 'subagents', 'forge-executor.md'), '# executor\n');
      ...
      for (const kind of layout.kinds) {
        ...
        if (kind.kind === 'skills') {
          ...
          assert.ok(!fs.existsSync(path.join(destDir, 'subagents', 'forge-executor.yaml')));
          assert.ok(!fs.existsSync(path.join(destDir, 'subagents', 'forge-executor.md')));
...
describe('installRuntimeArtifacts — legacy migrations run before layout copy', () => {
  test('claude: legacy commands/forge/dev-preferences.md migrated AND new skills written', (t) => {
    const configDir = createTempDir('forge-legacy-install-');
    t.after(() => cleanup(configDir));

    const legacyDir = path.join(configDir, 'commands', 'forge');
    ...
    assert.ok(fs.existsSync(path.join(configDir, 'skills', 'forge-dev-preferences', 'SKILL.md')));
    assert.ok(fs.existsSync(path.join(configDir, 'skills', 'forge-help', 'SKILL.md')));
  });

  test('hermes: legacy flat skills/forge-*/ migrated AND new nested skills/forge/forge-<stem>/ written (#947)', (t) => {
    const configDir = createTempDir('forge-legacy-hermes-install-');
    t.after(() => cleanup(configDir));

    const legacyFlatHelp = path.join(configDir, 'skills', 'forge-help');
    ...
    // #947: canonical path is skills/forge/forge-<stem>/ not skills/forge/<stem>/
    assert.ok(fs.existsSync(path.join(configDir, 'skills', 'forge', 'forge-help', 'SKILL.md')),
      'skills/forge/forge-help/SKILL.md must exist after install (#947)');
  });
});
...
describe('uninstallRuntimeArtifacts — legacy cleanup runs before layout removal', () => {
  test('hermes: both flat and nested layouts removed (#947: bare-stem dirs cleaned on uninstall)', (t) => {
    const { uninstallRuntimeArtifacts } = require('../bin/install.js');
    const configDir = createTempDir('forge-legacy-uninstall-hermes-');
    t.after(() => cleanup(configDir));

    const skillsDir = path.join(configDir, 'skills');
    const flatHelp = path.join(skillsDir, 'forge-help');
    fs.mkdirSync(flatHelp, { recursive: true });
    fs.writeFileSync(path.join(flatHelp, 'SKILL.md'), '# legacy flat\n');

    const nestedGsd = path.join(skillsDir, 'forge');
    // Seed a pre-#947 bare-stem GSD skill (no forge- prefix, from #3664 era)
    ...
    // Pre-#2841 flat skills/forge-help/ removed by legacy cleanup
    assert.ok(!fs.existsSync(flatHelp), 'flat forge-help must be removed');
    // skills/forge/ removed: bare-stem dirs cleaned + no forge-* dirs remain → empty → removed
    assert.ok(!fs.existsSync(nestedGsd), 'skills/forge/ must be removed after uninstall');
    // User content outside skills/forge/ preserved
    assert.ok(fs.existsSync(path.join(userSkill, 'SKILL.md')), 'user-skill must be preserved');
  });

  test('claude: legacy commands/forge/ cleaned AND new skills/ entries removed', (t) => {
    const { uninstallRuntimeArtifacts } = require('../bin/install.js');
    const configDir = createTempDir('forge-legacy-uninstall-claude-');
    t.after(() => cleanup(configDir));

    const skillsDir = path.join(configDir, 'skills');
    const gsdHelp = path.join(skillsDir, 'forge-help');
...
describe('skills wrapper threads install scope into converter isGlobal (regression: local installs must not leak global home paths)', () => {
  ...
  for (const { runtime, globalMarker, localMarker } of [
    ...
    test(`${runtime}: local skill content uses workspace path, not global home`, (t) => {
      const globalDir = createTempDir(`forge-ial-g-${runtime}-`);
      const localDir = createTempDir(`forge-ial-l-${runtime}-`);
