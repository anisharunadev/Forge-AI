...
// the initial drift check ran the bare PATH binary `forge-tools verify codebase-drift`.
// On a shim-only install (forge-tools.cjs present, `forge-tools` not on PATH) that exits
// 127, `2>/dev/null` hides it, and the `|| echo` fallback marks the gate skipped —
// so post-execution drift detection silently never runs. The fix resolves forge-tools
// through the runtime shim launcher (forge_run), defining the canonical preamble once in
...
const GATE_MD = path.join(
  __dirname, '..', 'forge-core', 'workflows', 'execute-phase', 'steps', 'codebase-drift-gate.md',
);
const SNIPPET_FILE = path.join(__dirname, '..', 'forge-core', 'workflows', '_runtime-launcher.snippet.sh');
...
describe('bug #619 — codebase-drift-gate resolves forge-tools via the runtime shim, not the bare PATH binary', () => {
  ...
  test('the drift check resolves forge-tools via the shim launcher (forge_run), not the bare binary (#619)', () => {
    const content = readGate();
    assert.match(
      content,
      /DRIFT=\$\(forge_run verify codebase-drift 2>\/dev\/null \|\| echo '\{"skipped":true,"reason":"sdk-failed"\}'\)/,
      'drift check must call `forge_run verify codebase-drift` with the non-blocking skip fallback',
    );
    assert.doesNotMatch(
      content,
      /\bforge-tools verify codebase-drift\b/,
      'the bare `forge-tools verify codebase-drift` PATH-binary call (the #619 bug) must be gone',
    );
  });
  ...
  test('exactly one canonical launcher preamble, in the drift-check block, before any launcher call (#619)', () => {
    ...
    assert.ok(
      block0.indexOf(snippet) < block0.indexOf('forge_run verify codebase-drift'),
      'the preamble must precede the forge_run drift call in the same block',
    );

    // The auto-remap block reuses forge_run but must NOT carry its own preamble.
  ...
  test('shipped drift-check block runs the shim (forge-tools.cjs), not skip, on a shim-only install (#619)', () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-619-'));
    try {
      // Shim-only topology: forge-tools.cjs present under RUNTIME_DIR; no `forge-tools` on PATH.
      const binDir = path.join(tmp, 'forge-core', 'bin');
      ...
      assert.match(out, /SHIM_RAN/, 'the drift check must execute the resolved shim, proving forge_run resolution');
  ...
  test('red-proof: the old bare `forge-tools` form would skip when forge-tools is not on PATH', () => {
    // Documents the #619 bug: the pre-fix bare-binary call, with no `forge-tools` on PATH,
    // hits the 127 → `|| echo` skip path even though the shim (forge-tools.cjs) exists.
    const oldForm =
      'DRIFT=$(forge-tools verify codebase-drift 2>/dev/null || echo \'{"skipped":true,"reason":"sdk-failed"}\'); printf "%s" "$DRIFT"';
    const out = execFileSync('bash', ['-c', 'export PATH=/nonexistent-empty-path; ' + oldForm], {
      env: { ...process.env },
      encoding: 'utf8',
    });
    assert.match(out, /sdk-failed/, 'sanity: the bare-binary form skips without forge-tools on PATH — the bug the fix removes');
  });
});
