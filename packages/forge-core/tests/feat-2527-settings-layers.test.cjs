...
/**
 * Feature test for #2527 — /forge-settings expands to 22 settings grouped into
 ...
 * step, the confirmation table, the ~/.forge/defaults.json save step, and the
...
const SETTINGS_PATH = path.join(__dirname, '..', 'forge-core', 'workflows', 'settings.md');
const {
  VALID_CONFIG_KEYS,
  isCentralConfigKey,
  isValidConfigKey,
    ...
    test('save_as_defaults step references every new field', () => {
      ...
      for (const field of NEW_FIELDS) {
        assert.ok(
          ...
          `save_as_defaults step must persist "${field}" into ~/.forge/defaults.json`
        );
      }
    });
  });
