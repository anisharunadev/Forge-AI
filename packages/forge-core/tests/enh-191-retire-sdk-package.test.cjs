...
test('enhancement #191: sdk package artifacts are removed from repository layout', () => {
  ...
  const shimPath = path.join(ROOT, 'bin', 'forge-sdk.js');

  assert.equal(fs.existsSync(sdkDir), false, 'sdk/ directory must be deleted');
  assert.equal(fs.existsSync(shimPath), false, 'bin/forge-sdk.js must be deleted');
});

test('enhancement #191: published package no longer exposes forge-sdk artifacts', () => {
  const pkg = readPackageJson();

  assert.equal(Object.prototype.hasOwnProperty.call(pkg.bin || {}, 'forge-sdk'), false,
    'package.json bin must not expose forge-sdk');
  assert.equal(pkg.bin && pkg.bin['forge-tools'], 'forge-core/bin/forge-tools.cjs',
    'package.json bin.forge-tools must point to forge-core/bin/forge-tools.cjs');
...
test('enhancement #191: installer does not maintain forge-sdk shim compatibility path', () => {
