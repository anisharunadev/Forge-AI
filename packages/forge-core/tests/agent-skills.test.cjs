...
/**
 * Run forge-tools and capture BOTH stdout and stderr on success.
...
const { loadTrustedGlobalRoots, validatePath } = require('../forge-core/bin/lib/security.cjs');
...
describe('agent-skills command', () => {
  ...
  test('returns empty block when no config exists', () => {
    const r = runAgentSkillsJson(['agent-skills', 'forge-executor'], tmpDir);
    assert.ok(r.success, `Command failed: ${r.error}`);
    assert.strictEqual(r.ir.agent_type, 'forge-executor');
  ...
  test('returns empty block for unconfigured agent type', () => {
    writeConfig(tmpDir, {
      agent_skills: {
        'forge-executor': ['skills/test-skill'],
      },
    });
    const r = runAgentSkillsJson(['agent-skills', 'forge-planner'], tmpDir);
    assert.ok(r.success, `Command failed: ${r.error}`);
    assert.strictEqual(r.ir.agent_type, 'forge-planner');
    assert.strictEqual(r.ir.block, '');
  });
  ...
  test('returns block for configured agent with single string path', () => {
    ...
    writeConfig(tmpDir, {
      agent_skills: {
        'forge-executor': 'skills/my-skill',
      },
    });
  ...
  test('handles multiple skill paths', () => {
    ...
    writeConfig(tmpDir, {
      agent_skills: {
        'forge-executor': ['skills/skill-a', 'skills/skill-b'],
      },
    });
  ...
  test('warns for nonexistent skill path but does not error', () => {
    writeConfig(tmpDir, {
      agent_skills: {
        'forge-executor': ['skills/nonexistent'],
      },
    });
...
describe('agent-skills empty-resolution diagnostics', () => {
  ...
  test('configured agent whose only skill is missing → warnings[] names the path and the aggregate drop', () => {
    writeConfig(tmpDir, {
      agent_skills: { 'forge-phase-researcher': ['references/other-skill'] },
    });

    const r = runAgentSkillsJson(['agent-skills', 'forge-phase-researcher'], tmpDir);
    assert.ok(r.success, `Command failed: ${r.error}`);
  ...
  test('configured agent with all skills missing → aggregate WARNING on stderr naming the agent', () => {
    writeConfig(tmpDir, {
      agent_skills: { 'forge-planner': ['references/a', 'references/b'] },
    });

    const r = runGsdToolsWithStderr(['agent-skills', '--json', 'forge-planner'], tmpDir, {
      HOME: tmpDir,
      USERPROFILE: tmpDir,
    });
    ...
    assert.ok(
      r.stderr.includes('[agent-skills] WARNING') &&
        r.stderr.includes('forge-planner') &&
  ...
  test('partial resolution: one valid + one missing → block present, NO aggregate warning, skipped path still listed', () => {
    ...
    writeConfig(tmpDir, {
      agent_skills: { 'forge-executor': ['skills/present', 'skills/absent'] },
    });
  ...
  test('all skills resolve → warnings[] is empty', () => {
    ...
    writeConfig(tmpDir, {
      agent_skills: { 'forge-executor': ['skills/only'] },
    });
  ...
  test('unconfigured agent → warnings[] empty (no skills configured is not a drop)', () => {
    writeConfig(tmpDir, {
      agent_skills: { 'forge-executor': ['skills/whatever'] },
    });
  ...
  test('malformed (non-array, non-string) configured value → flagged in warnings[], not a silent drop', () => {
    ...
    writeConfig(tmpDir, {
      agent_skills: { 'forge-executor': 42 },
    });
...
describe('config-set agent_skills', () => {
  ...
  test('can set agent_skills via dot notation', () => {
    const result = runGsdTools(
      ['config-set', 'agent_skills.forge-executor', '["skills/my-skill"]'],
      tmpDir,
    ...
    assert.deepStrictEqual(
      config.agent_skills['forge-executor'],
      ['skills/my-skill'],
...
describe('agent-skills global: prefix', () => {
  ...
  beforeEach(() => {
    ...
    fakeHome = fs.mkdtempSync(path.join(require('os').tmpdir(), 'forge-1992-home-'));
  ...
  test('global:valid-skill resolves to $HOME/.claude/skills/valid-skill/SKILL.md', () => {
    ...
    writeConfig(tmpDir, {
      agent_skills: { 'forge-executor': ['global:valid-skill'] },
    });

    const r = runAgentSkillsJson(
      ['agent-skills', 'forge-executor'], tmpDir, { HOME: fakeHome, USERPROFILE: fakeHome }
    );
    assert.ok(r.success, `Command failed: ${r.error}`);
  ...
  test('global:invalid!name is rejected by regex and skipped', () => {
    writeConfig(tmpDir, {
      agent_skills: { 'forge-executor': ['global:invalid!name'] },
    });
  ...
  test('global:missing-skill is skipped when directory is absent', () => {
    writeConfig(tmpDir, {
      agent_skills: { 'forge-executor': ['global:missing-skill'] },
    });
  ...
  test('mix of global: and project-relative paths both resolve correctly', () => {
    ...
    writeConfig(tmpDir, {
      agent_skills: { 'forge-executor': ['global:shadcn', 'skills/local-skill'] },
    });
  ...
  test('global: with empty name produces clear warning and skips', () => {
    writeConfig(tmpDir, {
      agent_skills: { 'forge-executor': ['global:'] },
    });
...
describe('loadTrustedGlobalRoots', () => {
  ...
  test('drops non-string entries from the array', () => {
    ...
    const realDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-tgr-ns-'));
    try {
  ...
  test('keeps absolute paths — real dirs are kept and canonicalized', () => {
    ...
    const dir1 = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-tgr-d1-'));
    const dir2 = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-tgr-d2-'));
    try {
  ...
  test('expands leading ~/ to os.homedir() — kept only if the dir exists', () => {
    ...
    const subdir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-tgr-tilde-'));
    try {
  ...
  test('trusted root that is a symlink is canonicalized to the link target', () => {
    const realTarget = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-tgr-symtgt-'));
    const symlinkPath = path.join(os.tmpdir(), `forge-tgr-symlink-${Date.now()}`);
  ...
  test('de-duplicates entries by canonical path', () => {
    ...
    const realDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-tgr-dedup-'));
    try {
      const realPath = fs.realpathSync(realDir);
...
// triggers a symlink escape scenario through forge-tools subprocess invocation).
...
describe('trusted_global_roots guard logic', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-52-trusted-'));
    externalDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-52-external-'));
  ...
  test('with unrelated trusted root — skill still rejected', () => {
    const skillMd = path.join(externalDir, 'SKILL.md');
    const unrelatedDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-52-unrelated-'));
    try {
...
//
// These tests exercise the full CLI path (runAgentSkillsJson → forge-tools →
...
describe('trusted_global_roots e2e CLI (#52)', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempProject();
    fakeHome = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-52-e2e-home-'));
    globalSkillsDir = path.join(fakeHome, '.claude', 'skills');
    fs.mkdirSync(globalSkillsDir, { recursive: true });
    sharedRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-52-e2e-shared-'));
  ...
  test('REGRESSION: symlinked-escape skill with NO agent_skills_security in config → block is empty', (t) => {
    ...
    writeConfig(tmpDir, {
      runtime: 'claude',
      agent_skills: { 'forge-executor': ['global:shared-skill'] },
    });
  ...
  test('FEATURE NOTE: accepted-via-trusted-root emits NOTE on stderr', (t) => {
    ...
    const r = runGsdToolsWithStderr(
      ['agent-skills', '--json', 'forge-executor'],
      tmpDir,
  ...
  test('NEGATIVE: symlink escape with unrelated trusted root (existing dir) → block is empty', (t) => {
    ...
    const unrelatedRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-52-e2e-unrelated-'));
...
describe('bug #1243: plugin-namespaced agent skills', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempProject();
    fakeHome = fs.mkdtempSync(path.join(require('os').tmpdir(), 'forge-1243-home-'));
    globalSkillsDir = path.join(fakeHome, '.claude', 'skills');
    fs.mkdirSync(globalSkillsDir, { recursive: true });
  });
  ...
  test('happy: global:coderabbit:code-review (claude) emits directive naming coderabbit:code-review, no @-line, no path', () => {
    writeConfig(tmpDir, {
      runtime: 'claude',
      agent_skills: { 'forge-executor': ['global:coderabbit:code-review'] },
    });
  ...
  test('mixed: path-resolvable global + namespaced → @-include AND directive in block', () => {
    ...
    writeConfig(tmpDir, {
      runtime: 'claude',
      agent_skills: {
        'forge-executor': ['global:my-local-skill', 'global:vendor:remote-skill'],
      },
    });
  ...
  test('precedence: bare global:foo not-on-disk → not found/skipped, no directive', () => {
    ...
    writeConfig(tmpDir, {
      runtime: 'claude',
      agent_skills: { 'forge-executor': ['global:foo'] },
    });
  ...
  test('negative: global:../evil rejected', () => {
    writeConfig(tmpDir, {
      runtime: 'claude',
      agent_skills: { 'forge-executor': ['global:../evil'] },
    });
    const r = runAgentSkillsJson(
      ['agent-skills', 'forge-executor'], tmpDir, { HOME: fakeHome, USERPROFILE: fakeHome }
    );
  ...
  test('negative: global:a::b rejected (empty segment)', () => {
    writeConfig(tmpDir, {
      runtime: 'claude',
      agent_skills: { 'forge-executor': ['global:a::b'] },
    });
    const r = runAgentSkillsJson(
      ['agent-skills', 'forge-executor'], tmpDir, { HOME: fakeHome, USERPROFILE: fakeHome }
    );
  ...
  test('negative: global::x rejected (leading colon)', () => {
    writeConfig(tmpDir, {
      runtime: 'claude',
      agent_skills: { 'forge-executor': ['global::x'] },
    });
    const r = runAgentSkillsJson(
      ['agent-skills', 'forge-executor'], tmpDir, { HOME: fakeHome, USERPROFILE: fakeHome }
    );
  ...
  test('cross-runtime: namespaced + codex runtime → no directive (skipped/warned)', () => {
    writeConfig(tmpDir, {
      runtime: 'codex',
      agent_skills: { 'forge-executor': ['global:vendor:remote-skill'] },
    });
  ...
  test('HYRUM regression: include-only block is BYTE-IDENTICAL to expected format', () => {
    ...
    writeConfig(tmpDir, {
      runtime: 'claude',
      agent_skills: { 'forge-executor': ['global:shadcn', 'skills/local'] },
    });
  ...
  test('grant: all 22 agent_skills consumer agents have Skill in their tools frontmatter', () => {
    const CONSUMER_AGENTS = [
      'forge-advisor-researcher',
      'forge-assumptions-analyzer',
      'forge-code-fixer',
      'forge-code-reviewer',
  ...
  test('grant: exact set of agents with Skill equals the 22 consumers (drift guard)', () => {
    ...
    // allow-test-rule: source-text-is-the-product (#1243)
    const AGENTS_DIR = path.join(__dirname, '..', 'agents');
    const agentFiles = fs.readdirSync(AGENTS_DIR).filter((f) => f.startsWith('forge-') && f.endsWith('.md'));
...
describe('agent-skills — Resolution Provenance (#1415)', () => {
  ...
  test('not_configured: agent not in map → configured:false, reason:not_configured, no stderr warning', () => {
    writeConfig(tmpDir, {
      agent_skills: { 'forge-executor': ['skills/foo'] },
    });
    const r = runGsdToolsWithStderr(['agent-skills', '--json', 'forge-planner'], tmpDir, {
      HOME: tmpDir,
      USERPROFILE: tmpDir,
  ...
  test('configured_empty: agent_skills[X]=[] → configured:true, reason:configured_empty, stderr WARNING, skills_count:0', () => {
    writeConfig(tmpDir, {
      agent_skills: { 'forge-executor': [] },
    });
    const r = runGsdToolsWithStderr(['agent-skills', '--json', 'forge-executor'], tmpDir, {
      HOME: tmpDir,
      USERPROFILE: tmpDir,
    });
    assert.ok(r.success, `Command failed: ${r.stderr}`);
  ...
  test('configured_unresolved: configured path that does not exist → reason:configured_unresolved, stderr WARNING', () => {
    writeConfig(tmpDir, {
      agent_skills: { 'forge-executor': ['skills/nonexistent-1415'] },
    });
    const r = runGsdToolsWithStderr(['agent-skills', '--json', 'forge-executor'], tmpDir, {
      HOME: tmpDir,
      USERPROFILE: tmpDir,
  ...
  test('resolved: valid configured path → configured:true, reason:resolved, block non-empty', () => {
    ...
    fs.writeFileSync(path.join(skillDir, 'SKILL.md'), '# My Skill\n');
    writeConfig(tmpDir, {
      agent_skills: { 'forge-executor': ['skills/my-skill-1415'] },
    });
    const r = runAgentSkillsJson(['agent-skills', 'forge-executor'], tmpDir);
    assert.ok(r.success, `Command failed: ${r.error}`);
  ...
  test('cwd-drift: invoking from descendant subdir resolves config from project root', () => {
    ...
    writeConfig(tmpDir, {
      agent_skills: { 'forge-executor': ['skills/drift-skill'] },
    });
    // Invoke from a descendant subdirectory
    const deepDir = path.join(tmpDir, 'src', 'feature');
    fs.mkdirSync(deepDir, { recursive: true });
    const r = runAgentSkillsJson(['agent-skills', 'forge-executor'], deepDir);
    assert.ok(r.success, `Command failed: ${r.error}`);
    assert.strictEqual(r.ir.configured, true);
    assert.strictEqual(r.ir.reason, 'resolved');
  ...
  test('P3 (#1416): --json IR includes value.block and value.skills_count matching flat fields (back-compat)', () => {
    ...
    writeConfig(tmpDir, {
      agent_skills: { 'forge-executor': ['skills/p3-skill'] },
    });
    const r = runAgentSkillsJson(['agent-skills', 'forge-executor'], tmpDir);
    assert.ok(r.success, `Command failed: ${r.error}`);
...
describe('#1400 regression: plain agent-skills output survives pipe/file stdout', () => {
  ...
  // (how workflows consume it via `$(forge_run query agent-skills <type>)`)
  ...
  test('writes the full block to a redirected file (non-empty, not truncated)', () => {
    const { status, contents } = runPlainToFile('forge-executor');
  ...
  test('plain file output equals the --json .block content byte-for-byte', () => {
    const { contents } = runPlainToFile('forge-executor');
    const jsonResult = runAgentSkillsJson(['agent-skills', 'forge-executor'], tmpDir, {
      HOME: tmpDir,
      USERPROFILE: tmpDir,
    });
  ...
  test('writes a >64 KB block through a pipe without truncation (pipe-buffer boundary)', () => {
    ...
    writeConfig(tmpDir, { agent_skills: { 'forge-executor': skillPaths } });
    ...
    const result = spawnSync(
      process.execPath,
      [TOOLS_PATH, 'query', 'agent-skills', 'forge-executor'],
      {
        cwd: tmpDir,
        encoding: 'utf-8',
