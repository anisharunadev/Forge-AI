/**
 * Integration tests for forge-context-monitor.js auto-record on CRITICAL (#1974).
 *
 * Verifies:
...
const HOOK_PATH = path.resolve(__dirname, '..', 'hooks', 'forge-context-monitor.js');
...
/**
 * Run forge-tools state record-session synchronously.
...
describe('#1974 context exhaustion auto-record', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-1974-'));
    ...
    // Minimal config.json required by forge-tools
  ...
  // (hook successfully resolves forge-tools.cjs from any working directory) is already covered
