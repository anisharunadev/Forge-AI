...
const VERIFY_CJS = path.join(__dirname, '..', 'forge-core', 'bin', 'lib', 'verify.cjs');

// Build paths to doc files
const PLANNER_MD = path.join(__dirname, '..', 'agents', 'forge-planner.md');
const ANTIPATTERNS_MD = path.join(__dirname, '..', 'forge-core', 'references', 'planner-antipatterns.md');
...
describe('doc-contract: guidance prose is in place', () => {
  // Case 20: forge-planner.md has the new guidance
  test('case 20 — forge-planner.md contains Region-scoped negative gates + reference', () => {
    const content = fs.readFileSync(PLANNER_MD, 'utf8');
    assert.ok(
      content.includes('Region-scoped negative gates'),
      'forge-planner.md must include "Region-scoped negative gates"',
    );
    assert.ok(
      content.includes('planner-antipatterns.md'),
      'forge-planner.md must reference planner-antipatterns.md',
    );
  });
...
describe('AC3: executable proof — file-wide ban vs region-scoped simultaneously satisfiable', () => {
  test('case 22 — grep/sed proof: both gates simultaneously satisfiable', () => {
    ...
    const tmpFile = path.join(os.tmpdir(), `forge-968-proof-${process.pid}.py`);
    const pyContent = [
      'def make_page():',
    ...
    try {
      ...
      const regionFile = path.join(os.tmpdir(), `forge-968-region-${process.pid}.py`);
      ...
      try {
        ...
        const reindexFile = path.join(os.tmpdir(), `forge-968-reindex-${process.pid}.py`);
