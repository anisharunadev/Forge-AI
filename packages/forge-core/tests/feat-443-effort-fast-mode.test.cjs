...
describe('#443 effort cascade', () => {
  ...
  test('no config -> forge-planner (heavy) defaults to "xhigh" via tier default', () => {
    // forge-planner is heavy tier; manifest default for heavy is xhigh
    assert.strictEqual(resolveEffortInternal(tmpDir, 'forge-planner'), 'xhigh');
  });

  test('routing_tier_defaults: light (forge-codebase-mapper) -> "low"', () => {
    // forge-codebase-mapper routingTier=light, default for light is "low"
    assert.strictEqual(resolveEffortInternal(tmpDir, 'forge-codebase-mapper'), 'low');
  });

  test('routing_tier_defaults: standard (forge-executor) -> "high"', () => {
    // forge-executor routingTier=standard, default for standard is "high"
    assert.strictEqual(resolveEffortInternal(tmpDir, 'forge-executor'), 'high');
  });

  test('routing_tier_defaults: heavy (forge-planner) -> "xhigh"', () => {
    // forge-planner routingTier=heavy, default for heavy is "xhigh"
    assert.strictEqual(resolveEffortInternal(tmpDir, 'forge-planner'), 'xhigh');
  });
  ...
  test('effort.routing_tier_defaults override beats tier default', () => {
    ...
    assert.strictEqual(resolveEffortInternal(tmpDir, 'forge-planner'), 'medium');
  });
  ...
  test('effort.agent_overrides beats routing_tier_defaults', () => {
    writeConfig(tmpDir, {
      effort: {
        ...
        agent_overrides: { 'forge-planner': 'low' },
      },
    });
    assert.strictEqual(resolveEffortInternal(tmpDir, 'forge-planner'), 'low');
  });

  test('opts.override beats agent_overrides', () => {
    writeConfig(tmpDir, {
      effort: { agent_overrides: { 'forge-planner': 'low' } },
    });
    assert.strictEqual(resolveEffortInternal(tmpDir, 'forge-planner', { override: 'minimal' }), 'minimal');
  });
  ...
  test('invalid override falls through to agent_overrides', () => {
    ...
    assert.strictEqual(resolveEffortInternal(tmpDir, 'forge-planner', { override: 'turbo' }), 'low');
  });

  test('invalid agent_overrides value falls through to routing_tier_defaults', () => {
    writeConfig(tmpDir, {
      effort: {
        agent_overrides: { 'forge-planner': 123 },
        routing_tier_defaults: { heavy: 'medium' },
      },
    });
    assert.strictEqual(resolveEffortInternal(tmpDir, 'forge-planner'), 'medium');
  ...
  test('invalid effort.default falls through to hardcoded "high" (no routing_tier_defaults set)', () => {
    ...
    assert.strictEqual(resolveEffortInternal(tmpDir, 'forge-planner'), 'high');
  });
  ...
  test('effort block is non-object (string) -> effortCfg=null -> uses manifest tier default xhigh', () => {
    ...
    // Non-object effort => effortCfg=null; forge-planner heavy tier manifest default = xhigh
    assert.strictEqual(resolveEffortInternal(tmpDir, 'forge-planner'), 'xhigh');
  });
...
describe('#443 fast_mode cascade', () => {
  ...
  test('no config -> defaults to false', () => {
    assert.strictEqual(resolveFastModeInternal(tmpDir, 'forge-planner'), false);
  });
  ...
  test('fast_mode.enabled=true -> true when no tier/agent overrides', () => {
    ...
    // For forge-planner (heavy), tier default is false — falls through to enabled=true
    assert.strictEqual(resolveFastModeInternal(tmpDir, 'forge-planner'), true);
  });
  ...
  test('fast_mode.routing_tier_defaults.light=true -> light agent gets true', () => {
    ...
    assert.strictEqual(resolveFastModeInternal(tmpDir, 'forge-codebase-mapper'), true);
  });
  ...
  test('fast_mode.agent_overrides beats routing_tier_defaults', () => {
    writeConfig(tmpDir, {
      fast_mode: {
        ...
        agent_overrides: { 'forge-codebase-mapper': true },
      },
    });
    assert.strictEqual(resolveFastModeInternal(tmpDir, 'forge-codebase-mapper'), true);
  });
  ...
  test('opts.override beats agent_overrides', () => {
    writeConfig(tmpDir, {
      fast_mode: { agent_overrides: { 'forge-planner': true } },
    });
    assert.strictEqual(resolveFastModeInternal(tmpDir, 'forge-planner', { override: false }), false);
  });
  ...
  test('string "true" NOT accepted as fast_mode override', () => {
    ...
    const result = resolveFastModeInternal(tmpDir, 'forge-planner');
    assert.strictEqual(typeof result, 'boolean');
  });

  test('string "true" in opts.override NOT accepted', () => {
    // opts.override must be strict boolean — string falls through
    const result = resolveFastModeInternal(tmpDir, 'forge-planner', { override: 'true' });
...
describe('#443 resolveEffortForTier escalation', () => {
  ...
  test('dynamic_routing disabled -> attempt ignored, returns base effort', () => {
    // forge-planner heavy -> xhigh baseline
    const base = resolveEffortForTier(tmpDir, 'forge-planner', 0);
    const attempt1 = resolveEffortForTier(tmpDir, 'forge-planner', 1);
  ...
  test('dynamic_routing enabled, escalate_on_failure=false -> attempt ignored', () => {
    ...
    const base = resolveEffortForTier(tmpDir, 'forge-codebase-mapper', 0);
    const attempt1 = resolveEffortForTier(tmpDir, 'forge-codebase-mapper', 1);
  ...
  test('dynamic_routing enabled, attempt=1 -> one step up from base', () => {
    ...
    // forge-codebase-mapper: light -> effort 'low'; attempt=1 -> 'medium'
    assert.strictEqual(resolveEffortForTier(tmpDir, 'forge-codebase-mapper', 0), 'low');
    assert.strictEqual(resolveEffortForTier(tmpDir, 'forge-codebase-mapper', 1), 'medium');
  });
  ...
  test('escalation clamps at "max"', () => {
    ...
    // xhigh -> max -> max (clamp)
    const result = resolveEffortForTier(tmpDir, 'forge-planner', 99);
  ...
  test('respects max_escalations cap', () => {
    ...
    const at1 = resolveEffortForTier(tmpDir, 'forge-codebase-mapper', 1);
    const at2 = resolveEffortForTier(tmpDir, 'forge-codebase-mapper', 2);
...
describe('#443 resolve-execution CLI command', () => {
  ...
  beforeEach(() => {
    ...
    // HOME isolation to prevent ~/.forge/defaults.json bleed
  ...
  test('default (claude) runtime -> effort present, effort_param=output_config.effort, fast_mode_supported=false', () => {
    const result = runGsdTools(['resolve-execution', 'forge-planner'], tmpDir, { HOME: tmpDir });
  ...
  test('--effort flag overrides config effort', () => {
    const result = runGsdTools(
      ['resolve-execution', 'forge-planner', '--effort', 'low'],
      tmpDir,
      { HOME: tmpDir }
    );
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('--fast-mode flag honored', () => {
    const result = runGsdTools(
      ['resolve-execution', 'forge-planner', '--fast-mode', 'true'],
      tmpDir,
      { HOME: tmpDir }
    );
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('--attempt flag triggers escalation', () => {
    ...
    const result0 = runGsdTools(
      ['resolve-execution', 'forge-codebase-mapper', '--attempt', '0'],
      tmpDir,
      { HOME: tmpDir }
    );
    const result1 = runGsdTools(
      ['resolve-execution', 'forge-codebase-mapper', '--attempt', '1'],
      tmpDir,
      { HOME: tmpDir }
    );
  ...
  test('--raw prints effort string', () => {
    const result = runGsdTools(
      ['resolve-execution', 'forge-planner', '--raw'],
      tmpDir,
      { HOME: tmpDir }
    );
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('#443 resolve-model emits effort (unified)', () => {
  ...
  test('resolve-model on claude runtime emits effort (not null)', () => {
    const result = runGsdTools(['resolve-model', 'forge-planner'], tmpDir, { HOME: tmpDir });
    assert.ok(result.success, `Command failed: ${result.error}`);
    const output = JSON.parse(result.output);
...
describe('#443 QA matrix — malformed effort/fast_mode configs', () => {
  ...
  test('effort.default=123 (numeric) -> gracefully falls through', () => {
    ...
    // forge-planner is heavy, tier default xhigh is used instead
    const result = resolveEffortInternal(tmpDir, 'forge-planner');
...
describe('#443 config schema: new effort/fast_mode keys valid', () => {
  ...
  test('effort.agent_overrides.<agent-id> is valid (dynamic pattern)', () => {
    assert.ok(isValidConfigKey('effort.agent_overrides.forge-planner'));
  ...
  test('fast_mode.agent_overrides.<agent-id> is valid', () => {
    assert.ok(isValidConfigKey('fast_mode.agent_overrides.forge-planner'));
  });
...
describe('#443 resolve-execution: deterministic arg parsing (flags-first ordering)', () => {
  ...
  test('flags-first: --effort low forge-planner resolves forge-planner (NOT "low" as agent)', () => {
    // BUG: before fix, agentTypeArg = 'low' (first non-dash token) -> unknown_agent:true
    const result = runGsdTools(
      ['resolve-execution', '--effort', 'low', 'forge-planner'],
      tmpDir,
      { HOME: tmpDir }
    );
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('flags-first: --attempt 1 forge-codebase-mapper resolves forge-codebase-mapper (NOT "1" as agent)', () => {
    // BUG: before fix, agentTypeArg = '1' -> unknown_agent:true
    const result = runGsdTools(
      ['resolve-execution', '--attempt', '1', 'forge-codebase-mapper'],
      tmpDir,
      { HOME: tmpDir }
    );
    assert.ok(result.success, `Command failed: ${result.error}`);
    const output = JSON.parse(result.output);
    assert.ok(!output.unknown_agent, `forge-codebase-mapper must be resolved, got: ${JSON.stringify(output)}`);
  });

  test('agent-first parity: forge-planner --effort low produces same effort as flags-first', () => {
  ...
  test('error: two positional agents -> non-zero exit', () => {
    const result = runGsdTools(
      ['resolve-execution', 'forge-planner', 'forge-executor'],
      tmpDir,
      { HOME: tmpDir }
    );
  ...
  test('error: --attempt notanumber -> non-zero exit, clear error', () => {
    const result = runGsdTools(
      ['resolve-execution', '--attempt', 'notanumber', 'forge-planner'],
      tmpDir,
      { HOME: tmpDir }
    );
  ...
  test('error: trailing --effort (no value) -> non-zero exit', () => {
    const result = runGsdTools(
      ['resolve-execution', 'forge-planner', '--effort'],
      tmpDir,
      { HOME: tmpDir }
    );
