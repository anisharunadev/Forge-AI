...
// commands/forge/*.md files ARE what the runtime loads — testing their
...
const NAMESPACE_SKILLS = [
  { file: 'ns-workflow.md', name: 'forge-workflow' },
  { file: 'ns-project.md',  name: 'forge-project' },
  { file: 'ns-review.md',   name: 'forge-quality' },
  { file: 'ns-context.md',  name: 'forge-context' },
  { file: 'ns-manage.md',   name: 'forge-manage' },
  { file: 'ns-ideate.md',   name: 'forge-ideate' },
];
...
// skills, e.g. `forge-map-codebase --fast` for the former `forge-scan`).
const FLAG_FORM_PARENTS = new Set([
  'forge-code-review',     // --fix absorbs former forge-code-review-fix
  'forge-map-codebase',    // --fast absorbs scan, --query absorbs intel
]);
...
describe('Namespace skill bodies carry a routing table', () => {
  for (const { file } of NAMESPACE_SKILLS) {
    ...
    test(`${file} — body has at least one Invoke target`, () => {
      ...
      assert.ok(hasInvoke, `${file} body must reference at least one forge-* sub-skill`);
    });
  }
});

// ── Context guard contract on forge-health ──────────────────────────────
...
describe('forge-health --context flag is wired into command + workflow', () => {
  const HEALTH_CMD = path.join(COMMANDS_DIR, 'health.md');
  const HEALTH_WORKFLOW = path.join(__dirname, '..', 'forge-core', 'workflows', 'health.md');

  test('commands/forge/health.md argument-hint advertises --context', () => {
  ...
  test('commands/forge/health.md body documents the three-state utilization table', () => {
    const raw = fs.readFileSync(HEALTH_CMD, 'utf-8');
  ...
  test('forge-core/workflows/health.md has a context_check step', () => {
  ...
  test('workflow context_check invokes forge-sdk query validate.context', () => {
    const raw = fs.readFileSync(HEALTH_WORKFLOW, 'utf-8');
    ...
    assert.match(
      stepBody,
      /forge_run\s+query\s+validate\.context/,
      'context_check must call `forge_run query validate.context`',
    );
...
describe('namespace nesting completeness (#69)', () => {
  ...
  test('every router requires entry resolves to a real concrete skill file', () => {
    ...
    assert.deepStrictEqual(
      bad,
      [],
      `Router requires entries with no matching commands/forge/<stem>.md: ${bad.join(', ')}`,
    );
  });
  ...
  test("each router's routing-table rows reference only its own required sub-skills (plus flag variants)", () => {
    ...
    for (const [routerStem, children] of routerRequires) {
      ...
      // Extract forge-<stem> tokens from table data rows (last cell), strip flags
      for (const line of fm._body.split('\n')) {
        ...
        for (const match of lastCell.matchAll(/\bgsd-([a-z][a-z0-9-]*)/g)) {
          ...
          if (!childSet.has(stem)) {
            bad.push(`${routerStem}: body table references forge-${stem} but it's not in requires`);
          }
        }
      }
    }
...
describe('Namespace router targets resolve to surviving skills', () => {
  ...
  for (const f of fs.readdirSync(COMMANDS_DIR)) {
    ...
    if (base === 'extract_learnings') surviving.add('forge-extract-learnings');
  }

  for (const { file } of NAMESPACE_SKILLS) {
    test(`${file} — every routing target resolves`, () => {
      const fm = readNamespaceFile(file);
      // Extract every forge-<name> token that appears in a table-row right column.
      // Strip flag suffixes (`forge-foo --bar` → `forge-foo`) before resolving.
      ...
      assert.ok(targets.size > 0, `${file} routing table must reference at least one forge-* target`);
      ...
      assert.deepStrictEqual(
        unresolved,
        [],
        `${file} routes to skills that don't exist in commands/forge/: ${unresolved.join(', ')}`,
      );
    });
  }
});
