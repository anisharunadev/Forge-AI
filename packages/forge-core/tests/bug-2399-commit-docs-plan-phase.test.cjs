...
describe('plan-phase commit_docs support (#2399)', () => {
  test('plan-phase.md exists', () => {
    assert.ok(fs.existsSync(PLAN_PHASE_PATH), 'forge-core/workflows/plan-phase.md must exist');
  });
  ...
  test('plan-phase.md uses forge-sdk query commit for the plan commit', () => {
    const content = fs.readFileSync(PLAN_PHASE_PATH, 'utf-8');
    // Must use forge-sdk query commit (not raw git) so commit_docs guard in forge-tools is respected
    assert.ok(
      content.includes('forge-sdk query commit') || content.includes('forge-tools') || content.includes('forge-sdk'),
      'plan-phase.md plan commit step must use forge-sdk query commit (not raw git commit)'
    );
  });
});
