...
const commandPath = path.join(repoRoot, 'commands', 'forge', 'milestone-summary.md');
const workflowPath = path.join(repoRoot, 'forge-core', 'workflows', 'milestone-summary.md');

describe('milestone-summary command', () => {
  test('command file exists', () => {
    assert.ok(fs.existsSync(commandPath), 'commands/forge/milestone-summary.md should exist');
  });

  test('command has correct frontmatter name', () => {
    const content = fs.readFileSync(commandPath, 'utf-8');
    assert.ok(content.includes('name: forge:milestone-summary'), 'should have correct command name');
  });
...
describe('milestone-summary fixture-based artifact discovery', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-ms-test-'));
  });
...
describe('audit.cjs module (#2158)', () => {
  ...
  test('auditOpenArtifacts returns structured result with counts', () => {
    const { auditOpenArtifacts } = require('../forge-core/bin/lib/audit.cjs');
  ...
  test('formatAuditReport returns string with header', () => {
    const { auditOpenArtifacts, formatAuditReport } = require('../forge-core/bin/lib/audit.cjs');
...
describe('complete-milestone workflow has pre-close audit gate (#2158)', () => {
  const completeMilestoneContent = fs.readFileSync(
    path.join(__dirname, '..', 'forge-core', 'workflows', 'complete-milestone.md'),
    'utf8',
  );
...
describe('verify-work workflow has phase artifact check (#2157)', () => {
  const verifyWorkContent = fs.readFileSync(
    path.join(__dirname, '..', 'forge-core', 'workflows', 'verify-work.md'),
    'utf8',
  );
...
describe('state.md template has Deferred Items section (#2158)', () => {
  const stateTemplate = fs.readFileSync(
    path.join(__dirname, '..', 'forge-core', 'templates', 'state.md'),
    'utf8',
  );
