 ...
 * forge-core/bin/lib/legacy-cleanup.cjs using real temp dirs so the
...
const { planLegacyCleanup, applyLegacyCleanup } = require(
  path.join(__dirname, '..', 'forge-core', 'bin', 'lib', 'legacy-cleanup.cjs')
);
...
const OLD_PACKAGE_SIGNAL = 'forge-core' + '-cc';

// ─── Helpers ─────────────────────────────────────────────────────────────────

function mkTmpDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'forge-607-'));
}
...
describe('issue-607 legacy-cleanup: planLegacyCleanup', () => {
  ...
  test('flags a hook file in hooks/ whose content contains the old package signal', () => {
    const hookFile = path.join(configDir, 'hooks', 'forge-check-update-worker.js');
  ...
  test('does NOT flag a file whose content does not reference the old package', () => {
    const hookFile = path.join(configDir, 'hooks', 'forge-check-update-worker.js');
    writeFile(hookFile, '// installed via @forge-ai/forge-core\nconsole.log("ok");');
  ...
  test('regression #607 (data-loss): user custom forge-*.js with NO old-package content must NOT appear in plan', () => {
    // Previously the orphaned-hook-by-name rule would flag any forge-*.js not in
    // MANAGED_HOOKS — deleting user-authored hooks. This is the key regression test.
    const userHook = path.join(configDir, 'hooks', 'forge-my-custom.js');
    writeFile(userHook, '// my custom hook — does not reference the old package');

    const plan = planLegacyCleanup([configDir], { homeDir });
    const entry = plan.find((p) => p.path === userHook);
    assert.equal(entry, undefined, 'user custom forge-*.js with no old-package content must NOT be in plan');
  });

  test('regression #607 (data-loss): user custom forge-*.sh with NO old-package content must NOT appear in plan', () => {
    const userHook = path.join(configDir, 'hooks', 'forge-my-custom.sh');
    writeFile(userHook, '#!/bin/sh\n# my custom shell hook, clean');

    const plan = planLegacyCleanup([configDir], { homeDir });
    const entry = plan.find((p) => p.path === userHook);
    assert.equal(entry, undefined, 'user custom forge-*.sh with no old-package content must NOT be in plan');
  });

  // ── self-deletion regression: forge-core/ subtree must NOT be scanned ──

  test('regression #607 (self-deletion): a code file under forge-core/bin/lib/ containing old-package signal must NOT be flagged', () => {
    // The subtree 'forge-core' is no longer scanned — the current package's
    // own infra lives there and would falsely match if scanned.
    const libFile = path.join(configDir, 'forge-core', 'bin', 'lib', 'legacy-cleanup.cjs');
    writeFile(libFile, "'use strict';\nconst SIG = 'forge-core' + '-cc';\nmodule.exports = {};");

    const plan = planLegacyCleanup([configDir], { homeDir });
    const entry = plan.find((p) => p.path === libFile);
    assert.equal(entry, undefined, 'code file in forge-core/ subtree must NOT appear in plan (subtree not scanned)');
  });
  ...
  test('regression #607: CHANGELOG.md containing old-package signal must NOT be flagged', () => {
    ...
    const changelogFile = path.join(configDir, 'forge-core', 'CHANGELOG.md');
    writeFile(changelogFile, '# Changelog\n\nMigrated from ' + OLD_PACKAGE_SIGNAL + ' to @forge-ai/forge-core.');
  ...
  test('regression #607: a workflow .md file containing old-package signal must NOT be flagged', () => {
    const workflowMd = path.join(configDir, 'forge-core', 'workflows', 'update.md');
  ...
  test('flags the legacy shared cache when it exists with reason legacy-shared-cache', () => {
    const cachePath = path.join(homeDir, '.cache', 'forge', 'forge-update-check.json');
  ...
  test('de-duplicates candidates when two configDirs share same absolute path (same dir listed twice)', () => {
    const signalFile = path.join(configDir, 'hooks', 'forge-old-feature.sh');
  ...
  test('plan entries are sorted by path', () => {
    writeFile(path.join(configDir, 'hooks', 'forge-zzz-last.js'), '// ' + OLD_PACKAGE_SIGNAL);
    writeFile(path.join(configDir, 'hooks', 'forge-aaa-first.js'), '// ' + OLD_PACKAGE_SIGNAL);
  ...
  test('plan entries only ever have known reasons', () => {
    writeFile(path.join(configDir, 'hooks', 'forge-worker.js'), '// ' + OLD_PACKAGE_SIGNAL);
    const cachePath = path.join(homeDir, '.cache', 'forge', 'forge-update-check.json');
    writeFile(cachePath, '{}');
    // Stale skill file (#1453)
    const staleSkillPath = path.join(configDir, 'skills', 'forge-docs-update', 'SKILL.md');
    writeFile(staleSkillPath, '@$HOME/.codex/' + 'get-shit-done' + '/workflows/docs-update.md\n'); // forge-allow-legacy-name
    // User custom hook — should NOT appear
    writeFile(path.join(configDir, 'hooks', 'forge-my-custom.js'), '// user hook, clean');

    const plan = planLegacyCleanup([configDir], { homeDir });
    const validReasons = new Set(['content-references-old-package', 'legacy-shared-cache', 'stale-get-shit-done-path']); // forge-allow-legacy-name
    for (const entry of plan) {
      assert.ok(validReasons.has(entry.reason), `unexpected reason: ${entry.reason}`);
    }
  });

  // ── #1453 stale skill path in ~/.agents/skills/forge-* ──────────────────────

  test('#1453: flags a SKILL.md whose content contains a get-shit-done path reference with reason stale-get-shit-done-path', () => { // forge-allow-legacy-name
    // Simulate a stale ~/.agents/skills/forge-docs-update/SKILL.md left by an
    // older GSD install that embedded the pre-rename runtime path.
    const staleSkillFile = path.join(configDir, 'skills', 'forge-docs-update', 'SKILL.md');
    writeFile(
      staleSkillFile,
    ...
    assert.equal(entry.reason, 'stale-get-shit-done-path'); // forge-allow-legacy-name
  });

  test('#1453: does NOT flag a SKILL.md whose content contains the new forge-core path', () => {
    // A freshly installed SKILL.md references the new runtime directory name.
    const freshSkillFile = path.join(configDir, 'skills', 'forge-docs-update', 'SKILL.md');
    writeFile(
      freshSkillFile,
      '---\nname: forge-docs-update\n---\n' +
      '@$HOME/.codex/forge-core/workflows/docs-update.md\n'
    );

    const plan = planLegacyCleanup([configDir], { homeDir });

    const entry = plan.find((p) => p.path === freshSkillFile);
    assert.equal(entry, undefined, 'fresh SKILL.md (forge-core path) must NOT appear in plan');
  });
  ...
  test('#1453: flags SKILL.md with stale path but preserves SKILL.md in the same dir without stale path', () => {
    // Two skill dirs: one stale (get-shit-done ref), one fresh (forge-core ref). // forge-allow-legacy-name
    const staleSkill = path.join(configDir, 'skills', 'forge-docs-update', 'SKILL.md');
    const freshSkill = path.join(configDir, 'skills', 'forge-help', 'SKILL.md');
    writeFile(staleSkill, '@$HOME/.codex/' + 'get-shit-done' + '/workflows/docs-update.md\n'); // forge-allow-legacy-name
    writeFile(freshSkill, '@$HOME/.codex/forge-core/workflows/help.md\n');
    ...
    assert.equal(staleEntry.reason, 'stale-get-shit-done-path'); // forge-allow-legacy-name
    assert.equal(freshEntry, undefined, 'fresh SKILL.md must NOT appear in plan');
  });

  test('#1453: regression — after upgrade to forge-core 1.5.0, stale ~/.agents/skills/forge-docs-update/SKILL.md is removed', () => {
    // Simulate the exact scenario from issue #1453:
    // ~/.agents/skills/forge-docs-update/SKILL.md references the old get-shit-done runtime. // forge-allow-legacy-name
    ...
    const staleSkill  = path.join(agentsDir, 'skills', 'forge-docs-update', 'SKILL.md');
    writeFile(
      staleSkill,
      '---\nname: forge-docs-update\n---\n' +
      '@$HOME/.Codex/' + 'get-shit-done' + '/workflows/docs-update.md\n' // forge-allow-legacy-name
    ...
    assert.ok(entry, 'stale ~/.agents/skills/forge-docs-update/SKILL.md must appear in plan');
    assert.equal(entry.reason, 'stale-get-shit-done-path'); // forge-allow-legacy-name
...
describe('issue-607 legacy-cleanup: applyLegacyCleanup', () => {
  ...
  test('dryRun:true removes nothing and returns dryRun:true + all paths in skipped', () => {
    ...
    const signalHook = path.join(configDir, 'hooks', 'forge-check-update-worker.js');
    writeFile(signalHook, '// ' + OLD_PACKAGE_SIGNAL);
    const cacheFile  = path.join(homeDir, '.cache', 'forge', 'forge-update-check.json');
    writeFile(cacheFile, '{}');
  ...
  test('apply removes flagged files and returns them in removed[]', () => {
    ...
    writeFile(managedHook, '// @forge-ai/forge-core only');
