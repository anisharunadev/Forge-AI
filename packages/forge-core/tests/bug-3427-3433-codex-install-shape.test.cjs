...
describe('#3427 + #3433 — Codex installer avoids duplicate skills and mixed hook representation', { concurrency: false }, () => {
  ...
  beforeEach(() => {
    ...
    tmpRoot = createTempDir('forge-3427-3433-');
  ...
  test('regenerates managed forge-* skill copies and preserves unrelated user skills (#3562 reverses prior #3427/#3433 behaviour)', () => {
    ...
    fs.mkdirSync(path.join(codexHome, 'skills', 'forge-help'), { recursive: true });
    fs.writeFileSync(path.join(codexHome, 'skills', 'forge-help', 'SKILL.md'), legacySkillBody);
    const legacyHash = crypto.createHash('sha256').update(legacySkillBody).digest('hex');
    fs.writeFileSync(path.join(codexHome, 'forge-file-manifest.json'), JSON.stringify({
      version: 1,
      files: {
        'skills/forge-help/SKILL.md': legacyHash,
      },
    }, null, 2));
    ...
    // #3562: $gsd-* commands are discoverable only when skills/forge-*/SKILL.md
    // exists. The installer must regenerate (not remove) the managed forge-*
    // directories.
    assert.equal(entries.includes('forge-help'), true);
    const refreshedBody = fs.readFileSync(path.join(skillsDir, 'forge-help', 'SKILL.md'), 'utf8');
    ...
    assert.equal(frontmatter.name, 'forge-help', 'refreshed SKILL.md frontmatter must declare name: forge-help');

    // Unrelated user skills are preserved — the regen scope is `forge-*` only.
    assert.equal(entries.includes('custom-user-skill'), true);
  });

  test('stores managed SessionStart update hook in hooks.json and removes inline forge hook from config.toml', () => {
    const configToml = [
      '[features]',
      'codex_hooks = true',
      '',
      ...
      'type = "command"',
      'command = "node /tmp/legacy/.codex/hooks/forge-check-update.js"',
      '',
    ].join('\n');
    ...
    assert.equal(tomlCommands.some((cmd) => cmd.includes('forge-check-update.js')), false);
    ...
    const gsdCommands = sessionStartCommands.filter((cmd) => cmd.includes('forge-check-update'));
  ...
  test('uninstall removes managed SessionStart hook from hooks.json but preserves user hooks', () => {
    ...
    fs.writeFileSync(path.join(hooksDir, 'forge-check-update.js'), '// managed hook\n');
    const managedHookPath = path.join(codexHome, 'hooks', 'forge-check-update.js').replace(/\\/g, '/');
