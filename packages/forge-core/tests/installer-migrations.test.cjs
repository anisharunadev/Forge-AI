...
const firstTimeBaselineMigration = require('../forge-core/bin/lib/installer-migrations/000-first-time-baseline.cjs');

function createTempInstall() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'forge-installer-migrations-'));
}
...
function legacyCodexHook(configDir) {
  return {
    hooks: [
      {
        type: 'command',
        command: `node "${path.join(configDir, 'hooks', 'forge-check-update.js')}"`,
      },
    ],
  };
}
...
test('records a first-time baseline while preserving user-owned artifacts', () => {
  ...
  try {
    writeFile(configDir, 'forge-core/workflows/plan.md', 'managed workflow\n');
    writeFile(configDir, 'forge-core/USER-PROFILE.md', 'user profile\n');
    writeManifest(configDir, {
      'forge-core/workflows/plan.md': sha256('managed workflow\n'),
    });
    ...
    assert.equal(fs.readFileSync(path.join(configDir, 'forge-core/workflows/plan.md'), 'utf8'), 'managed workflow\n');
    assert.equal(fs.readFileSync(path.join(configDir, 'forge-core/USER-PROFILE.md'), 'utf8'), 'user profile\n');
      ...
      [
        {
          ...
          relPath: 'forge-core/workflows/plan.md',
          classification: 'managed-pristine',
        },
        {
          type: 'baseline-preserve-user',
          relPath: 'forge-core/USER-PROFILE.md',
...
test('blocks stale GSD-looking baseline artifacts for explicit user choice', () => {
  const configDir = createTempInstall();
  try {
    writeFile(configDir, 'hooks/forge-retired-hook.js', 'old forge hook\n');
    writeManifest(configDir, {});
    ...
    assert.equal(fs.readFileSync(path.join(configDir, 'hooks/forge-retired-hook.js'), 'utf8'), 'old forge hook\n');
    assert.deepEqual(
      ...
      [
        {
          type: 'prompt-user',
          relPath: 'hooks/forge-retired-hook.js',
...
test('records known generated agent artifacts so profile cleanup can remove them', () => {
  const configDir = createTempInstall();
  try {
    writeFile(configDir, 'agents/forge-executor.md', 'old generated agent\n');
    writeFile(configDir, 'agents/forge-executor.toml', 'old generated agent config\n');
    writeFile(configDir, 'agents/forge-local-experiment.md', 'user experiment\n');
    writeManifest(configDir, {});
    ...
    assert.deepEqual(
      ...
      [
        {
          type: 'record-baseline',
          relPath: 'agents/forge-executor.md',
          classification: 'unknown',
        },
        {
          type: 'record-baseline',
          relPath: 'agents/forge-executor.toml',
          classification: 'unknown',
        },
        {
          type: 'prompt-user',
          relPath: 'agents/forge-local-experiment.md',
          classification: 'stale-gsd-looking',
        },
      ]
    );
    assert.deepEqual(result.blocked.map((action) => action.relPath), ['agents/forge-local-experiment.md']);
    assert.equal(fs.readFileSync(path.join(configDir, 'agents/forge-executor.md'), 'utf8'), 'old generated agent\n');
    assert.equal(fs.readFileSync(path.join(configDir, 'agents/forge-executor.toml'), 'utf8'), 'old generated agent config\n');
    assert.equal(fs.readFileSync(path.join(configDir, 'agents/forge-local-experiment.md'), 'utf8'), 'user experiment\n');
  } finally {
    cleanup(configDir);
  }
});
...
test('classifies large files without loading the whole file through readFileSync', (t) => {
  ...
  const relPath = 'skills/forge-large/SKILL.md';
...
test('successful migration rollback removes run-scoped backup directories', (t) => {
  ...
  assert.equal(
    fs.readdirSync(path.join(configDir, 'forge-migration-journal')).some((name) => name.includes('backups')),
    false
  );
});
...
test('refuses to run migrations while another installer owns the migration lock', (t) => {
  const configDir = createTempInstall();
  t.after(() => cleanup(configDir));
  fs.writeFileSync(path.join(configDir, 'forge-install-migration.lock'), 'held by test\n', 'utf8');
...
test('reports lock release failures after migration work completes', (t) => {
  ...
  fs.unlinkSync = (targetPath) => {
    if (path.basename(String(targetPath)) === 'forge-install-migration.lock') {
      ...
      appliedMigrations: [
        {
          ...
          journal: 'forge-migration-journal/prior.json',
        },
      ],
    });
...
test('rolls back touched files and leaves state unchanged when apply fails', () => {
  ...
  try {
    ...
    assert.equal(
      fs.existsSync(path.join(configDir, 'forge-migration-journal')) &&
        fs.readdirSync(path.join(configDir, 'forge-migration-journal')).some((name) =>
  ...
  try {
    ...
    const plan = planInstallerMigrations({
      ...
      migrations: discoverInstallerMigrations({
        migrationsDir: path.join(__dirname, '..', 'forge-core', 'bin', 'lib', 'installer-migrations'),
      }),
      scope: 'global',
      now: () => '2026-05-11T00:00:05.000Z',
    });
...
test('runs a Codex legacy hooks.json cleanup migration without removing user hooks', () => {
  ...
  try {
    writeFile(
      ...
      JSON.stringify({
        SessionStart: [
          ...
          userHook('node "/Users/example/bin/forge-check-update.js"'),
        ],
      }, null, 2)
    );
    writeManifest(configDir, {});
    ...
    assert.deepEqual(commands, [
      ...
      'node "/Users/example/bin/forge-check-update.js"',
    ]);
...
test('skips runtime-specific migration records for other runtimes', () => {
  ...
  try {
    ...
    const hooksJson = JSON.parse(fs.readFileSync(path.join(configDir, 'hooks.json'), 'utf8'));
    assert.equal(hooksJson.SessionStart[0].hooks[0].command, `node "${path.join(configDir, 'hooks', 'forge-check-update.js')}"`);
...
test('shipped installer-migration checksums are locked to a committed baseline (issue #670 guardrail)', () => {
  ...
  const EXPECTED_CHECKSUMS = {
    ...
    '2026-06-02-rename-get-shit-done-to-forge-core':
      'sha256:3a9f1d97f64097fb313203d19c6d93a187a38df61dd299afa5eef73e16124e95',
    // Migration 004: prune stale forge-pristine/get-shit-done/ snapshots (#934) // forge-allow-legacy-name
    '2026-06-09-prune-stale-pristine-get-shit-done': // forge-allow-legacy-name
      'sha256:6555dd044659276fbc204e81793cd92c5315d54e7316bcdd82d2c98d15a7e9e8',
  };

  const { DEFAULT_MIGRATIONS_DIR, migrationChecksum: computeChecksum } = require('../forge-core/bin/lib/installer-migrations.cjs');
