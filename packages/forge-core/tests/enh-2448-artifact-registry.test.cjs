...
/**
 * Tests for canonical artifact registry and forge-health W019 lint (#2448).
 */
...
const { isCanonicalPlanningFile, CANONICAL_EXACT } = require('../forge-core/bin/lib/artifacts.cjs');
const { cmdValidateHealth } = require('../forge-core/bin/lib/verify.cjs');

const _dirsToClean = [];
after(() => { for (const d of _dirsToClean) helpers.cleanup(d); });

function makeTempProject(files = {}) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-2448-'));
...
describe('artifacts.cjs — isCanonicalPlanningFile', () => {
  ...
  test('returns true for RETROSPECTIVE.md (produced by /forge-complete-milestone)', () => {
...
describe('forge-health W019 — unrecognized .planning/ root files', () => {
  ...
  test('templates/README.md exists and documents W019', () => {
    const readme = fs.readFileSync(
      path.join(__dirname, '../forge-core/templates/README.md'), 'utf-8'
    );
