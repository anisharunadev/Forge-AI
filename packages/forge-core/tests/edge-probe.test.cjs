 ...
 * the BUILT artifact (`forge-core/bin/lib/edge-probe.cjs`), which
...
const BUILT_SCRIPT = path.join(__dirname, '..', 'forge-core', 'bin', 'lib', 'edge-probe.cjs');
...
describe('edge-probe: golden fixtures', () => {
  const root = path.join(__dirname, '..', 'forge-core', 'references', 'edge-probe-fixtures');
