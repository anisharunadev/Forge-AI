...
const migration = require('../forge-core/bin/lib/installer-migrations/004-prune-stale-pristine-snapshots.cjs');

function createTempDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'forge-migration-004-test-'));
}
...
describe('plan() — stale pristine subdir absent', () => {
  test('returns empty array when forge-pristine/get-shit-done/ does not exist', () => { // forge-allow-legacy-name
    const configDir = createTempDir();
    try {
      // Only forge-pristine/forge-core/ exists — no legacy subdir.
      writeFile(configDir, 'forge-pristine/forge-core/workflows/plan.md', 'pristine snapshot\n');
  ...
  test('returns empty array when forge-pristine/ does not exist at all', () => {
    const configDir = createTempDir();
    try {
      writeManifest(configDir, {});
      const actions = migration.plan(makePlanCtx(configDir));
...
describe('plan() — stale pristine files present', () => {
  test('emits remove-managed for each file under forge-pristine/get-shit-done/', () => { // forge-allow-legacy-name
    const configDir = createTempDir();
    try {
      writeFile(configDir, 'forge-pristine/get-shit-done/workflows/plan.md', 'old pristine\n'); // forge-allow-legacy-name
      writeFile(configDir, 'forge-pristine/get-shit-done/skills/forge-foo/SKILL.md', 'old skill\n'); // forge-allow-legacy-name
      writeManifest(configDir, {});
      ...
      for (const action of actions) {
        ...
        assert.ok(
          action.relPath.replace(/\\/g, '/').startsWith('forge-pristine/get-shit-done/'), // forge-allow-legacy-name
          `relPath should start with forge-pristine/get-shit-done/, got: ${action.relPath}`, // forge-allow-legacy-name
        );
  ...
  test('emits exactly one remove-managed per file (correct relPaths)', () => {
    const configDir = createTempDir();
    try {
      writeFile(configDir, 'forge-pristine/get-shit-done/workflows/execute-phase.md', 'pristine\n'); // forge-allow-legacy-name
      writeManifest(configDir, {});

      const actions = migration.plan(makePlanCtx(configDir));
      assert.equal(actions.length, 1);
      const relPathNorm = actions[0].relPath.replace(/\\/g, '/');
      assert.equal(relPathNorm, 'forge-pristine/get-shit-done/workflows/execute-phase.md'); // forge-allow-legacy-name
    } finally {
      cleanup(configDir);
    }
  });
  ...
  test('actions include classification override to managed-pristine', () => {
    const configDir = createTempDir();
    try {
      writeFile(configDir, 'forge-pristine/get-shit-done/workflows/plan.md', 'pristine snapshot\n'); // forge-allow-legacy-name
      writeManifest(configDir, {});
...
describe('plan() — stale pristine root is a symlink', () => {
  test('returns empty array when forge-pristine/get-shit-done/ is a symlink', () => { // forge-allow-legacy-name
    const configDir = createTempDir();
    const externalDir = createTempDir();
    try {
      writeFile(externalDir, 'workflows/plan.md', 'pristine content\n');
      // Create forge-pristine/ as a real dir but make get-shit-done/ a symlink. // forge-allow-legacy-name
      fs.mkdirSync(path.join(configDir, 'forge-pristine'), { recursive: true });
      const legacyLink = path.join(configDir, 'forge-pristine', 'get-shit-done'); // forge-allow-legacy-name
...
describe('plan() — symlinked entry inside stale pristine dir is skipped', () => {
  test('symlinked file inside stale pristine dir is not included in plan actions', () => {
    ...
    try {
      // A real file inside forge-pristine/get-shit-done/ // forge-allow-legacy-name
      writeFile(configDir, 'forge-pristine/get-shit-done/workflows/plan.md', 'real pristine\n'); // forge-allow-legacy-name
      ...
      const symlinkPath = path.join(configDir, 'forge-pristine', 'get-shit-done', 'workflows', 'symlinked.md'); // forge-allow-legacy-name
...
describe('plan() — integration: stale pristine files are removed', () => {
  test('stale forge-pristine/get-shit-done/ file is removed after apply', () => { // forge-allow-legacy-name
    const configDir = createTempDir();
    try {
      const fileContent = 'old pristine snapshot content written by forge installer\n';
      writeFile(configDir, 'forge-pristine/get-shit-done/workflows/plan.md', fileContent); // forge-allow-legacy-name
      writeManifest(configDir, {});
      ...
      const stillThere = fs.existsSync(path.join(configDir, 'forge-pristine', 'get-shit-done', 'workflows', 'plan.md')); // forge-allow-legacy-name
