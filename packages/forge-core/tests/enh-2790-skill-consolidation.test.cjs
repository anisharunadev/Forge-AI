...
// ---------------------------------------------------------------------------
// Allowlisted set of user-invocable skills (commands/forge/*.md, ns-* excluded).
...
describe('absorbed skills are removed', () => {
  ...
  for (const [name, reason] of absorbed) {
    test(`commands/forge/${name}.md does NOT exist (${reason})`, () => {
      assert.ok(
...
describe('outright deleted dead skills are removed', () => {
  const deleted = [
    ...
    // from-forge2          → import --from-forge2 (pending #3131)
  ];

  for (const name of deleted) {
    test(`commands/forge/${name}.md does NOT exist`, () => {
      assert.ok(
        !fs.existsSync(skillPath(name)),
...
describe('#3131 re-wired workflows: standalone command files must not exist', () => {
  const rewired = [
    ...
    ['from-forge2',              'absorbed into import.md --from-forge2'],
  ];
...
describe('#3131 re-wired workflows: parent command argument-hints advertise the new flags', () => {
  ...
  test('import.md argument-hint contains --from-forge2', () => {
    const fm = parseFrontmatter(skillPath('import'));
    assert.ok(
      (fm['argument-hint'] || '').includes('--from-forge2'),
      'import.md argument-hint does not contain --from-forge2. got: ' + (fm['argument-hint'] || '(none)'),
    );
  });
});
...
describe('#3131 re-wired workflows: parent command bodies dispatch to workflow files', () => {
  ...
  test('import.md body references from-forge2', () => {
    assert.ok(
      bodyContains('import', 'from-forge2'),
      'import.md body does not reference from-forge2 — --from-forge2 flag dispatch is missing',
    );
  });
});
...
describe('settings.md is kept (merged into config entry point or remains standalone)', () => {
  test('commands/forge/settings.md still exists', () => {
    assert.ok(
