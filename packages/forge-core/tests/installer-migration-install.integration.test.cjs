...
function assertHasGsdDirectory(root, relPath) {
  assert.ok(
    listDirNames(root, relPath).some((name) => name.startsWith('forge-')),
...
function assertFreshInstallContract(runtime, targetDir) {
  ...
  if (contract.workflowPayload !== false) {
    assert.equal(
      fs.readFileSync(path.join(targetDir, 'forge-core', 'VERSION'), 'utf8'),
      pkg.version,
      `${runtime} should install the package VERSION`
    );
    assert.ok(
      fs.existsSync(path.join(targetDir, 'forge-core', 'bin', 'forge-tools.cjs')),
      `${runtime} should install the GSD tool payload`
    );
    assert.ok(
      fs.existsSync(path.join(targetDir, 'forge-file-manifest.json')),
      `${runtime} should write the install manifest`
    );

    const manifest = JSON.parse(fs.readFileSync(path.join(targetDir, 'forge-file-manifest.json'), 'utf8'));
    ...
    assert.ok(
      manifest.files['forge-core/VERSION'],
      `${runtime} manifest should track the installed VERSION file`
    );
  } else {
    assert.equal(
      fs.existsSync(path.join(targetDir, 'forge-core')),
      false,
  ...
  if (contract.surface === 'flat-skills') {
    // Pre-#3562: codex was special-cased to expect zero forge-* skill dirs
    ...
    // Hermes layout uses prefix: '' — skill dirs have bare stem names (no forge- prefix).
    // Assert that the category dir contains at least one skill dir with SKILL.md.
    const hermesGsdDir = path.join(targetDir, 'skills', 'forge');
    const hermesSkillCount = fs.existsSync(hermesGsdDir)
      ? fs.readdirSync(hermesGsdDir, { withFileTypes: true })
          ...
          .length
      : 0;
    assert.ok(hermesSkillCount > 0, `skills/forge should contain generated GSD entries (got ${hermesSkillCount})`);
    assert.ok(
      fs.existsSync(path.join(targetDir, 'skills', 'forge', 'DESCRIPTION.md')),
      'Hermes should install the nested GSD category description'
    );
  } else if (contract.surface === 'flat-command') {
    assert.ok(
      listDirNames(targetDir, 'command').some((name) => name.startsWith('forge-') && name.endsWith('.md')),
      `${runtime} should install flattened command markdown files`
    );
  } else if (contract.surface === 'commands-gsd') {
    assert.ok(
      listDirNames(targetDir, path.join('commands', 'forge')).length > 0,
      `${runtime} should install commands/forge entries`
    );
    ...
    assert.ok(
      fs.existsSync(path.join(targetDir, 'agents', 'subagents', 'forge-executor.yaml')),
      'Kimi should install GSD subagent YAML'
    );
  } else if (contract.surface === 'clinerules') {
    // #787: Cline now uses the .clinerules/ directory form (rules at forge.md).
    assert.match(
      fs.readFileSync(path.join(targetDir, '.clinerules', 'gsd.md'), 'utf8'),
      /GSD workflows live in `forge-core\/workflows\/`/,
  ...
  if (contract.surface !== 'kimi-skills-agents' && contract.surface !== 'global-artifacts-noop') {
    assert.ok(
      listDirNames(targetDir, 'agents').some((name) => name.startsWith('forge-')),
...
describe('installer migration install integration', { concurrency: false }, () => {
  ...
  beforeEach(() => {
    tmpRoot = createTempDir('forge-install-migrations-');
  ...
  test('blocks install before materialization when baseline needs explicit user choice', () => {
    writeFile(codexHome, 'hooks/forge-retired-hook.txt', 'old forge hook\n');
    ...
    assert.equal(fs.readFileSync(path.join(codexHome, 'hooks/forge-retired-hook.txt'), 'utf8'), 'old forge hook\n');
    assert.equal(fs.existsSync(path.join(codexHome, 'skills')), false);
    assert.equal(fs.existsSync(path.join(codexHome, 'forge-core', 'VERSION')), false);
  });
  ...
  test('rolls back applied migrations when package materialization fails for non-Codex installs', () => {
    ...
    assert.throws(
      () => captureConsole(() =>
        withEnv('CLAUDE_CONFIG_DIR', claudeHome, () =>
          withWriteFailure(path.join(claudeHome, 'forge-core', 'VERSION'), () => install(true, 'claude'))
        )
      ),
    ...
    assert.equal(fs.existsSync(path.join(claudeHome, 'forge-install-state.json')), false);
  });
  ...
  test('rolls back completed runtime migrations when a later runtime install fails', () => {
      ...
      () => captureConsole(() =>
        withEnv('CLAUDE_CONFIG_DIR', claudeHome, () =>
          withEnv('CODEX_HOME', codexHome, () =>
            withWriteFailure(path.join(codexHome, 'forge-core', 'VERSION'), () =>
    ...
    assert.equal(fs.existsSync(path.join(codexHome, 'forge-install-state.json')), false);
  });
  ...
  for (const runtime of SUPPORTED_RUNTIMES) {
    test(`runs a full end-to-end install for ${runtime}`, () => {
      ...
      const installState = JSON.parse(fs.readFileSync(path.join(targetDir, 'forge-install-state.json'), 'utf8'));
      assert.ok(
    ...
    test(`blocks ambiguous GSD-looking user-choice artifacts for ${runtime}`, () => {
      ...
      fs.mkdirSync(targetDir, { recursive: true });
      writeFile(targetDir, 'forge-core/forge-retired-tool.cjs', 'old ambiguous artifact\n');
      ...
      const output = stripAnsi(`${result.stdout}\n${result.stderr}`);
      assert.match(output, /Installer migrations/);
      assert.match(output, /blocked\s+forge-core\/forge-retired-tool\.cjs/);
      assert.match(output, /installer migration blocked/);
      assert.equal(
        fs.readFileSync(path.join(targetDir, 'forge-core/forge-retired-tool.cjs'), 'utf8'),
        'old ambiguous artifact\n'
      );
      assert.equal(fs.existsSync(path.join(targetDir, 'forge-core', 'VERSION')), false);
    });
  }
});
