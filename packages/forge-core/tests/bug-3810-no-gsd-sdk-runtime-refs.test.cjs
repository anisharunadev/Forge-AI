...
// runtime loads and executes. Asserting that text carries no retired `forge-sdk`
...
/**
 * Regression guard: no `forge-sdk` references in runtime-facing surfaces (#339).
 *
 * The `@openforge/forge-sdk` package and its `forge-sdk` binary were retired (ADR 0174,
 * #191). The bulk runtime cleanup is already done — this test locks it in so a
 * `forge-sdk` / `GSD_SDK` reference cannot creep back into a shipped prompt or hook
 ...
 *     surface. (It carries zero `forge-sdk` references today; the SDK-shim
 *     verification subsystem was removed in #515 and the shim retired in #522.)
 *   - `forge-core/bin/` — executable library code, not deployed prompt text; it
 ...
 *
 * Complements `tests/forge-tools-path-refs.test.cjs`, which only catches the
 * `forge-sdk query` binary-invocation form; this catches ANY runtime reference.
 */
...
const RUNTIME_SURFACES = [
  ...
  { dir: path.join('forge-core', 'workflows'), exts: ['.md', '.sh', '.json'] },
  { dir: path.join('forge-core', 'references'), exts: ['.md'] },
  // Prompt surfaces the installer deep-copies and the runtime loads via
  // `@~/.claude/forge-core/templates/*.md` anchors in workflows/commands; the
  // lone config.json under templates/ ships too. (#691 review)
  { dir: path.join('forge-core', 'templates'), exts: ['.md', '.json'] },
  { dir: path.join('forge-core', 'contexts'), exts: ['.md'] },
  { dir: path.join('commands', 'forge'), exts: ['.md'] },
  { dir: 'agents', exts: ['.md'] },
...
// forge-sdk, gsd_sdk, GSD-SDK, GSD_SDK, etc.
...
describe('#339 no forge-sdk references in runtime surfaces', () => {
  test('shipped prompts and hooks carry no retired forge-sdk reference', () => {
    const violations = [];
    ...
    assert.strictEqual(
      violations.length,
      0,
      'Runtime surfaces must not reference the retired forge-sdk binary/package — ' +
        'use forge-tools instead.\nViolations:\n' + violations.join('\n')
    );
  });
  ...
  test('at least one file per configured extension is scanned (guards against an empty sweep)', () => {
    ...
    // forge-core/workflows the .md files alone keep a per-surface count > 0, so
