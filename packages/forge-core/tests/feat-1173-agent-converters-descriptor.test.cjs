 ...
 * The .gsd-source convention expects the marker to point at a commands/forge dir;
 * agents/ is resolved as a sibling of commands/. So we set up:
 *   <tmproot>/
 *     commands/forge/       (empty, satisfies the sibling check)
 *     agents/
 *       forge-planner.md
 *     .gsd-source         <- points to <tmproot>/commands/gsd
 */
function makeFixtureRoot(agentFiles) {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-1173-root-'));
  const commandsDir = path.join(root, 'commands', 'forge');
  const agentsDir = path.join(root, 'agents');
  fs.mkdirSync(commandsDir, { recursive: true });
  fs.mkdirSync(agentsDir, { recursive: true });
  // .gsd-source marker must point to commands/forge so that agentsSourceRoot resolves to agents/
...
describe('feat-1173: stageAgentsForRuntimeWithConverter', () => {
  test('is exported from install-profiles', () => {
    const installProfiles = require(path.join(ROOT, 'forge-core', 'bin', 'lib', 'install-profiles.cjs'));
    assert.strictEqual(
  ...
  test('applies converter to each staged agent file', (t) => {
    const { stageAgentsForRuntimeWithConverter } = require(path.join(ROOT, 'forge-core', 'bin', 'lib', 'install-profiles.cjs'));

    const agentsDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-1173-agents-'));
    fs.writeFileSync(path.join(agentsDir, 'forge-planner.md'), CLAUDE_AGENT_SOURCE, 'utf8');
    fs.writeFileSync(path.join(agentsDir, 'forge-executor.md'), CLAUDE_AGENT_SOURCE.replace('forge-planner', 'forge-executor'), 'utf8');
    ...
    assert.deepStrictEqual(stagedFiles, ['forge-executor.md', 'forge-planner.md']);
  ...
  test('non-existent srcAgentsDir returns srcAgentsDir unchanged', () => {
    const { stageAgentsForRuntimeWithConverter } = require(path.join(ROOT, 'forge-core', 'bin', 'lib', 'install-profiles.cjs'));
    const ghost = path.join(os.tmpdir(), 'forge-1173-no-exist-' + Date.now());
    const converter = (c) => c;
  ...
  test('only copies .md files (ignores non-.md)', (t) => {
    ...
    assert.deepStrictEqual(stagedFiles, ['forge-planner.md'], 'only .md files should be staged');
  });
});
...
describe('feat-1173: dispatchKindEntry agents converter wiring', () => {
  test('agents kind with convertClaudeAgentToCopilotAgent converter applies copilot conversion', (t) => {
    const fixtureRoot = makeFixtureRoot([{ name: 'forge-planner.md', content: CLAUDE_AGENT_SOURCE }]);
    t.after(() => {
      cleanup(fixtureRoot);
      cleanupStagedSkills();
    });
    ...
    const stagedFile = path.join(stagedDir, 'forge-planner.md');
  ...
  test('agents kind with convertClaudeAgentToCodexAgent converter applies codex conversion', (t) => {
    ...
    const stagedContent = fs.readFileSync(path.join(stagedDir, 'forge-planner.md'), 'utf8');
  ...
  test('scope threads isGlobal to a scope-aware converter (global vs local differ)', (t) => {
    ...
    const stageFor = (scope) => {
      ...
      return fs.readFileSync(path.join(agentKind.stage(profile), 'forge-planner.md'), 'utf8');
    };
...
describe('feat-1173: real registry claude agents kind has converter=null (backward compat)', () => {
  test('claude local artifacts layout has agents entry with converter=null', () => {
    const registry = require(path.join(ROOT, 'forge-core', 'bin', 'lib', 'capability-registry.cjs'));
