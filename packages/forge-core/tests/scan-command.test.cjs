...
describe('scan command', () => {
  test('scan is now a --fast flag on map-codebase.md (#2790)', () => {
    const p = path.join(__dirname, '..', 'commands', 'forge', 'map-codebase.md');
    assert.ok(fs.existsSync(p), 'commands/forge/map-codebase.md should exist');
  ...
  test('workflow file exists', () => {
    const p = path.join(__dirname, '..', 'forge-core', 'workflows', 'scan.md');
    assert.ok(fs.existsSync(p), 'forge-core/workflows/scan.md should exist');
  });
  ...
  test('workflow references forge-codebase-mapper', () => {
    const p = path.join(__dirname, '..', 'forge-core', 'workflows', 'scan.md');
    const content = fs.readFileSync(p, 'utf-8');
    assert.ok(content.includes('forge-codebase-mapper'),
      'Workflow should reference the forge-codebase-mapper agent');
  });
});
