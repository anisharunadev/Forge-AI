 ...
 * GSD Workspace Tests
 *
 * Tests for /forge-new-workspace, /forge-list-workspaces, /forge-remove-workspace
 * init functions and integration with forge-tools routing.
 */
...
const { detectChildRepos } = require('../forge-core/bin/lib/init.cjs');
...
describe('detectChildRepos', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('forge-ws-test-');
  });
...
// ─── cmdInitNewWorkspace via forge-tools ──────────────────────────────────────
...
// ─── cmdInitListWorkspaces via forge-tools ────────────────────────────────────
...
describe('init list-workspaces', () => {
  ...
  test('finds workspaces with WORKSPACE.md', () => {
    const wsBase = path.join(tmpDir, 'forge-workspaces');
...
// ─── cmdInitRemoveWorkspace via forge-tools ───────────────────────────────────
...
describe('workspace worktree integration', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('forge-ws-integ-');
...
describe('workspace command files', () => {
   ...
   * Each line of the form `@~/.claude/forge-core/workflows/foo.md` becomes
  ...
  function executionContextIncludes(body) {
    ...
    for (const blk of blocks) {
      for (const line of blk.split('\n')) {
        ...
        // Normalize away the home-prefix and the `.claude/forge-core/` root
        // so the test only cares about the workflow path tail.
        const rel = t.replace(/^@~?\/?(?:\.claude\/)?(?:forge-core\/)?/, '');
        targets.push(rel);
      }
    }
    return targets;
  ...
  test('consolidated workspace.md command declares correct frontmatter contract (#2790)', () => {
    ...
    const { fm } = parseCommandFile(path.join(baseDir, 'commands/forge/workspace.md'));
    assert.equal(fm.name, 'forge:workspace', `workspace.md frontmatter name must be "forge:workspace"; got "${fm.name}"`);
  ...
  test('workspace.md @-includes the new-workspace workflow', () => {
    const { body } = parseCommandFile(path.join(baseDir, 'commands/forge/workspace.md'));
  ...
  test('new-workspace workflow exists', () => {
    const content = fs.readFileSync(path.join(baseDir, 'forge-core/workflows/new-workspace.md'), 'utf8');
    assert.ok(
      content.includes('init new-workspace') || content.includes('init.new-workspace'),
      'expected init new-workspace (CJS) or forge-sdk query init.new-workspace'
    );
  ...
  test('list-workspaces workflow exists', () => {
    const content = fs.readFileSync(path.join(baseDir, 'forge-core/workflows/list-workspaces.md'), 'utf8');
    assert.ok(
      content.includes('init list-workspaces') || content.includes('init.list-workspaces'),
      'expected init list-workspaces or forge-sdk query init.list-workspaces'
    );
  });

  test('remove-workspace workflow exists', () => {
    const content = fs.readFileSync(path.join(baseDir, 'forge-core/workflows/remove-workspace.md'), 'utf8');
    assert.ok(
      content.includes('init remove-workspace') || content.includes('init.remove-workspace'),
      'expected init remove-workspace or forge-sdk query init.remove-workspace'
    );
    assert.ok(content.includes('git worktree remove'));
  });
});

// ─── Routing in forge-tools ───────────────────────────────────────────────────

describe('workspace routing in forge-tools', () => {
  let tmpDir;
