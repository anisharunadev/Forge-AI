...
// Reads shipped source files (commands/forge/*.md, agents/*.md, bin/install.js) as
...
// runtime-slash.cjs eliminated literal /forge: refs from runtime CJS, so reading
...
describe('convertClaudeToCopilotContent', () => {
  ...
  test('converts forge: to forge- in command names', () => {
    assert.strictEqual(
      convertClaudeToCopilotContent('run /forge:health or forge:progress'),
      'run /forge-health or forge-progress'
    );
  });
  ...
  test('handles mixed content in local mode', () => {
    const input = 'Config at ~/.claude/settings and $HOME/.claude/config.\n' +
      ...
      'Run forge:health and /forge:progress.';
    ...
    assert.ok(result.includes('forge-health'), 'command name converted');
    assert.ok(result.includes('/forge-progress'), 'slash command converted');
  });
...
Body content here referencing ~/.claude/foo and forge:health.`;

    const result = convertClaudeCommandToCopilotSkill(input, 'forge-health');
    const fm = parseFrontmatter(result);
    assert.equal(fm.name, 'forge-health', 'name uses param');
    ...
    assert.ok(result.includes('forge-health'), 'CONV-07 applied to body');
    assert.ok(!result.includes('forge:health'), 'no forge: references remain');
  });
...
Help content.`;
    ...
    const result = convertClaudeCommandToCopilotSkill(input, 'forge-help');
    const fm = parseFrontmatter(result);
    assert.equal(fm.name, 'forge-help', 'name set');
...
Progress body.`;
    ...
    const result = convertClaudeCommandToCopilotSkill(input, 'forge-progress');
...
Body.`;
    ...
    const result = convertClaudeCommandToCopilotSkill(input, 'forge-new-milestone');
...
Check ~/.claude/settings and ./.claude/local and $HOME/.claude/global.`;
    ...
    const result = convertClaudeCommandToCopilotSkill(input, 'forge-test');
    assert.ok(result.includes('.github/settings'), 'tilde path converted to local');
...
Check ~/.claude/settings and ./.claude/local and $HOME/.claude/global.`;
    ...
    const result = convertClaudeCommandToCopilotSkill(input, 'forge-test', null, null, true);
    assert.ok(result.includes('~/.copilot/settings'), 'tilde path converted to global');
    assert.ok(result.includes('./.github/local'), 'dot-slash path converted');
...
Run forge:health and /forge:progress for diagnostics.`;

    const result = convertClaudeCommandToCopilotSkill(input, 'forge-test');
    assert.ok(result.includes('forge-health'), 'forge:health converted');
    assert.ok(result.includes('/forge-progress'), '/forge:progress converted');
    assert.ok(!result.match(/forge:[a-z]/), 'no forge: command refs remain');
  });

  test('handles content without frontmatter (local mode)', () => {
    const input = 'Just some markdown with ~/.claude/path and forge:health.';
    const result = convertClaudeCommandToCopilotSkill(input, 'forge-test');
    assert.ok(result.includes('.github/path'), 'CONV-06 applied (local)');
    assert.ok(result.includes('forge-health'), 'CONV-07 applied');
...
Body.`;
    ...
    const result = convertClaudeCommandToCopilotSkill(input, 'forge-execute-phase');
    assert.ok(result.includes('agent: forge-planner'), 'agent field preserved');
  });
});
...
Body.`;
    ...
    const result = convertClaudeAgentToCopilotAgent(input);
    const fm = parseFrontmatter(result);
    assert.equal(fm.name, 'forge-executor', 'name preserved');
...
Check ~/.claude/settings and run forge:health.`;
...
Check ~/.claude/settings and run forge:health.`;
  ...
  test('handles content without frontmatter (local mode)', () => {
    const input = 'Just markdown with ~/.claude/path and forge:test.';
    const result = convertClaudeAgentToCopilotAgent(input);
    assert.ok(result.includes('.github/path'), 'CONV-06 applied (local)');
    assert.ok(result.includes('forge-test'), 'CONV-07 applied');
    assert.ok(!result.includes('---'), 'no frontmatter added');
  });
});
...
describe('installRuntimeArtifacts (copilot integration)', () => {
  // Pivoted from copyCommandsAsCopilotSkills(srcDir, tempDir, 'forge') shim to
  ...
  const srcDir = path.join(__dirname, '..', 'commands', 'forge');
  let configDir;

  beforeEach(() => {
    configDir = createTempDir('forge-copilot-skills-');
  });
  ...
  test('creates skill folders from source commands', () => {
    ...
    assert.ok(fs.existsSync(path.join(skillsDir, 'forge-health')), 'forge-health folder exists');
    assert.ok(fs.existsSync(path.join(skillsDir, 'forge-health', 'SKILL.md')), 'forge-health/SKILL.md exists');
    assert.ok(fs.existsSync(path.join(skillsDir, 'forge-help')), 'forge-help folder exists');
    assert.ok(fs.existsSync(path.join(skillsDir, 'forge-progress')), 'forge-progress folder exists');

    // Count forge-* directories — should match number of source command files
  ...
  test('skill content has Copilot frontmatter format', () => {
    ...
    const skillsDir = path.join(configDir, 'skills');
    const skillContent = fs.readFileSync(path.join(skillsDir, 'forge-health', 'SKILL.md'), 'utf8');
    // Frontmatter format checks
    assert.ok(skillContent.startsWith('---\nname: forge-health\n'), 'starts with name: forge-health');
    ...
    assert.ok(!skillContent.match(/forge:[a-z]/), 'no forge: command references');
  });

  test('generates forge-autonomous skill from autonomous.md command', () => {
    // Fail-fast: source command must exist
    const srcFile = path.join(srcDir, 'autonomous.md');
    assert.ok(fs.existsSync(srcFile), 'commands/forge/autonomous.md must exist as source');

    installRuntimeArtifacts('copilot', configDir, 'global', resolvedProfileFull);

    const skillsDir = path.join(configDir, 'skills');
    // Skill folder and file created
    assert.ok(fs.existsSync(path.join(skillsDir, 'forge-autonomous')), 'forge-autonomous folder exists');
    assert.ok(fs.existsSync(path.join(skillsDir, 'forge-autonomous', 'SKILL.md')), 'forge-autonomous/SKILL.md exists');

    const skillContent = fs.readFileSync(path.join(skillsDir, 'forge-autonomous', 'SKILL.md'), 'utf8');
    const fm = parseFrontmatter(skillContent);

    // Frontmatter: name converted from forge:autonomous to forge-autonomous
    assert.equal(fm.name, 'forge-autonomous', 'name is forge-autonomous');
    assert.equal(
      fm.description,
  ...
  test('autonomous skill body converts forge: to forge- (CONV-07)', () => {
    ...
    // forge:autonomous references should be converted to forge-autonomous
    assert.ok(!result.match(/forge:[a-z]/), 'no forge: command references remain after conversion');
    // Specific: forge:discuss-phase, forge:plan-phase, forge:execute-phase mentioned in body
    // The body references forge-tools.cjs (not a forge: command) — those should be unaffected
    // But /forge:autonomous → /forge-autonomous, forge:discuss-phase → forge-discuss-phase etc.
    if (srcContent.includes('forge:autonomous')) {
      assert.ok(result.includes('forge-autonomous'), 'forge:autonomous converted to forge-autonomous');
    }
  ...
  test('cleans up old skill directories on re-run', () => {
    ...
    const staleDir = path.join(skillsDir, 'forge-old-stale-skill');
    ...
    assert.ok(fs.existsSync(path.join(skillsDir, 'forge-health')), 'real dirs still exist');
    // Stale GSD-prefixed dir is removed by pre-prune
    assert.ok(!fs.existsSync(staleDir), 'stale forge-* dir removed by pre-prune');
...
describe('Copilot agent conversion - real files', () => {
  ...
  test('converts forge-executor agent correctly', () => {
    const content = fs.readFileSync(path.join(agentsSrc, 'forge-executor.md'), 'utf8');
  ...
  test('converts agent with mcp wildcard tools correctly', () => {
    const content = fs.readFileSync(path.join(agentsSrc, 'forge-phase-researcher.md'), 'utf8');
    const result = convertClaudeAgentToCopilotAgent(content);
...
describe('Copilot content conversion - engine files', () => {
  test('converts engine .md files correctly (local mode default)', () => {
    const healthMd = fs.readFileSync(
      path.join(__dirname, '..', 'forge-core', 'workflows', 'health.md'), 'utf8'
    );
    ...
    assert.ok(!result.match(/\/forge:[a-z]/), 'no /forge: command references remain');
    assert.ok(!result.match(/(?<!\/)forge:[a-z]/), 'no bare forge: command references remain');
  ...
  test('converts engine .cjs files correctly', () => {
    // #3584: bin/lib/*.cjs no longer hardcodes `/forge:<cmd>` literals — runtime
    ...
    // colon-form references (commands/forge/*.md, workflow .md files, etc.), so
    ...
    const synthetic = [
      ...
      'On error, run /forge:health --repair to regenerate.',
      'For phase work, use /forge:execute-phase 1.',
    ].join('\n');
    const result = convertClaudeToCopilotContent(synthetic);

    assert.ok(!result.match(/forge:[a-z]/), 'no forge: references remain after conversion');
    assert.ok(result.includes('forge-new-project'), 'forge:new-project converted to hyphen form');
    assert.ok(result.includes('forge-health'), 'forge:health converted to hyphen form');
    assert.ok(result.includes('forge-execute-phase'), 'forge:execute-phase converted to hyphen form');
  });
});
...
describe('Copilot instructions merge/strip', () => {
  ...
  describe('mergeCopilotInstructions', () => {
    ...
    beforeEach(() => {
      tmpMergeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-merge-'));
    });
...
describe('Copilot lifecycle hook config (#786)', () => {
  describe('buildCopilotHookConfig', () => {
    ...
    test('executing the bash hook body produces valid sessionStart JSON', { skip: process.platform === 'win32' }, () => {
      ...
      const [entry] = buildCopilotHookConfig().hooks.sessionStart;
      const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-hook-exec-'));
      try {
        ...
        assert.ok(/forge-new-project/.test(parsedAbsent.additionalContext), 'absent branch suggests forge-new-project');
    ...
    test('hook command references no external script path (cannot dangle)', () => {
      ...
      // The GSD Copilot hook is inline, so it must not reference hooks/forge-*.js|sh.
      assert.ok(!/hooks\/forge-[\w-]+\.(js|cjs|sh)/.test(entry.bash), 'bash body references no forge hook script file');
      assert.ok(!/hooks\/forge-[\w-]+\.(js|cjs|sh)/.test(entry.powershell), 'powershell body references no forge hook script file');
    });
  ...
  describe('writeCopilotHookConfig', () => {
    ...
    beforeEach(() => {
      tmpHookDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-copilot-hook-'));
    });
    ...
    test('writes hooks/forge-session.json under the config dir', () => {
...
describe('Copilot uninstall skill removal', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-uninstall-'));
  });
  ...
  test('identifies forge-* skill directories for removal', () => {
    // Create Copilot-like skills directory structure
    const skillsDir = path.join(tmpDir, 'skills');
    fs.mkdirSync(path.join(skillsDir, 'forge-foo'), { recursive: true });
    fs.writeFileSync(path.join(skillsDir, 'forge-foo', 'SKILL.md'), '# Foo');
    fs.mkdirSync(path.join(skillsDir, 'forge-bar'), { recursive: true });
    fs.writeFileSync(path.join(skillsDir, 'forge-bar', 'SKILL.md'), '# Bar');
    ...
    // Test the pattern: read skills, filter forge-* entries
    ...
    assert.deepStrictEqual(gsdSkills.sort(), ['forge-bar', 'forge-foo'], 'identifies forge-* skills');
...
describe('Copilot manifest and patches fixes', () => {
  ...
  test('writeManifest hashes skills for Copilot runtime', () => {
    // Create minimal forge-core dir (required by writeManifest)
    const gsdDir = path.join(tmpDir, 'forge-core', 'bin');
    fs.mkdirSync(gsdDir, { recursive: true });
    fs.writeFileSync(path.join(gsdDir, 'verify.cjs'), '// verify stub');

    // Create Copilot skills directory
    const skillDir = path.join(tmpDir, 'skills', 'forge-test');
  ...
  describe('reportLocalPatches', () => {
    ...
    test('reportLocalPatches shows /forge-update --reapply for Copilot', () => {
      // Create patches directory with metadata
      const patchesDir = path.join(tmpDir, 'forge-local-patches');
      fs.mkdirSync(patchesDir, { recursive: true });
      fs.writeFileSync(path.join(patchesDir, 'backup-meta.json'), JSON.stringify({
        from_version: '1.0',
        files: ['skills/forge-test/SKILL.md']
      }));
      ...
      // Asserts the consolidated form. /forge-reapply-patches was removed in
      // 1.39 (PR #2824) and folded into a flag on /forge-update — see #3010.
      // Negative assertion guards against regression to the dead command.
      assert.ok(output.includes('/forge-update --reapply'), 'uses consolidated /forge-update --reapply form for Copilot');
      assert.ok(!output.includes('/forge-reapply-patches'), 'does not reference removed /forge-reapply-patches command');
      assert.ok(!output.includes('/forge:reapply-patches'), 'does not use colon format');
    });

    test('reportLocalPatches shows /forge-update --reapply for Claude', () => {
      // Create patches directory with metadata
      const patchesDir = path.join(tmpDir, 'forge-local-patches');
      fs.mkdirSync(patchesDir, { recursive: true });
      fs.writeFileSync(path.join(patchesDir, 'backup-meta.json'), JSON.stringify({
        from_version: '1.0',
        files: ['forge-core/bin/verify.cjs']
      }));
      ...
      assert.ok(result.length > 0, 'returns patched files list');
      const output = logs.join('\n');
      assert.ok(output.includes('/forge-update --reapply'), 'uses consolidated /forge-update --reapply form for Claude');
      assert.ok(!output.includes('/forge-reapply-patches'), 'does not reference removed /forge-reapply-patches command');
      assert.ok(!output.includes('/forge:reapply-patches'), 'does not use colon format for Claude');
    });
  });
});
...
const EXPECTED_SKILLS = fs.readdirSync(path.join(__dirname, '..', 'commands', 'forge'))
  .filter(f => f.endsWith('.md')).length;
// Source-roster count (forge-*.md basenames) — shared helper.
...
describe('E2E: Copilot full install verification', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-e2e-'));
  ...
  test('installs expected number of skill directories', () => {
    ...
    const entries = fs.readdirSync(skillsDir, { withFileTypes: true });
    const gsdSkills = entries.filter(e => e.isDirectory() && e.name.startsWith('forge-'));
  ...
  test('installs expected number of agent files', () => {
    ...
    const gsdAgents = files.filter(f => f.startsWith('forge-') && f.endsWith('.agent.md'));
  ...
  test('installs all expected agent files', () => {
    const agentsDir = path.join(tmpDir, '.github', 'agents');
    const files = fs.readdirSync(agentsDir);
    const gsdAgents = files.filter(f => f.startsWith('forge-') && f.endsWith('.agent.md')).sort();
    const expected = [
      'forge-advisor-researcher.agent.md',
      'forge-ai-researcher.agent.md',
      'forge-assumptions-analyzer.agent.md',
      'forge-code-fixer.agent.md',
      'forge-code-reviewer.agent.md',
      'forge-codebase-mapper.agent.md',
      'forge-debug-session-manager.agent.md',
      'forge-debugger.agent.md',
      'forge-doc-classifier.agent.md',
      'forge-doc-synthesizer.agent.md',
      'forge-doc-verifier.agent.md',
      'forge-doc-writer.agent.md',
      'forge-domain-researcher.agent.md',
      'forge-eval-auditor.agent.md',
      'forge-eval-planner.agent.md',
      'forge-executor.agent.md',
      'forge-framework-selector.agent.md',
      'forge-integration-checker.agent.md',
      'forge-intel-updater.agent.md',
      'forge-mempalace-curator.agent.md',
      'forge-nyquist-auditor.agent.md',
      'forge-pattern-mapper.agent.md',
      'forge-phase-researcher.agent.md',
      'forge-plan-checker.agent.md',
      'forge-planner.agent.md',
      'forge-project-researcher.agent.md',
      'forge-research-synthesizer.agent.md',
      'forge-roadmapper.agent.md',
      'forge-security-auditor.agent.md',
      'forge-ui-auditor.agent.md',
      'forge-ui-checker.agent.md',
      'forge-ui-researcher.agent.md',
      'forge-user-profiler.agent.md',
  ...
  test('emits a Copilot lifecycle hook config (#786)', () => {
    const hookPath = path.join(tmpDir, '.github', 'hooks', 'forge-session.json');
    assert.ok(fs.existsSync(hookPath), '.github/hooks/forge-session.json should exist');
  ...
  test('creates manifest with correct structure', () => {
    const manifestPath = path.join(tmpDir, '.github', 'forge-file-manifest.json');
    assert.ok(fs.existsSync(manifestPath), 'forge-file-manifest.json should exist');
  ...
  test('manifest contains expected file categories', () => {
    ...
    const engineEntries = keys.filter(k => k.startsWith('forge-core/'));
    ...
    assert.ok(engineEntries.length > 0,
      'Should have forge-core/ engine manifest entries');
  });
  ...
  test('engine directory contains required subdirectories and files', () => {
    const engineDir = path.join(tmpDir, '.github', 'forge-core');
...
describe('E2E: Copilot uninstall verification', () => {
  ...
  test('removes engine directory', () => {
    ...
    assert.ok(!fs.existsSync(engineDir),
      'forge-core directory should not exist after uninstall');
  });
  ...
  test('removes the Copilot lifecycle hook config (#786)', () => {
    const hookPath = path.join(tmpDir, '.github', 'hooks', 'forge-session.json');
    assert.ok(!fs.existsSync(hookPath), 'forge-session.json should not exist after uninstall');
  });
  ...
  describe('preserves non-GSD content', () => {
    ...
    beforeEach(() => {
      td = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-e2e-preserve-'));
...
describe('E2E: Copilot global install (#786)', () => {
  ...
  beforeEach(() => {
    projectDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-e2e-gproj-'));
    configDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-e2e-gcfg-'));
  ...
  test('writes the lifecycle hook config under the global config dir', () => {
    const hookPath = path.join(configDir, 'hooks', 'forge-session.json');
...
describe('Claude uninstall preserves user-generated files (#1423)', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-preserve-'));
  ...
  test('preserves USER-PROFILE.md across uninstall', () => {
    const profilePath = path.join(tmpDir, '.claude', 'forge-core', 'USER-PROFILE.md');
  ...
  test('preserves dev-preferences.md across uninstall', () => {
    const prefsDir = path.join(tmpDir, '.claude', 'commands', 'forge');
  ...
  test('still removes GSD engine files during uninstall', () => {
    ...
    const binDir = path.join(tmpDir, '.claude', 'forge-core', 'bin');
  ...
  test('clean uninstall when no user files exist', () => {
    ...
    const gsdDir = path.join(tmpDir, '.claude', 'forge-core');
    const cmdDir = path.join(tmpDir, '.claude', 'commands', 'forge');
    // Directories should be fully removed when no user files to preserve
    assert.ok(!fs.existsSync(gsdDir), 'forge-core/ should not exist after clean uninstall');
    assert.ok(!fs.existsSync(cmdDir), 'commands/forge/ should not exist after clean uninstall');
  });
});
...
describe('#1182 convertClaudeAgentToCopilotAgent exported from runtime-artifact-conversion module', () => {
  const _gsdLibDirModule = path.join(__dirname, '..', 'forge-core', 'bin', 'lib');
  ...
  test('convertClaudeAgentToCopilotAgent via module produces correct output (local mode)', () => {
    ...
    assert.ok(result.includes('name: forge-executor'), 'name preserved');
    assert.ok(result.includes('color: yellow'), 'color preserved');
  });
