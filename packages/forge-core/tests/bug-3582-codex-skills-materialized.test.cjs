 ...
 * commands from `~/.codex/forge-core/workflows/*.md` or `agents/*.md`;
 ...
 * commands/forge/*.md, with Claude-flavored command frontmatter rewritten
...
/**
 * Walk commands/forge/**\/*.md and return the set of skill names the installer
 ...
 * `forge-<dir>-<file>` with the .md stripped.
 */
...
function runCodexInstallCaptured() {
  const codexHome = createTempDir('forge-3582-codex-');
  const logs = [];
  const warnings = [];
...
describe('bug-3582: Codex global install materializes the skill surface', { concurrency: false }, () => {
  ...
  test('writes the exact expected set of forge-*/SKILL.md skills (deepEqual on name set)', () => {
    ...
    // same-count `forge-*` directory cannot pass.
    const expected = [...expectedSkillNames()].sort();
    assert.deepStrictEqual(
      [...actualNames].sort(),
      expected,
      `installed Codex skills must exactly match commands/forge/**/*.md (one skill per command)`,
    );
  ...
  test('representative skills named in the issue report are present', () => {
    ...
    const representative = [
      'forge-map-codebase',     // the literal command from the bug report
      'forge-execute-phase',
      'forge-plan-phase',
      'forge-new-project',
      'forge-health',
  ...
  test('installed Codex skills do not ask agents to run bare forge-tools commands', () => {
    const skillsDir = path.join(installRun.codexHome, 'skills');
    const skillDirs = fs.readdirSync(skillsDir, { withFileTypes: true })
      .filter(e => e.isDirectory() && e.name.startsWith('forge-'))
      .map(e => e.name);
