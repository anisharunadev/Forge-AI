 ...
 *
 * Issue: #1592 (forge-ai/forge-core).
 */
...
const PLAN_PHASE = fs.readFileSync(
  path.join(REPO_ROOT, 'forge-core', 'workflows', 'plan-phase.md'),
  'utf8',
);
...
describe('#1592 — plan-phase host dispatches the drift plan:pre gate before planning', () => {
  ...
  test('§5.65 invokes the verify codebase-drift check', () => {
    ...
    assert.match(PLAN_PHASE, /forge_run verify codebase-drift/, '§5.65 must invoke `verify codebase-drift`');
  });
  ...
  test('the drift pre-check runs BEFORE the planner spawn (load-bearing ordering)', () => {
    ...
    const plannerIdx = PLAN_PHASE.indexOf('## 8. Spawn forge-planner Agent');
