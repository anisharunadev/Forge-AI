...
const WORKFLOW = path.join(__dirname, '..', 'forge-core', 'workflows', 'mvp-phase.md');
...
function parseMvpPhaseContract(content) {
  ...
  const planPhaseStepIndex = lowerLines.findIndex(line => line.includes('## 7. delegate to /forge plan-phase'));
...
describe('mvp-phase workflow', () => {
  ...
  test('delegates to /forge plan-phase after ROADMAP write', () => {
