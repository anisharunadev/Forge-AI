...
/**
 * Bug #410: finishInstall writes ~/.forge/defaults.json for non-Claude runtimes
...
// ~/.forge/ even if the guard is missing.
...
const FAKE_HOME = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-410-test-'));
...
const SETTINGS_PATH = path.join(FAKE_HOME, `forge-test-settings-${process.pid}.json`);
