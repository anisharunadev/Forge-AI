...
/**
 * SDK CLI integration tests for `forge-tools validate context`.
 *
...
describe('forge-tools validate context — CLI argument errors', () => {
...
describe('forge-tools validate context — JSON vs human rendering', () => {
  ...
  test('human mode (default) prints percent, state, and recommendation', () => {
    ...
    assert.match(r.output, /\/forge-thread/);
  });
  ...
  test('human mode omits the recommendation line for healthy state', () => {
    ...
    assert.doesNotMatch(r.output, /\/forge-thread/, 'healthy output must not nag the user');
  });
});

describe('forge-tools validate context — recommendation copy per state', () => {
