 ...
 *
 * Tests for migrating Claude Code from commands/forge/ to skills/forge-xxx/SKILL.md
...
function setupConfigDir(tmpDir, commandFiles) {
  const srcDir = path.join(tmpDir, 'commands', 'forge');
...
describe('convertClaudeCommandToClaudeSkill', () => {
  ...
  test('emits hyphen-form name (forge-<cmd>) from hyphen-form dir (#2808)', () => {
    ...
    // forge-next (hyphen, #2808) so Claude Code autocomplete shows canonical form.
    const result = convertClaudeCommandToClaudeSkill(input, 'forge-next');
    assert.ok(result.includes('name: forge-next'), 'frontmatter name uses hyphen form (#2808)');
  });

  test('preserves body content while normalizing forge: command references (#3583)', () => {
    // The body transformer now rewrites forge: references (colon → hyphen) but must
    // leave all other custom prose, tags, and structure intact.
    const body = '\n<objective>\nSee /forge:plan-phase and forge:review for details.\n</objective>\n\n<process>\nStep 1.\nStep 2.\n</process>\n';
    const input = [
      '---',
      'name: forge:test',
    ...
    assert.ok(result.includes('See /forge-plan-phase'), 'rewritten command reference visible');
    assert.ok(result.includes('<process>'), 'process tag preserved');
    assert.ok(result.includes('Step 1.'), 'step text preserved');

    // #3583: forge: references in body are normalized to hyphen form
    assert.ok(result.includes('/forge-plan-phase'), 'colon command ref rewritten to hyphen');
    assert.ok(result.includes('forge-review'), 'bare colon ref rewritten to hyphen');
    assert.ok(!result.includes('forge:plan-phase'), 'no colon form should survive in body');
    assert.ok(!result.includes('forge:review'), 'no colon form should survive in body');
  });
  ...
  test('preserves agent field', () => {
    ...
    const result = convertClaudeCommandToClaudeSkill(input, 'forge-plan-phase');
  ...
  test('handles content with no frontmatter', () => {
    ...
    const result = convertClaudeCommandToClaudeSkill(input, 'forge-plain');
...
describe('installRuntimeArtifacts (claude global) — skill layout', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-claude-skills-test-'));
  });
  ...
  test('creates correct directory structure skills/forge-xxx/SKILL.md', () => {
    ...
    assert.ok(
      fs.existsSync(path.join(skillsDir, 'forge-next', 'SKILL.md')),
      'skills/forge-next/SKILL.md exists'
    );
    assert.ok(
      fs.existsSync(path.join(skillsDir, 'forge-health', 'SKILL.md')),
      'skills/forge-health/SKILL.md exists'
    );
  });
  ...
  test('cleans up old skills before installing new ones (uninstall+install cycle)', () => {
    ...
    const staleDir = path.join(skillsDir, 'forge-old-command');
    fs.mkdirSync(staleDir, { recursive: true });
    fs.writeFileSync(path.join(staleDir, 'SKILL.md'), 'stale content');

    // Production sequence: uninstall wipes forge-* entries, install writes fresh ones
  ...
  test('does not remove non-GSD skills', () => {
    ...
    // Install (no pre-uninstall: uninstall only removes forge-* prefixed entries)
    installRuntimeArtifacts('claude', configDir, 'global', resolvedProfileFull);
    ...
    // Also survives uninstall (uninstall only removes forge-* prefixed entries)
    uninstallRuntimeArtifacts('claude', configDir, 'global');
    assert.ok(
      fs.existsSync(otherDir),
  ...
  // No subdirectories exist under commands/forge/ in production.
  ...
  test('no-ops on install when source directory has no .md files', () => {
    // Create an empty (but existing) commands/forge dir with no .md files.
    ...
    const emptySrc = path.join(tmpDir, 'empty-commands', 'forge');
    ...
    installRuntimeArtifacts('claude', configDir, 'global', resolvedProfileFull);
    const skillsDir = path.join(configDir, 'skills');
    // If skills dir was created it must contain no forge-* entries
    if (fs.existsSync(skillsDir)) {
      const gsdEntries = fs.readdirSync(skillsDir).filter(n => n.startsWith('forge-'));
      assert.strictEqual(gsdEntries.length, 0, 'no forge-* skills created when src has no .md files');
    }
  });
});
...
describe('installRuntimeArtifacts path replacement in Claude global skills (#1653)', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-claude-path-test-'));
  });
  ...
  test('replaces ~/.claude/ and $HOME/.claude/ paths with absolute configDir prefix on global install', () => {
    ...
    const { configDir } = setupConfigDir(tmpDir, {
      'manager.md': [
        ...
        '',
        '<execution_context>',
        '@~/.claude/forge-core/workflows/manager.md',
        '@$HOME/.claude/forge-core/references/ui-brand.md',
        '</execution_context>',
      ].join('\n'),
    });
    ...
    const content = fs.readFileSync(
      path.join(configDir, 'skills', 'forge-manager', 'SKILL.md'), 'utf8'
    );
    ...
    assert.ok(
      content.includes(expectedPrefix + 'forge-core/workflows/manager.md'),
      'tilde path rewritten to absolute configDir prefix'
    );
    assert.ok(
      content.includes(expectedPrefix + 'forge-core/references/ui-brand.md'),
  ...
  test('replaces $HOME/.claude/ paths with absolute configDir prefix on global install', () => {
    const { configDir } = setupConfigDir(tmpDir, {
      'debug.md': '---\nname: forge:debug\ndescription: Debug\n---\n\n@$HOME/.claude/forge-core/workflows/debug.md',
    });

    installRuntimeArtifacts('claude', configDir, 'global', resolvedProfileFull);

    const content = fs.readFileSync(
      path.join(configDir, 'skills', 'forge-debug', 'SKILL.md'), 'utf8'
    );
    assert.ok(!content.includes('$HOME/.claude/'), 'no $HOME/.claude/ paths remain');
    const expectedPrefix = path.resolve(configDir).replace(/\\/g, '/') + '/';
    assert.ok(
      content.includes(expectedPrefix + 'forge-core/workflows/debug.md'),
  ...
  test('global install rewrites ~/.claude/ paths to absolute configDir form', () => {
    ...
    const { configDir } = setupConfigDir(tmpDir, {
      'next.md': '---\nname: forge:next\ndescription: Next\n---\n\n@~/.claude/forge-core/workflows/next.md',
    });

    installRuntimeArtifacts('claude', configDir, 'global', resolvedProfileFull);

    const content = fs.readFileSync(
      path.join(configDir, 'skills', 'forge-next', 'SKILL.md'), 'utf8'
    );
    const expectedPrefix = path.resolve(configDir).replace(/\\/g, '/') + '/';
    assert.ok(
      content.includes(expectedPrefix + 'forge-core/workflows/next.md'),
...
describe('Legacy commands/forge/ cleanup', () => {
  let tmpDir;

  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-legacy-test-'));
  });
  ...
  test('install removes legacy commands/forge/ directory when present', () => {
    const { configDir } = setupConfigDir(tmpDir, {
      'next.md': '---\nname: forge:next\ndescription: Advance\n---\n\nBody.',
    });

    // Create a mock legacy commands/forge/ directory inside configDir
    ...
    // commands/forge/ for claude runtime (it's the legacy location for global).
    installRuntimeArtifacts('claude', configDir, 'global', resolvedProfileFull);

    assert.ok(!fs.existsSync(legacyDir), 'legacy commands/forge/ removed');
    assert.ok(
      fs.existsSync(path.join(configDir, 'skills', 'forge-next', 'SKILL.md')),
      'new skill installed'
    );
  });
});
...
describe('writeManifest tracks skills/ for Claude', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-manifest-test-'));
  });
  ...
  test('manifest includes skills/forge-xxx/SKILL.md entries for Claude runtime', () => {
    // Create skills directory structure (as install would)
    const skillsDir = path.join(tmpDir, 'skills');
    const skillDir = path.join(skillsDir, 'forge-next');
    fs.mkdirSync(skillDir, { recursive: true });
    fs.writeFileSync(path.join(skillDir, 'SKILL.md'), 'skill content');

    // Create forge-core directory (required by writeManifest)
    ...
    const manifest = JSON.parse(
      fs.readFileSync(path.join(tmpDir, 'forge-file-manifest.json'), 'utf8')
    );
    ...
    assert.ok(
      skillEntries.some(k => k === 'skills/forge-next/SKILL.md'),
      'manifest has skills/forge-next/SKILL.md'
    );

    // Should NOT have commands/forge/ entries
    const cmdEntries = Object.keys(manifest.files).filter(k =>
      k.startsWith('commands/forge/')
    );
    assert.strictEqual(cmdEntries.length, 0, 'manifest has no commands/forge/ entries');
  });
});
