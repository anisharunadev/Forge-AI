 ...
 * 2026-06-02-rename-get-shit-done-to-forge-core  // forge-allow-legacy-name
 *
 * Verifies plan() logic for:
 * 1. Legacy dir absent -> empty plan (idempotency)
 * 2. forge-core absent + legacy has managed files -> remove-managed actions emitted
...
// Load compiled module (build:lib compiles src/*.cts -> forge-core/bin/lib/*.cjs)
const migration = require('../forge-core/bin/lib/installer-migrations/003-rename-get-shit-done-to-forge-core.cjs');
...
function createTempDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'forge-migration-003-test-'));
}
...
describe('plan() — legacy dir absent', () => {
  test('returns empty array when legacy dir does not exist', () => {
    ...
    try {
      // Create forge-core/ but NOT legacy dir
      fs.mkdirSync(path.join(configDir, 'forge-core'), { recursive: true });
...
// ---------------------------------------------------------------------------
// Case 2: forge-core absent + legacy has managed files -> remove-managed emitted
// (first-upgrade scenario: migrations run BEFORE new runtime is materialized)
// ---------------------------------------------------------------------------

describe('plan() — forge-core not yet present (pre-materialization)', () => {
  test('emits remove-managed actions even when forge-core/ does not yet exist', () => {
    const configDir = createTempDir();
    try {
      // Real first-upgrade: legacy dir exists, managed files are there,
      // but forge-core/ has NOT been materialized yet.
      const fileContent = 'managed workflow\n';
      writeFile(configDir, 'get-shit-done/workflows/plan.md', fileContent); // forge-allow-legacy-name
      writeManifest(configDir, {
        'get-shit-done/workflows/plan.md': sha256(fileContent), // forge-allow-legacy-name
      });
      // Deliberately do NOT create forge-core/

      const actions = migration.plan(makePlanCtx(configDir));
      assert.equal(actions.length, 1);
      assert.equal(actions[0].type, 'remove-managed');
      assert.equal(actions[0].relPath, 'get-shit-done/workflows/plan.md'); // forge-allow-legacy-name
    } finally {
      cleanup(configDir);
    }
  });
...
describe('plan() — managed-pristine files', () => {
  test('returns remove-managed action for each managed-pristine legacy file', () => {
    ...
    try {
      ...
      writeFile(configDir, 'get-shit-done/workflows/plan.md', fileA); // forge-allow-legacy-name
      writeFile(configDir, 'get-shit-done/skills/forge-foo/SKILL.md', fileB); // forge-allow-legacy-name
      // Do NOT pre-create forge-core/ — reflects real pre-materialization timing
      writeManifest(configDir, {
        'get-shit-done/workflows/plan.md': sha256(fileA), // forge-allow-legacy-name
        'get-shit-done/skills/forge-foo/SKILL.md': sha256(fileB), // forge-allow-legacy-name
      });
      ...
      for (const action of actions) {
        ...
        assert.ok(action.relPath.startsWith('get-shit-done/'), `relPath should start with legacy prefix, got: ${action.relPath}`); // forge-allow-legacy-name
...
describe('plan() — managed-modified files', () => {
  test('returns backup-and-remove action for managed-modified legacy file', () => {
    ...
    try {
      ...
      writeFile(configDir, 'get-shit-done/workflows/plan.md', modifiedContent); // forge-allow-legacy-name
      // Do NOT pre-create forge-core/ — reflects real pre-materialization timing
      writeManifest(configDir, {
        // manifest records the original hash, but file has been modified
        'get-shit-done/workflows/plan.md': sha256(originalContent), // forge-allow-legacy-name
      });
...
describe('plan() — unknown user file', () => {
  test('returns baseline-preserve-user for unknown file (not in manifest)', () => {
    ...
    try {
      ...
      assert.equal(actions[0].relPath, 'get-shit-done/my-custom-file.md'); // forge-allow-legacy-name
    } finally {
      cleanup(configDir);
    }
  });
  ...
  test('unknown user file under legacy dir is NOT removed by migration', () => {
    ...
    try {
      ...
      writeFile(configDir, 'get-shit-done/my-custom-file.md', userContent); // forge-allow-legacy-name
      // Do NOT pre-create forge-core/ — reflects real pre-materialization timing
      writeManifest(configDir, {});

      const { planInstallerMigrations, applyInstallerMigrationPlan } = require('../forge-core/bin/lib/installer-migrations.cjs');
      ...
      const stillThere = fs.readFileSync(path.join(configDir, 'get-shit-done/my-custom-file.md'), 'utf8'); // forge-allow-legacy-name
...
describe('plan() — legacy root is a symlink', () => {
  test('returns empty array when legacy root is a symlink (symlink safety)', () => {
    ...
    try {
      ...
      const legacyLink = path.join(configDir, 'get-shit-done'); // forge-allow-legacy-name
...
describe('plan() — symlinked entry inside legacy dir is skipped', () => {
  test('symlinked file inside legacy dir is not included in plan actions', () => {
    ...
    try {
      ...
      const legacyDir = path.join(configDir, 'get-shit-done'); // forge-allow-legacy-name
      fs.mkdirSync(legacyDir, { recursive: true });

      // Add a real managed file.
      const realContent = 'real managed file\n';
      writeFile(configDir, 'get-shit-done/real.md', realContent); // forge-allow-legacy-name
      ...
      writeManifest(configDir, {
        'get-shit-done/real.md': sha256(realContent), // forge-allow-legacy-name
      ...
      assert.equal(actions[0].relPath, 'get-shit-done/real.md'); // forge-allow-legacy-name
...
describe('plan() — mixed classifications', () => {
  test('handles pristine, modified, and unknown files together', () => {
    ...
    try {
      ...
      writeFile(configDir, 'get-shit-done/pristine.md', pristineContent); // forge-allow-legacy-name
      writeFile(configDir, 'get-shit-done/modified.md', modifiedContent); // forge-allow-legacy-name
      writeFile(configDir, 'get-shit-done/user.md', userContent); // forge-allow-legacy-name
      // Do NOT pre-create forge-core/ — reflects real pre-materialization timing
      writeManifest(configDir, {
        'get-shit-done/pristine.md': sha256(pristineContent), // forge-allow-legacy-name
        'get-shit-done/modified.md': sha256(originalContent), // forge-allow-legacy-name
      });
      ...
      assert.equal(byPath['get-shit-done/pristine.md'], 'remove-managed'); // forge-allow-legacy-name
      assert.equal(byPath['get-shit-done/modified.md'], 'backup-and-remove'); // forge-allow-legacy-name
      assert.equal(byPath['get-shit-done/user.md'], 'baseline-preserve-user'); // forge-allow-legacy-name
    } finally {
      cleanup(configDir);
    }
  });
