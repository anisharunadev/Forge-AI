 ...
 *
 * Defects covered:
 *   #3664 Defect #1 — stale skills/forge/forge-<stem>/ dirs on Hermes upgrade
...
//
// #947 REVERSES #3664: the canonical layout is now skills/forge/forge-<stem>/ again.
// The migration now removes bare-stem dirs (from #3664: prefix='') and writes
// the forge-prefixed layout. Pre-existing forge-prefixed dirs (the "intermediate"
// layout from before #3664) are now the CANONICAL dirs and are kept / updated.

describe('Defect #1 regression (#3664 reversed by #947): bare-stem dirs removed, forge- prefix written', () => {
  test('installRuntimeArtifacts removes bare-stem skills/forge/<stem>/ dirs and writes forge- prefixed layout', (t) => {
    const configDir = createTempDir('forge-hermes-reg1-');
    ...
    // Pre-create #3664-era bare-stem Hermes layout (no forge- prefix, now stale).
    ...
    // Sibling non-gsd dir inside skills/forge/ must survive
    ...
    assert.ok(!fs.existsSync(path.join(nestedGsdDir, 'help')),
      'skills/forge/help/ (bare-stem from #3664) must be removed (#947)');
    assert.ok(!fs.existsSync(path.join(nestedGsdDir, 'quick')),
      'skills/forge/quick/ (bare-stem from #3664) must be removed (#947)');
    // Canonical forge- prefixed layout must be written
    assert.ok(fs.existsSync(path.join(nestedGsdDir, 'forge-help', 'SKILL.md')),
      'skills/forge/forge-help/SKILL.md must exist after install (#947 canonical layout)');
...
describe('Defect #2 regression (Qwen, #3664): --qwen --profile=core writes skills/forge-*/, not commands/forge/', () => {
  test('spawn --qwen --global --profile=core: skills/forge-*/ written, no commands/forge/', (t) => {
    const root = createTempDir('forge-qwen-reg2-');
    ...
    assert.ok(skillDirs.length >= 1, 'at least one forge-* skill dir must exist');
    assert.ok(
      skillDirs.some(e => fs.existsSync(path.join(qwenSkillsDir, e.name, 'SKILL.md'))),
      'at least one skills/forge-*/SKILL.md must exist'
    );

    const commandsGsd = path.join(root, 'commands', 'forge');
    if (fs.existsSync(commandsGsd)) {
      const mdFiles = fs.readdirSync(commandsGsd).filter(f => f.endsWith('.md'));
      assert.strictEqual(mdFiles.length, 0, `commands/forge/ must not contain .md files (Defect #2). Found: ${mdFiles.join(', ')}`);
    }
  });
});

describe('Defect #2 regression (Hermes, #3664): --hermes --profile=core writes skills/forge/, not commands/forge/', () => {
  test('spawn --hermes --global --profile=core: skills/forge/ written, no commands/forge/', (t) => {
    const root = createTempDir('forge-hermes-reg2-');
    t.after(() => cleanup(root));
    ...
    const hermesSkillsGsd = path.join(root, 'skills', 'forge');
...
describe('M1 (#2973, #947): --hermes --global --profile=core migrates dev-preferences → skills/forge/forge-dev-preferences/SKILL.md', () => {
  test('dev-preferences migrated to nested Hermes location, legacy source removed', (t) => {
    const root = createTempDir('forge-hermes-m1-');
    t.after(() => cleanup(root));

    const legacyDir = path.join(root, 'commands', 'forge');
    ...
    // #947: Hermes uses prefix='forge-' so dev-preferences lands at forge-dev-preferences/ (not dev-preferences/)
    const skillFile = path.join(root, 'skills', 'forge', 'forge-dev-preferences', 'SKILL.md');
    assert.ok(fs.existsSync(skillFile),
      'skills/forge/forge-dev-preferences/SKILL.md must exist (M1+#947: forge- prefix, nested)');
...
describe('M2 (#2973): --qwen --global --profile=core migrates dev-preferences → skills/forge-dev-preferences/SKILL.md', () => {
  test('dev-preferences migrated to flat Qwen location, legacy source removed', (t) => {
    const root = createTempDir('forge-qwen-m2-');
    t.after(() => cleanup(root));
    ...
    const skillFile = path.join(root, 'skills', 'forge-dev-preferences', 'SKILL.md');
    assert.ok(fs.existsSync(skillFile),
      'skills/forge-dev-preferences/SKILL.md must exist (M2: flat Qwen layout)');
...
describe('M3 (#2973): --claude --global --profile=core migrates dev-preferences → skills/forge-dev-preferences/SKILL.md', () => {
  test('dev-preferences migrated to flat Claude-global location, legacy source removed', (t) => {
    const root = createTempDir('forge-claude-m3-');
    t.after(() => cleanup(root));
    ...
    const skillFile = path.join(root, 'skills', 'forge-dev-preferences', 'SKILL.md');
    assert.ok(fs.existsSync(skillFile),
      'skills/forge-dev-preferences/SKILL.md must exist (M3)');
...
describe('U1 (#2973): uninstallRuntimeArtifacts qwen migrates dev-preferences → skills/forge-dev-preferences/SKILL.md', () => {
  test('commands/forge/ removed, dev-preferences migrated to skills skill', (t) => {
    const configDir = createTempDir('forge-qwen-uninstall-u1-');
    t.after(() => cleanup(configDir));
    ...
    const skillFile = path.join(configDir, 'skills', 'forge-dev-preferences', 'SKILL.md');
    assert.ok(fs.existsSync(skillFile), 'skills/forge-dev-preferences/SKILL.md must exist (U1)');
    assert.strictEqual(fs.readFileSync(skillFile, 'utf8'), '# my qwen prefs\n');
  });
});
...
describe('U2 (#2973): uninstallRuntimeArtifacts claude/global migrates dev-preferences → skills/forge-dev-preferences/SKILL.md', () => {
  test('commands/forge/ removed, dev-preferences migrated to skills skill', (t) => {
    const configDir = createTempDir('forge-claude-uninstall-u2-');
    t.after(() => cleanup(configDir));
    ...
    const skillFile = path.join(configDir, 'skills', 'forge-dev-preferences', 'SKILL.md');
    assert.ok(fs.existsSync(skillFile), 'skills/forge-dev-preferences/SKILL.md must exist (U2)');
    assert.strictEqual(fs.readFileSync(skillFile, 'utf8'), '# my claude prefs\n');
  });
});
...
describe('U3 (#2973, #947): uninstallRuntimeArtifacts hermes migrates dev-preferences → skills/forge/forge-dev-preferences/SKILL.md', () => {
  test('commands/forge/ NOT recreated, dev-preferences at nested Hermes location with forge- prefix', (t) => {
    const configDir = createTempDir('forge-hermes-uninstall-u3-');
    t.after(() => cleanup(configDir));
    ...
    assert.ok(!fs.existsSync(path.join(legacyDir, 'dev-preferences.md')),
      'commands/forge/dev-preferences.md must not exist after hermes uninstall (U3)');

    // #947: Hermes uses prefix='forge-' so dev-preferences lands at forge-dev-preferences/ (not dev-preferences/)
    const skillFile = path.join(configDir, 'skills', 'forge', 'forge-dev-preferences', 'SKILL.md');
    assert.ok(fs.existsSync(skillFile),
      'skills/forge/forge-dev-preferences/SKILL.md must exist at HERMES nested location (U3+#947)');
    assert.strictEqual(fs.readFileSync(skillFile, 'utf8'), '# my hermes prefs\n');
  });
});
...
describe('mergeClaudePermissions (#768): fresh settings object', () => {
  ...
  test('includes Bash(npx forge-core *) in allow', () => {
    const settings = {};
    mergeClaudePermissions(settings);
    assert.ok(settings.permissions.allow.includes('Bash(npx forge-core *)'),
      'permissions.allow must contain Bash(npx forge-core *)');
  });
...
describe('mergeClaudePermissions (#768): end-to-end install writes permissions to settings.json', () => {
  test('--claude --global install writes GSD allow/deny entries to settings.json', (t) => {
    const root = createTempDir('forge-claude-perm-install-');
    t.after(() => cleanup(root));
    ...
    assert.ok(settings.permissions.allow.includes('Bash(npx forge-core *)'),
      'settings.json permissions.allow must include Bash(npx forge-core *)');
    assert.ok(settings.permissions.allow.includes('Read(.planning/*)'),
  ...
  test('non-claude runtime (gemini) does NOT write GSD allow/deny permissions to settings.json', (t) => {
    const root = createTempDir('forge-gemini-perm-install-');
    t.after(() => cleanup(root));
    ...
    if (fs.existsSync(settingsPath)) {
      ...
      assert.ok(!allow.includes('Bash(npx forge-core *)'),
        'Gemini settings.json must NOT include Bash(npx forge-core *) in permissions.allow');
    }
  });

  test('--claude --global reinstall is idempotent (no duplicate permission entries)', (t) => {
    const root = createTempDir('forge-claude-perm-idempotent-');
    t.after(() => cleanup(root));
  ...
  test('--claude --global uninstall removes GSD permission entries from settings.json', (t) => {
    const root = createTempDir('forge-claude-perm-uninstall-');
    t.after(() => cleanup(root));
    ...
    assert.ok((afterInstall.permissions?.allow ?? []).includes('Bash(npx forge-core *)'),
...
describe('#976 regression: installer does not duplicate managed hooks when registered in command+args form', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('forge-976-args-form-');
  ...
  test('does not add a second SessionStart entry when forge-check-update is already in args-form', () => {
    const targetDir = path.join(tmpDir, '.claude');
    fs.mkdirSync(targetDir, { recursive: true });

    // Pass 1: run install with no pre-existing settings to create the
    // forge-file-manifest.json that the installer migration uses to decide
    ...
    stubHooksIntoDir(targetDir, ['forge-check-update.js']);
    ...
    const hookPath = path.join(targetDir, 'hooks', 'forge-check-update.js');
    ...
    // Count all hook entries (at any nesting level) that reference forge-check-update.
    ...
    const matching = allEntries.filter(h =>
      (typeof h.command === 'string' && h.command.includes('forge-check-update')) ||
      (Array.isArray(h.args) && h.args.some(a => typeof a === 'string' && a.includes('forge-check-update')))
    );
    ...
    assert.strictEqual(
      ...
      1,
      [
        'Expected exactly 1 hook entry referencing forge-check-update after install,',
        `got ${matching.length}.`,
  ...
  test('rewriteLegacyManagedNodeHookCommands leaves args-form launcher entries unchanged', () => {
    ...
    const launcherCommand = '/usr/local/bin/node-launcher';
    const hookPath = '/Users/user/.claude/hooks/forge-check-update.js';
    const settings = {
      hooks: {
        SessionStart: [
          {
...
describe('#1004 regression: installer does not duplicate managed hooks when registered in http form', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('forge-1004-http-form-');
    previousCwd = process.cwd();
    process.chdir(tmpDir);
  ...
  test('does not add a second SessionStart entry when forge-check-update is already in http form', () => {
    const targetDir = path.join(tmpDir, '.claude');
    fs.mkdirSync(targetDir, { recursive: true });
    ...
    const hookUrl = 'http://127.0.0.1:18923/hooks/forge-check-update';
    const preExistingSettings = {
      hooks: {
        SessionStart: [
          {
    ...
    const matching = allEntries.filter(h =>
      (typeof h.command === 'string' && h.command.includes('forge-check-update')) ||
      (Array.isArray(h.args) && h.args.some(a => typeof a === 'string' && a.includes('forge-check-update'))) ||
      (typeof h.url === 'string' && h.url.includes('forge-check-update'))
    );
