/**
 * Regression tests for bug #260 — forge-worktree-path-guard.js
 *
 ...
 * worktree. The prose guard in forge-executor.md step 0b is skipped under load,
 ...
 *   7. install.js has an fs.existsSync guard for forge-worktree-path-guard.js
 */
...
const HOOK_PATH = path.join(__dirname, '..', 'hooks', 'forge-worktree-path-guard.js');
...
function makeMainRepo() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-260-main-'));
  git(dir, ['init', '-q']);
...
function makeWorktree(mainRepo, branchName) {
  ...
  const wtDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-260-wt-'));
...
describe('bug #260: forge-worktree-path-guard.js', () => {
  ...
  describe('sibling path is blocked', () => {
    test('path that shares prefix with worktree root but is a sibling exits 2', () => {
      ...
      const base = realp(fs.mkdtempSync(path.join(os.tmpdir(), 'forge-260-sib-base-')));
...
describe('#1342 — GSD-activity gate + fail-open for no-repo targets', () => {
  ...
  test('(1) non-agent linked worktree: Write to a path outside all git repos exits 0 (no block)', () => {
    ...
    const plansDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-1342-plans-'));
    try {
  ...
  test('(5) GSD-managed worktree: target outside all git repos exits 0 (fail open, not #260 vector)', () => {
    ...
    const externalDir = realp(fs.mkdtempSync(path.join(os.tmpdir(), 'forge-1342-ext-')));
    try {
...
describe('install.js guard for forge-worktree-path-guard.js', () => {
  let src;
  ...
  test('install.js checks fs.existsSync before registering forge-worktree-path-guard.js', () => {
    ...
    assert.ok(
      ...
      'forge-worktree-path-guard.js in settings.json. Without this guard, the hook ' +
      'is registered even when the .js file was never copied (root cause of #1754).'
    );
  });

  test('install.js emits a skip warning when forge-worktree-path-guard.js is missing', () => {
    const anchorIdx = src.indexOf('hasWorktreePathGuardHook');
    assert.ok(anchorIdx !== -1, 'hasWorktreePathGuardHook not found in install.js');
    ...
    assert.ok(
      block.includes('Skipped') && block.includes('forge-worktree-path-guard'),
      'install.js must emit a skip warning mentioning forge-worktree-path-guard when the file is not found'
    );
  });
});
