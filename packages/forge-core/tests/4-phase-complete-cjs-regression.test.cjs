...
/**
 * Regression test for issue #4 (forge-ai/forge-core):
...
const phaseModule = require('../forge-core/bin/lib/phase.cjs');
 ...
 */
function createFixture(prefix = 'forge-4-regression-') {
...
function capturePhaseComplete(cwd, phaseNum) {
  // We invoke forge-tools directly for the full CJS path, but with GSD_DISABLE_SDK_BRIDGE=1
...
function create4ColFixture(existingDate, alreadyComplete = true) {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-1161-4col-'));
...
function create5ColFixture(existingDate, alreadyComplete = true) {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-1161-5col-'));
  const planDir = path.join(tmpDir, '.planning');
  const phasesDir = path.join(planDir, 'phases');
  fs.mkdirSync(phasesDir, { recursive: true });
...
function createVerificationFixture(verFmStatus) {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-1159-verif-'));
  const planDir = path.join(tmpDir, '.planning');
  const phasesDir = path.join(planDir, 'phases');
  const phase01Dir = path.join(phasesDir, '01-foundation');
  fs.mkdirSync(phase01Dir, { recursive: true });
...
function createDeferredReqFixture({ includeMissingActive = false } = {}) {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-1159-deferred-'));
  const planDir = path.join(tmpDir, '.planning');
  const phasesDir = path.join(planDir, 'phases');
  const phase01Dir = path.join(phasesDir, '01-foundation');
  fs.mkdirSync(phase01Dir, { recursive: true });
...
describe('issue #1159 (Defect B): deferred/future requirement IDs must not trigger traceability warning', () => {
  ...
  test(
    ...
    () => {
      ...
      tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-1159-subhead-'));
      const planDir = path.join(tmpDir, '.planning');
      const phasesDir = path.join(planDir, 'phases');
      const phase01Dir = path.join(phasesDir, '01-foundation');
      fs.mkdirSync(phase01Dir, { recursive: true });
