...
const SPEC_PHASE_PATH = path.join(__dirname, '..', 'forge-core', 'workflows', 'spec-phase.md');
...
// PROB-13: ADR-550 D6 canon-referral breadcrumb to /forge:secure-phase.
test('PROB-13: Step 5.6 emits the canon-referral breadcrumb to /forge:secure-phase', () => {
  const block = extractStep56Block(readSpecPhase());
  assert.match(
    block,
    /secure-phase|canon[^.]*security|OWASP|GDPR/i,
    'Step 5.6 must emit a canon-referral breadcrumb (e.g. owned by /forge:secure-phase) for canon-security items (ADR-550 D6, PROB-13)'
  );
});
