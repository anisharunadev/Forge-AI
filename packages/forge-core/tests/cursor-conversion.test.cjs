...
`;
    ...
    const result = convertClaudeCommandToCursorSkill(input, 'forge-quick');
...
`;
    ...
    const result = convertClaudeCommandToCursorSkill(input, 'forge-plan-phase');

    assert.ok(result.includes('/forge-execute-phase 17'), 'slash command remains slash-prefixed');
    assert.ok(result.includes('/forge-help'), 'existing slash command is preserved');
    assert.ok(result.includes('forge-progress'), 'non-slash forge: references still normalize');
    assert.ok(!result.includes('/forge:execute-phase'), 'legacy colon command form is removed');
  });
});
...
`;
    ...
    assert.ok(nameMatch, 'frontmatter contains name field');
    assert.strictEqual(nameMatch[1], 'forge-planner', 'agent name is plain scalar');
    assert.ok(!result.includes('name: "forge-planner"'), 'quoted agent name is not emitted');
  });
});
...
`;
  ...
  test('normalizes forge: colon slash commands to forge- hyphen form', () => {
    const input = `---
name: plan-phase
description: Plan a phase
---

Next step: /forge:execute-phase 17
`;

    const result = convertClaudeCommandToCursorCommand(input);
    assert.ok(result.includes('/forge-execute-phase 17'), 'colon form should become hyphen form');
    assert.ok(!result.includes('/forge:execute-phase'), 'colon form should be removed');
  });
...
describe('_applyRuntimeRewrites(cursor) — bare-form ~/.claude regression (#1356)', () => {
  ...
  const COMPOUND_INPUT = [
    'Config dir: ~/.claude',
    'Also: $HOME/.claude',
    'Slash form: ~/.claude/forge-core/foo.md',
  ...
  test('trailing-slash ~/.claude/forge-core/foo.md is rewritten exactly once (no doubling)', () => {
    const input = '~/.claude/forge-core/foo.md';
    const result = _applyRuntimeRewrites(input, 'cursor', CURSOR_PATH_PREFIX);
    assert.ok(result.includes('~/.cursor/forge-core/foo.md'), `slash form must be rewritten; got: ${result}`);
