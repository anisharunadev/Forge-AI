...
describe('plan-phase §5.6 UI Design Contract Gate', () => {
  ...
  test('§5.6 invokes forge_run check ui-plan-gate UNCONDITIONALLY (not inside a gate loop)', () => {
    const section = extractSection56(workflowPath);
    ...
    assert.ok(
      ...
      '§5.6 must run `forge_run check ui-plan-gate` unconditionally (not gated inside a kind==gate loop)'
    );
  ...
  test('§5.6 establishes GATE= before any branch-5 step dispatch (check precedes step)', () => {
    ...
    const gateIdx = section.indexOf('GATE=$(forge_run check ui-plan-gate');
    const branch5Idx = section.indexOf('Branch 5');
    assert.ok(gateIdx !== -1, '§5.6 must contain `GATE=$(forge_run check ui-plan-gate` assignment');
  ...
  test('§5.6 invokes forge_run check with gate check.query (generic dispatch — ui.plan-gate or ui-plan-gate)', () => {
    const section = extractSection56(workflowPath);
    ...
    assert.ok(
      ...
      '§5.6 must invoke `forge_run check` with the gate\'s check.query (generic dispatch — not hardcoded for a specific check)'
    );
  });
  ...
  test('Branch 5: AUTO_CHAIN=true → step hooks dispatched via forge-${ref.skill}', () => {
    const section = extractSection56(workflowPath);
    assert.ok(
      section.includes('AUTO_CHAIN') && section.includes('forge-${ref.skill}'),
      '§5.6 Branch 5 must dispatch step hooks via forge-${ref.skill} in pipeline mode'
    );
  });
  ...
  test('Branch 6: recommendation block contains EXACT warning heading + /forge:ui-phase + --skip-ui', () => {
    const section = extractSection56(workflowPath);
    ...
    assert.ok(
      section.includes('/forge:ui-phase'),
      '§5.6 must include /forge:ui-phase recommendation in the block'
    );
    assert.ok(
  ...
  test('does NOT contain hard-blocking exit redirect to /forge-ui-phase (old pattern)', () => {
    const content = fs.readFileSync(workflowPath, 'utf8');
