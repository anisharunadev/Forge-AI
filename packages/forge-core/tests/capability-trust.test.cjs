...
test('reserved namespace: forge-, forge-core-, anthropic- are reserved (case-insensitive)', () => {
  assert.strictEqual(trust.checkReservedNamespace('forge-foo').reserved, true);
  assert.strictEqual(trust.checkReservedNamespace('forge-core-foo').reserved, true);
...
test('install trust: reserved namespace blocks (and suppresses consent)', () => {
  const v = trust.evaluateInstallTrust({
    parsed: gitSpec,
    manifest: { id: 'forge-evil', version: '1.0.0', hooks: [{ event: 'E', script: 'h.js' }] },
    hostVersion: '1.6.0',
  });
  assert.strictEqual(v.allowed, false);
  assert.strictEqual(v.requiresConsent, false);
...
test('install trust: engines mismatch blocks with a compatVersions hint when available', () => {
  const v = trust.evaluateInstallTrust({
    parsed: gitSpec,
    manifest: { id: 'cap', version: '3.0.0', engines: { forge: '>=2.0.0' }, compatVersions: { '1.4.0': '>=1.5.0 <2.0.0' } },
    hostVersion: '1.6.0',
  });
  assert.strictEqual(v.allowed, false);
...
test('install trust: multiple gates accumulate multiple block reasons', () => {
  const v = trust.evaluateInstallTrust({
    parsed: evilSpec,
    manifest: { id: 'forge-core-x', version: '3.0.0', engines: { forge: '>=9.0.0' } },
    strictKnownRegistries: ['github.com'],
    hostVersion: '1.6.0',
  });
  assert.strictEqual(v.allowed, false);
