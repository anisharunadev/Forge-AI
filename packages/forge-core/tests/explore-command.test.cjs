...
describe('explore command', () => {
  test('command file exists', () => {
    const p = path.join(__dirname, '..', 'commands', 'forge', 'explore.md');
    assert.ok(fs.existsSync(p), 'commands/forge/explore.md should exist');
  });

  test('command file has required frontmatter', () => {
    const p = path.join(__dirname, '..', 'commands', 'forge', 'explore.md');
    const content = fs.readFileSync(p, 'utf-8');
    assert.ok(content.includes('name: forge:explore'), 'Command must have name frontmatter');
  ...
  test('workflow file exists', () => {
    const p = path.join(__dirname, '..', 'forge-core', 'workflows', 'explore.md');
