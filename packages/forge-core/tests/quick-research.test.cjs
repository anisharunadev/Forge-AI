 ...
 *
 * Validates the --research flag for /forge-quick:
 ...
 * - Workflow spawns forge-phase-researcher for research
 */
...
describe('quick command: --research in frontmatter', () => {
  ...
  test('quick.md exists', () => {
    assert.ok(fs.existsSync(commandPath), 'commands/forge/quick.md should exist');
  });
...
describe('quick workflow: research step', () => {
  ...
  test('research step spawns forge-phase-researcher', () => {
    content = fs.readFileSync(workflowPath, 'utf-8');
    ...
    assert.ok(
      researchSection.includes('subagent_type="forge-phase-researcher"'),
      'research step should spawn forge-phase-researcher agent'
    );
  });
