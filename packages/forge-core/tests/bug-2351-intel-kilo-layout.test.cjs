 ...
 *
 * forge-intel-updater used hardcoded canonical paths (`agents/*.md`,
 * `commands/forge/*.md`, `hooks/*.js`, etc.) that assumed the standard
 * `.claude/` runtime layout. Under a `.kilo` install, the runtime root is
 * `.kilo/`, and the command directory is `command/` (not `commands/forge/`).
...
const AGENT_PATH = path.join(__dirname, '..', 'agents', 'forge-intel-updater.md');
...
describe('bug #2351: intel updater kilo layout support', () => {
  ...
  test('scope section includes layout detection step', () => {
    ...
    assert.ok(
      hasDetection,
      'forge-intel-updater.md must instruct the agent to detect the runtime layout ' +
  ...
  test('scope section maps .kilo/command path (not commands/gsd)', () => {
    ...
    assert.ok(
      ...
      'scope section must include .kilo/command path — kilo uses "command/" not "commands/forge/"'
    );
  });
