 ...
 *   (B) the bash surface — `forge-tools query project-instruction-file --runtime <r>`,
 *       consumed by forge-core/workflows/new-project.md to set $INSTRUCTION_FILE.
 *
...
const GSD_TOOLS_PATH = path.join(ROOT, 'forge-core', 'bin', 'forge-tools.cjs');
...
describe('bug #1529: getProjectInstructionFile ↔ forge-tools query parity', () => {
  for (const runtime of RUNTIMES) {
    ...
    test(`Node function and CLI query agree for runtime=${label}`, () => {
      ...
      assert.strictEqual(
        fromQuery,
        fromFunction,
        `forge-tools query project-instruction-file --runtime ${label} returned "${fromQuery}" but getProjectInstructionFile() returned "${fromFunction}"; the two surfaces drifted.`,
      );
    });
  }
});
...
describe('bug #1529: new-project.md workflow uses the shared policy query', () => {
  ...
  // shared `forge_run query project-instruction-file` query rather than a hardcoded
  // codex-only `if/else` branch; there is no typed IR for "this bash block calls a
  // specific forge-tools query instead of a hardcoded mapping".
  ...
  test('workflow derives INSTRUCTION_FILE from the shared query', () => {
    assert.ok(
      /INSTRUCTION_FILE=\$\(forge_run query project-instruction-file --runtime "\$RUNTIME"\)/.test(workflow),
      'new-project.md must derive INSTRUCTION_FILE via `forge_run query project-instruction-file --runtime "$RUNTIME"` (the shared policy adapter)',
    );
  });
