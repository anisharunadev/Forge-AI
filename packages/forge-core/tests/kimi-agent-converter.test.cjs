...
Read ~/.claude/forge-core when source-package context is needed.`;
...
describe('buildKimiAgentArtifacts', () => {
  test('builds a root Kimi agent YAML contract with explicit subagent paths', () => {
    const result = buildKimiAgentArtifacts({
      ...
      subagents: [
        { path: 'agents/forge-executor.md', content: EXECUTOR_AGENT },
      ],
      requestedSubagents: ['forge-executor', 'forge-missing'],
    });
    ...
    assert.ok(result.root.yaml.includes('forge-executor:'), 'known GSD subagent key is canonical');
    assert.ok(
      result.root.yaml.includes('path: ./subagents/forge-executor.yaml'),
      'known GSD subagent path is relative to root YAML'
    );
    assert.ok(!result.root.yaml.includes('forge-missing'), 'unknown requested subagent is excluded');
  });
  ...
  test('emits separate frontmatter-free Markdown prompts for root and subagents', () => {
    const result = buildKimiAgentArtifacts({
      ...
      requestedSubagents: ['forge-executor'],
    });

    const executor = result.subagents.find((artifact) => artifact.name === 'forge-executor');
    assert.ok(executor, 'executor subagent artifact exists');

    assert.equal(executor.yamlPath, 'agents/subagents/forge-executor.yaml');
    assert.equal(executor.promptPath, 'agents/subagents/forge-executor.md');
    assert.ok(executor.yaml.includes('system_prompt_path: ./forge-executor.md'));
    ...
    assert.ok(!result.root.prompt.includes('~/.claude/forge-core'), 'Claude-specific path is neutralized');
  });
  ...
  test('diagnoses unknown subagents and unsupported inputs instead of emitting invalid names', () => {
    ...
    assert.ok(
      result.diagnostics.some((item) => item.code === 'kimi_unknown_subagent' && item.value === 'forge-missing'),
