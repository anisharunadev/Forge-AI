...
const WORKFLOW_PATH = path.join(ROOT, 'forge-core', 'workflows', 'code-review.md');
const FIXER_PATH = path.join(ROOT, 'agents', 'forge-code-fixer.md');
const REVIEWER_PATH = path.join(ROOT, 'agents', 'forge-code-reviewer.md');
...
// ---------------------------------------------------------------------------
describe('Bug 3 — forge-code-fixer BL-* inclusion in filter sets', () => {
  test('finding_parser documents BL-\\d+ as Critical-tier-equivalent', () => {
    ...
    assert.ok(parserStart !== -1, 'forge-code-fixer.md must have a <finding_parser> block');
  ...
  test('parse_findings step documents severity as "Critical (CR-* or BL-*)"', () => {
    ...
    assert.ok(stepStart !== -1, 'forge-code-fixer.md must have a parse_findings step');
...
// ---------------------------------------------------------------------------
// REVIEWER CONTRACT — forge-code-reviewer.md must acknowledge BL-/blocker: as
// an accepted alternative to CR-/critical: (tier-equivalent).
// ---------------------------------------------------------------------------
describe('Reviewer contract — forge-code-reviewer.md label-equivalence', () => {
  test('write_review step documents blocker: as accepted alternative to critical:', () => {
    ...
    const stepEnd = src.indexOf('</step>', stepStart);
    assert.ok(stepStart !== -1, 'forge-code-reviewer.md must have a write_review step');
    const stepSection = src.slice(stepStart, stepEnd);
    assert.ok(
