...
const capabilityRegistry = require('../forge-core/bin/lib/capability-registry.cjs');
...
describe('VALID_CONFIG_KEYS correctness', () => {
  ...
  test('#2531: hooks.workflow_guard must be in VALID_CONFIG_KEYS (used by hook, documented)', () => {
    assert.ok(
      ...
      'hooks.workflow_guard is read by forge-workflow-guard.js hook and documented in CONFIGURATION.md'
    );
  });
  ...
  test('#2533: workflow.max_discuss_passes must be in VALID_CONFIG_KEYS (used in discuss-phase.md)', () => {
    assert.ok(
      ...
      'workflow.max_discuss_passes is read in discuss-phase.md via forge-sdk query config-get'
    );
  });
