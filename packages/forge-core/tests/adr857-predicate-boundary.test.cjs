...
const LIB = path.join(REPO_ROOT, 'forge-core', 'bin', 'lib');
...
describe('ADR-857 predicate boundary: substrate loads as core (no capabilities active)', () => {
  test('probe-core.cjs resolves from forge-core/bin/lib (core path)', () => {
  ...
  test('edge-probe.cjs resolves from forge-core/bin/lib (core path)', () => {
    assert.ok(
...
describe('ADR-857 predicate boundary: substrate lives in core, not in capabilities/', () => {
  test('probe-core.cjs is resolvable from forge-core/bin/lib — the core module tier', () => {
    // Must resolve from core lib, not from any capability folder
    const resolved = require.resolve(PROBE_CORE_PATH);
    assert.ok(
      resolved.includes(path.join('forge-core', 'bin', 'lib')),
      `probe-core.cjs must resolve from forge-core/bin/lib (got: ${resolved})`
    );
    assert.ok(
  ...
  test('edge-probe.cjs is resolvable from forge-core/bin/lib — the core module tier', () => {
    const resolved = require.resolve(EDGE_PROBE_PATH);
    assert.ok(
      resolved.includes(path.join('forge-core', 'bin', 'lib')),
      `edge-probe.cjs must resolve from forge-core/bin/lib (got: ${resolved})`
    );
    assert.ok(
      !resolved.includes('capabilities'),
