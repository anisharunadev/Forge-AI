 ...
 *
 * Multiple GSD agents (forge-ui-checker, forge-planner) entered unbounded Read
 ...
 * forge-pattern-mapper was fixed in #2312; this covers the remaining agents.
 *
...
describe('bug #2346: agent read loop guards', () => {
  ...
  describe('forge-ui-checker', () => {
    const agentPath = path.join(AGENTS_DIR, 'forge-ui-checker.md');
    const content = fs.readFileSync(agentPath, 'utf-8');

    test('agent file exists', () => {
      assert.ok(fs.existsSync(agentPath), 'agents/forge-ui-checker.md must exist');
    });

    test('has <critical_rules> block', () => {
      assert.ok(
        content.includes('<critical_rules>'),
        'forge-ui-checker.md must have a <critical_rules> block to prevent unbounded read loops (#2346)'
      );
    });
  ...
  describe('forge-planner', () => {
    const agentPath = path.join(AGENTS_DIR, 'forge-planner.md');
    const content = fs.readFileSync(agentPath, 'utf-8');

    test('agent file exists', () => {
      assert.ok(fs.existsSync(agentPath), 'agents/forge-planner.md must exist');
    });

    test('has <critical_rules> block', () => {
      assert.ok(
        content.includes('<critical_rules>'),
        'forge-planner.md must have a <critical_rules> block to prevent unbounded read loops (#2346)'
      );
    });
