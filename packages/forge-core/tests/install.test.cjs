...
describe('getGlobalConfigDir — all runtimes default paths', () => {
  ...
  // no registry entry.
  const { runtimes: _registryRuntimes } = require('../forge-core/bin/lib/capability-registry.cjs');
...
describe('getGlobalConfigDir/getConfigDirFromHome — antigravity 2.x layout detection', () => {
  ...
  test('uses ~/.gemini/antigravity-ide when legacy dir is absent and ide dir exists', () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-antigravity-ide-'));
    try {
  ...
  test('uses ~/.gemini/antigravity-cli when only cli dir exists', () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-antigravity-cli-'));
    try {
  ...
  // The probeExists marker (forge-core/VERSION) makes the dir GSD installed into win.
  test('coexistence: legacy antigravity + GSD-marked antigravity-cli both present → resolves to antigravity-cli', (t) => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-antigravity-coexist-'));
    t.after(() => cleanup(home));
    // Both dirs exist on disk...
    fs.mkdirSync(path.join(home, '.gemini', 'antigravity'), { recursive: true });
    fs.mkdirSync(path.join(home, '.gemini', 'antigravity-cli', 'forge-core'), { recursive: true });
    // ...but only the cli dir carries the GSD marker.
    fs.writeFileSync(path.join(home, '.gemini', 'antigravity-cli', 'forge-core', 'VERSION'), '1.6.0\n');
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    assert.strictEqual(
      getGlobalConfigDir('antigravity'),
  ...
  test('coexistence: legacy antigravity carries the GSD marker (real 1.x install) → resolves to legacy even when cli dir exists bare', (t) => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-antigravity-legacy-marked-'));
    t.after(() => cleanup(home));
    fs.mkdirSync(path.join(home, '.gemini', 'antigravity', 'forge-core'), { recursive: true });
    fs.writeFileSync(path.join(home, '.gemini', 'antigravity', 'forge-core', 'VERSION'), '1.5.0\n');
    fs.mkdirSync(path.join(home, '.gemini', 'antigravity-cli'), { recursive: true });
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    assert.strictEqual(
...
describe('getConfigDirFromHome — spot-checks', () => {
  ...
  test('antigravity returns .agents (local) and legacy fallback global path when no 2.x dirs exist', () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-antigravity-empty-'));
...
describe('install/uninstall — hermes (nested skills/forge/<router>/skills/<stem>/ layout)', () => {
  let tmpDir;
  let previousCwd;

  beforeEach(() => {
    tmpDir = createTempDir('forge-hermes-install-');
  ...
  test('installs GSD into ./.hermes and removes it cleanly', () => {
    ...
    // hermes nests: skills/forge/forge-<router>/skills/<stem>/SKILL.md (#947 — canonical forge- prefix)
    const hermesHelpPath = nestedSkillPath(path.join(targetDir, 'skills', 'forge'), 'forge-', 'help');
    assert.ok(fs.existsSync(hermesHelpPath),
      `help SKILL.md must exist at nested path: ${path.relative(targetDir, hermesHelpPath)}`);
    assert.ok(fs.existsSync(path.join(targetDir, 'skills', 'forge', 'DESCRIPTION.md')),
    ...
    assert.ok(
      Object.keys(manifest.files).some(f =>
        f.startsWith('skills/forge/forge-' + CHILD_ROUTER['help'] + '/skills/help/')
      ),
    ...
    assert.ok(!fs.existsSync(hermesHelpPath));
    assert.ok(!fs.existsSync(path.join(targetDir, 'skills', 'forge')));
  ...
  test('installed SKILL.md frontmatter conforms to Hermes spec', () => {
    ...
    const targetDir = path.join(tmpDir, '.hermes');
    const categoryDir = path.join(targetDir, 'skills', 'forge');
    ...
    assert.strictEqual(descFm.name, 'forge');
...
describe('install/uninstall — qwen (nested skills/forge-<router>/skills/<stem>/ layout)', () => {
  let tmpDir;
  let previousCwd;

  beforeEach(() => {
    tmpDir = createTempDir('forge-qwen-install-');
    previousCwd = process.cwd();
    process.chdir(tmpDir);
  });
  ...
  test('installs GSD into ./.qwen and removes it cleanly', () => {
    ...
    // qwen nests: skills/forge-<router>/skills/<stem>/SKILL.md
    const qwenHelpPath = nestedSkillPath(path.join(targetDir, 'skills'), 'forge-', 'help');
    ...
    assert.ok(
      Object.keys(manifest.files).some(f =>
        f.startsWith('skills/forge-' + CHILD_ROUTER['help'] + '/skills/help/')
      )
    );
...
describe('install/uninstall — trae (nested skills/forge-<router>/skills/<stem>/ layout)', () => {
  let tmpDir;
  let previousCwd;

  beforeEach(() => {
    tmpDir = createTempDir('forge-trae-install-');
    previousCwd = process.cwd();
    process.chdir(tmpDir);
  });
  ...
  test('installs GSD into ./.trae and removes it cleanly (typed IR result)', () => {
    ...
    // trae nests: skills/forge-<router>/skills/<stem>/SKILL.md
    const traeHelpPath = nestedSkillPath(path.join(targetDir, 'skills'), 'forge-', 'help');
...
describe('uninstall skills cleanup — hermes', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('forge-hermes-uninstall-');
    previousCwd = process.cwd();
    process.chdir(tmpDir);
  });
  ...
  test('removes skills/forge/ category dir', () => {
    install(false, 'hermes');
    const targetDir = path.join(tmpDir, '.hermes');
    const categoryDir = path.join(targetDir, 'skills', 'forge');
...
for (const runtime of ['hermes', 'qwen']) {
  describe(`no Claude references leak into ${runtime} install`, () => {
    ...
    beforeEach(() => {
      tmpDir = createTempDir(`forge-${runtime}-refs-`);
      previousCwd = process.cwd();
      process.chdir(tmpDir);
...
describe('resolveKiloConfigPath', () => {
  let tmpDir;
  beforeEach(() => { tmpDir = createTempProject('forge-kilo-'); });
...
describe('configureKiloPermissions', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempProject('forge-kilo-perms-');
  ...
  test('writes GSD permissions to kilo.json when config is missing', () => {
    ...
    const gsdPath = `${configDir.replace(/\\/g, '/')}/forge-core/*`;
  ...
  test('writes permissions to an explicit config dir argument', () => {
    ...
    const config = JSON.parse(fs.readFileSync(configPath, 'utf8'));
    const gsdPath = `${explicitDir.replace(/\\/g, '/')}/forge-core/*`;
    assert.strictEqual(config.permission.read[gsdPath], 'allow');
    assert.strictEqual(config.permission.external_directory[gsdPath], 'allow');
  });
});
...
describe('Kilo integration — install/uninstall behaviour', () => {
  ...
  const updateWorkflowSrc = fs.readFileSync(
    path.join(__dirname, '..', 'forge-core', 'workflows', 'update.md'), 'utf8');
  // #498: update.md's runtime/scope/config-dir resolution moved into the tested
  // projection forge-core/bin/lib/update-context.cjs. Custom-config-dir
  // detection (kilo.jsonc, KILO_CONFIG) is now asserted there.
  const updateContextSrc = fs.readFileSync(
    path.join(__dirname, '..', 'forge-core', 'bin', 'lib', 'update-context.cjs'), 'utf8');
  ...
  beforeEach(() => {
    tmpDir = createTempDir('forge-kilo-integration-');
    previousCwd = process.cwd();
    process.chdir(tmpDir);
  ...
  test('uninstall removes GSD permissions from the resolved kilo config path', () => {
    ...
    const gsdGlob = `${configDir.replace(/\\/g, '/')}/forge-core/*`;
    assert.ok(
    ...
    // still exist (Kilo preserves user settings) but the forge-core/* entries
    // must be gone.
...
describe('install — changeset CLI lands at scripts/changeset/cli.cjs (#935)', () => {
  ...
  // Before this fix, scripts/ was never copied and /forge-update changelog preview
  // silently failed on every real install.
  let tmpDir;
  let previousCwd;

  beforeEach(() => {
    tmpDir = createTempDir('forge-changeset-install-');
    previousCwd = process.cwd();
    process.chdir(tmpDir);
  });
  ...
  test('installed cli.cjs can run extract subcommand end-to-end (#935)', () => {
    ...
    const claudeDir = path.join(tmpDir, '.claude');
    const cliPath = path.join(claudeDir, 'scripts', 'changeset', 'cli.cjs');
    // Use the CHANGELOG.md that was installed into forge-core/ (installed by the installer)
    const changelogPath = path.join(claudeDir, 'forge-core', 'CHANGELOG.md');
    assert.ok(fs.existsSync(changelogPath), 'CHANGELOG.md must be installed under forge-core/');
    const { spawnSync } = require('node:child_process');
...
describe('install — fix-slash-commands.cjs lands at scripts/fix-slash-commands.cjs (#1223)', () => {
  ...
  // config dir by the installer so forge-core/bin/lib/command-roster.cjs can require it
  // via '../../../scripts/fix-slash-commands.cjs'. Before this fix, the file was never
  // installed and every forge-tools command crashed with MODULE_NOT_FOUND (#1223).
  let tmpDir;
  let previousCwd;

  beforeEach(() => {
    tmpDir = createTempDir('forge-fix-slash-install-');
    previousCwd = process.cwd();
    process.chdir(tmpDir);
  });
  ...
  test('installed forge-tools.cjs query loads without MODULE_NOT_FOUND (#1223)', () => {
    // End-to-end smoke: spawning forge-tools.cjs must not crash with MODULE_NOT_FOUND.
    // This directly exercises the command-roster → fix-slash-commands require chain.
    install(false, 'claude');
    const claudeDir = path.join(tmpDir, '.claude');
    const gsdToolsPath = path.join(claudeDir, 'forge-core', 'bin', 'forge-tools.cjs');
    assert.ok(fs.existsSync(gsdToolsPath), 'pre-condition: forge-tools.cjs must be installed');
    const { spawnSync } = require('node:child_process');
    const result = spawnSync(
      process.execPath,
    ...
    assert.ok(
      !result.stderr.includes('MODULE_NOT_FOUND'),
      `forge-tools.cjs must not crash with MODULE_NOT_FOUND; stderr=${result.stderr}`,
    );
    assert.ok(
      !result.stderr.includes('Cannot find module'),
      `forge-tools.cjs must resolve all modules; stderr=${result.stderr}`,
    );
  });
...
// ─── Section N: readCmdNames() tolerates absent commands/forge/ dir (#1223) ────────

describe('readCmdNames() — tolerates missing commands/forge directory (#1223)', () => {
  // Regression guard: on installs where commands/forge/ does not exist (e.g. skill-based
  ...
  test('readCmdNames() returns [] from a context where commands/forge/ is absent', () => {
    ...
    // <tmpRoot>/scripts/ — making COMMANDS_DIR = path.join(__dirname,'..','commands','forge')
    ...
    // No commands/forge/ exists anywhere under or adjacent to tmpRoot.
    const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-readcmdnames-absentdir-'));
    try {
      ...
      // → COMMANDS_DIR = path.join(tmpScriptsDir, '..', 'commands', 'forge') = <tmpRoot>/commands/gsd
      ...
      assert.ok(
        ...
        `readCmdNames() must not throw when commands/forge/ is absent; stderr=${spawnResult.stderr}`,
      );
      assert.strictEqual(spawnResult.status, 0,
        `readCmdNames() must return [] (exit 0) when commands/forge/ is absent; ` +
...
describe('antigravity local install writes to .agents/ canonical dir (#791)', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('forge-antigravity-791-');
    previousCwd = process.cwd();
    process.chdir(tmpDir);
  });
  ...
  test('install writes workspace skills under .agents/skills/', () => {
    ...
    assert.ok(skillEntries.length > 0, 'at least one forge-* skill must be installed under .agents/skills/');
  ...
  test('install writes concrete skills at the immediate level AGY scans', () => {
    ...
    assert.ok(
      fs.existsSync(path.join(skillsDir, 'forge-progress', 'SKILL.md')),
      'AGY scans immediate skill folders, so /forge-progress must be installed at .agents/skills/forge-progress/SKILL.md',
    );
    assert.ok(
      fs.existsSync(path.join(skillsDir, 'forge-verify-work', 'SKILL.md')),
      'AGY scans immediate skill folders, so /forge-verify-work must be installed at .agents/skills/forge-verify-work/SKILL.md',
    );
    assert.ok(
      !fs.existsSync(path.join(skillsDir, 'forge-ns-workflow', 'skills', 'progress', 'SKILL.md')),
  ...
  test('installed agent files reference .agents/ not ~/.claude/ or bare .agent/', () => {
    ...
    assert.ok(agentFiles.length > 0, 'pre-condition: at least one forge-* agent must be installed');
  ...
  test('global antigravity install still writes to ~/.gemini/antigravity (unchanged)', () => {
    const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-ag-global-'));
    const savedHome = process.env.HOME;
    const savedUserProfile = process.env.USERPROFILE;
    const savedAntigravityConfig = process.env.ANTIGRAVITY_CONFIG_DIR;
    delete process.env.ANTIGRAVITY_CONFIG_DIR;
...
describe('windsurf local install writes workflow slash commands (#1615)', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('forge-windsurf-1085-');
    previousCwd = process.cwd();
    process.chdir(tmpDir);
  });
  ...
  test('install writes workspace workflows under .windsurf/workflows/', () => {
    ...
    assert.ok(workflowEntries.length > 0, 'at least one forge-* workflow must be installed under .windsurf/workflows/');
    assert.ok(fs.existsSync(path.join(workflowsDir, 'forge-help.md')), 'forge-help.md workflow must exist');
  });
  ...
  test('installed workflow content references command body, not skill locations', () => {
    ...
    const workflowEntries = fs.readdirSync(workflowsDir, { withFileTypes: true })
      .filter(e => e.isFile() && e.name.startsWith('forge-') && e.name.endsWith('.md'));
    assert.ok(workflowEntries.length > 0, 'pre-condition: at least one forge-* workflow must be installed');
    for (const workflowEntry of workflowEntries) {
      const workflowFile = path.join(workflowsDir, workflowEntry.name);
      const content = fs.readFileSync(workflowFile, 'utf8');
      assert.ok(
        content.includes(`${tmpDir}/.windsurf/forge-core/commands/forge/`.replace(/\\/g, '/')),
  ...
  // after install. Pre-fix, forge-core/ was copied AFTER workflows were written;
  // a throw or kill in that window left workflows pointing at missing files.
  // Post-fix, forge-core/ is copied first. This behavioral invariant catches
  ...
  test('every workflow @-reference target exists on disk after install (#1629 Finding A)', () => {
    ...
    const commandsGsdDir = path.join(tmpDir, '.windsurf', 'forge-core', 'commands', 'forge');
    assert.ok(fs.existsSync(commandsGsdDir),
      `forge-core/commands/forge/ must exist at ${commandsGsdDir} so workflows can delegate to it`);

    for (const workflowEntry of workflowEntries) {
      // Workflow naming convention: forge-<stem>.md → delegates to commands/forge/<stem>.md
      const stem = workflowEntry.name.replace(/^gsd-/, '').replace(/\.md$/, '');
      const targetFile = path.join(commandsGsdDir, `${stem}.md`);
      assert.ok(
        fs.existsSync(targetFile),
        `${workflowEntry.name} delegates to commands/forge/${stem}.md, but that file does not exist at ${targetFile}`,
      );
    }
  });

  test('global windsurf install does not write unsupported workflows or skills', () => {
    const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-ws-global-'));
    const savedHome = process.env.HOME;
    const savedUserProfile = process.env.USERPROFILE;
  ...
  test('global windsurf install still installs shared workflow assets', () => {
    const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-ws-global-c-'));
    const savedHome = process.env.HOME;
    const savedUserProfile = process.env.USERPROFILE;
    const savedWindsurfConfig = process.env.WINDSURF_CONFIG_DIR;
    delete process.env.WINDSURF_CONFIG_DIR;
    ...
    try {
      const result = install(true, 'windsurf');
      assert.ok(fs.existsSync(path.join(result.configDir, 'forge-core', 'workflows', 'update.md')),
        'global windsurf install should still copy shared forge-core workflow assets');
...
// ─── #1629 Finding B: legacy .devin/skills/forge-* cleanup on Windsurf reinstall ─
describe('cleanupWindsurfLegacyDevinSkills — removes pre-#1615 skill artifacts (#1629)', () => {
  ...
  test('removes GSD-managed forge-* dirs under .devin/skills/', (t) => {
    const tmpDir = createTempDir('forge-1629b-cleanup-');
    t.after(() => cleanup(tmpDir));

    // Stage legacy .devin/skills/forge-*/ artifacts (pre-#1615 layout)
    const legacySkillsDir = path.join(tmpDir, '.devin', 'skills');
    for (const skill of ['forge-help', 'forge-plan-phase', 'forge-ship']) {
    ...
    assert.strictEqual(removed, 3, 'should remove exactly 3 forge-* dirs');
    for (const skill of ['forge-help', 'forge-plan-phase', 'forge-ship']) {
      assert.ok(
  ...
  test('preserves user-owned non-gsd- content under .devin/skills/', (t) => {
    const tmpDir = createTempDir('forge-1629b-preserve-');
    t.after(() => cleanup(tmpDir));

    const legacySkillsDir = path.join(tmpDir, '.devin', 'skills');
    // Stage mixed content: legacy GSD + user-authored + user-owned forge-dev-preferences
    fs.mkdirSync(path.join(legacySkillsDir, 'forge-help'), { recursive: true });
    fs.writeFileSync(path.join(legacySkillsDir, 'forge-help', 'SKILL.md'), '# legacy\n');
    ...
    fs.mkdirSync(path.join(legacySkillsDir, 'forge-dev-preferences'), { recursive: true });
    fs.writeFileSync(path.join(legacySkillsDir, 'forge-dev-preferences', 'SKILL.md'), '# prefs\n');

    const removed = cleanupWindsurfLegacyDevinSkills(tmpDir);

    assert.strictEqual(removed, 1, 'only forge-help should be removed (forge-dev-preferences is user-owned)');
    assert.ok(!fs.existsSync(path.join(legacySkillsDir, 'forge-help')), 'legacy forge-help removed');
    assert.ok(
      fs.existsSync(path.join(legacySkillsDir, 'my-custom-skill')),
      'user-authored my-custom-skill must be preserved',
    );
    assert.ok(
      fs.existsSync(path.join(legacySkillsDir, 'forge-dev-preferences')),
      'user-owned forge-dev-preferences must be preserved (#2973)',
    );
  ...
  test('skips symlinks pointing outside the .devin tree (escape guard)', (t) => {
    const tmpDir = createTempDir('forge-1629b-symlink-');
    t.after(() => cleanup(tmpDir));
    ...
    fs.symlinkSync(outsideTarget, path.join(legacySkillsDir, 'forge-symlinked'));

    const removed = cleanupWindsurfLegacyDevinSkills(tmpDir);

    assert.strictEqual(removed, 0, 'symlinked forge-* dir must not be removed');
    assert.ok(
      fs.existsSync(path.join(legacySkillsDir, 'forge-symlinked')),
  ...
  test('no-op when .devin/skills/ does not exist', () => {
    const tmpDir = createTempDir('forge-1629b-noop-');
    const removed = cleanupWindsurfLegacyDevinSkills(tmpDir);
    assert.strictEqual(removed, 0, 'should return 0 when .devin/skills/ is absent');
    cleanup(tmpDir);
  });

  test('install(false, "windsurf") removes pre-existing .devin/skills/forge-* on reinstall', (t) => {
    ...
    const tmpDir = createTempDir('forge-1629b-e2e-');
    t.after(() => cleanup(tmpDir));
    const previousCwd = process.cwd();
    process.chdir(tmpDir);
    try {
      // Stage pre-#1615 artifacts
      const legacyDir = path.join(tmpDir, '.devin', 'skills', 'forge-help');
      ...
      assert.ok(
        !fs.existsSync(path.join(tmpDir, '.devin', 'skills', 'forge-help')),
        'pre-existing .devin/skills/forge-help should be removed by fresh windsurf install',
      );
...
//   3. Negative: forge-nyquist-auditor has NO disallowedTools key (legitimately writes+edits).
...
const GROUP_A_767 = ['forge-plan-checker', 'forge-integration-checker', 'forge-ui-checker'];
const GROUP_B_767 = ['forge-verifier', 'forge-doc-verifier', 'forge-eval-auditor', 'forge-ui-auditor'];
...
function runGlobalInstall767(runtime, tmpHome) {
  ...
  const isolatedHome = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-767-home-'));
...
describe('#767 Claude install: Group A agents have disallowedTools = {Write, Edit, MultiEdit}', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('forge-767-claude-a-');
...
describe('#767 Claude install: Group B agents have disallowedTools = {Edit, MultiEdit}', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('forge-767-claude-b-');
    claudeHome = path.join(tmpDir, 'claude-home');
    fs.mkdirSync(claudeHome, { recursive: true });
    runGlobalInstall767('claude', claudeHome);
  });
...
describe('#767 Claude install: forge-nyquist-auditor has no disallowedTools (legitimately writes+edits)', () => {
  let tmpDir;
  let claudeHome;

  beforeEach(() => {
    tmpDir = createTempDir('forge-767-claude-nyquist-');
    claudeHome = path.join(tmpDir, 'claude-home');
    fs.mkdirSync(claudeHome, { recursive: true });
    runGlobalInstall767('claude', claudeHome);
  });
  ...
  test('forge-nyquist-auditor.md has NO disallowedTools key', () => {
    const fm = readFrontmatterText(path.join(claudeHome, 'agents', 'forge-nyquist-auditor.md'));
    const tools = parseDisallowedToolsSet(fm);
    assert.equal(tools, null,
      `forge-nyquist-auditor must NOT have disallowedTools in Claude frontmatter\nFrontmatter:\n${fm}`);
  });
});
...
describe('#767 Gemini install: read-only agents do NOT contain disallowedTools (cross-runtime leak guard)', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('forge-767-gemini-');
