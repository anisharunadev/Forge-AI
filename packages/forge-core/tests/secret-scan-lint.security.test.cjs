...
describe('lint: grandfathered entries (backward compat)', { skip: IS_WINDOWS }, () => {
  ...
  const GRANDFATHER_FIXTURE = [
    ...
    'forge-core/workflows/plan-phase.md',
    '',
  ].join('\n');
...
describe('secret-scan.sh default mode: regression test', { skip: IS_WINDOWS }, () => {
  test('existing .secretscanignore entry is still honoured in default mode', () => {
    // The file forge-core/workflows/plan-phase.md is listed in .secretscanignore.
    ...
    const planPhase = path.join(PROJECT_ROOT, 'forge-core', 'workflows', 'plan-phase.md');
    ...
    const result = spawnSync(SECRET_SCAN, ['--file', 'forge-core/workflows/plan-phase.md'], {
      encoding: 'utf-8',
      timeout: 10000,
      cwd: PROJECT_ROOT,
    });
