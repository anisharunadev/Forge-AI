...
describe('bug #969 A — ensureBuiltArtifacts rebuilds stale artifacts', () => {
   ...
   * forge-core/bin/lib/*.cjs or the real tsbuildinfo. (Regression from #996 fixed here.)
   */
  function makeTempProject() {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-bug969-'));
   ...
   * touches forge-core/bin/lib/core.cjs or the real tsbuildinfo. (Fixed from #996.)
   */
   ...
   * the real forge-core/bin/lib tree.
   */
...
describe('bug #969 B — runGsdTools kill-signal discrimination', () => {
  const TOOLS_PATH = path.join(__dirname, '..', 'forge-core', 'bin', 'forge-tools.cjs');
    ...
    try {
      ...
      if (err.killed || err.signal != null || err.code === 'ETIMEDOUT') {
        throw new Error(
          ...
          `forge-tools was killed before completion ` +
   ...
   *   forge-tools output from it, causing a confusing downstream assertion fail.
   *
   * PASS-AFTER (fix):
   ...
   * real forge-tools invocation (even `--help` takes >1ms to start node).
   */
  test('throws a resource-starvation error when subprocess is killed/times out', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-969-'));
    try {
      // 1ms timeout guarantees ETIMEDOUT / killed before forge-tools can respond.
      assert.throws(
  ...
  test('returns { success: true } for a normal fast command with generous timeout', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-969-'));
    try {
      // 30s timeout; forge-tools --help completes in well under 1s.
  ...
  /**
   * Verify that a clean non-zero exit (a real forge-tools application error, not
