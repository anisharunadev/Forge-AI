 ...
 *
 * Context: on Windows (and occasionally other platforms), forge-planner's
...
const PLAN_PHASE = path.join(
  __dirname, '..', 'forge-core', 'workflows', 'plan-phase.md'
);

const PLANNER_AGENT = path.join(__dirname, '..', 'agents', 'forge-planner.md');
const PLANNER_CHUNKED_REF = path.join(__dirname, '..', 'forge-core', 'references', 'planner-chunked.md');
const CONFIG_SCHEMA = path.join(__dirname, '..', 'forge-core', 'bin', 'lib', 'config-schema.cjs');
...
describe('forge-planner.md — references planner-chunked.md (#2310)', () => {
  const plannerContent = fs.readFileSync(PLANNER_AGENT, 'utf-8');

  test('forge-planner.md references planner-chunked.md for chunked return formats', () => {
    assert.ok(
      plannerContent.includes('planner-chunked.md'),
      'forge-planner.md must reference planner-chunked.md for ## OUTLINE COMPLETE / ## PLAN COMPLETE formats'
    );
  });
});
