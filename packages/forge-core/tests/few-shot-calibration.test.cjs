...
const REFS_DIR = path.join(__dirname, '..', 'forge-core', 'references', 'few-shot-examples');
...
describe('few-shot calibration examples', () => {
  ...
  describe('agent files reference few-shot examples', () => {
    test('forge-plan-checker.md contains reference to plan-checker few-shot examples', () => {
      const content = readFile(path.join(AGENTS_DIR, 'forge-plan-checker.md'));
      assert.match(content, /@~\/\.claude\/forge-core\/references\/few-shot-examples\/plan-checker\.md/);
    });

    test('forge-verifier.md contains reference to verifier few-shot examples', () => {
      const content = readFile(path.join(AGENTS_DIR, 'forge-verifier.md'));
      assert.match(content, /@~\/\.claude\/forge-core\/references\/few-shot-examples\/verifier\.md/);
    });
  });
