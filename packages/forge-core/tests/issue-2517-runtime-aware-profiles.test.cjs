 ...
 * developer's real `~/.forge/defaults.json` cannot bleed into assertions
...
const {
  ...
  resolveTierEntry,
} = require('../forge-core/bin/lib/model-resolver.cjs');
const {
  RUNTIME_PROFILE_MAP,
  KNOWN_RUNTIMES,
} = require('../forge-core/bin/lib/model-catalog.cjs');
const { renderEffortForRuntime } = require('../forge-core/bin/lib/model-catalog.cjs');
const { isValidConfigKey } = require('../forge-core/bin/lib/config-schema.cjs');
...
// Without this, a developer's real `~/.forge/defaults.json` (e.g. one with
...
function isolateHome() {
  ...
  _isolatedHome = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-home-iso-'));
...
describe('issue #2517: backwards compat — no runtime key set', () => {
  ...
  test('balanced profile returns Claude alias when runtime absent', () => {
    ...
    // forge-planner balanced -> opus
    assert.strictEqual(resolveModelInternal(tmpDir, 'forge-planner'), 'opus');
  });

  test('inherit profile still returns "inherit" with no runtime', () => {
    writeConfig(tmpDir, { model_profile: 'inherit' });
    assert.strictEqual(resolveModelInternal(tmpDir, 'forge-planner'), 'inherit');
  });
  ...
  test('resolve_model_ids:true still maps alias -> full Claude ID with no runtime', () => {
    ...
    assert.strictEqual(resolveModelInternal(tmpDir, 'forge-planner'), 'claude-opus-4-8');
  });
  ...
  test('resolve_model_ids:"omit" still returns "" with no runtime', () => {
    ...
    assert.strictEqual(resolveModelInternal(tmpDir, 'forge-planner'), '');
  });

  test('effort resolves universally but render param is null when runtime absent', () => {
    writeConfig(tmpDir, { model_profile: 'balanced' });
    const eff = resolveEffortInternal(tmpDir, 'forge-planner');
  ...
  test('adaptive profile still works without runtime (#1713/#1806)', () => {
    ...
    assert.strictEqual(resolveModelInternal(tmpDir, 'forge-planner'), 'opus');
    assert.strictEqual(resolveModelInternal(tmpDir, 'forge-codebase-mapper'), 'haiku');
  });
});
...
describe('issue #2517: runtime "claude" is a no-op for resolution (finding #4)', () => {
  ...
  test('effort is first-class on Claude (emits output_config.effort)', () => {
    ...
    // forge-planner is heavy tier → default effort 'xhigh'
...
describe('issue #2517: runtime "codex" — Codex tier resolution', () => {
  ...
  test('opus tier -> gpt-5.5 model; heavy-tier agent -> xhigh effort on codex', () => {
    ...
    // forge-planner quality -> opus -> gpt-5.5 (model unchanged)
    assert.strictEqual(resolveModelInternal(tmpDir, 'forge-planner'), 'gpt-5.5');
    // forge-planner is heavy routing tier → effort 'xhigh' → rendered model_reasoning_effort
    const eff = resolveEffortInternal(tmpDir, 'forge-planner');
  ...
  test('sonnet tier -> gpt-5.4 model; heavy-tier agent -> xhigh effort on codex', () => {
    ...
    assert.strictEqual(resolveModelInternal(tmpDir, 'forge-roadmapper'), 'gpt-5.4');
    // forge-roadmapper is heavy routing tier → effort 'xhigh' (not catalog medium)
    const eff = resolveEffortInternal(tmpDir, 'forge-roadmapper');
    const rendered = renderEffortForRuntime('codex', eff);
    assert.strictEqual(rendered.param, 'model_reasoning_effort');
    assert.strictEqual(rendered.value, 'xhigh');
  });
  ...
  test('haiku tier -> gpt-5.4-mini model; light-tier agent -> low effort on codex', () => {
    ...
    assert.strictEqual(resolveModelInternal(tmpDir, 'forge-codebase-mapper'), 'gpt-5.4-mini');
    // forge-codebase-mapper is light routing tier → effort 'low' (not catalog medium)
    const eff = resolveEffortInternal(tmpDir, 'forge-codebase-mapper');
    const rendered = renderEffortForRuntime('codex', eff);
    assert.strictEqual(rendered.param, 'model_reasoning_effort');
  ...
  test('adaptive profile resolves on Codex (no #1713/#1806 regression)', () => {
    ...
    // forge-planner adaptive -> opus -> gpt-5.5
    assert.strictEqual(resolveModelInternal(tmpDir, 'forge-planner'), 'gpt-5.5');
    // forge-codebase-mapper adaptive -> haiku -> gpt-5.4-mini
    assert.strictEqual(resolveModelInternal(tmpDir, 'forge-codebase-mapper'), 'gpt-5.4-mini');
  });
  ...
  test('inherit profile still returns "inherit" on Codex; effort still resolves universally', () => {
    ...
    // forge-planner (heavy tier) → 'xhigh'; rendered to codex param.
    const eff = resolveEffortInternal(tmpDir, 'forge-planner');
    const rendered = renderEffortForRuntime('codex', eff);
    assert.strictEqual(rendered.param, 'model_reasoning_effort');
    assert.strictEqual(rendered.value, 'xhigh');
...
describe('issue #2517: precedence chain', () => {
  ...
  test('per-agent model_overrides wins over runtime tier resolution', () => {
    writeConfig(tmpDir, {
      runtime: 'codex',
      model_profile: 'quality',
      model_overrides: { 'forge-planner': 'gpt-5.4-mini' },
    });
    assert.strictEqual(resolveModelInternal(tmpDir, 'forge-planner'), 'gpt-5.4-mini');
  });
  ...
  test('model_profile_overrides[runtime][tier] beats built-in defaults', () => {
    ...
    // forge-planner quality -> opus -> overridden to gpt-5-pro
    assert.strictEqual(resolveModelInternal(tmpDir, 'forge-planner'), 'gpt-5-pro');
    // haiku not overridden — fall back to spec defaults
    // forge-codebase-mapper quality -> sonnet -> gpt-5.4
    assert.strictEqual(resolveModelInternal(tmpDir, 'forge-codebase-mapper'), 'gpt-5.4');
  });
  ...
  test('partial profile_overrides — only opus overridden, sonnet uses default', () => {
    ...
    // forge-planner balanced -> opus -> overridden to gpt-5-pro
    assert.strictEqual(resolveModelInternal(tmpDir, 'forge-planner'), 'gpt-5-pro');
    // forge-roadmapper balanced -> sonnet -> spec default
    assert.strictEqual(resolveModelInternal(tmpDir, 'forge-roadmapper'), 'gpt-5.4');
  });
  ...
  test('per-agent override beats profile override beats default', () => {
    writeConfig(tmpDir, {
      ...
      model_overrides: { 'forge-planner': 'custom-model' },
    });
    assert.strictEqual(resolveModelInternal(tmpDir, 'forge-planner'), 'custom-model');
  });
});
...
describe('issue #2517: field-merge of overrides with built-in defaults (finding #2)', () => {
  ...
  test('string-shorthand override: model is overridden; unified effort derives from routing tier', () => {
    ...
    // config-driven path (forge-planner heavy tier → 'xhigh'), not from the catalog.
    writeConfig(tmpDir, {
      runtime: 'codex',
      model_profile: 'quality',
      model_profile_overrides: { codex: { opus: 'gpt-5-pro' } },
  ...
  test('partial-object override (no model) keeps model from built-in; unified effort from routing tier', () => {
    ...
    // effort comes from routing_tier_defaults (forge-planner heavy → 'xhigh').
    writeConfig(tmpDir, {
      runtime: 'codex',
      model_profile: 'quality',
...
describe('issue #2517: unknown runtime render param is null (effort does not leak to install path)', () => {
  ...
  test('unknown runtime: model resolves via override; render param is null (no wire param leaked)', () => {
    ...
    assert.strictEqual(resolveModelInternal(tmpDir, 'forge-planner'), 'mystery-opus');
...
describe('issue #2517: unknown runtime + safe fallback', () => {
  ...
  test('unknown runtime falls back to Claude-alias safe default (no Codex IDs leaked)', () => {
    ...
    const resolved = resolveModelInternal(tmpDir, 'forge-planner');
...
describe('issue #2517: loadConfig warns on unknown runtime/tier (findings #10, #13)', () => {
  const { loadConfig } = require('../forge-core/bin/lib/config-loader.cjs');
  let tmpDir;
  let origWrite;
  let captured;
  beforeEach(() => {
...
describe('issue #2517: install end-to-end — per-project config reaches Codex TOML (finding #1)', () => {
  ...
  test('readGsdRuntimeProfileResolver picks up runtime from .planning/config.json', () => {
    // No ~/.forge/defaults.json (HOME is isolated tmpdir). Per-project config alone
    ...
    const entry = resolver.resolve('forge-planner');
    assert.deepStrictEqual(entry, { model: 'gpt-5.5', reasoning_effort: 'xhigh' });
  });

  test('per-project config wins over global ~/.forge/defaults.json', () => {
  ...
  test('generated TOML always includes model_reasoning_effort even when model_profile_overrides sets reasoning_effort to empty (#443 unified)', () => {
    ...
    // forge-planner is a heavy-tier agent → unified default resolves to "xhigh".
    writeConfig(tmpDir, {
      runtime: 'codex',
      model_profile: 'quality',
    ...
    // forge-planner is heavy-tier, so with no effortCfg the manifest tier default applies → xhigh.
    assert.match(toml, /^model_reasoning_effort = "xhigh"$/m);
  });
  ...
  test('inline require paths resolve relative to install.js __dirname (finding #6)', () => {
    ...
    const libDir = path.join(installDir, '..', 'forge-core', 'bin', 'lib');
...
describe('issue #2612: runtime "gemini" — Gemini tier resolution', () => {
  ...
  test('opus tier -> gemini-3.1-pro-preview', () => {
    ...
    assert.strictEqual(resolveModelInternal(tmpDir, 'forge-planner'), 'gemini-3.1-pro-preview');
  });

  test('sonnet tier -> gemini-3-flash', () => {
    writeConfig(tmpDir, { runtime: 'gemini', model_profile: 'balanced' });
    assert.strictEqual(resolveModelInternal(tmpDir, 'forge-roadmapper'), 'gemini-3-flash');
  });

  test('haiku tier -> gemini-2.5-flash-lite', () => {
    writeConfig(tmpDir, { runtime: 'gemini', model_profile: 'budget' });
    assert.strictEqual(resolveModelInternal(tmpDir, 'forge-codebase-mapper'), 'gemini-2.5-flash-lite');
  });
...
describe('issue #2612: runtime "qwen" — Qwen tier resolution', () => {
  ...
  test('opus tier -> qwen3-max-2026-01-23', () => {
    ...
    assert.strictEqual(resolveModelInternal(tmpDir, 'forge-planner'), 'qwen3-max-2026-01-23');
  });

  test('sonnet tier -> qwen3-coder-plus', () => {
    writeConfig(tmpDir, { runtime: 'qwen', model_profile: 'balanced' });
    assert.strictEqual(resolveModelInternal(tmpDir, 'forge-roadmapper'), 'qwen3-coder-plus');
  });

  test('haiku tier -> qwen3-coder-next', () => {
    writeConfig(tmpDir, { runtime: 'qwen', model_profile: 'budget' });
    assert.strictEqual(resolveModelInternal(tmpDir, 'forge-codebase-mapper'), 'qwen3-coder-next');
  });
...
describe('issue #2612: runtime "opencode" — OpenCode tier resolution', () => {
  ...
  test('opus tier -> anthropic/claude-opus-4-8', () => {
    ...
    assert.strictEqual(resolveModelInternal(tmpDir, 'forge-planner'), 'anthropic/claude-opus-4-8');
  });

  test('sonnet tier -> anthropic/claude-sonnet-4-6', () => {
    writeConfig(tmpDir, { runtime: 'opencode', model_profile: 'balanced' });
    assert.strictEqual(resolveModelInternal(tmpDir, 'forge-roadmapper'), 'anthropic/claude-sonnet-4-6');
  });

  test('haiku tier -> anthropic/claude-haiku-4-5', () => {
    writeConfig(tmpDir, { runtime: 'opencode', model_profile: 'budget' });
    assert.strictEqual(resolveModelInternal(tmpDir, 'forge-codebase-mapper'), 'anthropic/claude-haiku-4-5');
  });
...
describe('issue #2612: runtime "copilot" — Copilot tier resolution', () => {
  ...
  test('sonnet tier -> claude-sonnet-4-6', () => {
    ...
    assert.strictEqual(resolveModelInternal(tmpDir, 'forge-roadmapper'), 'claude-sonnet-4-6');
  });

  test('haiku tier -> claude-haiku-4-5', () => {
    writeConfig(tmpDir, { runtime: 'copilot', model_profile: 'budget' });
    assert.strictEqual(resolveModelInternal(tmpDir, 'forge-codebase-mapper'), 'claude-haiku-4-5');
  });
...
describe('issue #2612: partial override merge for new Group A runtimes', () => {
  ...
  test('gemini.opus override wins; sonnet and haiku use built-in defaults', () => {
    ...
    // opus is overridden
    assert.strictEqual(resolveModelInternal(tmpDir, 'forge-planner'), 'gemini-3-ultra');
    // sonnet not overridden — built-in default (quality -> sonnet for forge-codebase-mapper)
    assert.strictEqual(resolveModelInternal(tmpDir, 'forge-codebase-mapper'), 'gemini-3-flash');
  });
  ...
  test('qwen.opus override wins; sonnet and haiku use built-in defaults', () => {
    ...
    // opus is overridden
    assert.strictEqual(resolveModelInternal(tmpDir, 'forge-planner'), 'qwen3-max-custom');
    // sonnet not overridden — quality -> sonnet for forge-codebase-mapper
    assert.strictEqual(resolveModelInternal(tmpDir, 'forge-codebase-mapper'), 'qwen3-coder-plus');
  });
  ...
  test('opencode.sonnet override wins; opus and haiku still use built-in defaults', () => {
    ...
    // forge-planner balanced -> opus -> built-in default
    assert.strictEqual(resolveModelInternal(tmpDir, 'forge-planner'), 'anthropic/claude-opus-4-8');
    // forge-roadmapper balanced -> sonnet -> overridden
    assert.strictEqual(resolveModelInternal(tmpDir, 'forge-roadmapper'), 'anthropic/claude-sonnet-4-7');
    // forge-codebase-mapper balanced -> haiku -> built-in default (haiku not overridden)
    assert.strictEqual(resolveModelInternal(tmpDir, 'forge-codebase-mapper'), 'anthropic/claude-haiku-4-5');
  });
  ...
  test('copilot.haiku override wins; opus and sonnet still use built-in defaults', () => {
    ...
    // forge-codebase-mapper budget -> haiku -> overridden
    assert.strictEqual(resolveModelInternal(tmpDir, 'forge-codebase-mapper'), 'claude-haiku-4-6');
    // forge-planner budget -> sonnet -> built-in default
    assert.strictEqual(resolveModelInternal(tmpDir, 'forge-planner'), 'claude-sonnet-4-6');
  });
});
