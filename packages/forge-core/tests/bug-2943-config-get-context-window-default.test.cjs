 ...
 *
 * `forge-tools.cjs config-get context_window` (and the SDK equivalent) threw
...
// "Key not found"; now the test passes `--json-errors` to forge-tools and
...
const { ERROR_REASON } = require(path.join(__dirname, '..', 'forge-core', 'bin', 'lib', 'io.cjs'));
...
describe('bug-2943: config-get returns schema default for context_window', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-test-2943-'));
