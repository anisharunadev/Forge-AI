 ...
 * completely absent from bin/install.js. Running `npx @forge-ai/forge-core`
 ...
 * - Installs forge-core/ engine with path replacement
 */
...
describe('Cline markdown conversion', () => {
  ...
  test('replaces .claude/ paths with .cline/', () => {
    const result = convertClaudeToCliineMarkdown('See ~/.claude/forge-core/');
  ...
  test('replaces .claude/skills/ with .cline/skills/', () => {
    const result = convertClaudeToCliineMarkdown('skills at .claude/skills/forge-executor');
...
describe('Cline install (local)', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('forge-cline-test-');
  ...
  test('install creates .clinerules directory with forge.md (#787 directory form)', () => {
    install(false, 'cline');
  ...
  test('.clinerules/gsd.md contains GSD instructions', () => {
    ...
    assert.ok(content.includes('GSD') || content.includes('forge'), '.clinerules/gsd.md must reference GSD');
  });

  test('install creates forge-core engine directory', () => {
    install(false, 'cline');
    const engineDir = path.join(tmpDir, 'forge-core');
    assert.ok(fs.existsSync(engineDir), 'forge-core directory must exist after install');
  });
    ...
    function scanDir(dir) {
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          ...
          const hasLeaked = /~\/\.claude\/(?:forge-core|commands|agents|hooks)|HOME\/\.claude\/(?:forge-core|commands|agents|hooks)/.test(content);
