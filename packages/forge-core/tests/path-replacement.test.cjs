...
const { _computePathPrefix } = require('../forge-core/bin/lib/runtime-artifact-conversion.cjs');
...
describe('pathPrefix computation', () => {
  ...
  test('target outside home uses absolute path', () => {
    const prefix = _computePathPrefix({
      ...
      isOpencode: false,
      isWindowsHost: false,
      resolvedTarget: '/opt/forge/.claude',
      homeDir: '/home/user',
    });
    assert.strictEqual(prefix, '/opt/forge/.claude/');
...
describe('source .md files have no quoted-tilde shell patterns', () => {
  ...
  const dirsToCheck = ['commands', 'forge-core', 'agents'].map(d => path.join(repoRoot, d));
