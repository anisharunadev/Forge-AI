...
/**
 * Governance tests for the forge-core repository root layout.
 *
 ...
 * AGENTS.md to process.cwd() when `forge install copilot` is run inside a repo
...
test('repo-layout: root AGENTS.md is absent — no ad-hoc AI instruction file committed alongside CONTEXT.md', () => {
  ...
  assert.equal(
    ...
    [
      ...
      'This file is written by `forge install copilot` (bin/install.js, local Copilot path, issue #786)',
