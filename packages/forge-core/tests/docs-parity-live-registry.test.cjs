...
// tests the deployed contract. The commands/forge/*.md reads in the helper are
 ...
 *   - forge-core/workflows/help.md  (deprecation-trail prose)
...
const ALLOWED_HISTORICAL_MENTIONS = new Set([
  path.join(ROOT, 'forge-core', 'workflows', 'help.md'),
...
// syntax placeholders — they match the /forge-* regex but are NOT user-typable
...
const INTERNAL_COMPONENT_SLUGS = new Set([
  ...
  // (e.g. "/forge-command-name [args]"). It is not a registered command.
  'command-name',
  'command',

  // forge-tools.cjs — the legacy Node CLI binary (bin/forge-tools.cjs).
  // Docs reference it as a path component in shell examples, not as a slash command.
  // Example: node "$HOME/.claude/forge-core/bin/forge-tools.cjs" state validate
  'tools',

  // Hook scripts — internal runtime hooks, not user-invocable slash commands.
  //   hooks/forge-statusline.js       — session statusline hook
  //   hooks/forge-context-monitor.js  — context-window monitor hook
  //   hooks/forge-update-banner.js    — update-available banner hook
  //   hooks/forge-graphify-update.sh  — knowledge-graph auto-update PostToolUse hook (#3347)
  // These appear in docs as file-path references (e.g. "forge-statusline.js reads
  ...
  // forge-update-check.json — background update-check CACHE FILE, not a slash command.
  // ARCHITECTURE.md references "~/.cache/forge/forge-update-check.json" as a path;
  // the regex captures "/forge-update-check" from the path component.
  'update-check',

  // Internal agent names referenced in ARCHITECTURE.md tables of agents.
  // These are spawned agents (forge-planner, etc.), not user-typable slash commands.
  'planner',

  // Malformed token from SDK init reference: "/forge-init-" appears as a truncated
  // prefix in CLI-TOOLS.md describing the forge-sdk init command family
  // (e.g., "gsd-sdk query init.phase-op 12"). The regex captures "/forge-init-"
  ...
  // ~/forge-workspaces/ — filesystem directory path used by /forge-workspace.
  // Docs reference "~/forge-workspaces/<name>" as the default workspace directory
  // in shell examples and option tables (e.g. "--path /target (default: ~/forge-workspaces/<name>)").
  // The regex captures "/forge-workspaces" from the path component. The LIVE slash
  // command is "/forge-workspace" (singular) — not "/forge-workspaces" (plural).
  'workspaces',

  // Portuguese translation of "command" — pt-BR/ARCHITECTURE.md uses "/forge-comando"
  // as the localized equivalent of the "/forge-command-name" English placeholder
  // in an architecture flow diagram. Not a registered command.
  'comando',

  // GitHub repository name: zh-CN/README.md references "github.com/rokicool/forge-opencode"
  // as an external community project URL. The regex captures "/forge-opencode" from
  // the URL path. Not a user-typable slash command in this product.
  'opencode',

  // forge-sdk — the @openforge/forge-sdk npm package and `forge-sdk query` CLI binary.
  // Docs reference it as a package name (e.g. `@openforge/forge-sdk`) and CLI tool
  // (e.g. `forge-sdk query init phase-op 12`). The regex captures "/forge-sdk" from
  // the npm scope path separator in `@openforge/forge-sdk`. Not a user-typable slash command.
  'sdk',

  // Smoke-test directory path — locale docs reference "/tmp/forge-smoke-$(date +%s)"
  // as a temporary directory path in bash code-block examples. The regex captures
  // "/forge-smoke-" from the filesystem path. Not a slash command.
  'smoke-',

  // Template placeholders — zh-CN/references/ui-brand.md used "/forge-alternative-1"
  // and "/forge-alternative-2" as unfilled placeholders in a UI template example.
  ...
  // forge-sync-skills — installed Claude skill directory name (also a workflow
  // under forge-core/workflows/sync-skills.md), but NOT a registered
  // slash command (no commands/forge/sync-skills.md). Docs reference it as a
  // filesystem path component, e.g. "~/.agents/skills/forge-sync-skills/" in
  // docs/discussions/grok-build-support-2026-05.md. The regex captures
  // "/forge-sync-skills" from the path. Invoked via Skill(skill="gsd-sync-skills").
  'sync-skills',

  // forge-test-runner — GitHub repository name: "github.com/forge-ai/forge-test-runner".
  // docs/contributing/bootstrap.md references it as a hyperlink target:
  //   [gsd-test-runner](https://github.com/forge-ai/forge-test-runner)
  // The regex captures "/forge-test-runner" from the URL path component. This is
  // an external tool repo, not a user-typable slash command in this product.
  'test-runner',

  // forge-core — GitHub repository name: "forge-ai/forge-core".
  // docs/adr/22-plan-drift-guard.md references it as an issue tracker link:
  //   forge-ai/forge-core#22
  // The regex captures "/forge-core" from the org/repo path separator. This is
...
function extractCommandTokens(content) {
  ...
  function isInternal(token) {
    // Strip the /forge- or /forge: or $gsd- prefix to get the slug
  ...
  // `/`, `_`, or `-`. This prevents matching the `/forge-core` substring inside
  // the org/repo path `forge-ai/forge-core` (and similar path-embedded segments)
  // while still matching real invocations preceded by BOL, space, backtick, or
  // `(`. Fixes false-positive class identified in #489.
  const allSlash = (stripped.match(/(?<![A-Za-z0-9/_-])\/forge-[a-z0-9][a-z0-9-]*/g) || []);
  const allColon = (stripped.match(/(?<![A-Za-z0-9/_-])\/forge:[a-z0-9][a-z0-9-]*/g) || []);
...
describe('getLiveCommandTokens() — helper contract', () => {
  ...
  test('returns a non-empty set (commands/forge/ has registered commands)', () => {
    const result = getLiveCommandTokens();
    assert.ok(result.size > 0, 'live registry must contain at least one token');
  });

  test('contains /forge-help (from commands/forge/help.md name: forge:help)', () => {
    const result = getLiveCommandTokens();
    assert.ok(result.has('/forge-help'), 'registry must contain /forge-help');
  });

  test('contains /forge:help (Gemini form)', () => {
    const result = getLiveCommandTokens();
    assert.ok(result.has('/forge:help'), 'registry must contain /forge:help');
  });

  test('contains $forge-help (Codex form)', () => {
    const result = getLiveCommandTokens();
    assert.ok(result.has('$forge-help'), 'registry must contain $forge-help');
  });

  test('contains /forge-plan-phase (from commands/forge/plan-phase.md)', () => {
    const result = getLiveCommandTokens();
    assert.ok(result.has('/forge-plan-phase'), 'registry must contain /forge-plan-phase');
  });

  test('contains exactly 3 tokens per slug (slash, colon, dollar)', () => {
    const result = getLiveCommandTokens();
    // Every /forge-slug should have a matching /forge:slug and $gsd-slug
    const slashTokens = [...result].filter(t => t.startsWith('/forge-'));
    for (const slash of slashTokens) {
      const slug = slash.slice('/forge-'.length);
      assert.ok(
        result.has(`/forge:${slug}`),
        `registry must contain Gemini form /forge:${slug} for slash form ${slash}`
      );
      assert.ok(
        result.has(`$gsd-${slug}`),
  ...
  test('does NOT contain removed /forge-reapply-patches', () => {
    const result = getLiveCommandTokens();
    assert.ok(!result.has('/forge-reapply-patches'), 'registry must NOT contain removed /forge-reapply-patches');
  });

  test('does NOT contain removed /forge-code-review-fix', () => {
    const result = getLiveCommandTokens();
    assert.ok(!result.has('/forge-code-review-fix'), 'registry must NOT contain removed /forge-code-review-fix');
  });

  test('does NOT contain removed /forge-status', () => {
    const result = getLiveCommandTokens();
    assert.ok(!result.has('/forge-status'), 'registry must NOT contain removed /forge-status');
  });
...
describe('getLiveCommandTokens() — fixture contract', () => {
  test('parses forge:foo frontmatter and emits 3 canonical tokens', () => {
    // This test validates the parsing logic against a known-good fixture
    // by inspecting the live registry for commands/forge/help.md (name: forge:help).
    // Fixture file tests are done inline since the helper reads commands/forge/ only.
    // The canonical token contract:
    //   name: forge:foo → /forge-foo, /forge:foo, $gsd-foo
    const registry = getLiveCommandTokens();
    // We know help.md has name: forge:help
    const slug = 'help';
    assert.ok(registry.has(`/forge-${slug}`), `must have /forge-${slug}`);
    assert.ok(registry.has(`/forge:${slug}`), `must have /forge:${slug}`);
    assert.ok(registry.has(`$gsd-${slug}`), `must have $gsd-${slug}`);
  });

  test('parses forge-slug frontmatter (ns-* commands) and emits 3 tokens', () => {
    // ns-context.md has name: forge-context (dash-style, no colon)
    const registry = getLiveCommandTokens();
    assert.ok(registry.has('/forge-context'), 'must have /forge-context (from ns-context.md)');
    assert.ok(registry.has('/forge:context'), 'must have /forge:context (Gemini form)');
...
describe('adversarial: polarity inversion catches drift deny-list misses', () => {
  test('renaming a live command without updating docs would fail this test (demonstrated via token absence)', () => {
    // If /forge-progress were renamed to /forge-status-new, the old /forge-progress
    // token would not appear in the live registry, and any doc referencing
    // /forge-progress would fail. The deny-list shape would have passed silently
    ...
    const registry = getLiveCommandTokens();
    assert.ok(registry.has('/forge-progress'), '/forge-progress must be live (not renamed to /forge-status)');
    assert.ok(!registry.has('/forge-status'), '/forge-status must be absent (was deleted, replaced by /forge-progress)');
  });

  test('freshly-deleted command /forge-check-todos is absent from registry', () => {
    const registry = getLiveCommandTokens();
    assert.ok(!registry.has('/forge-check-todos'), '/forge-check-todos must not be in the live registry');
  });

  test('freshly-deleted command /forge-new-workspace is absent from registry', () => {
    const registry = getLiveCommandTokens();
    assert.ok(!registry.has('/forge-new-workspace'), '/forge-new-workspace must not be in the live registry');
  });

  test('freshly-deleted command /forge-plan-milestone-gaps is absent from registry', () => {
    const registry = getLiveCommandTokens();
    assert.ok(!registry.has('/forge-plan-milestone-gaps'), '/forge-plan-milestone-gaps must not be in the live registry');
  });

  test('freshly-deleted command /forge-research-phase is absent from registry', () => {
    const registry = getLiveCommandTokens();
    assert.ok(!registry.has('/forge-research-phase'), '/forge-research-phase must not be in the live registry');
  });
});
...
describe('extractCommandTokens() — repo-path false-positive regression (#489)', () => {
  test('forge-ai/forge-core#22 repo path does NOT produce a /forge-core token', () => {
    // Before the lookbehind fix, /forge-core inside `forge-ai/forge-core#22`
    // would be matched by the slash regex — a false positive.
    const { slash, colon, dollar } = extractCommandTokens(
      'see forge-ai/forge-core#22 for details'
    );
    const all = [...slash, ...colon, ...dollar];
    assert.ok(
      !all.includes('/forge-core'),
      'repo path forge-ai/forge-core#22 must not produce a /forge-core token; got: ' + all.join(', ')
    );
    assert.strictEqual(all.length, 0, 'expected zero tokens from a bare repo-path string; got: ' + all.join(', '));
  });

  test('space-preceded /forge-totally-not-a-real-command is still extracted (real invocation)', () => {
    ...
    const { slash } = extractCommandTokens(
      'run /forge-totally-not-a-real-command here'
    );
    assert.ok(
      slash.has('/forge-totally-not-a-real-command'),
      'invocation after whitespace must be extracted; slash set: ' + [...slash].join(', ')
    );
  });

  test('backtick-wrapped `/forge-plan` is still extracted (real invocation)', () => {
    // Backtick-wrapped commands (common in markdown) must still be captured.
    const { slash } = extractCommandTokens(
      'use `/forge-plan` to plan'
    );
    assert.ok(
      slash.has('/forge-plan'),
