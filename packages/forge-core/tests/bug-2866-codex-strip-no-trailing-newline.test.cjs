 ...
 * saw `forge-check-update` already present, skipped writing the new
...
const SHAPES = {
  'Shape 1 (legacy forge-update-check)': [
    '# GSD Hooks',
    '[[hooks]]',
    'event = "SessionStart"',
    'command = "node /Users/USER/.codex/hooks/forge-update-check.js"',
  ].join('\n'),
  'Shape 2 (flat [[hooks]] + forge-check-update)': [
    '# GSD Hooks',
    '[[hooks]]',
    'event = "SessionStart"',
    'command = "node /Users/USER/.codex/hooks/forge-check-update.js"',
  ].join('\n'),
...
describe('bug-2866: stripStaleGsdHookBlocks handles end-of-file without trailing newline', () => {
  ...
  function assertStripped(out, shape, scenario) {
    ...
    const staleCmd = [...shape_.keys.entries()].find(([_, v]) =>
      /forge-(update-check|check-update)/.test(v));
    assert.strictEqual(staleCmd, undefined,
      `(${shape}, ${scenario}) no key may carry a stale forge-*-update command, got: ${staleCmd && staleCmd.join('=')}`);
  ...
  test('user-authored [[hooks.UserPromptSubmit]] is preserved', () => {
    ...
    // GSD-managed `forge-(check-update|update-check).js` command.
    const input = [
      '[history]',
      'persistence = "save-all"',
