...
describe('autonomous --interactive flag (#1413)', () => {
  const workflowPath = path.join(__dirname, '..', 'forge-core', 'workflows', 'autonomous.md');
  const commandPath = path.join(__dirname, '..', 'commands', 'forge', 'autonomous.md');
  ...
  test('workflow uses discuss-phase skill in interactive mode', () => {
    // Per #2697 the user-facing form is the hyphen invariant forge-discuss-phase;
    ...
    // forge-discuss-phase mention in prose) and falsely pass. Instead, isolate
    ...
    const skillCall = /Skill\(\s*skill\s*=\s*['"]forge-discuss-phase['"]/.test(branch);
    assert.ok(
      skillCall,
      `INTERACTIVE branch must invoke Skill(skill="forge-discuss-phase"). Got branch:\n${branch}`,
    );
  });
