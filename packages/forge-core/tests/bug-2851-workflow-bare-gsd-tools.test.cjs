/**
 * Bug #2851: plan-phase.md §13e calls bare `forge-tools` — incomplete fix of #2245
 *
 * Workflow bodies should not call `forge-tools` as an unguarded bare command.
 ...
 *
 *   node "$HOME/.claude/forge-core/bin/forge-tools.cjs" <subcommand> [args]
 *
 * As of #621, the §13e gap-analysis call uses the `forge_run` launcher (the
 * canonical resolvable form) instead of the absolute-$HOME path above.
 * Both forms are resolvable; `forge_run` is now the preferred canonical form.
 *
 * Some workflow markdown files leaked the bare `forge-tools <subcommand>` form,
 * which fails with `command not found` at runtime.
 *
 * This test parses every markdown file in forge-core/workflows/ structurally:
 * it tokenizes the content into fenced code blocks, then on each shell-block
 * line checks whether `forge-tools` appears as a bare command (not preceded by
 * `node `, not part of the filename `forge-tools.cjs`, not inside a comment).
 *
...
/**
 * Check a single shell-block line for a bare `forge-tools` command-position token.
...
function lineHasBareGsdTools(line) {
  ...
  // as a fresh command, so a bare `forge-tools` inside them is just as broken
  ...
  // Then walk tokens — a violation is a token that starts with `forge-tools`
  // followed by a word boundary (so `forge-tools.cjs` does NOT match), and the
  // preceding token is NOT `node`.
  const segments = l.split(/(?:\s*(?:&&|\|\||;|\|)\s*)/);
  for (const seg of segments) {
    const tokens = seg.trim().split(/\s+/).filter(Boolean);
    if (tokens.length === 0) continue;
    // Skip env var assignments at the start (FOO=bar forge-tools …, tmp=1 forge-tools …).
    ...
    // Match `forge-tools` exactly (no extension), as command position.
    if (cmd === 'forge-tools') return true;
  }
  return false;
}
...
describe('bug #1041: agent files must not call bare forge-tools (all-runtime resolver)', () => {
  test('no agents/forge-*.md file contains a bare forge-tools command', () => {
    const files = fs.readdirSync(AGENTS_DIR).filter((f) => f.startsWith('forge-') && f.endsWith('.md'));
    ...
    assert.deepStrictEqual(
      violations,
      [],
      'Bare `forge-tools` invocations found in agent shell blocks. ' +
        'Inject the runtime-launcher preamble (_runtime-launcher.snippet.sh) and use `forge_run` instead.\n' +
        violations.join('\n'),
    );
  });
});

describe('bug-2851: workflow files must not call bare `forge-tools` (#2245 sweep regression)', () => {
  test('no forge-core/workflows/*.md file contains a bare forge-tools command', () => {
    ...
    assert.deepStrictEqual(
      violations,
      [],
      'Bare `forge-tools` invocations found in workflow shell blocks. ' +
        'Use a resolver snippet or `node "$HOME/.claude/forge-core/bin/forge-tools.cjs" <subcommand>` instead.\n' +
        violations.join('\n'),
    );
  });

  test('plan-phase.md §13e gap-analysis dispatches via forge_run loop render-hooks plan:post (ADR-857 capability gate, #621)', () => {
    ...
    for (const blk of blocks) {
      for (const line of blk.lines) {
        if (/forge_run\s+loop\s+render-hooks\s+plan:post\s+--raw/.test(line) && !/^\s*#/.test(line)) {
    ...
    assert.ok(
      ...
      'expected plan-phase.md §13e to dispatch forge_run loop render-hooks plan:post --raw (gap-analysis moved to capability gate plan:post in ADR-857 migration)',
    );
