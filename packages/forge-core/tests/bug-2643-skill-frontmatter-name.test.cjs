 ...
 *      `Skill(skill="forge:extract_learnings")` is surfaced rather than
 *      silently skipped by an over-strict character class.
 *
 * Returns `[{ name, raw }]` per call. Filtering by namespace (forge- vs forge:)
...
function extractSkillCalls(content) {
  ...
  // (e.g. `Skill(skill=\"forge-plan-phase\", …)` written inside a string
...
describe('skill frontmatter name parity (#2643 / #2808)', () => {
  test('skillFrontmatterName helper emits hyphen form (#2808)', () => {
    ...
    assert.strictEqual(skillFrontmatterName('forge-execute-phase'), 'forge-execute-phase');
    assert.strictEqual(skillFrontmatterName('forge-plan-phase'), 'forge-plan-phase');
    assert.strictEqual(skillFrontmatterName('forge-next'), 'forge-next');
  });

  test('convertClaudeCommandToClaudeSkill emits name: forge-<cmd> (hyphen)', () => {
    const input = '---\nname: old\ndescription: test\n---\n\nBody.';
    const result = convertClaudeCommandToClaudeSkill(input, 'forge-execute-phase');
    ...
    assert.strictEqual(
      nameValue,
      'forge-execute-phase',
      `frontmatter name: must be 'forge-execute-phase' (hyphen form), got '${nameValue}'`
    );
  });

  test('no workflow uses deprecated Skill(skill="forge:<cmd>") colon form', () => {
  ...
  test('every workflow Skill(skill="gsd-<cmd>") resolves to an emitted skill name', () => {
    ...
    for (const f of workflowFiles) {
      ...
      for (const n of extractSkillNamesHyphen(src)) {
        // Skip template expressions (e.g. `forge-${ref.skill}`): these are
