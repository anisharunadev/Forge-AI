 ...
 * - Agent frontmatter (forge-code-reviewer, forge-code-fixer)
...
const PLUGIN_WORKFLOWS_DIR = process.env.GSD_PLUGIN_ROOT || path.join(os.homedir(), '.claude', 'forge-core', 'workflows');
...
describe('CR-AGENT: code review agent frontmatter', () => {
  test('forge-code-reviewer.md has required frontmatter fields', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'forge-code-reviewer.md'), 'utf-8');
    const frontmatter = content.split('---')[1] || '';

    assert.ok(frontmatter.includes('name:'), 'forge-code-reviewer missing name:');
    assert.ok(frontmatter.includes('description:'), 'forge-code-reviewer missing description:');
    assert.ok(frontmatter.includes('tools:'), 'forge-code-reviewer missing tools:');
    assert.ok(frontmatter.includes('color:'), 'forge-code-reviewer missing color:');
  });

  test('forge-code-fixer.md has required frontmatter fields', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'forge-code-fixer.md'), 'utf-8');
    const frontmatter = content.split('---')[1] || '';

    assert.ok(frontmatter.includes('name:'), 'forge-code-fixer missing name:');
    assert.ok(frontmatter.includes('description:'), 'forge-code-fixer missing description:');
    assert.ok(frontmatter.includes('tools:'), 'forge-code-fixer missing tools:');
    assert.ok(frontmatter.includes('color:'), 'forge-code-fixer missing color:');
  });

  test('forge-code-reviewer.md has Read, Bash, Glob, Grep, Write tools', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'forge-code-reviewer.md'), 'utf-8');
    const frontmatter = content.split('---')[1] || '';

    assert.ok(frontmatter.includes('Read'), 'forge-code-reviewer missing Read tool');
    assert.ok(frontmatter.includes('Bash'), 'forge-code-reviewer missing Bash tool');
    assert.ok(frontmatter.includes('Glob'), 'forge-code-reviewer missing Glob tool');
    assert.ok(frontmatter.includes('Grep'), 'forge-code-reviewer missing Grep tool');
    assert.ok(frontmatter.includes('Write'), 'forge-code-reviewer missing Write tool');
  });

  test('forge-code-fixer.md has Read, Edit, Write, Bash, Grep, Glob tools', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'forge-code-fixer.md'), 'utf-8');
    const frontmatter = content.split('---')[1] || '';

    assert.ok(frontmatter.includes('Read'), 'forge-code-fixer missing Read tool');
    assert.ok(frontmatter.includes('Edit'), 'forge-code-fixer missing Edit tool');
    assert.ok(frontmatter.includes('Write'), 'forge-code-fixer missing Write tool');
    assert.ok(frontmatter.includes('Bash'), 'forge-code-fixer missing Bash tool');
  });

  test('forge-code-reviewer.md does not have skills: in frontmatter', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'forge-code-reviewer.md'), 'utf-8');
    const frontmatter = content.split('---')[1] || '';

    assert.ok(!frontmatter.includes('skills:'),
      'forge-code-reviewer has skills: in frontmatter — breaks Gemini CLI');
  });

  test('forge-code-fixer.md does not have skills: in frontmatter', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'forge-code-fixer.md'), 'utf-8');
    const frontmatter = content.split('---')[1] || '';

    assert.ok(!frontmatter.includes('skills:'),
      'forge-code-fixer has skills: in frontmatter — breaks Gemini CLI');
  });

  test('forge-code-fixer.md rollback uses git checkout (not Write tool)', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'forge-code-fixer.md'), 'utf-8');
    assert.ok(content.includes('git checkout --'),
      'forge-code-fixer rollback should use git checkout -- {file} for atomic rollback');
    assert.ok(!content.includes('PRE_FIX_CONTENT'),
      'forge-code-fixer should not use PRE_FIX_CONTENT in-memory capture (use git checkout instead)');
  });

  test('forge-code-fixer.md success_criteria consistent with rollback strategy (git checkout)', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'forge-code-fixer.md'), 'utf-8');
    ...
    assert.ok(successCriteria.includes('git checkout'),
      'forge-code-fixer success_criteria must reference git checkout rollback');
    assert.ok(!successCriteria.includes('Write tool with captured'),
      'forge-code-fixer success_criteria must not say Write tool for rollback');
  });

  test('forge-code-fixer.md flags logic-bug fixes for human review', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'forge-code-fixer.md'), 'utf-8');
    assert.ok(content.includes('requires human verification'),
      'forge-code-fixer should flag logic-bug fixes as requiring human verification');
  });

  test('forge-code-reviewer.md REVIEW.md spec includes files_reviewed_list field', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'forge-code-reviewer.md'), 'utf-8');
    assert.ok(content.includes('files_reviewed_list'),
      'forge-code-reviewer REVIEW.md frontmatter spec must include files_reviewed_list for --auto scope persistence');
  });
});
...
describe('CR-CMD: code review command structure', () => {
  test('code-review.md has correct frontmatter name: forge:code-review', () => {
    const content = fs.readFileSync(path.join(COMMANDS_DIR, 'code-review.md'), 'utf-8');
    const frontmatter = content.split('---')[1] || '';

    assert.ok(frontmatter.includes('name: forge:code-review'),
...
describe('CR-WORKFLOW: code review workflow structure', () => {
  ...
  test('code-review.md workflow references forge-code-reviewer agent', () => {
    const content = fs.readFileSync(path.join(WORKFLOWS_DIR, 'code-review.md'), 'utf-8');

    assert.ok(content.includes('forge-code-reviewer'),
      'code-review.md workflow does not reference forge-code-reviewer agent');
  });
  ...
  test('code-review-fix.md workflow references forge-code-fixer agent', () => {
    const content = fs.readFileSync(path.join(WORKFLOWS_DIR, 'code-review-fix.md'), 'utf-8');

    assert.ok(content.includes('forge-code-fixer'),
      'code-review-fix.md workflow does not reference forge-code-fixer agent');
  });
...
describe('CR-INTEGRATION: workflow integration points', () => {
  ...
  test('autonomous.md contains forge-code-review skill invocation', () => {
    ...
    // (forge-code-review); colon form (forge:code-review) is the legacy
    ...
    assert.ok(skillNames.includes('forge-code-review'),
      `autonomous.md must invoke Skill(skill="gsd-code-review", ...); found skills: ${JSON.stringify(skillNames)}`);
    assert.ok(!skillNames.includes('forge:code-review'),
      'autonomous.md must not use legacy colon form forge:code-review (canonical is hyphen form)');
  });

  test('autonomous.md auto-fix uses consolidated forge-code-review --fix invocation (#2790)', () => {
    // After #2790, forge-code-review-fix was absorbed into forge-code-review as
    // the --fix flag. The autonomous workflow must invoke the consolidated
    // form, not the deleted forge-code-review-fix skill.
    const content = fs.readFileSync(path.join(WORKFLOWS_DIR, 'autonomous.md'), 'utf-8');

    const invocations = parseWorkflowSkillInvocations(content);
    const skillNames = invocations.map(inv => inv.skill);
    assert.ok(!skillNames.includes('forge-code-review-fix'),
      `autonomous.md must not invoke deleted forge-code-review-fix skill (consolidated into --fix); found: ${JSON.stringify(skillNames)}`);
    assert.ok(!skillNames.includes('forge:code-review-fix'),
      'autonomous.md must not use legacy colon form forge:code-review-fix');

    // Find a forge-code-review invocation that carries the --fix flag (the
    // consolidated auto-fix entry point).
    const fixInvocation = invocations.find(inv => {
      if (inv.skill !== 'forge-code-review') return false;
  ...
  test('autonomous.md contains --auto flag on consolidated --fix invocation (#2790)', () => {
    ...
    // Find the forge-code-review invocation that carries --fix (the consolidated
    ...
    assert.ok(argTokens.has('--auto'),
      `autonomous.md forge-code-review-fix args missing --auto flag; got args="${fixInvocation.args}"`);
  });
});
