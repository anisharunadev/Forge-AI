...
/**
 * Tests for /forge-edit-phase (#2617)
 *
 * Covers:
...
const COMMAND_PATH = path.join(ROOT, 'commands', 'forge', 'phase.md');
const WORKFLOW_PATH = path.join(ROOT, 'forge-core', 'workflows', 'edit-phase.md');

// ─── File existence ──────────────────────────────────────────────────────────

describe('edit-phase: file existence', () => {
  test('commands/forge/phase.md exists (absorbed edit-phase in #2790)', () => {
    assert.ok(fs.existsSync(COMMAND_PATH), 'commands/forge/phase.md should exist (consolidates edit-phase)');
  });

  test('forge-core/workflows/edit-phase.md exists', () => {
    assert.ok(fs.existsSync(WORKFLOW_PATH), 'forge-core/workflows/edit-phase.md should exist');
  });
});
...
describe('edit-phase: command file structure', () => {
  test('consolidated phase.md has correct name frontmatter (#2790)', () => {
    ...
    assert.ok(/^name:\s*forge:phase/m.test(content), 'name should be forge:phase (consolidated)');
  });
...
describe('edit-phase: documentation registration', () => {
  test('INVENTORY.md routes edit-phase workflow through consolidated /forge-phase --edit (#2790)', () => {
    // #2790 absorbed /forge-edit-phase into /forge-phase as the --edit flag. The
    ...
    // "Invoked by" column documents /forge-phase --edit (not the deleted form).
    ...
    assert.ok(
      /\/forge-phase\s+--edit/.test(invokedBy),
      `edit-phase.md row must list "/forge-phase --edit" as caller; got: "${invokedBy}"`
    );
    assert.ok(
      !/\/forge-edit-phase\b/.test(invokedBy),
      `edit-phase.md row must not still cite the deleted /forge-edit-phase command; got: "${invokedBy}"`
    );
  });
  ...
  test('INVENTORY-MANIFEST.json contains /forge-phase in commands (#2790: edit-phase absorbed into phase.md)', () => {
    // #2790: /forge-edit-phase was absorbed into /forge-phase as the --edit flag.
    // The manifest now records /forge-phase instead of /forge-edit-phase.
    const manifest = JSON.parse(
      fs.readFileSync(path.join(ROOT, 'docs', 'INVENTORY-MANIFEST.json'), 'utf-8')
    );
    assert.ok(
      manifest.families.commands.includes('/forge-phase'),
      'INVENTORY-MANIFEST.json must list /forge-phase in commands (absorbed /forge-edit-phase via #2790)'
    );
  });
  ...
  test('docs/COMMANDS.md documents /forge-phase (absorbed /forge-edit-phase via --edit flag, #2790)', () => {
    const commands = fs.readFileSync(
      path.join(ROOT, 'docs', 'COMMANDS.md'),
      'utf-8'
    );
    assert.ok(
      commands.includes('/forge-phase'),
      'docs/COMMANDS.md must document /forge-phase (which absorbed /forge-edit-phase via --edit flag in #2790)'
    );
  });
});
