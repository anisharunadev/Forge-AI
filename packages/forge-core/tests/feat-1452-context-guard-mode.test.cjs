 ...
 *
 * Modes:
 *   "warn"  (default) — emit a structured warning + recommend /forge:pause-work
...
describe('execute-phase.md documents the context_guard step', () => {
  ...
  beforeEach(() => {
    executePhase = fs.readFileSync(
      path.join(REPO_ROOT, 'forge-core', 'workflows', 'execute-phase.md'),
      'utf-8',
    );
    ...
    const refPath = path.join(REPO_ROOT, 'forge-core', 'references', 'execute-phase-context-guard.md');
...
describe('context-budget.md documents POOR-tier trigger action', () => {
  ...
  beforeEach(() => {
    contextBudget = fs.readFileSync(
      path.join(REPO_ROOT, 'forge-core', 'references', 'context-budget.md'),
      'utf-8',
    );
  });
...
describe('planning-config.md documents workflow.context_guard_mode', () => {
  test('includes the key in the reference table', () => {
    const planningConfig = fs.readFileSync(
      path.join(REPO_ROOT, 'forge-core', 'references', 'planning-config.md'),
      'utf-8',
    );
    assert.ok(
