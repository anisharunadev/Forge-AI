 ...
 *
 * Files affected: agents/forge-debugger.md (configDir = ~/.claude) and
 * forge-core/workflows/update.md (comment with e.g. ~/.claude).
 */
...
describe('convertClaudeToAntigravityContent bare path replacement (#2418)', () => {
  describe('global install', () => {
    ...
    test('still replaces ~/.claude/ (with trailing slash) correctly', () => {
      const input = 'See ~/.claude/forge-core/workflows/';
      const result = convertClaudeToAntigravityContent(input, true);
      assert.ok(
        result.includes('~/.gemini/antigravity/forge-core/workflows/'),
    ...
    test('does not double-replace ~/.claude/ paths', () => {
      const input = 'See ~/.claude/forge-core/';
      const result = convertClaudeToAntigravityContent(input, true);
  ...
  describe('installed files contain no bare ~/.claude references after conversion', () => {
    ...
    test('forge-debugger.md has no leaked ~/.claude after global Antigravity conversion', () => {
      const debuggerPath = path.join(repoRoot, 'agents', 'forge-debugger.md');
      ...
      assert.strictEqual(
        matches, null,
        `forge-debugger.md still contains leaked .claude paths after Antigravity conversion: ${matches}`
      );
    });

    test('update.md has no leaked ~/.claude after global Antigravity conversion', () => {
      const updatePath = path.join(repoRoot, 'forge-core', 'workflows', 'update.md');
