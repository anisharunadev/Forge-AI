 ...
 * commands exercised through forge-tools.cjs via execSync.
 *
 ...
 *
 * `config-ensure-section` is invoked through a spawned `forge-tools.cjs` child.
 ...
 * stderr (an OS-level kill, not a forge-tools application error; see the
...
describe('config-ensure-section command', () => {
  ...
  test('creates config.json with expected structure and types', () => {
    ...
    // Verify structure and types — exact values may vary if ~/.forge/defaults.json exists
...
describe('config-new-project command', () => {
  ...
  test('creates full config with all expected keys', () => {
    ...
    assert.strictEqual(config.git.phase_branch_template, 'forge/phase-{phase}-{slug}');
    assert.strictEqual(config.git.milestone_branch_template, 'forge/{milestone}-{slug}');
...
describe('config-set/config-get context', () => {
  ...
  test('all three context profile files exist', () => {
    const contextsDir = path.join(__dirname, '..', 'forge-core', 'contexts');
