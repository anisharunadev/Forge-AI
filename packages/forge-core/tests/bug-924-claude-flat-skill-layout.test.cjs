 ...
 *
 * PR #883 accidentally nested concrete forge-* skills 3 levels deep for the
 * Claude global install:
 *
 *   ~/.claude/skills/forge-ns-<router>/skills/<stem>/SKILL.md
 *
 ...
 * Direct `Skill(skill="forge-plan-phase")` calls stopped working.
 *
 * Fix: revert Claude to the FLAT layout — concrete skills at the top level:
 *
 *   ~/.claude/skills/forge-<name>/SKILL.md
 *
...
const COMMANDS_GSD = path.join(ROOT, 'commands', 'forge');
...
const { applySurface } = require('../forge-core/bin/lib/surface.cjs');
...
describe('bug-924: claude global install uses flat skill layout (concrete skills discoverable)', () => {
  ...
  before(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-924-claude-flat-'));
  ...
  test('claude global: concrete skills are at the TOP LEVEL of skills/ (flat, directly discoverable)', () => {
    ...
    const topLevel = fs.readdirSync(skillsDir).filter((n) => n.startsWith('forge-'));

    // Flat layout must have MANY more than 6 top-level forge-* entries (concrete skills).
    // Pre-#924-fix nested layout had exactly 6 (only routers). Flat must have >= 60.
    assert.ok(
      topLevel.length >= 60,
      `Claude global must have >= 60 forge-* top-level skill dirs (concrete flat layout). ` +
      `Got ${topLevel.length}: [${topLevel.slice(0, 10).join(', ')}${topLevel.length > 10 ? ', …' : ''}]. ` +
      'Nested layout detected — #924 regression: Claude must be flat.',
    );
  });

  test('claude global: forge-plan-phase is directly at the top level of skills/', () => {
    const skillsDir = path.join(tmpDir, 'skills');
    const planPhaseDir = path.join(skillsDir, 'forge-plan-phase');
    assert.ok(
      fs.existsSync(path.join(planPhaseDir, 'SKILL.md')),
      `skills/forge-plan-phase/SKILL.md must exist at top level for Claude global install. ` +
      'Concrete skill buried in nested layout — #924 regression.',
    );
  });

  test('claude global: forge-execute-phase is directly at the top level of skills/', () => {
    const skillsDir = path.join(tmpDir, 'skills');
    assert.ok(
      fs.existsSync(path.join(skillsDir, 'forge-execute-phase', 'SKILL.md')),
      `skills/forge-execute-phase/SKILL.md must exist at top level for Claude global install.`,
    );
  });

  test('claude global: forge-code-review is directly at the top level of skills/', () => {
    const skillsDir = path.join(tmpDir, 'skills');
    assert.ok(
      fs.existsSync(path.join(skillsDir, 'forge-code-review', 'SKILL.md')),
      `skills/forge-code-review/SKILL.md must exist at top level for Claude global install.`,
    );
  });

  test('claude global: forge-ns-workflow is at the top level as a concrete skill (no nested skills/ subdir)', () => {
    const skillsDir = path.join(tmpDir, 'skills');
    const nsWorkflowDir = path.join(skillsDir, 'forge-ns-workflow');
    assert.ok(
      fs.existsSync(path.join(nsWorkflowDir, 'SKILL.md')),
      `skills/forge-ns-workflow/SKILL.md must exist at top level (router as concrete skill).`,
    );

    // In the FLAT layout, forge-ns-workflow/ must NOT have a skills/ subdir.
    // A skills/ subdir means nested layout was applied (the #924 regression).
    assert.ok(
      !fs.existsSync(path.join(nsWorkflowDir, 'skills')),
      `skills/forge-ns-workflow/skills/ must NOT exist in flat layout (nested layout detected — #924 regression).`,
    );
  });

  test('claude global: no concrete skill is nested under forge-ns-*/skills/<stem>/SKILL.md', () => {
    const skillsDir = path.join(tmpDir, 'skills');
    const topLevel = fs.readdirSync(skillsDir).filter((n) => n.startsWith('forge-ns-'));
...
describe('bug-924: applySurface on claude preserves flat layout (no re-nesting)', () => {
  ...
  before(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-924-surface-'));
    installRuntimeArtifacts('claude', tmpDir, 'global', RESOLVED_FULL);
  });
  ...
  test('claude: applySurface keeps concrete skills at the top level (flat, no re-nesting)', () => {
    ...
    // Sanity: install must produce flat layout (>= 60 top-level forge-* dirs)
    const topLevelAfterInstall = fs.readdirSync(skillsDir).filter((n) => n.startsWith('forge-'));
    assert.ok(
      topLevelAfterInstall.length >= 60,
      `Install must produce flat layout with >= 60 forge-* dirs. Got ${topLevelAfterInstall.length}.`,
    );
    ...
    const topLevelAfterSurface = fs.readdirSync(skillsDir).filter((n) => n.startsWith('forge-'));
    assert.ok(
      topLevelAfterSurface.length >= 60,
      `After applySurface: must still have >= 60 forge-* top-level dirs (flat). ` +
      `Got ${topLevelAfterSurface.length}. Re-nesting detected.`,
    );

    // forge-plan-phase must remain directly accessible
    assert.ok(
      fs.existsSync(path.join(skillsDir, 'forge-plan-phase', 'SKILL.md')),
      'After applySurface: forge-plan-phase/SKILL.md must remain at top level.',
    );
  });
});
