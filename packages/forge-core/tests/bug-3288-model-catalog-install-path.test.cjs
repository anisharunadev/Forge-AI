 ...
 *      forge-core/bin/shared/model-catalog.json (co-located inside the
 *      forge-core/ payload).
 *
...
const MODEL_CATALOG_CJS = path.join(REPO_ROOT, 'forge-core', 'bin', 'lib', 'model-catalog.cjs');
const MODEL_CATALOG_JSON = path.join(REPO_ROOT, 'forge-core', 'bin', 'shared', 'model-catalog.json');
...
//   <tmp>/.claude/forge-core/bin/lib/model-catalog.cjs  (copy of real file)
...
describe('bug #3288: model-catalog.cjs install-layout resolution', () => {
  ...
  test('OLD layout (3-level __dirname, no co-located json) fails to require', () => {
    ...
    //   <tmpRoot>/.claude/forge-core/bin/lib/model-catalog.cjs  (copy of the real CJS)
    //   sdk/shared/model-catalog.json                              ABSENT
    const gsdLibDir = path.join(tmpRoot, '.claude', 'forge-core', 'bin', 'lib');
...
`;
  ...
  test('NEW layout (co-located bin/shared/model-catalog.json) resolves correctly', () => {
    ...
    //   <tmpRoot>/.claude/forge-core/bin/lib/model-catalog.cjs (copy of real CJS)
    //   <tmpRoot>/.claude/forge-core/bin/shared/model-catalog.json (co-located copy)
    const gsdBinDir = path.join(tmpRoot, '.claude', 'forge-core', 'bin');
