 ...
 * created under GSD_TEMP_DIR (path.join(os.tmpdir(), 'forge')), not directly
...
describe('bug-866: profile-pipeline temp dirs under GSD_TEMP_DIR', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('forge-866-');
  ...
  test('extract-messages output_file is under GSD_TEMP_DIR, not os.tmpdir() root', () => {
    ...
    const expectedGsdTempDir = path.join(isolatedSysTmp, 'forge');
    ...
    // Must NOT be directly under the isolated systmp root (i.e., no forge-pipeline-*
    // at depth 1 of isolatedSysTmp).
    const rel = path.relative(isolatedSysTmp, outputFile);
    const depth1Dir = rel.split(path.sep)[0];
    assert.ok(
      !depth1Dir.startsWith('forge-pipeline-'),
      `output_file must not be in isolatedSysTmp/forge-pipeline-* but got depth-1 dir: "${depth1Dir}"`
    );
  });
  ...
  test('profile-sample output_file is under GSD_TEMP_DIR, not os.tmpdir() root', () => {
    ...
    const depth1Dir = rel.split(path.sep)[0];
    assert.ok(
      !depth1Dir.startsWith('forge-profile-'),
      `output_file must not be in isolatedSysTmp/forge-profile-* but got depth-1 dir: "${depth1Dir}"`
    );
  });
});
