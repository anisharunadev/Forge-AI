 ...
 *
 * `/forge-ingest-docs` was broken because:
 * 1. `workflows/ingest-docs.md` called `forge-sdk query init.ingest-docs` but the
 *    installed binary is `forge-tools` (not `forge-sdk`).
 * 2. `forge-tools init` had no `ingest-docs` case in its dispatch switch.
 *
 * The fix:
 * - Added `case 'ingest-docs'` to the `init` switch in `forge-tools.cjs`.
 * - Exported `cmdInitIngestDocs` from `init.cjs`.
 * - Updated `workflows/ingest-docs.md` to call `forge-tools init ingest-docs`.
 *
...
const WORKFLOW_FILE = path.join(REPO_ROOT, 'forge-core', 'workflows', 'ingest-docs.md');
...
describe('bug-2801: forge-tools init ingest-docs handler exists', () => {
  let tmpDir;

  beforeEach(() => {
    tmpDir = createTempProject('forge-test-2801-');
  });
...
describe('bug-2801: ingest-docs.md workflow calls forge-tools not forge-sdk', () => {
  test('no bash code block in ingest-docs.md calls forge-sdk', () => {
    ...
    // since forge-sdk can appear in subshell expansions like $(forge-sdk query ...).
    ...
    assert.deepStrictEqual(
      sdkCalls,
      [],
      `workflow bash blocks still reference forge-sdk (should use forge-tools): ${sdkCalls.join(', ')}`
    );
  });

  test('ingest-docs.md init step uses the forge_run launcher (#637)', () => {
    const content = fs.readFileSync(WORKFLOW_FILE, 'utf-8');
    ...
    const bashLines = [...content.matchAll(codeBlockRe)]
      .flatMap((m) => m[1].split('\n'))
      .filter((l) => !/^\s*#/.test(l));
    // #637 routes ingest-docs through the resolved `forge_run` launcher instead of
    // the hardcoded `node "$HOME/.../forge-tools.cjs"` path (which misses global
    // installs). The legacy bare `forge-tools` form remains the bug and is still
    // rejected by bug-2851's repo-wide guard.
    const initLine = bashLines.find((l) =>
      /\bforge_run\s+init\s+ingest-docs\b/.test(l)
    );
    assert.ok(initLine, 'workflow must invoke init ingest-docs via the forge_run launcher (#637)');
  });

  test('cmdInitIngestDocs is exported from init.cjs', () => {
    const init = require(path.join(REPO_ROOT, 'forge-core', 'bin', 'lib', 'init.cjs'));
