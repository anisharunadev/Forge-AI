...
/**
 * Regression for #3677 — installed agent bodies leak `/forge:<cmd>` colon refs
 ...
 *     does no namespace work). The retired `/forge:<cmd>` colon refs leak.
 *
...
describe('bug #3677 — agent body colon-namespace leak (Claude / Qwen / Hermes)', () => {
  ...
  describe('C — normalizeAgentBodyForRuntime applies transformContentToHyphen iff predicate is true', () => {
    ...
    const inputBody = [
      '# Agent prose',
      '',
      'Run `/forge:execute-phase 1 --tdd` to execute the phase.',
      'Then `/forge:verify-work 1` to verify.',
      'Reference unchanged: `forge-sdk query commit` (this is a CLI binary, not a slash command).',
    ].join('\n');
    // Only known commands from commands/forge/*.md should be rewritten; forge-sdk
    ...
    test('C1: claude — rewrites both colon refs to hyphen', () => {
      ...
      assert.ok(!out.includes('/forge:execute-phase'), 'colon form for execute-phase must be gone');
      assert.ok(!out.includes('/forge:verify-work'), 'colon form for verify-work must be gone');
      assert.ok(out.includes('forge-sdk query commit'), 'forge-sdk (CLI binary) must not be touched');
    });

    test('C2: qwen — same transform applies', () => {
      const out = normalizeAgentBodyForRuntime(inputBody, 'qwen', cmdNames);
      assert.ok(out.includes('/forge-execute-phase'));
      assert.ok(!out.includes('/forge:execute-phase'));
    });
    ...
    test('C4: gemini — colon refs preserved (intentional namespace)', () => {
      ...
      assert.ok(out.includes('/forge:execute-phase'), 'Gemini intentionally uses colon namespace; do not rewrite');
      assert.ok(!out.includes('/forge-execute-phase'), 'Gemini agents must NOT have hyphen form');
    });
  ...
  describe('D — sanity check: the underlying transform actually works against real cmd names', () => {
    test('D1: transformContentToHyphen rewrites /forge:<cmd> to /forge-<cmd> for known cmds only', () => {
      const out = transformContentToHyphen(
        'A /forge:execute-phase B /forge:unknown-cmd C /forge-sdk D',
        ['execute-phase'],
      );
      assert.ok(out.includes('/forge-execute-phase'), 'known cmd rewritten');
      assert.ok(out.includes('/forge:unknown-cmd'), 'unknown cmd preserved (longest-first matcher only rewrites registered names)');
      assert.ok(out.includes('/forge-sdk'), 'forge-sdk (binary, not slash command) preserved');
    });
  });
  ...
  // worth keeping: real-source efficacy against every `agents/forge-*.md` (the
  ...
  describe('E — real-source efficacy + idempotence (ported from #3681, credit: johnzilla)', () => {
    ...
    // Roster regex matches any registered command in `forge:<cmd>` form with a
    // negative lookbehind (so `myforge:foo` is ignored) and a non-word lookahead
    // (so `plan-phase-extra` is not a false match for `plan-phase`).
    const roster = () => new RegExp(
      `(?<![a-zA-Z0-9_-])forge:(${[...cmdNames].sort((a, b) => b.length - a.length).join('|')})(?=[^a-zA-Z0-9_-]|$)`,
    );
    ...
    test('E1: every agents/forge-*.md transforms clean — no roster colon refs survive', () => {
      ...
      for (const f of fs.readdirSync(agentsDir)) {
        if (!f.startsWith('forge-') || !f.endsWith('.md')) continue;
    ...
    test('E2: idempotent — transform of already-hyphenated input is a no-op', () => {
      const input = 'use /forge-plan-phase next, then /forge-execute-phase';
      assert.strictEqual(
    ...
    test('E3: word boundary — /forge:plan-phase-extra is not a roster match', () => {
      assert.strictEqual(
        transformContentToHyphen('/forge:plan-phase-extra', cmdNames),
        '/forge:plan-phase-extra',
      );
    });

    test('E4: rewrites bare `forge:<cmd>` shorthand (no leading slash)', () => {
      const out = transformContentToHyphen(
        'Spawned by the forge:execute-phase orchestrator.',
        cmdNames,
      );
      assert.strictEqual(out, 'Spawned by the forge-execute-phase orchestrator.');
    });
  });
});
