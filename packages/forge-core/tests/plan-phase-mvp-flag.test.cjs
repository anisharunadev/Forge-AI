...
const WORKFLOW = path.join(__dirname, '..', 'forge-core', 'workflows', 'plan-phase.md');
...
function parseWorkflowContract(content) {
  ...
  const plannerLines = lines.filter(line => line.includes('planner') || line.includes('forge-planner'));
...
describe('plan-phase workflow — --mvp flag', () => {
  ...
  test('planner spawn passes MVP_MODE to forge-planner', () => {
