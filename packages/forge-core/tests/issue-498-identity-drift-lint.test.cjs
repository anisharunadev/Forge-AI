...
const SEAM = { packageName: '@openforge/get-shit-done-redux', repoSlug: 'forge-ai/get-shit-done-redux' };

describe('Issue #498: findCoordinateDrift (pure)', () => {
  test('a correct package literal is not drift', () => {
    const v = findCoordinateDrift('run npx -y @openforge/get-shit-done-redux@latest', SEAM);
    assert.deepEqual(v, []);
  });

  test('a stale package literal (post-rename) is flagged', () => {
    const v = findCoordinateDrift('npx @openforge/get-shit-done-classic@latest', SEAM);
    assert.equal(v.length, 1);
    assert.equal(v[0].found, '@openforge/get-shit-done-classic');
    assert.equal(v[0].expected, SEAM.packageName);
    assert.equal(v[0].kind, 'package');
  });

  test('a different package (@openforge/forge-sdk) is NOT a forge-core coordinate', () => {
    assert.deepEqual(findCoordinateDrift("require('@openforge/forge-sdk')", SEAM), []);
  });

  test('a correct github repo slug is not drift', () => {
    const v = findCoordinateDrift('https://github.com/forge-ai/get-shit-done-redux/issues', SEAM);
    assert.deepEqual(v, []);
  });
  ...
  test('reports 1-based line numbers', () => {
    const text = 'line1\nnpx @openforge/get-shit-done-OLD@latest\nline3';
