 ...
 *
 * Agent definitions in `agents/forge-*.md` are loaded verbatim into the agent's
 ...
 *      extracting shared boilerplate to `forge-core/references/`, not a +N bump.
 ...
 *   - https://github.com/forge-ai/forge-core/issues/717  (bytes, not lines)
...
const isGsdAgent = (f) => f.startsWith('forge-');
...
//   XL      56 KiB — high-water forge-debugger 51,043 → ~6.3 KB headroom
//   LARGE   48 KiB — high-water forge-executor 42,342 → ~6.8 KB headroom
//   DEFAULT 24 KiB — high-water forge-ui-researcher 19,095 → ~5.5 KB headroom
...
describe('SIZE: agent tier hard caps (issue #1074)', () => {
  ...
  // it — crossing it means extract shared boilerplate to forge-core/references/.
  for (const agent of ALL_AGENTS) {
    ...
    test(`${agent} (${tier}) stays within the ${tier} hard cap (${cap} bytes)`, () => {
      ...
      assert.ok(
        ...
        `to forge-core/references/ and load it lazily.`
      );
    });
  }
});
  ...
  test('every agent matches its committed baseline', () => {
    ...
    assertFileBaseline({
      ...
      updateHint:
        ...
        'then justify any growth in your PR (or extract shared boilerplate to forge-core/references/).',
    });
  });
});
