...
// commands/forge/*.md text IS what the runtime loads — testing description
 ...
 *
 * Verifies:
 * 1. All skill descriptions in commands/forge/*.md are <= 100 chars
...
/**
 * Get all .md files in commands/forge/ with their descriptions.
 */
...
describe('description length budget', () => {
  test('all commands/forge/*.md descriptions are <= 100 chars', () => {
...
describe('lint-descriptions.cjs', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-lint-desc-test-'));
  });
