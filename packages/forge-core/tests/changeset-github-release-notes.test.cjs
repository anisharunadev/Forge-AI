...
function createTaggedRepo() {
  const repo = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-release-notes-'));
  ...
  writeFragment(repo, 'fix-install-sdk', 'Fixed', 101, '**`forge-sdk` now installs reliably** — persistent PATH is checked.');
  writeFragment(repo, 'remove-intel-noise', 'Removed', 102, '**`forge-intel-updater` no longer emits layout detection noise** — ordinary projects stay quiet.');
...
describe('changeset github release notes: tag-range renderer (#3382)', () => {
  ...
  test('CLI writes a notes file suitable for gh release edit --notes-file', () => {
    ...
    const generated = renderGithubReleaseNotes({
      ...
      installCommand: 'npx @forge-ai/forge-core@latest',
    });
