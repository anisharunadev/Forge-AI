...
const LEGACY_PKG_SIGNAL = 'forge-core' + '-cc';
...
describe('#607 --dry-run flag: spawned installer exits 0 and mutates nothing', () => {
  ...
  beforeEach(() => {
    tmpHome = mkTmp('forge-607-dryhome-');
  });
  ...
  test('exits 0; stdout names artifact and contains "dry"; no install; artifact preserved; no double-print', () => {
    ...
    const legacyHook = path.join(tmpHome, '.gemini', 'hooks', 'forge-old-update-worker.cjs');
    writeFile(legacyHook, `// installed via ${LEGACY_PKG_SIGNAL}\nconsole.log("old worker");`);

    // Seed the legacy shared cache file
    const legacyCache = path.join(tmpHome, '.cache', 'forge', 'forge-update-check.json');
    ...
    // No actual install happened — .claude/forge-core must not exist
    const installDir = path.join(tmpHome, '.claude', 'forge-core');
    assert.equal(
    ...
    const updateCacheFileName = require(
      path.join(REPO_ROOT, 'forge-core', 'bin', 'lib', 'package-identity.cjs')
    ).updateCacheFileName;
    const perPkgCacheFile = path.join(tmpHome, '.cache', 'forge', updateCacheFileName);
  ...
  test('--uninstall --dry-run prints "does not preview --uninstall" warning and exits 0', () => {
    ...
    // No uninstall occurred — .claude/forge-core must not have been removed
...
describe('#607 cleanupLegacyGsdCc: exported helper unit tests', () => {
  ...
  test('dryRun:true — plan lists seeded artifact; nothing removed', () => {
    ...
    const legacyHook = path.join(homeDir, '.gemini', 'hooks', 'forge-old-update-worker.cjs');
    writeFile(legacyHook, `// installed via ${LEGACY_PKG_SIGNAL}\nconsole.log("old worker");`);
  ...
  test('dryRun:false — seeded leftover removed; dev-preferences.md preserved', () => {
    ...
    const devPrefs = path.join(homeDir, '.gemini', 'forge-core', 'dev-preferences.md');
