...
describe('bug #851: Codex adapter documents multi_agent_v1 schema limitation and fallback', () => {
  ...
  //     not equivalent to typed forge-planner/forge-executor execution.
  test('(c) not-equivalent label: generic-agent workaround is labeled as NOT equivalent to typed dispatch', () => {
    ...
    const namesTypedAgent =
      sectionC.includes('forge-planner') ||
      sectionC.includes('forge-executor') ||
      sectionC.includes('typed GSD agent') ||
      sectionC.includes('typed forge-');
    ...
    assert.ok(
      namesTypedAgent,
      'Section C must name at least one typed GSD agent (forge-planner, forge-executor, or "typed GSD agent")',
    );
    assert.ok(
      hasNotEquivalent,
