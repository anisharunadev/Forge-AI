 ...
 *
 * Two bugs in the `audit-open` dispatch case in bin/forge-tools.cjs:
 *
...
describe('audit-open — output shape (#2911)', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempProject('forge-bug-2911-');
  });
  ...
  test('--json mode emits parseable JSON matching auditOpenArtifacts shape', () => {
    ...
    // Shape contract from auditOpenArtifacts() in forge-core/bin/lib/audit.cjs.
