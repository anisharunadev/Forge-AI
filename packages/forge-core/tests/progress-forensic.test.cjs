...
/**
 * Tests for --forensic flag on /forge-progress (#2189)
 *
...
describe('#2189: progress --forensic flag', () => {
  test('progress command argument-hint includes --forensic', () => {
    const command = fs.readFileSync(
      path.join(__dirname, '..', 'commands', 'forge', 'progress.md'), 'utf8'
    );
  ...
  test('COMMANDS.md documents --forensic flag for forge-progress', () => {
    const commands = fs.readFileSync(
      path.join(__dirname, '..', 'docs', 'COMMANDS.md'), 'utf8'
    );
    assert.ok(
      commands.includes('--forensic'),
      'COMMANDS.md should document --forensic flag for forge-progress'
    );
  });
});
 ...
 *
 * /forge-progress reported a phase as complete and routed to the next phase even
...
describe('#1107: progress routing consults verification.status before reporting complete', () => {
  ...
  test('gaps_found routes to plan-phase --gaps (Route V.gaps)', () => {
    ...
    assert.ok(
      ...
      'Route V.gaps must route to /forge:plan-phase {phase} --gaps'
    );
  });
  ...
  test('human_needed routes to verify-work (Route V.human)', () => {
    ...
    assert.ok(
      ...
      'Route V.human must route to /forge:verify-work {phase}'
    );
  });
