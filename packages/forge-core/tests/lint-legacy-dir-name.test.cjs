...
function createTempRepo() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-lint-legacy-test-'));
...
describe('lint-legacy-dir-name — forge-allow-legacy-name marker', () => {
  test('exits 0 when forbidden token is on a line with the allow marker', () => {
    ...
    try {
      ...
      writeFile(dir, 'src/migration.js', `const legacyRoot = '${bare}'; // forge-allow-legacy-name\n`);
