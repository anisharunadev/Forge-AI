...
// Asserts the portable reference doc (forge-core/references/edge-probe.md) keeps its
// worked-example JSON blocks in sync with the source-of-truth fixture files under
// forge-core/references/edge-probe-fixtures/. The fixtures are the canonical data; the
...
const docPath = path.join(__dirname, '..', 'forge-core', 'references', 'edge-probe.md');
const fixturesRoot = path.join(__dirname, '..', 'forge-core', 'references', 'edge-probe-fixtures');
const adrPath = path.join(__dirname, '..', 'docs', 'adr', '550-spec-phase-probe-contract.md');
const specTemplatePath = path.join(__dirname, '..', 'forge-core', 'templates', 'spec.md');
