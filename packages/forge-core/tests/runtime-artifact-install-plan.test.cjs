...
const { createRuntimeArtifactInstallPlan } = require('../forge-core/bin/lib/runtime-artifact-install-plan.cjs');
...
describe('createRuntimeArtifactInstallPlan', () => {
  test('stages layout kinds in order and projects rewritten source dirs', () => {
    const configDir = path.join(os.tmpdir(), 'forge-plan-config');
    const calls = [];
    const rewriteCalls = [];
    const layout = {
      runtime: 'claude',
  ...
  test('uses real command rewrite seam by default', (t) => {
    const stagedCommands = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-install-plan-commands-'));
    const configDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-install-plan-config-'));
    t.after(() => {
