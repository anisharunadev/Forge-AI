...
// `forge-tools commit` against a temp project and assert on its structured
// JSON return envelope plus the git index state. No raw-text matching on
// rendered output.

/**
 * Regression for #3678 — forge-executor force-commits .planning/ files when
 * commit_docs is false.
 *
 * Root cause: the executor agent prompt (agents/forge-executor.md) tells the
 * agent to call `forge-sdk query commit "docs(...)" --files .planning/...`
...
describe('bug #3678 — executor must respect commit_docs:false', () => {
  ...
  describe('A — agent prompt teaches the agent how to handle commit_docs:false', () => {
    test('A1: agent body explicitly references the SDK skipped envelope', () => {
      ...
      assert.ok(
        ...
        'agents/forge-executor.md must teach the agent how to recognize the '
        + 'skipped envelope from `forge-sdk query commit` (one of: '
    ...
    test('A2: agent body explicitly forbids raw git fallback when SDK skips', () => {
      ...
      assert.ok(
        ...
        'agents/forge-executor.md must contain an explicit "do not fall back to '
  ...
  describe('B — SDK behavior: commit_docs:false leaves repo state untouched', () => {
    ...
    test('B1: commit returns committed:false with skipped envelope', () => {
      ...
      assert.ok(result.success, `forge-tools commit should exit 0 even when skipped: ${result.error || ''}`);
  ...
  test('checklist carve-out preserved for intentional skip', () => {
    ...
    assert.ok(
      checklistLine,
      'agents/forge-executor.md must contain a "Final metadata commit made" checklist line',
    );
    assert.ok(
  ...
  describe('C — structural ban on raw force-add in GSD-managed bodies', () => {
    ...
    test('C2: no workflow body contains `git add -f` / `git add --force`', () => {
      const offenders = scanForForceAdd(path.join(REPO_ROOT, 'forge-core', 'workflows'));
      assert.deepStrictEqual(
        offenders,
        [],
