...
const { checkLatestVersion, CHECK_REASON, PACKAGE_NAME } = require(
  path.join(ROOT, 'forge-core', 'bin', 'check-latest-version.cjs'),
);
...
describe('Bug #2992: deterministic latest-version check', () => {
  test('PACKAGE_NAME is the constant @forge-ai/forge-core (no callers can override)', () => {
...
describe('Bug #2992: error paths', () => {
  const { checkLatestVersion, CHECK_REASON } = require(require('node:path').join(__dirname, '..', 'forge-core', 'bin', 'check-latest-version.cjs'));
...
describe('Issue #815: --next dist-tag support', () => {
  ...
  test('buildViewArgs() defaults to the bare latest spec (byte-for-byte unchanged)', () => {
    assert.deepEqual(buildViewArgs(), ['view', '@forge-ai/forge-core', 'version']);
    assert.deepEqual(buildViewArgs('latest'), ['view', '@forge-ai/forge-core', 'version']);
  });

  test('buildViewArgs("next") targets the @next dist-tag', () => {
    assert.deepEqual(buildViewArgs('next'), ['view', '@forge-ai/forge-core@next', 'version']);
  });
