// Migrated to typed-IR (#2974): the forge-session-state.sh and
// forge-phase-boundary.sh hooks now emit Claude Code SessionStart/PostToolUse
...
// plain text. forge-validate-commit.sh already emitted JSON ({ decision,
...
function createTempProject(prefix = 'forge-hook-test-') {
...
describe('hook file validation', () => {
  test('forge-session-state.sh exists', () => {
    const hookPath = path.join(HOOKS_DIR, 'forge-session-state.sh');
    assert.ok(fs.existsSync(hookPath), 'forge-session-state.sh should exist');
  });

  test('forge-validate-commit.sh exists', () => {
    const hookPath = path.join(HOOKS_DIR, 'forge-validate-commit.sh');
    assert.ok(fs.existsSync(hookPath), 'forge-validate-commit.sh should exist');
  });

  test('forge-phase-boundary.sh exists', () => {
    const hookPath = path.join(HOOKS_DIR, 'forge-phase-boundary.sh');
    assert.ok(fs.existsSync(hookPath), 'forge-phase-boundary.sh should exist');
  });

  test('forge-session-state.sh is executable', { skip: isWindows ? 'Windows has no POSIX file permissions' : false }, () => {
    const hookPath = path.join(HOOKS_DIR, 'forge-session-state.sh');
    const stat = fs.statSync(hookPath);
    assert.ok((stat.mode & 0o111) !== 0, 'forge-session-state.sh should be executable');
  });

  test('forge-validate-commit.sh is executable', { skip: isWindows ? 'Windows has no POSIX file permissions' : false }, () => {
    const hookPath = path.join(HOOKS_DIR, 'forge-validate-commit.sh');
    const stat = fs.statSync(hookPath);
    assert.ok((stat.mode & 0o111) !== 0, 'forge-validate-commit.sh should be executable');
  });

  test('forge-phase-boundary.sh is executable', { skip: isWindows ? 'Windows has no POSIX file permissions' : false }, () => {
    const hookPath = path.join(HOOKS_DIR, 'forge-phase-boundary.sh');
    const stat = fs.statSync(hookPath);
    assert.ok((stat.mode & 0o111) !== 0, 'forge-phase-boundary.sh should be executable');
  });
});
...
describe('installer hook registration', () => {
  test('GSD_UNINSTALL_HOOKS includes all 3 opt-in bash hooks', () => {
    ...
    assert.ok(
      GSD_UNINSTALL_HOOKS.includes('forge-validate-commit.sh'),
      'GSD_UNINSTALL_HOOKS must include forge-validate-commit.sh'
    );
    assert.ok(
      GSD_UNINSTALL_HOOKS.includes('forge-session-state.sh'),
      'GSD_UNINSTALL_HOOKS must include forge-session-state.sh'
    );
    assert.ok(
      GSD_UNINSTALL_HOOKS.includes('forge-phase-boundary.sh'),
      'GSD_UNINSTALL_HOOKS must include forge-phase-boundary.sh'
    );
  });
  ...
  test('buildHookCommand generates a command string for forge-validate-commit.sh', () => {
    ...
    const cmd = buildHookCommand(tmpConfigDir, 'forge-validate-commit.sh', { platform: 'linux' });
    ...
    if (cmd !== null) {
      assert.ok(
        cmd.includes('forge-validate-commit.sh'),
...
describe('opt-in gating behavior', { skip: isWindows ? 'bash hooks require unix shell' : false }, () => {
  ...
  test('validate-commit is a no-op when config.json is absent', (t) => {
    ...
    const bareDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-hook-bare-'));
...
describe('hook execution when enabled', { skip: isWindows ? 'bash hooks require unix shell' : false }, () => {
  ...
  test('session-state exits 0 without .planning/ (in enabled project)', (t) => {
    ...
    const noStateDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-hook-nostate-'));
