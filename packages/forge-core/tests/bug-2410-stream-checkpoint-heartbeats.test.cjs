...
/**
 * Bug #2410 — /forge:manager background execute-phase Task fails with
...
describe('bug #2410: checkpoint heartbeat format is user-documented', () => {
  ...
  test('COMMANDS.md documents the [checkpoint] format under /forge-manager', () => {
    const managerIdx = commandsDoc.indexOf('### `/forge-manager`');
    assert.ok(managerIdx !== -1, '/forge-manager section should exist');
    const section = commandsDoc.slice(managerIdx, managerIdx + 4000);
    assert.ok(
      /\[checkpoint\]/.test(section),
      'COMMANDS.md /forge-manager section should document [checkpoint] heartbeat markers'
    );
    assert.ok(
