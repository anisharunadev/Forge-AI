...
/**
 * Bug #2954: keep `help.md` and the live `commands/forge/*` slash surface
 ...
 * either `forge:foo` or `forge-foo` — Claude Code surfaces both as `/forge-foo`),
 * NOT from the filename, because some files (e.g. `ns-context.md`) ship a
 * different slash name (`forge-context`) than their filename suggests.
 *
 * Also covers `do.md`, the dispatcher invoked at runtime by
 * `/forge:progress --do`: any `/forge[-:]<name>` token in its routing table must
...
const HELP_MD = path.join(ROOT, 'forge-core', 'workflows', 'help', 'modes', 'full.md');
const DO_MD = path.join(ROOT, 'forge-core', 'workflows', 'do.md');
...
/**
 * Returns the set of slash-base-names actually shipped under commands/forge/.
 * A "slash-base-name" is the part after `/forge-` — e.g. for frontmatter
 * `name: forge:foo` or `name: forge-foo`, the slash-base-name is `foo`.
 */
function listShippedSlashBaseNames() {
  ...
  for (const entry of fs.readdirSync(COMMANDS_DIR, { withFileTypes: true })) {
    ...
    let base = null;
    if (fmName.startsWith('forge:')) base = fmName.slice(4);
    else if (fmName.startsWith('forge-')) base = fmName.slice(4);
...
function extractSlashReferences(contents) {
  ...
  // paths like @forge-ai/forge-core where `/forge-` appears inside a package URL).
  // Negative lookahead (?![\w-]*\/): excludes filesystem path segments like
  // `/forge-core/bin` where the captured name is followed by a `/`, which would
...
describe('Bug #2954: help.md ↔ commands/forge/ bidirectional parity', () => {
  ...
  test('every shipped /gsd[-:]<name> command is documented in help.md', () => {
    ...
    assert.deepEqual(
      undocumented,
      [],
      `commands shipped under commands/forge/ with no /gsd[-:]<name> reference in help.md: ${undocumented.join(', ')}`,
    );
  });
  ...
  test('every --flag in a command\'s argument-hint appears in help.md', () => {
    ...
    for (const [command, flags] of flagsByCommand) {
      for (const flag of flags) {
        // Accept `/forge-<command> --<flag>` (precise) OR a bare `--<flag>` token
        // anywhere in help.md (good enough for shared flags like `--force` that
        // appear under multiple commands' descriptions).
        const preciseDash = `/forge-${command} --${flag}`;
        const preciseColon = `/forge:${command} --${flag}`;
