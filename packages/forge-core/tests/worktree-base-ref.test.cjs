 ...
 *
 * Seam: forge-core/bin/lib/worktree-base-ref.cjs
...
const MODULE_PATH = path.join(
  __dirname, '..', 'forge-core', 'bin', 'lib', 'worktree-base-ref.cjs'
);
...
describe('evaluateWorktreeBaseDegrade', () => {
  ...
  test('HEAD != origin/HEAD → degrade, reason head-diverged-from-fork, MSG_DIVERGED', () => {
    ...
    const expectedMsg = `⚠ Worktree base mismatch: HEAD (${HEAD_SHA.slice(0, 8)}) differs from origin/HEAD (${FORK_SHA.slice(0, 8)}). Running this phase sequentially on the main working tree. To keep parallel worktrees, set worktree.baseRef:"head" in .claude/settings.local.json (or run: forge-tools worktree set-baseref). See #683.`;
  ...
  test('origin/HEAD fails AND symbolic-ref fails → degrade, reason fork-ref-unknown, MSG_UNKNOWN', () => {
    ...
    assert.strictEqual(result.forkRef, null);
    assert.strictEqual(result.forkSha, null);
    const expectedMsg = `⚠ Cannot determine the worktree fork base (origin/HEAD unresolved). Running this phase sequentially on the main working tree to avoid a base mismatch. To keep parallel worktrees, set worktree.baseRef:"head" in .claude/settings.local.json (or run: forge-tools worktree set-baseref). See #683.`;
    assert.strictEqual(result.message, expectedMsg);
  });
