 ...
 * Tests run against the REAL registry and compiled .cjs in forge-core/bin/lib/.
 */
...
const { setCapabilityState } = require('../forge-core/bin/lib/capability-writer.cjs');
const { readSurface } = require('../forge-core/bin/lib/surface.cjs');

// ─── Helpers ──────────────────────────────────────────────────────────────────

/**
 * Create a temp runtimeConfigDir and cwd pair for a test.
 * - rcd has no .forge-profile file → default 'full' profile → installedSkills='*'
...
describe('capability-writer: setCapabilityState', () => {
  ...
  test('CLI: capability set ui --off exits 0; capability set ui --on exits 0; unknown id exits non-zero', () => {
    ...
    try {
      ...
      const gsdToolsBin = path.resolve(__dirname, '../forge-core/bin/forge-tools.cjs');
  ...
  test('Fix A: disable when explicitAdds holds cap skill stems → skill stems removed, ui.enabled===false, no errors', () => {
    ...
    try {
      ...
      const surfacePath = path.join(rcd, '.forge-surface.json');
