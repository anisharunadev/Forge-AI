 ...
 *   `/forge-execute-phase N --auto` with parallelization: true, use_worktrees: false
...
const WORKFLOW_PATH = path.join(__dirname, '..', 'forge-core', 'workflows', 'execute-plan.md');
...
describe('bug #2661: execute-plan.md update_roadmap gating', () => {
  ...
  test('update_roadmap step exists and invokes roadmap.update-plan-progress', () => {
    ...
    assert.ok(
      /forge_run query roadmap\.update-plan-progress/.test(step),
  ...
  test('use_worktrees: false mode — sync call is gated to fire (the #2661 reproducer)', () => {
    ...
    // After #3797 architectural fix, callsites use forge_run
    assert.ok(
      /IS_WORKTREE.*!=.*"true"[\s\S]*?forge_run query roadmap\.update-plan-progress/.test(step),
  ...
  test('use_worktrees: true mode — sync call does NOT fire (single-writer contract)', () => {
    ...
    const callIdx = bash.search(/forge_run query roadmap\.update-plan-progress/);
...
describe('bug #2661: roadmap update-plan-progress handler (use_worktrees: false)', () => {
  let tmpDir;
  beforeEach(() => { tmpDir = createTempProject('forge-2661-'); });
