 ...
 *
 * /forge-code-review-fix cleanup tail is non-transactional. If the agent is
 ...
 * orphan worktree and branch — a future run, /forge-resume-work, or
 * /forge-progress can detect and complete the recovery.
 */

'use strict';

// allow-test-rule: source-text-is-the-product
// The forge-code-fixer agent's working instructions ARE the product — Claude
...
describe('bug-2839: /forge-code-review-fix cleanup is transactional', () => {
  let agentPath;
  let agentContent;
  let frontmatter;

  before(() => {
    agentPath = path.join(__dirname, '..', 'agents', 'forge-code-fixer.md');
    assert.ok(fs.existsSync(agentPath), 'agents/forge-code-fixer.md must exist');
  ...
  test('agent declares a recovery sentinel filename', () => {
    assert.ok(
      ...
      `forge-code-fixer.md must reference the recovery sentinel ${SENTINEL_NAME} so an interrupted cleanup tail is discoverable (#2839)`
    );
  });
  ...
  test('sentinel records worktree path, branch, and padded_phase as JSON fields', () => {
    for (const key of ['worktree_path', 'branch', 'padded_phase']) {
      assert.ok(
        ...
        `recovery sentinel must record \`${key}\` so a future /forge-resume-work or /forge-progress can locate the orphan state (#2839)`
      );
    }
  });
