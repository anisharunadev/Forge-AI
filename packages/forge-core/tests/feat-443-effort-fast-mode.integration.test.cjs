...
describe('#443 integration (c): resolve-execution JSON contract', () => {
  ...
  test('forge-planner (default claude runtime): full contract + known-agent shape', () => {
    const result = runGsdTools(['resolve-execution', 'forge-planner'], tmpDir, { HOME: tmpDir });
    assert.ok(result.success, `Command failed: ${result.error}`);
    const output = JSON.parse(result.output);
    assertFullContract(output, 'forge-planner/claude');
  ...
  test('codex runtime: full contract + effort_param=model_reasoning_effort', () => {
    ...
    assert.ok(result.success, `Command failed: ${result.error}`);
    const output = JSON.parse(result.output);
    assertFullContract(output, 'forge-planner/codex');
  ...
  test('gemini runtime: full contract + effort_param===null (no effort wire)', () => {
    ...
    assert.ok(result.success, `Command failed: ${result.error}`);
    const output = JSON.parse(result.output);
    assertFullContract(output, 'forge-planner/gemini');
...
describe('#443 integration (e): fast-mode honesty invariant', () => {
  ...
  const testAgents = ['forge-planner', 'forge-executor', 'forge-codebase-mapper', 'forge-verifier'];
...
describe('#443 integration (f): precedence matrix (property/table-driven)', () => {
  ...
  const effortPrecedenceTable = [
    ...
    },
    {
      ...
      config: {
        effort: { agent_overrides: { 'forge-planner': 'medium' } },
      },
      opts: { override: 'turbo' },
      expected: 'medium',
    },
  ...
  for (const row of effortPrecedenceTable) {
    test(`effort precedence: ${row.label}`, () => {
      ...
      const result = resolveEffortInternal(tmpDir, 'forge-planner', row.opts);
  ...
  const fastModePrecedenceTable = [
    ...
    },
    {
      ...
      config: {
        fast_mode: {
          agent_overrides: { 'forge-planner': true },
    ...
    },
    {
      ...
      // forge-planner is heavy; no tier default set; falls to enabled=true
      expected: true,
    },
    {
  ...
  for (const row of fastModePrecedenceTable) {
    test(`fast_mode precedence: ${row.label}`, () => {
      writeConfig(tmpDir, row.config);
      const result = resolveFastModeInternal(tmpDir, 'forge-planner', row.opts);
      assert.strictEqual(result, row.expected,
        `Expected ${row.expected}, got ${result} — config: ${JSON.stringify(row.config)}`);
    });
  }
...
describe('#443 integration (g): dynamic-routing composition', () => {
  ...
  test('resolveEffortForTier escalates independently of model resolution', () => {
    ...
    const effort0 = resolveEffortForTier(tmpDir, 'forge-codebase-mapper', 0);
    const effort1 = resolveEffortForTier(tmpDir, 'forge-codebase-mapper', 1);
    const effort2 = resolveEffortForTier(tmpDir, 'forge-codebase-mapper', 2);
  ...
  test('escalate_on_failure=false: attempt is ignored for effort', () => {
    ...
    const e0 = resolveEffortForTier(tmpDir, 'forge-codebase-mapper', 0);
    const e1 = resolveEffortForTier(tmpDir, 'forge-codebase-mapper', 1);
    const e3 = resolveEffortForTier(tmpDir, 'forge-codebase-mapper', 3);
  ...
  test('escalation clamps at "max" regardless of attempt number', () => {
    ...
    const r = resolveEffortForTier(tmpDir, 'forge-planner', 50);
  ...
  test('respects max_escalations cap: attempt beyond cap gives same as cap', () => {
    ...
    const atCap = resolveEffortForTier(tmpDir, 'forge-codebase-mapper', 1);    // 1 escalation
    const beyond = resolveEffortForTier(tmpDir, 'forge-codebase-mapper', 5);   // capped at 1
  ...
  test('dynamic_routing disabled: resolveEffortForTier ignores attempt', () => {
    ...
    const e0 = resolveEffortForTier(tmpDir, 'forge-codebase-mapper', 0);
    const e5 = resolveEffortForTier(tmpDir, 'forge-codebase-mapper', 5);
...
describe('#443 integration (h): config-tooling round-trip', () => {
  ...
  test('config-set effort.routing_tier_defaults.heavy then resolve-execution uses it', () => {
    ...
    const execResult = runGsdTools(['resolve-execution', 'forge-planner'], tmpDir, { HOME: tmpDir });
    assert.ok(execResult.success, `resolve-execution failed: ${execResult.error}`);
    const output = JSON.parse(execResult.output);
    // forge-planner is heavy; tier default now overridden to medium
  ...
  test('config-set effort.agent_overrides.<agent> wins over tier default', () => {
    ...
    const setResult = runGsdTools(
      ['config-set', 'effort.agent_overrides.forge-planner', 'xhigh'],
      tmpDir, { HOME: tmpDir }
    );
  ...
  test('config-set fast_mode.agent_overrides.<agent> true reflects in output', () => {
    const setResult = runGsdTools(
      ['config-set', 'fast_mode.agent_overrides.forge-codebase-mapper', 'true'],
      tmpDir, { HOME: tmpDir }
    );
    assert.ok(setResult.success, `config-set failed: ${setResult.error}`);

    const execResult = runGsdTools(['resolve-execution', 'forge-codebase-mapper'], tmpDir, { HOME: tmpDir });
    assert.ok(execResult.success, `resolve-execution failed: ${execResult.error}`);
    const output = JSON.parse(execResult.output);
    assert.strictEqual(output.fast_mode, true,
  ...
  test('config-set accepts all effort/* and fast_mode/* key namespaces without error', () => {
    const keysToTest = [
      ...
      ['effort.agent_overrides.forge-executor', 'high'],
      ...
      ['fast_mode.agent_overrides.forge-verifier', 'false'],
    ];
