...
/**
 * Regression tests for #3727 — /forge-code-review N --fix silently no-ops.
 *
...
const FLAGS_LIB = path.join(ROOT, 'forge-core', 'bin', 'lib', 'code-review-flags.cjs');
...
describe('#3727 — parseCodeReviewFlags: typed IR for code-review argv', () => {
  test('code-review workflow parses --fix flag and dispatches forge-code-fixer', () => {
...
describe('#3727 — code-review.md structural dispatch contract', () => {
  ...
  test('dispatch_fix step references forge-code-fixer agent (via code-review-fix.md chain)', () => {
    // The dispatch step must show that fixing will occur, either directly or
    // by loading code-review-fix.md (which in turn spawns forge-code-fixer).
    const stepStart = src.indexOf('<step name="dispatch_fix">');
    const stepEnd = src.indexOf('</step>', stepStart);
    const stepSection = src.slice(stepStart, stepEnd);
    assert.ok(
      stepSection.includes('forge-code-fixer') || stepSection.includes('code-review-fix.md'),
      'dispatch_fix step must reference forge-code-fixer agent or code-review-fix.md workflow'
    );
  });
});
