 ...
 *
 * `forge-sdk query <subcommand> --help` returned the top-level SDK USAGE
 ...
 * before the registry handler / forge-tools.cjs fallback could render
 * useful help.
 *
 * Two-layer fix:
 ...
 *   2. forge-core/bin/forge-tools.cjs — render the top-level usage on
 *      --help instead of erroring. Anti-hallucination invariant from
 *      #1818 is preserved (the destructive command never executes).
 *
 * Tests the integration: invoke forge-tools.cjs the same way the SDK
...
// #3026 CR (Major outside-diff): the SDK fallback wraps forge-tools.cjs.
// When forge-tools emits plain-text help (exit 0), the SDK previously
// JSON.parsed stdout and threw "Unexpected token 'U'". Verify the fix
// by invoking the built SDK end-to-end and asserting:
//   - exit 0
//   - stdout contains the forge-tools usage
...
describe('bug #3026 (CR Major outside-diff): SDK forwards plain-text help from forge-tools fallback', () => {
  test('forge-sdk query phase --help (fallback path) returns usage, not a JSON parse error', (t) => {
    ...
    // registry, so it routes through the forge-tools.cjs fallback. That is
    ...
    // The fallback forge-tools.cjs emits exit 0 with usage on stdout.
    ...
    assert.ok(/Usage:\s*forge-tools/.test(result.stdout) && /Commands:/.test(result.stdout),
      `usage must reach stdout: ${result.stdout}`);
  });
});

describe('bug #3019: forge-tools renders usage on --help instead of erroring', () => {
  test('bare forge-tools (no args) renders usage', () => {
    const result = runGsdTools([]);
    // No args path: error() helper emits to stderr and exits non-zero,
    // but the message body is the usage.
    assert.strictEqual(result.success, false);
    assert.ok(/Usage:\s*forge-tools/.test(result.error));
    assert.ok(/Commands:/.test(result.error));
  });

  test('forge-tools --help renders usage on stdout, exits 0', () => {
  ...
  test('forge-tools -h renders usage on stdout, exits 0', () => {
    const result = runGsdTools(['-h']);
    assert.strictEqual(result.success, true);
    assert.ok(isUsageOutput(result.output));
  });

  test('forge-tools <subcommand> --help renders usage (does not run subcommand)', () => {
