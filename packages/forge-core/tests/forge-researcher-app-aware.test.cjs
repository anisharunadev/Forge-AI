 ...
 *
 * Validates that forge-phase-researcher maps capabilities to architectural
 * tiers before diving into framework-specific research. Also validates
 * that forge-planner and forge-plan-checker consume the Architectural
...
const TEMPLATES_DIR = path.join(__dirname, '..', 'forge-core', 'templates');

// ─── Phase Researcher: Architectural Responsibility Mapping ─────────────────

describe('phase-researcher: Architectural Responsibility Mapping', () => {
  const researcherPath = path.join(AGENTS_DIR, 'forge-phase-researcher.md');
  ...
  test('contains Architectural Responsibility Mapping step', () => {
    assert.ok(
      ...
      'forge-phase-researcher.md must contain "Architectural Responsibility Map"'
    );
  });
...
describe('planner: Architectural Responsibility Map sanity check', () => {
  ...
  test('references Architectural Responsibility Map', () => {
    assert.ok(
      content.includes('Architectural Responsibility Map'),
      'forge-planner.md must reference the Architectural Responsibility Map'
    );
  });
  ...
  test('includes sanity check against the map', () => {
    ...
    assert.ok(
      ...
      'forge-planner.md must include a sanity check against the Architectural Responsibility Map'
    );
  });
});
...
describe('plan-checker: Architectural Tier verification dimension', () => {
  ...
  test('has verification dimension for architectural tier', () => {
    assert.ok(
      ...
      'forge-plan-checker.md must have a verification dimension for architectural tier mapping'
    );
  });
