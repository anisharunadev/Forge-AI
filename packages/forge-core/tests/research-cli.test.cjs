 ...
 * CLI commands (forge-tools dispatch layer).
 *
 * Conventions:
 ...
 *   - HOME is overridden via runGsdTools env param to sandbox ~/.forge/ writes
 */
...
function makeTempDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'forge-research-test-'));
}
...
describe('research-plan: cache hit via pre-seeded store', () => {
  test('returns cache.hit:true for pre-seeded question; no fetch property', () => {
    ...
    try {
      ...
      const researchStore = require('../forge-core/bin/lib/research-store.cjs');
      ...
      // Seed the cache directly, passing homeDir so it writes into tmpDir/.forge/
...
describe('FINDING-1: package-legitimacy check flag parser correctness', () => {
  ...
  test('--bad-flag pkgA pkgB → unknown-flag error on stderr, non-zero exit, pkgA and pkgB not consumed as flag values', () => {
    ...
    try {
      ...
      //
      // Green proof: the current parser (forge-tools.cjs lines 1951-1952) hits
