...
const AUTONOMOUS_PATH = path.join(__dirname, '..', 'forge-core', 'workflows', 'autonomous.md');
...
describe('Workflow .md structural guard (#3718)', () => {
  ...
  // `forge_run check ui-plan-gate` (a CLI command that internally calls checkUiPresence
...
describe('UI gate resolves the helper against RUNTIME_DIR, not the consuming repo (#448)', () => {
  ...
  const GATE_SNIPPET = [
    ...
    'UI_GATE_JS=$(for _c in "$_GSD_RT/forge-core/bin/lib/ui-safety-gate.cjs" "$_GSD_RT/bin/lib/ui-safety-gate.cjs" "$_GSD_RT/.claude/bin/lib/ui-safety-gate.cjs" "$HOME/.claude/forge-core/bin/lib/ui-safety-gate.cjs" "$HOME/.claude/bin/lib/ui-safety-gate.cjs"; do [ -f "$_c" ] && { echo "$_c"; break; }; done)',
  ...
  test('UI text is detected (HAS_UI=0) from a project without bin/lib', () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-consuming-'));
    try {
  ...
  test('UI gate found via forge-core/bin/lib/ in installed layout (no root bin/lib/)', () => {
    // Regression for #448: installed RUNTIME_DIR has forge-core/bin/lib/ but NOT root bin/lib/.
    // The probe must find the helper at the installed path, not silently no-op to HAS_UI=0.
    const fakeRuntime = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-installed-rt-'));
    const consumingProject = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-consuming-'));
    try {
      const installedLibDir = path.join(fakeRuntime, 'forge-core', 'bin', 'lib');
      fs.mkdirSync(installedLibDir, { recursive: true });
      fs.copyFileSync(
        path.join(REPO_ROOT, 'forge-core', 'bin', 'lib', 'ui-safety-gate.cjs'),
      ...
      assert.strictEqual(res.status, 0, `bash failed: ${res.stderr}`);
      assert.strictEqual(res.stdout.trim(), '0',
        'helper must be found via forge-core/bin/lib/ in installed layout and report UI present');
    } finally {
      cleanup(fakeRuntime);
