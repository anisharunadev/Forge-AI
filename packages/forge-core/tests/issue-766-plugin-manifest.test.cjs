...
const identity = require(path.join(ROOT, 'forge-core', 'bin', 'lib', 'package-identity.cjs'));
...
describe('A: .claude-plugin/plugin.json', () => {
  ...
  test('name equals identity.binName ("forge-core")', (t) => {
  ...
  test('commands field is "./commands/forge/" and that dir exists with at least one .md file', (t) => {
    if (!manifest) { t.skip('manifest could not be parsed'); return; }
    assert.equal(manifest.commands, './commands/forge/', 'commands must be "./commands/forge/"');
...
describe('B: hooks/hooks.json', () => {
  ...
  test('forge-context-monitor.js entry has timeout === 10', (t) => {
    if (!hooksConfig) { t.skip('hooks.json could not be parsed'); return; }
    let found = false;
    for (const eventEntries of Object.values(hooksConfig.hooks)) {
      for (const entry of eventEntries) {
        for (const hook of entry.hooks) {
          if (hook.command && hook.command.includes('forge-context-monitor.js')) {
            found = true;
            assert.equal(hook.timeout, 10, 'forge-context-monitor.js must have timeout === 10');
          }
        }
      }
    }
    assert.ok(found, 'forge-context-monitor.js entry was not found in hooks.json');
  });
});
...
describe('C: plugin.json schema validation', () => {
  ...
  test(
    ...
    () => {
      const pluginRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-plugin-validate-'));
      try {
...
describe('D: always-on hook contract drift guard', () => {
  ...
  test('SessionStart: one no-matcher group with forge-ensure-canonical-path.js then forge-check-update.js', () => {
    // #997: forge-ensure-canonical-path.js is wired alongside forge-check-update.js
    // in the single SessionStart no-matcher group. It must run FIRST so the
    // canonical ~/.claude/forge-core path (and its @-include targets) exist before
    ...
    assert.equal(
      noMatcherHooks[0].script, 'forge-ensure-canonical-path.js',
      'forge-ensure-canonical-path.js must be the FIRST SessionStart hook (#997)'
    );
    assert.equal(
      noMatcherHooks[0].timeout, 5,
      'forge-ensure-canonical-path.js must have a small timeout (5s) — symlink setup is fast'
    );
    assert.equal(
      noMatcherHooks[1].script, 'forge-check-update.js',
      'forge-check-update.js must remain a SessionStart hook'
    );
    assert.equal(noMatcherHooks[1].timeout, undefined, 'forge-check-update.js must NOT have a timeout field');
  });

  test('PreToolUse Write|Edit group: forge-prompt-guard.js (timeout 5) + forge-read-guard.js (timeout 5)', () => {
    const map = buildHookMap();
    ...
    assert.equal(hooks[0].script, 'forge-prompt-guard.js', 'first hook must be forge-prompt-guard.js');
    assert.equal(hooks[0].timeout, 5, 'forge-prompt-guard.js must have timeout 5');
    assert.equal(hooks[1].script, 'forge-read-guard.js', 'second hook must be forge-read-guard.js');
    assert.equal(hooks[1].timeout, 5, 'forge-read-guard.js must have timeout 5');
  });

  test('PreToolUse Write|Edit|MultiEdit group: forge-worktree-path-guard.js (timeout 5)', () => {
    const map = buildHookMap();
    const groups = map['PreToolUse'];
    assert.ok(groups, 'PreToolUse must be present in hooks.json');
    ...
    assert.equal(hooks[0].script, 'forge-worktree-path-guard.js', 'hook must be forge-worktree-path-guard.js');
    assert.equal(hooks[0].timeout, 5, 'forge-worktree-path-guard.js must have timeout 5');
  });

  test('PostToolUse Bash|Edit|Write|MultiEdit|Agent|Task group: forge-context-monitor.js (timeout 10)', () => {
    const map = buildHookMap();
    ...
    assert.equal(hooks[0].script, 'forge-context-monitor.js', 'hook must be forge-context-monitor.js');
    assert.equal(hooks[0].timeout, 10, 'forge-context-monitor.js must have timeout 10');
  });

  test('PostToolUse Read group: forge-read-injection-scanner.js (timeout 5)', () => {
    const map = buildHookMap();
    const groups = map['PostToolUse'];
    assert.ok(groups, 'PostToolUse must be present in hooks.json');
    ...
    assert.equal(hooks[0].script, 'forge-read-injection-scanner.js', 'hook must be forge-read-injection-scanner.js');
    assert.equal(hooks[0].timeout, 5, 'forge-read-injection-scanner.js must have timeout 5');
  });
});
...
//
// forge-ensure-canonical-path.js must be shipped + wired so plugin installs get a
// real ~/.claude/forge-core directory (with the immutable bundled subdirs
// symlinked) — otherwise every `@~/.claude/forge-core/...` include in agents /
// commands / templates resolves to nothing and agents fail (#997).
describe('F: #997 forge-ensure-canonical-path.js is shipped and wired', () => {
  const HOOK_BASENAME = 'forge-ensure-canonical-path.js';
...
describe('G: #997 ensureCanonicalPath() behavioural regression', () => {
  const { ensureCanonicalPath, dirLinkType, MANAGED_SUBDIRS } =
    require(path.join(ROOT, 'hooks', 'forge-ensure-canonical-path.js'));
  ...
  beforeEach(() => {
    tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-997-'));
  });
  afterEach(() => {
  ...
  test('plugin layout: creates ~/.claude/forge-core with all subdirs symlinked to the bundle', () => {
    ...
    const canonical = path.join(homeDir, '.claude', 'forge-core');
  ...
  test('no plugin context: CLAUDE_PLUGIN_ROOT unset → no-op (classic/npm install path)', () => {
    ...
    assert.ok(!fs.existsSync(path.join(homeDir, '.claude', 'forge-core')), 'must not create canonical dir');
  });

  test('boundary: bundled forge-core dir missing under plugin root → no-op', () => {
    const pluginRoot = path.join(tmp, 'plugin-empty');
    fs.mkdirSync(pluginRoot, { recursive: true }); // no forge-core/ inside
    const homeDir = path.join(tmp, 'home');
    fs.mkdirSync(path.join(homeDir, '.claude'), { recursive: true });
    const result = ensureCanonicalPath({ pluginRoot, homeDir, platform: 'linux', env: {} });
    assert.equal(result.status, 'noop');
  ...
  test('self-heal: a stale symlink (pointing at a removed prior plugin version) is pruned and recreated', () => {
    ...
    const stalePrior = path.join(tmp, 'plugin-OLD', 'forge-core', 'references');
    ...
    assert.equal(
      fs.realpathSync(linkPath),
      fs.realpathSync(path.join(pluginRoot, 'forge-core', 'references')),
  ...
  test('self-heal: a managed symlink pointing at the wrong (but existing) target is repointed', () => {
    ...
    const otherDir = path.join(tmp, 'plugin-OTHER', 'forge-core', 'workflows');
    ...
    assert.equal(
      ...
      fs.realpathSync(path.join(pluginRoot, 'forge-core', 'workflows')),
      'workflows must be repointed to the current bundle'
    );
  });

  test('security: bundled forge-core that symlinks OUTSIDE the plugin root is rejected', () => {
    const pluginRoot = path.join(tmp, 'plugin-evil');
    fs.mkdirSync(pluginRoot, { recursive: true });
    // Attacker places a symlink at <pluginRoot>/forge-core pointing outside root.
    const outside = path.join(tmp, 'OUTSIDE');
    fs.mkdirSync(path.join(outside, 'references'), { recursive: true });
    fs.symlinkSync(outside, path.join(pluginRoot, 'forge-core'), 'dir');
    const homeDir = path.join(tmp, 'home');
    fs.mkdirSync(path.join(homeDir, '.claude'), { recursive: true });
    const result = ensureCanonicalPath({ pluginRoot, homeDir, platform: 'linux', env: {} });
    ...
    assert.equal(result.reason, 'no-plugin-bundle');
    assert.ok(
      !fs.existsSync(path.join(homeDir, '.claude', 'forge-core', 'references')),
  ...
  test('CLAUDE_CONFIG_DIR honoured: canonical path is created under the custom config dir', () => {
    ...
    assert.equal(result.status, 'ensured');
    assert.equal(result.canonicalDir, path.join(customCfg, 'forge-core'));
    assert.ok(fs.lstatSync(path.join(customCfg, 'forge-core', 'references')).isSymbolicLink());
  });

  test('canonical path is itself a symlink → no-op (never writes links through a user-pointed symlink)', () => {
    // A user pointed ~/.claude/forge-core at some other directory via a symlink.
...
// plugin-installed GSD exposes `forge-core:<skill>` the native Claude Code way.
// The skills are generated from `commands/forge/*.md` by
...
describe('H: skills surface projection (#1596)', () => {
  ...
  test('skills/ dir exists with at least one forge-*/SKILL.md', () => {
    assert.ok(fs.existsSync(SKILLS_DIR), `skills/ dir must exist (run: npm run gen:plugin-skills -- --write): ${SKILLS_DIR}`);
    const entries = fs.readdirSync(SKILLS_DIR, { withFileTypes: true });
    const skillDirs = entries.filter(e => e.isDirectory() && e.name.startsWith('forge-'));
    assert.ok(skillDirs.length > 0, 'skills/ must contain at least one forge-*/ directory');
  ...
  test('parity: one skill dir per command file (DEFECT.GENERATIVE-FIX)', () => {
    const commandsDir = path.resolve(ROOT, 'commands', 'forge');
    const commandFiles = fs.readdirSync(commandsDir).filter(f => f.endsWith('.md'));
    const skillDirs = fs.readdirSync(SKILLS_DIR, { withFileTypes: true })
      .filter(e => e.isDirectory() && e.name.startsWith('forge-'));
    assert.equal(
      skillDirs.length, commandFiles.length,
      `skills/forge-*/ count (${skillDirs.length}) must equal commands/forge/*.md count (${commandFiles.length}). ` +
