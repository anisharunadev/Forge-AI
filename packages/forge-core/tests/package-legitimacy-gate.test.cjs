...
const RESEARCHER = path.join(AGENTS, 'forge-phase-researcher.md');
const PLANNER = path.join(AGENTS, 'forge-planner.md');
const EXECUTOR = path.join(AGENTS, 'forge-executor.md');
...
describe('forge-phase-researcher.md — package-legitimacy seam invocation', () => {
  let model;
  ...
  test('invokes forge-tools query package-legitimacy check inside a fenced code block', () => {
...
describe('forge-phase-researcher.md — Package Legitimacy Audit section in template', () => {
  ...
  test('Package Legitimacy Audit table has required columns', () => {
    ...
    // 'slopcheck' column renamed to 'Verdict' to reflect the code seam (forge-tools query package-legitimacy)
...
describe('forge-phase-researcher.md — ecosystem-specific package verification', () => {
  let model;
...
describe('forge-phase-researcher.md — no npx --yes auto-download', () => {
  let model;
...
describe('forge-phase-researcher.md — WebSearch-origin package tagging', () => {
  let model;
...
describe('forge-planner.md — checkpoint gate for [ASSUMED]/[SUS] packages', () => {
  let model;
...
describe('forge-planner.md — supply-chain row in threat_model template', () => {
  let planTemplate;
...
describe('forge-planner.md — no npx --yes auto-download', () => {
  test('does not invoke npx --yes inside a code block', () => {
    const model = readModel(PLANNER);
    const found = model.codeBlocks.some((block) => hasAllTokens(block, ['npx', '--yes']));
...
describe('forge-executor.md — package installs excluded from RULE 3 auto-fix', () => {
  let model;
