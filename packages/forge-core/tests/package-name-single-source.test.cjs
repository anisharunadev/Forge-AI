 ...
 *
 * Scans runtime files (bin/install.js, forge-core/bin/**, scripts/*.cjs)
 * and FAILS if the literal `@forge-ai/forge-core` appears in a
...
const LITERAL = '@forge-ai/forge-core';
const IDENTITY_MODULE = path.join(ROOT, 'forge-core', 'bin', 'lib', 'package-identity.cjs');

// Files to scan: bin/install.js + everything under forge-core/bin/ + touched scripts
function getRuntimeFiles() {
  ...
  // forge-core/bin/**/*.cjs and *.js (recursive)
  ...
  collectDir(path.join(ROOT, 'forge-core', 'bin'));
...
test('no hardcoded @forge-ai/forge-core literals in runtime non-comment code lines (#516)', () => {
  ...
  assert.deepEqual(
    violations,
    [],
    `Found ${violations.length} hardcoded @forge-ai/forge-core literal(s) in non-comment code lines:\n` +
    violations.map(v => `  ${v}`).join('\n') +
    '\n\nReplace each with the PACKAGE_NAME imported from forge-core/bin/lib/package-identity.cjs'
  );
});
