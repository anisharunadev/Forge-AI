 ...
 * Writer-path guard: contract assertions on quick.md + forge-executor.md (source-text-is-the-product)
 */
...
const auditModule = require('../forge-core/bin/lib/audit.cjs');
const { auditOpenArtifacts } = auditModule;
const { cleanup } = require('./helpers.cjs');

const TEMPLATES_DIR = path.resolve(__dirname, '..', 'forge-core', 'templates');
const QUICK_MD = path.resolve(__dirname, '..', 'forge-core', 'workflows', 'quick.md');
const EXECUTOR_MD = path.resolve(__dirname, '..', 'agents', 'forge-executor.md');
...
describe('bug #950: quick-task SUMMARY must carry status: complete', () => {
  ...
  test('[TEMPLATE CONTRACT] summary.md contains status: complete in frontmatter', () => {
    ...
    assert.ok(
      fm !== null,
      'forge-core/templates/summary.md: could not locate a YAML frontmatter block (leading --- or fenced ```markdown --- block)'
    );
    assert.ok(
      /^status:\s*complete\s*$/m.test(fm),
      `forge-core/templates/summary.md: \`status: complete\` not found in the frontmatter block.\n` +
  ...
  test('[TEMPLATE CONTRACT] summary-minimal.md contains status: complete in frontmatter', () => {
    ...
    assert.ok(
      fm !== null,
      'forge-core/templates/summary-minimal.md: could not locate a leading YAML frontmatter block'
    );
    assert.ok(
      /^status:\s*complete\s*$/m.test(fm),
      `forge-core/templates/summary-minimal.md: \`status: complete\` not found in the frontmatter block.\n` +
      `Frontmatter extracted:\n${fm}`
    );
  });
  ...
  test('[TEMPLATE CONTRACT] summary-standard.md contains status: complete in frontmatter', () => {
    ...
    assert.ok(
      fm !== null,
      'forge-core/templates/summary-standard.md: could not locate a leading YAML frontmatter block'
    );
    assert.ok(
      /^status:\s*complete\s*$/m.test(fm),
      `forge-core/templates/summary-standard.md: \`status: complete\` not found in the frontmatter block.\n` +
      `Frontmatter extracted:\n${fm}`
    );
  });
  ...
  test('[TEMPLATE CONTRACT] summary-complex.md contains status: complete in frontmatter', () => {
    ...
    assert.ok(
      fm !== null,
      'forge-core/templates/summary-complex.md: could not locate a leading YAML frontmatter block'
    );
    assert.ok(
      /^status:\s*complete\s*$/m.test(fm),
      `forge-core/templates/summary-complex.md: \`status: complete\` not found in the frontmatter block.\n` +
      `Frontmatter extracted:\n${fm}`
    );
  });

  // ── Writer-path contract checks ───────────────────────────────────────────
  // Guards quick.md and forge-executor.md so a future edit removing `status: complete`
  ...
  test('[WRITER-PATH] quick.md constraints require status: complete in SUMMARY frontmatter', () => {
    ...
    assert.ok(
      ...
      'forge-core/workflows/quick.md must instruct the executor to write `status: complete` in the SUMMARY frontmatter. ' +
      'The <constraints> block must contain the `status: complete` requirement so a future edit cannot silently drop it.'
    );
  });

  test('[WRITER-PATH] forge-executor.md frontmatter spec requires status: complete', () => {
    const content = fs.readFileSync(EXECUTOR_MD, 'utf-8');
    assert.ok(
      /status[\s\S]{0,40}complete/.test(content),
      'agents/forge-executor.md must document `status: complete` as a required SUMMARY frontmatter field. ' +
