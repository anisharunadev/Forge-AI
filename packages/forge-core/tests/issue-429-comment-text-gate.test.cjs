...
describe('doc-contract: agent/reference .md files carry the deployed contract text', () => {
  test('forge-planner.md contains <comment_text_discipline> block', () => {
    const content = fs.readFileSync(PLANNER_MD, 'utf8');
    assert.ok(content.includes('<comment_text_discipline>'), 'forge-planner.md must contain <comment_text_discipline>');
  });

  test('forge-planner.md contains a usage example (<!-- planner-discipline-allow: ...)', () => {
    const content = fs.readFileSync(PLANNER_MD, 'utf8');
    assert.ok(
      content.includes('<!-- planner-discipline-allow:'),
      'forge-planner.md must contain an HTML comment example of the allowlist syntax',
    );
  });
...
describe('allowlist-syntax parity: doc marker == runtime marker', () => {
  ...
  test('ALLOW_PREFIX appears in both forge-planner.md and planner-antipatterns.md', () => {
    // allow-test-rule: source-text-is-the-product
    ...
    assert.ok(
      ...
      `forge-planner.md must contain "${ALLOW_PREFIX}"`,
    );
    assert.ok(
