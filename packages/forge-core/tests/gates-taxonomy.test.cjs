...
const GATES_REF = path.join(ROOT, 'forge-core', 'references', 'gates.md');

describe('gates taxonomy (#1715)', () => {
  test('reference file exists', () => {
    assert.ok(
      fs.existsSync(GATES_REF),
      'forge-core/references/gates.md must exist'
    );
  });
  ...
  test('plan-phase.md references gates.md', () => {
    const planPhase = path.join(ROOT, 'forge-core', 'workflows', 'plan-phase.md');
  ...
  test('execute-phase.md references gates.md', () => {
    const execPhase = path.join(ROOT, 'forge-core', 'workflows', 'execute-phase.md');
  ...
  test('forge-plan-checker.md references gates.md in required_reading block', () => {
    const planChecker = path.join(ROOT, 'agents', 'forge-plan-checker.md');
    const content = fs.readFileSync(planChecker, 'utf-8');
    const match = content.match(/<required_reading>\r?\n([\s\S]*?)\r?\n<\/required_reading>/);
    assert.ok(
      match,
      'forge-plan-checker.md must have a <required_reading> block'
    );
    assert.ok(
      match[1].includes('references/gates.md'),
      'forge-plan-checker.md must reference gates.md inside <required_reading>'
    );
  });

  test('forge-verifier.md references gates.md in required_reading block', () => {
    const verifier = path.join(ROOT, 'agents', 'forge-verifier.md');
    const content = fs.readFileSync(verifier, 'utf-8');
    const match = content.match(/<required_reading>\r?\n([\s\S]*?)\r?\n<\/required_reading>/);
    assert.ok(
      match,
      'forge-verifier.md must have a <required_reading> block'
    );
    assert.ok(
      match[1].includes('references/gates.md'),
      'forge-verifier.md must reference gates.md inside <required_reading>'
    );
  });
