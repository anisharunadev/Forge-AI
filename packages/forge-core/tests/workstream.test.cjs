...
const { migrateToWorkstreams, getOtherActiveWorkstreams } = require('../forge-core/bin/lib/workstream.cjs');
...
function getSessionPointerDir(tmpDir) {
  ...
  const projectId = crypto
    ...
    .digest('hex')
    .slice(0, 16);
  return path.join(os.tmpdir(), 'forge-workstream-sessions', projectId);
}
...
// ─── Integration: forge-tools --ws flag ────────────────────────────────────────

describe('forge-tools --ws flag integration', () => {
  let tmpDir;
...
describe('path traversal rejection', () => {
  ...
  describe('setActiveWorkstream rejects invalid names directly', () => {
    const { setActiveWorkstream } = require('../forge-core/bin/lib/planning-workspace.cjs');
    for (const name of maliciousNames) {
