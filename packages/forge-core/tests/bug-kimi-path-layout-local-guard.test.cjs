...
} = require(path.join(ROOT, 'forge-core', 'bin', 'lib', 'runtime-homes.cjs'));
const {
  resolveRuntimeArtifactLayout,
} = require(path.join(ROOT, 'forge-core', 'bin', 'lib', 'runtime-artifact-layout.cjs'));
const {
  getGlobalDir,
...
describe('Kimi runtime homes', () => {
  test('default Kimi global root is recommended ~/.config/agents when no generic skills root exists', () => {
    const tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-kimi-home-default-'));
    try {
      withEnv({ KIMI_CONFIG_DIR: undefined, XDG_CONFIG_HOME: undefined, HOME: tmpHome, USERPROFILE: tmpHome }, () => {
        ...
        assert.strictEqual(
          getGlobalSkillDir('kimi', 'forge-help'),
          path.join(tmpHome, '.config', 'agents', 'skills', 'forge-help'),
        );
  ...
  test('Kimi root resolution follows first-existing generic skills directory order', () => {
    const tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-kimi-home-existing-'));
    try {
  ...
  test('XDG_CONFIG_HOME does not change Kimi default root', () => {
    const tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-kimi-home-xdg-'));
    try {
  ...
  test('Kimi global install reuses existing ~/.agents/skills when recommended root is absent', () => {
    const tmpProject = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-kimi-existing-agents-project-'));
    const tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-kimi-existing-agents-home-'));
    try {
      ...
      assert.strictEqual(
        fs.existsSync(path.join(tmpHome, '.agents', 'skills', 'forge-new-project', 'SKILL.md')),
        true,
      );
      assert.strictEqual(
        fs.existsSync(path.join(tmpHome, '.config', 'agents', 'skills', 'forge-new-project', 'SKILL.md')),
        false,
      );
    } finally {
      cleanup(tmpProject);
...
describe('Kimi runtime artifact layout', () => {
  test('global layout stages Kimi skills and agents while local layout remains guarded', () => {
    ...
    assert.strictEqual(globalLayout.kinds[0].prefix, 'forge-');
    ...
    assert.strictEqual(globalLayout.kinds[1].prefix, 'forge');
...
describe('Kimi local install guard', () => {
  test('--kimi --local exits successfully without writing local Kimi project artifacts', () => {
    const tmpProject = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-kimi-local-project-'));
    const tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-kimi-local-home-'));
    try {
      const result = spawnSync(
        process.execPath,
  ...
  test('--kimi --global writes converted Kimi skills and agent artifacts', () => {
    const tmpProject = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-kimi-global-project-'));
    const tmpConfig = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-kimi-global-config-'));
    const tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-kimi-global-home-'));
    try {
      const result = spawnSync(
        process.execPath,
      ...
      const skillFile = path.join(tmpConfig, 'skills', 'forge-new-project', 'SKILL.md');
      assert.ok(fs.existsSync(skillFile), 'must write forge-new-project/SKILL.md');
      const skillContent = fs.readFileSync(skillFile, 'utf8');
      assert.match(skillContent, /^name: forge-new-project$/m);
      assert.match(skillContent, /\/skill:gsd-new-project/);
      assert.match(skillContent, /forge-core\/workflows\/new-project\.md/);
      assert.doesNotMatch(skillContent, /@~\/\.claude\/forge-core|@\$HOME\/\.claude\/forge-core/);
      ...
      const executorYaml = path.join(tmpConfig, 'agents', 'subagents', 'forge-executor.yaml');
      const executorPrompt = path.join(tmpConfig, 'agents', 'subagents', 'forge-executor.md');
      assert.ok(fs.existsSync(rootYaml), 'must write agents/gsd.yaml');
      assert.ok(fs.existsSync(rootPrompt), 'must write agents/gsd.md');
      assert.ok(fs.existsSync(executorYaml), 'must write agents/subagents/forge-executor.yaml');
      assert.ok(fs.existsSync(executorPrompt), 'must write agents/subagents/forge-executor.md');
      ...
      assert.ok(fs.existsSync(path.join(tmpConfig, 'forge-core', 'workflows', 'new-project.md')), 'must write workflow payloads used by Kimi skills');
      const manifestPath = path.join(tmpConfig, 'forge-file-manifest.json');
      assert.ok(fs.existsSync(manifestPath), 'must write forge-file-manifest.json for Kimi installs');
      const manifest = JSON.parse(fs.readFileSync(manifestPath, 'utf8'));
      assert.ok(manifest.files['skills/forge-new-project/SKILL.md'], 'manifest tracks generated Kimi skill');
      ...
      assert.ok(manifest.files['agents/subagents/forge-executor.yaml'], 'manifest tracks Kimi subagent YAML');
      assert.ok(manifest.files['agents/subagents/forge-executor.md'], 'manifest tracks Kimi subagent prompt');
      assert.ok(manifest.files['forge-core/workflows/new-project.md'], 'manifest tracks installed workflow payload');
  ...
  test('--kimi --global backs up local edits to generated skills and agent artifacts on reinstall', () => {
    const tmpProject = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-kimi-reinstall-project-'));
    const tmpConfig = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-kimi-reinstall-config-'));
    const tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-kimi-reinstall-home-'));
    ...
    try {
      ...
      const skillFile = path.join(tmpConfig, 'skills', 'forge-new-project', 'SKILL.md');
      const agentPrompt = path.join(tmpConfig, 'agents', 'subagents', 'forge-executor.md');
      ...
      assert.match(secondOutput, /\/skill:forge-update --reapply/);
      assert.doesNotMatch(secondOutput, /Run\s+\/forge-update --reapply/);

      const skillBackup = path.join(tmpConfig, 'forge-local-patches', 'skills', 'forge-new-project', 'SKILL.md');
      const agentBackup = path.join(tmpConfig, 'forge-local-patches', 'agents', 'subagents', 'forge-executor.md');
      ...
      const meta = JSON.parse(fs.readFileSync(path.join(tmpConfig, 'forge-local-patches', 'backup-meta.json'), 'utf8'));
      assert.ok(meta.files.includes('skills/forge-new-project/SKILL.md'));
      assert.ok(meta.files.includes('agents/subagents/forge-executor.md'));
    } finally {
      cleanup(tmpProject);
      cleanup(tmpConfig);
      cleanup(tmpHome);
  ...
  test('--kimi --global --uninstall removes GSD artifacts and preserves non-GSD Kimi content', () => {
    const tmpProject = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-kimi-uninstall-project-'));
    const tmpConfig = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-kimi-uninstall-config-'));
    const tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-kimi-uninstall-home-'));
    const env = installerEnv({ HOME: tmpHome, USERPROFILE: tmpHome });
    ...
    try {
      ...
      assert.ok(!fs.existsSync(path.join(tmpConfig, 'skills', 'forge-new-project')), 'must remove generated Kimi skills');
      ...
      assert.ok(!fs.existsSync(path.join(tmpConfig, 'agents', 'subagents', 'forge-executor.yaml')), 'must remove generated Kimi subagent YAML');
      assert.ok(!fs.existsSync(path.join(tmpConfig, 'agents', 'subagents', 'forge-executor.md')), 'must remove generated Kimi subagent prompt');
      assert.ok(!fs.existsSync(path.join(tmpConfig, 'forge-core')), 'must remove installed workflow payload');
      assert.ok(!fs.existsSync(path.join(tmpConfig, 'forge-file-manifest.json')), 'must remove the manifest');
