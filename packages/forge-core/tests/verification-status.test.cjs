 ...
 *  7. PARITY: VERIFIER_STATUSES covered by routing table; forge-verifier.md emitted statuses covered
...
} = require('../forge-core/bin/lib/verification.cjs');
...
function mkPhaseDir(suffix) {
  return fs.mkdtempSync(path.join(os.tmpdir(), `forge-651-${suffix}-`));
}
...
describe('verification-status', () => {
  ...
  test('status: gaps_found in "03-foo" dir → next_command includes phase token 03', () => {
    ...
    const baseDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-651-parent-'));
    ...
    try {
      ...
      assert.equal(result.next_command, '/forge:plan-phase 03 --gaps');
    } finally {
      cleanup(baseDir);
    }
  });
  ...
  test('no *-VERIFICATION.md file → status missing, next_command execute-phase', () => {
    ...
    try {
      ...
      assert.equal(result.next_command, '/forge:execute-phase');
  ...
  // (b) Parse agents/forge-verifier.md for emitted statuses via /→ \*\*status:\s*([a-z_]+)\*\*/g,
  ...
  test('PARITY: forge-verifier.md emitted statuses all have routing table entries', () => {
    const verifierPath = path.join(__dirname, '..', 'agents', 'forge-verifier.md');
    ...
    assert.ok(
      ...
      'No emitted statuses found in forge-verifier.md — regex or file path may be wrong. ' +
    ...
    for (const s of emittedStatuses) {
      assert.ok(
        s in VERIFICATION_ROUTING_TABLE,
        `forge-verifier.md emits status '${s}' but VERIFICATION_ROUTING_TABLE has no entry for it. ` +
          'Add a route or remove/rename the status in forge-verifier.md.',
      );
    }
  });
  ...
  test('nonexistent phase directory → missing', () => {
    const nonexistent = path.join(os.tmpdir(), 'forge-651-nonexistent-' + Date.now());
  ...
  test('ship.md still emits the PHASE_VERIFICATION_INCOMPLETE gate sentinel (contract anchor for #651 consolidation)', () => {
    const shipMdPath = path.join(__dirname, '..', 'forge-core', 'workflows', 'ship.md');
