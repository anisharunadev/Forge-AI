...
const {
  ...
  VALID_PHASE_TYPES,
  MODEL_PROFILES,
} = require('../forge-core/bin/lib/model-profiles.cjs');
const { isValidConfigKey } = require('../forge-core/bin/lib/config-schema.cjs');

const { createTempDir, cleanup } = require('./helpers.cjs');
const makeTmp = (prefix) => createTempDir(`forge-3023-${prefix}-`);
...
describe('#3023 resolver: models.<phase_type> overrides profile-based tier', () => {
  ...
  test('phase-type alone — research agents get the phase-type tier, planner gets profile default', () => {
    ...
    // forge-phase-researcher is a research agent — should pick up 'haiku'
    // from the phase-type slot, not 'sonnet' from the balanced profile.
    assert.equal(resolveModelInternal(projectDir, 'forge-phase-researcher'), 'haiku');
    // forge-codebase-mapper is also research → haiku
    assert.equal(resolveModelInternal(projectDir, 'forge-codebase-mapper'), 'haiku');
    // forge-planner is planning, no models.planning set → falls through to
    // profile (balanced → opus per MODEL_PROFILES).
    assert.equal(resolveModelInternal(projectDir, 'forge-planner'), 'opus');
  });
  ...
  test('per-agent override beats phase-type (acceptance criterion b)', () => {
    writeConfig(projectDir, {
      model_profile: 'balanced',
      models: { research: 'haiku' },
      model_overrides: { 'forge-phase-researcher': 'opus' },
    });
    // The targeted per-agent override wins for that one agent.
    assert.equal(resolveModelInternal(projectDir, 'forge-phase-researcher'), 'opus');
    // Other research agents still pick up the phase-type tier.
    assert.equal(resolveModelInternal(projectDir, 'forge-codebase-mapper'), 'haiku');
    assert.equal(resolveModelInternal(projectDir, 'forge-research-synthesizer'), 'haiku');
  });
  ...
  test('phase-type beats profile (acceptance criterion c)', () => {
    ...
    assert.equal(resolveModelInternal(projectDir, 'forge-phase-researcher'), 'haiku');
    assert.equal(resolveModelInternal(projectDir, 'forge-codebase-mapper'), 'haiku');
    // forge-planner is planning, no slot set, profile=quality → opus.
    assert.equal(resolveModelInternal(projectDir, 'forge-planner'), 'opus');
  });
  ...
  test('issue example: opus for planning/discuss/execution, sonnet for research/verification/completion', () => {
    ...
    assert.equal(resolveModelInternal(projectDir, 'forge-executor'), 'opus');
    // Research agents → sonnet
    assert.equal(resolveModelInternal(projectDir, 'forge-phase-researcher'), 'sonnet');
    // Verification agents → sonnet
    assert.equal(resolveModelInternal(projectDir, 'forge-verifier'), 'sonnet');
  });
  ...
  test('phase-type "inherit" is honored (preserves existing inherit semantics)', () => {
    ...
    assert.equal(resolveModelInternal(projectDir, 'forge-phase-researcher'), 'inherit');
  });
  ...
  test('unrecognized tier value falls through to profile (typo safety) — CR follow-up', () => {
    ...
    assert.equal(resolveModelInternal(projectDir, 'forge-phase-researcher'), 'sonnet');
    assert.equal(resolveModelInternal(projectDir, 'forge-codebase-mapper'), 'haiku',
      'forge-codebase-mapper at balanced is haiku per profile, unaffected by typo');
  });
  ...
  test('phase-type override wins over profile=inherit (CR Major) — model resolver', () => {
    ...
    // forge-executor (execution) must get the phase-type opus, not inherit.
    assert.equal(resolveModelInternal(projectDir, 'forge-executor'), 'opus');
  });
  ...
  test('phase-type "haiku" wins over profile=inherit; agents without a slot still inherit', () => {
    ...
    assert.equal(resolveModelInternal(projectDir, 'forge-planner'), 'inherit');
  });
  ...
  test('profile=inherit with no models block still returns inherit (no regression)', () => {
    ...
    assert.equal(resolveModelInternal(projectDir, 'forge-executor'), 'inherit');
    assert.equal(resolveModelInternal(projectDir, 'forge-phase-researcher'), 'inherit');
  });
  ...
  test('profile=inherit with models block but agent has no slot → inherit', () => {
    ...
    // forge-executor (execution slot) is not set → falls through to inherit.
    assert.equal(resolveModelInternal(projectDir, 'forge-executor'), 'inherit');
  });
});
...
const { resolveEffortInternal } = require('../forge-core/bin/lib/model-resolver.cjs');
...
describe('#3023 + #443: unified effort resolver (resolveEffortInternal) for Codex', () => {
  ...
  test('effort derives from AGENT_DEFAULT_TIERS (routing), not phase-type; forge-executor is standard → high', () => {
    // Under unification, effort is config-driven via routing_tier_defaults.
    // forge-executor has routing tier 'standard' → default effort 'high', regardless
    ...
    const eff = resolveEffortInternal(projectDir, 'forge-executor');
  ...
  test('effort resolves universally even when models.execution=inherit', () => {
    ...
    // Effort always resolves from routing_tier_defaults: forge-executor (standard) → 'high'.
    writeConfig(projectDir, {
      runtime: 'codex',
      model_profile: 'balanced',
  ...
  test('per-agent model_overrides does not affect effort (effort is routing-tier-based)', () => {
    ...
    // forge-executor (standard tier) → 'high' regardless.
    writeConfig(projectDir, {
      runtime: 'codex',
      model_profile: 'balanced',
      models: { execution: 'opus' },
      model_overrides: { 'forge-executor': 'openai/gpt-5' },
    });
    const eff = resolveEffortInternal(projectDir, 'forge-executor');
    assert.equal(eff, 'high');
    const rendered = renderEffortForRuntime('codex', eff);
  ...
  test('profile=inherit does not affect effort; effort resolves from routing tier', () => {
    ...
    // forge-executor (standard routing tier) → 'high' even with model_profile='inherit'.
    writeConfig(projectDir, {
      runtime: 'codex',
      model_profile: 'inherit',
      models: { execution: 'opus' },
...
describe('#3023 config-schema: models.<phase_type> validation', () => {
  ...
  test('unknown phase-type is rejected (acceptance criterion d)', () => {
    ...
    assert.equal(isValidConfigKey('models.forge-planner'), false,
