/**
 * Enhancement #2500: forge-codebase-mapper (arch focus) rich architecture output
 *
 * The codebase/ARCHITECTURE.md produced by forge-codebase-mapper was a sparse
 * structural inventory — file listings and module relationships. After a major
 * refactor, research/ARCHITECTURE.md (created at /forge-new-project) goes stale
...
// The forge-codebase-mapper ARCHITECTURE.md template is the instruction set
...
const AGENT_PATH = path.join(__dirname, '..', 'agents', 'forge-codebase-mapper.md');

describe('enh-2500: forge-codebase-mapper arch focus — rich architecture output', () => {
  let agentContent;
  let archTemplate;

  before(() => {
    assert.ok(fs.existsSync(AGENT_PATH), 'agents/forge-codebase-mapper.md must exist');
