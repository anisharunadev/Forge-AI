...
const commandPath = path.join(repoRoot, 'commands', 'forge', 'forensics.md');
const workflowPath = path.join(repoRoot, 'forge-core', 'workflows', 'forensics.md');

describe('forensics command', () => {
  test('command file exists', () => {
    assert.ok(fs.existsSync(commandPath), 'commands/forge/forensics.md should exist');
  });

  test('command has correct frontmatter', () => {
    const content = fs.readFileSync(commandPath, 'utf-8');
    assert.ok(content.includes('name: forge:forensics'), 'should have correct command name');
...
describe('forensics workflow', () => {
  ...
  test('workflow submits issues to forge-ai/forge-core, not the current repo', () => {
    const content = fs.readFileSync(workflowPath, 'utf-8');
    ...
    assert.match(
      content,
      /gh issue create[\s\S]{0,250}--repo\s+forge-ai\/forge-core/,
      'gh issue create must use --repo forge-ai/forge-core to avoid submitting to the user\'s current project repo'
    );
  });

  test('workflow checks bug label in forge-ai/forge-core, not the current repo', () => {
    const content = fs.readFileSync(workflowPath, 'utf-8');
    // Regex is more robust than a fixed-length slice to formatting changes
    assert.match(
      content,
      /gh label list[\s\S]{0,250}--repo\s+forge-ai\/forge-core/,
      'gh label list must target forge-ai/forge-core'
    );
  });
  ...
  test('workflow updates STATE.md', () => {
    ...
    assert.ok(
      ...
      'should update STATE.md via state record-session (CJS or forge-sdk query)'
    );
  });
...
describe('forensics fixture-based tests', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-forensics-test-'));
  });
