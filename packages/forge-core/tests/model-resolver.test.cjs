...
const modelResolver = require('../forge-core/bin/lib/model-resolver.cjs');
...
function makeTempProject(prefix = 'forge-model-resolver-test-') {
...
describe('resolveModelInternal', () => {
  ...
  test('no config -> balanced profile -> forge-planner resolves to a string', () => {
    const model = resolveModelInternal(tmpDir, 'forge-planner');
  ...
  test('model_overrides takes precedence over everything else', () => {
    writeConfig(tmpDir, { model_overrides: { 'forge-planner': 'my-custom-model' } });
    assert.strictEqual(resolveModelInternal(tmpDir, 'forge-planner'), 'my-custom-model');
  });

  test('model_profile=quality -> opus-class model for forge-planner', () => {
  ...
  test('model_profile=budget -> haiku-class model for forge-planner', () => {
  ...
  test('models config per phase type overrides profile tier', () => {
    ...
    // forge-planner maps to planning phase type; config says opus
  ...
  test('runtime non-claude + model_profile_overrides for runtime tier', () => {
    ...
    // forge-codebase-mapper is light tier -> haiku in balanced profile
    const model = resolveModelInternal(tmpDir, 'forge-codebase-mapper');
    assert.ok(typeof model === 'string' && model.length > 0);
  });
});
...
describe('resolveEffortInternal', () => {
  ...
  test('invocation override beats everything', () => {
    writeConfig(tmpDir, { effort: { agent_overrides: { 'forge-planner': 'low' } } });
...
describe('resolveFastModeInternal', () => {
  ...
  test('opts.override=true beats config', () => {
    writeConfig(tmpDir, { fast_mode: { agent_overrides: { 'forge-planner': false } } });
    assert.strictEqual(resolveFastModeInternal(tmpDir, 'forge-planner', { override: true }), true);
  });
  ...
  test('agent_overrides beats enabled', () => {
    writeConfig(tmpDir, {
      fast_mode: { enabled: true, agent_overrides: { 'forge-planner': false } },
    });
    assert.strictEqual(resolveFastModeInternal(tmpDir, 'forge-planner'), false);
  });
...
describe('resolveEffortForTier', () => {
  ...
  test('dynamic_routing disabled -> attempt has no effect', () => {
    ...
    const at1 = resolveEffortForTier(tmpDir, 'forge-planner', 1);
  ...
  test('escalation clamps at "max"', () => {
    ...
    assert.strictEqual(resolveEffortForTier(tmpDir, 'forge-planner', 99), 'max');
  });
});
...
describe('resolveModelForTier', () => {
  ...
  test('no dynamic_routing -> falls back to resolveModelInternal', () => {
    const fromForTier = resolveModelForTier(tmpDir, 'forge-planner');
    const fromInternal = resolveModelInternal(tmpDir, 'forge-planner');
    assert.strictEqual(fromForTier, fromInternal);
  });

  test('model_overrides wins before dynamic routing logic', () => {
    writeConfig(tmpDir, {
      model_overrides: { 'forge-planner': 'override-model' },
      dynamic_routing: {
        enabled: true,
        tier_models: { light: 'haiku', standard: 'sonnet', heavy: 'opus' },
        escalate_on_failure: true,
    ...
    assert.strictEqual(resolveModelForTier(tmpDir, 'forge-planner'), 'override-model');
  });
  ...
  test('dynamic_routing + tier_models + attempt=0 -> default tier model', () => {
    ...
    // forge-codebase-mapper is 'light' tier
    assert.strictEqual(resolveModelForTier(tmpDir, 'forge-codebase-mapper', 0), 'haiku-custom');
  });
  ...
  test('dynamic_routing + attempt=1 escalates tier', () => {
    ...
    // forge-codebase-mapper light -> attempt=1 -> standard
    assert.strictEqual(resolveModelForTier(tmpDir, 'forge-codebase-mapper', 1), 'sonnet-custom');
  });
});
...
describe('ADVERSARIAL: edge cases', () => {
  ...
  test('resolveEffortInternal: invalid override "turbo" falls through to tier default', () => {
    const result = resolveEffortInternal(tmpDir, 'forge-planner', { override: 'turbo' });
    // forge-planner is heavy -> tier default xhigh
  ...
  test('resolveEffortForTier: max_escalations=0 caps escalation', () => {
    ...
    const at0 = resolveEffortForTier(tmpDir, 'forge-codebase-mapper', 0);
