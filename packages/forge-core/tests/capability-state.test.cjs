...
const {
  ...
  _resolveManifest,
} = require('../forge-core/bin/lib/capability-state.cjs');
...
// ─── cmdCapabilityState — end-to-end via forge-tools CLI ──────────────────────
//
// Because cmdCapabilityState destructures `output` at module load time, patching
// core.cjs after the fact is ineffective. We instead invoke forge-tools via
// spawnSync so each test gets a fresh process with stdout captured.

const { spawnSync } = require('node:child_process');

const gsdToolsPath = path.resolve(__dirname, '..', 'forge-core', 'bin', 'forge-tools.cjs');
...
describe('cmdCapabilityState — end-to-end via forge-tools CLI', () => {
  let tmpConfigDir;
  let tmpConfigDirCore;
  let tmpConfigDirUiDisabled;

  before(() => {
    // Tmp runtime config dir without .forge-profile (defaults to 'full')
    ...
    fs.writeFileSync(path.join(tmpConfigDirCore, '.forge-profile'), 'core\n', 'utf8');

    tmpConfigDirUiDisabled = fs.mkdtempSync(path.join(os.tmpdir(), 'cap-state-cfg-ui-disabled-'));
    fs.writeFileSync(
      path.join(tmpConfigDirUiDisabled, '.forge-surface.json'),
      JSON.stringify({
        baseProfile: 'full',
        disabledClusters: ['ui'],
        explicitAdds: [],
  ...
  test('emits envelope with runtimeConfigDir and capabilities array', () => {
    ...
    assert.strictEqual(result.status, 0, `forge-tools exited ${result.status}: ${result.stderr}`);
...
//
// In an installed runtime (e.g. Codex) the commands/forge source tree is absent.
// Only <configDir>/skills/forge-*/SKILL.md files exist. Prior to this fix,
...
describe('regressions: installed-runtime capability surface (#1160)', () => {
  ...
  test('_loadInstalledSkillsManifest: scans forge-<stem>/SKILL.md dirs and produces stems', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'cap-ism-scan-'));
    try {
      // Create installed skill dirs
      const secureDir = path.join(tmpDir, 'skills', 'forge-secure-phase');
      const validateDir = path.join(tmpDir, 'skills', 'forge-validate-phase');
  ...
  test('_loadInstalledSkillsManifest: parses requires from SKILL.md frontmatter', () => {
    ...
    try {
      const skillDir = path.join(tmpDir, 'skills', 'forge-my-skill');
  ...
  test('_loadInstalledSkillsManifest: directory with no SKILL.md is NOT registered (parity with loadSkillsManifest)', () => {
    ...
    try {
      fs.mkdirSync(path.join(tmpDir, 'skills', 'forge-nodoc'), { recursive: true });
  ...
  test('_resolveManifest: uses commandsGsdDir when it exists', () => {
    const realCommandsGsdDir = path.resolve(__dirname, '..', 'commands', 'forge');
    ...
    try {
      ...
      assert.ok(manifest instanceof Map);
      // The real commands/forge has many skills; manifest should be non-empty
  ...
  describe('end-to-end CLI with installed skill layout', () => {
    ...
    before(() => {
      ...
      fs.writeFileSync(path.join(tmpInstalledConfigDir, '.forge-profile'), 'full\n', 'utf8');
      const secureDir = path.join(tmpInstalledConfigDir, 'skills', 'forge-secure-phase');
      const validateDir = path.join(tmpInstalledConfigDir, 'skills', 'forge-validate-phase');
      fs.mkdirSync(secureDir, { recursive: true });
      fs.mkdirSync(validateDir, { recursive: true });
      fs.writeFileSync(path.join(secureDir, 'SKILL.md'), makeSkillMd('secure-phase'), 'utf8');
      fs.writeFileSync(path.join(validateDir, 'SKILL.md'), makeSkillMd('validate-phase'), 'utf8');
    ...
    test('security capability: enabled=true in installed runtime with security_enforcement=true', () => {
      ...
      assert.strictEqual(result.status, 0, `forge-tools exited ${result.status}:\nstdout: ${result.stdout}\nstderr: ${result.stderr}`);
      const envelope = JSON.parse(result.stdout);
  ...
  // ── TRUE installed-runtime layout: forge-tools runs where commands/forge is unreachable ──
  // The block above runs the repo's forge-tools.cjs, where commands/forge source IS
  // reachable by walk-up, so it cannot exercise the bug. This block copies the
  // runtime executable tree (forge-core/bin + scripts + package.json) into a temp
  ...
  describe('true installed layout (commands/forge unreachable)', () => {
    let installRoot;
    ...
    before(() => {
      ...
      fs.mkdirSync(path.join(installRoot, 'forge-core'), { recursive: true });
      fs.cpSync(
        path.join(repoRoot, 'forge-core', 'bin'),
        path.join(installRoot, 'forge-core', 'bin'),
        { recursive: true },
      );
      fs.cpSync(
      ...
      installedGsdTools = path.join(installRoot, 'forge-core', 'bin', 'forge-tools.cjs');

      // 2. Installed runtime config dir: full profile + skills/forge-*/SKILL.md only.
      installedConfigDir = fs.mkdtempSync(path.join(os.tmpdir(), 'cap-state-instcfg-'));
      fs.writeFileSync(path.join(installedConfigDir, '.forge-profile'), 'full\n', 'utf8');
      for (const stem of ['secure-phase', 'validate-phase']) {
        const skillDir = path.join(installedConfigDir, 'skills', `forge-${stem}`);
        fs.mkdirSync(skillDir, { recursive: true });
    ...
    test('commands/forge is genuinely unreachable from the installed forge-tools', () => {
      // Sanity: no commands/forge anywhere under the install root.
      const probe = path.join(installRoot, 'commands', 'forge');
...
describe('isCapabilityActive — convenience predicate (Phase 2)', () => {
  ...
  test('isCapabilityActive returns same value as the entry active field (e2e)', () => {
    ...
    const { resolveCapabilityRuntimeState } = require('../forge-core/bin/lib/capability-state.cjs');
...
describe('isCapabilityActive cross-runtime detection (GSD_RUNTIME → config.runtime → claude)', () => {
  ...
  //
  // Fixture layout:
  //   CODEX_HOME        → tmpCodexDir/   ← .forge-surface.json: full profile, graphify SURFACED
  //   CLAUDE_CONFIG_DIR → tmpClaudeDir/ ← .forge-surface.json: full profile, graphify NOT surfaced
  //   GSD_RUNTIME=codex
  ...
  before(() => {
    ...
    fs.writeFileSync(
      path.join(tmpCodexDir, '.forge-surface.json'),
    ...
    fs.writeFileSync(
      path.join(tmpClaudeDir, '.forge-surface.json'),
...
describe('#1459 IC-04: capability-state threads gsdHome to the overlay loader', () => {
  ...
  const loader = require('../forge-core/bin/lib/capability-loader.cjs');
