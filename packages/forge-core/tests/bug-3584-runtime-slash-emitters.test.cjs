 ...
 * fracture recommendations) no longer emit the unroutable `/forge:<cmd>` colon
...
// Helper: assert no string field anywhere in `value` (recursive) contains '/forge:'.
function assertNoColonForm(value, label) {
  ...
  while (stack.length > 0) {
    ...
    if (typeof v === 'string') {
      assert.ok(
        !v.includes('/forge:'),
        `${p}: must not contain deprecated /forge: form, got ${JSON.stringify(v)}`,
      );
...
describe('bug-3584: init manager recommendedActions emit hyphen form', () => {
  ...
  test('skills runtime (claude) emits /forge-<cmd> in recommended_actions[].command', () => {
    ...
    for (const cmd of commands) {
      assert.ok(
        cmd.startsWith('/forge-'),
        `recommended_actions command must start with /forge- for skills-based runtimes, got ${cmd}`,
      );
      assert.ok(
        !cmd.includes('/forge:'),
        `recommended_actions command must not contain /forge: colon form, got ${cmd}`,
      );
    }
...
describe('bug-3584: phase add persists hyphen form into ROADMAP.md', () => {
  ...
  test('phase add persists hyphen-form slash command in roadmap get-phase payload', () => {
    ...
    assert.ok(
      !getPayload.section.includes('/forge:plan-phase'),
      `persisted phase section must not contain /forge:plan-phase, got: ${getPayload.section}`,
    );
    assert.ok(
      getPayload.section.includes('/forge-plan-phase'),
      `persisted phase section must contain /forge-plan-phase, got: ${getPayload.section}`,
    );
  });
});
...
describe('bug-3584: validate health emits hyphen form in fix hints', () => {
  test('validate health on a broken project returns hyphen-form fix strings', (t) => {
    ...
    assert.ok(
      ...
      'at least one fix hint should reference a /forge- slash command',
    );

    for (const fix of fixesWithSlash) {
      assert.ok(
        !fix.includes('/forge:'),
        `validate health fix hint must not contain /forge: colon form, got ${JSON.stringify(fix)}`,
      );
    }
...
describe('bug-3584: validate context recommendation uses hyphen form', () => {
  test('warning/critical recommendations emit /forge-thread', (t) => {
    ...
    assert.ok(
      !payload.recommendation.includes('/forge:'),
      `recommendation must not contain /forge:, got ${JSON.stringify(payload.recommendation)}`,
    );
    assert.ok(
      payload.recommendation.includes('/forge-thread'),
      `recommendation must reference /forge-thread, got ${JSON.stringify(payload.recommendation)}`,
    );
  });
});
...
describe('bug-3584: validate health uses formatter for codex runtime too', () => {
  test('validate health under codex emits $gsd-<cmd> in fix strings (positive assertion)', (t) => {
    ...
    // Collect fixes that mention any forge slash-command form so we can lock
    ...
    assert.ok(
      ...
      'validate health on a bare tmpdir must produce at least one fix hint referencing a forge command',
    );
    ...
    for (const fix of fixesWithGsdRef) {
      ...
      assert.ok(
        !fix.includes('/forge:'),
        `codex validate health fix must not contain /forge: colon form, got ${JSON.stringify(fix)}`,
      );
      assert.ok(
        !fix.includes('/forge-'),
        `codex validate health fix must not contain /forge- (skills) form, got ${JSON.stringify(fix)}`,
      );
    }
  });
});
