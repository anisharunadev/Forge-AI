...
const NEST = [
  ...
  { runtime: 'cline',       scope: 'global', skillsSub: 'skills',     prefix: 'forge-' },
  { runtime: 'qwen',        scope: 'global', skillsSub: 'skills',     prefix: 'forge-' },
  { runtime: 'hermes',      scope: 'global', skillsSub: 'skills/gsd', prefix: 'forge-' }, // #947: restored canonical prefix
  { runtime: 'augment',     scope: 'global', skillsSub: 'skills',     prefix: 'forge-' },
  { runtime: 'trae',        scope: 'global', skillsSub: 'skills',     prefix: 'forge-' },
];
...
function runInstall(runtime, scope, resolved) {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), `forge-nest-test-${runtime}-`));
...
for (const { runtime, scope, skillsSub, prefix } of NEST) {
  describe(`${runtime} (nested layout)`, () => {
    ...
    test(`${runtime}: exactly 6 top-level router bundles, no concrete skill at top level`, () => {
      ...
      // All nested runtimes (incl. Hermes after #947) use prefix='forge-'.
...
// ---------------------------------------------------------------------------
// claude extra: total top-level forge- count must be >= 60 (FLAT, #924)
//
// Pre-#924 (nested) this block asserted exactly 6 (only routers).
// Post-#924 (flat) Claude has all concrete skills at the top level.
// ---------------------------------------------------------------------------

describe('claude: total top-level forge- entries >= 60 (flat layout, #924)', () => {
  let tmpDir;
  ...
  test('claude: >= 60 forge-* top-level skill entries (concrete flat layout, not nested)', () => {
    ...
    assert.ok(
      topLevel.length >= 60,
      `Expected >= 60 forge-* top-level entries under claude/skills (flat layout after #924 fix). ` +
...
for (const { runtime, scope, skillsSub } of FLAT) {
  describe(`${runtime} (flat layout)`, () => {
    ...
    test(`${runtime}: stays flat — concrete skills remain top-level, no nesting`, () => {
      ...
      const topLevel = fs.readdirSync(skillsDir);
      const gsdEntries = topLevel.filter((n) => n.startsWith('forge-'));

      // For flat runtimes, there should be many more than 6 top-level forge- entries
      assert.ok(
        gsdEntries.length >= 60,
        `Flat runtime ${runtime} must have >= 60 forge-* top-level entries (concrete skills), got ${gsdEntries.length}`,
      );

      // No router dir should contain a skills/ subdirectory (nesting must not have been applied)
      const routerDirsPresent = topLevel.filter((n) => n.startsWith('forge-ns-'));
