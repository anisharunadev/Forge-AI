/**
 * Drift guard for forge:plan-phase workflow (#22)
 *
 ...
 *     API-SURFACE.md via `forge-tools intel api-surface` and injects it into the
...
describe('plan-phase workflow: intel.enabled gate for API-SURFACE injection (#22)', () => {
  ...
  test('workflow runs forge-tools intel api-surface to regenerate surface', () => {
    assert.ok(
      workflow.includes('intel api-surface'),
      'workflow must call `forge_run intel api-surface` (or equivalent) to regenerate API-SURFACE.md'
    );
  });
