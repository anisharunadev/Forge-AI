...
const { resolveModelInternal } = require('../forge-core/bin/lib/model-resolver.cjs');
...
describe('bug #1829: model_profile "inherit" — resolveModelInternal', () => {
  ...
  test('returns "inherit" for forge-planner when model_profile is "inherit"', () => {
    writeConfig(tmpDir, { model_profile: 'inherit' });
    assert.strictEqual(resolveModelInternal(tmpDir, 'forge-planner'), 'inherit');
  });

  test('returns "inherit" for forge-executor when model_profile is "inherit"', () => {
    writeConfig(tmpDir, { model_profile: 'inherit' });
    assert.strictEqual(resolveModelInternal(tmpDir, 'forge-executor'), 'inherit');
  });

  test('returns "inherit" for forge-phase-researcher when model_profile is "inherit"', () => {
    writeConfig(tmpDir, { model_profile: 'inherit' });
    assert.strictEqual(resolveModelInternal(tmpDir, 'forge-phase-researcher'), 'inherit');
  });

  test('returns "inherit" for forge-codebase-mapper when model_profile is "inherit"', () => {
    writeConfig(tmpDir, { model_profile: 'inherit' });
    assert.strictEqual(resolveModelInternal(tmpDir, 'forge-codebase-mapper'), 'inherit');
  });

  test('returns "inherit" for forge-verifier when model_profile is "inherit"', () => {
    writeConfig(tmpDir, { model_profile: 'inherit' });
    assert.strictEqual(resolveModelInternal(tmpDir, 'forge-verifier'), 'inherit');
  });

  test('returns "inherit" for unknown agent with inherit profile', () => {
    writeConfig(tmpDir, { model_profile: 'inherit' });
    assert.strictEqual(resolveModelInternal(tmpDir, 'forge-nonexistent'), 'inherit');
  });

  test('per-agent override takes precedence over inherit profile', () => {
    writeConfig(tmpDir, {
      model_profile: 'inherit',
      model_overrides: { 'forge-executor': 'haiku' },
    });
    // Override wins even when profile is inherit
    assert.strictEqual(resolveModelInternal(tmpDir, 'forge-executor'), 'haiku');
...
describe('bug #1829: model_profile "inherit" — resolve-model CLI', () => {
  ...
  test('CLI resolve-model returns "inherit" for forge-executor with inherit profile', () => {
    fs.writeFileSync(
      path.join(tmpDir, '.planning', 'config.json'),
  ...
  test('CLI resolve-model returns "inherit" for forge-planner with inherit profile', () => {
    fs.writeFileSync(
      path.join(tmpDir, '.planning', 'config.json'),
      JSON.stringify({ model_profile: 'inherit' }, null, 2)
    );
