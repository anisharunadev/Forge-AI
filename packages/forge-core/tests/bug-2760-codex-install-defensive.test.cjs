...
describe('#2760 defect 3 — Hooks AoT preservation across install/uninstall/reinstall', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-2760-d3-'));
  ...
  test('fresh install emits the two-level nested AoT schema (#2773)', () => {
    ...
    const managed = sessionStartCommands.filter((cmd) => /forge-check-update/.test(cmd));
    assert.equal(managed.length, 1, 'hooks.json must contain exactly one managed forge-check-update command');
    assert.ok(
  ...
  test('preserves user [[hooks.SessionStart]] entries and registers managed GSD handler in hooks.json', () => {
    ...
    assert.ok(
      hooksJsonCommands.some((cmd) => typeof cmd === 'string' && /forge-check-update/.test(cmd)),
  ...
  test('reinstall replaces flat [[hooks]] + event form with nested schema', () => {
    ...
    const legacyConfig = [
      ...
      '[[hooks]]',
      'event = "SessionStart"',
      'command = "node /old/path/to/forge-check-update.js"',
      '',
    ].join('\n');
    ...
    const hooksJsonCommands = readHooksSessionStartCommands(codexHome);
    const gsdHandlers = hooksJsonCommands.filter((cmd) => /forge-check-update/.test(cmd));
...
describe('#2760 fix 2 — Strip purges invalid legacy [agents] / [[agents]] regardless of marker', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-2760-f2-'));
    codexHome = path.join(tmpDir, 'codex-home');
  });
  ...
  test('strips bare [agents] single-bracket block (no GSD marker, arbitrary user keys)', () => {
    ...
    // must itself be a table (the forge-* struct form).
    assert.ok(
    ...
    const gsdAgents = Object.keys(parsed.agents).filter((k) => k.startsWith('forge-'));
    assert.ok(
  ...
  test('strips [[agents]] sequence-form block without GSD marker (third-party / marker-edited-out)', () => {
    ...
    // a table-of-tables with forge-* struct keys.
    assert.ok(
      parsed.agents && typeof parsed.agents === 'object' && !Array.isArray(parsed.agents),
...
describe('#2760 fix 3 — Post-write Codex schema validation', { concurrency: false }, () => {
  test('passes a clean config produced by GSD install', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-2760-f3a-'));
    try {
  ...
  test('aborts install and restores pre-install backup when post-write validation fails', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-2760-f3b-'));
...
describe('#2760 fix 4 — Write-failure rollback (atomic write + snapshot restore)', { concurrency: false }, () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-2760-f4-'));
    codexHome = path.join(tmpDir, 'codex-home');
...
describe('#2760 CR4 finding 2 — Legacy flat [[hooks]] block migrates to namespaced AoT on reinstall', { concurrency: false }, () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-2760-cr4-f2-'));
    codexHome = path.join(tmpDir, 'codex-home');
  });
  ...
  test('pre-install legacy flat [[hooks]] forge-check-update + user namespaced [[hooks.SessionStart]] → post-install converges on namespaced AoT', () => {
    ...
    //     for forge-check-update. The pre-CR4 strip step would short-circuit
    ...
    const userPlusLegacy = [
      ...
      '[[hooks]]',
      'event = "SessionStart"',
      'command = "node /old/path/hooks/forge-check-update.js"',
      '',
    ].join('\n');
    ...
    // No duplicate forge-check-update entries — exactly one managed entry.
    const gsdEntries = hooksJsonCommands.filter((cmd) => typeof cmd === 'string' && /forge-check-update/.test(cmd));
    assert.equal(gsdEntries.length, 1,
      'exactly one forge-check-update entry after migration, got: ' + gsdEntries.length);
  });
});
...
describe('#2760 CR4 finding 1 — atomicWriteFileSync failure aborts install (post-write fatal)', { concurrency: false }, () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-2760-cr4-f1-'));
    codexHome = path.join(tmpDir, 'codex-home');
    originalRenameSync = fs.renameSync;
...
describe('#2760 CR5 finding 1 — pre-write failures abort install (outer catch fatal)', { concurrency: false }, () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-2760-cr5-f1-'));
    codexHome = path.join(tmpDir, 'codex-home');
    originalConsoleLog = console.log;
    consoleOutput = [];
    console.log = (...args) => { consoleOutput.push(args.join(' ')); };
...
describe('#2760 CR5 finding 3 — migration emits namespaced AoT (no flat/namespaced mixing)', { concurrency: false }, () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-2760-cr5-f3-'));
    codexHome = path.join(tmpDir, 'codex-home');
  });
  ...
  test('user has [[hooks.AfterTool]] AND legacy [hooks.SessionStart] → post-install both namespaced, no flat AoT', () => {
    ...
    // GSD's managed forge-check-update entry also lives in the namespaced array.
    const hooksJsonCommands = readHooksSessionStartCommands(codexHome);
    assert.ok(
      hooksJsonCommands.some((cmd) => typeof cmd === 'string' && /forge-check-update/.test(cmd)),
      'managed forge-check-update entry must appear in hooks.json SessionStart entries: ' +
        JSON.stringify(hooksJsonCommands)
    );
