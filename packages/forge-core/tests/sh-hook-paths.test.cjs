 ...
 *
 * #2046 (macOS/Linux): The three .sh hooks (forge-validate-commit.sh,
 * forge-session-state.sh, forge-phase-boundary.sh) were registered in
...
// buildHookCommand was extracted to forge-core/bin/lib/runtime-hooks-surface.cjs
...
const SH_HOOKS = [
  { name: 'forge-validate-commit.sh' },
  { name: 'forge-session-state.sh' },
  { name: 'forge-phase-boundary.sh' },
];
...
describe('bugs #2045 #2046: .sh hook paths must be absolute and quoted', () => {
  ...
  describe('buildHookCommand', () => {
    test('returns a bash command for .sh hookName', () => {
      ...
      const cmd = buildHookCommand(TEST_CONFIG_DIR, 'forge-validate-commit.sh', HOOK_OPTS);
    ...
    test('buildHookCommand produces bash runner for .sh and node runner for .js', () => {
      ...
      const shCmd = buildHookCommand(TEST_CONFIG_DIR, 'forge-validate-commit.sh', HOOK_OPTS);
      assert.ok(
      ...
      const jsCmd = buildHookCommand(TEST_CONFIG_DIR, 'forge-something.js', HOOK_OPTS);
      assert.ok(
    ...
    describe(`${name} — buildHookCommand output`, () => {
      ...
      for (const platform of ['linux', 'win32']) {
        test(`${platform}: configDir is absolute and double-quoted (fixes #2045 spaces)`, () => {
          ...
          // e.g. bash "/home/first last/.claude/hooks/forge-validate-commit.sh"
          //       or  "/home/first last/.claude/hooks/forge-validate-commit.sh"
          assert.ok(
