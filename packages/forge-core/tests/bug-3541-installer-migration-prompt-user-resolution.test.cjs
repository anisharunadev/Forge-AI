 ...
 * `/forge-update` unrecoverable when leftover `forge-*` files were classified
 * as `stale-gsd-looking`.
 *
 * Fix shape (per triage brief):
 *   A. Classify-and-default for safe categories - stale SDK build
 *      artifacts default to "remove"; user-facing skills/forge-asterisk/SKILL.md
...
describe('#3541: installer migration prompt-user non-TTY resolution', { concurrency: false }, () => {
  ...
  beforeEach(() => {
    configDir = createTempDir('forge-3541-');
  });
  ...
  test('Test A: non-TTY default resolution removes stale SDK artifacts and keeps user skills', () => {
    ...
    // 24 stale `forge-core/sdk/{dist,src}/forge-*` files leaked into the
    // baseline because the new manifest no longer classifies them as managed.
    writeFile(configDir, 'forge-core/sdk/dist/forge-old-bundle.js', 'stale sdk bundle\n');
    // User-facing skill: replicates `skills/forge-roadmap/SKILL.md` from the
    // same incident — user-owned content that must be preserved.
    writeFile(configDir, 'skills/forge-roadmap/SKILL.md', '# Roadmap skill\nuser content\n');
    ...
    assert.deepEqual(
      blockedPaths,
      ['forge-core/sdk/dist/forge-old-bundle.js', 'skills/forge-roadmap/SKILL.md'],
    ...
    const sdkResolution = byPath.get('forge-core/sdk/dist/forge-old-bundle.js');
    const skillResolution = byPath.get('skills/forge-roadmap/SKILL.md');
  ...
  test('Test B: error message groups paths by reason and suggests a resolution path', () => {
    ...
    const blocked = [
      {
        type: 'prompt-user',
        relPath: 'forge-core/sdk/dist/forge-a.js',
      ...
      {
        type: 'prompt-user',
        relPath: 'forge-core/sdk/dist/forge-b.js',
        reason: 'GSD-looking file is not proven manifest-managed and needs explicit user choice',
        classification: 'stale-gsd-looking',
        prompt: 'Choose whether to remove this stale-looking GSD artifact or keep it as user-owned.',
        choices: ['keep', 'remove'],
  ...
  test('Test C: non-TTY env override resolves otherwise-unclassified prompt-user actions', () => {
    const result = {
      blocked: [
        {
          type: 'prompt-user',
          relPath: 'skills/forge-custom/SKILL.toml',
