 ...
 *  3. e2e CLI tests via `forge-tools drift-guard severity/authority`.
 *  4. Structural test that plan-review-convergence.md invokes `forge_run drift-guard`.
 */
...
} = require('../forge-core/bin/lib/plan-drift-guard.cjs');
...
describe('forge-tools drift-guard — CLI e2e', () => {
  let tmpDir;

  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-drift-guard-'));
...
// ── 5. Structural test: plan-review-convergence.md invokes forge_run drift-guard

describe('plan-review-convergence.md uses forge_run drift-guard seam', () => {
  const WORKFLOW_PATH = path.join(
    __dirname, '..', 'forge-core', 'workflows', 'plan-review-convergence.md',
  );

  test('workflow contains forge_run drift-guard authority call', () => {
    const content = fs.readFileSync(WORKFLOW_PATH, 'utf-8');
    assert.ok(
      content.includes('forge_run drift-guard authority'),
      'plan-review-convergence.md must contain: forge_run drift-guard authority',
    );
  });
  ...
  test('workflow drift-guard authority call includes --raw (prevents JSON-quoted capture)', () => {
    ...
    assert.match(
      content,
      /forge_run drift-guard authority --raw/,
  ...
  test('workflow contains forge_run drift-guard severity call', () => {
    const content = fs.readFileSync(WORKFLOW_PATH, 'utf-8');
    assert.ok(
      content.includes('forge_run drift-guard severity'),
      'plan-review-convergence.md must contain: forge_run drift-guard severity',
    );
  });
  ...
  test('workflow drift-guard severity call passes --authority flag', () => {
    ...
    assert.match(
      content,
      /forge_run drift-guard severity[^\n]*--authority/,
