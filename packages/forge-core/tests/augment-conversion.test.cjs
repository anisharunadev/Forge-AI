...
`;
    ...
    const result = convertClaudeCommandToAugmentSkill(input, 'forge-quick');
...
`;
    ...
    const result = convertClaudeCommandToAugmentSkill(input, 'forge-plan-phase');
...
`;
    ...
    const result = convertClaudeCommandToAugmentSkill(input, 'forge-test');
...
`;
    ...
    assert.ok(result.includes('name: forge-test'), 'name preserved');
...
describe('convertClaudeToAugmentMarkdown', () => {
  ...
  test('normalizes forge: to forge-', () => {
    const input = 'Run /forge:new-project for new projects.';
    const result = convertClaudeToAugmentMarkdown(input);
    assert.strictEqual(result, 'Run /forge-new-project for new projects.');
  });
});