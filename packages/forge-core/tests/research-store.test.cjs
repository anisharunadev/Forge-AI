...
const {
  ...
  putResearch,
  getResearch,
} = require('../forge-core/bin/lib/research-store.cjs');
...
describe('research-store: getResearch missing key', () => {
  ...
  beforeEach(() => {
    tmpCwd = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-rs-cwd-'));
    tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-rs-home-'));
  });
...
describe('research-store: resolveStorePath tiers', () => {
  ...
  test("source 'curated' → under injected homeDir/.forge/research-cache", () => {
...
describe('research-store: I1 regression — source is the tier axis', () => {
  ...
  beforeEach(() => {
    tmpCwd = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-rs-i1-cwd-'));
    tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-rs-i1-home-'));
  });
  ...
  test('source:web + kind:docs → file in PROJECT dir, NOT user dir', () => {
    ...
    assert.ok(!fs.existsSync(userFile), 'file should NOT exist in user dir (~/.forge/research-cache)');
  });
  ...
  test('source:curated + kind:web → file in USER dir, NOT project dir', () => {
    ...
    assert.ok(fs.existsSync(userFile), 'file should exist in user dir (~/.forge/research-cache)');
...
describe('research-store: W4a regression — prefer fresh over stale across tiers', () => {
  ...
  beforeEach(() => {
    tmpCwd = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-rs-w4-cwd-'));
    tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-rs-w4-home-'));
  });
...
describe('research-store: W4b regression — blank version caps TTL', () => {
  ...
  beforeEach(() => {
    tmpCwd = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-rs-w4b-cwd-'));
    tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-rs-w4b-home-'));
  });
...
describe('research-store: isValidResearchKey exported', () => {
  const { isValidResearchKey } = require('../forge-core/bin/lib/research-store.cjs');
...
describe('research-store: putResearch rejects traversal key', () => {
  ...
  beforeEach(() => {
    tmpCwd = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-rs-trav-cwd-'));
    tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-rs-trav-home-'));
  });
...
describe('research-store: getResearch rejects traversal key', () => {
  ...
  beforeEach(() => {
    tmpCwd = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-rs-trav2-cwd-'));
    tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-rs-trav2-home-'));
  });
...
describe('research-store: getResearch rejects malformed cache metadata', () => {
  ...
  beforeEach(() => {
    tmpCwd = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-rs-malformed-cwd-'));
    tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-rs-malformed-home-'));
  });
