...
function runMinimalInstall({ runtime, scope, extraArgs = [] }) {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `forge-${runtime}-${scope}-`));
  try {
...
function manifestSkillSet(manifest) {
  ...
  for (const key of Object.keys(manifest.files)) {
    if (key.startsWith('skills/')) {
      ...
      // OpenCode/Kilo: command/forge-<cmd>.md
      const file = key.split('/')[1];
      out.add(file.replace(/^gsd-/, '').replace(/\.md$/, ''));
    } else if (key.startsWith('commands/forge/')) {
      ...
      // Claude local (#1367 fix): flat commands/forge-<cmd>.md
      const file = key.split('/')[1];
...
function collectSkillBasenamesOnDisk(configDir) {
  ...
  if (fs.existsSync(skillsDir)) {
    for (const entry of fs.readdirSync(skillsDir, { withFileTypes: true })) {
      ...
      } else if (entry.isFile() && entry.name.startsWith('forge-') && entry.name.endsWith('.md')) {
  ...
  const commandsGsdDir = path.join(configDir, 'commands', 'forge');
  ...
  // Claude local (#1367 fix): flat forge-*.md files at commands/ level
