...
// commands/forge/*.md files ARE the deployed registry — reading their frontmatter
 ...
 * registry: commands/forge/*.md (one file per registered command).
 *
 * Each command file has YAML frontmatter with a `name:` field:
 *   name: forge:slug    (colon-style — most commands)
 *   name: forge-slug    (dash-style — ns-* namespace commands)
 *
 * For each slug, three canonical token forms are emitted:
 *   /forge-slug   — Claude / non-Gemini runtimes
 *   /forge:slug   — Gemini runtime
 ...
 *   name: forge:slug     → slug = "slug"
 *   name: forge-slug     → slug = "slug"
 *   name: "forge:slug"   → slug = "slug"  (quoted)
...
function parseSlug(content, filePath) {
  ...
  // The value must be one of: forge:<slug> or forge-<slug>
  ...
  if (!nameMatch) {
    throw new Error(
      ...
      `(expected "name: forge:<slug>" or "name: forge-<slug>")`
    );
  }

  return nameMatch[2]; // the slug after "forge:" or "forge-"
}

/**
 * Returns the Set<string> of all canonical slash-command tokens derived from
 * commands/forge/*.md. Memoized — safe to call repeatedly without extra fs I/O.
 *
...
function getLiveCommandTokens() {
  ...
  for (const fileName of entries) {
    ...
    tokens.add(`/forge-${slug}`);   // Claude / non-Gemini
    tokens.add(`/forge:${slug}`);   // Gemini
