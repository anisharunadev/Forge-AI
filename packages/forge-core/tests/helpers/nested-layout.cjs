 ...
 * each commands/forge/ns-*.md router's `requires:` frontmatter list with the
...
const { parseRequires } = require('../../forge-core/bin/lib/install-profiles.cjs');

const COMMANDS_GSD = path.join(__dirname, '..', '..', 'commands', 'forge');
...
/**
 * Read a router's `requires:` child list from commands/forge/<routerStem>.md
 ...
 * prefix is '' for runtimes that nest under a skills/forge parent dir (hermes),
 * or 'forge-' for flat-prefixed runtimes (claude, cline, qwen, …).
 */
