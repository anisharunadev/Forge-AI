...
const MODEL_PROFILES = require('../forge-core/bin/lib/model-profiles.cjs').MODEL_PROFILES;
 ...
 * produces. forge-tools.cjs lives at <configDir>/forge-core/bin/forge-tools.cjs,
...
describe('init commands: agents_installed field (#1371)', () => {
  ...
  // Point the SDK at the repo's agents/ dir (sibling of forge-core/) via the
  ...
  test('init execute-phase includes agents_installed=true when agents exist', () => {
    ...
    assert.strictEqual(output.agents_installed, true,
      'agents_installed should be true when GSD_AGENTS_DIR has forge-*.md files');
  });
...
describe('validate health: agent installation check W010 (#1371)', () => {
  ...
  test('health check reports healthy when agents are installed (repo layout)', () => {
    // In the repo, agents/ exists as a sibling of forge-core/, so the
    // health check should find them via the forge-tools.cjs path resolution
...
describe('checkAgentsInstalled: Copilot .agent.md format (#1512)', () => {
  ...
  test('agents_installed=true when agents exist as .agent.md (Copilot format)', () => {
    // Simulate a Copilot install: agents are named forge-*.agent.md, not forge-*.md
...
describe('checkAgentsInstalled: Kimi agents/subagents layout', () => {
  test('Kimi install is detected by init, validate agents, and health checks', () => {
    const tmpDir = createTempProject('forge-kimi-agent-status-project-');
    const tmpConfig = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-kimi-agent-status-config-'));
    const tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-kimi-agent-status-home-'));
...
describe('bug #1058: validate agents detects manifest-backed .md/.toml pair drift', () => {
  // All real agents/forge-*.md files in the repo (used to populate fixtures)
  const REPO_AGENTS_DIR_1058 = path.resolve(__dirname, '..', 'agents');

  /**
   * Copy all forge-*.md files from the repo agents dir into destDir.
   */
  ...
  /**
   * Build a forge-file-manifest.json whose files map includes both .md and .toml
  ...
  test('agents_found=false and incomplete non-empty when .toml files are absent but manifest expects them', () => {
    ...
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-bug-1058-missing-toml-'));
    const agentsDir = path.join(tmpDir, 'agents');
    fs.mkdirSync(agentsDir, { recursive: true });
    ...
    fs.writeFileSync(
      path.join(tmpDir, 'forge-file-manifest.json'),
  ...
  test('agents_found=false and incomplete non-empty when .md files are absent but manifest expects them', () => {
    ...
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-bug-1058-missing-md-'));
    const agentsDir = path.join(tmpDir, 'agents');
    fs.mkdirSync(agentsDir, { recursive: true });
...
`);
  ...
  test('agents_found=true and incomplete=[] when both .md and .toml files are present', () => {
    ...
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-bug-1058-complete-pair-'));
    const agentsDir = path.join(tmpDir, 'agents');
    fs.mkdirSync(agentsDir, { recursive: true });
  ...
  test('no spurious incomplete flags when no manifest is present (protects claude/bundled installs)', () => {
    ...
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-bug-1058-no-manifest-'));
    const agentsDir = path.join(tmpDir, 'agents');
    fs.mkdirSync(agentsDir, { recursive: true });
    ...
    // Explicitly do NOT write forge-file-manifest.json
