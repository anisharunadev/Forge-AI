/**
 * Bug #3608: /forge:update must model Antigravity as a first-class runtime, so an
 ...
 * `forge-core/bin/lib/update-context.cjs` (resolveUpdateContext). The
...
} = require(path.join(ROOT, 'forge-core', 'bin', 'lib', 'update-context.cjs'));
const UPDATE_MD = path.join(ROOT, 'forge-core', 'workflows', 'update.md');
...
describe('bug #3608 / #498: update-context models Antigravity as a first-class runtime', () => {
  ...
  test('behavioral: an Antigravity install resolves to runtime "antigravity", not "gemini"', () => {
    ...
    const verFile = normKey(path.join(agDir, 'forge-core', 'VERSION'));
    const markerFile = normKey(path.join(agDir, 'forge-core', 'workflows', 'update.md'));
    const fakeFs = {
