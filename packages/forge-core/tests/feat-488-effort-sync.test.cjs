// Tests for forge-tools effort sync command (#488)
...
const GSD_TOOLS = path.resolve(__dirname, '../forge-core/bin/forge-tools.cjs');
...
describe('feat-488: effort sync command', () => {
  test('dry-run mode reports pending changes without writing files', () => {
    ...
    const agentPath = path.join(agentsDir, 'forge-planner.md');
    fs.writeFileSync(agentPath, AGENT_WITH_EFFORT);
    writePlanningConfig(tmpDir, { default: 'high', agent_overrides: { 'forge-planner': 'xhigh' } });

    const { cmdEffortSync } = require('../forge-core/bin/lib/commands.cjs');
    ...
    assert.equal(result.changes[0].agent, 'forge-planner');
  ...
  test('--apply mode rewrites effort: frontmatter to new config value', () => {
    ...
    const agentPath = path.join(agentsDir, 'forge-planner.md');
    fs.writeFileSync(agentPath, AGENT_WITH_EFFORT);
    writePlanningConfig(tmpDir, { default: 'low', agent_overrides: { 'forge-planner': 'xhigh' } });
  ...
  test('skips agents where effort: already matches config', () => {
    ...
    writePlanningConfig(tmpDir, { agent_overrides: { 'forge-planner': 'xhigh' } });
  ...
  test('injects effort: into agent files that lack the frontmatter key', () => {
    ...
    const agentsDir = makeAgentsDir(tmpDir);
    const agentPath = path.join(agentsDir, 'forge-executor.md');
  ...
  test('home-default effort config gap: applies home-level effort when project config has no effort section', () => {
    // The key #488 scenario: user changed ~/.forge/defaults.json effort settings
    ...
    // developer's real ~/.forge/defaults.json.
  ...
  test('CLI dispatcher: effort sync --apply routes through forge-tools correctly', () => {
    ...
    // forge-tools may print a startup banner before the JSON payload — parse from the first `{`.
