 ...
 *
 * Verifies that the forge-verifier agent filters gaps addressed in later
...
describe('verifier deferred-items filtering (#1624)', () => {
  ...
  describe('agents/forge-verifier.md', () => {
    ...
    test('contains Step 9b for filtering deferred items', () => {
      ...
      assert.ok(
        ...
        'forge-verifier.md should contain Step 9b or "Filter Deferred" section'
      );
    });
    ...
    test('Step 9b references roadmap analyze for cross-referencing', () => {
      ...
      assert.ok(
        ...
        'Step 9b should reference roadmap analyze (CJS or forge-sdk query) for loading full milestone data'
      );
    });
  ...
  describe('forge-core/workflows/verify-phase.md', () => {
    const workflowPath = path.join(ROOT, 'forge-core', 'workflows', 'verify-phase.md');
    let workflowContent;
  ...
  describe('forge-core/references/planner-gap-closure.md', () => {
    const closurePath = path.join(ROOT, 'forge-core', 'references', 'planner-gap-closure.md');
    let closureContent;
