 ...
 *
 * forge-check-update-worker.js uses a MANAGED_HOOKS array (now in the shared
 ...
 *
 * The original bug: the 3 bash hooks (forge-phase-boundary.sh,
 * forge-session-state.sh, forge-validate-commit.sh) were missing from
...
describe('bug #2136: MANAGED_HOOKS must include all shipped hook files', () => {
  // List all GSD-managed hook files in hooks/ (names starting with "forge-")
  const shippedHooks = fs.readdirSync(HOOKS_DIR)
    .filter(f => f.startsWith('forge-') && (f.endsWith('.js') || f.endsWith('.sh')));
  ...
  test('every shipped forge-*.js hook is in MANAGED_HOOKS', () => {
  ...
  test('every shipped forge-*.sh hook is in MANAGED_HOOKS', () => {
