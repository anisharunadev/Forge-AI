 ...
 * forge-session-state.sh, forge-validate-commit.sh, and forge-phase-boundary.sh
...
describe('#1834: installer deploys .sh hooks alongside .js hooks', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('forge-install-1834-');
  });
  ...
  test('forge-session-state.sh is present after install', () => {
    const hooksDir = runInstaller(tmpDir);
    const target = path.join(hooksDir, 'forge-session-state.sh');
    assert.ok(
      fs.existsSync(target),
      'forge-session-state.sh must be installed to hooks/ — missing file causes SessionStart hook errors'
    );
  });

  test('forge-validate-commit.sh is present after install', () => {
    const hooksDir = runInstaller(tmpDir);
    const target = path.join(hooksDir, 'forge-validate-commit.sh');
    assert.ok(
      fs.existsSync(target),
      'forge-validate-commit.sh must be installed to hooks/ — missing file causes PreToolUse hook errors'
    );
  });

  test('forge-phase-boundary.sh is present after install', () => {
    const hooksDir = runInstaller(tmpDir);
    const target = path.join(hooksDir, 'forge-phase-boundary.sh');
    assert.ok(
      fs.existsSync(target),
      'forge-phase-boundary.sh must be installed to hooks/ — missing file causes PostToolUse hook errors'
    );
  });
