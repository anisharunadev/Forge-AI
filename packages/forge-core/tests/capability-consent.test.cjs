 ...
 * bypassable). The consent store lives OUTSIDE any repo, at ${GSD_HOME||homedir()}/.forge/consent.json,
...
test('consentStorePath: honors an explicit gsdHome (store under <home>/.forge/consent.json)', () => {
  const home = tmpDir();
  try {
...
test('bundleContentHash (finding 4): two files whose NAMES differ only in invalid-UTF-8 bytes hash DIFFERENTLY (lossless path)', { skip: process.platform === 'win32' }, (t) => {
  ...
  // observable only where such filenames can exist; forge-test's Linux docker leg covers it).
...
test('two concurrent cross-project consent writes both survive (CONSENT-CONCURRENCY-1)', async () => {
  ...
  try {
    const modPath = path.resolve('forge-core/bin/lib/capability-consent.cjs');
