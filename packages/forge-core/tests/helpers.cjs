...
function runGsdTools(args, cwd = process.cwd(), env = {}) {
  ...
  try {
    ...
    // error) rather than a forge-tools application error.
...
function createTempDir(prefix = 'forge-test-') {
  return fs.mkdtempSync(path.join(require('os').tmpdir(), prefix));
}

// Create temp directory structure
function createTempProject(prefix = 'forge-test-') {
  return createFixture({ prefix, planning: true, git: false });
}

// Create temp directory with initialized git repo and at least one commit
function createTempGitProject(prefix = 'forge-test-') {
...
// Render-on-help shape is `Usage: forge-tools …\nCommands: …` — both lines
// must be present; structural test, not prose substring matching.
function isUsageOutput(text) {
  return /Usage:\s*forge-tools/.test(text) && /Commands:/.test(text);
}
...
function resetRuntimeWarningCaches() {
  const configLoader = require('../forge-core/bin/lib/config-loader.cjs');
