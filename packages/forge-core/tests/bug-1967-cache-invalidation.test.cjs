 ...
 *   - SDK callers that require() forge-tools.cjs as a module
...
const state = require('../forge-core/bin/lib/state.cjs');
...
describe('buildStateFrontmatter cache invalidation (#1967)', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-1967-cache-'));
