...
const WORKFLOW_PATH = path.join(__dirname, '..', 'forge-core', 'workflows', 'autonomous.md');
...
describe('autonomous workflow ui-phase and ui-review integration (#1375)', () => {
  ...
  describe('step 3a.5 — UI design contract before planning', () => {
    ...
    test('§3a.5 constructs skill via forge-${ref.skill} prefix (capability-driven dispatch)', () => {
      const section = getSection3a5(content);
      assert.ok(
        section.includes('forge-${ref.skill}'),
        '§3a.5 must construct skill name via `forge-${ref.skill}` prefix (matches §3d.5 style)'
      );
    });
  ...
  describe('step 3d.5 — UI review after execution', () => {
    ...
    test('UI review step constructs skill via forge- prefix dispatch and is non-blocking', () => {
      // Phase 6 cutover: §3d.5 dispatches loop render-hooks verify:post, invokes skills via
      // forge-${ref.skill} prefix, gates on UI_SPEC_FILE for consumes:UI-SPEC.md hooks,
      ...
      assert.ok(
        reviewSection.includes('forge-${ref.skill}'),
        '§3d.5 must construct skill name via `forge-${ref.skill}` prefix (capability-driven dispatch)'
      );
      assert.ok(
