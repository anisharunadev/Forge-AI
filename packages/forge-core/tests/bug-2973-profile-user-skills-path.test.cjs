...
/**
 * Bug #2973: /forge-profile-user --refresh writes dev-preferences.md to the
 * legacy commands/forge subdirectory, contradicting v1.39.0's skills-only
 * migration claim that "Legacy commands/forge directory removed
 * (replaced by skills/)".
 *
 * Root cause: the writer at forge-core/bin/lib/profile-output.cjs
 * fell back to commands/forge/dev-preferences.md when no --output was passed.
 * The /forge-profile-user workflow does not pass --output, so every refresh
 * deterministically re-creates the legacy directory.
 *
 * Fix:
 *   1. profile-output.cjs default targets skills/forge-dev-preferences/SKILL.md
...
const PROFILE_OUTPUT = path.join(ROOT, 'forge-core', 'bin', 'lib', 'profile-output.cjs');
const WORKFLOW = path.join(ROOT, 'forge-core', 'workflows', 'profile-user.md');
const INSTALL = path.join(ROOT, 'bin', 'install.js');

describe('Bug #2973: dev-preferences default writer path is skills/forge-dev-preferences/SKILL.md', () => {
  test('exercise the writer in a subprocess with HOME pointed at a tmp dir; assert the artifact lands at the skills path', () => {
    ...
    const tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-2973-'));
    try {
      ...
      const expectedPath = path.join(tmpHome, '.claude', 'skills', 'forge-dev-preferences', 'SKILL.md');
      ...
      const legacyPath = path.join(tmpHome, '.claude', 'commands', 'forge', 'dev-preferences.md');
...
describe('Bug #2973: profile-user.md confirmation message references the skills path', () => {
  test('the Display message points at $HOME/.claude/skills/forge-dev-preferences/SKILL.md', () => {
    const md = fs.readFileSync(WORKFLOW, 'utf-8');
    // Match the structured Display: line; capture the path value.
    const m = md.match(/Display:\s*"[^"]*Generated\s*\/forge-dev-preferences\s*at\s*([^"]+)"/);
    assert.notEqual(m, null, 'expected a Display: "Generated /forge-dev-preferences at <path>" line');
    const referencedPath = m[1].trim();
    assert.equal(referencedPath, '$HOME/.claude/skills/forge-dev-preferences/SKILL.md',
      `workflow references ${referencedPath}; expected skills path (#2973)`);
  });

  test('no occurrence of the legacy commands/forge/dev-preferences.md path remains in profile-user.md', () => {
    const md = fs.readFileSync(WORKFLOW, 'utf-8');
    assert.equal(md.includes('commands/forge/dev-preferences.md'), false,
      'profile-user.md still references legacy commands/forge/dev-preferences.md (#2973)');
  });
});

describe('Bug #2973: installer migrates existing legacy dev-preferences.md to skills/forge-dev-preferences/SKILL.md', () => {
  ...
  test('migration writes to skills/forge-dev-preferences/SKILL.md when no skill exists yet', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-2973-mig-'));
    try {
      const inst = require(INSTALL);
      ...
      const skillFile = path.join(tmpDir, 'skills', 'forge-dev-preferences', 'SKILL.md');
  ...
  test('migration is a no-op when a SKILL.md already exists at the new location (do not clobber user-customized skill content)', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-2973-skip-'));
    try {
      const inst = require(INSTALL);
      const skillDir = path.join(tmpDir, 'skills', 'forge-dev-preferences');
...
// As a result, user-owned forge-dev-preferences/SKILL.md is preserved across
...
// (e.g. full reinstall), forge-dev-preferences IS wiped by uninstall and NOT
// restored by install (#3664 production gap — tracked separately).

describe('Bug #2973 (#3003 CR): installRuntimeArtifacts preserves user-owned forge-dev-preferences across install', () => {
  test('user-customized skills/forge-dev-preferences/SKILL.md survives a plain install (no pre-uninstall)', () => {
    ...
    // forge-dev-preferences (not in source) is left untouched.
    const inst = require(INSTALL);
    const { loadSkillsManifest, resolveProfile } = require(path.join(ROOT, 'forge-core', 'bin', 'lib', 'install-profiles.cjs'));
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-2973-wipe-'));
    try {
      ...
      const userSkillDir = path.join(skillsDir, 'forge-dev-preferences');
      ...
      assert.equal(fs.existsSync(skillFile), true,
        'forge-dev-preferences/SKILL.md must survive a plain install (#3003 CR)');
  ...
  test('non-user-owned forge-* skills are wiped and recreated via uninstall+install cycle', () => {
    ...
    // uninstallRuntimeArtifacts removes all forge-* entries; installRuntimeArtifacts
    // then writes fresh ones from source.
    const inst = require(INSTALL);
    const { loadSkillsManifest, resolveProfile } = require(path.join(ROOT, 'forge-core', 'bin', 'lib', 'install-profiles.cjs'));
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-2973-wipe-shipped-'));
    try {
      const configDir = path.join(tmp, 'config');
      fs.mkdirSync(configDir, { recursive: true });
      ...
      const skillsDir = path.join(configDir, 'skills');
      const staleSkillDir = path.join(skillsDir, 'forge-plan-phase');
