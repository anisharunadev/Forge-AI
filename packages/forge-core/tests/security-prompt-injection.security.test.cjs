 ...
 *
 * Seam scope per #3596:
 *   - hooks/forge-prompt-guard.js          — stdin/stdout JSON contract
 *   - hooks/forge-read-injection-scanner.js
 *   - forge-core/bin/lib/security.cjs      — sanitizer + validators
 *   - forge-core/bin/lib/workstream-name-policy.cjs
 *   - forge-core/bin/forge-tools.cjs CLI     — full-stack contract
 *
...
const PROMPT_GUARD_HOOK = path.join(REPO_ROOT, 'hooks', 'forge-prompt-guard.js');
const READ_SCANNER_HOOK = path.join(REPO_ROOT, 'hooks', 'forge-read-injection-scanner.js');
...
const {
  ...
  validatePhaseNumber,
  validateFieldName,
} = require('../forge-core/bin/lib/security.cjs');
const {
  toWorkstreamSlug,
  hasInvalidPathSegment,
  isValidActiveWorkstreamName,
} = require('../forge-core/bin/lib/workstream-name-policy.cjs');
...
function sentinelPath(label) {
  return path.join(
    os.tmpdir(),
    `forge-3596-sentinel-${label}-${process.pid}-${Date.now()}`,
  );
}
...
describe('CLI: hostile workstream names cannot escape or execute', () => {
  let tmpDir;
  beforeEach(() => { tmpDir = createTempGitProject('forge-3596-ws-'); });
  ...
  test('--ws traversal value is rejected before any planning IO', () => {
    const escape = path.join(tmpDir, '..', '..', '..', 'forge-3596-traverse-marker');
...
describe('CLI: fake-token env values are never echoed back', () => {
  let tmpDir;
  beforeEach(() => { tmpDir = createTempGitProject('forge-3596-secret-'); });
  afterEach(() => { cleanup(tmpDir); });
...
// ─── Hook: forge-prompt-guard advisory contract ───────────────────────────────

describe('forge-prompt-guard: hostile .planning/ writes are advised, not blocked', () => {
...
// ─── Hook: forge-read-injection-scanner advisory contract ─────────────────────

describe('forge-read-injection-scanner: hostile reads are flagged with severity', () => {
...
describe('validatePath: hostile path values are rejected before write', () => {
  let tmpDir;
  beforeEach(() => { tmpDir = createTempGitProject('forge-3596-path-'); });
  afterEach(() => { cleanup(tmpDir); });
  ...
  test('symlink escaping the base is rejected', () => {
    ...
    const outside = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-3596-escape-'));
...
// and hooks/forge-read-injection-scanner.js (inlined for hook independence).
  ...
  test('every canonical MARKDOWN_LINK_PATTERN source string appears in the hook source', () => {
    ...
    for (const entry of MARKDOWN_LINK_PATTERNS) {
      ...
      assert.ok(
        ...
        `Hook forge-read-injection-scanner.js must contain pattern source: ${src}`,
      );
    }
  });
});
