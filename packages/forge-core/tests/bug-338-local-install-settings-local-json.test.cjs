...
describe('#338 case 1: fresh local Claude install writes to settings.local.json', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-338-local-'));
  });
...
describe('#338 case 2: global Claude install continues to write to settings.json', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-338-global-'));
  });
...
describe('#338 case 3: migration of prior local install GSD entries from settings.json to settings.local.json', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-338-migrate-'));
  });
  ...
  test('GSD hook entries are moved from settings.json to settings.local.json', (t) => {
        ...
        SessionStart: [
          {
            hooks: [
              {
                type: 'command',
                command: `${process.execPath} ${path.join(claudeDir, 'hooks', 'forge-check-update.js')}`,
              }
            ]
          }
        ],
        PostToolUse: [
          {
            ...
            hooks: [
              {
                type: 'command',
                command: `${process.execPath} ${path.join(claudeDir, 'hooks', 'forge-context-monitor.js')}`,
                timeout: 10,
              }
            ]
          }
    ...
    const hasGsdUpdateHook = sessionStartHooks.some(
      entry => entry && entry.hooks && Array.isArray(entry.hooks) &&
        entry.hooks.some(h => h && h.command && h.command.includes('forge-check-update'))
    );
    assert.ok(
      hasGsdUpdateHook,
      'settings.local.json must contain the migrated forge-check-update hook after migration'
    );
  });
