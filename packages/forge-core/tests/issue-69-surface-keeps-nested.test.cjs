...
// assert the flat layout (>= 60 top-level forge-* entries, concrete skills discoverable).
...
describe('issue-69: applySurface preserves nested skill layout (no re-flatten)', () => {
  ...
  test('cline global full: applySurface keeps 6 router dirs and nested forge-ns-manage/skills/help/SKILL.md', (t) => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-69-surface-'));
    ...
    assert.strictEqual(
      ...
      6,
      `Install must produce exactly 6 forge-* top-level dirs (routers). Got ${topLevelAfterInstall.length}: [${topLevelAfterInstall.join(', ')}]`,
    );
    assert.ok(
      fs.existsSync(path.join(skillsDir, 'forge-ns-workflow', 'skills', 'plan-phase', 'SKILL.md')),
      'After install: forge-ns-workflow/skills/plan-phase/SKILL.md must exist',
    );
    ...
    assert.strictEqual(
      ...
      6,
      `After applySurface: expected exactly 6 forge-* top-level dirs (routers only). Got ${topLevelAfterSurface.length}: [${topLevelAfterSurface.join(', ')}]. ` +
    ...
    assert.ok(
      fs.existsSync(path.join(skillsDir, 'forge-ns-workflow', 'skills', 'plan-phase', 'SKILL.md')),
      'After applySurface: forge-ns-workflow/skills/plan-phase/SKILL.md must still exist (nested layout preserved)',
    );
    ...
    assert.ok(
      ...
      'After applySurface: forge-plan-phase/ must NOT exist at top level (#69 re-flatten regression guard)',
    );
  });
  ...
  test('claude global full: install produces flat layout and applySurface preserves it (#924)', (t) => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-924-69-'));
    t.after(() => { try { cleanup(tmpDir); } catch { /* best-effort */ } });
    ...
    // Install must produce FLAT layout (>= 60 forge-* dirs)
    const topLevelAfterInstall = fs.readdirSync(skillsDir).filter((n) => n.startsWith('forge-'));
    assert.ok(
      topLevelAfterInstall.length >= 60,
      `Claude install must produce >= 60 forge-* top-level dirs (flat, #924). Got ${topLevelAfterInstall.length}.`,
    );

    // forge-plan-phase must be directly at top level
    assert.ok(
      fs.existsSync(path.join(skillsDir, 'forge-plan-phase', 'SKILL.md')),
      'After claude install: forge-plan-phase/SKILL.md must be at top level (flat layout, #924)',
    );

    // No nested skills/ subdirs under forge-ns-* in Claude
    assert.ok(
      !fs.existsSync(path.join(skillsDir, 'forge-ns-workflow', 'skills')),
      'After claude install: forge-ns-workflow/skills/ must NOT exist (flat layout, no nesting, #924)',
    );
    ...
    assert.ok(
      ...
      `After applySurface: claude must still have >= 60 forge-* dirs (flat preserved). Got ${topLevelAfterSurface.length}.`,
    );

    assert.ok(
      fs.existsSync(path.join(skillsDir, 'forge-plan-phase', 'SKILL.md')),
      'After applySurface: forge-plan-phase/SKILL.md must remain at top level (#924)',
    );
  });
});
