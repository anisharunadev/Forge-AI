 ...
 * /forge:surface enable <cluster> — the advertised use-case from ADR-0011.
 *
...
describe('PROFILES.core — ADR-0011 expand contract', () => {
  test("PROFILES.core includes 'surface' so users can expand via /forge:surface enable", () => {
    ...
    // ADR-0011 documents that --profile=core users expand via /forge:surface enable <cluster>.
