 ...
 *
 * Structural assertions for the /forge-import command and workflow files.
 */
...
const CMD_PATH = path.join(__dirname, '..', 'commands', 'forge', 'import.md');
const WF_PATH = path.join(__dirname, '..', 'forge-core', 'workflows', 'import.md');

// ─── File Existence ────────────────────────────────────────────────────────────

describe('import command file structure', () => {
  test('command file exists', () => {
    assert.ok(fs.existsSync(CMD_PATH), 'commands/forge/import.md should exist');
  });

  test('workflow file exists', () => {
    assert.ok(fs.existsSync(WF_PATH), 'forge-core/workflows/import.md should exist');
  });
});
...
describe('import command frontmatter', () => {
  ...
  test('has name field', () => {
    assert.match(content, /^name:\s*forge:import$/m);
  });
...
describe('import command references', () => {
  ...
  test('references the import workflow', () => {
    assert.ok(
      content.includes('@~/.claude/forge-core/workflows/import.md'),
      'command should reference the workflow via @~/.claude/forge-core/workflows/import.md'
    );
  });
});
...
describe('import workflow content', () => {
  ...
  test('includes plan-checker validation gate', () => {
    assert.ok(
      content.includes('forge-plan-checker'),
      'workflow should delegate validation to forge-plan-checker'
    );
  });

  test('no-args usage display is present', () => {
    assert.ok(
      content.includes('Usage: /forge:import') || content.includes('Usage: /forge-import'),
