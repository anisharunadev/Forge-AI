 ...
 * - forge-security-auditor agent frontmatter and structure
...
const COMMANDS_DIR = path.join(REPO_ROOT, 'commands', 'forge');
const WORKFLOWS_DIR = path.join(REPO_ROOT, 'forge-core', 'workflows');
const TEMPLATES_DIR = path.join(REPO_ROOT, 'forge-core', 'templates');

// ─── 1. Agent frontmatter — forge-security-auditor.md ─────────────────────────

describe('SECURE: forge-security-auditor agent', () => {
  const agentPath = path.join(AGENTS_DIR, 'forge-security-auditor.md');

  test('agent file exists', () => {
    assert.ok(
      fs.existsSync(agentPath),
      'forge-security-auditor.md must exist in agents/'
    );
  });
  ...
  test('name is forge-security-auditor', () => {
    const content = fs.readFileSync(agentPath, 'utf-8');
    const frontmatter = content.split('---')[1] || '';
    assert.ok(
      frontmatter.includes('name: forge-security-auditor'),
      'name must be forge-security-auditor'
    );
  });
...
describe('SECURE: secure-phase command file', () => {
  ...
  test('command file exists', () => {
    assert.ok(
      ...
      'secure-phase.md must exist in commands/forge/'
    );
  });

  test('has valid frontmatter with name forge:secure-phase', () => {
    const content = fs.readFileSync(cmdPath, 'utf-8');
    const frontmatter = content.split('---')[1] || '';
    assert.ok(
      frontmatter.includes('name: forge:secure-phase'),
      'name must be forge:secure-phase'
    );
  });
...
describe('SECURE: secure-phase workflow file', () => {
  ...
  test('workflow file exists', () => {
    assert.ok(
      ...
      'secure-phase.md must exist in forge-core/workflows/'
    );
  });

  test('contains forge-security-auditor reference', () => {
    const content = fs.readFileSync(wfPath, 'utf-8');
    assert.ok(
      content.includes('forge-security-auditor'),
      'must reference forge-security-auditor agent'
    );
  });
...
describe('SECURE: SECURITY.md template', () => {
  ...
  test('template exists', () => {
    assert.ok(
      ...
      'SECURITY.md must exist in forge-core/templates/'
    );
  });
...
describe('SECURE: config.json security defaults', () => {
  ...
  test('config template exists', () => {
    assert.ok(
      ...
      'config.json must exist in forge-core/templates/'
    );
  });
...
describe('SECURE: VALIDATION.md security columns', () => {
  ...
  test('VALIDATION.md template exists', () => {
    assert.ok(
      ...
      'VALIDATION.md must exist in forge-core/templates/'
    );
  });
...
describe('SECURE: per-threat severity gate (#1626)', () => {
  const plannerPath = path.join(AGENTS_DIR, 'forge-planner.md');
  const auditorPath = path.join(AGENTS_DIR, 'forge-security-auditor.md');
  const tplPath = path.join(TEMPLATES_DIR, 'SECURITY.md');
  const configDocPath = path.join(REPO_ROOT, 'forge-core', 'references', 'planning-config.md');

  // ── planner: Severity column in threat register header ──────────────────
  test('forge-planner.md threat_model register header has Severity column', () => {
  ...
  test('forge-planner.md security instruction assigns severity to each threat', () => {
    const content = fs.readFileSync(plannerPath, 'utf-8');
    assert.ok(
  ...
  test('forge-planner.md checklist has Severity item', () => {
    const content = fs.readFileSync(plannerPath, 'utf-8');
    assert.ok(
  ...
  test('forge-security-auditor.md block_on domain is severity vocabulary', () => {
  ...
  test('forge-security-auditor.md defines severity ordering critical > high > medium > low', () => {
    const content = fs.readFileSync(auditorPath, 'utf-8');
    assert.ok(
  ...
  test('forge-security-auditor.md threats_open counts only open threats at or above block_on', () => {
    const content = fs.readFileSync(auditorPath, 'utf-8');
    assert.ok(
  ...
  test('forge-security-auditor.md documents non-blocking below-threshold opens', () => {
    const content = fs.readFileSync(auditorPath, 'utf-8');
    assert.ok(
  ...
  test('forge-security-auditor.md BLOCKER classification conditions blocking on severity threshold (no unconditional all-open-blocks language)', () => {
    const content = fs.readFileSync(auditorPath, 'utf-8');
  ...
  test('forge-security-auditor.md states fail-closed rule for missing/unranked severity', () => {
    const content = fs.readFileSync(auditorPath, 'utf-8');
    assert.ok(
