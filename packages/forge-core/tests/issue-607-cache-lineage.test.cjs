 ...
 * are correctly enforced across forge-update-banner.js, forge-statusline.js,
...
const { PACKAGE_NAME, updateCacheFileName } = require('../forge-core/bin/lib/package-identity.cjs');
const { buildBannerOutput } = require('../hooks/forge-update-banner.js');
const { evaluateUpdateCache } = require('../hooks/forge-statusline.js');

// ─── Package identity constants ──────────────────────────────────────────────

describe('package-identity exports', () => {
  test('PACKAGE_NAME is @forge-ai/forge-core', () => {
    assert.equal(PACKAGE_NAME, '@forge-ai/forge-core');
  });

  test('updateCacheFileName is per-package filename', () => {
    assert.equal(updateCacheFileName, 'forge-update-check-forge-ai-forge-core.json');
  });
});
...
describe('buildBannerOutput lineage guard', () => {
  ...
  test('returns banner when package_name matches PACKAGE_NAME', () => {
    ...
    assert.ok(out.systemMessage.includes('/forge:update'));
  });
...
describe('evaluateUpdateCache lineage guard', () => {
  ...
  test('returns staleWarning=stale when stale_hooks present and matching package_name', () => {
    const r = evaluateUpdateCache({
      ...
      latest: '1.3.0',
      package_name: '@forge-ai/forge-core',
      stale_hooks: [{ file: 'forge-statusline.js', hookVersion: '1.2.0', installedVersion: '1.3.0' }],
    });
  ...
  test('returns staleWarning=dev when installed > latest (dev install) and matching package_name', () => {
    const r = evaluateUpdateCache({
      ...
      latest: '1.3.0',
      package_name: '@forge-ai/forge-core',
      stale_hooks: [{ file: 'forge-statusline.js', hookVersion: '1.2.0', installedVersion: '2.0.0' }],
    });
