...
before(() => {
  ...
  conversion = require('../forge-core/bin/lib/runtime-artifact-conversion.cjs');
});
...
describe('_computePathPrefix', () => {
  ...
  test('backslash-style resolvedTarget is normalized to forward slashes (#1615 regression)', () => {
    ...
    assert.strictEqual(prefix, 'C:/Users/runner/AppData/Local/Temp/forge-1615-windsurf/');
...
describe('rewriteStagedSkillBodies', () => {
  test('rewrites .md files in-place for cursor runtime', () => {
    const stagedDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-test-staged-'));
    const configDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-test-config-'));
    try {
      ...
      const skillDir = path.join(stagedDir, 'forge-test-skill');
  ...
  test('with injected homedir: global under home uses $HOME prefix', () => {
    ...
    const HOME = path.resolve(os.tmpdir(), 'forge-1511-fake-home');
    const configDir = path.join(HOME, '.cursor');
    const stagedDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-test-staged-'));
    try {
      const skillDir = path.join(stagedDir, 'forge-help');
      fs.mkdirSync(skillDir, { recursive: true });
...
describe('rewriteStagedCommandBodies', () => {
  test('returns a temp dir (not the source dir) with rewritten content', () => {
    const stagedDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-test-cmd-'));
    const configDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-test-config-'));
    let tempDir;
    try {
...
// on any exception and NOT leave an orphaned forge-cmd-rewrites-* directory.
// ---------------------------------------------------------------------------
...
describe('applyRuntimeContentRewritesForCommandsInPlace — error-path tempDir cleanup', () => {
  test('rmSync is called on the tempDir when readFileSync throws (deterministic monkeypatch)', () => {
    ...
    const stagedDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-test-error-path-'));
    fs.writeFileSync(path.join(stagedDir, 'x.md'), '# test\n');

    const before = new Set(
      fs.readdirSync(os.tmpdir()).filter(n => n.startsWith('forge-cmd-rewrites-'))
    );
    ...
    try {
      ...
      const after = fs.readdirSync(os.tmpdir()).filter(n => n.startsWith('forge-cmd-rewrites-'));
      ...
      cleanup(stagedDir);
      // Clean up any genuinely leaked forge-cmd-rewrites-* dirs so the runner stays clean.
...
describe('layout module no longer exports getInstallExports', () => {
  test('getInstallExports is not on the layout module export', () => {
    process.env['GSD_TEST_MODE'] = '1';
    const layout = require('../forge-core/bin/lib/runtime-artifact-layout.cjs');
    assert.equal(
...
describe('single-owner reference-identity guard (ADR-1508 / #1511 Phase 2)', () => {
  ...
  before(() => {
    ...
    conversionCjs = require('../forge-core/bin/lib/runtime-artifact-conversion.cjs');
  });
