...
const { mergeFederatedConfig } = require('../forge-core/bin/lib/federated-config.cjs');
...
describe('real registry: capability config keys are federated', () => {
  test('with real capability-registry, configSchema keys are accepted through the federated channel', () => {
    const capRegistry = require('../forge-core/bin/lib/capability-registry.cjs');
    const configSchemaFromRegistry = capRegistry.configSchema;

    const configSchemaModule = require('../forge-core/bin/lib/config-schema.cjs');
