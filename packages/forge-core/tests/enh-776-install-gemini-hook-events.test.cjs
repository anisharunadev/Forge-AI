...
// forge-file-manifest.json), then add the stubs, then run install again. On the
 ...
 *
 * Pass 1: install() with no hook stubs — writes forge-file-manifest.json.
...
describe('enh-776: Gemini install registers 3 new hook events', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('forge-776-gemini-');
  ...
  test('BeforeAgent / AfterAgent / BeforeModel all use forge-context-monitor', () => {
...
describe('enh-776: Claude install does NOT register Gemini-only hook events', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('forge-776-claude-');
    previousCwd = process.cwd();
    process.chdir(tmpDir);
...
describe('enh-776: Antigravity install does NOT register Gemini-only hook events', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('forge-776-antigravity-');
    previousCwd = process.cwd();
    process.chdir(tmpDir);
...
describe('enh-776: Gemini install is idempotent across persisted reinstalls', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('forge-776-idem-');
    previousCwd = process.cwd();
    process.chdir(tmpDir);
  });
...
describe('enh-776: Gemini uninstall removes new hook event entries', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('forge-776-uninstall-');
    previousCwd = process.cwd();
    process.chdir(tmpDir);
...
describe('enh-776: hooksConfig.enabled:false warning during Gemini install', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('forge-776-hookscfg-');
    previousCwd = process.cwd();
    process.chdir(tmpDir);
