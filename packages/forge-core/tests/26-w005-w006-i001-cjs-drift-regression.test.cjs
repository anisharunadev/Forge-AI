...
/**
 * Regression tests for issue #26 (forge-ai/forge-core).
 ...
 *
 * References:
 *   - Issue #26 (forge-ai/forge-core) — three drift items + reproducer
...
//   node .claude/forge-core/bin/forge-tools.cjs validate health
...
describe('Drift item W005 — phaseDirNameRe: 999.X-name dirs must not trigger W005', () => {
  ...
  before(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-26-d1-'));
  ...
  test('phaseDirNameRe is exported from validate.cjs', () => {
    const gen = require('../forge-core/bin/lib/validate.cjs');
...
describe('Drift item W006-archived — MILESTONE_ARCHIVE_DIR_RE and PHASE_TOKEN_FROM_DIR_RE', () => {
  ...
  before(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-26-d2-'));
    const { planningDir, phasesDir } = mkplanning(tmpDir);
    writeProjectMd(planningDir);
...
describe('Drift item I001 — canonicalPlanStem: long PLAN stem matches short SUMMARY stem', () => {
  ...
  before(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-26-d3-'));
    const { planningDir, phasesDir } = mkplanning(tmpDir);
    writeProjectMd(planningDir);
