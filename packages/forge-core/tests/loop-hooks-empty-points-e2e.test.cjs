...
const GSD_TOOLS = path.join(__dirname, '..', 'forge-core', 'bin', 'forge-tools.cjs');
const realRegistry = require('../forge-core/bin/lib/capability-registry.cjs');
const { resolveLoopHooks, renderLoopHooks } = require('../forge-core/bin/lib/loop-resolver.cjs');

// ─── Fixture helpers ──────────────────────────────────────────────────────────

/** Create a bare temp dir with .planning/ layout (no config.json) */
function makeTempProject() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-loop-e2e-'));
...
function makeBareDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'forge-loop-bare-'));
}

/** Spawn forge-tools via raw spawnSync; returns { status, stdout, stderr } */
