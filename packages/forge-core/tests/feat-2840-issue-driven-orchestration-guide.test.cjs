...
function parseGuide() {
  ...
  // forge-new-workspace, .planning/, etc. must survive).
  ...
  // guide. This prevents drift if /forge-* command names are renamed.
  ...
  const conceptPairs = conceptMappingSection
    ? {
        ...
        workspaceCommand: /\/forge-workspace\s+--new/.test(conceptMappingSection),
        executionCommand:
          /\/forge-manager/.test(conceptMappingSection) ||
          /\/forge-autonomous/.test(conceptMappingSection),
        verifyCommand: /\/forge-verify-work/.test(conceptMappingSection),
        reviewCommand: /\/forge-review/.test(conceptMappingSection),
        shipCommand: /\/forge-ship/.test(conceptMappingSection),
      }
    : null;
...
describe('issue-driven-orchestration guide (#2840)', () => {
  ...
  test('concept mapping section exists and pairs Symphony concepts with GSD primitives', () => {
    ...
    const expected = {
      ...
      workspaceCommand: '/forge-workspace --new must appear in the concept mapping',
      executionCommand:
        '/forge-manager or /forge-autonomous must appear in the concept mapping',
      verifyCommand: '/forge-verify-work must appear in the concept mapping',
      reviewCommand: '/forge-review must appear in the concept mapping',
      shipCommand: '/forge-ship must appear in the concept mapping',
    };
