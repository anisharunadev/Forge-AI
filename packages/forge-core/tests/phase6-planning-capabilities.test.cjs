...
const PLAN_PHASE_PATH = path.join(ROOT, 'forge-core', 'workflows', 'plan-phase.md');
...
describe('ADR-857 phase 6 planning capability migration', () => {
  ...
  test('research and pattern mapper prompts are capability-owned fragments', () => {
    ...
    assert.equal(research.ref.agent, 'forge-phase-researcher');
    ...
    assert.equal(patternMapper.ref.agent, 'forge-pattern-mapper');
