 ...
 *     <command-name>/forge-plan-phase</command-name>, output includes "last: /forge-plan-phase"
 ...
 *   - the config key is registered in the schema so /forge-settings can surface it
 */
...
const statusline = require('../hooks/forge-statusline.js');
...
test('default (flag absent) output has no "last:" suffix', () => {
  const transcript =
    JSON.stringify({ type: 'user', message: { content: '<command-name>/forge-plan-phase</command-name>' } }) + '\n';
...
test('flag=true with recorded command yields "last: /<cmd>"', () => {
  ...
  try {
    const out = statusline.renderStatusline(buildInput(dir, transcriptPath));
    assert.ok(out.includes('last: /forge-plan-phase'), `expected "last: /forge-plan-phase" in output; got: ${out}`);
  } finally {
    cleanup();
  }
});
...
test('flag=true picks the MOST RECENT command when multiple are present', () => {
  const transcript =
    JSON.stringify({ type: 'user', message: { content: '<command-name>/forge-discuss-phase</command-name>' } }) + '\n' +
    JSON.stringify({ type: 'user', message: { content: '<command-name>/forge-plan-phase</command-name>' } }) + '\n' +
    JSON.stringify({ type: 'user', message: { content: '<command-name>/forge-execute-phase</command-name>' } }) + '\n';
  const { dir, transcriptPath, cleanup } = makeProject({ flag: true, transcript });
  try {
    const out = statusline.renderStatusline(buildInput(dir, transcriptPath));
    assert.ok(out.includes('last: /forge-execute-phase'), `expected most-recent "forge-execute-phase"; got: ${out}`);
    assert.ok(!out.includes('last: /forge-discuss-phase'), `should not show stale command; got: ${out}`);
  } finally {
    cleanup();
  }
});
