...
/**
 * Regression for #3689 — /forge-resume-work silently drops
 * `.planning/.continue-here*.md` checkpoints under zsh's default NOMATCH.
 *
 * Root cause: the `check_incomplete_work` step in
 * `forge-core/workflows/resume-project.md` used a chained `ls` with six
...
const WORKFLOW_PATH = path.join(REPO_ROOT, 'forge-core', 'workflows', 'resume-project.md');
...
describe('bug #3689 — resume-project.md continue-here scan under zsh NOMATCH', () => {
  ...
  before(() => {
    tmpDir = createTempDir('forge-bug-3689-');
