// allow-test-rule: source-text-is-the-product — commands/forge/*.md files ARE the
 ...
 *
 * Authoritative behavioral contract for every commands/forge/*.md file.
 ...
 *
 * Contract:
 *   1. name:          present, non-empty, starts with forge: or forge-
...
const GSD_ROOT     = path.join(ROOT, 'forge-core');
...
describe('command contract: name field (ADR-0002)', () => {
  for (const { name, full } of commandFiles) {
    test(`${name}: name: present and starts with forge: or forge-`, () => {
      ...
      assert.ok(
        ...
        `${name}: name: must start with "forge:" or "forge-", got "${fm.name.trim()}"`,
      );
    });
  }
});
