/**
 * Tests for forge-statusline.js GSD state display helpers.
 *
 * Covers:
...
} = require('../hooks/forge-statusline.js');
...
describe('readGsdState', () => {
  const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-statusline-test-'));
...
describe('context meter respects CLAUDE_CODE_AUTO_COMPACT_WINDOW (#2219)', () => {
  ...
  const hookPath = path.join(__dirname, '..', 'hooks', 'forge-statusline.js');
  ...
  test('bridge used_pct is raw (CC-consistent) regardless of ACW setting (#2451)', () => {
    ...
    // This ensures forge-context-monitor warning messages match CC native /context.
...
describe('todo-resolution: resolves in_progress task from the newest matching todos file (#305)', () => {
  ...
  test('resolves in_progress task from the newest matching todos file (#305)', (t) => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-305-'));
    t.after(() => {
      cleanup(tempDir);
    });
