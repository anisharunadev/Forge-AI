...
const { resolveProfile, loadSkillsManifest, stageSkillsForProfile } = require('../forge-core/bin/lib/install-profiles.cjs');
const { resolveSurface, writeSurface, listSurface } = require('../forge-core/bin/lib/surface.cjs');
...
function tmpDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'forge-cap-cons-'));
}
...
describe('FIX-3 core install: stageSkillsForProfile honors tier:core capabilities', () => {
  test('real registry: core-profile staged set equals PROFILES.core (no-op today)', (_t) => {
    ...
    const { PROFILES } = require('../forge-core/bin/lib/install-profiles.cjs');
  ...
  test('synthetic tier:core registry: capability skill appears in stageSkillsForProfile output', (t) => {
    ...
    const synSrcDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-fix3-src-'));
    const stageDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-fix3-stage-'));
  ...
  test('FIX-3 documented behavior: minimal==core; tier:core capability IS in minimal', () => {
    ...
    const { MINIMAL_SKILL_ALLOWLIST, PROFILES } = require('../forge-core/bin/lib/install-profiles.cjs');
