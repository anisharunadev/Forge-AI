/**
 * Tests for forge-core/bin/lib/intel.cjs
 *
 * Covers: query, status, diff, validate, snapshot, patch-meta,
 * extract-exports, enabled/disabled gating, and CLI routing via forge-tools.
 */
...
const {
  ...
  INTEL_FILES,
} = require('../forge-core/bin/lib/intel.cjs');

const { isCapabilityActive } = require('../forge-core/bin/lib/capability-state.cjs');
...
// .forge-surface.json (no disabled clusters) so intel is surfaced deterministically.
...
function makeSurfacedConfigDir() {
  const dir = createTempDir('forge-intel-surface-cfg-');
  fs.writeFileSync(
...
describe('intel tri-state gate hermetic regression (isCapabilityActive cutover)', () => {
  ...
  beforeEach(() => {
    tmpConfigDir = createTempDir('forge-intel-tristate-cfg-');
    tmpProjectDir = createTempProject('forge-intel-tristate-proj-');
    ...
    // Empty CLAUDE_CONFIG_DIR (no .forge-surface.json) → defaults to 'full' profile.
...
// ─── CLI routing via forge-tools ──────────────────────────────────────────────

describe('forge-tools intel subcommands', () => {
  let tmpDir;
  let surfacedConfigDir;
  let savedEnv;
...
describe('#1000 regression: forge-intel-updater emits canonical intel filenames', () => {
  // allow-test-rule: source-text-is-the-product — agents/forge-intel-updater.md IS the
  ...
  const agentPromptPath = path.join(__dirname, '..', 'agents', 'forge-intel-updater.md');
  ...
  test('references every canonical INTEL_FILES name', () => {
    for (const filename of Object.values(INTEL_FILES)) {
      assert.ok(
        ...
        `forge-intel-updater.md must instruct writing the canonical intel file "${filename}" (from INTEL_FILES) that the forge-tools intel CLI reads; it was missing.`,
      );
    }
  });
  ...
  test('does not reference orphaned short filenames the CLI never reads', () => {
    ...
    for (const shortName of forbidden) {
      ...
      assert.strictEqual(
        ...
        0,
        `forge-intel-updater.md must not reference orphaned short name "${shortName}" the intel CLI never reads. Offending line(s):\n${offendingLines.join('\n')}`,
      );
    }
  });
});
