      ...
      valid: [
        ...
        },
        {
          code: `
            const fs = require('fs');
            const path = require('path');
            const content = fs.readFileSync(path.join(__dirname, '..', 'forge-core', 'workflows', 'config.json'), 'utf-8');
      ...
      invalid: [
        {
          code: `
            const fs = require('fs');
            const path = require('path');
            const src = fs.readFileSync(path.join(__dirname, '..', 'forge-core', 'bin', 'lib', 'io.cjs'), 'utf-8');
  ...
  test('valid: require() of a .cjs file is allowed (not readFileSync)', () => {
    ruleTester.run('no-source-grep', noSourceGrep, {
      valid: [
        {
          code: `
            const mod = require('../forge-core/bin/lib/io.cjs');
            mod.someMethod();
          `,
          filename: 'tests/foo.test.cjs',
        },
