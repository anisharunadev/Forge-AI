 ...
 * CJS phase CLI layer (phase.cjs + forge-tools.cjs): add, remove, complete,
...
const GSD_TOOLS_BIN = path.resolve(__dirname, '..', 'forge-core', 'bin', 'forge-tools.cjs');
...
const { comparePhaseNum, normalizePhaseName } = require('../forge-core/bin/lib/phase-id.cjs');
...
/**
 * Run `forge-tools phase complete <phase>` for the phase-complete regression
...
function runPhaseComplete(tmpDir, { phase = '1', tolerateExit = false } = {}) {
  try {
    ...
    if (err.killed || err.signal != null || err.code === 'ETIMEDOUT') {
      throw new Error(
        `forge-tools phase complete ${phase} was killed before completion ` +
...
describe('bug #1998: phase complete updates overview checkbox', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-1998-'));
...
describe('bug #2005: phase complete updates plan count when milestone is inside <details>', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-2005-'));
    planningDir = path.join(tmpDir, '.planning');
    fs.mkdirSync(planningDir, { recursive: true });
...
describe('bug #2526: phase complete warns about unregistered REQ-IDs', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-2526-'));
    planningDir = path.join(tmpDir, '.planning');
    fs.mkdirSync(planningDir, { recursive: true });
...
{
  const PMG_WF = path.join(__dirname, '..', 'forge-core', 'workflows', 'plan-milestone-gaps.md');
  const IMPORT_WF = path.join(__dirname, '..', 'forge-core', 'workflows', 'import.md');
  const BACKLOG_WF = path.join(__dirname, '..', 'forge-core', 'workflows', 'add-backlog.md');
  ...
  describe('bug-3298 — import.md must not construct bare {NN}-{slug} phase dirs', () => {
    ...
    test('plan_convert step must call init.phase-op to resolve the prefixed dir', () => {
      ...
      assert.ok(
        ...
        'import.md must call forge-sdk query init.phase-op to get expected_phase_dir with project_code prefix',
      );
    });
  });
...

{
  ...
  describe('bug #3517: phase.complete leaves STATE.md with stale fields', () => {
    ...
    beforeEach(() => {
      tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-3517-'));
    });
