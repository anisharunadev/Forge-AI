...
const REVIEW_FAMILY = [
  { file: 'code-review.md', agentTypes: ['forge-code-reviewer'] },
  { file: 'code-review-fix.md', agentTypes: ['forge-code-fixer', 'forge-code-reviewer'] },
  { file: 'eval-review.md', agentTypes: ['forge-eval-auditor'] },
];
...
describe('agent_skills injection — review-family workflows (#991)', () => {
  for (const { file, agentType } of cases) {
    test(`${file} queries + injects agent_skills for every ${agentType} spawn`, () => {
      ...
      //    the same `forge_run query agent-skills <type>` idiom as the ~20
      //    sibling workflows (plan-phase, execute-phase, secure-phase, ...).
      const assignRe = new RegExp(
        '([A-Z][A-Z0-9_]*)=\\$\\(\\s*forge_run query agent-skills ' + escapeRe(agentType) + '\\s*\\)',
      );
      const m = content.match(assignRe);
      assert.ok(
        m,
        `${file}: missing \`VAR=$(forge_run query agent-skills ${agentType})\` — configured agent_skills are never queried (#991)`,
      );
      const varName = m[1];
