...
/**
 * /forge-ultraplan-phase [BETA] Tests
 *
...
const CMD_PATH = path.join(__dirname, '..', 'commands', 'forge', 'ultraplan-phase.md');
const WF_PATH = path.join(__dirname, '..', 'forge-core', 'workflows', 'ultraplan-phase.md');

// ─── File Existence ────────────────────────────────────────────────────────────

describe('ultraplan-phase file existence', () => {
  test('command file exists', () => {
    assert.ok(fs.existsSync(CMD_PATH), 'commands/forge/ultraplan-phase.md should exist');
  });

  test('workflow file exists', () => {
    assert.ok(fs.existsSync(WF_PATH), 'forge-core/workflows/ultraplan-phase.md should exist');
  });
});
...
describe('ultraplan-phase command frontmatter', () => {
  ...
  test('has correct name field', () => {
    assert.match(content, /^name:\s*forge:ultraplan-phase$/m);
  });
...
describe('ultraplan-phase command references', () => {
  ...
  test('references the ultraplan-phase workflow', () => {
    assert.ok(
      content.includes('@~/.claude/forge-core/workflows/ultraplan-phase.md'),
...
describe('ultraplan-phase workflow runtime gate', () => {
  ...
  test('error message references /forge-plan-phase as local alternative', () => {
    assert.ok(
      content.includes('forge:plan-phase') || content.includes('forge-plan-phase'),
      'error message should direct users to /forge-plan-phase as the local alternative'
    );
  });
});
...
describe('ultraplan-phase workflow initialization', () => {
  ...
  test('loads GSD phase context via forge-sdk query init.plan-phase', () => {
    // After #3797 architectural fix, callsites use forge_run
    assert.ok(
      content.includes('forge_run query init.plan-phase'),
      'workflow must load phase context via forge_run query init.plan-phase',
    );
  });

  test('handles missing .planning directory', () => {
    assert.ok(
      content.includes('forge-new-project') || content.includes('/forge-new-project') || content.includes('forge:new-project'),
      'workflow should direct user to /forge-new-project when .planning is missing'
    );
  });
});
...
describe('ultraplan-phase workflow return path', () => {
  ...
  test('directs user to run /forge-import --from after ultraplan completes', () => {
    assert.ok(content.includes('forge-import') || content.includes('forge:import'), 'workflow must direct user to run /forge:import --from with the saved file path');
  });

  test('mentions the --from flag for forge-import', () => {
    assert.ok(content.includes('--from'), 'workflow should reference /forge-import --from <file-path>');
  });

  test('return-path instructions appear before the /ultraplan trigger', () => {
    const ultraplanTriggerIndex = content.indexOf('/ultraplan');
    const importIndex = content.indexOf('forge-import');
    assert.ok(
      importIndex < ultraplanTriggerIndex,
      'return-path instructions (forge-import) must appear before /ultraplan trigger so they are visible in scroll-back'
    );
  });
});
...
describe('ultraplan-phase workflow isolation', () => {
  ...
  test('does NOT directly write PLAN.md files', () => {
    assert.ok(
      ...
      'workflow must NOT directly write PLAN.md — delegates to /forge-import --from'
    );
  });
