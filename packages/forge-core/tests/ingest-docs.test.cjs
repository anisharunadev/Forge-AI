 ...
 *
 * Structural assertions for /forge-ingest-docs (#2387). Agents and workflows
...
const { extractFrontmatter } = require('../forge-core/bin/lib/frontmatter.cjs');

const ROOT = path.join(__dirname, '..');
const CMD_PATH = path.join(ROOT, 'commands', 'forge', 'ingest-docs.md');
const WF_PATH = path.join(ROOT, 'forge-core', 'workflows', 'ingest-docs.md');
const CLASSIFIER_PATH = path.join(ROOT, 'agents', 'forge-doc-classifier.md');
const SYNTHESIZER_PATH = path.join(ROOT, 'agents', 'forge-doc-synthesizer.md');
const CONFLICT_ENGINE_PATH = path.join(ROOT, 'forge-core', 'references', 'doc-conflict-engine.md');

// ─── File Existence ────────────────────────────────────────────────────────────

describe('ingest-docs file structure (#2387)', () => {
  test('command file exists', () => {
    assert.ok(fs.existsSync(CMD_PATH), 'commands/forge/ingest-docs.md should exist');
  });
  test('workflow file exists', () => {
    assert.ok(fs.existsSync(WF_PATH), 'forge-core/workflows/ingest-docs.md should exist');
  });
  test('classifier agent exists', () => {
    assert.ok(fs.existsSync(CLASSIFIER_PATH), 'agents/forge-doc-classifier.md should exist');
  });
  test('synthesizer agent exists', () => {
    assert.ok(fs.existsSync(SYNTHESIZER_PATH), 'agents/forge-doc-synthesizer.md should exist');
  });
...
describe('ingest-docs command frontmatter', () => {
  ...
  test('has name field', () => {
    assert.match(content, /^name:\s*forge:ingest-docs$/m);
  });
...
describe('ingest-docs command references', () => {
  ...
  test('references the ingest-docs workflow', () => {
    assert.ok(
      content.includes('@~/.claude/forge-core/workflows/ingest-docs.md'),
      'command must @-reference its workflow'
    );
  });
  test('references the doc-conflict-engine', () => {
    assert.ok(
      content.includes('@~/.claude/forge-core/references/doc-conflict-engine.md'),
      'command must load the shared conflict-engine contract'
    );
  });
  test('references gate-prompts', () => {
    assert.ok(
      content.includes('@~/.claude/forge-core/references/gate-prompts.md'),
...
describe('ingest-docs workflow content', () => {
  ...
  test('spawns forge-doc-classifier and forge-doc-synthesizer', () => {
    assert.ok(
      content.includes('forge-doc-classifier'),
      'workflow must spawn forge-doc-classifier'
    );
    assert.ok(
      content.includes('forge-doc-synthesizer'),
      'workflow must spawn forge-doc-synthesizer'
    );
  });
  ...
  test('routes to forge-roadmapper in new mode', () => {
    assert.ok(
      content.includes('forge-roadmapper'),
      'new mode must delegate to forge-roadmapper'
    );
  });
...
describe('forge-doc-classifier agent', () => {
...
describe('forge-doc-synthesizer agent', () => {
...
describe('import command adopts shared conflict-engine', () => {
  const cmdContent = fs.readFileSync(path.join(ROOT, 'commands', 'forge', 'import.md'), 'utf-8');
  const wfContent = fs.readFileSync(path.join(ROOT, 'forge-core', 'workflows', 'import.md'), 'utf-8');

  test('import command loads doc-conflict-engine reference', () => {
    assert.ok(
      cmdContent.includes('@~/.claude/forge-core/references/doc-conflict-engine.md'),
      '/forge-import must load the shared conflict-engine contract'
    );
  });
