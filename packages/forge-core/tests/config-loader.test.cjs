...
function makeTempProject(prefix = 'forge-cfg-loader-test-') {
...
describe('loadConfigResolved — provenance', () => {
  ...
  test('source is "global-defaults" when no .planning exists but ~/.forge/defaults.json readable', () => {
    const homeTmp = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-home-test-'));
    ...
    try {
      ...
      const noPlanning = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-noplanning-'));
      try {
  ...
  test('source is "builtin-defaults" when no .planning and no global defaults', () => {
    const noPlanning = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-noplanning2-'));
    const homeTmp = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-nohome-'));
    const origGsdHome = process.env['GSD_HOME'];
    try {
      // Point GSD_HOME to a directory with no .forge/defaults.json
      process.env['GSD_HOME'] = homeTmp;
      const result = loadConfigResolved(noPlanning);
      assert.equal(result.source, 'builtin-defaults');
      assert.equal(result.degraded, false);
