 ...
 * directories like skills/forge-custom-skill/SKILL.md were never walked and got
 * silently destroyed during /forge-update.
 */
...
describe('detect-custom-files — skills/ directory missing from GSD_MANAGED_DIRS (#2942)', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('forge-2942-skills-');
  });
  ...
  // Test 1: detects custom GSD-prefixed skill in skills/forge-<name>/SKILL.md
  test('detects custom skill file at skills/forge-<name>/SKILL.md', () => {
    writeManifest(tmpDir, {
      'skills/forge-planner/SKILL.md': '# GSD Planner Skill\n',
    });

    // User-added custom GSD-prefixed skill — NOT in manifest
    writeCustomFile(tmpDir, 'skills/forge-test-custom/SKILL.md', '# My Custom Skill\n');
    ...
    assert.ok(
      json.custom_files.includes('skills/forge-test-custom/SKILL.md'),
      `skills/forge-test-custom/SKILL.md should be in custom_files; got: ${JSON.stringify(json.custom_files)}`
    );
  });
  ...
  // Test 3: regression guard — still detects custom files in forge-core/workflows/
  test('regression: still detects custom files in forge-core/workflows/', () => {
    writeManifest(tmpDir, {
      'forge-core/workflows/plan-phase.md': '# Plan Phase\n',
      'skills/forge-planner/SKILL.md': '# GSD Planner Skill\n',
    });

    writeCustomFile(tmpDir, 'forge-core/workflows/custom-workflow.md', '# My Custom Workflow\n');
    ...
    const json = JSON.parse(result.output);
    assert.ok(
      json.custom_files.includes('forge-core/workflows/custom-workflow.md'),
  ...
  test('custom_count matches custom_files.length when multiple custom forge-prefixed skills exist', () => {
    writeManifest(tmpDir, {
      'skills/forge-planner/SKILL.md': '# GSD Planner Skill\n',
    });

    writeCustomFile(tmpDir, 'skills/forge-test-custom/SKILL.md', '# Custom Skill One\n');
    writeCustomFile(tmpDir, 'skills/forge-another-custom/SKILL.md', '# Custom Skill Two\n');
