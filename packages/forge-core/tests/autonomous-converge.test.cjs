...
const COMMAND_PATH = path.join(REPO_ROOT, 'commands', 'forge', 'autonomous.md');
const WORKFLOW_PATH = path.join(REPO_ROOT, 'forge-core', 'workflows', 'autonomous.md');
...
describe('autonomous --converge flag (#711)', () => {
  ...
  test('workflow routes planning through plan-review-convergence when enabled', () => {
    ...
    assert.match(
      ...
      'non-interactive converge mode should call forge-plan-review-convergence',
    );
    assert.match(
      workflow,
  ...
  test('docs show autonomous convergence usage', () => {
    ...
    assert.match(howTo, /\/forge-autonomous --only 4 --converge/, 'how-to should show single-phase converge usage');
  });
});
