...
`
  ...
  describe('--cwd override', () => {
    ...
    beforeEach(() => {
      outsideDir = fs.mkdtempSync(path.join(require('os').tmpdir(), 'forge-test-outside-'));
    });
...
const { stateExtractField, stateReplaceField, stateReplaceFieldWithFallback } = require('../forge-core/bin/lib/state.cjs');
...
// command that just failed ("...is SDK-only. Use: forge-tools query
// state.add-roadmap-evolution ..."), and no CJS handler existed after the SDK
// retirement (ADR-0174). Every `/forge:phase insert` and `/forge:phase --edit` run
...
`);
  ...
  test('shell metacharacters in --note are stored literally, never executed', () => {
    ...
    const probe = path.join(tmpDir, 'forge-pwn-1140');
...
describe('regressions: table-format STATE.md (#1162)', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempProject('forge-1162-');
  ...
  test('missing STATE.md returns updated:false gracefully', () => {
    ...
    const missingDir = createTempProject('forge-1162-missing-');
    try {
...
describe('regressions: table-format STATE.md (#1162) — updateCurrentPositionFields preserve-authored invariants', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempProject('forge-1162-f2-');
    statePath = path.join(tmpDir, '.planning', 'STATE.md');
  });
...
function make1255TempProject(stateContent) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-1255-'));
...
function make1257TempProject(stateContent) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-1257-'));
  const planningDir = path.join(dir, '.planning');
  fs.mkdirSync(planningDir, { recursive: true });
