 ...
 * commands/forge/ for the slash menu to read on older configurations.
 *
...
const SETTINGS_PATH = path.join(os.tmpdir(), `forge-test-settings-${process.pid}.json`);
...
describe('Bug #2957: claude+global post-install message', () => {
  test('claude+global message tells the user to restart and offers skill-name fallback', () => {
    ...
    assert.match(output, /\/forge-new-project/, 'should still mention /forge-new-project');
    assert.match(output, /forge-new-project skill/i, 'should mention the skill name fallback');
    assert.doesNotMatch(
      output,
  ...
  test('claude+local message keeps the original /forge-new-project instruction', () => {
