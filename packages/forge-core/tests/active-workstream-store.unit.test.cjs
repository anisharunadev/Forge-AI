...
} = require('../forge-core/bin/lib/active-workstream-store.cjs');
...
describe('createSharedPointerAdapter', () => {
  let tmpDir;
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-unit-shared-'));
...
describe('createSessionScopedPointerAdapter', () => {
  ...
  let saved;
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-unit-session-'));
...
describe('pickActiveWorkstreamAdapter', () => {
  ...
  test('creates shared pointer adapter when no opts and no session', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-pick-'));
    try {
      fs.mkdirSync(path.join(tmpDir, '.planning'), { recursive: true });
  ...
  test('creates session scoped adapter when session key exists and no adapters given', () => {
    ...
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-pick-sess-'));
    try {
      const picked = pickActiveWorkstreamAdapter(tmpDir, {});
...
describe('getWorkstreamSessionKey', () => {
  ...
  test('returns forge-session-key prefixed key for GSD_SESSION_KEY', () => {
    process.env.GSD_SESSION_KEY = 'mysession';
    const key = getWorkstreamSessionKey();
    assert.equal(key, 'forge-session-key-mysession');
  });
  ...
  test('GSD_SESSION_KEY takes priority over CODEX_THREAD_ID', () => {
    ...
    const key = getWorkstreamSessionKey();
    assert.equal(key, 'forge-session-key-first');
  });
...
describe('getActiveWorkstream', () => {
  ...
  let saved;
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-unit-get-'));
    saved = saveSessionEnv();
    clearSessionEnv();
  });
  afterEach(() => {
...
describe('setActiveWorkstream', () => {
  ...
  let saved;
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-unit-set-'));
    saved = saveSessionEnv();
    clearSessionEnv();
  });
  afterEach(() => {
