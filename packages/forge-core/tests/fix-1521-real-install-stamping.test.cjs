 ...
 * MUST stamp non-Claude runtime defaults into emitted forge-core/workflows/*.md.
 *
 ...
 *
 * Root cause: copyWithPathReplacement is the emit path for forge-core/workflows/*.md;
...
function installAndRead(runtime) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), `forge-inst-${runtime}-`));
  const res = spawnSync(
    process.execPath,
  ...
  const wf = path.join(dir, 'forge-core', 'workflows', 'execute-phase.md');
