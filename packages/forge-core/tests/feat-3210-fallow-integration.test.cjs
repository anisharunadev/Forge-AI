...
describe('feat-3210: fallow integration module', () => {
  test('normalizes structural findings from a fallow report', () => {
    const { normalizeFallowReport } = require('../forge-core/bin/lib/fallow-runner.cjs');
  ...
  test('falls back to node_modules/.bin/fallow when PATH does not contain fallow', () => {
    ...
    const tmp = fs.mkdtempSync(path.join(baseTmp, 'forge-fallow-bin-'));
  ...
  test('ignores non-executable PATH candidate on non-Windows; prefers .cmd over bare extensionless on Windows', () => {
    ...
    if (process.platform === 'win32') {
      ...
      const tmp = fs.mkdtempSync(path.join(baseTmp, 'forge-fallow-win-'));
      try {
      ...
      const tmp = fs.mkdtempSync(path.join(baseTmp, 'forge-fallow-nonexec-'));
      try {
        const pathDir = path.join(tmp, 'bin');
        fs.mkdirSync(pathDir, { recursive: true });
  ...
  test('throws actionable error when fallow is enabled but binary is unavailable', () => {
    const { requireFallowBinary } = require('../forge-core/bin/lib/fallow-runner.cjs');
    // N2: use shared helper
    const baseTmp = getWritableTmp();
    const tmp = fs.mkdtempSync(path.join(baseTmp, 'forge-fallow-missing-'));
    assert.throws(
...
describe('feat-3210: M2 - node_modules/.bin resolution order', () => {
  test('resolveFallowBinary prefers node_modules/.bin over PATH when both exist', () => {
    ...
    // N2: use shared helper
    const baseTmp = getWritableTmp();
    const tmp = fs.mkdtempSync(path.join(baseTmp, 'forge-fallow-order-'));
    try {
...
describe('feat-3210 / #1012: code-review workflow invokes fallow with the real CLI', () => {
  ...
  const workflowSrc = fs.readFileSync(
    path.join(ROOT, 'forge-core', 'workflows', 'code-review.md'),
    'utf8',
  );
...
describe('feat-3210: workflow and config contracts', () => {
  test('config schema allows code_quality.fallow.* keys in CJS and runtime manifest', () => {
    ...
    const manifestPath = path.join(ROOT, 'forge-core', 'bin', 'shared', 'config-schema.manifest.json');
  ...
  test('config-set accepts code_quality.fallow keys', () => {
    ...
    const tmpDir = createTempProject('forge-fallow-config-');
    try {
      const cases = [
  ...
  // Live agent output is covered by /forge-code-review e2e runs downstream.
  test('reviewer prompt defines ## Structural Findings (fallow) heading and review context echoes it', () => {
    const reviewer = fs.readFileSync(path.join(ROOT, 'agents', 'forge-code-reviewer.md'), 'utf8');
    const reviewContext = fs.readFileSync(path.join(ROOT, 'forge-core', 'contexts', 'review.md'), 'utf8');
    ...
    assert.ok(
      ...
      'forge-code-reviewer.md must define ## Structural Findings (fallow) section heading',
    );
