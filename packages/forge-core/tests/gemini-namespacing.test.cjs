 ...
 * 
 * Verifies that slash commands are correctly converted to colon format (/forge:)
...
describe('Gemini Slash Command Namespacing (Regex)', () => {
  test('converts simple slash commands', () => {
    const input = 'Run /forge-plan-phase to start.';
    const expected = 'Run /forge:plan-phase to start.';
    assert.strictEqual(convertSlashCommandsToGeminiMentions(input), expected);
  });

  test('preserves URLs with /forge- in them', () => {
    const input = 'Documentation: https://example.com/forge-tools/info';
    assert.strictEqual(convertSlashCommandsToGeminiMentions(input), input);
  });

  // The roster check is the safety property: a token like /forge-plan-phase IS
  ...
  test('preserves URLs even when path contains a KNOWN command name', () => {
    const input = 'See https://example.com/forge-plan-phase for context.';
    assert.strictEqual(convertSlashCommandsToGeminiMentions(input), input);
  });

  test('preserves sub-paths: bin/forge-tools.cjs', () => {
    const input = 'See bin/forge-tools.cjs for details.';
    assert.strictEqual(convertSlashCommandsToGeminiMentions(input), input);
  });

  test('preserves sub-paths even when leaf is a KNOWN command name', () => {
    // bin/forge-plan-phase looks like a known command but is a file path.
    // The leading / on a sub-path follows a non-slash char so the regex
    // boundary is the safety net here, not the roster.
    const input = 'Reference bin/forge-plan-phase for details.';
    assert.strictEqual(convertSlashCommandsToGeminiMentions(input), input);
  });

  test('preserves root-relative paths with extensions: /forge-tools.cjs', () => {
    const input = 'Load /forge-tools.cjs now.';
    assert.strictEqual(convertSlashCommandsToGeminiMentions(input), input);
  });

  test('preserves agent names: forge-planner', () => {
    const input = 'The forge-planner agent will help you.';
    assert.strictEqual(convertSlashCommandsToGeminiMentions(input), input);
  });

  test('converts commands in backticks', () => {
    const input = 'Run `/forge-new-project` in a terminal.';
    const expected = 'Run `/forge:new-project` in a terminal.';
    assert.strictEqual(convertSlashCommandsToGeminiMentions(input), expected);
  });
  ...
  test('converts commands ending with punctuation', () => {
    ...
    // any other entry currently in commands/forge/.
    const input = 'Run /forge-help. Or /forge-health!';
    const expected = 'Run /forge:help. Or /forge:health!';
    assert.strictEqual(convertSlashCommandsToGeminiMentions(input), expected);
  });
  ...
  test('roster has loaded — non-empty (would otherwise silently no-op all conversions)', () => {
    ...
    // empty Set (because commands/forge/ was not found), every conversion
    // becomes a no-op — exactly the bug this code exists to prevent.
    const result = convertSlashCommandsToGeminiMentions('Run /forge-plan-phase.');
    assert.strictEqual(result, 'Run /forge:plan-phase.',
      'Roster failed to load — all /forge- conversions would silently no-op');
  });
});
...
describe('Gemini Markdown Processor', () => {
  test('handles command to TOML conversion', () => {
    ...
    assert.match(parsed.prompt, /\/forge:help/, 'prompt must contain namespaced command');
    assert.doesNotMatch(parsed.prompt, /\/forge-help/, 'prompt must not retain hyphen form');
  });
  ...
  test('strips <sub> tags from Gemini markdown output (#2768 regression)', () => {
    ...
    const input = 'Run <sub>tiny</sub> /forge-help now.';
    ...
    assert.match(result, /\/forge:help/, 'slash command must still be converted');
  });
...
describe('Gemini Install (Behavioral)', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('forge-gemini-test-');
    tmpHome = createTempDir('forge-gemini-home-');
    previousCwd = process.cwd();
    previousHome = process.env.HOME;
    previousUserprofile = process.env.USERPROFILE;
    process.chdir(tmpDir);
    // #3037: isolate HOME so the developer's real ~/.gemini/commands/forge/
    // doesn't trigger the local-install conflict-avoidance skip path. This
    // test wants to assert that the local install populates commands/forge/
  ...
  test('install creates correct directory structure for Gemini', () => {
    ...
    const commandsDir = path.join(tmpDir, '.gemini', 'commands', 'forge');
    ...
    assert.match(parsed.prompt, /\/forge:[a-z][a-z0-9-]*/,
      'installed plan-phase.toml prompt must contain at least one /forge: reference');
    assert.doesNotMatch(parsed.prompt, /(?<![A-Za-z0-9./])\/forge-plan-phase\b/,
      'installed plan-phase.toml prompt must not retain unconverted /forge-plan-phase');
  });
});
