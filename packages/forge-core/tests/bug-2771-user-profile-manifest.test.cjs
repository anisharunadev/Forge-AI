 ...
 *
 * USER-PROFILE.md is a user-owned artifact created/refreshed by /forge-profile-user.
 * preserveUserArtifacts() correctly preserves it across reinstalls. But writeManifest()
 * also records it under "forge-core/USER-PROFILE.md" with a SHA-256 of whatever was
 ...
 * "Found N locally modified GSD file(s) — backed up to forge-local-patches/" warning.
 *
...
describe('#2771: USER-PROFILE.md is excluded from forge-file-manifest.json', () => {
  let tmpDir;

  beforeEach(() => { tmpDir = createTempDir('forge-2771-manifest-'); });
  afterEach(() => { cleanup(tmpDir); });

  test('writeManifest excludes forge-core/USER-PROFILE.md even when present on disk', () => {
    runInstaller(tmpDir);

    // Simulate /forge-profile-user creating USER-PROFILE.md
    ...
    assert.ok(
      !Object.prototype.hasOwnProperty.call(manifest.files, 'forge-core/USER-PROFILE.md'),
      'manifest.files must NOT contain forge-core/USER-PROFILE.md — it is a user artifact, not distribution'
    );
  });
});
...
describe('#2771: USER-PROFILE.md is still preserved across reinstall', () => {
  ...
  beforeEach(() => { tmpDir = createTempDir('forge-2771-preserve-'); });
  afterEach(() => { cleanup(tmpDir); });
  ...
  test('USER-PROFILE.md content survives reinstall (preservation regression guard)', () => {
    ...
    const profilePath = path.join(tmpDir, 'forge-core', 'USER-PROFILE.md');
    const content = '# Profile\n\nUser content from /forge-profile-user.\n';
...
describe('#2771: refreshed USER-PROFILE.md does not trigger local-patches warning', () => {
  ...
  beforeEach(() => { tmpDir = createTempDir('forge-2771-patches-'); });
  afterEach(() => { cleanup(tmpDir); });
  ...
  test('saveLocalPatches does not classify a refreshed USER-PROFILE.md as a local patch', () => {
    ...
    // /forge-profile-user creates USER-PROFILE.md (v1)
    const profilePath = path.join(tmpDir, 'forge-core', 'USER-PROFILE.md');
    ...
    // /forge-profile-user --refresh rewrites USER-PROFILE.md (v2 != v1)
    ...
    const patchFile = path.join(patchesDir, 'forge-core', 'USER-PROFILE.md');
    assert.ok(
      !fs.existsSync(patchFile),
      'USER-PROFILE.md must NOT appear in forge-local-patches/ — it is a user artifact, not a modified distribution file'
    );
...
describe('#2771: legacy manifest entries for USER_OWNED_ARTIFACTS are normalized', () => {
  ...
  beforeEach(() => { tmpDir = createTempDir('forge-2771-legacy-'); });
  afterEach(() => { cleanup(tmpDir); });
  ...
  test('pre-existing manifest entry for USER-PROFILE.md does not trigger patches warning', () => {
    ...
    manifest.files['forge-core/USER-PROFILE.md'] = 'deadbeef'.repeat(8); // stale hash
    fs.writeFileSync(manifestPath, JSON.stringify(manifest, null, 2));

    // /forge-profile-user --refresh rewrites USER-PROFILE.md
    fs.writeFileSync(profilePath, '# Profile v2 — refreshed\n');
...
describe('manifest path safety', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('forge-manifest-path-safety-');
