 ...
 *      They must call `forge_run query git.base-branch` instead.
...
function createGitRepo(opts = {}) {
  const { prefix = 'forge-1146-', defaultBranch = 'master' } = opts;
...
/**
 * Create a .planning dir so forge-tools resolveProjectRoot doesn't bail.
 */
...
/**
 * Write a forge config.json with git.base_branch set.
 */
...
const WORKFLOW_DIR = path.join(__dirname, '..', 'forge-core', 'workflows');
...
describe('#1146: git.base-branch resolver', () => {
  ...
  test('A. config override git.base_branch → returned immediately', (t) => {
    const dir = createGitRepo({ prefix: 'forge-1146-a-', defaultBranch: 'master' });
    t.after(() => cleanup(dir));
    addPlanning(dir);
  ...
  test('B. origin/HEAD symref resolves → returned', (t) => {
    ...
    const originDir = createGitRepo({ prefix: 'forge-1146-b-origin-', defaultBranch: 'main' });
    const worktreeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-1146-b-wt-'));
  ...
  test('C. KEY REGRESSION — master repo, origin/HEAD unset → returns "master" not "main"', (t) => {
    ...
    const originDir = createGitRepo({ prefix: 'forge-1146-c-origin-', defaultBranch: 'master' });
    const cloneDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-1146-c-clone-'));
  ...
  test('D. No remote, local branch "master" present, "main" absent → returns "master"', (t) => {
    const dir = createGitRepo({ prefix: 'forge-1146-d-', defaultBranch: 'master' });
    t.after(() => cleanup(dir));
    addPlanning(dir);
  ...
  test('E. No remote, local branch "main" present → returns "main"', (t) => {
    const dir = createGitRepo({ prefix: 'forge-1146-e-', defaultBranch: 'main' });
    t.after(() => cleanup(dir));
    addPlanning(dir);
  ...
  test('F. No remote, no main/master local branch → returns "main" (last resort default)', (t) => {
    const dir = createGitRepo({ prefix: 'forge-1146-f-', defaultBranch: 'develop' });
    t.after(() => cleanup(dir));
    addPlanning(dir);
  ...
  test('A2. config override with flat base_branch key (legacy form) → returned immediately', (t) => {
    const dir = createGitRepo({ prefix: 'forge-1146-a2-', defaultBranch: 'master' });
    t.after(() => cleanup(dir));
    addPlanning(dir);
  ...
  test('H. No remote, both "main" and "master" local branches exist → returns "main" (main wins tie-break)', (t) => {
    ...
    const dir = createGitRepo({ prefix: 'forge-1146-h-', defaultBranch: 'master' });
    t.after(() => cleanup(dir));
    addPlanning(dir);
  ...
  test('G. Anti-regression: all five affected workflows use forge_run query git.base-branch, not bare :-main / :-master', () => {
    // The root-cause pattern: DEFAULT_BRANCH=${DEFAULT_BRANCH:-main} or BASE_BRANCH="${BASE_BRANCH:-main}"
    // After fix: workflows call forge_run query git.base-branch and remove the bare fallback.
    const BAD_PATTERN = /\$\{(?:DEFAULT_BRANCH|BASE_BRANCH):-(?:main|master)\}/;
    const RESOLVER_CALL = /forge_run query git\.base-branch/;
    ...
    for (const wfPath of AFFECTED_WORKFLOWS) {
      ...
      assert.ok(
        ...
        'Must be replaced with forge_run query git.base-branch (Issue #1146).',
      );

      assert.ok(
        RESOLVER_CALL.test(content),
        `${name} does not call \`forge_run query git.base-branch\`. ` +
...
const gitBaseBranch = require(path.join(__dirname, '..', 'forge-core', 'bin', 'lib', 'git-base-branch.cjs'));
...
describe('#1268 gitWorktreeInfoInternal: relocation to git-base-branch', () => {
  test('gitWorktreeInfoInternal(createTempGitProject()) returns {inside:true, worktreeRoot:<non-empty string>}', (t) => {
    const dir = createTempGitProject('forge-wt-info-');
    t.after(() => cleanup(dir));
  ...
  test('gitWorktreeInfoInternal(createTempDir()) returns {inside:false, worktreeRoot:null} for a non-git dir', (t) => {
    const dir = createTempDir('forge-wt-info-nongit-');
    t.after(() => cleanup(dir));
    const result = gitBaseBranch.gitWorktreeInfoInternal(dir);
  ...
  test('gitWorktreeInfoInternal never throws (non-git dir)', (t) => {
    const dir = createTempDir('forge-wt-info-nothrow-');
    t.after(() => cleanup(dir));
...
describe('#1406: setGsdConfig prototype-pollution guard', () => {
  test('rejects __proto__ as a key segment', (t) => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-1406-'));
    t.after(() => cleanup(dir));
