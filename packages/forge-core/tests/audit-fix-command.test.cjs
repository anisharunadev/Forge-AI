...
describe('AUDIT-FIX: command file', () => {
  ...
  test('command file exists', () => {
    assert.ok(
      ...
      'audit-fix.md must exist in commands/forge/'
    );
  });

  test('has valid frontmatter with name forge:audit-fix', () => {
    const content = fs.readFileSync(cmdPath, 'utf-8');
    const frontmatter = content.split('---')[1] || '';
    assert.ok(
      frontmatter.includes('name: forge:audit-fix'),
      'name must be forge:audit-fix'
    );
  });
  ...
  test('has argument-hint in frontmatter for /forge-help discoverability', () => {
    const content = fs.readFileSync(cmdPath, 'utf-8');
    const frontmatter = content.split('---')[1] || '';
    assert.ok(
      frontmatter.includes('argument-hint:'),
      'must have argument-hint in frontmatter for /forge-help discoverability'
    );
  });
...
describe('AUDIT-FIX: workflow file', () => {
  ...
  test('workflow file exists', () => {
    assert.ok(
      ...
      'audit-fix.md must exist in forge-core/workflows/'
    );
  });
  ...
  test('has <available_agent_types> listing forge-executor', () => {
    const content = fs.readFileSync(wfPath, 'utf-8');
    assert.ok(
    ...
    assert.ok(
      content.includes('forge-executor'),
      'must list forge-executor as available agent type'
    );
  });
});
