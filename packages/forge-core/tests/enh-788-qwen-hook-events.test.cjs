 ...
 *
 * Qwen Code supports 15 hook events; forge previously registered only
 ...
 *
 * All three are wired to forge-context-monitor.js — the same hook used for
...
const STUB_HOOKS = [
  ...
  'forge-config-reload.js', // Added in #770
];
...
describe('enh-788: Qwen install registers 3 new hook events', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('forge-788-qwen-');
  ...
  test('UserPromptSubmit is NOT registered (handler not yet implemented for that payload shape)', () => {
    // forge-prompt-guard exits unless tool_name is Write|Edit — it is a no-op
  ...
  test('FileChanged is NOT registered for Qwen (Claude-only event)', () => {
    // forge-config-reload / FileChanged is a Claude Code-only registration.
...
describe('enh-788 (updated by #770): Claude install registers context lifecycle events', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('forge-788-claude-');
    previousCwd = process.cwd();
    process.chdir(tmpDir);
...
describe('enh-788: Qwen install is idempotent across persisted reinstalls', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('forge-788-idem-');
    previousCwd = process.cwd();
    process.chdir(tmpDir);
...
describe('enh-788: Qwen uninstall removes new hook event entries', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('forge-788-uninstall-');
    previousCwd = process.cwd();
    process.chdir(tmpDir);
