 ...
 * (e.g., forge-intel-*.js) should NOT be flagged as stale by forge-check-update.js.
 *
 * The stale hooks scanner should only check hooks that are part of the current
 * distribution, not every forge-*.js file in the hooks directory.
 *
...
const CHECK_UPDATE_PATH = path.join(__dirname, '..', 'hooks', 'forge-check-update.js');
const WORKER_PATH = path.join(__dirname, '..', 'hooks', 'forge-check-update-worker.js');
...
describe('orphaned hooks stale detection (#1750)', () => {
  test('MANAGED_HOOKS is an array and does not use a broad forge-* wildcard', () => {
    // The scanner must reference a known set of managed hook filenames,
    // not a broad startsWith('forge-') filter that catches orphaned files.
    ...
    for (const entry of MANAGED_HOOKS) {
      ...
      assert.ok(entry.startsWith('forge-'), `MANAGED_HOOKS entry '${entry}' must start with forge-`);
    }
  });

  test('forge-check-update-worker.js imports managed-hooks-registry.cjs (not inline array)', () => {
    const content = fs.readFileSync(WORKER_PATH, 'utf8');
    assert.ok(
      content.includes('managed-hooks-registry.cjs'),
      'forge-check-update-worker.js must require managed-hooks-registry.cjs'
    );
    // The inline MANAGED_HOOKS array must no longer be in the worker
    assert.ok(
      !content.includes('const MANAGED_HOOKS = ['),
      'forge-check-update-worker.js must not define MANAGED_HOOKS inline — it should import from managed-hooks-registry.cjs'
    );
  });

  test('forge-check-update.js spawns the worker by file path (not inline -e code)', () => {
    const content = fs.readFileSync(CHECK_UPDATE_PATH, 'utf8');
    assert.ok(
      content.includes('forge-check-update-worker.js'),
      'forge-check-update.js must reference forge-check-update-worker.js as the spawn target'
    );
    assert.ok(
      !content.includes("'-e'"),
      'forge-check-update.js must not use node -e inline code (logic moved to worker file)'
    );
  });
  ...
  test('MANAGED_HOOKS is a superset of all forge-* hooks in HOOKS_TO_COPY (all extensions)', () => {
    // Every hook-named file in HOOKS_TO_COPY (matching the forge-* naming pattern
    ...
    assert.ok(Array.isArray(HOOKS_TO_COPY), 'HOOKS_TO_COPY must be an array');
    const gsdHooks = HOOKS_TO_COPY.filter(h => h.startsWith('forge-'));
    assert.ok(gsdHooks.length >= 5, `expected at least 5 forge-* hooks in HOOKS_TO_COPY, got ${gsdHooks.length}`);
  ...
  test('orphaned hook filenames are NOT in MANAGED_HOOKS', () => {
    const orphanedHooks = [
      'forge-intel-index.js',
      'forge-intel-prune.js',
      'forge-intel-session.js',
    ];
  ...
  test('every same-dir require() target of a shipped hook is itself shipped (#606)', () => {
    // Regression guard for #606: forge-check-update-worker.js does
    ...
    // dir into sibling package dirs (e.g. forge-core/) whose shipping is governed
