 ...
 * forge-prompt-guard handler exits unless tool_name is Write|Edit, so it would be
...
describe('enh-772: ensureCodexHooksJsonEvent registers SubagentStart, Stop, PostToolUse', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('forge-772-events-');
    stubHookFile(tmpDir, 'forge-context-monitor.js');
  });
  ...
  for (const eventName of ['SubagentStart', 'Stop', 'PostToolUse']) {
    ...
    test(`${eventName}: hook entry uses forge-context-monitor`, () => {
      const fakeRunner = '"/usr/local/bin/node"';
      ensureCodexHooksJsonEvent(tmpDir, eventName, {
        absoluteRunner: fakeRunner,
        platform: 'linux',
      ...
      const handlers = hooksJsonHandlersForEvent(hooksJson, eventName);
      assert.ok(
        handlers.some(h => h.command && h.command.includes('forge-context-monitor')),
        `${eventName} hook must use forge-context-monitor; got: ${JSON.stringify(handlers)}`
      );
    });
    ...
    test(`${eventName}: hook entry has type: 'command'`, () => {
      ...
      const hooksJson = readHooksJson(tmpDir);
      const handlers = hooksJsonHandlersForEvent(hooksJson, eventName);
      const entry = handlers.find(h => h.command && h.command.includes('forge-context-monitor'));
...
describe('enh-772: commandWindows parity — ensureCodexHooksJsonSessionStart emits commandWindows', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('forge-772-cmdwin-');
    stubHookFile(tmpDir, 'forge-check-update.js');
  });
  ...
  test('POSIX platform: command references forge-check-update.js (not .cmd)', () => {
    const fakeRunner = '"/usr/local/bin/node"';
    ...
    const entry = handlers[0];
    assert.ok(
      entry.command && entry.command.includes('forge-check-update'),
      `POSIX command must reference forge-check-update; got: ${entry.command}`
    );
    assert.ok(
  ...
  test('Windows platform: SessionStart hook is written with commandWindows pointing to .cmd shim', () => {
    ...
    if (result.wrote) {
      ...
      assert.ok(
        entry.commandWindows.includes('forge-check-update') && entry.commandWindows.includes('.cmd'),
        `commandWindows must reference forge-check-update.cmd on win32; got: ${entry.commandWindows}`
      );
    }
  });
});
...
describe('enh-772: ensureCodexHooksJsonEvent is idempotent', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('forge-772-idem-');
    stubHookFile(tmpDir, 'forge-context-monitor.js');
  });
...
describe('enh-772: removeCodexHooksJsonEvent removes managed entries', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('forge-772-remove-');
    stubHookFile(tmpDir, 'forge-context-monitor.js');
  });
...
describe('enh-772: reconcileCodexHooksJsonEvent preserves user-owned entries', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('forge-772-preserve-');
  });
  ...
  test('user-owned SubagentStart entry is preserved when GSD entry is registered', () => {
    ...
    reconcileCodexHooksJsonEvent(tmpDir, 'SubagentStart', {
      managedCommand: '"/usr/local/bin/node" "/home/me/.codex/hooks/forge-context-monitor.js"',
    });
