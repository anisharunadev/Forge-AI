...
/**
 * Tests for `/forge-settings-advanced` — power-user configuration command (#2528).
 *
 * Covers:
 ...
 *   - /forge-settings confirmation output advertises /forge-settings-advanced
...
const COMMAND_PATH = path.join(ROOT, 'commands', 'forge', 'config.md');
const WORKFLOW_PATH = path.join(ROOT, 'forge-core', 'workflows', 'settings-advanced.md');
const SETTINGS_WORKFLOW_PATH = path.join(ROOT, 'forge-core', 'workflows', 'settings.md');
...
const SPEC_FIELDS = {
  ...
  git: [
    ...
    { key: 'git.phase_branch_template',      default: 'forge/phase-{phase}-{slug}' },
    { key: 'git.milestone_branch_template',  default: 'forge/{milestone}-{slug}' },
  ],
  runtime: [
...
describe('forge-settings-advanced — file scaffolding', () => {
  ...
  test('workflow file exists at forge-core/workflows/settings-advanced.md', () => {
  ...
  test('command frontmatter has name, description, allowed-tools', () => {
    ...
    const fm = fmMatch[1];
    assert.match(fm, /name:\s*forge:config/, 'frontmatter missing name (forge:config)');
...
describe('forge-settings-advanced — workflow structure', () => {
  let workflow;
  try {
...
describe('forge-settings-advanced — VALID_CONFIG_KEYS coverage', () => {
  for (const key of ALL_SPEC_KEYS) {
    test(`VALID_CONFIG_KEYS contains "${key}"`, () => {
      assert.ok(
        VALID_CONFIG_KEYS.has(key),
        `VALID_CONFIG_KEYS missing ${key} — add it to forge-core/bin/lib/config-schema.cjs`
      );
    });
  }
});

// ─── /forge-settings mentions /forge-settings-advanced ────────────────────────────

describe('/forge-settings advertises /forge-settings-advanced', () => {
  test('settings workflow mentions canonical /forge-config --advanced', () => {
    const text = fs.readFileSync(SETTINGS_WORKFLOW_PATH, 'utf-8');
    assert.ok(
      text.includes('/forge:config --advanced'),
      'forge-core/workflows/settings.md must mention /forge:config --advanced'
    );
    assert.ok(
      !text.includes('forge-settings-advanced') && !text.includes('forge:settings-advanced'),
      'forge-core/workflows/settings.md must not mention legacy /forge-settings-advanced variants'
    );
  });
});
...
describe('forge-settings-advanced — config merge preserves unrelated keys', () => {
...
describe('forge-settings-advanced — negative scenarios', () => {
  ...
  // The /forge-settings-advanced workflow promises non-numeric input is never
  ...
  test('config-set on a numeric key stores non-numeric input verbatim as string (workflow layer must reject before reaching here)', (t) => {
    ...
    // /forge-settings-advanced workflow. If a future change adds a numeric
