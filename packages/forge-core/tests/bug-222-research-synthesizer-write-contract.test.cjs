...
const SYNTHESIZER_PATH = path.join(REPO_ROOT, 'agents', 'forge-research-synthesizer.md');
...
describe('bug #222 recurrence: orchestrator self-heals when synthesizer returns SUMMARY.md inline', () => {
  const WORKFLOWS = [
    path.join(REPO_ROOT, 'forge-core', 'workflows', 'new-project.md'),
    path.join(REPO_ROOT, 'forge-core', 'workflows', 'new-milestone.md'),
  ];
  ...
  for (const wf of WORKFLOWS) {
    ...
    test(`${name} has the #222 synthesizer SUMMARY.md self-heal guard`, () => {
      ...
      assert.match(text, /forge:write-continue|verify-summary/i,
      ...
      assert.match(text, /forge-roadmapper[\s\S]{0,200}?(missing|incomplete|do NOT)/i,
        `${name} must block spawning forge-roadmapper when SUMMARY.md is missing or incomplete.`);
    ...
    test(`${name} runs the #222 self-heal AFTER the synthesizer and BEFORE forge-roadmapper`, () => {
      const text = fs.readFileSync(wf, 'utf8');
      ...
      const roadIdx = text.indexOf('subagent_type="forge-roadmapper"');
      assert.ok(synthIdx >= 0, `${name}: synthesizer dispatch not found`);
      assert.ok(healIdx >= 0, `${name}: #222 self-heal block not found`);
      assert.ok(roadIdx >= 0, `${name}: forge-roadmapper dispatch not found`);
      assert.ok(healIdx > synthIdx, `${name}: self-heal must come AFTER the synthesizer dispatch`);
      assert.ok(roadIdx > healIdx, `${name}: self-heal must come BEFORE the forge-roadmapper dispatch`);
    });
  }
});
