...
const { resolveUpdateContext } = require(
  path.join(ROOT, 'forge-core', 'bin', 'lib', 'update-context.cjs'),
);
...
function ver(dir) { return `${dir}/forge-core/VERSION`; }
function marker(dir) { return `${dir}/forge-core/workflows/update.md`; }
...
describe('forge-tools update-context (CLI): emits the JSON contract', () => {
  test('--config-dir fixture resolves to the documented 4-field JSON', () => {
    const tmp = nodeFs.mkdtempSync(path.join(os.tmpdir(), 'forge-uc-'));
    try {
      nodeFs.mkdirSync(path.join(tmp, 'forge-core', 'workflows'), { recursive: true });
      nodeFs.writeFileSync(path.join(tmp, 'forge-core', 'VERSION'), '1.42.0\n');
      nodeFs.writeFileSync(path.join(tmp, 'forge-core', 'workflows', 'update.md'), 'x');
...
describe('resolveUpdateContext: parity with the old inline bash (adversarial-review)', () => {
  ...
  test('fast path also requires the marker: VERSION-only preferredConfigDir -> 0.0.0', () => {
    ...
    const custom = '/opt/forge-partial';
    const fs = fakeFs({ [ver(custom)]: '1.41.0\n' }); // marker absent
    const r = resolveUpdateContext({
