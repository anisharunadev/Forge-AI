'use strict';
/**
 * Regression guard — bug(#853): /forge-manager and /forge-autonomous --interactive
...
describe('bug-853 — manager/autonomous gate background dispatch by runtime', () => {
  ...
  test('manager.md gates background dispatch on codex and runs plan/execute inline otherwise', () => {
    ...
    assert.match(
      MANAGER,
      /Otherwise \(Claude Code or any other non-Codex runtime\)[\s\S]{0,400}?Skill\(skill="forge-plan-phase"/,
    );
    // Inline is the default/else branch for execute — anchored on the explicit non-Codex label
    assert.match(
      MANAGER,
      /Otherwise \(Claude Code or any other non-Codex runtime\)[\s\S]{0,400}?Skill\(skill="forge-execute-phase"/,
    );
  });
  ...
  test('autonomous.md gates interactive background dispatch on codex; runs plan/execute inline otherwise', () => {
    // Codex block: run_in_background=true appears within the codex branch and forge-plan-phase is nearby
    assert.match(AUTONOMOUS, /If `RUNTIME` is `codex`[\s\S]{0,1200}?run_in_background=true[\s\S]{0,600}?forge-plan-phase/);
    // Codex block: run_in_background=true appears within the codex branch and forge-execute-phase is nearby
    assert.match(AUTONOMOUS, /If `RUNTIME` is `codex`[\s\S]{0,3000}?run_in_background=true[\s\S]{0,200}?forge-execute-phase/);
