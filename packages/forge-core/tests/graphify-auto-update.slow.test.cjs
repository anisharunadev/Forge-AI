...
} = require('../forge-core/bin/lib/configuration.cjs');
...
describe('auto-update', () => {
  ...
  const HOOK = path.join(__dirname, '..', 'hooks', 'forge-graphify-update.sh');

  function createTempGitRepo(opts = {}) {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-3347-'));
  ...
  describe('hook — HEAD-advancing command matchers',
    ...
    for (const cmd of [
      ...
      // #3653 — `forge-tools query commit` invokes git via spawnSync('git', [...]),
      ...
      'forge-tools query commit "docs: probe" --files .planning/STATE.md',
      'npx forge-tools query commit "docs: probe" --files .planning/STATE.md',
    ]) {
    ...
    test('does NOT dispatch on SDK commit-to-subrepo prefix collision', (t) => {
      ...
      const r = runHook(
        tmpDir,
        {
          tool_name: 'Bash',
          tool_input: {
            command: 'forge-tools query commit-to-subrepo "msg" --files packages/foo',
          },
        },
        { pathPrepend: mockBin },
      );
    ...
    // `forge-tools query` verbs (phase.complete, roadmap.update-plan-progress,
    // state.begin-phase) mutate .md files but do NOT advance HEAD; matching
    // them would cause a spurious rebuild per state mutation.
    for (const cmd of [
      'forge-tools query phase.complete 109',
      'forge-tools query roadmap.update-plan-progress 109 W001',
      'forge-tools query state.begin-phase 110',
    ]) {
