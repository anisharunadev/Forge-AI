/**
 * Bug #3037: Gemini global+local install creates duplicate /forge:* commands
 ...
 *   → both ~/.gemini/commands/forge/ and PROJECT/.gemini/commands/forge/ contain
 *     65 overlapping command filenames.
 *   → Gemini conflict detection renames every overlapping command to
 *     /workspace.forge:* and /user.forge:*, breaking the documented /forge:*
 *     namespace.
 *
...
describe('bug #3037: Gemini global+local install must not create duplicate command scopes', () => {
  ...
  beforeEach(() => {
    tmpHome = createTempDir('forge-3037-home-');
    tmpProject = createTempDir('forge-3037-work-');
  ...
  test('global install populates HOME/.gemini/commands/gsd', () => {
    ...
    const globalCmds = path.join(tmpHome, '.gemini', 'commands', 'forge');
  ...
  test('local install after global does NOT populate PROJECT/.gemini/commands/forge (avoids /forge:* namespace conflict)', () => {
    ...
    // Assertion: the local commands/forge/ directory must NOT exist (or must
    ...
    const localCmds = path.join(tmpProject, '.gemini', 'commands', 'forge');
    const localFiles = listCommandFiles(localCmds);
    assert.equal(
      localFiles.length,
      0,
      `local install must skip commands/forge/ when global already exists; ` +
  ...
  test('local install when HOME has hand-dropped overrides UNDER commands/forge/ (but no full GSD) still populates locally', () => {
    ...
    // command at ~/.gemini/commands/forge/<thing>.toml without ever
    ...
    // produce by accident.
    const homeGsdDir = path.join(tmpHome, '.gemini', 'commands', 'forge');
    ...
    assert.ok(
      localFiles.length > 0,
      `local install must proceed when HOME/.gemini/commands/forge contains ` +
        `only user overrides (not the full GSD canary set); ` +
        `found ${localFiles.length} files at ${localCmds}`
    );
  });

  test('local install when HOME/.gemini exists but commands/forge is absent (non-GSD Gemini user) still populates locally', () => {
