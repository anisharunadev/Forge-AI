...
const {
  PROFILING_QUESTIONS,
  CLAUDE_INSTRUCTIONS,
} = require('../forge-core/bin/lib/profile-output.cjs');
...
describe('write-profile command', () => {
  ...
  test('#1114: default output resolves the active runtime config home (codex)', () => {
    ...
    assert.strictEqual(out.profile_path, path.join(codexHome, 'forge-core', 'USER-PROFILE.md'));
  ...
  test('#1114: default output is the .claude config home for the claude runtime', () => {
    ...
    assert.ok(
      out.profile_path.endsWith(path.join('.claude', 'forge-core', 'USER-PROFILE.md')),
      `claude profile must be under .claude/forge-core; got ${out.profile_path}`
    );
...
describe('generate-claude-md command', () => {
  ...
  test('skills fallback mentions the normalized project roots', () => {
    ...
    assert.ok(!content.includes('forge-core/skills'));
  });
...
describe('generate-dev-preferences command', () => {
  ...
  test('uses runtime-aware skills dir for codex by default', () => {
    ...
    assert.ok(result.success, `Failed: ${result.error}`);
    const out = JSON.parse(result.output);
    assert.strictEqual(out.command_path, path.join(codexHome, 'skills', 'forge-dev-preferences', 'SKILL.md'));
  ...
  test('uses runtime-aware skills dir for cline by default (#782)', () => {
    ...
    const out = JSON.parse(result.output);
    assert.strictEqual(out.command_path, path.join(clineHome, 'skills', 'forge-dev-preferences', 'SKILL.md'));
  });
});
