 ...
 *
 * The legacy `forge-update-check` hook migration in bin/install.js uses two
 ...
 *
 * Test approach: write a `.codex/config.toml` with a stale forge-update-check
...
describe('#2698: CRLF stale forge-update-check block is removed on Codex reinstall', () => {
  let tmpDir;

  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-crlf-install-2698-'));
  });
...
function writeCodexConfigWithStaleHooks(dir, headerEol, bodyEol) {
    ...
    const staleBlock = [
      ...
      '[[hooks]]',
      'event = "SessionStart"',
      'command = "node /old/path/forge-update-check.js"',
    ].join(bodyEol);
  ...
  test('LF config.toml: stale forge-update-check block removed on reinstall', (t) => {
    ...
    assert.ok(
      !content.includes('forge-update-check'),
      'Stale forge-update-check entry must be removed from LF config.toml (#2698)'
    );
    ...
    assert.equal(
      ...
      true,
      'New forge-check-update hook must appear in hooks.json after reinstall'
    );
  });

  test('CRLF config.toml: stale forge-update-check block removed on reinstall', (t) => {
    const origCwd = process.cwd();
    t.after(() => { process.chdir(origCwd); });
    process.chdir(tmpDir);
    ...
    assert.ok(
      !content.includes('forge-update-check'),
      'Stale forge-update-check entry must be removed from CRLF config.toml (#2698)'
    );
    const hooksJsonCommands = readHooksSessionStartCommands(path.join(tmpDir, '.codex'));
    assert.equal(
      hooksJsonCommands.some((cmd) => cmd.includes('forge-check-update')),
  ...
  test('mixed-EOL config.toml: stale block with LF header but CRLF body removed on reinstall', (t) => {
    ...
    assert.ok(
      !content.includes('forge-update-check'),
      [
        'Stale forge-update-check block with mixed LF/CRLF endings must be removed (#2698).',
