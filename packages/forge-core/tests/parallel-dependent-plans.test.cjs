 ...
 *
 * Validates that:
 * 1. forge-planner.md assign_waves step explicitly checks files_modified overlap
...
const PLANNER_AGENT_PATH = path.join(__dirname, '..', 'agents', 'forge-planner.md');
...
// ---------------------------------------------------------------------------
// forge-planner.md — wave assignment must account for files_modified overlap
// ---------------------------------------------------------------------------

describe('forge-planner agent: files_modified wave ordering', () => {
  test('planner agent file exists', () => {
    assert.ok(fs.existsSync(PLANNER_AGENT_PATH), 'agents/forge-planner.md should exist');
  });
  ...
  test('assign_waves step contains explicit overlap → later-wave rule', () => {
    ...
    assert.ok(assignWavesMatch, 'assign_waves step should exist in forge-planner.md');
