...
const {
  ...
  CANONICAL_POINTS,
} = require('../forge-core/bin/lib/loop-resolver.cjs');
...
before(() => {
  ...
  fs.writeFileSync(
    path.join(tmpUiDisabledConfigDir, '.forge-surface.json'),
    JSON.stringify({
      baseProfile: 'full',
      disabledClusters: ['ui'],
      explicitAdds: [],
...
describe('renderLoopHooks', () => {
  ...
  test('step hook renders agent ref and inline prompt fragment', () => {
    ...
    const rendered = renderLoopHooks(resolved);
    assert.match(rendered, /agent:forge-phase-researcher/);
...
// ─── 9. End-to-end cmdLoopRenderHooks (via forge-tools subprocess) ─────────────

const { spawnSync } = require('node:child_process');
const ROOT = path.resolve(__dirname, '..');
const GSD_TOOLS = path.join(ROOT, 'forge-core', 'bin', 'forge-tools.cjs');

describe('cmdLoopRenderHooks end-to-end (via forge-tools)', () => {
...
describe('Phase 4 regression: capabilityStatesById gates on active (not enabled) for config-disabled capability', () => {
  test('[regression] enabled=true active=false + no `when` guard → hook NOT rendered (state.active gate)', () => {
    ...
    const registry = makeRegistry({
      point: 'plan:pre',
      steps: [{ capId: 'test-cap', ref: { skill: 'forge-test-skill' } }],
...
describe('ADR-1244 D2: fail-closed gate injection for skipped overlay caps with gates', () => {
  ...
  //
  // Fixture: overlay cap declares a gate at execute:wave:post with engines.forge: ">=99.0.0"
  ...
  test('skipped gate-kind overlay cap → BLOCKING synthetic gate at its declared point', (t) => {
    ...
    // - has engines.forge: ">=99.0.0" (incompatible → will be skipped at load)
    ...
    const capManifest = {
      ...
      tier: 'standard',
      requires: [],
      engines: { forge: '>=99.0.0' },  // intentionally incompatible → always skipped
    ...
    // Invoke forge-tools via subprocess so stdout is the real fd-1 (io.cjs writes via writeSync).
