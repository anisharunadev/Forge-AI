...
//   - COMMANDS.md mentions forge-tools intel api-surface
...
const NEW_PROJECT_PATH = path.join(ROOT, 'forge-core', 'workflows', 'new-project.md');
const SETTINGS_PATH = path.join(ROOT, 'forge-core', 'workflows', 'settings.md');
...
describe('feat-22-surfacing-docs', () => {
  ...
  test('COMMANDS.md mentions intel api-surface', () => {
    ...
    assert.ok(
      ...
      'COMMANDS.md must document the forge-tools intel api-surface command'
    );
  });
