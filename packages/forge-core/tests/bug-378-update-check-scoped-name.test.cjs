 ...
 * querying the SCOPED package name (@forge-ai/forge-core) when it asks
 * npm for the latest version.
 *
 * Background (#378): the worker once hardcoded the unscoped 'forge-core',
 ...
 * Identity seam (`forge-core/bin/lib/package-identity.cjs`). The seam's value
 ...
 *      packageName === the scoped '@forge-ai/forge-core'.
 *
 * Source-grep policy: this test reads hook source via readFileSync. The repo's
 * lint-no-source-grep rule targets bin/lib/forge-core — hooks/ is out of
...
const SEAM = require('../forge-core/bin/lib/package-identity.cjs');
const { PACKAGE_NAME } = require('../forge-core/bin/check-latest-version.cjs');
...
describe('bug #378 / #498: update worker queries the scoped name via the seam', () => {
  ...
  test('package.json name is the scoped @forge-ai/forge-core', () => {
    const pkg = JSON.parse(fs.readFileSync(PKG_PATH, 'utf8'));
    assert.equal(pkg.name, '@forge-ai/forge-core');
  });

  test('worker does NOT hardcode the unscoped forge-core as a string literal', () => {
    assert.doesNotMatch(
      workerCodeOnly(),
      /['"]forge-core['"]/,
      "Worker must not pass the unscoped 'forge-core' to npm — it 404s.",
    );
  });
  ...
  test('check-latest-version PACKAGE_NAME is single-sourced from the seam', () => {
    ...
    assert.equal(SEAM.packageName, '@forge-ai/forge-core');
  });
});
