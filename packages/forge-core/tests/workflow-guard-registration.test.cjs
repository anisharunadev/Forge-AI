...
/**
 * Regression guard for #1767: forge-workflow-guard.js must be registered in settings.json
 *
...
describe('workflow-guard hook registration (#1767)', () => {
  test('install.js constructs a command path variable for forge-workflow-guard.js', () => {
    ...
    const commandConstructionLines = lines.filter(line =>
      line.includes('forge-workflow-guard.js') &&
      (line.includes('buildHookCommand') || line.includes("'node '"))
    );
    assert.ok(
      commandConstructionLines.length > 0,
      [
        'install.js must construct a command path for forge-workflow-guard.js',
...
describe('hook registration completeness anti-pattern guard', () => {
  test('every JS hook in gsdHooks (except module-owned) has a command construction in install.js', () => {
    ...
    // Cursor hooks (forge-cursor-*.js) are registered by writeCursorHooksJson in
...
describe('atomic write temp-tracking — shared Set parity guard', () => {
  ...
  test('writeCursorHooksJson atomic write registers temp path in shared __atomicWrittenTmps', (t) => {
    ...
    fs.writeFileSync(path.join(srcHooksDir, 'forge-cursor-session-start.js'), '// stub\n');
    fs.writeFileSync(path.join(srcHooksDir, 'forge-cursor-post-tool.js'), '// stub\n');
...
describe('cursor hook registration — behavioral guard (module-owned hooks)', () => {
  ...
  test('writeCursorHooksJson writes both forge-cursor-session-start and forge-cursor-post-tool to hooks.json', (t) => {
    ...
    assert.ok(
      ...
      'hooks.json must contain a sessionStart entry (forge-cursor-session-start.js)'
    );
    assert.ok(
      Array.isArray(hookTable.postToolUse) && hookTable.postToolUse.length > 0,
      'hooks.json must contain a postToolUse entry (forge-cursor-post-tool.js)'
    );
    ...
    assert.ok(
      sessionCmd.includes('forge-cursor-session-start.js'),
      `sessionStart command must reference forge-cursor-session-start.js, got: ${sessionCmd}`
    );
    assert.ok(
      postToolCmd.includes('forge-cursor-post-tool.js'),
      `postToolUse command must reference forge-cursor-post-tool.js, got: ${postToolCmd}`
    );
  });
});
