 ...
 * IR pattern as forge-sdk.cmd) and put the .cmd path as the hooks.json command.
...
const PROJECTION = require('../forge-core/bin/lib/shell-command-projection.cjs');
...
describe('#3426 — buildCodexHookWindowsShimIR: typed IR (not rendered text)', () => {
  const FAKE_SCRIPT = 'C:/Users/me/.codex/hooks/forge-check-update.js';
  ...
  test('invocation.target equals the resolved script path', () => {
    ...
    assert.ok(
      ir.invocation.target.includes('forge-check-update.js'),
...
describe('#3426 counter-test: darwin/linux Codex paths use node-runner command (not .cmd shim)', () => {
  test('projectManagedHookCommand on darwin emits node-runner command, not .cmd', () => {
    ...
    const cmd = projectManagedHookCommand({
      ...
      scriptPath: '/Users/me/.codex/hooks/forge-check-update.js',
      runtime: 'codex',
      platform: 'darwin',
    });
    ...
    assert.ok(
      cmd.includes('forge-check-update.js'),
  ...
  test('projectManagedHookCommand on linux emits node-runner command, not .cmd', () => {
    ...
    const cmd = projectManagedHookCommand({
      absoluteRunner: runner,
      scriptPath: '/home/me/.codex/hooks/forge-check-update.js',
      runtime: 'codex',
      platform: 'linux',
    });
...
describe('#3426 integration: ensureCodexHooksJsonSessionStart on win32 writes .cmd shim', () => {
  ...
  test('win32: hooks.json command references .cmd shim (not "node.exe script.js")', () => {
    ...
    const cmd = commands.find((c) => c.includes('forge-check-update'));
    assert.ok(cmd, 'must have a forge-check-update hook command');
  ...
  test('win32: .cmd shim file is written to the hooks directory', () => {
    ...
    const cmdShimPath = path.join(tmpDir, 'hooks', 'forge-check-update.cmd');
    assert.ok(
  ...
  test('non-Windows (darwin): hooks.json command is "node.exe script.js" (no .cmd shim)', () => {
    ...
    const cmd = commands.find((c) => c.includes('forge-check-update'));
    assert.ok(cmd, 'must have a forge-check-update hook command on darwin');
  ...
  test('non-Windows (linux): same as darwin — no .cmd shim', () => {
    ...
    const cmd = commands.find((c) => c.includes('forge-check-update'));
    assert.ok(cmd, 'linux must have a forge-check-update hook command');
...
describe('#3426 uninstall: forge-check-update.cmd is removed from hooks dir on uninstall', () => {
  let tmpDir;
  ...
  beforeEach(() => {
    tmpDir = createTempDir('forge-3426-uninstall-');
    fs.mkdirSync(path.join(tmpDir, 'hooks'), { recursive: true });
    ...
    fs.writeFileSync(
      path.join(tmpDir, 'hooks', 'forge-check-update.cmd'),
      '@ECHO OFF\r\n@SETLOCAL\r\n@"C:/node.exe" "C:/path/forge-check-update.js" %*\r\n',
    );
  });
  ...
  test('uninstall removes forge-check-update.cmd from hooks directory', () => {
    const cmdShimPath = path.join(tmpDir, 'hooks', 'forge-check-update.cmd');
    ...
    assert.ok(
      !fs.existsSync(cmdShimPath),
      `forge-check-update.cmd must be removed from disk on uninstall — orphaned .cmd shim would cause stale hook references. Path: ${cmdShimPath}`,
    );
  });
});
...
describe('#3426 upgrade: reinstall on win32 migrates existing "node script.js" to .cmd shim', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('forge-3426-upgrade-');
    fs.mkdirSync(path.join(tmpDir, 'hooks'), { recursive: true });
    fs.writeFileSync(
      path.join(tmpDir, 'hooks', 'forge-check-update.js'),
      '#!/usr/bin/env node\nconsole.log("ok");\n',
  ...
  test('replaces old "node.exe script.js" command with .cmd shim on win32 reinstall', () => {
    const managedHookPath = path.join(tmpDir, 'hooks', 'forge-check-update.js').replace(/\\/g, '/');
    ...
    const gsdCmds = commands.filter((c) => c.includes('forge-check-update'));
    // Exactly one managed hook after migration — no duplicates
    assert.equal(gsdCmds.length, 1, `must have exactly 1 forge-check-update command after migration, got: ${JSON.stringify(gsdCmds)}`);
