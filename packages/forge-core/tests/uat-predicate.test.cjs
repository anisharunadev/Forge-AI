...
const {
  ...
  analyzeMarkdown,
  evaluateUatPassed,
} = require('../forge-core/bin/lib/uat-predicate.cjs');
const { cleanup } = require('./helpers.cjs');

// ─── Helpers ──────────────────────────────────────────────────────────────────

function makeTmpDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'forge-uat-pred-test-'));
}
  ...
  test('fc: inserting result:passed inside wrapper context never flips a failing UAT to passed', () => {
    ...
    fc.assert(
      fc.property(wrappers, fc.nat(3), (wrap, extraCount) => {
        ...
        const fcDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-fc-uat-'));
        try {
