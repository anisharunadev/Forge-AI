 ...
 * - If CONTEXT.md is missing: primary suggestion is /forge-discuss-phase
 * - If CONTEXT.md exists: primary suggestion is /forge-plan-phase
 */
...
const workflowPath = path.resolve(
  __dirname, '..', 'forge-core', 'workflows', 'execute-phase.md'
);
...
describe('bug #2002: offer_next checks CONTEXT.md before suggesting next step', () => {
  ...
  test('offer_next presents /forge-discuss-phase when CONTEXT.md does not exist', () => {
    content = content || fs.readFileSync(workflowPath, 'utf-8');
    ...
    // "does not exist" and "forge-discuss-phase" in the offer_next step
    ...
    assert.ok(
      ...
      /forge-discuss-phase.*recommended|recommended.*forge-discuss-phase/i.test(offerNextSection),
      'offer_next must present /forge-discuss-phase as primary when CONTEXT.md does not exist'
    );
  });

  test('offer_next presents /forge-plan-phase when CONTEXT.md exists', () => {
    content = content || fs.readFileSync(workflowPath, 'utf-8');
    const offerNextIdx = content.indexOf('offer_next');
    assert.ok(offerNextIdx !== -1, 'offer_next step must exist');
    ...
    assert.ok(
      ...
      'offer_next must present /forge-plan-phase as primary when CONTEXT.md exists'
    );
  });
