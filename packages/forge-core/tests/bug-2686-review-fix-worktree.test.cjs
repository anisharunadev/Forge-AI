 ...
 *
 * The forge-code-fixer agent (spawned by /forge-code-review-fix) operated directly
...
describe('bug-2686: review-fix agent worktree isolation', () => {
  ...
  before(() => {
    const agentPath = path.join(__dirname, '..', 'agents', 'forge-code-fixer.md');
  ...
  test('agent instructions include git worktree add before any branch-switching checkout or commit', () => {
    ...
    assert.ok(
      worktreePos !== -1,
      'forge-code-fixer.md must include a "git worktree add" instruction to isolate operations from the main working tree (#2686)'
    );
    ...
    // either `git commit` directly or `forge-sdk query commit`
  ...
  test('agent instructions include worktree cleanup after completion', () => {
    assert.ok(
      ...
      'forge-code-fixer.md must include worktree cleanup (git worktree remove) to avoid leaking tmp directories (#2686)'
    );
  });
  ...
  test('agent instructions use a /tmp path for the worktree', () => {
    ...
    assert.ok(
      hasTmpWorktreePath,
      'forge-code-fixer.md must define a worktree variable at a /tmp/sv-... path, consistent with other GSD agents (#2686)'
    );
  });
});
