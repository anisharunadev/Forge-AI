...
describe('MODEL_PROFILES', () => {
  test('contains every shipped forge agent file on disk (#3229)', () => {
...
describe('VALID_PROFILES', () => {
  ...
  test('includes all MODEL_PROFILES keys plus inherit', () => {
    const fromData = Object.keys(MODEL_PROFILES['forge-planner']);
...
describe('getAgentToModelMapForProfile', () => {
  test('returns correct models for balanced profile', () => {
    ...
    assert.strictEqual(map['forge-planner'], 'opus');
    assert.strictEqual(map['forge-codebase-mapper'], 'haiku');
    assert.strictEqual(map['forge-verifier'], 'sonnet');
  });

  test('returns correct models for budget profile', () => {
    const map = getAgentToModelMapForProfile('budget');
    assert.strictEqual(map['forge-planner'], 'sonnet');
    assert.strictEqual(map['forge-phase-researcher'], 'haiku');
  });

  test('returns correct models for quality profile', () => {
    const map = getAgentToModelMapForProfile('quality');
    assert.strictEqual(map['forge-planner'], 'opus');
    assert.strictEqual(map['forge-executor'], 'opus');
  });

  test('returns correct models for adaptive profile', () => {
    const map = getAgentToModelMapForProfile('adaptive');
    assert.strictEqual(map['forge-planner'], 'opus', 'planner should use opus in adaptive');
    assert.strictEqual(map['forge-debugger'], 'opus', 'debugger should use opus in adaptive');
    assert.strictEqual(map['forge-executor'], 'sonnet', 'executor should use sonnet in adaptive');
    assert.strictEqual(map['forge-codebase-mapper'], 'haiku', 'mapper should use haiku in adaptive');
    assert.strictEqual(map['forge-plan-checker'], 'haiku', 'checker should use haiku in adaptive');
  });

  // ─── resolution order: override > profile > default ─────────────────────────
  // Uses forge-phase-researcher because it has visibly distinct values at every
  ...
  describe('resolution order: override > profile > default', () => {
    ...
    const AGENT = 'forge-phase-researcher';
...
describe('formatAgentToModelMapAsTable', () => {
  test('produces a table with header and separator', () => {
    const map = { 'forge-planner': 'opus', 'forge-executor': 'sonnet' };
    ...
    assert.ok(table.includes('forge-planner'), 'should list agent');
