...
// window on Windows (the "forge-core" flash).
'use strict';
...
describe('bug #685: Windows spawns must set windowsHide:true (no console-window flash)', () => {
  test('forge-context-monitor record-session spawn sets windowsHide', () => {
    const region = regionBetween(read('hooks/forge-context-monitor.js'), "'record-session'", '.unref()');
  ...
  test('forge-worktree-path-guard SPAWNOPT sets windowsHide', () => {
    const region = regionBetween(read('hooks/forge-worktree-path-guard.js'), 'const SPAWNOPT', '};');
    assert.match(region, /windowsHide:\s*true/, 'forge-worktree-path-guard SPAWNOPT must set windowsHide: true');
  });

  test('forge-workflow-guard currentBranch spawnSync sets windowsHide', () => {
    const region = regionBetween(read('hooks/forge-workflow-guard.js'), "spawnSync('git', ['branch'", '});');
    assert.match(region, /windowsHide:\s*true/, 'forge-workflow-guard git-branch spawn must set windowsHide: true');
  });
  ...
  test('forge-check-update spawn retains windowsHide (precedent guard)', () => {
    assert.match(read('hooks/forge-check-update.js'), /windowsHide:\s*true/,
      'forge-check-update.js must keep windowsHide: true');
  });
