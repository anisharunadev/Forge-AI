'use strict';
/**
 * Regression test for bug #891: forge_run launcher must probe non-Claude
 * runtime homes before emitting the hard error.
 *
 * The last-resort $HOME/.claude/forge-core branch is Claude Code-specific.
 * Every non-Claude runtime (Hermes, Cursor, Codex, Copilot, Windsurf, …)
 * installs forge-core into a *different* directory that the shim never tried,
 * causing a false-positive fatal ERROR on all non-Claude runtimes when
 * RUNTIME_DIR is not set and forge-tools is not on PATH.
 *
 * Asserts:
 * (A) Snippet contains all expected non-Claude runtime home probes (structural).
 * (B) HERMES_HOME behavioral: when RUNTIME_DIR misses and forge-tools is NOT on
 *     PATH, a stub at ${HERMES_HOME}/forge-core/bin/forge-tools.cjs is invoked.
 * (C) Default Hermes path behavioral: stub at $HOME/.hermes/forge-core/bin/
 *     forge-tools.cjs is invoked when HERMES_HOME is not set.
 * (D) Resolution order: non-Claude homes are probed BEFORE the hard error,
 *     and AFTER the $HOME/.claude branch.
 * (E) Propagation: all workflow .md files using forge_run contain each probe
...
// fallback arms in the forge_run launcher snippet -- asserts literal substring
...
// forge-core install location). Mirrors src/runtime-homes.cts getGlobalConfigDir().
const EXPECTED_RUNTIME_PROBES = {
  hermes:      '.hermes}/forge-core/bin/',
  cursor:      '.cursor}/forge-core/bin/',
  codex:       '.codex}/forge-core/bin/',
  gemini:      '.gemini}/forge-core/bin/',
  copilot:     '.copilot}/forge-core/bin/',
  windsurf:    '.codeium/windsurf}/forge-core/bin/',
  augment:     '.augment}/forge-core/bin/',
  trae:        '.trae}/forge-core/bin/',
  qwen:        '.qwen}/forge-core/bin/',
  codebuddy:   '.codebuddy}/forge-core/bin/',
  cline:       '.cline}/forge-core/bin/',
  grok:        '.agents}/forge-core/bin/',
  antigravity: '.gemini/antigravity}/forge-core/bin/',
  opencode:    'opencode}/forge-core/bin/',
  kilo:        'kilo}/forge-core/bin/',
};
...
/**
 * Build a PATH with no forge-tools binary so the PATH fallback branch is skipped,
 * while guaranteeing that a bare `node` lookup still resolves regardless of whether
 * the real node binary co-locates with a global forge-tools shim (e.g. fnm/nvm/Homebrew).
 *
 * Strategy (POSIX only): create a temp dir containing only a `node` symlink →
 * process.execPath, prepend it to the forge-tools-filtered PATH.  The filtered
 * PATH excludes any directory that contains an executable `forge-tools`.
 *
 * On Windows the co-location bug does not apply (forge-tools resolves via .cmd/.ps1,
...
function buildIsolatedPath() {
  ...
  const nodeBinDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-891-node-'));
  try {
...
describe('bug-891: non-Claude runtime home fallback arms', () => {
  ...
  test('(A) snippet contains all non-Claude runtime home probes', () => {
    ...
    assert.deepStrictEqual(
      ...
      `_runtime-launcher.snippet.sh is missing fallback probes for non-Claude runtimes:\n` +
        missing.join('\n') +
        `\n\nAdd elif arms for each runtime home (e.g. "\${HERMES_HOME:-$HOME/.hermes}/forge-core/bin/...")` +
        ` before the hard-error else. Current snippet:\n${snippetContent.trim()}`,
    );
  });

  // ── (A2) Structural: probes appear AFTER .claude arm but BEFORE hard error ─
  test('(A2) non-Claude probes appear after .claude/forge-core arm and before hard error', () => {
    const snippetContent = fs.readFileSync(SNIPPET_FILE, 'utf8');
    const claudePos = snippetContent.indexOf('.claude/forge-core/bin/');
    const errorPos  = snippetContent.indexOf('exit 1');

    assert.ok(claudePos !== -1, 'Snippet must still contain .claude/forge-core/bin/ arm (regression guard)');
  ...
  //        forge-tools co-locate in the same PATH directory.                     ─
  //
  // Machine-independence guarantee: PATH is set to ONLY two controlled dirs —
  // fakeBinDir (holds both fake forge-tools AND a node symlink) plus a fresh
  ...
  test(
    '(B0) buildIsolatedPath: node is resolvable and forge-tools is not when they share a PATH dir',
    { skip: process.platform === 'win32' ? 'POSIX-only co-location scenario' : false },
    (t) => {
      // Build a fake bin dir that contains BOTH a forge-tools executable and a node
      // symlink, simulating a dev setup (fnm/nvm/Homebrew) where both land in the
      // same bin directory.
      const fakeBinDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-891-colocated-'));
      // A second fresh empty dir — contains neither forge-tools nor node.
      const emptyDir   = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-891-empty-'));
      t.after(() => cleanup(fakeBinDir));
      t.after(() => cleanup(emptyDir));

      // Fake forge-tools shim (executable file)
      const fakeGsdTools = path.join(fakeBinDir, 'forge-tools');
      fs.writeFileSync(fakeGsdTools, '#!/bin/sh\necho fake-forge-tools\n');
      fs.chmodSync(fakeGsdTools, 0o755);

      // node symlink pointing at the real interpreter (co-located with forge-tools)
      ...
      // (i) forge-tools must NOT be resolvable on the returned PATH
      const gsdToolsResolvable = returnedDirs.some((dir) => {
        try { fs.accessSync(path.join(dir, 'forge-tools'), fs.constants.X_OK); return true; }
        catch { return false; }
      });
      assert.equal(
        gsdToolsResolvable,
        false,
        'forge-tools must not be resolvable on the isolated PATH (home-fallback would be bypassed)',
      );
  ...
  test('(B) forge_run resolves ${HERMES_HOME}/forge-core/bin/ stub when set and local+PATH both miss', () => {
    const fakeHome       = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-891-home-b-'));
    const fakeHermesHome = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-891-hermes-'));
    const fakeRuntime    = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-891-rt-'));
    const { isolatedPath, nodeBinDir } = buildIsolatedPath();
    try {
      const hermesBinDir = path.join(fakeHermesHome, 'forge-core', 'bin');
      fs.mkdirSync(hermesBinDir, { recursive: true });

      const stubPath = path.join(hermesBinDir, 'forge-tools.cjs');
      fs.writeFileSync(
        stubPath,
      ...
      assert.ok(
        normStdout.includes('forge-core/bin/'),
        `Expected GSD_TOOLS to resolve into hermes forge-core/bin/, got:\n${stdout.trim()}`,
      );
      assert.ok(
  ...
  test('(C) forge_run resolves $HOME/.hermes/forge-core/bin/ stub when HERMES_HOME is unset', () => {
    const fakeHome    = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-891-home-'));
    const fakeRuntime = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-891-rt2-'));
    const { isolatedPath, nodeBinDir } = buildIsolatedPath();
    try {
      const hermesBinDir = path.join(fakeHome, '.hermes', 'forge-core', 'bin');
      fs.mkdirSync(hermesBinDir, { recursive: true });
      ...
      const normStdout = stdout.replace(/\\/g, '/');
      assert.ok(
        normStdout.includes('.hermes/forge-core/bin/'),
        `Expected GSD_TOOLS to resolve into .hermes/forge-core/bin/, got:\n${stdout.trim()}`,
      );
      assert.ok(
  ...
  test('(D) resolution order: .claude probe comes before hermes probe, hermes before hard error', () => {
    const snippetContent = fs.readFileSync(SNIPPET_FILE, 'utf8');
    const claudePos  = snippetContent.indexOf('.claude/forge-core/bin/');
    const hermesPos  = snippetContent.indexOf('.hermes}/forge-core/bin/');
    const errorPos   = snippetContent.indexOf('exit 1');

    assert.ok(claudePos  !== -1, 'Snippet must contain .claude/forge-core/bin/ arm');
    assert.ok(hermesPos  !== -1, 'Snippet must contain .hermes}/forge-core/bin/ arm');
  ...
  // ── (E) Propagation: workflow .md files using forge_run contain hermes probe ─
  test('(E) all workflow .md files using forge_run contain the hermes runtime home probe', () => {
    const HERMES_PROBE = '.hermes}/forge-core/bin/';
    ...
    assert.deepStrictEqual(
      missing,
      [],
      `These workflow files use forge_run but are missing the hermes runtime home probe ("${HERMES_PROBE}"). ` +
