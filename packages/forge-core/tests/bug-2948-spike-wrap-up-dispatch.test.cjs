 ...
 *
 * `/forge:spike --wrap-up` was silently no-oping because:
 * 1. `commands/forge/spike.md` listed `--wrap-up` as a flag but had no dispatch block.
 * 2. `workflows/spike.md` still referenced the deleted `/forge-spike-wrap-up` entry-point
 *    instead of the correct `/forge:spike --wrap-up` form.
 *
 * Fix:
 * - `commands/forge/spike.md` now has a dispatch block that routes `--wrap-up` to
 *   spike-wrap-up.md, and spike-wrap-up.md is listed in execution_context so the
 *   runtime can find it.
 * - `workflows/spike.md` companion references updated from `/forge-spike-wrap-up` to
 *   `/forge:spike --wrap-up`.
 */
...
const SPIKE_CMD_PATH = path.join(__dirname, '..', 'commands', 'forge', 'spike.md');
const SPIKE_WORKFLOW_PATH = path.join(__dirname, '..', 'forge-core', 'workflows', 'spike.md');
...
describe('bug-2948: /forge:spike --wrap-up dispatch wiring', () => {
  describe('commands/forge/spike.md — frontmatter and section contract', () => {
    test('spike.md command file exists and has valid frontmatter', () => {
      assert.ok(fs.existsSync(SPIKE_CMD_PATH), 'commands/forge/spike.md should exist');
  ...
  describe('forge-core/workflows/spike.md — companion references', () => {
    test('spike workflow file exists', () => {
      assert.ok(fs.existsSync(SPIKE_WORKFLOW_PATH), 'forge-core/workflows/spike.md should exist');
    });

    test('does NOT reference the deleted /forge-spike-wrap-up entry-point', () => {
      const fm = parseFrontmatter(fs.readFileSync(SPIKE_WORKFLOW_PATH, 'utf-8'));
      assert.ok(
        !fm._body.includes('/forge-spike-wrap-up'),
        'workflows/spike.md must not reference the deleted /forge-spike-wrap-up command; use /forge:spike --wrap-up instead'
      );
    });

    test('references /forge:spike --wrap-up as the canonical wrap-up invocation', () => {
      const fm = parseFrontmatter(fs.readFileSync(SPIKE_WORKFLOW_PATH, 'utf-8'));
      assert.ok(
        fm._body.includes('/forge:spike --wrap-up'),
        'workflows/spike.md must reference /forge:spike --wrap-up as the canonical wrap-up command'
      );
    });
  });
});
