...
function tmpDir(prefix) {
  return fs.mkdtempSync(path.join(os.tmpdir(), prefix || 'forge-ip-'));
}

function createFixtureSkillsDir() {
  const tmp = createTempDir('forge-stage-profile-');
...
function createFixtureAgentsDir() {
  const tmp = createTempDir('forge-agents-profile-');
  for (const name of ['forge-planner', 'forge-executor', 'forge-code-reviewer']) {
    fs.writeFileSync(path.join(tmp, `${name}.md`), `# ${name}\n`);
  }
  return tmp;
}
...
describe('stageSkillsForRuntimeAsSkills', () => {
  ...
  test('registers stagedDir in STAGED_DIRS after staging', (t) => {
    const src = createTempDir('forge-rta-src-');
    let stagedDir;
    t.after(() => {
      cleanup(src);
    ...
    stagedDir = stageSkillsForRuntimeAsSkills(src, { skills: '*' }, converter, 'forge-');
  ...
  test('non-existent srcCommandsDir returns srcCommandsDir unchanged', () => {
    const ghost = path.join(os.tmpdir(), 'forge-rta-no-exist-' + Date.now());
    const converter = (content, _skillName) => content;
    const result = stageSkillsForRuntimeAsSkills(ghost, { skills: '*' }, converter, 'forge-');
  ...
  test('skills Set filters: only matching stems land in stagedDir', (t) => {
    ...
    const converter = (content, _skillName) => content;
    stagedDir = stageSkillsForRuntimeAsSkills(src, { skills: new Set(['phase']) }, converter, 'forge-');
    const entries = fs.readdirSync(stagedDir).sort();
    assert.deepStrictEqual(entries, ['forge-phase']);
  });
  ...
  test('skills === "*" stages all md files as <prefix><stem>/SKILL.md', (t) => {
    ...
    stagedDir = stageSkillsForRuntimeAsSkills(src, { skills: '*' }, converter, 'forge-');
    const entries = fs.readdirSync(stagedDir).sort();
    assert.deepStrictEqual(entries, ['forge-alpha', 'forge-beta', 'forge-gamma']);
    for (const name of ['alpha', 'beta', 'gamma']) {
      const content = fs.readFileSync(path.join(stagedDir, `forge-${name}`, 'SKILL.md'), 'utf8');
...
describe('stageSkillsForProfile', () => {
  ...
  test('non-existent srcDir returns srcDir unchanged', () => {
    const ghost = path.join(os.tmpdir(), 'forge-no-exist-' + Date.now());
...
describe('stageAgentsForProfile', () => {
  ...
  test('non-full profile with agents Set copies only member agent files', (t) => {
    ...
    const agents = new Set(['forge-planner']);
    staged = stageAgentsForProfile(src, { skills: new Set(['plan-phase']), agents });
    const files = fs.readdirSync(staged).sort();
    assert.deepStrictEqual(files, ['forge-planner.md']);
  });

  test('non-existent srcAgentsDir returns srcAgentsDir unchanged', () => {
    const ghost = path.join(os.tmpdir(), 'forge-agents-no-exist-' + Date.now());
  ...
  test('standard profile — stageAgentsForProfile copies exactly the agents in resolvedProfile.agents', (t) => {
    ...
    assert.ok(resolved.agents instanceof Set && resolved.agents.size > 0,
      'standard profile must have >0 agents (plan-phase calls forge-planner etc)');
    let staged;
    t.after(() => {
      if (staged) cleanupStagedSkills();
    });
...
describe('resolveProfile', () => {
  ...
  test('standard — agents Set is non-empty; forge-planner and forge-plan-checker present', () => {
    const manifest = loadSkillsManifest(REAL_COMMANDS_DIR);
    ...
    assert.ok(result.agents.has('forge-planner'), 'standard should include forge-planner');
    assert.ok(result.agents.has('forge-plan-checker'), 'standard should include forge-plan-checker');
  });
  ...
  test('agents are derived from real manifest body text (forge-planner from plan-phase)', () => {
    const realManifest = loadSkillsManifest(REAL_COMMANDS_DIR);
    const result = resolveProfile({ modes: ['standard'], manifest: realManifest });
    assert.ok(result.agents.has('forge-planner'), 'forge-planner should be derived from plan-phase body');
  });

  test('agents transitively closed — plan-phase in standard brings its agents', () => {
    const manifest = loadSkillsManifest(REAL_COMMANDS_DIR);
    const result = resolveProfile({ modes: ['standard'], manifest });
    assert.ok(result.agents.has('forge-planner'));
  });
});
...
describe('loadSkillsManifest', () => {
  test('returns a Map', () => {
    const dir = tmpDir('forge-manifest-fixture-');
    try {
  ...
  test('skill with no requires: frontmatter maps to empty array', () => {
    const dir = tmpDir('forge-manifest-fixture-');
    try {
      writeSkill(dir, 'help', 'name: forge:help\ndescription: Help text');
      const m = loadSkillsManifest(dir);
  ...
  test('skill with requires: single value maps to array of one', () => {
    const dir = tmpDir('forge-manifest-fixture-');
    try {
      writeSkill(dir, 'add-tests', 'name: forge:add-tests\ndescription: Add tests\nrequires: [phase]');
      const m = loadSkillsManifest(dir);
  ...
  test('skill with requires: multiple values maps to full array', () => {
    const dir = tmpDir('forge-manifest-fixture-');
    try {
      writeSkill(dir, 'plan-phase', 'name: forge:plan-phase\ndescription: Plan\nrequires: [discuss-phase, phase, review, update]');
      const m = loadSkillsManifest(dir);
  ...
  test('skill with requires: empty array maps to empty array', () => {
    const dir = tmpDir('forge-manifest-fixture-');
    try {
      writeSkill(dir, 'explore', 'name: forge:explore\ndescription: Explore\nrequires: []');
      const m = loadSkillsManifest(dir);
  ...
  test('loads real commands/forge/ directory: >=60 skills, discuss-phase deps correct, help has no requires', () => {
...
describe('readActiveProfile / writeActiveProfile', () => {
  ...
  test('non-existent directory returns null (not throws)', () => {
    const ghost = path.join(os.tmpdir(), 'forge-marker-no-exist-' + Date.now());
  ...
  test('corrupt marker content (invalid chars) returns null', () => {
    const dir = tmpDir('forge-marker-');
    try {
      fs.writeFileSync(path.join(dir, '.forge-profile'), 'profile with spaces and !!!\n');
      const result = readActiveProfile(dir);
      assert.strictEqual(result, null);
    } finally {
      cleanup(dir);
  ...
  test('empty marker file returns null', () => {
    const dir = tmpDir('forge-marker-');
    try {
      fs.writeFileSync(path.join(dir, '.forge-profile'), '');
      const result = readActiveProfile(dir);
      assert.strictEqual(result, null);
    } finally {
      cleanup(dir);
  ...
  test('writeActiveProfile creates the directory if it does not exist', () => {
    const base = tmpDir('forge-marker-base-');
