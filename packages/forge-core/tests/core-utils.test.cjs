/**
 * Tests for src/core-utils.cts (compiled to forge-core/bin/lib/core-utils.cjs).
 *
...
const coreUtils = require('../forge-core/bin/lib/core-utils.cjs');
...
describe('detectSubRepos', () => {
  ...
  test('returns empty array for directory with no sub-repos', () => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-cu-test-'));
