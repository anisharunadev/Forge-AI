/**
 * Tests for forge-update-banner.js (#2795).
 *
 * The banner hook is an opt-in SessionStart consumer of the update cache that
 * forge-check-update-worker.js writes. When a user declines GSD's statusline,
...
const HOOK_PATH = path.join(__dirname, '..', 'hooks', 'forge-update-banner.js');
const {
  buildBannerOutput,
  shouldSuppressFailureWarning,
  RATE_LIMIT_SECONDS,
} = require('../hooks/forge-update-banner.js');
const { updateCacheFileName } = require('../forge-core/bin/lib/package-identity.cjs');
...
describe('buildBannerOutput', () => {
  ...
  test('returns banner envelope when update_available is true', () => {
    const out = buildBannerOutput({
      cache: { update_available: true, installed: '1.39.0', latest: '1.40.0', package_name: '@forge-ai/forge-core' },
      parseError: false,
      suppressFailureWarning: false,
    });
    ...
    assert.ok(
      out.systemMessage.includes('/forge:update'),
      'banner should reference /forge:update command'
    );
  });
  ...
  test('falls back to "unknown" when installed/latest missing', () => {
    const out = buildBannerOutput({
      cache: { update_available: true, package_name: '@forge-ai/forge-core' },
      parseError: false,
      suppressFailureWarning: false,
    });
    assert.ok(out);
...
describe('shouldSuppressFailureWarning', () => {
  function tmpDir() {
    return fs.mkdtempSync(path.join(os.tmpdir(), 'forge-banner-supp-'));
  }
...
describe('forge-update-banner.js end-to-end', () => {
  function setupHome() {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-banner-home-'));
    fs.mkdirSync(path.join(home, '.cache', 'forge'), { recursive: true });
    return home;
  }
  ...
  function writeCache(home, contents) {
    fs.writeFileSync(
      path.join(home, '.cache', 'forge', updateCacheFileName),
  ...
  test('emits valid SessionStart JSON when update_available=true', () => {
    ...
    try {
      writeCache(home, {
        ...
        installed: '1.39.0',
        latest: '1.40.0',
        package_name: '@forge-ai/forge-core',
      });
      const r = runHook(home);
      ...
      assert.ok(parsed.systemMessage.includes('/forge:update'));
    } finally {
      cleanup(home);
    }
  });
  ...
  test('suppresses repeat failure diagnostic within 24h via sentinel', () => {
    ...
    try {
      ...
      const sentinel = path.join(home, '.cache', 'forge', 'banner-failure-warned-at');
...
describe('install.js update-banner wiring', () => {
  ...
  test('buildUpdateBannerHookEntry produces a SessionStart hook entry', () => {
    ...
    const entry = installExports.buildUpdateBannerHookEntry(
      '"/usr/local/bin/node" "/home/u/.claude/hooks/forge-update-banner.js"'
    );
    ...
    assert.ok(
      entry.hooks[0].command.includes('forge-update-banner.js'),
