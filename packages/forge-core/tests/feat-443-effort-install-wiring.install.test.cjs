 ...
 *   5. Source agents/forge-planner.md has NO effort: key (injection is
 ...
 * directly (e.g. ~/.cache/forge update-check deletion, ~/.forge/defaults.json
 ...
 * $HOME/.claude/forge-core/bin/forge-tools.cjs is absent).
 *
...
function runGlobalInstall(runtime, tmpHome) {
  ...
  const isolatedHome = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-443-home-'));
...
// forge-planner: heavy → xhigh
// forge-codebase-mapper: light → low
// forge-executor: standard → high
...
describe('#443 Claude install: effort: injected into frontmatter', () => {
  ...
  beforeEach(() => {
    tmpDir = makeTmpDir('forge-443-claude-');
  ...
  test('forge-planner.md contains effort: xhigh (heavy tier default)', () => {
    runGlobalInstall('claude', claudeHome);
    const fm = readFrontmatter(path.join(claudeHome, 'agents', 'forge-planner.md'));
    assert.match(fm, /^effort:\s*xhigh$/m,
      `forge-planner frontmatter should have effort: xhigh\nActual:\n${fm}`);
  });

  test('forge-codebase-mapper.md contains effort: low (light tier default)', () => {
    runGlobalInstall('claude', claudeHome);
    const fm = readFrontmatter(path.join(claudeHome, 'agents', 'forge-codebase-mapper.md'));
    assert.match(fm, /^effort:\s*low$/m,
      `forge-codebase-mapper frontmatter should have effort: low\nActual:\n${fm}`);
  });

  test('forge-executor.md contains effort: high (standard tier default)', () => {
    runGlobalInstall('claude', claudeHome);
    const fm = readFrontmatter(path.join(claudeHome, 'agents', 'forge-executor.md'));
    assert.match(fm, /^effort:\s*high$/m,
      `forge-executor frontmatter should have effort: high\nActual:\n${fm}`);
  });
});
...
describe('#443 Gemini install: effort: absent (Gemini-safe)', () => {
  ...
  beforeEach(() => {
    tmpDir = makeTmpDir('forge-443-gemini-');
  ...
  test('forge-planner.md does NOT contain effort: (Gemini install)', () => {
    runGlobalInstall('gemini', geminiHome);
    const fm = readFrontmatter(path.join(geminiHome, 'agents', 'forge-planner.md'));
    assert.doesNotMatch(fm, /^effort:/m,
      `forge-planner (Gemini) frontmatter must NOT have effort:\nActual:\n${fm}`);
  });

  test('forge-executor.md does NOT contain effort: (Gemini install)', () => {
    runGlobalInstall('gemini', geminiHome);
    const fm = readFrontmatter(path.join(geminiHome, 'agents', 'forge-executor.md'));
    assert.doesNotMatch(fm, /^effort:/m,
      `forge-executor (Gemini) frontmatter must NOT have effort:\nActual:\n${fm}`);
  });
});
...
describe('#838 Codex install: inherited model omits model_reasoning_effort', () => {
  ...
  beforeEach(() => {
    tmpDir = makeTmpDir('forge-443-codex-');
  ...
  test('forge-planner.toml omits both model and model_reasoning_effort when model is inherited', () => {
    runGlobalInstall('codex', codexHome);
    const tomlContent = fs.readFileSync(
      path.join(codexHome, 'agents', 'forge-planner.toml'), 'utf8'
    );
    assert.doesNotMatch(tomlContent, /^model\s*=/m,
      `forge-planner.toml should omit model when inheriting Codex chat model\nActual:\n${tomlContent.slice(0, 500)}`);
    assert.doesNotMatch(tomlContent, /^model_reasoning_effort\s*=/m,
      `forge-planner.toml should omit model_reasoning_effort when model is inherited\nActual:\n${tomlContent.slice(0, 500)}`);
  });
});
...
describe('#443 Config-driven: effort.agent_overrides drives install-time effort', () => {
  ...
  beforeEach(() => {
    ...
    tmpDir = makeTmpDir('forge-443-cfg-');
    ...
    // Write a project config with effort.agent_overrides overriding forge-planner to 'low'.
  ...
  test('Claude .md gets effort: low when agent_overrides.forge-planner=low', () => {
    ...
    const fm = readFrontmatter(path.join(claudeHome, 'agents', 'forge-planner.md'));
    assert.match(fm, /^effort:\s*low$/m,
      `forge-planner should have effort: low from config override\nActual:\n${fm}`);
  });

  test('Codex .toml gets model_reasoning_effort = "low" when agent_overrides.forge-planner=low', () => {
    runGlobalInstall('codex', codexHome);
    const tomlContent = fs.readFileSync(
      path.join(codexHome, 'agents', 'forge-planner.toml'), 'utf8'
    );
    assert.match(tomlContent, /^model\s*=\s*"gpt-5.5"$/m,
      `forge-planner.toml should pin Codex model when runtime:"codex" is configured\nActual:\n${tomlContent.slice(0, 500)}`);
    assert.match(tomlContent, /^model_reasoning_effort\s*=\s*"low"$/m,
      `forge-planner.toml should have model_reasoning_effort = "low" from config override\nActual:\n${tomlContent.slice(0, 500)}`);
  });

  test('Codex .toml clamps effort max → xhigh when agent_overrides.forge-planner=max', () => {
    ...
    assert.match(tomlContent, /^model_reasoning_effort\s*=\s*"xhigh"$/m,
      `forge-planner.toml should clamp max → xhigh for Codex\nActual:\n${tomlContent.slice(0, 500)}`);
...
describe('#443 resolveInstallTimeEffort: invalid tokens fall through to valid effort', () => {
  ...
  beforeEach(() => {
    ...
    //           .claude/          <-- claudeHome
    //           .codex/           <-- codexHome
    tmpDir = makeTmpDir('forge-443-invalid-effort-');
    const projectDir = path.join(tmpDir, 'project');
    claudeHome = path.join(projectDir, '.claude');
    codexHome = path.join(projectDir, '.codex');
  ...
  test('effort.agent_overrides.forge-planner="bogus" (invalid) with valid default -> falls through to valid default', () => {
    // BUG before fix: "bogus" is returned and written verbatim
    writeProjectConfig({
      effort: {
        agent_overrides: { 'forge-planner': 'bogus' },
        default: 'medium',
      },
    });
    runGlobalInstall('claude', claudeHome);
...
describe('#443 Source purity: agents/forge-planner.md has no effort: key', () => {
  test('source agents/forge-planner.md frontmatter does not contain effort:', () => {
    const fm = readFrontmatter(path.join(SOURCE_AGENTS_DIR, 'forge-planner.md'));
    assert.doesNotMatch(fm, /^effort:/m,
      `Source agents/forge-planner.md must NOT contain effort: (injection is install-only)`);
  });

  test('source agents/forge-executor.md frontmatter does not contain effort:', () => {
    const fm = readFrontmatter(path.join(SOURCE_AGENTS_DIR, 'forge-executor.md'));
    assert.doesNotMatch(fm, /^effort:/m,
      `Source agents/forge-executor.md must NOT contain effort: (injection is install-only)`);
  });

  test('source agents/forge-codebase-mapper.md frontmatter does not contain effort:', () => {
    const fm = readFrontmatter(path.join(SOURCE_AGENTS_DIR, 'forge-codebase-mapper.md'));
    assert.doesNotMatch(fm, /^effort:/m,
      `Source agents/forge-codebase-mapper.md must NOT contain effort: (injection is install-only)`);
  });
});
