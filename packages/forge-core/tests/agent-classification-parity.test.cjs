 ...
 *   - ### forge-<name> headings BEFORE "## Advanced and Specialized Agents" → "primary"
 *   - ### forge-<name> headings INSIDE/AFTER that section → "advanced stub"
 *   - agents/forge-*.md files with NO heading in AGENTS.md → "inventory only"
 */
 ...
 *   primaryHeadings  — agent names (without "forge-" prefix) before the advanced section
 ...
 * ### forge-* headings are counted: primary if 'before', advanced if 'advanced',
 * ignored if 'past'.
 */
 ...
 * happens to contain a "| forge-..." row from polluting the result.
 *
...
function parseInventoryMd(raw) {
  ...
  for (const line of lines) {
    ...
    // Data rows: first cell must be a forge-* slug
    if (!cells[0].startsWith('forge-')) continue;
...
// Canonical source roster (sorted forge-* basenames without .md) — shared helper.
...
assert.ok(
  ...
  'AGENTS.md: no ### forge-* headings found before Advanced section — raw head:\n' + rawAgentsMd.slice(0, 300),
);

assert.ok(
  advancedHeadings.length > 0,
  'AGENTS.md: no ### forge-* headings found inside Advanced section — raw head:\n' + rawAgentsMd.slice(0, 300),
);

assert.ok(
  inventoryMap.size > 0,
  'INVENTORY.md: no | forge-* | rows parsed — raw head:\n' + rawInventoryMd.slice(0, 300),
);

assert.ok(
  agentFiles.length > 0,
  'agents/: no forge-*.md files found — check AGENTS_DIR path: ' + AGENTS_DIR,
);
...
describe('agent-classification-parity: AGENTS.md section structure is the single source of truth', () => {
   ...
   *      (slugs without the "forge-" prefix, as the prose uses)
   */
  test('AGENTS.md prose counts and advanced-agent parenthetical list are accurate', () => {
    ...
    assert.strictEqual(
      ...
      `AGENTS.md prose says "${prosePrimaryCount} primary agents" but there are ${primaryHeadings.length} ### forge-* headings before the Advanced section`,
    );
    ...
    assert.strictEqual(
      ...
      `AGENTS.md prose says "${advancedCountRaw} additional shipped agents" but there are ${advancedHeadings.length} ### forge-* headings in the Advanced section`,
    );

    // --- (c) parenthetical slug list ---
    // The prose lists short slugs without "forge-" prefix, e.g.:
    ...
    // Derive expected slugs from AGENTS.md headings (strip "forge-" prefix)
   ...
   *
   * Sub-checks:
   *   a) Every agents/forge-*.md appears exactly once in (primary ∪ advanced ∪ inventoryOnly)
  ...
  test('Roster completeness: every agent file is classified exactly once', () => {
    ...
    assert.deepStrictEqual(
      ...
      [],
      'agents/forge-*.md files missing from INVENTORY.md: ' + JSON.stringify(missingFromInventory),
    );
    ...
    assert.deepStrictEqual(
      phantomRows,
      [],
      'INVENTORY.md rows reference agents that have no agents/forge-*.md file: ' + JSON.stringify(phantomRows),
    );
