// allow-test-rule: source-text-is-the-product — agents/forge-intel-updater.md IS
 ...
 *
 * The "Runtime layout detection" block in forge-intel-updater.md ran
 * unconditionally on every project analysed, emitting:
 *
 *   Layout detection returned "unknown" — this project is not a GSD-system
 *   installation (no `.claude/forge-core/` or `.kilo/` runtime root).
 *
 ...
 * repo" check (package.json name === "@forge-ai/forge-core") so it runs ONLY when
...
const AGENT_PATH = path.join(ROOT, 'agents', 'forge-intel-updater.md');
...
describe('bug #3290 — Group A: layout-detection block must be gated or absent', () => {
  ...
  test('agent file exists', () => {
    assert.ok(fs.existsSync(AGENT_PATH), 'agents/forge-intel-updater.md must exist');
...
describe('bug #3290 — Group B: layout-detection verdict has no downstream consumers', () => {
   ...
   * of the forge-intel-updater layout detection (not general runtime references).
   *
  ...
  test('no file contains "Layout detection returned" (the noisy verdict phrase)', () => {
    ...
    assert.strictEqual(
      ...
      'phrase from the forge-intel-updater layout-detection block). Found:\n' +
  ...
  test('no agent or workflow instructs reading the layout-detection verdict output', () => {
    ...
    // the producing file (forge-intel-updater.md).
    ...
    for (const dir of SOURCE_DIRS) {
      ...
      for (const file of files) {
        ...
        if (path.basename(file) === 'forge-intel-updater.md') continue;
        const src = fs.readFileSync(file, 'utf-8');
