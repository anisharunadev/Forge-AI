 ...
 *
 * Validates the /forge-execute-phase --wave feature contract:
...
const COMMAND_PATH = path.join(__dirname, '..', 'commands', 'forge', 'execute-phase.md');
const WORKFLOW_PATH = path.join(__dirname, '..', 'forge-core', 'workflows', 'execute-phase.md');
...
const HELP_PATH = path.join(__dirname, '..', 'forge-core', 'workflows', 'help', 'modes', 'full.md');
...
describe('execute-phase command: --wave flag', () => {
  test('command file exists', () => {
    assert.ok(fs.existsSync(COMMAND_PATH), 'commands/forge/execute-phase.md should exist');
  });
...
describe('execute-phase docs: user-facing wave flag', () => {
  test('COMMANDS.md documents --wave usage', () => {
    ...
    assert.ok(
      content.includes('/forge-execute-phase 1 --wave 2'),
  ...
  test('help workflow documents --wave behavior', () => {
    ...
    assert.ok(
      content.includes('Usage: `/forge:execute-phase 5 --wave 2`') || content.includes('Usage: `/forge-execute-phase 5 --wave 2`'),
...
describe('phase-plan-index: wave grouping behavior', () => {
  test('phase-plan-index groups plans by wave (DAG-bucketing: P002 depends on P001)', () => {
    // allow-test-rule: behavioral — calls forge-tools and asserts structured output
    const fs = require('fs');
    const path = require('path');
  ...
  test('phase-plan-index defaults missing wave frontmatter to wave 1', () => {
    // allow-test-rule: behavioral — exercises forge-tools wave-defaulting logic
    const fs = require('fs');
    const path = require('path');
    const tmpDir = createTempProject();
    try {
...
describe('use_worktrees config: cross-workflow structural coverage', () => {
  const QUICK_PATH = path.join(__dirname, '..', 'forge-core', 'workflows', 'quick.md');
  const DIAGNOSE_PATH = path.join(__dirname, '..', 'forge-core', 'workflows', 'diagnose-issues.md');
  const EXECUTE_PLAN_PATH = path.join(__dirname, '..', 'forge-core', 'workflows', 'execute-plan.md');
  const PLANNING_CONFIG_PATH = path.join(__dirname, '..', 'forge-core', 'references', 'planning-config.md');
