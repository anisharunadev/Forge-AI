...
} = require(path.join(ROOT, 'forge-core', 'bin', 'lib', 'runtime-name-policy.cjs'));
...
describe('runtime-name-policy windsurf alias parity — manifest vs FALLBACK_ALIASES (#792)', () => {
  ...
  const manifestPath = path.join(ROOT, 'forge-core', 'bin', 'shared', 'runtime-aliases.manifest.json');
