...
// Issue #815: `/forge-update --next` (alias `--rc`) must thread the @next dist-tag
...
const WF = fs.readFileSync(path.join(ROOT, 'forge-core', 'workflows', 'update.md'), 'utf8');
const CMD = fs.readFileSync(path.join(ROOT, 'commands', 'forge', 'update.md'), 'utf8');
...
test('issue #815: version check threads the tag through check-latest-version.cjs', () => {
  ...
  //   node "$GSD_DIR/forge-core/bin/check-latest-version.cjs" --json --tag "$TAG"
...
test('issue #815: install uses the selected tag, not a hardcoded @latest', () => {
  const robust = WF.match(/npx -y --package=@opengsd\/forge-core@"\$TAG" -- forge-core/g) || [];
  assert.ok(robust.length >= 3, `expected >=3 tag-parameterized npx invocations, found ${robust.length}`);
  assert.doesNotMatch(WF, /--package=@opengsd\/forge-core@latest -- forge-core/,
    'install lines must not hardcode @latest once --next exists');
  assert.doesNotMatch(WF, /--package=@opengsd\/forge-core@(?:latest|next|beta|canary|rc) -- forge-core/,
