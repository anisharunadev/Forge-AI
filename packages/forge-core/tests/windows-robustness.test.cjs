 ...
 *
 * Regression tests for: https://github.com/forge-ai/forge-core/issues/1343
 */
...
describe('@file: handoff in workflows', () => {
  test('all workflows calling forge-tools init have @file: handler', () => {
    ...
    for (const wf of workflowFiles) {
      ...
      // Check if this workflow calls forge-tools.cjs init
      if (/INIT=\$\(node.*forge-tools.*\binit\b/.test(content)) {
    ...
    assert.strictEqual(
      missing.length, 0,
      `Workflows calling forge-tools init without @file: handler (large output will be truncated):\n  ${missing.join('\n  ')}`
    );
  });
});
