...
const EXECUTE_PHASE = path.join(ROOT, 'forge-core', 'workflows', 'execute-phase.md');
  ...
  return [...content.matchAll(/<step name="([^"]+)"[^>]*>([\s\S]*?)<\/step>/g)]
    .map((match) => {
      ...
      return {
        ...
        readsRuntimeConfig: body.includes('RUNTIME=$(forge_run query config-get runtime --default claude'),
...
describe('#3360 — Codex execute-phase fails closed for unsupported worktree isolation', () => {
  ...
  test('Codex adapter documents that worktree isolation has no direct spawn_agent mapping', () => {
    const header = getCodexSkillAdapterHeader('forge-execute-phase');
