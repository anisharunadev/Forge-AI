 ...
 * /forge:<cmd> command literals because the hook-copy loop in install.js had
 * no /forge: → /forge- rewrite for the claude runtime.
 *
 * Fix: the `.js` branch of the hook-copy loop now applies
 * `content.replace(/forge:/gi, 'forge-')` when
 ...
 *      no user-facing /forge: literals (// comment occurrences exempted).
...
/**
 * Return lines (from nonCommentLines) that contain a user-facing /forge: ref.
 */
function colonRefs(content) {
  return nonCommentLines(content).filter(line => /\/forge:/.test(line));
}
...
describe('bug #376 — prerequisite: hooks/dist is present', () => {
  ...
  test('hooks/dist contains at least one .js hook file with a /forge: literal', () => {
    ...
    assert.ok(
      ...
      'Expected at least one hooks/dist .js file with a non-comment /forge: literal ' +
...
// ---------------------------------------------------------------------------
// Suite 1 — Claude install: no /forge: colon refs in installed .js hook files
// ---------------------------------------------------------------------------
describe('bug #376 — Suite 1: Claude install rewrites /forge: → /forge- in hook .js files', () => {
  let tmpDir;

  before(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-376-claude-'));
  ...
  test('1b: installed .js hook files contain no user-facing /forge: colon refs', () => {
    const hooksDir = path.join(tmpDir, '.claude', 'hooks');
    ...
    assert.deepEqual(
      offenders,
      [],
      'Installed Claude hook .js files must not contain /forge:<cmd> colon refs ' +
      '(non-comment occurrences). The install-time rewriter must replace these with /forge-<cmd>. ' +
      'Offenders: ' + JSON.stringify(offenders, null, 2),
    );
  });

  test('1c: installed .js hook files DO contain the hyphen form /forge- (rewrite happened)', () => {
    const hooksDir = path.join(tmpDir, '.claude', 'hooks');
    ...
    const withHyphen = jsFiles.filter(({ full }) => {
      const content = fs.readFileSync(full, 'utf-8');
      return /\/forge-/.test(content);
    });

    assert.ok(
      withHyphen.length > 0,
      'At least one installed .js hook file must contain /forge- (confirming rewrite ran). ' +
      `Files checked: ${jsFiles.map(f => f.rel).join(', ')}`,
    );
  });
});
...
// ---------------------------------------------------------------------------
// Suite 2 — Cursor install regression: /forge: → /forge- still works (pre-existing)
//
// Note: Cursor installs its own hooks (forge-cursor-session-start.js and
// forge-cursor-post-tool.js) via the cursor-hooks-json installSurface (issue #777).
// It does NOT install the bundled Claude-style hooks/dist files (no forge-session-state.sh
// etc.). The Cursor /forge: rewrite applies in `copyWithPathReplacement` to JS files
// under the agent/skill tree (.cursor/forge-core/*.js etc). We verify that Cursor's
// installed .js files under .cursor/ have no /forge: colon refs, and that the hooks/
// directory contains only the Cursor-specific managed hooks.
// ---------------------------------------------------------------------------
describe('bug #376 — Suite 2: Cursor install still rewrites /forge: → /forge- (regression)', () => {
  let tmpDir;

  before(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-376-cursor-'));
  ...
  test('2b: Cursor-installed .js files contain no user-facing /forge: colon refs', () => {
    const cursorDir = path.join(tmpDir, '.cursor');
    if (!fs.existsSync(cursorDir)) return;

    // Infrastructure files whose /forge: occurrences are intentional implementation
    ...
    // by forge-core/bin/lib/command-roster.cjs on ALL runtimes (including Cursor).
    // It must be installed verbatim and must NOT be content-rewritten: it needs to
    // emit `/forge:${cmd}` for non-Cursor runtimes, and its /forge: strings are internal
    ...
    const offenders = [];
    for (const { rel, full } of jsFiles) {
      // Skip infrastructure files whose /forge: strings are intentional (see above).
    ...
    assert.deepEqual(
      offenders,
      [],
      'Cursor-installed .js files must not contain /forge:<cmd> colon refs. ' +
      'The existing Cursor branch in copyWithPathReplacement must still apply /forge:/gi → forge- rewrite. ' +
      'Offenders: ' + JSON.stringify(offenders, null, 2),
    );
  });

  test('2c: Cursor install creates a hooks/ directory with only Cursor-specific managed hooks', () => {
    // Since issue #777, Cursor installs forge-cursor-session-start.js and
    // forge-cursor-post-tool.js into <configDir>/hooks/. These are Cursor-native
    // hooks — NOT the bundled Claude-style hooks (no forge-session-state.sh etc.).
    ...
    const CLAUDE_BUNDLED_HOOKS = ['forge-session-state.sh', 'forge-context-monitor.js', 'forge-statusline.js'];
    ...
    assert.ok(
      fs.existsSync(path.join(hooksDir, 'forge-cursor-session-start.js')),
      'forge-cursor-session-start.js must be installed in .cursor/hooks/ (#777)',
    );
    assert.ok(
      fs.existsSync(path.join(hooksDir, 'forge-cursor-post-tool.js')),
      'forge-cursor-post-tool.js must be installed in .cursor/hooks/ (#777)',
    );
  });
});
...
describe('bug #376 — Suite 3: hooks/ source files are unchanged by install', () => {
  ...
  test('3a: hooks/dist .js source files still contain /forge: literals (not mutated)', () => {
    ...
    assert.ok(
      withColonRef.length > 0,
      'hooks/dist .js files must still contain /forge: literals after install — ' +
