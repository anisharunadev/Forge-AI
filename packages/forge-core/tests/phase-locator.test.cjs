/**
 * Tests for src/phase-locator.cts (compiled to forge-core/bin/lib/phase-locator.cjs).
 *
...
const phaseLocator = require('../forge-core/bin/lib/phase-locator.cjs');
...
describe('findPhaseInternal: active phase lookup', () => {
  ...
  test('returns null for falsy phase argument', () => {
    tmpDir = createTempProject('forge-pl-test-');
  ...
  test('returns null when phases dir does not exist', () => {
    ...
    tmpDir = fs.mkdtempSync(path.join(require('node:os').tmpdir(), 'forge-pl-test-'));
