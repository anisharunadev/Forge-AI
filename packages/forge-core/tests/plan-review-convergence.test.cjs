/**
 * Tests for forge:plan-review-convergence command (#2306)
 *
...
const COMMAND_PATH = path.join(__dirname, '..', 'commands', 'forge', 'plan-review-convergence.md');
const WORKFLOW_PATH = path.join(__dirname, '..', 'forge-core', 'workflows', 'plan-review-convergence.md');
const CONFIG_DOC_PATH = path.join(__dirname, '..', 'docs', 'CONFIGURATION.md');
const PLAN_PHASE_PATH = path.join(__dirname, '..', 'forge-core', 'workflows', 'plan-phase.md');
const PLANNER_REVIEWS_PATH = path.join(__dirname, '..', 'forge-core', 'references', 'planner-reviews.md');
const PLAN_CHECKER_PATH = path.join(__dirname, '..', 'agents', 'forge-plan-checker.md');
...
describe('plan-review-convergence command source (#2306)', () => {
  ...
  test('command name uses forge: prefix (installer converts to forge- on install)', () => {
    assert.ok(
      command.includes('name: forge:plan-review-convergence'),
      'command name must use forge: prefix so installer converts it to forge-plan-review-convergence'
    );
  });
  ...
  test('command references the workflow file via execution_context', () => {
    assert.ok(
      command.includes('@$HOME/.claude/forge-core/workflows/plan-review-convergence.md'),
  ...
  test('command declares Skill in allowed-tools (required for inline plan-phase invocations)', () => {
    assert.ok(
      ...
      'Skill must be in allowed-tools — command invokes forge-plan-phase inline via Skill() at depth 0 (#936 fix)'
    );
  });
...
describe('plan-review-convergence workflow: initialization (#2306)', () => {
  ...
  test('workflow calls forge-tools.cjs init plan-phase for initialization', () => {
    assert.ok(
      workflow.includes('forge-tools.cjs') && workflow.includes('init') && workflow.includes('plan-phase'),
      'workflow must initialize via forge-tools.cjs init plan-phase'
    );
  });
...
describe('plan-review-convergence workflow: config gate (#2306-v2)', () => {
  ...
  test('workflow exits with enable instructions when config key is false', () => {
    ...
    assert.ok(
      workflow.includes('forge config-set workflow.plan_review_convergence true') ||
...
describe('plan-review-convergence workflow: initial planning gate (#2306)', () => {
  ...
  test('workflow runs forge-plan-phase when no plans exist', () => {
    assert.ok(
      workflow.includes('forge-plan-phase'),
      'workflow must invoke forge-plan-phase when no plans exist'
    );
  });
...
describe('plan-review-convergence workflow: convergence loop (#2306)', () => {
  ...
  test('workflow spawns isolated review agent each cycle', () => {
    assert.ok(
      workflow.includes('forge-review'),
      'workflow must spawn Agent → forge-review each cycle'
    );
  });
  ...
  test('review agent spawn forwards --ws via GSD_WS (symmetric with replan agent)', () => {
    ...
    const reviewAgentBlock = workflow.match(/forge-review['"`,\s][\s\S]{0,300}?GSD_WS/);
    assert.ok(
      reviewAgentBlock ||
      (workflow.includes("'forge-review'") && workflow.includes('{GSD_WS}') &&
       workflow.indexOf('{GSD_WS}') < workflow.indexOf("'forge-plan-phase'")),
  ...
  test('workflow updates STATE.md on convergence', () => {
    assert.ok(
      ...
      'workflow must update STATE.md via forge-tools.cjs when converged'
    );
  });
  ...
  test('workflow invokes inline replan with --reviews flag', () => {
    assert.ok(
      ...
      'inline replan must pass --reviews so forge-plan-phase incorporates review feedback'
    );
  });
...
describe('plan-review-convergence config schema registration (#2306-v2)', () => {
  ...
  const { VALID_CONFIG_KEYS } = require('../forge-core/bin/lib/config-schema.cjs');
  ...
  test('workflow.plan_review_convergence is registered in config-schema.cjs', () => {
    assert.ok(
      ...
      "workflow.plan_review_convergence must be registered in VALID_CONFIG_KEYS in config-schema.cjs so forge config-set accepts it (#2306-v2)"
    );
  });
});
...
describe('plan-review-convergence reviews-mode incorporation contract (#724)', () => {
  ...
  test('plan-phase planner prompt says REVIEWS.md is feedback input, not the execution contract', () => {
    assert.ok(
      planPhase.includes('<review_incorporation_contract>') &&
        ...
        planPhase.includes('/forge:execute-phase primarily consumes PLAN.md'),
  ...
  test('planner reviews reference requires actionable findings to appear in PLAN.md or be deferred there', () => {
    assert.ok(
      plannerReviews.includes('/forge:execute-phase primarily consumes PLAN.md') &&
  ...
  test('forge-plan-checker has a Review Incorporation dimension for reviews mode', () => {
    assert.ok(
...
describe('plan-review-convergence local model config schema registration (#2306-local)', () => {
  ...
  test('review.ollama_host is registered in config-schema.cjs', () => {
    assert.ok(
      ...
      "review.ollama_host must be in VALID_CONFIG_KEYS so forge config-set accepts it"
    );
  });
  ...
  test('review.lm_studio_host is registered in config-schema.cjs', () => {
    assert.ok(
      ...
      "review.lm_studio_host must be in VALID_CONFIG_KEYS so forge config-set accepts it"
    );
  });
  ...
  test('review.llama_cpp_host is registered in config-schema.cjs', () => {
    assert.ok(
      ...
      "review.llama_cpp_host must be in VALID_CONFIG_KEYS so forge config-set accepts it"
    );
  });
});
...
// On Claude Code a depth-1 Agent has no Agent tool, so forge-plan-phase wrapped in
// Agent() cannot spawn forge-planner / forge-plan-checker → the replan loop breaks.
...
describe('plan-review-convergence workflow: inline plan-phase dispatch (#936)', () => {
  ...
  test('initial planning does NOT wrap forge-plan-phase inside Agent() (#936 fix)', () => {
    // Pre-fix: Agent( ... Skill('forge-plan-phase') ... ) in step 4
    // Post-fix: bare Skill(skill="forge-plan-phase") at orchestrator level
    const blocks = extractAgentBlocks(workflow);
    const wrapping = blocks.filter((b) =>
      /Skill\(\s*skill=['"]forge-plan-phase['"]/.test(b.blockText)
    );
    assert.deepStrictEqual(
      wrapping.map((b) => b.blockText.slice(0, 80).replace(/\n/g, '\\n')),
      [],
      'Initial planning must NOT wrap forge-plan-phase inside Agent() — run it inline so ' +
      'it can spawn forge-planner/forge-plan-checker at depth 1. See: bug #936'
    );
  });

  test('replan step does NOT wrap forge-plan-phase inside Agent() (#936 fix)', () => {
    ...
    assert.deepStrictEqual(
      wrapping.map((b) => b.blockText.slice(0, 80).replace(/\n/g, '\\n')),
      [],
      'Replan step must NOT wrap forge-plan-phase inside Agent() — the replan loop can ' +
      'never produce a plan on Claude Code when plan-phase is at depth 1. See: bug #936'
    );
  });

  test('workflow calls forge-plan-phase inline (bare Skill outside Agent block) (#936 fix)', () => {
    // After the fix there must be at least one bare Skill(skill="forge-plan-phase")
    ...
    assert.ok(
      /Skill\(\s*skill=["']forge-plan-phase["']/.test(masked),
      'plan-review-convergence must contain at least one bare Skill(skill="forge-plan-phase") ' +
  ...
  test('success_criteria describes inline plan-phase, not Agent → Skill (#936 fix)', () => {
    ...
    assert.ok(
      !successBlock.includes('via Agent → Skill("forge-plan-phase")'),
