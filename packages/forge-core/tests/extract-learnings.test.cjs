...
const COMMAND_PATH = path.join(__dirname, '..', 'commands', 'forge', 'extract-learnings.md');
const WORKFLOW_PATH = path.join(__dirname, '..', 'forge-core', 'workflows', 'extract-learnings.md');

describe('extract-learnings command', () => {
  test('command file exists', () => {
    assert.ok(fs.existsSync(COMMAND_PATH), 'commands/forge/extract-learnings.md should exist');
  });

  test('command file has correct name frontmatter', () => {
    const content = fs.readFileSync(COMMAND_PATH, 'utf-8');
    assert.ok(content.includes('name: forge:extract-learnings'), 'Command must have name: forge:extract-learnings');
  });
