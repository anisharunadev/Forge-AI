...
/**
 * Regression test for #3197 — forge-tools config-set rejects workflow._auto_chain_active.
 *
 * Root cause: RUNTIME_STATE_KEYS was added to sdk/src/query/config-schema.ts in #3162
 * but not to forge-core/bin/lib/config-schema.cjs, so forge-tools.cjs users still hit
...
describe('#3197 — forge-tools.cjs config-set workflow._auto_chain_active', () => {
  test('config-set workflow._auto_chain_active true succeeds via forge-tools.cjs (CJS path)', (t) => {
