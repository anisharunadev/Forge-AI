...
const PLAN_PHASE = path.join(__dirname, '..', 'forge-core', 'workflows', 'plan-phase.md');
const VERIFY_PHASE = path.join(__dirname, '..', 'forge-core', 'workflows', 'verify-phase.md');
const SCHEMA_MANIFEST_JSON = path.join(__dirname, '..', 'forge-core', 'bin', 'shared', 'config-schema.manifest.json');
...
describe('plan-phase decision-coverage gate (#2492)', () => {
  ...
  test('invokes the check.decision-coverage-plan handler', () => {
    assert.ok(
      ...
      'plan-phase.md must call forge-sdk query check.decision-coverage-plan',
    );
  });
...
describe('verify-phase decision-coverage gate (#2492)', () => {
  ...
  test('invokes the check.decision-coverage-verify handler', () => {
    assert.ok(
      ...
      'verify-phase.md must call forge-sdk query check.decision-coverage-verify',
    );
  });
