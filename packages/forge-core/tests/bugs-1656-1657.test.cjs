...
describe('#1656: community .sh hooks must be present in hooks/dist', () => {
  ...
  test('forge-session-state.sh exists in hooks/dist', () => {
    const p = path.join(HOOKS_DIST, 'forge-session-state.sh');
    assert.ok(fs.existsSync(p), 'forge-session-state.sh must be in hooks/dist/ so the installer can copy it');
  });

  test('forge-validate-commit.sh exists in hooks/dist', () => {
    const p = path.join(HOOKS_DIST, 'forge-validate-commit.sh');
    assert.ok(fs.existsSync(p), 'forge-validate-commit.sh must be in hooks/dist/ so the installer can copy it');
  });

  test('forge-phase-boundary.sh exists in hooks/dist', () => {
    const p = path.join(HOOKS_DIST, 'forge-phase-boundary.sh');
    assert.ok(fs.existsSync(p), 'forge-phase-boundary.sh must be in hooks/dist/ so the installer can copy it');
  });
});
