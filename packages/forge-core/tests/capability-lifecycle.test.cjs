...
const lifecycle = require('../forge-core/bin/lib/capability-lifecycle.cjs');
const ledgerMod = require('../forge-core/bin/lib/capability-ledger.cjs');
...
test('install: a reserved-namespace capability is blocked', async () => {
  ...
  const res = await lifecycle.installCapability('./x', {
    runtimeDir: dir, hostVersion: '1.6.0', _resolve: fakeResolve(declarativeCap('forge-evil')),
  });
  assert.strictEqual(res.status, 'blocked');
  assert.ok(res.blockReasons.some((r) => /reserved namespace/.test(r)));
  assert.ok(!fs.existsSync(path.join(dir, '.gsd', 'capabilities', 'forge-evil')));
});
...
test('integration: engines-incompatible local cap is blocked by the lifecycle, not the resolver throw (skipEnginesGate)', async () => {
  ...
  assert.ok(res.blockReasons.some((r) => /engines\.forge/.test(r)));
...
function recordPending(dir, id, version, { kind = 'upgrade', backupName, sharedFiles = [], sharedEdits = [] }) {
  ledgerMod.recordInstall(dir, {
    id, version, source: 's', integrity: '', files: ['.forge/capabilities/' + id], sharedEdits,
...
test('reconcile: committed leftover backup (no intent) rolls forward, dropping the backup', () => {
  ...
  ledgerMod.recordInstall(dir, { id: 'c', version: '2.0.0', source: 's', integrity: '', files: ['.forge/capabilities/c'], sharedEdits: [] });
  const report = lifecycle.reconcileCapabilities({ runtimeDir: dir });
    ...
    JSON.stringify({
      ...
      entries: {
        '../../precious': {
          ...
          files: ['.forge/capabilities/x'], sharedEdits: [],
...
test('remove: a held lock makes remove report "in progress" (does not mutate)', () => {
  ...
  const lockPath = path.join(dir, '.gsd', 'capabilities', '.lock');
  fs.mkdirSync(path.dirname(lockPath), { recursive: true });
  ledgerMod.recordInstall(dir, { id: 'q', version: '1.0.0', source: 's', integrity: '', files: ['.forge/capabilities/q'], sharedEdits: [] });
...
test('remove: a tampered ledger files[] routed through a symlink cannot delete outside runtimeDir', () => {
  ...
  ledgerMod.recordInstall(dir, {
    ...
    files: ['.forge/link/victim.txt'], sharedEdits: [],
  });
...
// On a corrupt ledger it must NOT create .forge/capabilities and must NOT create a .lock.
// ---------------------------------------------------------------------------

test('finding-2: removeCapability on a corrupt ledger does NOT acquire a lock or create .forge/capabilities (preflight precedes acquireLock)', () => {
  const dir = runtime();
  const ledgerPath = path.join(dir, ledgerMod.LEDGER_FILE_NAME);
  fs.mkdirSync(dir, { recursive: true });
  ...
  assert.ok(!fs.existsSync(capsRoot), 'precondition: .forge/capabilities must not exist yet');
  ...
  assert.ok(!fs.existsSync(capsRoot),
    '.forge/capabilities must NOT be created by acquireLock when the corruption preflight blocks first');
...
// On a corrupt ledger it must warn WITHOUT creating .forge/capabilities or a .lock.
// ---------------------------------------------------------------------------

test('finding-2: reconcileCapabilities on a corrupt ledger does NOT acquire a lock or create .forge/capabilities (preflight precedes acquireLock)', () => {
  const dir = runtime();
  fs.mkdirSync(dir, { recursive: true });
  fs.writeFileSync(path.join(dir, ledgerMod.LEDGER_FILE_NAME), '{ broken json ---');
  ...
  assert.ok(!fs.existsSync(capsRoot),
    '.forge/capabilities must NOT be created by acquireLock when the corruption preflight warns first');
  assert.ok(!fs.existsSync(path.join(capsRoot, '.lock')),
  ...
  const trackingResolve = async (spec, opts) => {
    ...
    fs.writeFileSync(path.join(staged, 'capability.json'), JSON.stringify(
      { id: 'pf', role: 'feature', version: '1.0.0', title: 'pf', description: 'x',
        tier: 'standard', requires: [], engines: { forge: '>=1.0.0' },
        runtimeCompat: { supported: ['*'], unsupported: [] },
...
test('root-fix-3: installCapability rejects unsafe id (constructor) before staging/promotion', async (_t) => {
    ...
    _resolve: async (spec, opts) => {
      ...
      fs.writeFileSync(path.join(staged, 'capability.json'), JSON.stringify(
        { id: 'constructor', role: 'feature', version: '1.0.0', title: 'evil',
          description: 'evil', tier: 'standard', requires: [], engines: { forge: '>=1.0.0' },
          runtimeCompat: { supported: ['*'], unsupported: [] },
          skills: [], agents: [], hooks: [], config: {}, steps: [], contributions: [], gates: [] }
      ), 'utf8');
  ...
  assert.ok(result.blockReasons && result.blockReasons.length > 0, 'must have blockReasons');
  // Must NOT have installed a bundle at .forge/capabilities/constructor
  assert.ok(
    !fs.existsSync(path.join(dir, '.gsd', 'capabilities', 'constructor')),
    'no .forge/capabilities/constructor bundle must be promoted',
  );
...
test('root-fix-3: upgradeCapability rejects unsafe id (constructor) before staging/promotion', async (_t) => {
  ...
  assert.ok(!fs.existsSync(path.join(dir, '.gsd', 'capabilities', 'constructor')),
    'no .forge/capabilities/constructor bundle must be promoted on upgrade');
});
...
const consentMod = require('../forge-core/bin/lib/capability-consent.cjs');
const trustMod = require('../forge-core/bin/lib/capability-trust.cjs');
 ...
 * content hash over the INSTALLED capDir (`<runtimeDir>/.forge/capabilities/<id>`) — exactly what the
...
const { loadRegistry } = require('../forge-core/bin/lib/capability-loader.cjs');
...
function plantEntry(dir, id, version, source) {
  ledgerMod.recordInstall(dir, {
    ...
    files: [`.forge/capabilities/${id}`], sharedEdits: [],
  });
}
