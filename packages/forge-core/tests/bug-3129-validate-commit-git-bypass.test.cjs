...
//
// forge-validate-commit.sh used `[[ "$CMD" =~ ^git[[:space:]]+commit ]]` to
...
// ── forge-validate-commit.sh source check ──────────────────────────────────────

describe('forge-validate-commit.sh delegates to git-cmd.js', () => {
  const hookSrc = fs.readFileSync(
    path.join(ROOT, 'hooks', 'forge-validate-commit.sh'), 'utf8',
  );

  test('hook no longer uses the stale ^git\\s+commit bash regex', () => {
    assert.ok(
      !hookSrc.includes('^git[[:space:]]+commit'),
      'forge-validate-commit.sh still uses the bypassed regex — fix not applied',
    );
  });
  ...
  test('hook delegates to git-cmd.js isGitSubcommand', () => {
    assert.ok(
      ...
      'forge-validate-commit.sh does not reference git-cmd.js or isGitSubcommand',
    );
  });
