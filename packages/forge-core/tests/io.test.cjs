/**
 * Tests for src/io.cts (compiled to forge-core/bin/lib/io.cjs).
 *
...
const io = require('../forge-core/bin/lib/io.cjs');
...
describe('output()', () => {
  const ioPath = path.resolve(__dirname, '../forge-core/bin/lib/io.cjs');
...
describe('GSD_TEMP_DIR', () => {
  test('resolves to <tmpdir>/gsd', () => {
    assert.strictEqual(io.GSD_TEMP_DIR, path.join(os.tmpdir(), 'forge'));
  });
});

describe('reapStaleTempFiles (via io)', () => {
  const TEST_PREFIX = 'forge-io-test-';
  ...
  test('does not throw when GSD_TEMP_DIR does not exist yet', () => {
    ...
    assert.doesNotThrow(() => {
      io.reapStaleTempFiles('forge-io-nonexistent-prefix-xyz-', { maxAgeMs: 0 });
    });
  });
});
