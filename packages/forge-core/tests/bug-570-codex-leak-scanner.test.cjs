 ...
 *   Fix: scan only files listed in forge-file-manifest.json.
 *
...
describe('#570 — Codex leak scanner sub-bugs', { concurrency: false }, () => {
  ...
  beforeEach(() => {
    ...
    tmpRoot = createTempDir('forge-570-');
  ...
  test('convertClaudeToCodexMarkdown replaces bare ~/.claude (no trailing slash)', () => {
    ...
    const out = convertClaudeCommandToCodexSkill(input, 'forge-test');
  ...
  test('writeManifest includes .toml agent files for Codex', () => {
    ...
    const tomlFiles = fs.existsSync(agentsDir)
      ? fs.readdirSync(agentsDir).filter((f) => f.startsWith('forge-') && f.endsWith('.toml'))
      : [];
    assert.ok(
      tomlFiles.length > 0,
      `Precondition: Codex install must write at least one forge-*.toml in agents/; found none in ${agentsDir}`,
    );

    const manifestPath = path.join(codexHome, 'forge-file-manifest.json');
    assert.ok(
      fs.existsSync(manifestPath),
      `forge-file-manifest.json must exist after install; not found at ${manifestPath}`,
    );
    ...
    const tomlManifestKeys = manifestKeys.filter(
      (k) => k.startsWith('agents/forge-') && k.endsWith('.toml'),
    );
    assert.ok(
      tomlManifestKeys.length > 0,
      `Expected at least one 'agents/forge-*.toml' key in manifest.files, but found none.\n` +
