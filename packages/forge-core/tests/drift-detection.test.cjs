...
describe('last_mapped_commit frontmatter', () => {
  let tmp;
  beforeEach(() => {
    tmp = createTempProject('forge-drift-');
...
describe('config-set drift validation via CLI', () => {
  let tmp;
  beforeEach(() => {
    tmp = createTempGitProject('forge-drift-cfg-');
  });
  afterEach(() => cleanup(tmp));
...
describe('forge-codebase-mapper --paths flag', () => {
  test('agent doc mentions --paths', () => {
    const doc = fs.readFileSync(
      path.join(__dirname, '..', 'agents', 'forge-codebase-mapper.md'),
      'utf8',
    );
...
// dispatches capability gates via `forge_run loop render-hooks execute:wave:post`,
...
describe('verify codebase-drift CLI', () => {
  let tmp;
  beforeEach(() => {
    tmp = createTempGitProject('forge-drift-cli-');
    fs.mkdirSync(path.join(tmp, '.planning', 'codebase'), { recursive: true });
  });
  afterEach(() => cleanup(tmp));
  ...
  test('never exits non-zero when git repo is missing (non-blocking)', () => {
    const nonGit = createTempProject('forge-drift-nongit-');
    try {
...
describe('verify codebase-drift — workflow config read from nested shape (#1493)', () => {
  let tmp;
  beforeEach(() => {
    tmp = createTempGitProject('forge-drift-1493-');
    fs.mkdirSync(path.join(tmp, '.planning', 'codebase'), { recursive: true });
  });
  afterEach(() => cleanup(tmp));
