...
describe('current-timestamp command', () => {
  ...
  test('dispatches directly to CJS handler (no SDK bridge) to avoid Windows native crash path', () => {
    const sourcePath = path.join(__dirname, '..', 'forge-core', 'bin', 'forge-tools.cjs');
    const source = fs.readFileSync(sourcePath, 'utf8');
    const match = source.match(/case 'current-timestamp':\s*\{[\s\S]*?\r?\n\s*break;\r?\n\s*\}/);

    assert.ok(match, 'current-timestamp case block must exist in forge-tools.cjs');
...
describe('resolve-model command', () => {
  ...
  test('known agent returns model and profile without unknown_agent', () => {
    const result = runGsdTools('resolve-model forge-planner', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('shipped-but-previously-missing agent resolves under quality profile (#3229)', () => {
    ...
    const result = runGsdTools('resolve-model forge-code-reviewer', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
    const output = JSON.parse(result.output);
  ...
  test('default profile fallback when no config exists', () => {
    ...
    const result = runGsdTools('resolve-model forge-executor', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('emits unified effort (not reasoning_effort) when runtime supports tiered effort', () => {
    ...
    // #443: effort is now the unified field (xhigh for forge-planner heavy tier default)
  ...
  test('does not include reasoning_effort for per-agent model_overrides (effort present instead)', () => {
    fs.writeFileSync(path.join(tmpDir, '.planning', 'config.json'), JSON.stringify({
      ...
      runtime: 'codex',
      models: { planning: 'opus' },
      model_overrides: { 'forge-planner': 'gpt-5.5' },
    }));
    const result = runGsdTools('resolve-model forge-planner', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('commit command', () => {
  ...
  test('creates strategy branch before first commit when branching_strategy is milestone', () => {
    ...
    assert.strictEqual(branch, 'forge/v1.0-initial-release', 'should be on milestone branch');
  });
  ...
  test('creates strategy branch before first commit when branching_strategy is phase', () => {
    ...
    const { execFileSync } = require('child_process');
    const branch = execFileSync('git', ['rev-parse', '--abbrev-ref', 'HEAD'], { cwd: tmpDir, encoding: 'utf-8' }).trim();
    assert.strictEqual(branch, 'forge/phase-01-setup', 'should be on phase branch');
  });
  ...
  test('decimal phase numbers are captured correctly in branching strategy', () => {
    ...
    const { execFileSync } = require('child_process');
    const branch = execFileSync('git', ['rev-parse', '--abbrev-ref', 'HEAD'], { cwd: tmpDir, encoding: 'utf-8' }).trim();
    assert.strictEqual(branch, 'forge/phase-45.14-golden-capture', 'should be on decimal phase branch, not integer-only');
  });
});
...
describe('groupFilesBySubrepo (#311)', () => {
  const { groupFilesBySubrepo } = require('../forge-core/bin/lib/commands.cjs');
...
describe('websearch command', () => {
  const { cmdWebsearch } = require('../forge-core/bin/lib/commands.cjs');
  let origFetch;
  let origApiKey;
  let origWriteSync;
  let captured;
...
describe('_wsParseRetryAfter (#308)', () => {
  const { _wsParseRetryAfter } = require('../forge-core/bin/lib/commands.cjs');
...
describe('pr-subrepo', () => {
  ...
  describe('regressions (#666 — cmdPrSubrepo seam)', () => {
    ...
    beforeEach(() => {
      rootDir = createTempDir('forge-666-root-');
  ...
  describe('workflow source invariants (#666 — pr-branch.md)', () => {
    ...
    const workflowPath = path.resolve(__dirname, '..', 'forge-core', 'workflows', 'pr-branch.md');
    let wfContent;
    ...
    test('delegates git work to forge_run query pr-subrepo — no inline git add -A in code', () => {
      wfContent = wfContent || fs.readFileSync(workflowPath, 'utf-8');
    ...
    test('dirty-scan rejects traversal, newline, and symlink entries before invoking git (security)', () => {
      ...
      const scanRoot = createTempDir('forge-666-scan-root-');
      const outsideDir = createTempDir('forge-666-scan-outside-');
      ...
      const traversalEntry = path.relative(scanRoot, outsideDir); // e.g. "../forge-666-scan-outside-XXXX"
