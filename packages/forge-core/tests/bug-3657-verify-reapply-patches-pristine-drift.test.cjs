...
/**
 * Bug #3657: verify-reapply-patches false-fails when forge-pristine/ snapshot is
...
const SCRIPT = path.join(ROOT, 'forge-core', 'bin', 'verify-reapply-patches.cjs');
...
before(() => {
  tmpRoot    = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-3657-'));
...
describe('Bug #3657: pristine-drift does not produce false FAIL_USER_LINES_MISSING', () => {
  ...
  test('exits 0 with reason=OK_PRISTINE_DRIFT_DETECTED when on-disk pristine hash does not match recorded hash', () => {
    ...
    const FILE = 'agents/forge-executor.md';
    ...
    // The installer later refreshed forge-pristine/ to v_new.
  ...
  test('handles mixed drift + real-failure across multiple files independently', () => {
    ...
    const DRIFT_FILE  = 'agents/forge-executor.md';
  ...
  test('Finding 1: drifted count and drifted_files aggregate correctly across multiple drifted files', () => {
    ...
    const FILE_A = 'agents/forge-executor.md';
  ...
  test('Finding 2: workflow Step 5a source contains drift-check section for DRIFTED_COUNT gate', () => {
    const workflowPath = path.join(ROOT, 'forge-core', 'workflows', 'reapply-patches.md');
...
describe('Bug #934: OK_NO_BASELINE when recordedHash present but pristine file absent', () => {
   ...
   * the forge-pristine/ snapshot is absent from disk (the installer's
  ...
  test('exits 0 with reason=OK_NO_BASELINE when recordedHash present but pristine absent', () => {
    ...
    const FILE = 'forge-core/workflows/execute-phase.md';
    ...
    const backupContent =
      ...
      'another upstream line removed upstream between forge-core releases here\n' +
    ...
    const installedContent =
      'brand-new upstream line that replaced the old content in forge-core 1.4.2\n' +
      'model: sonnet in frontmatter — this is the real user customisation line\n';

    // backup-meta.json records a hash (modern installer) but forge-pristine/ is absent.
  ...
  test('falls through to over-broad mode when pristine absent AND no recordedHash', () => {
    ...
    const FILE = 'forge-core/workflows/plan-phase.md';
