/**
 * /forge mvp-phase command — frontmatter contract test
...
const CMD = path.join(__dirname, '..', 'commands', 'forge', 'mvp-phase.md');
...
describe('/forge mvp-phase command frontmatter', () => {
  ...
  test('frontmatter declares correct command name', () => {
    ...
    assert.equal(contract.name, 'forge:mvp-phase');
  });
