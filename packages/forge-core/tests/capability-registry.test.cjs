...
const { LOOP_HOST_CONTRACT } = require('../forge-core/bin/lib/loop-host-contract.cjs');
...
const capValidatorModule = require('../forge-core/bin/lib/capability-validator.cjs');
...
describe('UI pilot capability', () => {
  ...
  test('UI pilot generates a registry with correct shape', () => {
    ...
    // byAgent maps forge-ui-checker and forge-ui-auditor to 'ui'
    assert.strictEqual(registry.byAgent['forge-ui-checker'], 'ui');
    assert.strictEqual(registry.byAgent['forge-ui-auditor'], 'ui');
      ...
      gates: [
        {
          ...
          check: { agentVerdict: { ref: 'forge-ui-checker', prompt: 'check' } },
          blocking: true,
          onError: 'halt',
        },
      ],
...
describe('validateCrossCapability adversarial cases', () => {
  ...
  test('tier-monotone violation: core requires full rejected', () => {
    const coreCap = {
      ...
      skills: ['core-skill'], agents: ['forge-core-agent'], hooks: [], config: {},
      steps: [], contributions: [], gates: [],
    };
    const fullCap = {
      id: 'full-cap', role: 'feature', title: 'Full', tier: 'full', requires: [],
      skills: ['full-skill'], agents: ['forge-full-agent'], hooks: [], config: {},
      steps: [], contributions: [], gates: [],
    };
  ...
  test('config key owned by two capabilities rejected', () => {
    ...
    const cap2 = {
      ...
      skills: ['other-skill'], agents: ['forge-other-agent'], hooks: [],
...
const REGISTRY_PATH = path.join(ROOT, 'forge-core', 'bin', 'lib', 'capability-registry.cjs');
...
describe('committed forge-core/bin/lib/capability-registry.cjs is not stale', () => {
...
describe('registry structure', () => {
  ...
  test('requiresClosure works for a cap with transitive requires', () => {
    const capA = {
      ...
      skills: ['a-skill'], agents: ['forge-a-agent'], hooks: [], config: {},
      steps: [], contributions: [], gates: [],
    };
    const capB = {
      id: 'cap-b', role: 'feature', title: 'B', tier: 'standard', requires: ['cap-c'],
      skills: ['b-skill'], agents: ['forge-b-agent'], hooks: [], config: {},
      steps: [], contributions: [], gates: [],
    };
    const capC = {
      id: 'cap-c', role: 'feature', title: 'C', tier: 'standard', requires: [],
      skills: ['c-skill'], agents: ['forge-c-agent'], hooks: [], config: {},
      steps: [], contributions: [], gates: [],
    };
...
describe('ADR-857 phase 6 planning feature capabilities', () => {
  const realRegistry = require('../forge-core/bin/lib/capability-registry.cjs');
  ...
  test('step.ref with both skill and agent is rejected', () => {
    const cap = {
      ...
      steps: [
        {
          point: 'plan:pre',
          ref: { skill: 'ui-phase', agent: 'forge-ui-checker' },  // BOTH keys — invalid
          produces: ['UI-SPEC.md'],
          consumes: ['CONTEXT.md'],
          when: 'workflow.ui_phase',
          onError: 'skip',
...
describe('double-prefix guard: step.ref.skill must not start with "forge-"', () => {
  // ref.skill is an unprefixed stem (e.g. "ui-review"). Workflow dispatch prepends
  // "forge-" at runtime. A stem already starting with "forge-" would produce "gsd-gsd-..."
  // at dispatch time, silently invoking a non-existent skill.

  test('ref.skill starting with "forge-" is rejected', () => {
    const cap = {
      ...UI_CAP,
      steps: [
        {
          point: 'verify:post',
          ref: { skill: 'forge-ui-review' },  // wrong: stem must NOT have forge- prefix
          produces: ['UI-REVIEW.md'],
          consumes: ['UI-SPEC.md'],
          when: 'workflow.ui_review',
          onError: 'skip',
    ...
    const errors = validateCapability(cap, 'ui');
    assert.ok(errors.length > 0, 'Expected an error for forge-prefixed ref.skill');
    assert.ok(
      errors.some((e) => e.includes('forge-') && (e.includes('double') || e.includes('unprefixed') || e.includes('must not start'))),
      'Error should mention the double-prefix problem, got: ' + JSON.stringify(errors),
    );
  });

  test('ref.skill without "forge-" prefix is accepted (stem only)', () => {
    const cap = {
      ...UI_CAP,
      steps: [
        {
    ...
    const prefixErrors = validateCapability(cap, 'ui').filter((e) => e.includes('forge-') && e.includes('stem'));
...
describe('ref membership check: step.ref.agent must be in cap.agents', () => {
  // A capability declares agents: ["forge-ui-checker", "forge-ui-auditor"].
  ...
  test('step.ref.agent NOT in cap.agents is rejected', () => {
    const cap = {
      ...
      steps: [
        {
          point: 'plan:pre',
          ref: { agent: 'forge-unknown-agent' },  // not in agents: ["forge-ui-checker", "forge-ui-auditor"]
          produces: ['UI-SPEC.md'],
          consumes: ['CONTEXT.md'],
          when: 'workflow.ui_phase',
          onError: 'skip',
    ...
    assert.ok(
      errors.some((e) => e.includes('forge-unknown-agent') && e.includes('not declared')),
  ...
  test('step.ref.agent IN cap.agents is accepted', () => {
    const cap = {
      ...
      steps: [
        {
          point: 'plan:pre',
          ref: { agent: 'forge-ui-checker' },  // declared in agents
          produces: ['UI-SPEC.md'],
          consumes: ['CONTEXT.md'],
          when: 'workflow.ui_phase',
          onError: 'skip',
    ...
    const errors = validateCapability(cap, 'ui');
    const membershipErrors = errors.filter((e) => e.includes('not declared') && e.includes('forge-ui-checker'));
    assert.deepEqual(
      membershipErrors, [],
...
describe('Fix: 3-node requires cycle (A→B→C→A) is detected', () => {
  test('three-node requires cycle is reported as an error', () => {
    const capA = {
      ...
      skills: ['cyc-a-skill'], agents: ['forge-cyc-a'], hooks: [], config: {},
      steps: [], contributions: [], gates: [],
    };
    const capB = {
      id: 'cyc-b', role: 'feature', title: 'CycB', tier: 'standard', requires: ['cyc-c'],
      skills: ['cyc-b-skill'], agents: ['forge-cyc-b'], hooks: [], config: {},
      steps: [], contributions: [], gates: [],
    };
    const capC = {
      id: 'cyc-c', role: 'feature', title: 'CycC', tier: 'standard', requires: ['cyc-a'],
      skills: ['cyc-c-skill'], agents: ['forge-cyc-c'], hooks: [], config: {},
      steps: [], contributions: [], gates: [],
    };
  ...
  test('agentVerdict gate with blocking:false generates zero errors', () => {
    ...
    const cap = {
      ...
      gates: [
        {
          point: 'execute:wave:post',
          check: { agentVerdict: { ref: 'forge-ui-checker', prompt: 'check ui' } },
...
describe('S1: fragment.path traversal guard', () => {
  ...
  test('fragment.path is materialized into inline registry content', (t) => {
    const capsDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-cap-fragment-'));
  ...
  test('step fragment.path is materialized into inline registry content', (t) => {
    const capsDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-cap-step-fragment-'));
    t.after(() => cleanup(capsDir));
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        runtimeCompat: { supported: ['*'], unsupported: [] },
        skills: [],
        agents: ['forge-phase-researcher'],
        hooks: [],
        config: {},
        steps: [{
          point: 'plan:pre',
          ref: { agent: 'forge-phase-researcher' },
          fragment: { path: 'fragments/plan-pre.md' },
...
describe('FIX 1: self-consume rejection in validateConsumesGlobal', () => {
  test('a step produces:["SELF.md"] and consumes:["SELF.md"] with no other producer is rejected', () => {
    const cap = {
      ...
      tier: 'standard', requires: [],
      skills: ['self-skill'], agents: ['forge-self-agent'], hooks: [], config: {},
      steps: [
        {
          point: 'plan:pre',
  ...
  test('a step produces:["SELF.md"] and consumes:["SELF.md"] but another capability produces SELF.md at an earlier point is accepted', () => {
    const producerCap = {
      ...
      tier: 'standard', requires: [],
      skills: ['producer-skill'], agents: ['forge-producer-agent'], hooks: [], config: {},
      steps: [
        {
...
describe('configSchema emission (ADR-857 phase 3b)', () => {
  ...
  test('committed capability-registry.cjs has configSchema with correct shape', () => {
    const registry = require('../forge-core/bin/lib/capability-registry.cjs');
...
describe('ADR-857 phase 4a: pending-reconciliation warnings (SOFT gate)', () => {
  ...
  test('ui reconciled: profileMembership.ui.profiles is ["full"] after tier:full reconciliation', () => {
    ...
    const { resolveProfile: rp } = require('../forge-core/bin/lib/install-profiles.cjs');
...
describe('ADR-1016 phase 5a: closed-vocab rejection — tightened validateRuntimeBody', () => {
  ...
  test('bad ArtifactKind.nesting → validation error', () => {
    const cap = makeRuntimeCap({
      ...
      runtime: {
        artifactLayout: {
          global: [{ kind: 'skills', destSubpath: 'skills', prefix: 'forge-', nesting: 'deep', recursive: false, converter: null }],
          local: [],
        },
      },
    });
...
describe('ADR-1016 phase 5a: validateArtifactLayout unit tests', () => {
  ...
  test('valid skills entry in global → no errors', () => {
    const errors = validateArtifactLayout('test', {
      global: [{ kind: 'skills', destSubpath: 'skills', prefix: 'forge-', nesting: 'flat', recursive: false, converter: null }],
      local: [],
    });
    assert.deepEqual(errors, []);
  });
  ...
  test('kimi-agents kind → no errors', () => {
    const errors = validateArtifactLayout('test', {
      global: [{ kind: 'kimi-agents', destSubpath: 'agents', prefix: 'forge', nesting: 'flat', recursive: false, converter: null }],
      local: [],
    });
    assert.deepEqual(errors, []);
  });
...
describe('FIX 3: tightened runtime validator — ArtifactKind field type checks', () => {
  test('ArtifactKind with recursive: "yes" (non-boolean) → validation error', () => {
    const cap = makeRuntimeCap({
      ...
      runtime: {
        artifactLayout: {
          global: [{ kind: 'skills', destSubpath: 'skills', prefix: 'forge-', nesting: 'flat', recursive: 'yes', converter: null }],
          local: [],
        },
      },
    });
  ...
  test('ArtifactKind with recursive: true (boolean) → no error', () => {
    const cap = makeRuntimeCap({
      ...
      runtime: {
        artifactLayout: {
          global: [{ kind: 'skills', destSubpath: 'skills', prefix: 'forge-', nesting: 'flat', recursive: true, converter: null }],
          local: [],
        },
      },
    });
  ...
  test('ArtifactKind with converter: 123 (non-string, non-null) → validation error', () => {
    const cap = makeRuntimeCap({
      ...
      runtime: {
        artifactLayout: {
          global: [{ kind: 'skills', destSubpath: 'skills', prefix: 'forge-', nesting: 'flat', recursive: false, converter: 123 }],
          local: [],
        },
      },
    });
  ...
  test('ArtifactKind with converter: "convertClaudeCommandToKiloSkill" (string) → no error', () => {
    const cap = makeRuntimeCap({
      ...
      runtime: {
        artifactLayout: {
          global: [{ kind: 'skills', destSubpath: 'skills', prefix: 'forge-', nesting: 'flat', recursive: true, converter: 'convertClaudeCommandToKiloSkill' }],
          local: [],
        },
      },
    });
...
describe('#1196 — discuss loop wiring + wired-point guard', () => {
  ...
  describe('Defect 1: discuss is a wireable loop host', () => {
    test('HOST_LOOP_FILES includes discuss-phase.md', () => {
      ...
      assert.ok(
        HOST_LOOP_FILES.includes('forge-core/workflows/discuss-phase.md'),
        `HOST_LOOP_FILES must include 'forge-core/workflows/discuss-phase.md'. Got: ${JSON.stringify(HOST_LOOP_FILES)}`,
      );
    });
  ...
  describe('Anti-pattern parity: host-file set has a single source of truth', () => {
    test('every STEP_WORKFLOWS entry file exists on disk and contains a forge:loop-host marker', () => {
      for (const { file, step } of STEP_WORKFLOWS) {
        const absPath = path.join(ROOT, 'forge-core', 'workflows', file);
        assert.ok(
        ...
        assert.match(
          content,
          /<!--\s*forge:loop-host/,
          `${file} (step: ${step}) is listed in STEP_WORKFLOWS but lacks a forge:loop-host marker`,
        );
      }
    });

    test('HOST_LOOP_FILES matches STEP_WORKFLOWS (single source of truth)', () => {
      const expectedFromStepWorkflows = STEP_WORKFLOWS.map((w) => 'forge-core/workflows/' + w.file);
      assert.deepEqual(
        HOST_LOOP_FILES,
    ...
    test('every workflow file carrying a forge:loop-host marker is present in STEP_WORKFLOWS', () => {
      const workflowsDir = path.join(ROOT, 'forge-core', 'workflows');
      ...
      for (const mdFile of allMdFiles) {
        ...
        const content = fs.readFileSync(absPath, 'utf8');
        if (/<!--\s*forge:loop-host/.test(content) && !stepWorkflowFiles.has(mdFile)) {
      ...
      assert.deepEqual(
        ...
        `These workflow files have a forge:loop-host marker but are absent from STEP_WORKFLOWS: ` +
  ...
  describe('Property: scanWiredPoints is a correct extractor', () => {
      ...
      fc.assert(
        fc.property(
          ...
          (subset) => {
            ...
            const lines = subset.map((p) => `HOOKS_JSON=$(forge_run loop render-hooks ${p} --raw)`);
            // Add some noise to exercise robustness
            const noise = ['# comment', 'echo hello', `forge_run loop some-other-command`, ''];
    ...
    test('NIT-02: scanWiredPoints does not match an incomplete occurrence (no point token after render-hooks)', () => {
      ...
      const incompleteText = 'HOOKS_JSON=$(forge_run loop render-hooks\n)';
...
//
// The validator now lives in forge-core/bin/lib/capability-validator.cjs and is
