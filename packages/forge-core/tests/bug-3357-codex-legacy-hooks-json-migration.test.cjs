...
const { readInstallState } = require('../forge-core/bin/lib/installer-migrations.cjs');
...
function legacyGsdHook(codexHome) {
  return {
    hooks: [{
      type: 'command',
      command: `node "${path.join(codexHome, 'hooks', 'forge-check-update.js')}"`,
    }],
  };
}
...
function tomlGsdHookCount(codexHome) {
  ...
  return sessionStart
    ...
    .filter((hook) => typeof hook.command === 'string' && hook.command.includes('forge-check-update'))
    .length;
}
...
describe('#3357 — Codex install removes legacy GSD hooks.json entries', { concurrency: false }, () => {
  ...
  beforeEach(() => {
    ...
    tmpRoot = createTempDir('forge-3357-');
  ...
  test('rewrites hooks.json to one managed SessionStart hook when file only had legacy managed entry', () => {
    ...
    const managed = commands.filter((cmd) => typeof cmd === 'string' && cmd.includes('forge-check-update'));
  ...
  test('preserves user hooks.json entries while removing the legacy GSD hook', () => {
    const userOwnedSameBasenameHook = {
      hooks: [{
        type: 'command',
        command: 'node "/Users/example/bin/forge-check-update.js"',
      }],
    };
    fs.writeFileSync(
      path.join(codexHome, 'hooks.json'),
    ...
    assert.equal(commands.includes('node "/Users/example/bin/forge-check-update.js"'), true);
...
describe('#1348 — reconcileCodexHooksJsonEvent canonical nested shape', { concurrency: false }, () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('forge-1348-');
  });
  ...
  test('(a) fresh/absent hooks.json writes nested { hooks: { SessionStart: [...] } } shape', () => {
    ...
    const FAKE_CMD = `"/usr/local/bin/node" "${path.join(tmpDir, 'hooks', 'forge-check-update.js').replace(/\\/g, '/')}"`;
  ...
  test('(b) legacy top-level shape: user entries migrate into hooks.SessionStart alongside managed entry', () => {
    ...
    const managedCount = allCommands.filter((c) => typeof c === 'string' && c.includes('forge-check-update')).length;
  ...
  test('(d) mixed nested + top-level shape: stray top-level event array is lifted and merged', () => {
    ...
    const managedCount = sessionCommands.filter((c) => typeof c === 'string' && c.includes('forge-check-update')).length;
