 ...
 * regex (`/^hooks\/forge-[^/]+\.(?:js|sh|cjs|mjs)$/`) that matches ANY file
 * named `hooks/forge-<name>.{js,sh,cjs,mjs}`, not only the 13 hook files
 ...
 *
 *   - User-authored custom hooks (e.g. `hooks/forge-personal-experiment.js`)
 ...
 * exported from the classifier module). Any `hooks/forge-<name>` file NOT in
...
describe('bug #3628: BUNDLED_GSD_HOOK_FILES is an explicit whitelist', () => {
  ...
  test('every entry is a hooks/-prefixed posix path', () => {
    for (const relPath of BUNDLED_GSD_HOOK_FILES) {
      ...
      assert.ok(
        relPath.includes('forge-'),
        `entry ${JSON.stringify(relPath)} must contain the "forge-" prefix`,
      );
    }
  });
  ...
  test('every forge-*.{js,sh,cjs,mjs} file in hooks/ is in BUNDLED_GSD_HOOK_FILES (no shipping drift)', () => {
    const hooksDir = path.join(__dirname, '..', 'hooks');
    const onDisk = fs
...
describe('bug #3628: classifyPromptUserAction whitelists shipped bundled hooks', () => {
  ...
  const USER_OWNED_OR_RETIRED = [
    'hooks/forge-personal-experiment.js',
    'hooks/forge-my-custom-guard.sh',
    'hooks/forge-team-policy.cjs',
    'hooks/forge-retired-hook.js',
    'hooks/forge-old-statusline.js',
    'hooks/forge-experimental.mjs',
  ];
  ...
  test('still does NOT classify nested forge-* directories (existing #3610 boundary preserved)', () => {
    assert.strictEqual(
      classifyPromptUserAction({ relPath: 'hooks/forge-helpers/index.js' }),
      null,
    );
  });
