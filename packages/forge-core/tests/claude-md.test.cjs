...
describe('generate-claude-md', () => {
  ...
  test('creates CLAUDE.md with workflow enforcement section', () => {
    ...
    assert.ok(content.includes('/forge-quick'));
    assert.ok(content.includes('/forge-debug'));
    assert.ok(content.includes('/forge-execute-phase'));
    assert.ok(!content.includes('/forge:quick'));
    assert.ok(!content.includes('/forge:execute-phase'));
...
describe('new-project workflow includes CLAUDE.md generation', () => {
  const workflowPath = path.join(__dirname, '..', 'forge-core', 'workflows', 'new-project.md');
...
describe('generate-claude-md skills section', () => {
  ...
  test('discovers skills from .codex/skills/ directory and ignores deprecated import-only roots', () => {
    ...
    const homeDir = fs.mkdtempSync(path.join(require('os').tmpdir(), 'forge-claude-skills-home-'));
    fs.mkdirSync(path.join(homeDir, '.claude', 'forge-core', 'skills', 'import-only'), { recursive: true });
    fs.writeFileSync(
      path.join(homeDir, '.claude', 'forge-core', 'skills', 'import-only', 'SKILL.md'),
  ...
  test('skips forge- prefixed skill directories', () => {
    const gsdSkillDir = path.join(tmpDir, '.claude', 'skills', 'forge-plan-phase');
    ...
    const content = fs.readFileSync(path.join(tmpDir, '.claude', 'CLAUDE.md'), 'utf-8');
    assert.ok(!content.includes('forge-plan-phase'));
