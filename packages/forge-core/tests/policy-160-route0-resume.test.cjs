 ...
 * SUMMARY.md files, causing /forge-next or /forge-progress to silently skip
...
describe('Route 0: resume_incomplete_phase invariant (#160)', () => {
  const nextMdPath = path.join(__dirname, '..', 'forge-core', 'workflows', 'next.md');
  const progressMdPath = path.join(__dirname, '..', 'forge-core', 'workflows', 'progress.md');
  ...
  describe('next.md', () => {
    ...
    test('routes to lowest-numbered incomplete phase via execute-phase', () => {
      ...
      assert.ok(
        content.includes('forge-execute-phase') || content.includes('forge:execute-phase'),
    ...
    test('scan uses forge_run (canonical resolver form, not bare forge-sdk)', () => {
      const content = fs.readFileSync(nextMdPath, 'utf8');
      const route0Start = content.indexOf('name="resume_incomplete_phase"');
      const route0End = content.indexOf('</step>', route0Start);
      const route0Block = content.slice(route0Start, route0End);
      // Must use forge_run, not bare forge-sdk
      assert.ok(
        route0Block.includes('forge_run'),
        'Route 0 scan must use forge_run (canonical resolver), not bare forge-sdk'
      );
      // Must NOT use bare forge-sdk (without $)
      const bareGsdSdkPattern = /(?<!\$)gsd-sdk/;
      assert.ok(
        !bareGsdSdkPattern.test(route0Block),
        'Route 0 must not use bare forge-sdk — use forge_run to match the file convention'
      );
    });
  ...
  describe('progress.md', () => {
    ...
    test('scan uses $ROADMAP already loaded (not a fresh bare forge-sdk call)', () => {
      const content = fs.readFileSync(progressMdPath, 'utf8');
      const route0Start = content.indexOf('Step 0: Resume-incomplete-phase');
      const route0End = content.indexOf('Step 1:', route0Start);
      const route0Block = content.slice(route0Start, route0End);
