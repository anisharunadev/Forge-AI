...
// commands/forge/*.md bodies are the deployed contract — cross-references between
...
const COMMANDS_DIR = path.resolve(__dirname, '..', 'commands', 'forge');
...
// Excludes path-y characters (~, ., /) so `~/forge-workspaces`, `./forge-foo`, `path/forge-bar` don't match.
// Trailing `(?![\w-]*\/)` rejects filesystem path segments like `${VAR}/forge-core/bin` (the
// runtime-launcher shim) where a non-path char (e.g. `}`) precedes `/forge-core/` — those are
// directory paths to the forge-core/ runtime, not slash-command references (#604 rename).
const REF_PATTERN = /(?<![a-zA-Z0-9_~./-])\/gsd[:-]([a-zA-Z0-9_-]+)(?![\w-]*\/)/g;

describe('bug-3683 command cross-reference invariant', () => {
  test('all /forge:<X> and /forge-<X> body refs resolve to known command base-names', () => {
