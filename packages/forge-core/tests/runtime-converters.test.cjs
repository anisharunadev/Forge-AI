...
for (const { label, convert, configDir } of flatRuntimeSuites) {
  describe(`${label} agent conversion (isAgent: true)`, () => {
    test('keeps name: field for agents', () => {
      ...
      assert.ok(frontmatter.includes('name: forge-executor'), 'name: should be preserved for agents');
    });
    ...
    test('strips skills: array', () => {
      ...
      assert.ok(!frontmatter.includes('forge-executor-workflow'), 'skill entries should be stripped');
    });
...
Use ./.claude/hooks/forge-statusline.js during local testing.
...
Use \${PHASE} in shell examples.
    ...
    assert.ok(frontmatter.includes('name: forge-codebase-mapper'), 'keeps name');
    ...
    assert.ok(!frontmatter.includes('forge-mapper-workflow'), 'drops skills list items');
  ...
  test('excludes Claude-only agent interaction tools from Gemini frontmatter', () => {
    const input = `---
name: forge-debug-session-manager
...
Run \`/forge:help\` to see the guide. AskUserQuestion when unsure.
`;
...
Body for /forge:ultraplan-phase.
`;
  ...
  for (const { label, convert } of cases) {
    describe(`${label} skill conversion`, () => {
      test('emits SKILL.md frontmatter with name matching the skill dir', () => {
        const out = convert(SKILL_SAMPLE_COMMAND, 'forge-help');
        assert.ok(out.startsWith('---\n'), 'opens with frontmatter');
        assert.match(out, /^name: forge-help$/m, 'name equals the skill name');
      });
      ...
      test('rewrites /forge: colon refs to hyphen form in the body', () => {
        const out = convert(SKILL_SAMPLE_COMMAND, 'forge-help');
        assert.ok(!/\/forge:/.test(out), 'no /forge: colon refs remain');
        assert.match(out, /\/forge-help/, 'colon ref rewritten to hyphen form');
      });

      test('quotes descriptions with leading YAML flow indicators ([BETA])', () => {
        const out = convert(SKILL_BETA_COMMAND, 'forge-ultraplan-phase');
      ...
      test('falls back to a synthetic description when none present', () => {
        const out = convert('Body only, no frontmatter.', 'forge-mystery');
        assert.match(out, /^name: forge-mystery$/m);
        assert.match(out, /^description: "Run GSD workflow forge-mystery\."$/m);
      });
    });
  }
});
