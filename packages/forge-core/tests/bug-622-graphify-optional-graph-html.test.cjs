...
// This test extracts the deployed Step 3 shell block from commands/forge/graphify.md
 ...
 *
 * The `/forge-graphify build` Step 3 shell chain in commands/forge/graphify.md
...
const GRAPHIFY_MD = path.join(__dirname, '..', 'commands', 'forge', 'graphify.md');
...
before(() => {
  sandbox = createTempDir('forge-622-sandbox-');
  fakeBin = createTempDir('forge-622-fakebin-');
  fakeHome = createTempDir('forge-622-fakehome-');
});
...
/**
 * Write a minimal forge-tools.cjs stub into fakeHome that exits 0 for any
 * invocation (covers the `graphify build snapshot` and `graphify status` steps).
 */
function writeFakeGsdTools() {
  const binDir = path.join(fakeHome, '.claude', 'forge-core', 'bin');
...
describe('bug #622: graph.html absence must not abort the Step 3 shell chain', () => {
  ...
  test('Step 3 bash block is present in graphify.md (sanity gate)', () => {
    assert.ok(block !== null, 'Step 3 bash block starting with "graphify update ." was not found in commands/forge/graphify.md');
