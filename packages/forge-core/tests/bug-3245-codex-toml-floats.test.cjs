...
describe('#3245 — install succeeds with TOML float in pre-existing config', { concurrency: false }, () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-3245-float-'));
...
describe('#3245 — idempotent rollback reverts skills/, agents/, and VERSION', { concurrency: false }, () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-3245-rollback-'));
    codexHome = path.join(tmpDir, 'codex-home');
  });
  ...
  test('validation failure rolls back skills/, agents/, and VERSION to pre-install state', () => {
    ...
    // skills/ — GSD writes forge-* subdirs here. All must be absent after rollback.
    ...
    if (fs.existsSync(skillsDir)) {
      ...
      assert.strictEqual(
        gsdSkills.length,
        0,
        'rollback must remove all forge-* skill directories: ' + gsdSkills.map(e => e.name).join(', ')
      );
    }

    // agents/ — GSD writes forge-*.md and forge-*.toml here. All must be absent.
    ...
    if (fs.existsSync(agentsDir)) {
      const gsdAgents = fs.readdirSync(agentsDir)
        .filter(f => f.startsWith('forge-') && (f.endsWith('.md') || f.endsWith('.toml')));
      assert.strictEqual(
        gsdAgents.length,
        0,
        'rollback must remove all forge-* agent files: ' + gsdAgents.join(', ')
      );
    }

    // VERSION — GSD writes forge-core/VERSION. Must be absent (wasn't there before).
    const versionPath = path.join(codexHome, 'forge-core', 'VERSION');
    assert.strictEqual(
  ...
  test('rollback is safe when fired before any snapshots were captured (very early failure)', () => {
    ...
    // Rollback removes all forge-* skill dirs it wrote. Even if skills/ was
    // created during the install, no forge-* dirs should survive after rollback.
    const skillsDir = path.join(codexHome, 'skills');
    ...
    assert.deepStrictEqual(
      remainingGsdSkills,
      [],
      'rollback must remove all forge-* skill dirs even when fired after minimal writes'
    );
  });

  test('rollback does not remove pre-existing user skills that GSD did not write', () => {
    // If the user has a custom skill dir (not forge-*) it must survive rollback.
    const skillsDir = path.join(codexHome, 'skills');
