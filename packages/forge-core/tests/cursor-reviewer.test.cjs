 ...
 *
 * Verifies that /forge-review includes Cursor CLI as a peer reviewer:
 *   - review.md workflow contains cursor detection, flag parsing, self-detection, invocation
 *   - commands/forge/review.md command file mentions --cursor flag
 *   - help.md lists --cursor in the /forge-review signature
...
describe('Cursor CLI reviewer in /forge-review (#1960)', () => {

  // --- review.md workflow ---

  describe('review.md workflow', () => {
    const reviewPath = path.join(ROOT, 'forge-core', 'workflows', 'review.md');
    let _content;
    ...
    test('invocation passes prompt as a file-path argument (not via stdin pipe)', () => {
      const c = fs.readFileSync(reviewPath, 'utf-8');
      assert.ok(
        c.includes('Read the file at /tmp/forge-review-prompt-'),
  ...
  describe('commands/forge/review.md', () => {
    const cmdPath = path.join(ROOT, 'commands', 'forge', 'review.md');

    test('mentions --cursor flag', () => {
      const c = fs.readFileSync(cmdPath, 'utf-8');
      assert.ok(
        c.includes('--cursor'),
        'commands/forge/review.md should mention --cursor flag'
      );
    });
    ...
    test('mentions Cursor in objective or context', () => {
      ...
      assert.ok(
        c.includes('Cursor'),
        'commands/forge/review.md should mention Cursor'
      );
    });
  });
  ...
  describe('help.md', () => {
    ...
    const helpPath = path.join(ROOT, 'forge-core', 'workflows', 'help', 'modes', 'full.md');

    test('lists --cursor in /forge-review signature', () => {
      const c = fs.readFileSync(helpPath, 'utf-8');
      assert.ok(
        c.includes('--cursor'),
        'help.md should list --cursor in the /forge-review command signature'
      );
    });
  });
