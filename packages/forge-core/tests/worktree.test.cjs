...
const DIAGNOSE_PATH = path.join(REPO_ROOT, 'forge-core', 'workflows', 'diagnose-issues.md');
const _GIT_INTEGRATION_PATH = path.join(REPO_ROOT, 'forge-core', 'references', 'git-integration.md');
...
describe('bug-2075: worktree deletion safeguards', () => {
  ...
  describe('Failure Mode B: git clean prohibition in executor agent', () => {
    test('forge-executor.md explicitly prohibits git clean in worktree context', () => {
      ...
      assert.ok(
        prohibitsGitClean,
        'forge-executor.md must explicitly prohibit git clean — running it inside a worktree deletes files committed on the feature branch (#2075 Failure Mode B)'
      );
    });

    test('forge-executor.md git clean prohibition explains the worktree data-loss risk', () => {
      const content = fs.readFileSync(EXECUTOR_AGENT_PATH, 'utf-8');
      ...
      assert.ok(gitCleanIdx > -1, 'forge-executor.md must mention git clean (to prohibit it)');
      ...
      assert.ok(
        ...
        'The git clean prohibition in forge-executor.md must explain why: git clean in a worktree deletes files that appear untracked but are committed on the feature branch'
      );
    });
  });
  ...
  describe('Defense-in-depth: post-commit deletion check (from #1977)', () => {
    test('forge-executor.md task_commit_protocol has post-commit deletion verification', () => {
      const content = fs.readFileSync(EXECUTOR_AGENT_PATH, 'utf-8');

      assert.ok(
        content.includes('--diff-filter=D'),
        'forge-executor.md must include --diff-filter=D to detect accidental file deletions after each commit'
      );

      // Must have a warning about unexpected deletions
      assert.ok(
        content.includes('DELETIONS') || content.includes('WARNING'),
        'forge-executor.md must emit a warning when a commit includes unexpected file deletions'
      );
    });
  });
...
describe('bug #2774 — worktree cleanup pipeline must not target the parent workspace', () => {
  describe('discovery pipeline (unit)', () => {
    test('selects only the agent worktree when workspace itself is a worktree', () => {
      ...
      const porcelain = [
        'worktree /Users/dev/upstream/forge-core',
        'HEAD abc123',
...
`;
  ...
  describe('end-to-end against real git worktrees',
    ...
    beforeEach(() => {
      ...
      upstream = makeTempUpstreamRepo('forge-2774-upstream-');

      workspacesParent = fs.mkdtempSync(
        path.join(os.tmpdir(), 'forge-2774-workspaces-')
      );
