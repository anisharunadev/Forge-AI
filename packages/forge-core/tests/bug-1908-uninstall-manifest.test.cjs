 ...
 *
 * `--uninstall` did not remove `forge-file-manifest.json` from the target
...
function createFakeInstall(prefix = 'forge-uninstall-test-') {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), prefix));

  // Simulate the minimum directory/file layout produced by the installer:
  // forge-core/ directory, agents/ directory, and the manifest file.
  fs.mkdirSync(path.join(dir, 'forge-core', 'workflows'), { recursive: true });
  fs.writeFileSync(path.join(dir, 'forge-core', 'workflows', 'execute-phase.md'), '# stub');

  fs.mkdirSync(path.join(dir, 'agents'), { recursive: true });
  fs.writeFileSync(path.join(dir, 'agents', 'forge-executor.md'), '# stub');

  const manifest = {
    version: '1.34.0',
    timestamp: new Date().toISOString(),
    files: {
      'forge-core/workflows/execute-phase.md': 'abc123',
      'agents/forge-executor.md': 'def456',
    },
  };
...
describe('uninstall — manifest cleanup (#1908)', () => {
  ...
  test('forge-file-manifest.json is removed after global uninstall', () => {
  ...
  test('forge-file-manifest.json is removed after local uninstall', () => {
    const manifestPath = path.join(tmpDir, MANIFEST_NAME);
    ...
    fs.mkdirSync(path.join(localDir, 'forge-core', 'workflows'), { recursive: true });
    fs.writeFileSync(path.join(localDir, 'forge-core', 'workflows', 'execute-phase.md'), '# stub');
