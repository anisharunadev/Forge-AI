 ...
 *
 * Kimi Agent Skills use forge-<command>/SKILL.md directories with lowercase
...
function sampleCommand() {
  return [
    ...
    'color: blue',
    'skills:',
    '  - forge-planner-workflow',
    'hooks:',
    '  PostToolUse:',
    '    - matcher: "Write|Edit"',
    '---',
    '',
    'Invoke /forge:new-project from slash form.',
    'Invoke forge:new-project from bare colon form.',
    'Invoke /forge-new-project from hyphen slash form.',
    'Invoke $gsd-new-project from shell-style form.',
    'Do not rewrite /forge-tools, forge-new-projector, or $gsd-planets.',
  ].join('\n');
}

describe('convertClaudeCommandToKimiSkill', () => {
  test('emits Kimi SKILL.md frontmatter with name and description only', () => {
    const result = convertClaudeCommandToKimiSkill(sampleCommand(), 'forge-new-project');

    assert.ok(result.startsWith('---\n'), 'frontmatter starts with ---');
    assert.ok(result.includes('\n---\n'), 'frontmatter closes with ---');
    assert.ok(result.includes('name: forge-new-project'), 'frontmatter name uses Kimi-safe hyphen form');
    assert.ok(
  ...
  test('rewrites GSD command references to Kimi skill invocation syntax', () => {
    ...
    assert.ok(!result.includes('/forge:new-project'), 'slash colon form is removed');
    assert.ok(!result.includes('forge:new-project'), 'bare colon form is removed');
    assert.ok(!result.includes('/forge-new-project'), 'slash hyphen form is removed');
  ...
  test('does not rewrite unrelated words or introduce Phase 3 artifacts', () => {
    ...
    assert.ok(result.includes('/forge-tools'), 'non-command slash token is preserved');
    assert.ok(result.includes('forge-new-projector'), 'longer hyphenated word is preserved');
