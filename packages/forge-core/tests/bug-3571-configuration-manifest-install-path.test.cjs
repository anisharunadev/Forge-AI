 ...
 * checkout sdk/shared path only, which breaks installed forge-tools.cjs because
 * runtime installs copy forge-core/ but not sdk/.
 */
...
const CONFIGURATION_CJS = path.join(REPO_ROOT, 'forge-core', 'bin', 'lib', 'configuration.cjs');
const SHARED_DIR = path.join(REPO_ROOT, 'forge-core', 'bin', 'shared');

const { install } = require('../bin/install.js');

const { createTempDir, cleanup } = require('./helpers.cjs');
const makeTmpDir = () => createTempDir('forge-3571-');
...
describe('bug #3571: configuration generated manifests resolve in install layout', () => {
  ...
  test('co-located bin/shared manifests let configuration.cjs load without sdk/shared', () => {
    const gsdBinDir = path.join(tmpRoot, '.codex', 'forge-core', 'bin');
  ...
  test('post-install: install() copies configuration manifests to co-located bin/shared', () => {
    ...
    const sharedDir = path.join(tmpRoot, '.codex', 'forge-core', 'bin', 'shared');
