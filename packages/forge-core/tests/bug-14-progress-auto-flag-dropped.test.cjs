...
describe('#14: /forge:progress --next --auto flag must be documented and propagated', () => {
  test('progress.md <flags> section documents --auto flag', () => {
    const command = fs.readFileSync(
      path.join(ROOT, 'commands', 'forge', 'progress.md'),
      'utf8'
    );
  ...
  test('next.md show_and_execute step handles --auto to chain steps', () => {
    const workflow = fs.readFileSync(
      path.join(ROOT, 'forge-core', 'workflows', 'next.md'),
      'utf8'
    );
  ...
  test('next.md --auto chaining re-invokes /forge:progress --next after step completion', () => {
    const workflow = fs.readFileSync(
      path.join(ROOT, 'forge-core', 'workflows', 'next.md'),
      'utf8'
    );

    // The workflow must contain instructions to re-invoke /forge:progress --next --auto
    // after the determined step completes, enabling the chain.
    assert.ok(
      workflow.includes('--next --auto'),
      'next.md must instruct re-invocation of /forge:progress --next --auto after step completion to enable chaining'
    );
  });
});
