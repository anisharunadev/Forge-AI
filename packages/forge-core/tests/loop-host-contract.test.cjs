...
const CONTRACT_PATH = path.join(ROOT, 'forge-core', 'bin', 'lib', 'loop-host-contract.cjs');
...
describe('crossCheckRoles', () => {
  ...
  test('passes when agent name is present in content', () => {
    const content = 'Agent(subagent_type="forge-planner") Agent(subagent_type="forge-phase-researcher") forge-plan-checker';
  ...
  test('fails when declared role has no agent reference in content', () => {
    const content = 'forge-phase-researcher forge-plan-checker'; // planner missing
    const errors = crossCheckRoles(content, ['researcher', 'planner', 'checker'], 'plan-phase.md');
    assert.strictEqual(errors.length, 1, 'expected exactly 1 error for missing planner');
    assert.match(errors[0], /planner.*forge-planner/);
  });

  test('fails when declared role is unknown (not in ROLE_TO_AGENT)', () => {
    const content = 'forge-executor forge-verifier';
...
describe('buildContract from real workflows', () => {
  test('produces a contract matching the inline LOOP_HOST_CONTRACT shape', () => {
    const contract = buildContract(); // reads real forge-core/workflows/
...
describe('buildContract cross-check drift guard', () => {
  ...
  test('rejects a block declaring a role whose agent is absent from the workflow', () => {
    // plan-phase.md declares 'researcher' but does NOT mention forge-phase-researcher
    const files = makeValidWorkflowFiles();
    files['plan-phase.md'] =
      '<!-- forge:loop-host\n' +
      'step: plan\n' +
      ...
      'forge-planner forge-plan-checker\n';

    const tmpDir = makeTempWorkflowsDir(files);
    try {
      assert.throws(
        () => buildContract(tmpDir),
        /forge-phase-researcher.*not referenced|researcher.*forge-phase-researcher/,
      );
    } finally {
      cleanup(tmpDir);
    }
...
describe('parseLoopHostBlock multiple-block and duplicate-key detection (FIX 2)', () => {
  test('throws when a file has two forge:loop-host marker blocks', () => {
    const block =
      '<!-- forge:loop-host\n' +
      'step: discuss\n' +
    ...
    assert.throws(
      ...
      /expected exactly one forge:loop-host marker block, found 2/,
    );
  });
  ...
  test('throws when a block has a duplicate "points" key', () => {
    ...
    assert.throws(
      () => parseLoopHostBlock(content, 'discuss-phase.md'),
      /duplicate key 'points' in forge:loop-host marker/,
    );
  });
  ...
  test('throws when a block has a duplicate "step" key', () => {
    ...
    assert.throws(
      () => parseLoopHostBlock(content, 'discuss-phase.md'),
      /duplicate key 'step' in forge:loop-host marker/,
    );
  });
...
describe('crossCheckRoles word-boundary match (FIX 3)', () => {
  test('forge-plan-checker-v2 does NOT satisfy required forge-plan-checker reference', () => {
    // Content has forge-plan-checker-v2 but NOT bare forge-plan-checker
    const content = 'Agent("forge-phase-researcher") Agent("forge-planner") forge-plan-checker-v2';
    const errors = crossCheckRoles(content, ['researcher', 'planner', 'checker'], 'plan-phase.md');
    assert.strictEqual(errors.length, 1, 'expected exactly 1 error for checker missing bare reference');
    assert.match(errors[0], /forge-plan-checker/);
  });

  test('forge-plan-checker (bare) still satisfies the checker role', () => {
    const content = 'Agent("forge-phase-researcher") Agent("forge-planner") forge-plan-checker something-else';
    const errors = crossCheckRoles(content, ['researcher', 'planner', 'checker'], 'plan-phase.md');
    assert.deepEqual(errors, []);
  });

  test('forge-plan-checker immediately followed by newline satisfies the checker role', () => {
    const content = 'forge-phase-researcher\nforge-planner\nforge-plan-checker\n';
    const errors = crossCheckRoles(content, ['researcher', 'planner', 'checker'], 'plan-phase.md');
    assert.deepEqual(errors, []);
  });
  ...
  test('buildContract rejects workflow referencing only -v2 agent variant', () => {
    const files = makeValidWorkflowFiles();
    // plan-phase.md refers to forge-plan-checker-v2 but not forge-plan-checker
    files['plan-phase.md'] =
      '<!-- forge:loop-host\n' +
      'step: plan\n' +
      'points: plan:pre, plan:post\n' +
      ...
      'consumes: CONTEXT.md\n' +
      '-->\n' +
      'forge-phase-researcher forge-planner forge-plan-checker-v2\n';
    const tmpDir = makeTempWorkflowsDir(files);
    try {
      assert.throws(
        () => buildContract(tmpDir),
        /forge-plan-checker.*not referenced|checker.*forge-plan-checker/,
      );
    } finally {
      cleanup(tmpDir);
    }
...
describe('temp-dir lifecycle', () => {
  test('makeTempWorkflowsDir creates a directory that can be cleaned up', () => {
    const files = { 'dummy.md': '<!-- forge:loop-host\nstep: discuss\n-->' };
    const tmpDir = makeTempWorkflowsDir(files);
