 ...
 *
 * generated `.opencode/agents/forge-*.md` files contained no `model:` frontmatter.
 *
...
const makeTmp = (prefix) => createTempDir(`forge-2794-${prefix}-`);
...
describe('bug-2794: readGsdRuntimeProfileResolver resolves opencode tier overrides', () => {
  ...
  test('resolves opencode sonnet tier to user-supplied model ID', () => {
    ...
    // forge-roadmapper balanced tier = sonnet — should resolve to override
    const entry = resolver.resolve('forge-roadmapper');
    assert.ok(entry !== null, 'expected entry for forge-roadmapper');
  ...
  test('resolver returns null for agent not in MODEL_PROFILES', () => {
    ...
    const entry = resolver.resolve('forge-nonexistent-agent');
...
describe('bug-2794: OpenCode agent install embeds model_profile_overrides model', () => {
  ...
  test('generated OpenCode agent frontmatter includes model from model_profile_overrides', () => {
    ...
    // forge-roadmapper is balanced -> sonnet tier
    const roadmapperPath = path.join(agentsDir, 'forge-roadmapper.md');
    assert.ok(fs.existsSync(roadmapperPath), 'forge-roadmapper.md should exist');
    const roadmapperContent = fs.readFileSync(roadmapperPath, 'utf-8');
    assert.match(
      roadmapperContent,
      /^model: anthropic\/claude-sonnet-4-7$/m,
      'forge-roadmapper should have sonnet model from model_profile_overrides'
    );

    // forge-planner is balanced -> opus tier
    const plannerPath = path.join(agentsDir, 'forge-planner.md');
    assert.ok(fs.existsSync(plannerPath), 'forge-planner.md should exist');
    const plannerContent = fs.readFileSync(plannerPath, 'utf-8');
    assert.match(
      plannerContent,
      /^model: anthropic\/claude-opus-4-7$/m,
      'forge-planner should have opus model from model_profile_overrides'
    );
  });
  ...
  test('explicit model_overrides[agent] wins over model_profile_overrides tier', () => {
    writeJson(path.join(projectDir, '.planning', 'config.json'), {
      ...
      model_profile: 'balanced',
      model_overrides: {
        'forge-roadmapper': 'explicit-winner-model',
      },
      model_profile_overrides: {
        opencode: {
    ...
    const roadmapperPath = path.join(projectDir, '.opencode', 'agents', 'forge-roadmapper.md');
  ...
  test('no model field when neither model_overrides nor model_profile_overrides is set', () => {
    ...
    if (fs.existsSync(roadmapperPath)) {
      ...
      // (forge-roadmapper balanced = sonnet; opencode has built-in sonnet defaults)
