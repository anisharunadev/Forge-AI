...
describe('SPAWN: spawn type consistency', () => {
  test('no "First, read agent .md" workaround pattern remains', () => {
    ...
    for (const dir of dirs) {
      ...
      for (const file of files) {
        ...
        const hasWorkaround = content.includes('First, read ~/.claude/agents/forge-');
        assert.ok(
          !hasWorkaround,
  ...
  test('diagnose-issues uses forge-debugger (not general-purpose)', () => {
    const content = fs.readFileSync(
      path.join(WORKFLOWS_DIR, 'diagnose-issues.md'), 'utf-8'
    );
    assert.ok(
      content.includes('subagent_type="forge-debugger"'),
      'diagnose-issues should spawn forge-debugger, not general-purpose'
    );
  });
...
describe('CLAUDEMD: CLAUDE.md compliance enforcement', () => {
  test('forge-plan-checker has Dimension 10: CLAUDE.md Compliance', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'forge-plan-checker.md'), 'utf-8');
    assert.ok(
      content.includes('Dimension 10: CLAUDE.md Compliance'),
      'forge-plan-checker must have Dimension 10 for CLAUDE.md compliance checking'
    );
    assert.ok(
      content.includes('claude_md_compliance'),
      'forge-plan-checker must use claude_md_compliance as dimension identifier'
    );
  });

  test('forge-phase-researcher has CLAUDE.md enforcement directive', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'forge-phase-researcher.md'), 'utf-8');
    assert.ok(
      content.includes('CLAUDE.md enforcement'),
      'forge-phase-researcher must enforce CLAUDE.md directives during research'
    );
    assert.ok(
      content.includes('Project Constraints (from CLAUDE.md)'),
      'forge-phase-researcher must output a Project Constraints section from CLAUDE.md'
    );
  });

  test('forge-executor has CLAUDE.md enforcement directive', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'forge-executor.md'), 'utf-8');
    assert.ok(
      content.includes('CLAUDE.md enforcement'),
      'forge-executor must enforce CLAUDE.md directives during execution'
    );
    assert.ok(
      content.includes('CLAUDE.md rule — it takes precedence over plan instructions'),
      'forge-executor must specify CLAUDE.md precedence over plan instructions'
    );
  });

  test('all three agents read CLAUDE.md in project_context', () => {
    const agents = ['forge-plan-checker', 'forge-phase-researcher', 'forge-executor'];
...
describe('VERIFY: data-flow trace, environment audit, and behavioral spot-checks', () => {
  test('forge-verifier has Step 4b: Data-Flow Trace', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'forge-verifier.md'), 'utf-8');
    assert.ok(
      content.includes('Step 4b: Data-Flow Trace'),
      'forge-verifier must have Step 4b for data-flow tracing'
    );
    assert.ok(
      content.includes('HOLLOW'),
      'forge-verifier must define HOLLOW status for wired-but-disconnected artifacts'
    );
    assert.ok(
      content.includes('DISCONNECTED'),
      'forge-verifier must define DISCONNECTED status for missing data sources'
    );
  });

  test('forge-verifier has Step 7b: Behavioral Spot-Checks', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'forge-verifier.md'), 'utf-8');
    assert.ok(
      content.includes('Step 7b: Behavioral Spot-Checks'),
      'forge-verifier must have Step 7b for behavioral spot-checks'
    );
    assert.ok(
      content.includes('SKIP'),
      'forge-verifier spot-checks must support SKIP status for untestable items'
    );
  });

  test('forge-verifier VERIFICATION.md template includes data-flow and spot-check sections', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'forge-verifier.md'), 'utf-8');
    assert.ok(
  ...
  test('forge-verifier success criteria include data-flow and spot-checks', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'forge-verifier.md'), 'utf-8');
    assert.ok(
      content.includes('Data-flow trace (Level 4)'),
  ...
  test('forge-phase-researcher has Step 2.6: Environment Availability Audit', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'forge-phase-researcher.md'), 'utf-8');
    assert.ok(
      content.includes('Step 2.6: Environment Availability Audit'),
      'forge-phase-researcher must have Step 2.6 for environment availability auditing'
    );
    assert.ok(
      content.includes('Environment Availability'),
      'forge-phase-researcher must include Environment Availability section in RESEARCH.md template'
    );
  });

  test('forge-phase-researcher success criteria include environment audit', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'forge-phase-researcher.md'), 'utf-8');
    assert.ok(
...
describe('DISCUSS: discussion log generation', () => {
  ...
  test('discussion-log template exists', () => {
    const templatePath = path.join(__dirname, '..', 'forge-core', 'templates', 'discussion-log.md');
    assert.ok(
...
describe('EDITWRITE: section-writer agents must have both Write and Edit in tools', () => {
  ...
  // clobber sibling sections. Same bug class as #571/#575 (fixed forge-doc-writer).
  // Issue #581.
  const SECTION_WRITER_AGENTS = [
    'forge-eval-planner',
    'forge-ai-researcher',
    'forge-domain-researcher',
    'forge-phase-researcher',
    'forge-ui-researcher',
    'forge-debug-session-manager',
    'forge-planner', // #973: planner lacked Edit; whole-file Write truncated ROADMAP.md
  ];
...
describe('COMPAT: agents must not use runtime-specific frontmatter keys', () => {
  ...
  const AGENTS_WITH_WRITE = ['forge-executor', 'forge-debugger'];
