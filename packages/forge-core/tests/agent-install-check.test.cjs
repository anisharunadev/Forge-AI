 ...
 *
 * Seam: forge-core/bin/lib/agent-install-check.cjs
...
const AGENT_INSTALL_CHECK_PATH = path.join(
  __dirname, '..', 'forge-core', 'bin', 'lib', 'agent-install-check.cjs'
);
const RUNTIME_HOMES_PATH = path.join(
  __dirname, '..', 'forge-core', 'bin', 'lib', 'runtime-homes.cjs'
);
...
const MODEL_PROFILES = require(path.join(__dirname, '..', 'forge-core', 'bin', 'lib', 'model-profiles.cjs')).MODEL_PROFILES;
...
describe('checkAgentsInstalled', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('forge-agent-check-');
