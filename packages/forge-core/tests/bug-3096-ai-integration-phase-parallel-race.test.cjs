...
//
// ai-integration-phase.md listed Steps 7+8 (forge-ai-researcher +
// forge-domain-researcher) without an explicit sequential ordering constraint.
// An orchestrator optimizing for speed could reasonably parallelize them
// since the sections appeared disjoint. When parallelized, forge-domain-researcher's
...
const src = fs.readFileSync(
  path.join(ROOT, 'forge-core', 'workflows', 'ai-integration-phase.md'),
  'utf8',
);
...
describe('bug #3096: ai-integration-phase sequential ordering and Edit-only discipline', () => {
  ...
  test('Step 7 forge-ai-researcher prompt includes Edit-only tool discipline', () => {
    // The discipline block must appear before </objective> for forge-ai-researcher
    const step7Idx = src.indexOf('## 7. Spawn forge-ai-researcher');
    const step8Idx = src.indexOf('## 8. Spawn forge-domain-researcher');
  ...
  test('Step 8 forge-domain-researcher prompt includes Edit-only tool discipline', () => {
    const step8Idx = src.indexOf('## 8. Spawn forge-domain-researcher');
    const step9Idx = src.indexOf('## 9. Spawn forge-eval-planner');
    assert.ok(step8Idx !== -1, 'Step 8 not found');
