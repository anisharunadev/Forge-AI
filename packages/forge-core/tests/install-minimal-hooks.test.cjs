...
describe('install-profiles: MINIMAL_SKILL_ALLOWLIST', () => {
  ...
  test('every allowlisted skill exists in commands/forge/', () => {
    ...
    for (const name of MINIMAL_SKILL_ALLOWLIST) {
      assert.ok(
        ...
        `${name} is allowlisted but commands/forge/${name}.md does not exist`,
      );
    }
  });
});
...
describe('install-profiles: stageSkillsForMode', () => {
  function createFixtureSkillsDir() {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-stage-fixture-'));
  ...
  test('minimal mode against non-existent source returns source path', () => {
    const ghost = path.join(os.tmpdir(), 'forge-stage-does-not-exist-' + Date.now());
    assert.strictEqual(stageSkillsForMode(ghost, 'minimal'), ghost);
  });

  test('minimal mode skips non-md files and subdirectories', () => {
    const src = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-stage-mixed-'));
    let staged;
    try {
...
describe('install-profiles: cleanupStagedSkills', () => {
  test('removes staged dirs created during process', () => {
    const src = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-stage-cleanup-'));
    fs.writeFileSync(path.join(src, 'plan-phase.md'), '# plan\n');
    try {
  ...
  test('exit handler registers at most once across many calls', () => {
    cleanupStagedSkills();
    const src = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-stage-exit-handler-'));
    fs.writeFileSync(path.join(src, 'plan-phase.md'), '# plan\n');
    try {
  ...
  test('mid-copy failure removes partial staged dir and re-throws', () => {
    cleanupStagedSkills();
    const src = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-stage-fail-'));
    fs.writeFileSync(path.join(src, 'plan-phase.md'), '# plan\n');
    ...
    fs.mkdtempSync = (prefix, ...rest) => {
      ...
      if (typeof prefix === 'string' && prefix.endsWith('forge-minimal-skills-')) stagedDir = out;
      return out;
    };
    let copyCount = 0;
    fs.copyFileSync = (s, d) => {
...
describe('install: manifest records mode for both profiles', () => {
  function manifestModeAfterInstall(extraArgs) {
    const targetDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-manifest-mode-'));
    try {
      spawnSync(
        process.execPath,
...
describe('install-minimal-backcompat: --minimal and --profile=core produce same manifest', () => {
  function installAndGetManifest(extraArgs) {
    const targetDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-backcompat-'));
    try {
      spawnSync(
        process.execPath,
        [INSTALL_SCRIPT, '--claude', '--global', '--config-dir', targetDir, ...extraArgs],
      ...
      const markerPath = path.join(targetDir, '.forge-profile');
  ...
  test('--minimal writes .forge-profile marker "core"', () => {
    const r = installAndGetManifest(['--minimal']);
    assert.strictEqual(r.profileMarker, 'core');
  });

  test('default install writes .forge-profile marker "full"', () => {
    const r = installAndGetManifest([]);
    assert.strictEqual(r.profileMarker, 'full');
  });

  test('--profile=core writes .forge-profile marker "core"', () => {
    const r = installAndGetManifest(['--profile=core']);
    assert.strictEqual(r.profileMarker, 'core');
  });

  test('--profile=standard writes .forge-profile marker "standard"', () => {
...
describe('install: Codex full → minimal downgrade cleans stale agent state', () => {
  test('--minimal removes stale .toml agents and strips [agents.gsd-*] from config.toml', () => {
    const targetDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-codex-downgrade-'));
    try {
      const agentsDir = path.join(targetDir, 'agents');
      fs.mkdirSync(agentsDir, { recursive: true });
      fs.writeFileSync(path.join(agentsDir, 'forge-executor.md'), 'stale\n');
      fs.writeFileSync(path.join(agentsDir, 'forge-planner.md'), 'stale\n');
      fs.writeFileSync(path.join(agentsDir, 'forge-executor.toml'), 'name = "forge-executor"\n');
      fs.writeFileSync(path.join(agentsDir, 'forge-planner.toml'), 'name = "forge-planner"\n');
      fs.writeFileSync(path.join(agentsDir, 'my-custom-agent.md'), 'user owns this\n');
      const codexConfig = [
        '# user-owned setting',
        'model = "gpt-5"',
        '',
        '# GSD Agent Configuration — managed by forge-core installer',
        '[agents.forge-executor]',
        'cmd = "stale"',
        '',
        '[agents.forge-planner]',
        'cmd = "stale"',
        '',
      ].join('\n');
      ...
      assert.ok(!remaining.includes('forge-executor.md'));
      assert.ok(!remaining.includes('forge-planner.md'));
      assert.ok(!remaining.includes('forge-executor.toml'));
      assert.ok(!remaining.includes('forge-planner.toml'));
      assert.ok(remaining.includes('my-custom-agent.md'));

      const configPath = path.join(targetDir, 'config.toml');
      if (fs.existsSync(configPath)) {
        const config = fs.readFileSync(configPath, 'utf8');
        assert.ok(!config.includes('[agents.forge-executor]'));
        assert.ok(!config.includes('[agents.forge-planner]'));
...
describe('install: Claude full → minimal downgrade removes stale agents', () => {
  test('--minimal removes stale forge-*.md agents but preserves user-owned agents', () => {
    const targetDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-claude-downgrade-'));
    try {
      const agentsDir = path.join(targetDir, 'agents');
      fs.mkdirSync(agentsDir, { recursive: true });
      fs.writeFileSync(path.join(agentsDir, 'forge-executor.md'), 'stale\n');
      ...
      assert.ok(!remaining.includes('forge-planner.md'));
      assert.ok(remaining.includes('my-custom-agent.md'));
      assert.deepStrictEqual(remaining.filter(f => f.startsWith('forge-')), []);
    } finally {
      cleanup(targetDir);
    }
  });
...
describe('#1755: .sh hooks are copied and executable after install', () => {
  let tmpDir;
  beforeEach(() => { tmpDir = createTempDir('forge-hook-copy-'); });
...
describe('install.js uninstall hooks registry (typed assertions)', () => {
  ...
  test('forge-workflow-guard.js is in GSD_UNINSTALL_HOOKS', () => {
    assert.ok(
      GSD_UNINSTALL_HOOKS.includes('forge-workflow-guard.js'),
      'GSD_UNINSTALL_HOOKS must include forge-workflow-guard.js'
    );
  });

  test('phantom forge-check-update.sh is NOT in GSD_UNINSTALL_HOOKS', () => {
    assert.ok(
      !GSD_UNINSTALL_HOOKS.includes('forge-check-update.sh'),
      'GSD_UNINSTALL_HOOKS must not include the phantom forge-check-update.sh entry'
    );
  });

  test('GSD_UNINSTALL_HOOKS covers all 3 opt-in bash hooks', () => {
    const required = ['forge-session-state.sh', 'forge-validate-commit.sh', 'forge-phase-boundary.sh'];
  ...
  test('GSD_UNINSTALL_HOOKS covers core JS hooks', () => {
    const coreJsHooks = [
      'forge-check-update.js', 'forge-statusline.js', 'forge-session-state.sh',
      'forge-context-monitor.js', 'forge-phase-boundary.sh', 'forge-prompt-guard.js',
      'forge-read-guard.js', 'forge-validate-commit.sh', 'forge-workflow-guard.js',
    ];
...
describe('writeManifest includes .sh hooks', () => {
  let tmpDir;
  beforeEach(() => {
    tmpDir = createTempDir('forge-manifest-');
  ...
  test('manifest contains .sh hook entries', () => {
    ...
    const manifest = JSON.parse(fs.readFileSync(path.join(tmpDir, 'forge-file-manifest.json'), 'utf8'));
    for (const sh of EXPECTED_SH_HOOKS) {
...
describe('uninstall settings cleanup preserves user hooks', () => {
  const isGsdHook = (cmd) =>
    cmd && (cmd.includes('forge-check-update') || cmd.includes('forge-statusline') ||
      cmd.includes('forge-session-state') || cmd.includes('forge-context-monitor') ||
      cmd.includes('forge-phase-boundary') || cmd.includes('forge-prompt-guard') ||
      cmd.includes('forge-read-guard') || cmd.includes('forge-validate-commit') ||
      cmd.includes('forge-workflow-guard'));
  ...
  test('mixed entry preserves user hooks', () => {
    const entries = [{
      matcher: 'Bash',
      hooks: [
        { type: 'command', command: 'node /path/forge-prompt-guard.js' },
  ...
  test('entry with only GSD hooks is fully removed', () => {
    const entries = [{
      hooks: [
        { type: 'command', command: 'node /path/forge-check-update.js' },
        { type: 'command', command: 'node /path/forge-statusline.js' },
      ],
    }];
  ...
  test('non-array hook entries are preserved (#1825)', () => {
    const entries = [
      ...
      { matcher: 'Bash', hooks: [{ type: 'command', command: 'node /path/forge-prompt-guard.js' }] },
  ...
  test('all GSD hook names are recognised', () => {
    const cmds = [
      'node /path/forge-check-update.js', 'node /path/forge-statusline.js',
      'bash /path/forge-session-state.sh', 'node /path/forge-context-monitor.js',
      'bash /path/forge-phase-boundary.sh', 'node /path/forge-prompt-guard.js',
      'node /path/forge-read-guard.js', 'bash /path/forge-validate-commit.sh',
      'node /path/forge-workflow-guard.js',
    ];
...
describe('Codex legacy forge-update-check migration', () => {
  const src = fs.readFileSync(path.join(__dirname, '..', 'bin', 'install.js'), 'utf8');

  test('install.js strips legacy forge-update-check hook blocks', () => {
    assert.ok(src.includes('forge-update-check') && src.includes('replace('));
  });
  ...
  test('migration regex removes LF legacy hook block', () => {
    const legacyBlock = ['[features]', 'codex_hooks = true', '',
      ...
      'command = "node /old/path/forge-update-check.js"', ''].join('\n');
    let content = legacyBlock.replace(
      /\n# GSD Hooks\n\[\[hooks\]\]\nevent = "SessionStart"\ncommand = "node [^\n]*forge-update-check\.js"\n/g, '\n',
    );
    assert.ok(!content.includes('forge-update-check'));
