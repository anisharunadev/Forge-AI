...
const {
  ...
  slugify,
  zeroPad,
} = require('../forge-core/bin/lib/gsd2-import.cjs');

// ─── Fixture Builders ──────────────────────────────────────────────────────

/** Build a minimal but complete GSD-2 .forge/ directory in tmpDir. */
...
describe('parseGsd2', () => {
  ...
  test('handles missing .forge/milestones/ gracefully', () => {
    const gsdDir = path.join(tmpDir, '.gsd');
...
describe('forge-tools from-forge2 CLI', () => {
  let tmpDir;
  ...
  test('--dry-run returns preview without writing files', () => {
    ...
    const result = runGsdTools(['from-forge2', '--dry-run', '--raw'], tmpDir);
  ...
  test('writes .planning/ directory with correct structure', () => {
    makeGsd2Project(tmpDir);
    const result = runGsdTools(['from-forge2', '--raw'], tmpDir);
    assert.ok(result.success, result.error);
    const parsed = JSON.parse(result.output);
  ...
  test('errors when no .forge/ directory present', () => {
    const result = runGsdTools(['from-forge2', '--raw'], tmpDir);
    const parsed = JSON.parse(result.output);
    assert.strictEqual(parsed.success, false);
    assert.ok(parsed.error.includes('No .forge/'));
  });
  ...
  test('--force overwrites existing .planning/', () => {
    ...
    const result = runGsdTools(['from-forge2', '--force', '--raw'], tmpDir);
    const parsed = JSON.parse(result.output);
    assert.strictEqual(parsed.success, true);
    assert.ok(fs.existsSync(path.join(tmpDir, '.planning', 'ROADMAP.md')));
  });
  ...
  test('--path resolves target directory', () => {
    ...
    const result = runGsdTools(['from-forge2', '--path', projectDir, '--dry-run', '--raw'], tmpDir);
    assert.ok(result.success, result.error);
    const parsed = JSON.parse(result.output);
    assert.strictEqual(parsed.dryRun, true);
    assert.ok(parsed.preview.includes('PROJECT.md'));
  ...
  test('completion state: S01 done → [x] in ROADMAP.md', () => {
    makeGsd2Project(tmpDir);
    runGsdTools(['from-forge2', '--raw'], tmpDir);
