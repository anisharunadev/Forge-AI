 ...
 *
 * History:
 *   #3443 re-establishes `/forge:<cmd>` as canonical in Claude-facing source text.
 *   The source repo is authored for Claude command registration under
 *   `.claude/commands/forge/` (namespaced slash commands), while non-Claude runtimes
 *   perform install-time conversion (for example `/forge:<cmd>` -> `/forge-<cmd>`).
 *
 ...
 *     templates/, hooks/, .clinerules): uses `/forge:<cmd>` (colon).
 ...
 *     *.generated.cjs, bug-3584 test file): use `/forge-<cmd>` (hyphen) per
 *     bug-3584's contract. Those files are EXCLUDED from this scan.
 *
 * Scoped invariant enforced here:
 *   No `/forge-<cmd>` pattern in Claude-facing source files, EXCLUDING the
...
// Runtime-emitter contexts: these files intentionally emit `/forge-<cmd>` (hyphen)
...
const SEARCH_DIRS = [
  // NOTE: forge-core/bin/lib is intentionally EXCLUDED from SEARCH_DIRS.
  ...
  path.join(ROOT, 'forge-core', 'templates'),
  COMMANDS_DIR,
...
const retiredPattern = new RegExp(`/forge-(${cmdNames.join('|')})(?=[^a-zA-Z0-9_-]|$)`);
...
describe('slash-command namespace invariant (#3443)', () => {
  test('commands/forge/ directory contains known command files', () => {
    assert.ok(cmdNames.length > 0, 'commands/forge/ must contain .md files');
  ...
  // forge-core/bin/lib/ is excluded entirely — runtime-slash.cjs and
  ...
  test('no /forge-<cmd> retired syntax in Claude-facing source files (scoped — excludes runtime-emitter contexts)', () => {
    const violations = [];
    ...
    assert.strictEqual(
      violations.length,
      0,
      `Found ${violations.length} retired /forge-<cmd> reference(s) — use /forge:<cmd> instead:\n${violations.slice(0, 10).join('\n')}`,
    );
  });
  ...
  describe('fix-slash-commands transformer behavior', () => {
    ...
    test('rewrites /forge-<cmd> to /forge:<cmd>', () => {
      const out = transformContent('See /forge-plan-phase for details.', liveCmdNames);
      assert.ok(out.includes('/forge:plan-phase'), `expected /forge:plan-phase, got: ${out}`);
      assert.ok(!out.includes('/forge-plan-phase'), `dash form must not survive, got: ${out}`);
    });

    test('rewrites multiple occurrences in one pass', () => {
      const out = transformContent('Run /forge-plan-phase then /forge-execute-phase.', liveCmdNames);
      assert.ok(out.includes('/forge:plan-phase'));
      assert.ok(out.includes('/forge:execute-phase'));
      assert.ok(!out.match(/\/forge-[a-z]/), `no dash form may remain, got: ${out}`);
    });

    test('does not rewrite canonical colon form (idempotent)', () => {
      const input = '/forge:plan-phase is the canonical name.';
      assert.strictEqual(transformContent(input, liveCmdNames), input,
        'transformer must be a no-op when input is already canonical');
    });

    test('does not rewrite forge-sdk or forge-tools (not slash commands)', () => {
      const input = 'Run /forge-sdk query and /forge-tools init.';
      assert.strictEqual(transformContent(input, liveCmdNames), input,
        'transformer must leave non-command identifiers alone');
    });

    test('respects word boundary — does not rewrite /forge-plan-phase-extra', () => {
      const out = transformContent('/forge-plan-phase-extra', liveCmdNames);
      assert.strictEqual(out, '/forge-plan-phase-extra',
  ...
  test('transformer leaves non-command identifiers untouched', () => {
    const { transformContent } = require(path.join(ROOT, 'scripts', 'fix-slash-commands.cjs'));
    const sample = 'Use /forge-sdk query and node bin/forge-tools.cjs';
    assert.strictEqual(
      transformContent(sample, cmdNames),
      sample,
      'forge-sdk and forge-tools are not slash commands and must remain untouched'
    );
  });
});
