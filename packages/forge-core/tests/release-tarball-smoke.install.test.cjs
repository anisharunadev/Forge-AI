...
describe('release-tarball-smoke', () => {
  ...
  before(async () => {
    ...
    packDir = createTempDir('forge-smoke-pack-');
    installPrefix = createTempDir('forge-smoke-prefix-');
    fixtureDir = createTempDir('forge-smoke-fixture-');
  ...
  // Also verifies that `forge-core --local --claude` (init) succeeds in
