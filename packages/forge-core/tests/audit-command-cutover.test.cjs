 ...
 * `case 'audit-uat':` and `case 'audit-open':` arms in forge-tools.cjs to the
...
const { routeAuditUat, routeAuditOpen } = require('../forge-core/bin/lib/audit-command-router.cjs');
...
describe('audit cutover: dispatch path (default-case → capability registry)', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempProject('forge-audit-cutover-');
  });
...
describe('audit cutover: output shape equivalence', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempProject('forge-audit-behavior-');
  });
  ...
  test('audit-uat --raw flag passes through (does not break dispatch)', () => {
    ...
    // --raw is a forge-tools global flag; it modifies output encoding but
...
describe('audit cutover: GSD_JSON_ERRORS mode (both commands succeed without structured error)', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempProject('forge-audit-jsonerr-');
  });
