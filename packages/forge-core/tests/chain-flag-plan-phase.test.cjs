...
describe('plan-phase chain flag preservation (#1620)', () => {
  const planPath = path.join(__dirname, '..', 'forge-core', 'workflows', 'plan-phase.md');
  const discussPath = path.join(__dirname, '..', 'forge-core', 'workflows', 'discuss-phase.md');
  // After the discuss-phase/modes split (#717), discuss-phase chain logic moved to modes/chain.md.
  const discussChainPath = path.join(__dirname, '..', 'forge-core', 'workflows', 'discuss-phase', 'modes', 'chain.md');
  const readDiscuss = () => {
