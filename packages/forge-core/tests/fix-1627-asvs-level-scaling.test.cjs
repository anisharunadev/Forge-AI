...
const REFS_DIR = path.join(ROOT, 'forge-core', 'references');
...
describe('SECURE: ASVS level scaling (#1627)', () => {
  ...
  describe('security-asvs-levels.md reference', () => {
    ...
    test('file exists', () => {
      assert.ok(fs.existsSync(refPath), 'forge-core/references/security-asvs-levels.md must exist');
    });
  ...
  // ── 2. forge-planner.md — no hardcoded L1 in disposition ──────────────────

  describe('forge-planner.md security disposition', () => {
    ...
    test('planner is under the 49152-char cap', () => {
      ...
      assert.ok(
        ...
        `forge-planner.md must be < 49152 chars (LF-normalized); got ${content.length}`
      );
    });
  });

  // ── 3. forge-security-auditor.md — scaled verification depth ──────────────

  describe('forge-security-auditor.md verification depth', () => {
  ...
  describe('secure-phase.md short-circuit conditioned on asvs_level', () => {
    const wfPath = path.join(ROOT, 'forge-core', 'workflows', 'secure-phase.md');
