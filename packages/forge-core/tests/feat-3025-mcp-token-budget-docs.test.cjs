 ...
 *
 * Verifies that forge-core/references/context-budget.md contains the
...
const CONTEXT_BUDGET_MD = path.join(ROOT, 'forge-core', 'references', 'context-budget.md');
...
function parseMcpBudgetSection(section) {
  ...
  const explainsHarnessNotGsd =
    /(gsd does(?:n[''’]t| not) (own|manage|control)|harness (concern|setting|controlled)|not a forge (setting|knob))/i.test(stripped);
