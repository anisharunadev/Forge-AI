...
//
// Fix introduces forge-core/bin/lib/runtime-homes.cjs with first-class
// support for all 15 supported runtimes, including:
//   - hermes: nested skills/forge/<skillName>/ layout (#2841)
...
describe('bug #3126: runtime-homes getGlobalConfigDir — defaults', () => {
  ...
  for (const [runtime, expected] of defaults) {
    test(`${runtime} default configDir`, () => {
      ...
      const { runtimes: _reg3126 } = require(path.join(ROOT, 'forge-core', 'bin', 'lib', 'capability-registry.cjs'));
...
describe('bug #3126: runtime-homes env-var overrides', () => {
  ...
  test('antigravity detects 2.x IDE dir when legacy dir is absent', () => {
    const home = require('node:fs').mkdtempSync(path.join(os.tmpdir(), 'forge-antigravity-home-'));
    try {
...
describe('bug #3126: runtime-homes getGlobalSkillsBase', () => {
  ...
  test('hermes: skills at <configDir>/skills/forge (nested layout #2841)', () => {
    withEnv('HERMES_HOME', undefined, () => {
      assert.strictEqual(
        getGlobalSkillsBase('hermes'),
        path.join(os.homedir(), '.hermes', 'skills', 'forge'),
      );
    });
  });
...
describe('bug #3126: runtime-homes getGlobalSkillDir', () => {
  test('cursor: <configDir>/skills/<skillName>', () => {
    withEnv('CURSOR_CONFIG_DIR', undefined, () => {
      assert.strictEqual(
        getGlobalSkillDir('cursor', 'forge-executor'),
        path.join(os.homedir(), '.cursor', 'skills', 'forge-executor'),
      );
    });
  });
  test('hermes: <configDir>/skills/forge/<skillName>', () => {
    withEnv('HERMES_HOME', undefined, () => {
      assert.strictEqual(
        getGlobalSkillDir('hermes', 'forge-executor'),
        path.join(os.homedir(), '.hermes', 'skills', 'forge', 'forge-executor'),
      );
    });
  });
  test('cline: returns ~/.cline/skills/forge-executor (skills-capable since v3.48.0 — #782)', () => {
    withEnv('CLINE_CONFIG_DIR', undefined, () => {
      assert.strictEqual(
        getGlobalSkillDir('cline', 'forge-executor'),
        path.join(os.homedir(), '.cline', 'skills', 'forge-executor'),
      );
    });
  });
});
...
describe('bug #3126: init.cjs uses runtime-homes not hardcoded .claude', () => {
  test('init.cjs has no hardcoded globalSkillsBase assignment to ~/.claude/skills', () => {
    ...
    const src = fs.readFileSync(
      path.join(ROOT, 'forge-core', 'bin', 'lib', 'init.cjs'),
      'utf8',
    );
    assert.ok(
