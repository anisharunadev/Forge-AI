 ...
 * the YAML frontmatter override format, and is referenced by forge-verifier.md.
 */
...
describe('verification overrides reference (#1747)', () => {
  ...
  describe('forge-core/references/verification-overrides.md', () => {
    const refPath = path.join(ROOT, 'forge-core', 'references', 'verification-overrides.md');
    let content;
    ...
    test('documents milestone audit surfacing', () => {
      content = content || fs.readFileSync(refPath, 'utf-8');
      assert.ok(
        content.includes('forge-audit-milestone') || content.includes('audit-milestone') || content.includes('milestone'),
  ...
  describe('agents/forge-verifier.md references overrides', () => {
    const verifierPath = path.join(ROOT, 'agents', 'forge-verifier.md');
    let verifierContent;

    test('forge-verifier.md exists', () => {
      assert.ok(fs.existsSync(verifierPath), 'forge-verifier.md should exist');
    ...
    test('references verification-overrides.md in required_reading', () => {
      ...
      assert.ok(
        ...
        'forge-verifier.md should reference verification-overrides.md'
      );
    });
    ...
    test('verifier includes Step 3b for override check before FAIL', () => {
      ...
      assert.ok(
        ...
        'forge-verifier.md should include a Step 3b override check'
      );
    });
    ...
    test('verifier frontmatter template includes overrides_applied', () => {
      ...
      assert.ok(
        ...
        'forge-verifier.md frontmatter template should include overrides_applied counter'
      );
    });
  });
});
