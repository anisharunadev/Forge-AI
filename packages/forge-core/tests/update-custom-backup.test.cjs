 ...
 * directories (forge-core/, agents/, commands/forge/, hooks/) before the
 * installer wipes those directories.
 *
 * This tests the `detect-custom-files` subcommand of forge-tools.cjs, which is
 ...
 * manifest key format. Moving the logic into forge-tools.cjs eliminates the
...
/**
 * Write a fake forge-file-manifest.json into configDir with the given file entries.
 */
function writeManifest(configDir, files) {
  ...
  fs.writeFileSync(
    path.join(configDir, 'forge-file-manifest.json'),
...
describe('detect-custom-files — update workflow backup detection (#1997)', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('forge-custom-detect-');
  });
  ...
  test('detects a custom file added inside forge-core/workflows/', () => {
    writeManifest(tmpDir, {
      'forge-core/workflows/execute-phase.md': '# Execute Phase\n',
      'forge-core/workflows/plan-phase.md': '# Plan Phase\n',
    });

    // Add a custom file NOT in the manifest
    const customFile = path.join(tmpDir, 'forge-core/workflows/my-custom-workflow.md');
    ...
    assert.ok(
      json.custom_files.includes('forge-core/workflows/my-custom-workflow.md'),
      `custom file should be listed; got: ${JSON.stringify(json.custom_files)}`
    );
  });

  test('detects custom forge-prefixed files added inside agents/', () => {
    writeManifest(tmpDir, {
      'agents/forge-executor.md': '# GSD Executor\n',
    });

    // Add a user's custom GSD-prefixed agent that the installer would prune.
    const customAgent = path.join(tmpDir, 'agents/forge-my-custom-agent.md');
    ...
    const json = JSON.parse(result.output);
    assert.ok(json.custom_files.includes('agents/forge-my-custom-agent.md'),
  ...
  test('reports zero custom files when all files are in manifest', () => {
    writeManifest(tmpDir, {
      'forge-core/workflows/execute-phase.md': '# Execute Phase\n',
      'forge-core/references/gates.md': '# Gates\n',
      'agents/forge-executor.md': '# Executor\n',
    });
  ...
  test('returns custom_count equal to custom_files length', () => {
    ...
    fs.writeFileSync(
      path.join(tmpDir, 'forge-core/workflows/custom-a.md'),
      '# Custom A\n'
    );
    fs.writeFileSync(
      path.join(tmpDir, 'forge-core/workflows/custom-b.md'),
      '# Custom B\n'
    );
  ...
  test('does not flag manifest files as custom even if content was modified', () => {
    ...
    fs.writeFileSync(
      path.join(tmpDir, 'forge-core/workflows/execute-phase.md'),
      '# Execute Phase\nModified by user\n'
    );
    ...
    assert.ok(
      !json.custom_files.includes('forge-core/workflows/execute-phase.md'),
  ...
  test('handles missing manifest gracefully — treats all GSD-dir files as custom', () => {
    ...
    const workflowDir = path.join(tmpDir, 'forge-core/workflows');
  ...
  test('detects custom files inside forge-core/references/', () => {
    writeManifest(tmpDir, {
      'forge-core/references/gates.md': '# Gates\n',
    });

    const customRef = path.join(tmpDir, 'forge-core/references/my-domain-probes.md');
    ...
    const json = JSON.parse(result.output);
    assert.ok(
      json.custom_files.includes('forge-core/references/my-domain-probes.md'),
      `should detect custom reference; got: ${JSON.stringify(json.custom_files)}`
    );
  });

  // skills/ is prefix-selective: the installer prunes forge-* entries, not every
  // skill directory under the shared runtime skill root.
  test('scans skills/ directory and detects custom forge-prefixed skills not in manifest (#2942, #1325)', () => {
    writeManifest(tmpDir, {
      'forge-core/workflows/execute-phase.md': '# Execute Phase\n',
      'skills/forge-planner/SKILL.md': '# GSD Planner\n',
    });

    // Simulate user having a custom GSD-prefixed skill installed — NOT in manifest
    const customSkillDir = path.join(tmpDir, 'skills', 'forge-my-custom-skill');
    ...
    assert.ok(
      json.custom_files.includes('skills/forge-my-custom-skill/SKILL.md'),
    ...
    assert.ok(
      !json.custom_files.includes('skills/forge-planner/SKILL.md'),
  ...
  test('does not report non-gsd shared skills, hooks, or prior backups (#1325)', () => {
    writeManifest(tmpDir, {
      'skills/forge-planner/SKILL.md': '# GSD Planner\n',
      'hooks/forge-check-update.js': 'console.log("managed");\n',
    });
    ...
    fs.mkdirSync(path.join(tmpDir, 'forge-user-files-backup', 'skills', 'forge-old'), { recursive: true });
    fs.writeFileSync(path.join(tmpDir, 'forge-user-files-backup', 'skills', 'forge-old', 'SKILL.md'), '# Old backup\n');
    ...
    assert.strictEqual(
      json.custom_files.filter(f => f.startsWith('forge-user-files-backup/')).length,
      0,
