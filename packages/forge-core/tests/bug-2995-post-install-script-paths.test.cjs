...
function fixtureRepo({ workflows, files }) {
  ...
  // files:     [ 'forge-core/bin/x.cjs', ... ]  — files to create in repo
  const repoRoot = fs.mkdtempSync(path.join(tmpRoot, 'repo-'));
  const workflowsDir = path.join(repoRoot, 'forge-core', 'workflows');
...
before(() => { tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-2995-')); });
...
describe('Bug #2995: post-install script-paths audit (#2995)', () => {
  ...
  test('returns { ok: true, findings: [] } when workflow refs an existing, installed-path script', () => {
    const { repoRoot, workflowsDir } = fixtureRepo({
      workflows: {
        'good.md': 'node "${GSD_HOME}/forge-core/bin/foo.cjs" --json\n',
      },
      files: ['forge-core/bin/foo.cjs'],
    });
    const r = auditWorkflowScriptPaths({
      workflowsDir,
      repoRoot,
      installedPrefixes: ['forge-core', 'commands', 'agents', 'hooks'],
    });
...
describe('Bug #2995: detection paths', () => {
  ...
  test('reports MISSING_FROM_REPO when the referenced file does not exist in the repo', () => {
    const { repoRoot, workflowsDir } = fixtureRepo({
      workflows: {
        'foo.md': 'node "${GSD_HOME}/forge-core/bin/typo.cjs" --json\n',
      },
      files: [],
    });
    const r = auditWorkflowScriptPaths({
      workflowsDir,
      repoRoot,
      installedPrefixes: ['forge-core'],
    });
    assert.equal(r.ok, false);
    assert.equal(r.findings.length, 1);
    assert.deepEqual(r.findings[0], {
      workflow: 'foo.md',
      path: 'forge-core/bin/typo.cjs',
  ...
  test('handles ${GSD_HOME:-$HOME/.claude}/... default-fallback syntax', () => {
    const { repoRoot, workflowsDir } = fixtureRepo({
      workflows: {
        'a.md': 'node "${GSD_HOME:-$HOME/.claude}/forge-core/bin/x.cjs"\n',
      },
      files: ['forge-core/bin/x.cjs'],
    });
    const r = auditWorkflowScriptPaths({
      workflowsDir,
      repoRoot,
    ...
    const { repoRoot, workflowsDir } = fixtureRepo({
      workflows: {
        'multi.md': [
          ...
          'node "${GSD_HOME}/forge-core/bin/b.cjs"',
          'node "${GSD_HOME}/forge-core/bin/missing.cjs"',
        ].join('\n') + '\n',
      },
      files: ['scripts/a.cjs', 'forge-core/bin/b.cjs'],
    });
    const r = auditWorkflowScriptPaths({
      workflowsDir,
      repoRoot,
...
describe('Bug #2995: real workflow audit', () => {
  ...
  const INSTALLED_PREFIXES = [
    'forge-core',  // workflows, references, bin/lib, templates
    'commands',       // commands/forge/*.md (Claude Code local + Gemini global)
    'skills',         // skills/forge-*/SKILL.md (Claude Code 2.1.88+ global, Codex, etc.)
    'agents',         // agents/forge-*.md
    'hooks',          // hooks/forge-*.{sh,js}
  ];
  ...
  // forge-core/bin/ which IS an installed prefix, closing the gap.)
  const KNOWN_GAPS = new Set();

  test('no NEW workflow refs fail to resolve at the deployed path (KNOWN_GAPS allow-listed)', () => {
    const r = auditWorkflowScriptPaths({
      workflowsDir: require('node:path').join(ROOT, 'forge-core', 'workflows'),
      repoRoot: ROOT,
  ...
  test('a reference that is both not-installed AND missing-from-repo emits both findings (no short-circuit)', () => {
    ...
    const r = auditWorkflowScriptPaths({
      workflowsDir,
      repoRoot,
      installedPrefixes: ['forge-core', 'agents', 'hooks', 'commands'],
    });
    assert.equal(r.ok, false);
