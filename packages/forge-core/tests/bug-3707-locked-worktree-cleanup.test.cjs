...
//   3. quick.md and execute-phase.md must wire forge-sdk query worktree.reap-orphans at startup.
...
const {
  ...
  reapOrphanWorktrees,
} = require('../forge-core/bin/lib/worktree-safety.cjs');
...
describe('bug-3707: executeWorktreeWaveCleanupPlan unlocks and retries on locked worktree', () => {
  ...
  beforeEach(() => {
    tmpBase = fs.mkdtempSync(path.join(resolvedTmpDir(), 'forge-3707-cleanup-'));
  });
...
describe('bug-3707: reapOrphanWorktrees', () => {
  ...
  beforeEach(() => {
    tmpBase = fs.mkdtempSync(path.join(resolvedTmpDir(), 'forge-3707-reap-'));
  });
...
describe('bug-3707: startup orphan sweep is wired into workflow entry points', () => {
  ...
  const EXECUTE_PHASE_PATH = path.join(__dirname, '..', 'forge-core', 'workflows', 'execute-phase.md');
  ...
  test('quick.md calls worktree.reap-orphans at startup when USE_WORKTREES is not false', () => {
    ...
    assert.ok(
      ...
      'quick.md must call forge-sdk query worktree.reap-orphans at startup'
    );
  ...
  test('execute-phase.md calls worktree.reap-orphans at startup when USE_WORKTREES is not false', () => {
    ...
    assert.ok(
      content.includes('worktree.reap-orphans'),
      'execute-phase.md must call forge-sdk query worktree.reap-orphans at startup'
    );
    assert.ok(
      /USE_WORKTREES.*!=.*false[\s\S]{0,200}worktree\.reap-orphans/m.test(content) ||
      /worktree\.reap-orphans[\s\S]{0,200}USE_WORKTREES.*!=.*false/m.test(content),
  ...
  test('worktree-safety module exports reapOrphanWorktrees', () => {
    const mod = require('../forge-core/bin/lib/worktree-safety.cjs');
...
describe('bug-3707: reapOrphanWorktrees — nowMs clock-injection BOUNDARY tests (#1191)', () => {
  ...
  beforeEach(() => {
    tmpBase = fs.mkdtempSync(path.join(resolvedTmpDir(), 'forge-3707-nowms-'));
  });
...
describe('bug-3707: reapOrphanWorktrees — adversarial edge cases', () => {
  ...
  beforeEach(() => {
    tmpBase = fs.mkdtempSync(path.join(resolvedTmpDir(), 'forge-3707-adv-'));
  });
