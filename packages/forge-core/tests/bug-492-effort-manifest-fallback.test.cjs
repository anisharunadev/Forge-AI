 ...
 * them to a per-test ~/.forge/defaults.json rooted under a tmpHome, pointed at
 ...
 * hits the ~/.forge/defaults.json branch. A test-scoped defaults.json injects
...
function createBareTmpDir(prefix = 'forge-test-') {
  return fs.mkdtempSync(path.join(os.tmpdir(), prefix));
}

/** Create a temp home dir and write effort config into .forge/defaults.json */
function createTmpHomeWithEffort(effortConfig) {
  const tmpHome = createBareTmpDir('forge-home-');
...
describe('#492 manifest effort fallback', () => {
  ...
  test('routing_tier_defaults manifest fallback still works when no config and no defaults.json', (t) => {
    ...
    // forge-planner's default tier is "heavy"; manifest routing_tier_defaults.heavy = "xhigh"
  ...
  test('global-defaults effort.agent_overrides wins over routing_tier_defaults when no project config', (t) => {
    // bare tmpDir (no .planning/) → loadConfig reads ~/.forge/defaults.json
    // which supplies effort.agent_overrides → resolveEffortInternal returns that value.
    const tmpDir = createBareTmpDir();
    const tmpHome = createTmpHomeWithEffort({ agent_overrides: { 'forge-planner': 'max' } });
    process.env.GSD_HOME = tmpHome;
    t.after(() => {
      delete process.env.GSD_HOME;
      cleanup(tmpDir);
    ...
    assert.strictEqual(resolveEffortInternal(tmpDir, 'forge-planner'), 'max');
  });
  ...
  test('global-defaults agent_overrides takes precedence over routing_tier_defaults', (t) => {
    ...
    const tmpHome = createTmpHomeWithEffort({
      agent_overrides: { 'forge-planner': 'minimal' },
    ...
    assert.strictEqual(resolveEffortInternal(tmpDir, 'forge-planner'), 'minimal');
  });
});
