...
//   1. Cache-stale: bare `npx -y @forge-ai/forge-core@<tag>` hits npx's local
//      cache and may pull an older version instead of the target tag.
//   2. Token-routing: Bash-tool wrappers misroute the `@` token in
//      `@forge-ai/forge-core@<tag>`, causing npm to error with
//      "Unknown command: @forge-ai/forge-core@<tag>".
//
// The robust form is:
//   npx -y --package=@forge-ai/forge-core@"$TAG" -- forge-core $ARGS
//
...
const UPDATE_WF = path.join(ROOT, 'forge-core', 'workflows', 'update.md');

const src = fs.readFileSync(UPDATE_WF, 'utf8');

test('bug #3130: update.md contains no bare npx invocations (cache-stale form)', () => {
  // Any occurrence of `npx -y @forge-ai/forge-core@<something>` without `--package=`
  // is the stale form that triggers the two failure modes.
  const stale = (src.match(/npx -y @opengsd\/forge-core@\S+[^\n]*/g) || []);
  assert.deepEqual(
    stale,
    [],
...
test('bug #3130: update.md has >=3 robust npx invocations (--package= + -- separator)', () => {
  ...
  const robust = (src.match(/npx -y --package=@opengsd\/forge-core@\S+ -- forge-core/g) || []);
  assert.ok(
    robust.length >= 3,
