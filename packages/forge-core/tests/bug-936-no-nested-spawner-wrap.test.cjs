'use strict';
/**
 * Structural guard — bug(#936): plan-review-convergence wrapped forge-plan-phase
 * in Agent() at TWO sites (initial planning + replan). On Claude Code, a depth-1
 * Agent has no Agent tool, so plan-phase cannot spawn forge-planner / forge-plan-checker
...
// We build BOTH the bare slug set and the forge-prefixed skill-name set.
...
// Skill invocations use "gsd-<slug>" (e.g. forge-plan-phase, forge-execute-phase).
// Build the regex from the prefixed names so it actually matches what workflows write.
const SPAWNER_GSD_NAMES = new Set([...SPAWNER_BASE_SLUGS].map((s) => `forge-${s}`));

// Build a regex that matches Skill(skill='forge-<spawner>') or Skill(skill="gsd-<spawner>")
...
describe('bug-936 — no workflow wraps a spawner skill inside Agent() without a RUNTIME carve-out', () => {
  test('spawner set is non-empty (self-check: subagent_type= grep must find files)', () => {
    ...
    // forge-plan-phase must be in the prefixed set used by the regex
    assert.ok(SPAWNER_GSD_NAMES.has('forge-plan-phase'), 'forge-plan-phase must be in SPAWNER_GSD_NAMES — the prefixed form used in Skill() calls');
  });
...
describe('bug-936 — plan-review-convergence runs plan-phase inline, not inside Agent()', () => {
  ...
  test('plan-review-convergence does NOT wrap forge-plan-phase inside Agent()', () => {
    // The anti-pattern: Agent( block whose body contains Skill(skill='forge-plan-phase')
    ...
    assert.deepStrictEqual(
      ...
      [],
      'plan-review-convergence must NOT wrap forge-plan-phase inside Agent(). ' +
      'Run it inline (bare Skill() at depth 0) so it can spawn forge-planner/forge-plan-checker. ' +
      'See: bug #936'
    );
  });

  test('plan-review-convergence calls forge-plan-phase inline (bare Skill call outside Agent block)', () => {
    // After the fix: at least one bare Skill(skill="forge-plan-phase") must appear
    ...
    const hasInlineCall = /Skill\(\s*skill=["']forge-plan-phase["']/.test(masked);
    assert.ok(
      hasInlineCall,
  ...
  test('plan-review-convergence still wraps forge-review inside Agent() (leaf — isolation is correct)', () => {
    // forge-review is a leaf (shells out via Bash, no subagent_type) so the Agent wrap is fine and intentional.
    const blocks = extractAgentBlocks(CONVERGENCE);
    const reviewWrap = blocks.some((b) => /Skill\(\s*skill=['"]forge-review['"]/.test(b.blockText));
    assert.ok(reviewWrap, 'forge-review must still be wrapped in Agent() — it is a Bash leaf and isolation is intentional');
  });
});
