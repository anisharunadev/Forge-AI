...
const { writeSurface, readSurface, resolveSurface, listSurface, applySurface } = require('../forge-core/bin/lib/surface.cjs');
const { loadSkillsManifest, writeActiveProfile, resolveProfile } = require('../forge-core/bin/lib/install-profiles.cjs');
const { resolveRuntimeArtifactLayout } = require('../forge-core/bin/lib/runtime-artifact-layout.cjs');
const { CLUSTERS, allClusteredSkills } = require('../forge-core/bin/lib/clusters.cjs');
...
function tmpDir(prefix) {
  return fs.mkdtempSync(path.join(os.tmpdir(), prefix || 'forge-ral-surf-'));
}

function createFixtureRuntime() {
  const base = createTempDir('forge-surface-apply-');
  const runtimeConfigDir = base;
  // #1367: claude local uses flat commands/ (not commands/forge/) — commandsDir is commands/.
...
describe('applySurface', () => {
  test('core profile: only core skills appear in commandsDir', (t) => {
    // #1367: claude local uses flat forge-<stem>.md files at commands/ (not commands/forge/<stem>.md).
    ...
    // After #1367: files are forge-<stem>.md (not bare stem.md). Strip the forge- prefix
    // to check against the REAL_COMMANDS_DIR (which still uses bare names).
    const files = fs.readdirSync(commandsDir).filter(f => f.startsWith('forge-') && f.endsWith('.md'));
    for (const file of files) {
      const bareName = file.slice('forge-'.length); // forge-help.md → help.md
      assert.ok(fs.existsSync(path.join(REAL_COMMANDS_DIR, bareName)), `unexpected file: ${file} (no source: ${bareName})`);
    }
    const expectedCore = [...resolved.skills].map(stem => `forge-${stem}.md`).sort();
  ...
  test('removes superseded files when profile shrinks', (t) => {
    ...
    // #1367: files are forge-<stem>.md in flat commands/
    const afterStandard = new Set(fs.readdirSync(commandsDir).filter(f => f.startsWith('forge-') && f.endsWith('.md')));
    ...
    const afterCore = new Set(fs.readdirSync(commandsDir).filter(f => f.startsWith('forge-') && f.endsWith('.md')));

    assert.ok(afterCore.size <= afterStandard.size, 'core should have fewer or equal files than standard');

    const expectedCore = [...resolvedCore.skills].map(stem => `forge-${stem}.md`).sort();
    assert.deepStrictEqual(
      [...afterCore].sort(),
      expectedCore,
    ...
    for (const file of afterCore) {
      const bareName = file.slice('forge-'.length);
      assert.ok(
  ...
  test('adds missing skill files from install source', (t) => {
    ...
    // #1367: flat forge-<stem>.md files at commands/ (not commands/forge/<stem>.md)
    assert.ok(
      fs.existsSync(path.join(commandsDir, 'forge-help.md')),
      'forge-help.md should be copied from install source (#1367: flat hyphen layout)'
    );
    assert.ok(
      fs.existsSync(path.join(commandsDir, 'forge-new-project.md')),
      'forge-new-project.md should be copied from install source (#1367: flat hyphen layout)'
    );
  });

  test('_syncGsdDir skills kind: adds missing skill dirs, removes stale prefix-matched dirs, preserves foreign dirs', (t) => {
    const { _syncGsdDir } = require('../forge-core/bin/lib/surface.cjs');

    const base = createTempDir('forge-surface-skills-');
    t.after(() => cleanup(base));
    const stagedDir = path.join(base, 'staged');
    const destDir = path.join(base, 'dest');
    fs.mkdirSync(destDir, { recursive: true });

    const stem1 = 'forge-help';
    const stem2 = 'forge-update';
    ...
    const staleDir = path.join(destDir, 'forge-old-skill');
    ...
    const skillsKind = { kind: 'skills', destSubpath: 'skills', prefix: 'forge-', stage: () => stagedDir };

    // Build a minimal manifest that includes the GSD-owned stems so that the
    // manifest-membership gate (Finding 1 fix) correctly identifies forge-old-skill
    // as GSD-owned and prunes it. Without a manifest the new code conservatively
    // preserves all forge-* dirs it cannot confirm are GSD-owned.
    ...
    assert.ok(fs.existsSync(path.join(destDir, stem1, 'SKILL.md')), 'forge-help/SKILL.md should be copied');
    assert.ok(fs.existsSync(path.join(destDir, stem2, 'SKILL.md')), 'forge-update/SKILL.md should be copied');
    // stale forge- dir removed (it's in the manifest so it is GSD-owned, but not in staged set)
    assert.ok(!fs.existsSync(staleDir), 'stale forge-old-skill dir should be removed');
  ...
  test('applySurface recreates missing destination directories', (t) => {
    const base = createTempDir('forge-surface-missing-dest-');
    t.after(() => cleanup(base));
    const runtimeConfigDir = base;
    writeActiveProfile(runtimeConfigDir, 'core');
    writeSurface(runtimeConfigDir, {
    ...
    // #1367: claude local uses flat commands/ (not commands/forge/)
    const commandsDir = path.join(runtimeConfigDir, 'commands');
    assert.ok(fs.existsSync(commandsDir), 'commands/ dir should be created even if initially absent');
    const files = fs.readdirSync(commandsDir).filter(f => f.startsWith('forge-') && f.endsWith('.md'));
    assert.ok(files.length > 0, 'commands/ should contain staged skill files (forge-*.md)');
    assert.ok(files.includes('forge-help.md'), 'forge-help.md should be present after applySurface on missing dest');
  });

  test('Hermes profile shrink: stale GSD skill dirs are removed; user skills preserved', (t) => {
    const { _syncGsdDir } = require('../forge-core/bin/lib/surface.cjs');

    const base = createTempDir('forge-surface-hermes-shrink-');
    t.after(() => cleanup(base));
    const stagedDir = path.join(base, 'staged');
    const destDir = path.join(base, 'dest');
    fs.mkdirSync(destDir, { recursive: true });

    fs.mkdirSync(path.join(stagedDir, 'forge-executor'), { recursive: true });
    fs.writeFileSync(path.join(stagedDir, 'forge-executor', 'SKILL.md'), '# executor\n', 'utf8');

    fs.mkdirSync(path.join(destDir, 'forge-executor'), { recursive: true });
    fs.writeFileSync(path.join(destDir, 'forge-executor', 'SKILL.md'), '# executor\n', 'utf8');
    fs.mkdirSync(path.join(destDir, 'forge-planner'), { recursive: true });
    fs.writeFileSync(path.join(destDir, 'forge-planner', 'SKILL.md'), '# planner\n', 'utf8');
    ...
    assert.ok(
      fs.existsSync(path.join(destDir, 'forge-executor', 'SKILL.md')),
      'forge-executor should be kept (in staged set)'
    );
    assert.ok(
      !fs.existsSync(path.join(destDir, 'forge-planner')),
      'forge-planner should be removed (in manifest but not in staged set — stale GSD skill)'
    );
    assert.ok(
  ...
  test('_syncGsdDir skills kind (hermes): preserves non-GSD user dir under skills/forge/ when kindPrefix is empty', (t) => {
    const { _syncGsdDir } = require('../forge-core/bin/lib/surface.cjs');

    const base = createTempDir('forge-surface-hermes-');
    t.after(() => cleanup(base));
    const stagedDir = path.join(base, 'staged');
    const destDir = path.join(base, 'dest');
    fs.mkdirSync(destDir, { recursive: true });
  ...
  // Regression guard for #816: applySurface must write forge-prefixed command files
  ...
  // `commands/`, augment `commands/`, kilo `command/`) with kind.prefix='forge-'.
  // _copyStaged names files `forge-<stem>.md` but the buggy _syncGsdDir copies
  // them as `<stem>.md` (unprefixed) and also deletes ALL .md files not in the
  // staged set, including user files.
  test('applySurface writes forge-prefixed command files matching install and preserves user commands (#816)', (t) => {
    const configDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-surface-816-'));
    ...
    // (a) At least one forge-prefixed command file must exist — on buggy code only
    //     unprefixed files like 'help.md' are written, so this assertion fails.
    assert.ok(
      files.some(f => f.startsWith('forge-') && f.endsWith('.md')),
      '#816: applySurface must write at least one forge-prefixed command file ' +
      '(e.g. forge-help.md) to match installRuntimeArtifacts/_copyStaged behaviour. ' +
    ...
    const unprefixed = gsdFiles.filter(f => !f.startsWith('forge-'));
    assert.deepStrictEqual(
      unprefixed,
      [],
      '#816: all GSD-owned command files must start with forge- to match install. ' +
  ...
  //
  // Matrix: opencode/kilo = flat command/ + prefix forge-;
  //         cursor/augment = flat commands/ + prefix forge-;
  //         gemini = namespaced commands/forge/ (no file prefix — dir is namespace).
  ...
  test('applySurface command-dir filenames match a fresh install for every command runtime (#816 parity)', async (t) => {
    ...
    for (const runtime of PARITY_RUNTIMES) {
      ...
      const installDir = fs.mkdtempSync(path.join(os.tmpdir(), `forge-816-install-${runtime}-`));
      const surfaceDir = fs.mkdtempSync(path.join(os.tmpdir(), `forge-816-surface-${runtime}-`));
  ...
  for (const runtime of ['cursor', 'codex']) {
    test(`applySurface rewrites ${runtime} skill bodies to the install pathPrefix, not the converter default ~/.claude path (#813)`, (t) => {
      ...
      const configDir = fs.mkdtempSync(path.join(os.tmpdir(), `forge-surface-813-${runtime}-`));
      t.after(() => cleanup(configDir));
      ...
      const manifest = loadSkillsManifest(REAL_COMMANDS_DIR);
      // skills/forge-<name>/SKILL.md (destSubpath 'skills', prefix 'forge-')
      ...
      // is cross-platform (Windows CI leg is not covered by local forge-test) (#813).
...
describe('resolveSurface', () => {
  test('no surface state + core base profile → identical to resolveProfile core', () => {
    const dir = tmpDir('forge-surface-resolve-');
    try {
  ...
  test('surface with baseProfile overrides .forge-profile marker', () => {
    const dir = tmpDir('forge-surface-resolve-');
    try {
      writeActiveProfile(dir, 'core');
      writeSurface(dir, {
...
describe('readSurface / writeSurface', () => {
  test('round-trips a complete surface state', () => {
    const dir = tmpDir('forge-surface-state-');
    try {
      const state = {
        baseProfile: 'standard',
        disabledClusters: ['utility'],
  ...
  test('non-existent directory returns null', () => {
    const ghost = path.join(os.tmpdir(), 'forge-surface-no-exist-' + Date.now());
  ...
  test('corrupt JSON returns null', () => {
    const dir = tmpDir('forge-surface-state-');
    try {
      fs.writeFileSync(path.join(dir, '.forge-surface.json'), '{not valid json', 'utf8');
      const result = readSurface(dir);
      assert.strictEqual(result, null);
    } finally {
      cleanup(dir);
  ...
  test('JSON missing baseProfile field returns null', () => {
    ...
    try {
      fs.writeFileSync(
        path.join(dir, '.forge-surface.json'),
  ...
  test('atomic write: result file is never a partial tmp file', () => {
    ...
    try {
      ...
      assert.ok(files.includes('.forge-surface.json'));
    } finally {
      cleanup(dir);
    }
  });
  ...
  test('writeSurface creates directory if it does not exist', () => {
    const base = tmpDir('forge-surface-state-');
...
describe('CLUSTERS data structure', () => {
  ...
  test('every cluster member is a real skill stem in commands/forge/', () => {
    ...
    for (const [cluster, members] of Object.entries(CLUSTERS)) {
      for (const stem of members) {
        if (!realStems.has(stem)) {
          mismatches.push(`${cluster}: "${stem}" not found in commands/forge/`);
        }
      }
    }
  ...
  test('union of all clusters covers every skill in commands/forge/', () => {
    const entries = fs.readdirSync(REAL_COMMANDS_DIR, { withFileTypes: true });
    const realStems = new Set(
      entries
        .filter(e => e.isFile() && e.name.endsWith('.md'))
...
describe('listSurface', () => {
  test('accepts parsed forge-file-manifest JSON objects without crashing (#322)', () => {
    const dir = tmpDir('forge-surface-list-');
    try {
...
// as kind='commands'; without the rewrite, /forge-surface would write workflow
...
describe('applySurface — commands kind path rewrite (#1615 adversarial review)', () => {
  test('windsurf workflow bodies are rewritten to install target (no raw ~/.claude/)', (t) => {
    const base = createTempDir('forge-surface-cmds-windsurf-');
    t.after(() => cleanup(base));
    const runtimeConfigDir = base;

    // Stage the canonical command body the workflow delegates to.
    const canonicalDir = path.join(runtimeConfigDir, 'forge-core', 'commands', 'forge');
    ...
    // Workflow files should be written to <configDir>/workflows/forge-*.md
    const workflowsDir = path.join(runtimeConfigDir, 'workflows');
    const workflowFiles = fs.existsSync(workflowsDir)
      ? fs.readdirSync(workflowsDir).filter((f) => f.startsWith('forge-') && f.endsWith('.md'))
      : [];
    assert.ok(workflowFiles.length > 0,
      `expected at least one forge-*.md workflow under ${workflowsDir}; got [${workflowFiles.join(', ')}]`);
