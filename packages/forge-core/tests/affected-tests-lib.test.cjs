...
// Fixture: tests/t.test.cjs -> ../forge-core/bin/lib/depA.cjs -> ./depB.cjs
// Changed: forge-core/bin/lib/depB.cjs
...
test('transitive: changing a deep dependency selects the test that depends on it', (t) => {
  const dir = makeFixture({
    'forge-core/bin/lib/depB.cjs': `'use strict';\nmodule.exports = { b: 1 };\n`,
    'forge-core/bin/lib/depA.cjs': `'use strict';\nconst depB = require('./depB.cjs');\nmodule.exports = { a: depB };\n`,
    'tests/t.test.cjs': `'use strict';\nconst depA = require('../forge-core/bin/lib/depA.cjs');\n`,
  });
  t.after(() => cleanup(dir));

  const reverseIndex = buildTransitiveReverseIndex(dir, ['tests/t.test.cjs']);
  const selected = pickAffectedTests(
    ['forge-core/bin/lib/depB.cjs'],
    ['tests/t.test.cjs'],
    reverseIndex,
  );
...
test('adversarial(b): missing require (gone file) — null resolve, no crash', (t) => {
  const dir = makeFixture({
    ...
    'tests/missing.test.cjs': `'use strict';\nconst x = require('../forge-core/bin/lib/gone.cjs');\n`,
  });
  t.after(() => cleanup(dir));
...
test('adversarial(c): .json dependency — changing data.json selects the test', (t) => {
  const dir = makeFixture({
    'forge-core/bin/lib/data.json': `{"key":"value"}`,
    'tests/json.test.cjs': `'use strict';\nconst data = require('../forge-core/bin/lib/data.json');\n`,
  });
  t.after(() => cleanup(dir));

  const reverseIndex = buildTransitiveReverseIndex(dir, ['tests/json.test.cjs']);
  const selected = pickAffectedTests(
    ['forge-core/bin/lib/data.json'],
    ['tests/json.test.cjs'],
    reverseIndex,
  );
...
test('adversarial(d): re-export chain — changing depB selects the test that requires the re-exporter', (t) => {
  const dir = makeFixture({
    ...
    'forge-core/bin/lib/reexporter.cjs': `'use strict';\nmodule.exports = require('./depB.cjs');\n`,
    'tests/reexport.test.cjs': `'use strict';\nconst x = require('../forge-core/bin/lib/reexporter.cjs');\n`,
  });
  t.after(() => cleanup(dir));
...
test('adversarial(f): WIDEN — changing a src file with no test dependents widens to unit/all', (t) => {
  ...
  const dir = makeFixture({
    'forge-core/bin/lib/orphan.cjs': `'use strict';\nmodule.exports = {};\n`,
  ...
  const dependents = reverseIndex.get('forge-core/bin/lib/orphan.cjs');
  assert.ok(
...
test('adversarial(g): dynamic require in changed file with no static dependents — widen backstop catches it', (t) => {
  ...
  const dir = makeFixture({
    'forge-core/bin/lib/dynamic.cjs': `'use strict';\nconst x = 'foo';\nconst m = require(\`./\${x}\`);\nmodule.exports = {};\n`,
  ...
  const selected = pickAffectedTests(
    ['forge-core/bin/lib/dynamic.cjs'],
    ['tests/unrelated.test.cjs'],
    reverseIndex,
    { detectWiden: true },
  );
...
test('regression(delete-only-source): deleting a source file triggers widen, never unit-smoke', (t) => {
  ...
  const dir = makeFixture({
    'forge-core/bin/lib/other.cjs': `'use strict';\nmodule.exports = {};\n`,
  ...
  const changedFiles = ['forge-core/bin/lib/gone.cjs'];
...
test('regression(rename-stale-old-path): deleted old path triggers widen, protecting stale importers', (t) => {
  ...
  const dir = makeFixture({
    ...
    'forge-core/bin/lib/newname.cjs': `'use strict';\nmodule.exports = { v: 2 };\n`,
    'tests/newname.test.cjs': `'use strict';\nconst x = require('../forge-core/bin/lib/newname.cjs');\n`,
  ...
  //   Delete(old) + Add(new)
  const changedFiles = [
    'forge-core/bin/lib/oldname.cjs', // deleted old path — absent from disk
    'forge-core/bin/lib/newname.cjs', // added new path — present on disk
  ];

  // Assert: oldname.cjs must have zero static dependents (not in graph)
  const oldDependents = reverseIndex.get('forge-core/bin/lib/oldname.cjs');
  assert.ok(
...
test('listTestFiles: subdir test files are included and selectable by pickAffectedTests', (t) => {
  ...
  const dir = makeFixture({
    ...
    'tests/dispatch/agent-dispatch.test.cjs': `'use strict';\nconst lib = require('../../forge-core/bin/lib/dispatch.cjs');\n`,
    'forge-core/bin/lib/dispatch.cjs': `'use strict';\nmodule.exports = { dispatch: true };\n`,
  });
  t.after(() => cleanup(dir));
