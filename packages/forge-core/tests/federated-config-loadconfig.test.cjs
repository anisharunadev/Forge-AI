...
function makeTempProject() {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-fed-cfg-test-'));
...
describe('FIX 2: overlay applied on the no-config path', () => {
  test('project with NO config.json → federated default is surfaced (non-central key)', () => {
    ...
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-fed-noconfig-'));
    tmpDirs.push(tmpDir);
    fs.mkdirSync(path.join(tmpDir, '.planning', 'phases'), { recursive: true });
  ...
  test('no-config path with REAL registry → capability defaults are surfaced', () => {
    _resetFederatedRegistryForTests();
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-fed-noconfig-real-'));
    tmpDirs.push(tmpDir);
    fs.mkdirSync(path.join(tmpDir, '.planning', 'phases'), { recursive: true });
    // No config.json
...
describe('ADR-1244 D2: overlay config-key federation (cwd-aware, real loader)', () => {
  const configSchema = require('../forge-core/bin/lib/config-schema.cjs');
  ...
  beforeEach(() => {
    ...
    );
    {
      ...
      const consent = require('../forge-core/bin/lib/capability-consent.cjs');
