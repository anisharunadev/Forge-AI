...
const modelCatalog = require('../forge-core/bin/lib/model-catalog.cjs');
...
const makeTmp = (prefix) => createTempDir(`forge-49-${prefix}-`);
...
describe('#49 resolveModelInternal: model_policy in the resolution chain', () => {
  ...
  test('model_policy fires before model_profile_overrides when both are set (model_policy wins)', () => {
    ...
    const result = resolveModelInternal(projectDir, 'forge-planner');
  ...
  test('model_policy with provider:"anthropic" + budget:"high" + runtime:"opencode" resolves to preset model', () => {
    writeConfig(projectDir, {
      runtime: 'opencode',
      model_profile: 'quality',  // forge-planner quality = opus tier
      model_policy: {
        provider: 'anthropic',
        budget: 'high',
      },
  ...
  test('model_policy is skipped when runtime is absent', () => {
    ...
    let result;
    assert.doesNotThrow(() => {
      result = resolveModelInternal(projectDir, 'forge-planner');
    });
  ...
  test('model_policy provider preset resolves to a Claude alias on runtime:"claude" (#1133)', () => {
    ...
    // forge-planner -> opus tier; anthropic-fable opus/high = claude-fable-5 -> alias "fable"
    assert.strictEqual(resolveModelInternal(projectDir, 'forge-planner'), 'fable');
  });
  ...
  test('model_policy works with implicit claude runtime (no runtime key) (#1133)', () => {
    ...
    // forge-executor -> sonnet tier; anthropic-fable sonnet/high = claude-fable-5 -> "fable"
    assert.strictEqual(resolveModelInternal(projectDir, 'forge-executor'), 'fable');
  });
  ...
  test('unmappable model_policy ID warns and falls back to the tier alias on claude (#1133)', () => {
    ...
    // forge-planner -> opus tier; anthropic-fable opus/low = claude-opus-4-5 (no alias) -> fall back to "opus"
    assert.strictEqual(resolveModelInternal(projectDir, 'forge-planner'), 'opus');
  });
  ...
  test('model_policy.runtime_tiers applies on runtime:"claude", mapped to alias (#1133)', () => {
    ...
    // forge-planner -> opus tier; runtime_tiers.claude.opus = claude-fable-5 -> "fable" (was a no-op pre-#1133)
    assert.strictEqual(resolveModelInternal(projectDir, 'forge-planner'), 'fable');
  });
  ...
  test('model_policy maps a built-in catalog model ID to its Claude alias via MODEL_ALIAS_MAP (#1133)', () => {
    ...
    // forge-planner -> opus tier; runtime_tiers.claude.opus = claude-opus-4-8 ->
  ...
  test('model_policy still returns full IDs on non-claude runtimes (#1133 regression)', () => {
    ...
    assert.strictEqual(resolveModelInternal(projectDir, 'forge-planner'), 'claude-fable-5');
  });
  ...
  test('model_policy absent + model_profile_overrides set → model_profile_overrides wins (back-compat)', () => {
    ...
    // forge-executor has balanced/opencode -> sonnet tier
    const result = resolveModelInternal(projectDir, 'forge-executor');
...
describe('#49 resolveModelInternal: unknown provider warning behavior', () => {
  ...
  test('unknown provider warning is deduplicated (emitted only once per config label)', () => {
    ...
    resolveModelInternal(projectDir, 'forge-planner');
    resolveModelInternal(projectDir, 'forge-executor');
    resolveModelInternal(projectDir, 'forge-verifier');
    const joined = captured.join('');
...
describe('#49 reasoning_effort in model_policy entries', () => {
  ...
  test('reasoning_effort in model_policy.runtime_tiers entry is returned verbatim; renderEffortForRuntime strips it when runtime not in RUNTIMES_WITH_REASONING_EFFORT', () => {
    ...
    const { renderEffortForRuntime, RUNTIMES_WITH_REASONING_EFFORT } = require('../forge-core/bin/lib/model-catalog.cjs');
...
describe('#49 resolveModelForTier: model_policy beats dynamic_routing', () => {
  ...
  test('model_policy wins over dynamic_routing.tier_models when both are set', () => {
    ...
    const result = resolveModelForTier(tmpDir, 'forge-executor', 0);
    // forge-executor is standard/sonnet tier; openai+low+sonnet preset model
    assert.ok(typeof result === 'string' && result.length > 0,
  ...
  test('model_overrides still beats model_policy in resolveModelForTier', () => {
    writeConfig(tmpDir, {
      ...
      model_overrides: { 'forge-planner': 'custom-model-id' },
    });
    assert.strictEqual(resolveModelForTier(tmpDir, 'forge-planner', 0), 'custom-model-id');
  });
  ...
  test('dynamic_routing.tier_models used normally when model_policy absent', () => {
    ...
    assert.strictEqual(resolveModelForTier(tmpDir, 'forge-executor', 0), 'my-custom-sonnet');
  });
  ...
  test('model_policy with Claude runtime does not interrupt dynamic_routing', () => {
    ...
    assert.strictEqual(resolveModelForTier(tmpDir, 'forge-executor', 0), 'my-sonnet');
  });
  ...
  test('model_policy value that is already a bare Claude alias is returned as-is on claude (#1133)', () => {
    ...
    // forge-planner → opus tier; runtime_tiers.claude.opus = "fable" is already a valid alias → "fable"
    assert.strictEqual(resolveModelInternal(tmpDir, 'forge-planner'), 'fable');
  });
});
