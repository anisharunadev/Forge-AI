...
describe('workflow.discuss_mode config', () => {
  test('config template includes discuss_mode default', () => {
    const template = JSON.parse(
      fs.readFileSync(path.join(__dirname, '..', 'forge-core', 'templates', 'config.json'), 'utf8')
    );
  ...
  test('discuss-phase command references both workflow files', () => {
    const command = fs.readFileSync(
      path.join(__dirname, '..', 'commands', 'forge', 'discuss-phase.md'), 'utf8'
    );
  ...
  test('assumptions workflow file exists and has required steps', () => {
    const workflow = fs.readFileSync(
      path.join(__dirname, '..', 'forge-core', 'workflows', 'discuss-phase-assumptions.md'), 'utf8'
    );
    const requiredSteps = [
  ...
  test('plan-phase gate references discuss_mode config', () => {
    const planPhase = fs.readFileSync(
      path.join(__dirname, '..', 'forge-core', 'workflows', 'plan-phase.md'), 'utf8'
    );
  ...
  test('plan-phase command argument-hint includes --text', () => {
    const command = fs.readFileSync(
      path.join(__dirname, '..', 'commands', 'forge', 'plan-phase.md'), 'utf8'
    );
  ...
  test('plan-phase init exposes text_mode in workflow flags', () => {
    const initSrc = fs.readFileSync(
      path.join(__dirname, '..', 'forge-core', 'bin', 'lib', 'init.cjs'), 'utf8'
    );
  ...
  test('progress workflow references discuss_mode', () => {
    const progress = fs.readFileSync(
      path.join(__dirname, '..', 'forge-core', 'workflows', 'progress.md'), 'utf8'
    );
  ...
  test('discuss-phase command mode-routing uses forge_run (shim-safe) not bare forge-tools', () => {
    const command = fs.readFileSync(
      path.join(__dirname, '..', 'commands', 'forge', 'discuss-phase.md'), 'utf8'
    );
    ...
    // Must use forge_run for the config lookup
    assert.ok(
      command.includes('forge_run query config-get workflow.discuss_mode'),
      'discuss-phase.md must use forge_run (not bare forge-tools) for discuss_mode lookup'
    );
    // Must NOT contain the bare footgun pattern: forge-tools immediately before the silent default
    assert.ok(
      !command.includes('forge-tools query config-get workflow.discuss_mode 2>/dev/null || echo'),
      'discuss-phase.md must NOT use bare forge-tools binary for discuss_mode lookup (shim-only install footgun)'
    );
  });
});
