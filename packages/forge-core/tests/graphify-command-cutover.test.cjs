 ...
 * `case 'graphify':` arm in forge-tools.cjs to the capability registry dispatch
...
const { routeGraphifyCommand } = require('../forge-core/bin/lib/graphify-command-router.cjs');
...
describe('graphify router: precise unit tests (recording mock)', () => {
  ...
  test('query missing term → error(usage msg, USAGE); graphifyQuery NOT called', () => {
    ...
    assert.ok(
      errFn.calls[0].msg.includes('Usage: forge-tools graphify query <term>'),
