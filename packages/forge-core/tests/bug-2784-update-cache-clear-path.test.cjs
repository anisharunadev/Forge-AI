 ...
 *
 * /forge-update cache-clear step only cleared per-runtime cache paths
 * (e.g. ~/.claude/cache/forge-update-check.json) but the SessionStart hook
 * (hooks/forge-check-update.js) writes to the shared tool-agnostic path
 * ~/.cache/forge/forge-update-check.json. After a successful update, the statusline
 * kept showing the stale "⬆ /forge-update" indicator because the actual cache
 * file was never deleted.
 *
 * Fix: add `rm -f "$HOME/.cache/forge/forge-update-check.json"` to the
 * run_update step's cache-clear block in forge-core/workflows/update.md.
 */
...
const CHECK_UPDATE_HOOK = path.join(REPO_ROOT, 'hooks', 'forge-check-update.js');

describe('bug-2784: update.md cache-clear covers shared cache path', () => {
  test('forge-check-update.js hook constructs cache dir from .cache and forge path segments', () => {
    ...
    assert.ok(
      segments.includes('forge'),
      `hook cacheDir path.join() must include 'forge' segment; got: ${JSON.stringify(segments)}`
    );
  });

  test('update.md run_update bash commands include rm for shared forge cache file', () => {
    ...
    const sharedCacheClearCmds = bashLines.filter(
      (line) => /^rm\b/.test(line) && line.includes('.cache/forge/forge-update-check') && line.includes('*.json')
    );
    assert.ok(
      sharedCacheClearCmds.length > 0,
      [
        'run_update step bash blocks must include an `rm` command targeting .cache/forge/forge-update-check*.json (glob form clearing legacy + per-package variants).',
