...
before(() => {
  ...
  upstreamDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-48-upstream-'));
  ...
  featureDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-48-feature-'));
...
describe('bug #48: orchestrator cwd-drift guard — executable e2e', () => {
  ...
  test('guard fails closed (exit 1) when not inside a git repo', (t) => {
    const nonRepoDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-48-nongit-'));
    try {
