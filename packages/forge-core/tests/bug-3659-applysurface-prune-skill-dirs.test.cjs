 ...
 *
 * applySurface did not prune ~/.claude/skills/forge-STEM dirs when a cluster
 ...
 *
 * The pruning ran against the wrong (non-existent) dir so stale forge-Y dirs
 ...
 *   2. Surface state file moves to <configDir>/.forge-surface.json at the config
 *      root, matching install/uninstall conventions.
 *   3. applySurface is called with scope='global' so the skills kind is active.
 *
 * Tests:
 *   a) disabled cluster forge-STEM dirs are REMOVED from ~/.claude/skills/
 *   b) forge-STEM dirs in the retain set are preserved
...
const { writeSurface, applySurface } = require('../forge-core/bin/lib/surface.cjs');
const { loadSkillsManifest } = require('../forge-core/bin/lib/install-profiles.cjs');
const { CLUSTERS } = require('../forge-core/bin/lib/clusters.cjs');
 ...
 * skillsDir  — analogous to ~/.claude/skills (contains forge-* dirs)
 *
 * Pre-populated with:
 *   forge-explore/SKILL.md  — in research_ideate cluster (will be disabled)
 *   forge-help/SKILL.md     — in core_loop cluster (will remain enabled)
 *   my-custom-skill/      — user-owned, not forge-prefixed (must never be touched)
 */
function createFixture() {
  const configDir = createTempDir('forge-bug3659-');
  const skillsDir = path.join(configDir, 'skills');
  fs.mkdirSync(skillsDir, { recursive: true });

  const gsdExplore = path.join(skillsDir, 'forge-explore');
...
/**
 * Extended fixture that also includes a user-created forge-* directory.
 * Used by the all-clusters-disabled counter-test to prove the manifest-membership
 * gate (Finding 1 fix) protects user-owned forge-* dirs from data loss.
 */
function createFixtureWithUserGsdDir() {
  const base = createFixture();
  const userGsdDir = path.join(base.skillsDir, 'forge-mything');
  fs.mkdirSync(userGsdDir, { recursive: true });
  fs.writeFileSync(path.join(userGsdDir, 'SKILL.md'), '# user skill\n', 'utf8');
  return { ...base, userGsdDir };
}

describe('bug-3659: applySurface prunes ~/.claude/skills/forge-*/ on cluster disable', () => {
  test('(a) disabled cluster forge-* dirs are removed from skills dir', (t) => {
    ...
    // scope='global' gives the skills kind for Claude (destSubpath='skills', prefix='forge-')
    const layout = resolveRuntimeArtifactLayout('claude', configDir, 'global');
    applySurface(configDir, layout, manifest, CLUSTERS);

    // forge-explore is in the research_ideate cluster which was disabled:
    // it must be pruned from skillsDir.
    assert.ok(
      !fs.existsSync(gsdExplore),
      'forge-explore/ must be removed from skills dir when research_ideate cluster is disabled'
    );

    // forge-help is in core_loop (not disabled) and must survive.
    assert.ok(
      fs.existsSync(gsdHelp),
      'forge-help/ must be preserved when its cluster is not disabled'
    );
  });

  test('(b) forge-* dirs in retained clusters are preserved', (t) => {
    ...
    assert.ok(
      fs.existsSync(gsdHelp),
      'forge-help/ must be preserved — core_loop cluster remains enabled'
    );
  });
  ...
  test('(d) idempotence: running applySurface twice produces identical on-disk state', (t) => {
    ...
    assert.ok(
      !fs.existsSync(gsdExplore),
      'forge-explore/ must remain absent after second applySurface call'
    );
  ...
  test('(e) all-clusters-disabled: all forge-owned dirs removed; user dirs and user forge-* dirs survive', (t) => {
    ...
    // Assertions:
    //   1. forge-explore/ — GSD-owned, disabled cluster → REMOVED
    //   2. forge-help/    — GSD-owned, disabled cluster → REMOVED
    //   3. my-custom-skill/ — user-owned, no forge- prefix → PRESERVED
    //   4. forge-mything/ — prefix match but NOT in manifest → PRESERVED (Finding 1 fix)
    ...
    assert.ok(
      !fs.existsSync(gsdExplore),
      'forge-explore/ must be removed when all clusters are disabled'
    );
    assert.ok(
      !fs.existsSync(gsdHelp),
      'forge-help/ must be removed when all clusters are disabled'
    );
    ...
    // 3. User-created forge-* dir NOT in the manifest must be preserved.
    //    This is the critical Finding 1 regression guard: without the manifest-membership
    //    gate, forge-mything/ would have been silently deleted.
    assert.ok(
      fs.existsSync(userGsdDir),
      'forge-mything/ (user-created forge-* dir not in manifest) must be preserved — ' +
