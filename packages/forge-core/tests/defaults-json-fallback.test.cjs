/**
 * GSD Tools Tests — ~/.forge/defaults.json fallback (#1683)
 *
 * When .planning/ does not exist (pre-project context), loadConfig() should
 * consult ~/.forge/defaults.json before returning hardcoded defaults.
...
describe('loadConfig ~/.forge/defaults.json fallback (#1683)', () => {
  ...
  test('pre-project, defaults.json exists → merges with hardcoded defaults', (t) => {
    ...
    // Create ~/.forge/defaults.json under fake GSD_HOME
