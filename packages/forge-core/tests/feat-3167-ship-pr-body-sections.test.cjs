/**
 * Regression tests for issue #3167: configurable /forge-ship PR body sections.
 */
...
function makeProject() {
  const tmpDir = createTempProject('forge-3167-');
  tmpDirs.push(tmpDir);
  return tmpDir;
}
...
describe('ship.pr_body_sections config (#3167)', () => {
  ...
  test('ship workflow composes configured sections as append-only extensions', () => {
    const workflow = readRepoFile('forge-core/workflows/ship.md');
  ...
  test('default config and documentation describe ship.pr_body_sections', () => {
    const template = JSON.parse(readRepoFile('forge-core/templates/config.json'));
    ...
    const planningConfig = readRepoFile('forge-core/references/planning-config.md');
  ...
  test('new-project onboarding can seed enabled or disabled PR body sections', () => {
    const workflow = readRepoFile('forge-core/workflows/new-project.md');
