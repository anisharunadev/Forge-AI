...
const {
  ...
  INSTALL_SURFACES,
} = require('../forge-core/bin/lib/runtime-config-adapter-registry.cjs');
...
describe('reconcileCursorHooksJson', () => {
  test('T1: creates new hooks.json with sessionStart and postToolUse', (t) => {
    ...
    const result = reconcileCursorHooksJson(hooksJsonPath, {
      sessionStart: managedEntry('/node /path/forge-cursor-session-start.js'),
      postToolUse:  managedEntry('/node /path/forge-cursor-post-tool.js'),
    });
  ...
  test('T2: idempotent — second run produces same content (no write)', (t) => {
    ...
    const hooksJsonPath = path.join(dir, 'hooks.json');
    const entries = {
      sessionStart: managedEntry('/node /forge-cursor-session-start.js'),
      postToolUse:  managedEntry('/node /forge-cursor-post-tool.js'),
    };
    ...
    fs.writeFileSync(hooksJsonPath, makeHooksJson({
      ...
      hooks: {
        sessionStart: [
          ...
          managedEntry('/node /forge-cursor-session-start.js'),
        ],
        postToolUse: [
          managedEntry('/node /forge-cursor-post-tool.js'),
        ],
      },
    }));
...
// ---------------------------------------------------------------------------
test('T17: forge-cursor-session-start.js emits JSON with additional_context', (t, done) => {
  const hooksDir = path.join(__dirname, '..', 'hooks');
...
// ---------------------------------------------------------------------------
test('T18: forge-cursor-post-tool.js emits {} for non-write tool names', (t, done) => {
  const hooksDir = path.join(__dirname, '..', 'hooks');
...
test('T20: GSD_CURSOR_SESSION_HOOK_SCRIPT and GSD_CURSOR_POST_TOOL_HOOK_SCRIPT are exported', () => {
  assert.equal(GSD_CURSOR_SESSION_HOOK_SCRIPT, 'forge-cursor-session-start.js');
  assert.equal(GSD_CURSOR_POST_TOOL_HOOK_SCRIPT, 'forge-cursor-post-tool.js');
});
