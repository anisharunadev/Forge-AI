 ...
 *
 * A user project normally does not contain forge-core/bin/forge-tools.cjs.
 * The snippets should still prefer RUNTIME_DIR for local/dev installs, then
 * fall back to the installed forge-tools binary on PATH.
 */
'use strict';
...
const WORKFLOW_PATH = path.join(__dirname, '..', 'forge-core', 'workflows', 'next.md');
 ...
 *   `_GSD_SHIM_NAME="forge-tools.cjs";` — return that line directly.
...
function extractResolverSnippet() {
  ...
  if (start === -1) {
    ...
    start = lines.findIndex((line) =>
      /^\s*#\s*Runtime launcher:.*prefer local forge-tools\.cjs.*installed forge-tools on PATH/.test(line)
    );
  }
  if (start === -1) {
...
function makeTempDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'forge-runtime-resolution-'));
}
...
describe('bug-3668: workflow SDK resolver supports installed user projects', () => {
  test('falls back to installed forge-tools when project-local runtime copy is absent', () => {
    // Bug #3668: when a user project has no local forge-core/bin/forge-tools.cjs,
    // the elif branch must resolve to the forge-tools binary on PATH.
    // RUNTIME_DIR points to a dir that has no forge-tools.cjs.
    ...
    // Place forge-tools stub on PATH (installed binary)
    writeExecutable(
      path.join(pathBin, 'forge-tools'),
      '#!/bin/sh\nprintf "installed:%s %s\\n" "$1" "$2"\n',
    );

    // NO forge-core/bin/forge-tools.cjs in runtimeNoLocal
    ...
    assert.match(output, /GSD_TOOLS=.+forge-tools(?:\s|$)/m);
    // The PATH stub must have been invoked
    assert.match(output, /installed:query state\.json/);
  });

  test('preserves RUNTIME_DIR local forge-tools.cjs preference over PATH fallback', () => {
    const tmp = makeTempDir();
    const project = path.join(tmp, 'user-project');
    const runtime = path.join(tmp, 'runtime');
    const pathBin = path.join(tmp, 'bin');
    fs.mkdirSync(project, { recursive: true });
    writeExecutable(path.join(pathBin, 'forge-tools'), '#!/bin/sh\nprintf "path-installed:%s %s\\n" "$1" "$2"\n');
    writeExecutable(
      path.join(runtime, 'forge-core', 'bin', 'forge-tools.cjs'),
    ...
    // The resolved bin is the RUNTIME_DIR local runtime (suffix /forge-core/bin/forge-tools.cjs)
    // Use .+ instead of \S* to handle paths with spaces (e.g. /Volumes/Mini Me/...)
    assert.match(norm, /GSD_TOOLS=.+\/forge-core\/bin\/forge-tools\.cjs(?:\s|$)/m);
