...
/**
 * Bug #503: /forge:update misclassifies local Antigravity (.agent) installs as claude
 *
 * The installer places a LOCAL Antigravity install in ./.agent/
 * (bin/install.js: getDirName('antigravity') === '.agent'). The /forge:update
...
const UPDATE_MD = fs.readFileSync(
  path.join(ROOT, 'forge-core', 'workflows', 'update.md'),
  'utf-8',
);
...
describe('/forge:update detects local Antigravity (.agent / .agents) installs (#503 / #791)', () => {
  test('projection resolves a LOCAL ./.agents install to the antigravity runtime (#791 canonical)', () => {
    const HOME = '/home/u';
    const CWD = '/work/proj';
    const agentsDir = `${CWD}/.agents`;
    const ffs = fakeFs({
      [`${agentsDir}/forge-core/VERSION`]: '1.50.0\n',
      [`${agentsDir}/forge-core/workflows/update.md`]: 'x',
    });
  ...
  test('projection resolves a LOCAL ./.agent install to the antigravity runtime (#503 backward-compat)', () => {
    ...
    const ffs = fakeFs({
      [`${agentDir}/forge-core/VERSION`]: '1.40.0\n',
      [`${agentDir}/forge-core/workflows/update.md`]: 'x',
    });
    const r = resolveUpdateContext({ home: HOME, cwd: CWD, env: {}, fs: ffs });
    assert.equal(
      r.runtime,
