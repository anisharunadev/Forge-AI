...
/**
 * Run forge-tools via spawnSync. Returns { status, stdout, stderr }.
...
describe('plan:pre surface cluster filter — ui disabled, tdd+intel active', () => {
  ...
  before(() => {
    ...
    // .forge-surface.json disabling UI cluster in same dir (config-dir = tmpDir)
    fs.writeFileSync(
      path.join(tmpDir, '.forge-surface.json'),
      JSON.stringify({
        baseProfile: 'full',
        disabledClusters: ['ui'],
        explicitAdds: [],
