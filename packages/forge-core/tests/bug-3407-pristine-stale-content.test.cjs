...
/**
 * Bug #3407: Installer leaves stale content in forge-pristine/
 *
 ...
 * those bytes into forge-pristine/.  For files changed between the old and new
 ...
 * but forge-pristine/ has new-release bytes) and skips to over-broad mode
 * instead of false-failing.  The root-cause stale write was never fixed.
 *
 * Fix: when a correctly-populated forge-pristine/ already exists from the
 ...
 * release source.  This ensures forge-pristine/ holds old-release bytes even
 ...
 *   forge-pristine/ entry was correctly set by the previous install, the
 *   forge-pristine/ file MUST still contain the old-release bytes, not the
...
// ─── Bug #3407: forge-pristine/ must preserve OLD-release bytes across upgrade ──
...
describe('Bug #3407: saveLocalPatches preserves old-release pristine across upgrade', () => {
  ...
  beforeEach((t) => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-3407-'));
   ...
   *
   * Timeline:
   *   Install v1: file content = OLD_RELEASE_CONTENT, forge-pristine/ROOT_FILE
   ...
   *
   * Expected AFTER fix: forge-pristine/ROOT_FILE still == OLD_RELEASE_CONTENT
   * Actual BEFORE fix:  forge-pristine/ROOT_FILE == NEW_RELEASE_CONTENT (stale)
   */
  test('forge-pristine/ retains old-release bytes when upgrading a user-modified file', () => {
    ...
    // Set up forge-pristine/ with OLD content (as correctly populated by previous install)
    ...
    // Assert: forge-pristine/ must still contain OLD-release bytes
    const pristineFile = path.join(pristineDir, relPath);
    assert.ok(
      fs.existsSync(pristineFile),
      `forge-pristine/${relPath} must exist after saveLocalPatches`
    );
    ...
    assert.equal(
      ...
      oldHash,
      [
        `forge-pristine/${relPath} must contain OLD-release bytes (sha256=${oldHash.slice(0, 12)}…)`,
  ...
  /**
   * Regression test for Codex finding: when forge-pristine/ entry is absent
   ...
   */
  test('forge-pristine/ is regenerated for missing entries when file is unchanged between releases', () => {
    ...
    // NOTE: forge-pristine/ does NOT exist (simulating post-buggy-run or first-time scenario)
    ...
    // === originalHash, and promoted the candidate into forge-pristine/.
    const pristineFile = path.join(configDir, 'forge-pristine', relPath);
    assert.ok(
      fs.existsSync(pristineFile),
      [
        `forge-pristine/${relPath} must exist after hash-validated regeneration.`,
    ...
    assert.equal(
      ...
      oldHash,
      [
        `forge-pristine/${relPath} must contain bytes matching originalHash after regeneration`,
   ...
   *
   * Timeline:
   *   Buggy run:  forge-pristine/<rel> was written with NEW_RELEASE_CONTENT
   ...
   *   - forge-pristine/<rel> must be ABSENT (over-broad fallback — correct).
   *
   ...
   *   - forge-pristine/<rel> must remain present with the correct bytes.
   *
   ...
   */
  test('stale forge-pristine/ entry (new-release bytes) is removed when file changed between releases', () => {
    ...
    // Pre-populate forge-pristine/ with NEW_RELEASE_CONTENT — the exact pre-fix bug artifact.
    ...
    // Result: forge-pristine/<rel> must be ABSENT — over-broad fallback is the safe outcome.
    const pristineFile = path.join(pristineDir, relPath);
    assert.strictEqual(
      fs.existsSync(pristineFile),
      false,
      [
        `expected forge-pristine/${relPath} to be absent after stale-pristine recovery.`,
  ...
  /**
   * Second scenario: forge-pristine/ does NOT pre-exist (first upgrade with no
   ...
   * release bytes either.  The correct outcome is: forge-pristine/ stays empty
   * for this file, and the verifier falls back to over-broad mode (safe).
   */
  test('forge-pristine/ stays empty when no prior pristine exists (first upgrade, no stale write)', () => {
    const OLD_RELEASE_CONTENT = '# Old Release Content\nThis is v1.\n';
    const NEW_RELEASE_CONTENT = '# New Release Content\nThis is v2 — changed.\n';
    const USER_MODIFIED_CONTENT = '# Old Release Content\nThis is v1.\n## User addition\nCustom.\n';
    ...
    // NOTE: forge-pristine/ does NOT exist yet (first upgrade)
    ...
    assert.strictEqual(
      ...
      false,
      [
        `expected forge-pristine/${relPath} to be absent when file changed across release boundary.`,
