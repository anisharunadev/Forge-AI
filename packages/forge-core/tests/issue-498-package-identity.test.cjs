...
// the generated runtime module forge-core/bin/lib/package-identity.cjs
...
const GENERATED = path.join(ROOT, 'forge-core', 'bin', 'lib', 'package-identity.cjs');
...
describe('Issue #498: deriveIdentity (pure, package.json -> coordinates)', () => {
  ...
  test('derives the real GSD coordinates from the repo package.json', () => {
    ...
    assert.equal(id.packageName, '@forge-ai/forge-core');
    assert.equal(id.binName, 'forge-core');
    assert.equal(id.repoSlug, 'forge-ai/forge-core');
  });

  test('deriveIdentity returns cacheSlug for @forge-ai/forge-core', () => {
    const real = require(path.join(ROOT, 'package.json'));
    const id = deriveIdentity(real);
    assert.equal(id.cacheSlug, 'forge-ai-forge-core');
  });

  test('deriveIdentity returns updateCacheFileName for @forge-ai/forge-core', () => {
    const real = require(path.join(ROOT, 'package.json'));
    const id = deriveIdentity(real);
    assert.equal(id.updateCacheFileName, 'forge-update-check-forge-ai-forge-core.json');
  });
});

describe('Issue #498: slugifyPackageName (pure helper for cache filename)', () => {
  test('slugifyPackageName strips leading @, replaces / with -, for @forge-ai/forge-core', () => {
    assert.equal(slugifyPackageName('@forge-ai/forge-core'), 'forge-ai-forge-core');
  });
...
describe('Issue #498: formatManualInstall (the npx fallback command)', () => {
  ...
  test('matches the literal update.md uses for the real package (global+claude)', () => {
    ...
    assert.equal(
      ...
      'npx -y --package=@forge-ai/forge-core@latest -- forge-core --claude --global',
    );
  });
});
...
describe('Issue #498: generated runtime module (baked, drift-checked)', () => {
  ...
  test('generated module exports cacheSlug matching @forge-ai/forge-core', () => {
    const id = require(GENERATED);
    assert.equal(id.cacheSlug, 'forge-ai-forge-core');
  });

  test('generated module exports updateCacheFileName matching @forge-ai/forge-core', () => {
    const id = require(GENERATED);
    assert.equal(id.updateCacheFileName, 'forge-update-check-forge-ai-forge-core.json');
  });
