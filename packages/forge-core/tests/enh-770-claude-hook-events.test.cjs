...
describe('enh-770: Claude install registers SubagentStop / Stop / PreCompact context hooks', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('forge-770-claude-ctx-');
  ...
  test('SubagentStop / Stop / PreCompact all use forge-context-monitor', () => {
    for (const event of ['SubagentStop', 'Stop', 'PreCompact']) {
      const cmds = hooksForEvent(settings, event);
      assert.ok(
        cmds.some(c => c.includes('forge-context-monitor')),
        `Event ${event} should use forge-context-monitor; got commands: ${JSON.stringify(cmds)}`
      );
    }
  });
});
...
describe('enh-770: Claude install registers FileChanged hook for .planning/config.json', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('forge-770-filechanged-');
    previousCwd = process.cwd();
    process.chdir(tmpDir);
    stubHooksIntoTarget(path.join(tmpDir, '.claude'));
  ...
  test('FileChanged hook uses forge-config-reload', () => {
    const cmds = hooksForEvent(settings, 'FileChanged');
    assert.ok(
      cmds.some(c => c.includes('forge-config-reload')),
      `FileChanged should use forge-config-reload; got commands: ${JSON.stringify(cmds)}`
    );
  });
...
describe('enh-770: Claude install is idempotent for the new hook events', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('forge-770-idem-');
    previousCwd = process.cwd();
    process.chdir(tmpDir);
    stubHooksIntoTarget(path.join(tmpDir, '.claude'));
  });
...
describe('enh-770: Uninstall removes new hook event entries', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('forge-770-uninstall-');
    previousCwd = process.cwd();
    process.chdir(tmpDir);
    stubHooksIntoTarget(path.join(tmpDir, '.claude'));
...
// ─── Suite 5: forge-config-reload.js hook script exists and has correct shape ───

describe('enh-770: forge-config-reload.js hook script', () => {
  const reloadScript = path.join(__dirname, '..', 'hooks', 'forge-config-reload.js');

  test('forge-config-reload.js exists in hooks/', () => {
    assert.ok(fs.existsSync(reloadScript),
      `forge-config-reload.js must exist at ${reloadScript}`);
  });

  test('forge-config-reload.js contains the forge-hook-version stamp', () => {
    ...
    assert.ok(
      ...
      'forge-config-reload.js must contain the forge-hook-version stamp for installer stamping'
    );
  });

  test('forge-config-reload.js reads from stdin and emits JSON output', () => {
    ...
    assert.ok(
      ...
      'forge-config-reload.js must read stdin and emit JSON output per hook protocol'
    );
  });

  test('forge-config-reload.js targets the FileChanged hook event', () => {
    ...
    assert.ok(
      ...
      'forge-config-reload.js must reference FileChanged in its hookSpecificOutput'
    );
  });
});
...
// ─── Suite 7: managed-hooks-registry includes forge-config-reload.js ───────────

describe('enh-770: managed-hooks-registry includes forge-config-reload.js', () => {
  test('MANAGED_HOOKS array includes forge-config-reload.js', () => {
    const { MANAGED_HOOKS } = require('../hooks/managed-hooks-registry.cjs');
    assert.ok(
      MANAGED_HOOKS.includes('forge-config-reload.js'),
      `MANAGED_HOOKS must include forge-config-reload.js; got: ${JSON.stringify(MANAGED_HOOKS)}`
    );
  });
});
