 ...
 *
 * The schema (forge-core/bin/shared/model-catalog.json `profiles` array) defines 5 valid
...
const SETTINGS_PATH = path.join(REPO_ROOT, 'forge-core', 'workflows', 'settings.md');
const CATALOG_PATH = path.join(REPO_ROOT, 'forge-core', 'bin', 'shared', 'model-catalog.json');
