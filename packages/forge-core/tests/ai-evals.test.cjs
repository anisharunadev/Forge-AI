 ...
 *
 * Validates the /forge-ai-integration-phase + /forge-eval-review contribution:
...
const REFERENCES_DIR = path.join(REPO_ROOT, 'forge-core', 'references');
...
describe('COMMAND: ai-integration-phase and eval-review frontmatter', () => {
  ...
  for (const cmd of commands) {
    test(`${cmd}.md exists`, () => {
      ...
      assert.ok(fs.existsSync(p), `commands/forge/${cmd}.md should exist`);
    });
  ...
  test('ai-integration-phase.md name is forge:ai-integration-phase', () => {
    const content = fs.readFileSync(path.join(COMMANDS_DIR, 'ai-integration-phase.md'), 'utf-8');
    assert.ok(content.includes('name: forge:ai-integration-phase'), 'ai-integration-phase command name mismatch');
  });

  test('eval-review.md name is forge:eval-review', () => {
    const content = fs.readFileSync(path.join(COMMANDS_DIR, 'eval-review.md'), 'utf-8');
    assert.ok(content.includes('name: forge:eval-review'), 'eval-review command name mismatch');
  });
});
...
describe('WORKFLOW: plan-phase.md AI nudge integration', () => {
  ...
  test('plan-phase.md references /forge-ai-integration-phase nudge', () => {
    const content = fs.readFileSync(planPhasePath, 'utf-8');
    assert.ok(
...
describe('WORKFLOW: ai-integration-phase and eval-review workflow files', () => {
  ...
  test('ai-integration-phase.md orchestrates 4 agents', () => {
    ...
    for (const agent of ['forge-framework-selector', 'forge-ai-researcher', 'forge-domain-researcher', 'forge-eval-planner']) {
      assert.ok(content.includes(agent), `ai-integration-phase.md should reference ${agent}`);
    }
  });

  test('eval-review.md references forge-eval-auditor', () => {
    const content = fs.readFileSync(path.join(WORKFLOWS_DIR, 'eval-review.md'), 'utf-8');
    assert.ok(content.includes('forge-eval-auditor'), 'eval-review.md should reference forge-eval-auditor');
  });
