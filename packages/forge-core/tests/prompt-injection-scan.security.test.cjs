...
const { scanForInjection } = require('../forge-core/bin/lib/security.cjs');
...
const ALLOWLIST = new Set([
  'forge-core/bin/lib/security.cjs',        // The security module itself
  'forge-core/workflows/discuss-phase.md',  // Large workflow (~50K) with power mode + i18n
  'forge-core/workflows/new-project.md',     // Large workflow (~50K) — agent install, runtime detect, brownfield map, #3491 worktree gating
  'forge-core/workflows/execute-phase.md',  // Large orchestration workflow (~51K) with wave execution + code-review gate
  'forge-core/workflows/plan-phase.md',      // Large orchestration workflow (~51K) with TDD mode integration
  'hooks/forge-prompt-guard.js',                  // The prompt guard hook
  'hooks/forge-read-injection-scanner.js',        // The read injection scanner (contains patterns)
...
const SIZE_ONLY_WORKFLOWS = new Set([
  'forge-core/workflows/docs-update.md',  // ~51K after fix-loop truncation guard (#571)
]);
...
describe('codebase prompt injection scan', () => {
  ...
  test('agent definition files are clean (injection patterns)', () => {
    ...
    for (const file of agentFiles) {
      ...
      // (path.relative returns 'forge-core\bin\...' on win32; allowlist
      // keys are POSIX 'forge-core/bin/...').
