...
const WORKFLOW = path.join(__dirname, '../forge-core/workflows/sync-skills.md');
const COMMAND = path.join(__dirname, '../commands/forge/sync-skills.md');
...
describe('sync-skills.md — required behavioral specs', () => {
  ...
  test('only forge-* directories are touched (non-GSD preservation)', () => {
    content = content || readWorkflow();
    assert.ok(
      content.includes('forge-*') && (content.includes('non-GSD') || content.includes('Non-GSD') || content.includes('not starting with')),
      'workflow must document that only forge-* dirs are modified'
    );
  });
...
// ── commands/forge/sync-skills.md ───────────────────────────────────────────────
// #2790: sync-skills.md was consolidated into update.md as the --sync flag.

describe('commands/forge/sync-skills.md', () => {
  test('sync-skills is now --sync flag on update.md (#2790)', () => {
    const updateCmd = path.join(__dirname, '../commands/forge/update.md');
    assert.ok(fs.existsSync(updateCmd), 'commands/forge/update.md must exist');
  ...
  test('sync-skills.md command file is deleted (#2790)', () => {
    assert.ok(!fs.existsSync(COMMAND), 'commands/forge/sync-skills.md should be deleted (consolidated into update.md)');
  });
});
...
describe('INVENTORY sync', () => {
  test('INVENTORY.md lists /forge-update --sync command (#2790: absorbed /forge-sync-skills)', () => {
    const inventory = fs.readFileSync(path.join(__dirname, '../docs/INVENTORY.md'), 'utf-8');
    assert.ok(inventory.includes('/forge-update --sync'), 'INVENTORY.md must list /forge-update --sync (absorbed /forge-sync-skills in #2790)');
  });
  ...
  test('INVENTORY-MANIFEST.json includes /forge-update (#2790: sync-skills absorbed into update.md --sync)', () => {
    // #2790: /forge-sync-skills was absorbed into /forge-update as the --sync flag.
    // The manifest now records /forge-update instead of /forge-sync-skills.
    const manifest = JSON.parse(
      fs.readFileSync(path.join(__dirname, '../docs/INVENTORY-MANIFEST.json'), 'utf-8')
    );
    assert.ok(
      manifest.families.commands.includes('/forge-update'),
      'INVENTORY-MANIFEST.json must include /forge-update in commands (absorbed /forge-sync-skills via #2790)'
    );
  });
