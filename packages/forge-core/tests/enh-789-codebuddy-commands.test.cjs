 ...
 *   - commands/forge-<stem>.md   — slash commands shown in the '/' menu
 *   - skills/forge-<stem>/SKILL.md — model-invocable skills
 *
 * Before #789 forge emitted only skills/. Because CodeBuddy skills default to
 * user-invocable:true (appear in '/'), emitting a commands/ surface AND leaving
 * skills user-invocable would duplicate every /forge-* entry. #789 therefore:
 *   1. emits commands/forge-<stem>.md (the '/' surface, peer-consistent with
 ...
 *
 * mcp.json is intentionally NOT written: forge ships no MCP server, and CodeBuddy's
 * mcp.json holds an `mcpServers` map of *external* servers to connect to —
 * there is nothing for forge to register. Same exclusion as #784/#785/#790.
 */
'use strict';
...
describe('enh-789 — codebuddy layout has commands + skills kinds', () => {
  ...
  test('codebuddy commands kind targets commands/ with forge- prefix', () => {
    const layout = resolveRuntimeArtifactLayout('codebuddy', '/tmp/fake-codebuddy-dir');
  ...
  test('codebuddy skills kind targets skills/ with forge- prefix', () => {
    const layout = resolveRuntimeArtifactLayout('codebuddy', '/tmp/fake-codebuddy-dir');
...
describe('enh-789 — convertClaudeCommandToCodebuddyCommand', () => {
  const SRC = [
    ...
    '---',
    '',
    'Use .claude/skills/ and run /forge:help. Claude Code reads CLAUDE.md.',
    '',
  ].join('\n');

  test('emits a description-only frontmatter (no Claude-specific name: forge:)', () => {
    const out = convertClaudeCommandToCodebuddyCommand(SRC, 'forge-new-project');
    assert.ok(out.startsWith('---\n'), 'must begin with frontmatter');
    assert.ok(/^description:/m.test(out), 'must carry a description field');
    assert.ok(!out.includes('name: forge:new-project'), 'must drop Claude colon-form name field');
  });
  ...
  test('converts body Claude-isms to CodeBuddy equivalents', () => {
    ...
    assert.ok(out.includes('/forge-help'), out);
...
describe('enh-789 — skills marked user-invocable:false', () => {
  test('convertClaudeCommandToCodebuddySkill emits user-invocable: false', () => {
    ...
    const out = convertClaudeCommandToCodebuddySkill(src, 'forge-help');
...
describe('enh-789 — installRuntimeArtifacts codebuddy emits commands and skills', () => {
  test('global codebuddy install: commands/forge-help.md and skills/forge-help/SKILL.md exist', (t) => {
    const configDir = createTempDir('forge-enh789-codebuddy-');
  ...
  test('installed commands/forge-help.md is CodeBuddy-compatible (no raw ~/.claude/, rebranded)', (t) => {
    const configDir = createTempDir('forge-enh789-content-');
    t.after(() => cleanup(configDir));
  ...
  test('installed skills/forge-help/SKILL.md is hidden from the / menu', (t) => {
    const configDir = createTempDir('forge-enh789-skillhide-');
    t.after(() => cleanup(configDir));

    installRuntimeArtifacts('codebuddy', configDir, 'global', RESOLVED_CORE);

    const skill = fs.readFileSync(path.join(configDir, 'skills', 'forge-help', 'SKILL.md'), 'utf8');
  ...
  test('command count matches skill count (profile parity)', (t) => {
    const configDir = createTempDir('forge-enh789-parity-');
    t.after(() => cleanup(configDir));
  ...
  test('full profile install: no $HOME/.codebuddy or ~/.codebuddy leak in any command', (t) => {
    ...
    // refs like `@$HOME/.claude/forge-core/...` (e.g. plan-review-convergence.md)
    ...
    const configDir = createTempDir('forge-enh789-noleak-');
    t.after(() => cleanup(configDir));
  ...
  test('full profile install does NOT mutate source commands/forge/ files', (t) => {
    ...
    const configDir = createTempDir('forge-enh789-full-');
    t.after(() => cleanup(configDir));
    ...
    assert.strictEqual(before, after, 'source commands/forge/help.md must not be mutated by the install');
  });
});
...
describe('enh-789 — uninstallRuntimeArtifacts removes codebuddy commands', () => {
  test('uninstall removes forge-* commands but preserves user commands', (t) => {
    const configDir = createTempDir('forge-enh789-uninstall-');
    t.after(() => cleanup(configDir));
...
describe('enh-789 — mcp.json excluded (gsd ships no MCP server)', () => {
  test('codebuddy install does not write mcp.json / .mcp.json', (t) => {
    const configDir = createTempDir('forge-enh789-mcp-excluded-');
    t.after(() => cleanup(configDir));
