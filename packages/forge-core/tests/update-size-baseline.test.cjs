...
describe('generateBaseline', () => {
  ...
  let outPath;
  beforeEach(() => {
    dir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-gen-baseline-'));
  ...
  test('predicate filters which files are baselined (agent path)', () => {
    ...
    fs.writeFileSync(path.join(agentDir, 'forge-one.md'), 'a\n');
    fs.writeFileSync(path.join(agentDir, 'forge-two.md'), 'bb\n');
    fs.writeFileSync(path.join(agentDir, 'README.md'), 'not an agent\n');

    const result = generateBaseline({
      dir: agentDir,
      outPath,
      predicate: (f) => f.startsWith('forge-'),
    });
    assert.strictEqual(result.count, 2, 'only forge-*.md files should be baselined');
    const written = JSON.parse(fs.readFileSync(outPath, 'utf-8'));
    assert.deepStrictEqual(Object.keys(written), ['forge-one.md', 'forge-two.md']);
  });
});
