...
// Verify that forge-context-monitor and forge-statusline reject session_id values
...
describe('forge-context-monitor session_id path traversal', () => {
  const monitorPath = path.join(__dirname, '..', 'hooks', 'forge-context-monitor.js');
...
describe('forge-statusline session_id path traversal', () => {
  const statuslinePath = path.join(__dirname, '..', 'hooks', 'forge-statusline.js');
  const tmpDir = os.tmpdir();
  ...
  test('does not write bridge file for session_id with ../ traversal', () => {
    const maliciousId = '../../../etc/forge-test';
...
describe('validatePromptStructure', () => {
  ...
  test('accepts known valid tags in agent files', () => {
    const text = [
      ...
      '<available_agent_types>forge-executor</available_agent_types>',
    ].join('\n');
