...
const SCAN_DIRS = ['agents', 'commands', path.join('forge-core', 'references')];

// Fix #3: broaden to catch backtick-delimited and split-quoted forms.
// Matches: node <optional-quote/backtick> ($HOME|${HOME}|~) <any non-newline chars> forge-tools.cjs
...
test('no hardcoded $HOME forge-tools.cjs in bash blocks of agents/, commands/, and forge-core/references/', () => {
...
test('each scan dir (agents/, commands/, forge-core/references/) exists and contains at least one bash block', () => {
