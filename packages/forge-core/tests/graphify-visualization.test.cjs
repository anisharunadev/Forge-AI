...
const {
  graphifyStatus,
} = require('../forge-core/bin/lib/graphify.cjs');
...
describe('mvp-viz', () => {
  // Contract: commands/forge/graphify.md documents MVP visual differentiation.
  ...
  const CMD = path.join(__dirname, '..', 'commands', 'forge', 'graphify.md');
...
describe('regressions', () => {
  // ── Regression for #3166 ────────────────────────────────────────────────────
  // /forge-graphify build lost artifacts because the skill spawned a Task
  ...
  const SKILL_PATH = path.join(__dirname, '..', 'commands', 'forge', 'graphify.md');
  ...
  test('a bash code block invokes the inline graphify update . pipeline', () => {
    ...
    assert.ok(
      bashBlocks.some(b => /forge_run\s+graphify build snapshot/.test(b.content)),
      'a bash code block must invoke `forge_run graphify build snapshot`'
    );
  });
  ...
  //   Gap 2: hooks/lib/forge-graphify-rebuild.sh not copied by installer
  ...
  describe('#3579 Gap 1: build-hooks.js packages every top-level hooks/*.sh into dist', () => {
    ...
    test('hooks/dist/forge-graphify-update.sh exists after build', () => {
      assert.ok(
        fs.existsSync(path.join(DIST_DIR_3579, 'forge-graphify-update.sh')),
        'expected hooks/dist/forge-graphify-update.sh to exist after build (Gap 1)'
      );
    });

    test('hooks/dist/lib/forge-graphify-rebuild.sh exists after build', () => {
      assert.ok(
        fs.existsSync(path.join(DIST_DIR_3579, 'lib', 'forge-graphify-rebuild.sh')),
        'expected hooks/dist/lib/forge-graphify-rebuild.sh to exist after build (Gap 2)'
      );
    });
  });
  ...
  describe('#3579: installer deploys graphify hook + lib helper to target', () => {
    ...
    before(() => {
      execFileSync(process.execPath, [BUILD_SCRIPT_3579], { encoding: 'utf-8', stdio: 'pipe' });
      tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-3579-install-'));
    ...
    test('hooks/forge-graphify-update.sh present at install target', () => {
      const dest = path.join(tmpDir, 'hooks', 'forge-graphify-update.sh');
      assert.ok(fs.existsSync(dest), `expected ${dest} to exist after install`);
    });

    test('hooks/lib/forge-graphify-rebuild.sh present at install target', () => {
      const dest = path.join(tmpDir, 'hooks', 'lib', 'forge-graphify-rebuild.sh');
      assert.ok(fs.existsSync(dest), `expected ${dest} to exist after install`);
    });

    test('installer does not warn about missing forge-graphify-update.sh', () => {
      assert.ok(
        !installStdout.includes('Missing expected hook: forge-graphify-update.sh'),
        `installer output must not warn about missing graphify hook; got:\n${installStdout}`
      );
      assert.ok(
        !installStdout.includes(
          'Skipped graphify auto-update hook — forge-graphify-update.sh not found'
        ),
