...
/**
 * #2529 — /forge-settings-integrations: configure third-party search and review integrations.
 *
 * Covers:
 ...
 *   - /forge:settings confirmation output mentions /forge:settings-integrations
...
const {
  VALID_CONFIG_KEYS,
  isValidConfigKey,
} = require('../forge-core/bin/lib/config-schema.cjs');

const REPO_ROOT = path.join(__dirname, '..');
// #2790: settings-integrations.md was consolidated into config.md as the --integrations flag.
const COMMAND_PATH = path.join(REPO_ROOT, 'commands', 'forge', 'config.md');
const WORKFLOW_PATH = path.join(REPO_ROOT, 'forge-core', 'workflows', 'settings-integrations.md');
const SKILL_PATH = path.join(REPO_ROOT, '.claude', 'skills', 'forge-settings-integrations.md');
const SETTINGS_WORKFLOW_PATH = path.join(REPO_ROOT, 'forge-core', 'workflows', 'settings.md');
...
describe('#2529 artifacts', () => {
  ...
  test('config.md frontmatter declares name forge:config and routes to --integrations', () => {
    const src = fs.readFileSync(COMMAND_PATH, 'utf-8');
    // #2790: consolidated command uses forge:config name
    assert.match(src, /name:\s*forge:config/);
    assert.match(src, /description:\s*.+/);
    assert.match(src, /allowed-tools:/);
    assert.match(src, /AskUserQuestion/);
  });

  test('workflow exists at forge-core/workflows/settings-integrations.md', () => {
...
describe('#2529 workflow — agent_skills injection', () => {
  ...
  test('agent_skills.<valid-slug> passes validator', () => {
    assert.ok(isValidConfigKey('agent_skills.forge-executor'));
    assert.ok(isValidConfigKey('agent_skills.forge-planner'));
...
describe('#2529 config-set round-trip', () => {
  ...
  test('config-set round-trips agent_skills.<agent-type>', (t) => {
    ...
    const r = runGsdTools(
      ['config-set', 'agent_skills.forge-executor', 'skill-a,skill-b'],
      tmp
    );
    assert.ok(r.success, `agent_skills.forge-executor set failed: ${r.error}`);
    const cfg = JSON.parse(fs.readFileSync(path.join(tmp, '.planning', 'config.json'), 'utf-8'));
    // Accept either array or string — validator accepts both shapes today.
    const v = cfg.agent_skills?.['forge-executor'];
    assert.ok(v === 'skill-a,skill-b' || (Array.isArray(v) && v.join(',') === 'skill-a,skill-b'),
      `expected agent_skills.forge-executor to contain both skills, got ${JSON.stringify(v)}`);
  });
});
...
describe('#2529 config merge safety', () => {
  ...
  test('setting agent_skills.forge-executor preserves unrelated review.models.codex', (t) => {
    const tmp = createTempProject();
    t.after(() => cleanup(tmp));
    runGsdTools(['config-ensure-section'], tmp);
    runGsdTools(['config-set', 'review.models.codex', 'codex exec'], tmp);

    const r = runGsdTools(['config-set', 'agent_skills.forge-planner', 'a,b'], tmp);
    assert.ok(r.success, `set failed: ${r.error}`);

    const cfg = JSON.parse(fs.readFileSync(path.join(tmp, '.planning', 'config.json'), 'utf-8'));
    assert.strictEqual(cfg.review?.models?.codex, 'codex exec', 'unrelated review.models.codex must be preserved');
    assert.ok(cfg.agent_skills?.['forge-planner'], 'agent_skills.forge-planner must be set');
  });
});

// ─── /forge-settings mentions /forge-settings-integrations ──────────────────────

describe('#2529 /forge-settings mentions new command', () => {
  test('settings workflow mentions canonical /forge-config --integrations', () => {
    const src = fs.readFileSync(SETTINGS_WORKFLOW_PATH, 'utf-8');
    assert.ok(
      src.includes('/forge:config --integrations'),
      'settings.md must mention /forge:config --integrations'
    );
    assert.ok(
      !src.includes('/forge-settings-integrations'),
      'settings.md must not mention the legacy /forge-settings-integrations variant'
    );
  });
});
