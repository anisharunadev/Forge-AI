...
/**
 * Tests for forge-read-guard.js PreToolUse hook.
 *
...
const HOOK_PATH = path.join(__dirname, '..', 'hooks', 'forge-read-guard.js');
...
describe('forge-read-guard hook', () => {
  let tmpDir;

  beforeEach(() => {
    tmpDir = createTempDir('forge-read-guard-');
  });
  ...
  test('hook is registered in build-hooks.js HOOKS_TO_COPY', () => {
    ...
    assert.ok(
      content.includes('forge-read-guard.js'),
      'forge-read-guard.js must be in HOOKS_TO_COPY so it ships in hooks/dist/'
    );
  });
  ...
  test('hook is registered in install.js uninstall hook list', () => {
    ...
    assert.ok(
      content.includes("'forge-read-guard.js'"),
      'forge-read-guard.js must be in the uninstall gsdHooks list'
    );
  });
