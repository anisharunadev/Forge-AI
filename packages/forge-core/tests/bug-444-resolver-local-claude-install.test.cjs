'use strict';
/**
 * Regression test for bug #444: forge_run resolver must probe
 * <repo-root>/.claude/forge-core/bin/forge-tools.cjs (the project-local
 ...
 * (B) Behavioral: when RUNTIME_DIR/forge-core/bin/ misses, but a stub
 *     exists ONLY at <repo-root>/.claude/forge-core/bin/forge-tools.cjs,
 *     forge_run resolves to that stub (no PATH stub, no HOME stub).
...
// arm in the forge_run launcher snippet -- asserts literal substring presence and exercises
...
const LOCAL_CLAUDE_PROBE = '_GSD_RUNTIME_ROOT}/.claude/forge-core/bin/';

/**
 * Build a PATH that strips forge-tools but keeps node and system binaries.
 * Accepts additional bin dirs to prepend.
 *
 * We cannot simply remove the whole directory that contains forge-tools because
 ...
 * having no forge-core/bin/ sub-path, so the resolver's first two checks
 * (RUNTIME_DIR/forge-core/bin/ and RUNTIME_DIR/.claude/forge-core/bin/)
 * are the only ones exercised before we hit our stub.
 *
 * The extra extraBefore dirs (e.g. noToolsBin) sit first but have no forge-tools
 * binary, so command -v forge-tools still falls back to PATH lookup. However,
 * the snippet's elif arm that uses `command -v forge-tools` will find the real
 * installed one unless we mask it. To mask it without losing node, we create
 * a noToolsBin dir that shadows forge-tools with a sentinel that must NOT be
...
function makeIsolatedPath(extraBefore = []) {
  ...
  // before command -v forge-tools — so the real forge-tools on PATH is never reached.
...
describe('bug-444: resolver finds repo-local .claude install', () => {
  ...
  test('(A) snippet file contains the repo-local .claude/ check arm before $HOME/.claude/', () => {
    ...
    const homeClaudeIdx = content.indexOf('$HOME/.claude/forge-core/bin/');
    assert.ok(
      homeClaudeIdx !== -1,
  ...
  test('(B) forge_run resolves repo-local .claude/forge-core/bin/ stub when no other locations present', () => {
    // Create a fake repo root with a stub ONLY at .claude/forge-core/bin/forge-tools.cjs
    // NO stub at forge-core/bin/, NOT on PATH, NOT in $HOME/.claude
    const fakeRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-444-root-'));
    const fakeHome = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-444-home-'));
    ...
    try {
      ...
      const localClaudeBinDir = path.join(fakeRoot, '.claude', 'forge-core', 'bin');
      fs.mkdirSync(localClaudeBinDir, { recursive: true });
      const stubPath = path.join(localClaudeBinDir, 'forge-tools.cjs');
      fs.writeFileSync(
        stubPath,
      ...
      // Keep node in PATH (needed to run the .cjs stub); remove forge-tools
      ...
      assert.ok(
        normStdout.includes('.claude/forge-core/bin/forge-tools.cjs'),
        `Expected GSD_TOOLS to resolve to .claude/forge-core/bin/forge-tools.cjs, got:\n${stdout.trim()}`,
      );
  ...
  test('(C) repo-local .claude/ install wins over $HOME/.claude/ when both exist', () => {
    const fakeRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-444-prec-root-'));
    const fakeHome = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-444-prec-home-'));
    const noToolsBin = path.join(fakeRoot, 'nobin');
    fs.mkdirSync(noToolsBin, { recursive: true });
    ...
    try {
      ...
      const localClaudeBinDir = path.join(fakeRoot, '.claude', 'forge-core', 'bin');
      fs.mkdirSync(localClaudeBinDir, { recursive: true });
      const localStubPath = path.join(localClaudeBinDir, 'forge-tools.cjs');
      fs.writeFileSync(
        localStubPath,
      ...
      const homeClaudeBinDir = path.join(fakeHome, '.claude', 'forge-core', 'bin');
      fs.mkdirSync(homeClaudeBinDir, { recursive: true });
      const homeStubPath = path.join(homeClaudeBinDir, 'forge-tools.cjs');
      fs.writeFileSync(
        homeStubPath,
