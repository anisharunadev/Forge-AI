'use strict';
/**
 * Parity test for bug #373: space-safe forge_run launcher
 *
 * Asserts:
 * (A) No retired GSD_SDK token remains in any workflow .md file.
 * (B) Each workflow .md that uses forge_run contains EXACTLY ONE canonical preamble
 *     (byte-equal to _runtime-launcher.snippet.sh), and it appears before the first
 *     forge_run call. NOT every bash block — exactly one per file (define once, use
 *     across blocks — original footprint).
 * (C) Space-safe behavioral: a RUNTIME_DIR path with spaces in it resolves
 *     and calls forge-tools.cjs correctly (no word-split, no {}).
 * (D) Loud guard behavioral: missing forge-tools.cjs exits non-zero and emits
 *     "not found" to stderr.
 * (E) PATH fallback behavioral: when no local forge-tools.cjs, the elif branch
 *     resolves to the forge-tools binary on PATH (#3668).
 * (F) Regression locks: the snippet file contains no /forge-tools substring; and
 *     no line in workflows/do.md matches /\/gsd[:-][a-z]/ (dispatcher-parity
 *     scanner must not read the preamble as a slash-command stub).
 * (H) Codex shim fallback: when PATH has no forge-tools, $HOME/.codex/forge-core/bin
 *     can satisfy forge_run for Codex shim-only installs.
 */

// allow-test-rule: structural parity/drift guard — asserts literal presence/absence of the canonical forge_run launcher and the retired $GSD_SDK / `/forge-tools` tokens across workflow markdown; there is no typed IR for "this source file does not contain substring X".
...
describe('runtime-launcher-parity (#373)', () => {
  ...
  test('(B) each workflow .md using forge_run contains exactly ONE canonical preamble, before the first forge_run call', () => {
    ...
    for (const f of files) {
      ...
      // Does this file use forge_run at all?
      const fileHasGsdRun = allBlockLines.some((l) => /\bforge_run\b/.test(l));
      ...
      // Verify preamble appears BEFORE the first forge_run call (in document order)
      // Find the line index of the preamble start vs the first forge_run call in the flat content
      const preamblePos = allContent.indexOf(preambleStr);
      const firstGsdRunPos = allContent.search(/\bforge_run\b/);

      // The first forge_run WITHIN the preamble itself (the function definition) is fine.
      // We need to verify that no forge_run CALL (i.e. forge_run used as a command, not in a
      // function definition body) appears before the preamble starts.
      // Simple check: preamble starts at or before the first forge_run occurrence
      if (preamblePos > firstGsdRunPos) {
        violations.push(
          `${rel}: preamble appears AFTER the first forge_run reference — it must precede all forge_run calls.`,
        );
      }
    }
    ...
    assert.deepStrictEqual(
      violations,
      [],
      'Files with forge_run calls have wrong preamble count or ordering:\n' +
        violations.join('\n---\n'),
    );
  });

  // ─── (C) Space-safe behavioral test ──────────────────────────────────────
  test('(C) forge_run works with a RUNTIME_DIR path containing spaces', () => {
    // Create temp dir whose path contains a space
    const base = fs.mkdtempSync(path.join(os.tmpdir(), 'gsd 373 '));
    try {
      const binDir = path.join(base, 'forge-core', 'bin');
      fs.mkdirSync(binDir, { recursive: true });

      // Stub forge-tools.cjs that prints its argv
      const stub = path.join(binDir, 'forge-tools.cjs');
      fs.writeFileSync(stub, '#!/usr/bin/env node\nconsole.log("STUB:" + process.argv.slice(2).join(","));\n');
      fs.chmodSync(stub, 0o755);

      // Build a shell script: set RUNTIME_DIR, source preamble, run forge_run
      const snippet = fs.readFileSync(SNIPPET_FILE, 'utf8');
      const scriptContent =
        `export RUNTIME_DIR=${JSON.stringify(base)}\n` +
        snippet +
        `\nforge_run query state.json\n`;
  ...
  test('(D) missing forge-tools.cjs and no PATH forge-tools causes loud non-zero exit with "not found" on stderr', () => {
    // Create temp dir with a space in the name, but NO forge-tools.cjs.
    // We ensure forge-tools is not on PATH by prepending a dir that has no
    // forge-tools binary (system binaries remain on PATH so bash/node work).
    const base = fs.mkdtempSync(path.join(os.tmpdir(), 'gsd 373 notools '));
    // Place a no-op dir first in PATH; no forge-tools stub there.
    ...
    try {
      ...
      // Build a PATH that has noToolsBin first (no forge-tools stub there) but retains
      // system paths needed for bash. Exclude any PATH entry that contains a forge-tools binary.
      const systemPaths = (process.env.PATH || '/usr/bin:/bin')
        .split(path.delimiter)
        .filter((p) => {
          try { fs.accessSync(path.join(p, 'forge-tools'), fs.constants.X_OK); return false; }
          catch { return true; }
        });
      ...
      assert.ok(threw, 'Expected the script to exit non-zero when forge-tools.cjs is missing and forge-tools is not on PATH');
      assert.ok(
  ...
  test('(E) PATH fallback: uses installed forge-tools when no local forge-tools.cjs present', () => {
    // Create a temp dir with NO local forge-core/bin/forge-tools.cjs.
    // Place an executable forge-tools stub on a dedicated PATH dir.
    // RUNTIME_DIR points somewhere that has no forge-tools.cjs.
    ...
    try {
      ...
      // Stub installed forge-tools binary that prints a marker
      const stubPath = path.join(pathBinDir, 'forge-tools');
      fs.writeFileSync(stubPath, '#!/bin/sh\necho "installed:$*"\n');
      fs.chmodSync(stubPath, 0o755);

      // RUNTIME_DIR points to base — no forge-core/bin/forge-tools.cjs there
      const snippet = fs.readFileSync(SNIPPET_FILE, 'utf8');
      const scriptContent =
        `export RUNTIME_DIR=${JSON.stringify(base)}\n` +
        snippet +
      ...
      // Assert by suffix (/bin/forge-tools, no .cjs extension) rather than absolute prefix
      ...
      assert.match(
        normStdout,
        /GSD_TOOLS=.+\/bin\/forge-tools(?:\s|$)/m,
        `Expected GSD_TOOLS to resolve to the installed PATH stub (suffix /bin/forge-tools), got: ${stdout.trim()}`,
      );
      assert.doesNotMatch(
        normStdout,
        /GSD_TOOLS=.+\.cjs/m,
  ...
  test('(G) snippet and all propagated workflow .md files contain the $HOME/.claude fallback arm between PATH check and hard error', () => {
    ...
    //   (1) local/RUNTIME_DIR  →  (2) PATH  →  (3) $HOME/.claude/forge-core/bin  →  (4) hard error
    // We probe for .claude/forge-core/bin (using ${_GSD_SHIM_NAME} indirection)
    // between the `command -v forge-tools` elif and the hard-error else branch.
    ...
    assert.ok(
      ...
      `_runtime-launcher.snippet.sh must contain the $HOME/.claude fallback arm (probing "${CLAUDE_HOME_PROBE}"). ` +
        `Add an elif arm that checks $HOME/.claude/forge-core/bin/\${_GSD_SHIM_NAME} before the hard-error else.`,
    );
    ...
    // Assert every propagated workflow .md file that uses forge_run also contains the probe
    const files = collectWorkflowFiles();
    const missing = [];
    for (const f of files) {
      const content = fs.readFileSync(f, 'utf8');
    ...
    assert.deepStrictEqual(
      missing,
      [],
      `These workflow files use forge_run but are missing the $HOME/.claude fallback arm ("${CLAUDE_HOME_PROBE}"). ` +
  ...
  test('(H) forge_run resolves $HOME/.codex/forge-core/bin/ shim when PATH has no forge-tools', () => {
    const CODEX_HOME_PROBE = '.codex/forge-core/bin/';
    ...
    assert.deepStrictEqual(
      missing,
      [],
      `These workflow files use forge_run but are missing the Codex fallback arm ("${CODEX_HOME_PROBE}"). ` +
        `Run \`node scripts/sync-runtime-launcher.cjs\` to propagate:\n` +
        missing.join('\n'),
    );

    const fakeHome = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-codex-home-'));
    const fakeRuntime = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-codex-rt-'));
    try {
      const codexBinDir = path.join(fakeHome, '.codex', 'forge-core', 'bin');
      fs.mkdirSync(codexBinDir, { recursive: true });
      const stubPath = path.join(codexBinDir, 'forge-tools.cjs');
      fs.writeFileSync(
        stubPath,
      ...
      const systemPaths = (process.env.PATH || '/usr/bin:/bin')
        .split(path.delimiter)
        .filter((p) => !hasExecutable(p, 'forge-tools'));
      ...
      const normStdout = stdout.replace(/\\/g, '/');
      assert.ok(
        normStdout.includes('.codex/forge-core/bin/'),
        `Expected GSD_TOOLS to resolve into .codex/forge-core/bin/, got:\n${stdout.trim()}`,
      );
      assert.ok(
  ...
  // ─── (F) Regression locks: no /forge-tools substring; no do.md dispatcher false-positive ──
  test('(F) snippet has no /forge-tools substring; do.md has no /gsd[:-][a-z] matches', () => {
    // (F1) The snippet must not contain the literal substring /forge-tools.
    // The _GSD_SHIM_NAME indirection ensures bin/${_GSD_SHIM_NAME} instead of
    // bin/forge-tools.cjs — so the do.md dispatcher regex /\/gsd[:-]([a-z]...)/ never
    // misreads a preamble line as a slash-command stub.
    const snippetContent = fs.readFileSync(SNIPPET_FILE, 'utf8');
    assert.ok(
      !snippetContent.includes('/forge-tools'),
      `_runtime-launcher.snippet.sh must not contain the literal "/forge-tools" substring. ` +
        `Use bin/\${_GSD_SHIM_NAME} indirection to keep the /gsd[:-] scanner from ` +
        `misreading it as a slash-command stub. Found in snippet:\n` +
        snippetContent.split('\n').filter((l) => l.includes('/forge-tools')).join('\n'),
    );

    // (F2) workflows/do.md must not contain the literal substring /forge-tools
    ...
    // /forge-tools as a slash-command stub named "tools" — which is not shipped.
    // Note: /forge:command references (with colon) in the dispatch table are
    ...
    const offendingLines = doMdContent
      .split('\n')
      .filter((l) => /\/forge-tools/.test(l));
    assert.deepStrictEqual(
      offendingLines,
      [],
      `workflows/do.md contains the literal "/forge-tools" substring which the dispatcher-parity ` +
...
// ─── Issue #381: standalone forge_run executable + CLAUDE_ENV_FILE persistence ──
describe('runtime-launcher-parity — standalone executable (#381)', () => {
  const BIN_DIR = path.join(__dirname, '..', 'forge-core', 'bin');
  const GSD_RUN_SRC = path.join(BIN_DIR, 'forge_run');

  // ─── (I) forge_run executable delegates to forge-tools.cjs beside it ──────────
  test('(I) forge_run executable delegates to forge-tools.cjs beside it', () => {
    const base = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-381-I-'));
    try {
      const binDir = path.join(base, 'forge-core', 'bin');
      fs.mkdirSync(binDir, { recursive: true });

      // Copy the real forge_run executable into the temp bin dir
      fs.copyFileSync(GSD_RUN_SRC, path.join(binDir, 'forge_run'));
      fs.chmodSync(path.join(binDir, 'forge_run'), 0o755);

      // Write a stub forge-tools.cjs that echoes its args
      fs.writeFileSync(
        path.join(binDir, 'forge-tools.cjs'),
        `console.log('GSD_TOOLS_STUB:' + process.argv.slice(2).join(' '))`,
      );

      const stdout = execFileSync('sh', [path.join(binDir, 'forge_run'), 'query', 'x'], {
        encoding: 'utf8',
      });
      assert.ok(
  ...
  test('(J) preamble persists bin dir to CLAUDE_ENV_FILE so a fresh shell resolves forge_run', () => {
    // Use a RUNTIME_DIR whose path contains a SPACE to prove single-quote safety.
    const baseParent = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-381-J-'));
    const base = path.join(baseParent, 'has space');
    try {
      const binDir = path.join(base, 'forge-core', 'bin');
      fs.mkdirSync(binDir, { recursive: true });

      // forge_run stub that prints GSD_RUN_STUB:<args>
      const gsdRunStub = path.join(binDir, 'forge_run');
      fs.writeFileSync(gsdRunStub, '#!/bin/sh\necho "GSD_RUN_STUB:$*"\n');
      fs.chmodSync(gsdRunStub, 0o755);

      // forge-tools.cjs stub (must exist for preamble first arm to win)
      fs.writeFileSync(path.join(binDir, 'forge-tools.cjs'), '// stub');
      ...
      assert.ok(
        envFileContent.includes('has space/forge-core/bin'),
      ...
      if (process.platform !== 'win32') {
        // Simulate a LATER fresh block that SOURCES the env file to get forge_run on PATH.
        ...
        const stdout = execFileSync('bash', ['-c', '. "$CLAUDE_ENV_FILE"; forge_run hello'], {
          encoding: 'utf8',
          env: {
            CLAUDE_ENV_FILE: envFile,
            PATH: process.env.PATH,
...
describe('runtime-launcher-parity — agents (#1041)', () => {
  ...
  test('(B-agents) each agent .md using forge_run contains exactly ONE canonical preamble, before the first forge_run call', () => {
    const preamble = expectedPreamble();
    const preambleStr = preamble.join('\n');
    ...
    for (const f of files) {
      ...
      // Does this file use forge_run at all?
      const fileHasGsdRun = allBlockLines.some((l) => /\bforge_run\b/.test(l));
      if (!fileHasGsdRun) continue; // agents without forge_run are not checked
    ...
    assert.deepStrictEqual(
      violations,
      [],
      'Agent files with forge_run calls have wrong preamble count or ordering:\n' +
        violations.join('\n---\n'),
    );
  });
});
