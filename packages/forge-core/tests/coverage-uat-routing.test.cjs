...
describe('verify-work.md is wired to the deterministic classifier (deployed contract)', () => {
  const VERIFY_WORK = fs.readFileSync(path.join(ROOT, 'forge-core', 'workflows', 'verify-work.md'), 'utf-8');
...
describe('execute-plan.md create_summary populates the coverage block (deployed contract)', () => {
  const EXECUTE_PLAN = fs.readFileSync(path.join(ROOT, 'forge-core', 'workflows', 'execute-plan.md'), 'utf-8');
...
describe('SUMMARY templates carry the coverage field (deployed contract)', () => {
  const templates = {
    main: fs.readFileSync(path.join(ROOT, 'forge-core', 'templates', 'summary.md'), 'utf-8'),
    standard: fs.readFileSync(path.join(ROOT, 'forge-core', 'templates', 'summary-standard.md'), 'utf-8'),
    complex: fs.readFileSync(path.join(ROOT, 'forge-core', 'templates', 'summary-complex.md'), 'utf-8'),
    minimal: fs.readFileSync(path.join(ROOT, 'forge-core', 'templates', 'summary-minimal.md'), 'utf-8'),
  };
