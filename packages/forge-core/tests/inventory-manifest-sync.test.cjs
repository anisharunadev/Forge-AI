...
const FAMILIES = [
  ...
  { name: 'commands',    dir: path.join(ROOT, 'commands', 'forge'),                  filter: (f) => f.endsWith('.md'),        toName: (f) => '/forge-' + f.replace(/\.md$/, '') },
  { name: 'workflows',   dir: path.join(ROOT, 'forge-core', 'workflows'),        filter: (f) => f.endsWith('.md'),        toName: (f) => f },
  { name: 'references',  dir: path.join(ROOT, 'forge-core', 'references'),       filter: (f) => f.endsWith('.md'),        toName: (f) => f },
  { name: 'cli_modules', dir: path.join(ROOT, 'forge-core', 'bin', 'lib'),       filter: (f) => f.endsWith('.cjs'),       toName: (f) => f },
