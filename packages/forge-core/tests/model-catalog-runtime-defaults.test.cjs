...
const { catalog, KNOWN_RUNTIMES } = require('../forge-core/bin/lib/model-catalog.cjs');
const { allRuntimes } = require('../bin/install.js');

const ROOT = path.join(__dirname, '..');
const SETTINGS_ADVANCED = fs.readFileSync(path.join(ROOT, 'forge-core', 'workflows', 'settings-advanced.md'), 'utf8');
const CONFIG_DOC = fs.readFileSync(path.join(ROOT, 'docs', 'CONFIGURATION.md'), 'utf8');
const catalogPath = path.join(ROOT, 'forge-core', 'bin', 'shared', 'model-catalog.json');
