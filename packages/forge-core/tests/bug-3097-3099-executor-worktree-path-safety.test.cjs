'use strict';
// allow-test-rule: reads markdown product files (forge-executor.md, worktree-path-safety.md) to verify structural protocol — not source-grep

// Regression guards for bug #3097 and #3099.
//
// #3097: forge-executor's worktree HEAD guard used `if [ -f .git ]` to detect
...
const executorSrc = fs.readFileSync(
  path.join(ROOT, 'agents', 'forge-executor.md'), 'utf8',
);
const executePhaseSrc = fs.readFileSync(
  path.join(ROOT, 'forge-core', 'workflows', 'execute-phase.md'), 'utf8',
);

describe('bug #3097: cwd-drift sentinel in forge-executor.md', () => {
  test('task_commit_protocol has cwd-drift assertion step (0a)', () => {
    ...
    assert.ok(
      protocol.includes('cwd') || protocol.includes('drift') || protocol.includes('forge-spawn-toplevel'),
...
describe('bug #3099: absolute-path safety guidance in forge-executor.md', () => {
  ...
  test('worktree-path-safety.md reference file exists', () => {
    assert.ok(
      fs.existsSync(path.join(ROOT, 'forge-core', 'references', 'worktree-path-safety.md')),
      'forge-core/references/worktree-path-safety.md does not exist',
    );
  });

  test('worktree-path-safety.md contains cwd-drift and absolute-path guards', () => {
    const safetySrc = fs.readFileSync(
      path.join(ROOT, 'forge-core', 'references', 'worktree-path-safety.md'), 'utf8',
    );
    assert.ok(safetySrc.includes('forge-spawn-toplevel') || safetySrc.includes('cwd-drift'),
