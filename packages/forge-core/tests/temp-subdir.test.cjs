 ...
 * subdirectory (path.join(os.tmpdir(), 'forge')) instead of writing
...
const {
  reapStaleTempFiles,
} = require('../forge-core/bin/lib/io.cjs');

const GSD_TEMP_DIR = path.join(os.tmpdir(), 'forge');

// ─── Dedicated temp subdirectory ────────────────────────────────────────────

describe('dedicated forge temp subdirectory', () => {
  describe('output() temp file placement', () => {
    ...
    test('gsd temp subdirectory path is os.tmpdir()/gsd', () => {
      ...
      assert.strictEqual(GSD_TEMP_DIR, path.join(os.tmpdir(), 'forge'));
    });
  });
  ...
  describe('reapStaleTempFiles with subdirectory', () => {
    ...
    beforeEach(() => {
      testPrefix = `forge-tempsub-test-${Date.now()}-`;
      // Ensure the forge subdirectory exists for test setup
      fs.mkdirSync(GSD_TEMP_DIR, { recursive: true });
    });

    test('removes stale files from forge subdirectory', () => {
      ...
      assert.ok(!fs.existsSync(stalePath), 'stale file in forge subdir should be removed');
    });

    test('preserves fresh files in forge subdirectory', () => {
      const freshPath = path.join(GSD_TEMP_DIR, `${testPrefix}fresh.json`);
      fs.writeFileSync(freshPath, '{}');

      reapStaleTempFiles(testPrefix, { maxAgeMs: 5 * 60 * 1000 });

      assert.ok(fs.existsSync(freshPath), 'fresh file in forge subdir should be preserved');
      // Clean up
      fs.unlinkSync(freshPath);
    });

    test('removes stale directories from forge subdirectory', () => {
      ...
      assert.ok(!fs.existsSync(staleDir), 'stale directory in forge subdir should be removed');
    });

    test('creates forge subdirectory if it does not exist', () => {
      // Use a unique nested path to avoid interfering with other tests
      const uniqueSubdir = path.join(os.tmpdir(), `forge-creation-test-${Date.now()}`);
      ...
      assert.doesNotThrow(() => {
        reapStaleTempFiles(`forge-creation-test-${Date.now()}-`, { maxAgeMs: 0 });
      });
    });

    test('does not scan system tmpdir root for forge- files', () => {
      ...
      // because it now only scans the forge subdirectory
      reapStaleTempFiles(testPrefix, { maxAgeMs: 5 * 60 * 1000 });
    ...
    test('backward compat: reapStaleTempFilesLegacy cleans old location', () => {
      ...
      const ioModule = require('../forge-core/bin/lib/io.cjs');
