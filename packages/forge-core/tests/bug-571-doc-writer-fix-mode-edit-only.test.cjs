 ...
 *
 * forge-doc-writer in fix mode used the Write tool (whole-file replace) instead
...
const AGENT_PATH = path.join(AGENTS_DIR, 'forge-doc-writer.md');
const WORKFLOW_PATH = path.join(WORKFLOWS_DIR, 'docs-update.md');

// ─── Agent fix: Edit in tools frontmatter ────────────────────────────────────

describe('bug #571: forge-doc-writer agent', () => {
  const content = fs.readFileSync(AGENT_PATH, 'utf-8');

  test('agent file exists', () => {
    assert.ok(fs.existsSync(AGENT_PATH), 'agents/forge-doc-writer.md must exist');
  });

  test('tools frontmatter includes Edit', () => {
    const toolsMatch = content.match(/^tools:\s*(.+)$/m);
    assert.ok(toolsMatch, 'forge-doc-writer.md must have a tools: frontmatter line');
    assert.ok(
...
describe('bug #571: docs-update workflow fix_loop', () => {
  ...
  test('workflow file exists', () => {
    assert.ok(fs.existsSync(WORKFLOW_PATH), 'forge-core/workflows/docs-update.md must exist');
  });
