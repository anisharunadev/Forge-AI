 ...
 * commands from forge-core/workflows/*.md or agents/*.md. It only registers
 ...
 * workflows on disk and no $gsd-* entrypoints after `npx @forge-ai/forge-core
 * --codex --global`.
 *
...
describe('#3562 — Codex install produces discoverable $gsd-* skill surface', { concurrency: false }, () => {
  ...
  beforeEach(() => {
    ...
    tmpRoot = createTempDir('forge-3562-');
  ...
  test('global install creates skills/forge-help/SKILL.md', () => {
    withCodexHome(codexHome, () => install(true, 'codex'));

    const skillPath = path.join(codexHome, 'skills', 'forge-help', 'SKILL.md');
    assert.ok(
      fs.existsSync(skillPath),
      `Codex install must create ${skillPath} so $forge-help is discoverable. ` +
  ...
  test('SKILL.md content has frontmatter expected by Codex skill discovery', () => {
    ...
    assert.equal(frontmatter.name, 'forge-help', 'SKILL.md frontmatter must declare name: forge-help so $forge-help resolves');
  });

  test('multiple core $gsd-* skills are produced (not just forge-help)', () => {
    withCodexHome(codexHome, () => install(true, 'codex'));
    ...
    // commands/forge/ directory holds dozens of *.md files; expecting more than
    ...
    assert.ok(
      gsdSkills.length >= 10,
      `Expected >= 10 generated forge-* skill directories, found ${gsdSkills.length}: ${gsdSkills.join(', ')}`,
    );
  });
