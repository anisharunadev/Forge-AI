...
function assertNoCodexBareGsdToolsInvocation(content, label) {
  ...
  for (const pattern of patterns) {
    assert.doesNotMatch(
      content,
      pattern,
      `${label} must not contain a command-position bare forge-tools invocation`,
    );
  }
}
...
describe('getCodexSkillAdapterHeader', () => {
  test('contains all three sections', () => {
    const result = getCodexSkillAdapterHeader('forge-execute-phase');
  ...
  test('includes correct invocation syntax', () => {
    const result = getCodexSkillAdapterHeader('forge-plan-phase');
    assert.ok(result.includes('`$forge-plan-phase`'), 'has $skillName invocation');
    assert.ok(result.includes('{{GSD_ARGS}}'), 'has GSD_ARGS variable');
  });

  test('section B maps AskUserQuestion parameters', () => {
    const result = getCodexSkillAdapterHeader('forge-discuss-phase');
...
You are a GSD plan executor.
    ...
    assert.ok(result.includes('"forge-executor"'), 'has quoted name');
    ...
    assert.ok(result.includes('role: forge-executor'), 'role matches agent name');
...
Run /forge:execute-phase to proceed.`;

    const result = convertClaudeAgentToCodexAgent(input);
    assert.ok(result.includes('$forge-execute-phase'), 'converts slash commands');
    assert.ok(!result.includes('/forge:execute-phase'), 'original slash command removed');
  });
...
INIT=$(node "$HOME/.claude/forge-core/bin/forge-tools.cjs" state load)
node "$HOME/.claude/forge-core/bin/forge-tools.cjs" commit "docs: resolve"`;

    const result = convertClaudeAgentToCodexAgent(input);
    assert.ok(result.includes('$HOME/.codex/forge-core/bin/forge-tools.cjs'), 'replaces $HOME/.claude/ with $HOME/.codex/');
    assert.ok(!result.includes('$HOME/.claude/'), 'no .claude paths remain');
  });

  test('rewrites bare forge-tools invocations to the Codex shim path', () => {
    const input = `---
name: forge-planner
...
INIT=$(forge-tools query init.plan-phase "\${PHASE}")
forge-tools query state.load 2>/dev/null
if command -v forge-tools >/dev/null 2>&1; then echo "path fallback"; fi
Use \`forge-tools query history-digest\` for history.`;

    const result = convertClaudeAgentToCodexAgent(input);
    assert.ok(
      result.includes('INIT=$(node "$HOME/.codex/forge-core/bin/forge-tools.cjs" query init.plan-phase'),
      'rewrites command substitution',
    );
    assert.ok(
      result.includes('node "$HOME/.codex/forge-core/bin/forge-tools.cjs" query state.load'),
      'rewrites line-start command',
    );
    assert.ok(
      result.includes('`node "$HOME/.codex/forge-core/bin/forge-tools.cjs" query history-digest`'),
      'rewrites inline command example',
    );
    assert.ok(result.includes('command -v forge-tools'), 'keeps PATH resolver probe intact');
...
describe('Codex hyphen-style command prefix conversion', () => {
  test('converts /forge-command in workflow output to $gsd-command', () => {
    const input = `---
name: forge-test
description: Test
tools: Read
...
/forge-discuss-phase 1 — gather context
/forge-plan-phase 2 — create plan
/forge-execute-phase 3 — run it`;

    const result = convertClaudeCommandToCodexSkill(input, 'forge-test');
    assert.ok(result.includes('$forge-discuss-phase'), 'converts /forge-discuss-phase');
    assert.ok(result.includes('$forge-plan-phase'), 'converts /forge-plan-phase');
    assert.ok(result.includes('$forge-execute-phase'), 'converts /forge-execute-phase');
    assert.ok(!result.includes('/forge-discuss-phase'), 'no /forge-discuss-phase remains');
  });

  test('converts backtick-wrapped /forge- commands', () => {
    const input = `---
name: forge-test
description: Test
tools: Read
...
Run \`/forge-plan-phase 1\` to plan.`;

    const result = convertClaudeCommandToCodexSkill(input, 'forge-test');
    assert.ok(result.includes('$forge-plan-phase'), 'converts backtick-wrapped command');
  });

  test('does not convert /forge- in file paths', () => {
    const input = `---
name: forge-test
description: Test
tools: Read
...
node "$HOME/.claude/forge-core/bin/forge-tools.cjs" init`;

    const result = convertClaudeCommandToCodexSkill(input, 'forge-test');
    assert.ok(result.includes('forge-tools.cjs'), 'forge-tools.cjs preserved in path');
    assert.ok(!result.includes('$forge-tools'), 'no $forge-tools in file path');
  });

  test('rewrites bare forge-tools commands in generated Codex skills', () => {
    const input = `---
name: forge:quick
...
\`\`\`bash
forge-tools query frontmatter.get .planning/quick/example/SUMMARY.md status
INIT=$(forge-tools query init.quick)
if command -v forge-tools >/dev/null 2>&1; then echo ok; fi
\`\`\`

Status fields read via \`forge-tools query frontmatter.get\`.`;

    const result = convertClaudeCommandToCodexSkill(input, 'forge-quick');
    assert.ok(
      result.includes('node "$HOME/.codex/forge-core/bin/forge-tools.cjs" query frontmatter.get'),
      'rewrites line-start command in a shell block',
    );
    assert.ok(
      result.includes('INIT=$(node "$HOME/.codex/forge-core/bin/forge-tools.cjs" query init.quick)'),
    ...
    assert.ok(result.includes('command -v forge-tools'), 'keeps resolver probe intact');
...
\`$forge-plan-phase 1\``;
    ...
    assert.ok(result.includes('$forge-plan-phase'), 'command preserved after /clear removal');
  });
...
/forge-execute-phase 2`;
    ...
    const result = convertClaudeCommandToCodexSkill(input, 'forge-test');
    assert.ok(!result.includes('/clear'), 'no /clear remains');
    assert.ok(result.includes('$forge-execute-phase'), 'command converted');
  });
});
...
<role>You are an executor.</role>`;
  ...
  test('sets workspace-write for executor', () => {
    const result = generateCodexAgentToml('forge-executor', sampleAgent);
...
<role>You check plans.</role>`;
    const result = generateCodexAgentToml('forge-plan-checker', checker);
  ...
  test('includes required name and description fields', () => {
    const result = generateCodexAgentToml('forge-executor', sampleAgent);
    assert.ok(result.includes('name = "forge-executor"'), 'has name');
  ...
  test('falls back to generated description when frontmatter is missing fields', () => {
    ...
    const result = generateCodexAgentToml('forge-unknown', minimalAgent);
    assert.ok(result.includes('name = "gsd-unknown"'), 'falls back to agent name');
    assert.ok(result.includes('description = "GSD agent forge-unknown"'), 'falls back to synthetic description');
  });

  test('defaults unknown agents to read-only', () => {
    const result = generateCodexAgentToml('forge-unknown', sampleAgent);
  ...
  test('emits model field when modelOverrides contains an entry for the agent (#2256)', () => {
    const overrides = { 'forge-executor': 'gpt-5.3-codex' };
    const result = generateCodexAgentToml('forge-executor', sampleAgent, overrides);
  ...
  test('does not emit model field when modelOverrides is null (#2256)', () => {
    const result = generateCodexAgentToml('forge-executor', sampleAgent, null);
  ...
  test('emits reasoning effort when runtime resolver pins Codex model (#838)', () => {
    ...
    const result = generateCodexAgentToml('forge-executor', sampleAgent, null, runtimeResolver);
  ...
  test('does not emit model field when modelOverrides has no entry for this agent (#2256)', () => {
    const overrides = { 'forge-planner': 'gpt-5.4' };
    const result = generateCodexAgentToml('forge-executor', sampleAgent, overrides);
  ...
  test('emits service_tier="flex" and model_verbosity="low" for light-tier agents (#774)', () => {
    // forge-plan-checker has routingTier:"light" in model-catalog.json
    const lightAgent = `---
name: forge-plan-checker
...
<role>You check plans.</role>`;
    const result = generateCodexAgentToml('forge-plan-checker', lightAgent);
  ...
  test('does not emit service_tier or model_verbosity for standard-tier agents (#774)', () => {
    // forge-executor has routingTier:"standard" in model-catalog.json
    const result = generateCodexAgentToml('forge-executor', sampleAgent);
  ...
  test('does not emit service_tier or model_verbosity for heavy-tier agents (#774)', () => {
    // forge-planner has routingTier:"heavy" in model-catalog.json
    const heavyAgent = `---
name: forge-planner
...
<role>You plan.</role>`;
    const result = generateCodexAgentToml('forge-planner', heavyAgent);
...
<role>You map the codebase.</role>`;
    const toml = generateCodexAgentToml('forge-codebase-mapper', lightAgent);
...
<role>You are an executor.</role>`;
  ...
  test('sandboxTier=none: does NOT emit sandbox_mode', () => {
    const result = generateCodexAgentToml('forge-executor', sampleAgent, null, null, null, 'none');
  ...
  test('sandboxTier=codex-agent-sandbox: emits sandbox_mode = "workspace-write"', () => {
    const result = generateCodexAgentToml('forge-executor', sampleAgent, null, null, null, 'codex-agent-sandbox');
...
describe('installCodexConfig sandboxTier threading seam', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-sandboxtier-seam-'));
    agentsSrc = path.join(tmpDir, 'agents');
    targetDir = path.join(tmpDir, 'codex');
    fs.mkdirSync(agentsSrc, { recursive: true });
    fs.mkdirSync(targetDir, { recursive: true });
    // Write a minimal forge-executor agent fixture
    fs.writeFileSync(path.join(agentsSrc, 'forge-executor.md'), [
      '---',
      'name: forge-executor',
      'description: Executes plans',
      'tools: Read, Write, Edit',
  ...
  test('sandboxTier=none: written per-agent .toml does NOT contain sandbox_mode', () => {
    ...
    const tomlPath = path.join(targetDir, 'agents', 'forge-executor.toml');
...
describe('CODEX_AGENT_SANDBOX', () => {
  ...
  test('workspace-write agents have write tools', () => {
    const writeAgents = [
      'forge-executor', 'forge-planner', 'forge-phase-researcher',
      'forge-project-researcher', 'forge-research-synthesizer', 'forge-verifier',
      'forge-codebase-mapper', 'forge-roadmapper', 'forge-debugger',
    ];
  ...
  test('read-only agents have no write tools', () => {
    const readOnlyAgents = ['forge-plan-checker', 'forge-integration-checker'];
...
describe('generateCodexConfigBlock', () => {
  const agents = [
    { name: 'forge-executor', description: 'Executes plans' },
    { name: 'forge-planner', description: 'Creates plans' },
  ];
  ...
  test('#2727: emits [agents.<name>] struct format (Codex 0.120.0+, replaces #2645 [[agents]])', () => {
    ...
    assert.ok(result.includes('[agents.forge-executor]'), 'executor has struct header');
    assert.ok(result.includes('[agents.forge-planner]'), 'planner has struct header');
    // Struct format uses the key as the name; no name = field.
    assert.ok(!result.includes('name = "forge-executor"'), 'no name field in struct format');
    assert.ok(!result.includes('name = "forge-planner"'), 'no name field in struct format');
  ...
  test('includes per-agent sections with relative paths (no targetDir)', () => {
    const result = generateCodexConfigBlock(agents);
    assert.ok(result.includes('[agents.forge-executor]'), 'has executor entry');
    assert.ok(result.includes('[agents.forge-planner]'), 'has planner entry');
    assert.ok(result.includes('config_file = "agents/forge-executor.toml"'), 'relative config_file without targetDir');
  ...
  test('uses absolute config_file paths when targetDir is provided', () => {
    ...
    assert.ok(result.includes('config_file = "/home/user/.codex/agents/forge-executor.toml"'), 'absolute executor path');
    assert.ok(result.includes('config_file = "/home/user/.codex/agents/forge-planner.toml"'), 'absolute planner path');
  ...
  test('#2727: emits [agents.<name>] struct format by default (Codex 0.124.0+)', () => {
    ...
    assert.ok(result.includes('[agents.forge-executor]'), 'executor uses struct header');
    assert.ok(result.includes('[agents.forge-planner]'), 'planner uses struct header');
...
describe('stripGsdFromCodexConfig', () => {
  ...
  test('removes legacy [agents.gsd-*] map sections (self-heal pre-#2645 configs)', () => {
    const content = `[agents.forge-executor]\ndescription = "test"\nconfig_file = "agents/forge-executor.toml"\n\n[agents.custom-agent]\ndescription = "user agent"\n`;
    const result = stripGsdFromCodexConfig(content);
    assert.ok(!result.includes('[agents.forge-executor]'), 'removes legacy GSD agent map section');
    assert.ok(result.includes('[agents.custom-agent]'), 'preserves user agent section');
  });

  test('#2645: removes [[agents]] array-of-tables entries whose name is forge-*', () => {
    const content = `[[agents]]\nname = "forge-executor"\ndescription = "test"\nconfig_file = "agents/forge-executor.toml"\n\n[[agents]]\nname = "custom-agent"\ndescription = "user agent"\n`;
    const result = stripGsdFromCodexConfig(content);
    assert.ok(!/name = "forge-executor"/.test(result), 'removes managed GSD [[agents]] entry');
    assert.ok(result.includes('name = "custom-agent"'), 'preserves user [[agents]] entry');
  });

  test('#2645: handles mixed legacy + new shapes and multiple user/forge entries in one file', () => {
    ...
    assert.ok(!result.includes('forge-executor'), 'removes legacy forge-executor');
    assert.ok(!/name\s*=\s*"forge-planner"/.test(result), 'removes new forge-planner');
    assert.ok(!/name\s*=\s*'forge-debugger'/.test(result), 'removes single-quoted forge-debugger');
...
describe('migrateCodexHooksMapFormat', () => {
  test('migrates flat [[hooks]] with event key to namespaced [[hooks.<EVENT>]] form', () => {
    ...
    const content = [
      ...
      '[[hooks]]',
      'event = "SessionStart"',
      'command = "node /home/.codex/hooks/forge-check-update.js"',
      '',
    ].join('\n');
    ...
    assert.strictEqual(parsed.hooks.SessionStart[0].hooks[0].command,
      'node /home/.codex/hooks/forge-check-update.js');
  ...
  test('converts [hooks.shell] to namespaced AoT [[hooks.shell]] (#2760 CR5 finding 3)', () => {
    ...
    assert.strictEqual(parsed.hooks.shell[0].hooks[0].command, 'node /home/.codex/hooks/forge-check-update.js');
...
describe('Codex hooks emit: migration produces namespaced AoT so managed-emit converges', () => {
  ...
  let tmpDir;
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-codex-fieldparity-'));
  });
  afterEach(() => {
    cleanup(tmpDir);
  });
...
describe('mergeCodexConfig', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-codex-merge-'));
  });
  ...
  test('case 1: creates new config.toml', () => {
    ...
    assert.ok(content.includes('[agents.forge-executor]'), 'has agent in struct format');
  ...
  test('case 2: replaces existing GSD block', () => {
    ...
    const newBlock = generateCodexConfigBlock([
      { name: 'forge-executor', description: 'Updated description' },
      { name: 'forge-planner', description: 'New agent' },
    ]);
    ...
    assert.ok(content.includes('[agents.forge-planner]'), 'has new agent in struct format');
  ...
  test('case 3 strips existing [agents.gsd-*] sections before appending fresh block', () => {
    ...
    const content = fs.readFileSync(configPath, 'utf8');
    // After merge, GSD block is after the marker. Count [agents.forge-executor] headers:
    // exactly one should exist (the one in the freshly-written GSD block).
    const gsdStructCount = (content.match(/^\[agents\.forge-executor\]\s*$/gm) || []).length;
    const markerCount = (content.match(new RegExp(GSD_CODEX_MARKER.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g')) || []).length;
    // Struct format does not use name = field
    assert.ok(!content.match(/^name = "forge-executor"/m), 'no name = field in struct format');

    assert.ok(content.includes('[model]'), 'preserves user content');
    assert.ok(content.includes('[agents.custom-agent]'), 'preserves non-GSD agent section');
    assert.strictEqual(gsdStructCount, 1, 'keeps exactly one [agents.forge-executor] struct entry');
  ...
  test('case 2 does not inject feature keys', () => {
    ...
    assert.ok(content.includes('[agents.forge-executor]'), 'has agent from fresh block in struct format');
  });
  ...
  test('case 2 strips leaked GSD-managed sections above marker in CRLF files', () => {
    ...
    assert.strictEqual(countMatches(beforeMarker, /^\[agents\.forge-executor\]\s*$/gm), 0, 'removes leaked GSD agent section above marker');
    // New struct format: exactly one [agents.forge-executor] header in the GSD block (after marker)
    assert.strictEqual(countMatches(content, /^\[agents\.forge-executor\]\s*$/gm), 1, 'exactly one struct agent header in GSD block');
    assert.strictEqual(countMatches(content, /name = "forge-executor"/g), 0, 'no name = field in struct format');
  ...
  test('case 2 strips bare [agents] tables (invalid in current Codex schema, #2760) and removes leaked GSD sections in CRLF files', () => {
    ...
    // New struct format: exactly one [agents.forge-executor] in the GSD block (after marker)
    assert.strictEqual(countMatches(content, /^\[agents\.forge-executor\]\s*$/gm), 1, 'exactly one struct agent header in GSD block');
    assert.strictEqual(countMatches(content, /name = "forge-executor"/g), 0, 'no name = field in struct format');
    assertUsesOnlyEol(content, '\r\n');
  });
...
describe('installCodexConfig (integration)', () => {
  ...
  beforeEach(() => {
    tmpTarget = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-codex-install-'));
  });
  ...
  (hasAgents ? test : test.skip)('generates config.toml and agent .toml files', () => {
    ...
    assert.ok(config.includes('[agents.forge-executor]'), 'has executor agent in struct format');
    ...
    assert.ok(fs.existsSync(path.join(agentsDir, 'forge-executor.toml')), 'executor .toml exists');
    assert.ok(fs.existsSync(path.join(agentsDir, 'forge-plan-checker.toml')), 'plan-checker .toml exists');

    const executorToml = fs.readFileSync(path.join(agentsDir, 'forge-executor.toml'), 'utf8');
    assert.ok(executorToml.includes('name = "forge-executor"'), 'executor has name');
    ...
    const checkerToml = fs.readFileSync(path.join(agentsDir, 'forge-plan-checker.toml'), 'utf8');
    assert.ok(checkerToml.includes('name = "forge-plan-checker"'), 'plan-checker has name');
  ...
  (hasAgents ? test : test.skip)('generated Codex agent .toml files do not call bare forge-tools', () => {
    const { installCodexConfig } = require('../bin/install.js');
    installCodexConfig(tmpTarget, agentsSrc);

    // Not the shared listAgentFiles() helper: reads the INSTALLED target dir and
    // filters generated forge-*.toml output, not the source .md roster.
    const agentsDir = path.join(tmpTarget, 'agents');
    const tomlFiles = fs.readdirSync(agentsDir)
      .filter((file) => file.startsWith('forge-') && file.endsWith('.toml'));
...
describe('Codex install hook configuration (e2e)', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-codex-e2e-'));
  ...
  test('Codex install copies hook file that is referenced in hooks.json (#2153)', () => {
    // Regression test: Codex install writes forge-check-update hook reference into
    ...
    assert.equal(
      hooksJsonCommands.some((cmd) => cmd.includes('forge-check-update')),
      true,
      'hooks.json references forge-check-update (.js on POSIX, .cmd on Windows)'
    );
    // The hook file must physically exist at the referenced path
    const hookFile = path.join(codexHome, 'hooks', 'forge-check-update.js');
    assert.ok(
      fs.existsSync(hookFile),
      `forge-check-update.js must exist at ${hookFile} — hooks.json references it (directly on POSIX, via .cmd shim on Windows) but file was not installed`
    );
  });
  ...
  test('fresh CODEX_HOME enables codex_hooks without draft root defaults', () => {
    ...
    const hooksJsonCommands = readHooksSessionStartCommands(codexHome);
    const gsdCommands = hooksJsonCommands.filter((cmd) => cmd.includes('forge-check-update'));
    ...
    if (process.platform === 'win32') {
      ...
      const expectedCmdPath = path.join(codexHome, 'hooks', 'forge-check-update.cmd').replace(/\\/g, '/');
      ...
      const expectedHookPath = path.join(codexHome, 'hooks', 'forge-check-update.js').replace(/\\/g, '/');
      const expectedCommand = `"${expectedRunner}" "${expectedHookPath}"`;
      assert.strictEqual(gsdCommands[0], expectedCommand, 'handler command must use absolute node runner pointing at forge-check-update.js (#3017)');
    }
  ...
  test('existing LF config without [features] gets one features block and preserves user content', () => {
    ...
    const hooksJsonCommands = readHooksSessionStartCommands(codexHome);
    const gsdEntries = hooksJsonCommands.filter((cmd) => cmd.includes('forge-check-update'));
  ...
  test('root inline-table features assignments are left untouched without appending invalid dotted keys or hooks', () => {
    ...
    assert.strictEqual(countMatches(content, /forge-check-update\.js/g), 0, 'does not add the GSD hook block when codex_hooks cannot be enabled safely');
    assert.ok(content.includes('[agents.forge-executor]'), 'still installs the managed agent block in struct format');
    assertNoDraftRootKeys(content);
  });
  ...
  test('mixed-EOL configs use the first newline style for inserted Codex content', () => {
    ...
    assert.ok(content.includes(`# GSD Agent Configuration — managed by forge-core installer\n`), 'writes the managed agent block using the first newline style');
...
describe('Codex uninstall symmetry for hook-enabled configs', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-codex-uninstall-'));
    codexHome = path.join(tmpDir, 'codex-home');
  });
  ...
  test('install then uninstall removes [features].codex_hooks while preserving other feature keys, comments, hooks, and CRLF', () => {
    ...
    assert.strictEqual(countMatches(cleaned, /forge-check-update\.js/g), 0, 'removes only the GSD update hook');
  ...
  test('install then uninstall removes dotted features.codex_hooks without creating a [features] table', () => {
    ...
    assert.strictEqual(countMatches(cleaned, /echo custom-after-command/g), 1, 'preserves non-GSD hooks');
    assert.strictEqual(countMatches(cleaned, /forge-check-update\.js/g), 0, 'removes the GSD update hook');
  });
...
describe('cleanupCodexSkillMetadataSidecars (#1326)', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-test-sidecar-cleanup-'));
  });
  ...
  test('Codex install does not emit managed agents/openai.yaml sidecars and removes stale ones (#1326)', () => {
    // forge-foo: managed skill with stale sidecar → sidecar removed, empty agents/ pruned
    const fooAgents = path.join(tmpDir, 'forge-foo', 'agents');
    fs.mkdirSync(fooAgents, { recursive: true });
    fs.writeFileSync(path.join(tmpDir, 'forge-foo', 'SKILL.md'), '---\nname: forge-foo\n---\nBody.\n');
    fs.writeFileSync(path.join(fooAgents, 'openai.yaml'), 'interface:\n  display_name: "foo"\n');

    // forge-dev-preferences: user-owned → sidecar PRESERVED
    const prefAgents = path.join(tmpDir, 'forge-dev-preferences', 'agents');
    fs.mkdirSync(prefAgents, { recursive: true });
    fs.writeFileSync(path.join(tmpDir, 'forge-dev-preferences', 'SKILL.md'), '---\nname: forge-dev-preferences\n---\nBody.\n');
    ...
    // forge-bar: managed skill with sidecar + another file in agents/ → sidecar removed, agents/ kept (has other.txt)
    const barAgents = path.join(tmpDir, 'forge-bar', 'agents');
    fs.mkdirSync(barAgents, { recursive: true });
    fs.writeFileSync(path.join(tmpDir, 'forge-bar', 'SKILL.md'), '---\nname: forge-bar\n---\nBody.\n');
    ...
    // forge-foo: sidecar removed and empty agents/ pruned
    assert.ok(!fs.existsSync(path.join(fooAgents, 'openai.yaml')),
      'forge-foo/agents/openai.yaml must be removed (managed stale sidecar)');
    assert.ok(!fs.existsSync(fooAgents),
      'forge-foo/agents/ must be pruned when empty after sidecar removal');

    // forge-dev-preferences: user-owned, sidecar preserved
    assert.ok(fs.existsSync(path.join(prefAgents, 'openai.yaml')),
      'forge-dev-preferences/agents/openai.yaml must be preserved (user-owned)');
    assert.strictEqual(fs.readFileSync(path.join(prefAgents, 'openai.yaml'), 'utf8'), userYaml,
      'forge-dev-preferences/agents/openai.yaml content must be unchanged');

    // forge-bar: sidecar removed but agents/ kept (still has other.txt)
    assert.ok(!fs.existsSync(path.join(barAgents, 'openai.yaml')),
      'forge-bar/agents/openai.yaml must be removed');
    assert.ok(fs.existsSync(barAgents),
      'forge-bar/agents/ must NOT be pruned (still contains other.txt)');
    assert.ok(fs.existsSync(path.join(barAgents, 'other.txt')),
      'forge-bar/agents/other.txt must be preserved');
  ...
  test('is a no-op for managed forge-* dirs with no agents/openai.yaml (#1326)', () => {
    // No sidecar present — should not throw, should not create anything
    const skillDir = path.join(tmpDir, 'forge-baz');
  ...
  test('does not delete through a symlinked agents/ directory (#1326)', { skip: process.platform === 'win32' }, () => {
    // Setup: a skills dir with forge-foo/ whose agents/ is a SYMLINK to an external dir.
    // The cleanup must not delete files through the symlink.
    const externalDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-test-symlink-ext-'));
    try {
      ...
      // Create forge-foo/ in the skills dir and make agents/ a symlink to externalDir.
      const skillDir = path.join(tmpDir, 'forge-foo');
      fs.mkdirSync(skillDir, { recursive: true });
      ...
      assert.ok(fs.existsSync(agentsLink),
        'forge-foo/agents symlink must still exist');
    } finally {
      cleanup(externalDir);
    }
  });
  ...
  test('Codex install does not create agents/openai.yaml sidecars for any managed skill (#1326)', () => {
    // Integration test: full Codex install must NOT produce any managed forge-*/agents/openai.yaml
    ...
    const gsdSkillDirs = fs.readdirSync(skillsDir, { withFileTypes: true })
      .filter(e => e.isDirectory() && e.name.startsWith('forge-') && e.name !== 'forge-dev-preferences');
    assert.ok(gsdSkillDirs.length > 0, 'install must create at least one managed forge-* skill directory');
