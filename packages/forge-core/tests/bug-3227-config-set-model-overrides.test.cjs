 ...
 *
 * `forge-sdk query config-set model_overrides.forge-plan-checker opus` was
...
const { DYNAMIC_KEY_PATTERNS, isValidConfigKey } = require('../forge-core/bin/lib/config-schema.cjs');

describe('#3227 — config-set accepts model_overrides.<agent-id>', () => {
  test('isValidConfigKey accepts model_overrides.forge-plan-checker', () => {
    assert.ok(
      isValidConfigKey('model_overrides.forge-plan-checker'),
      'model_overrides.forge-plan-checker must be accepted by isValidConfigKey'
    );
  });

  test('isValidConfigKey accepts model_overrides with various agent-id formats', () => {
    const validKeys = [
      'model_overrides.forge-executor',
      'model_overrides.forge-planner',
      'model_overrides.forge-codebase-mapper',
  ...
  test('config-set model_overrides.forge-plan-checker opus succeeds via forge-tools.cjs', (t) => {
    const tmpDir = createTempProject();
    t.after(() => cleanup(tmpDir));

    const result = runGsdTools(
      ['config-set', 'model_overrides.forge-plan-checker', 'opus'],
      tmpDir
    );
    assert.ok(
      result.success,
      [
        'config-set model_overrides.forge-plan-checker opus should succeed,',
        'got:',
  ...
  test('config-set model_overrides.forge-plan-checker opus writes to config.json', (t) => {
    const tmpDir = createTempProject();
    t.after(() => cleanup(tmpDir));

    runGsdTools(['config-set', 'model_overrides.forge-plan-checker', 'opus'], tmpDir);
    ...
    assert.ok(
      config.model_overrides !== undefined &&
        config.model_overrides['forge-plan-checker'] === 'opus',
      [
        'Expected model_overrides["forge-plan-checker"]: "opus" in config.json,',
        'got: ' + JSON.stringify(config),
      ].join('\n')
    );
  });

  test('config-get model_overrides.forge-plan-checker returns opus after config-set', (t) => {
    const tmpDir = createTempProject();
    t.after(() => cleanup(tmpDir));
    ...
    const getResult = runGsdTools(
      ['config-get', 'model_overrides.forge-plan-checker'],
      tmpDir
    );
    assert.ok(
      getResult.success,
      [
        'config-get model_overrides.forge-plan-checker should succeed,',
        'got:',
