 ...
 *
 * forge now emits skills to ~/.cline/skills/<name>/SKILL.md for Cline >= v3.48.
...
See ~/.claude/skills/forge-help/SKILL.md for reference.
Use \`/forge-help\` or Claude Code for details.
`;
...
Audit Nyquist validation coverage. See ~/.claude/skills/forge-help/SKILL.md for reference.
...
describe('convertClaudeCommandToClineSkill — unit', () => {
  test('emits frontmatter with name: forge-<stem>', () => {
    const result = convertClaudeCommandToClineSkill(SAMPLE_COMMAND, 'forge-execute-phase');
    const nameMatch = result.match(/^name:\s*(.+)$/m);
    assert.ok(nameMatch, 'frontmatter must contain name field');
    assert.ok(nameMatch[1].includes('forge-execute-phase'), 'name must start with forge-execute-phase');
  });
  ...
  test('frontmatter emits ONLY name and description — no allowed-tools/argument-hint/agent (RICH_COMMAND)', () => {
    const result = convertClaudeCommandToClineSkill(RICH_COMMAND, 'forge-validate-phase');
    const fm = parseFrontmatter(result);
    assert.ok(fm !== null, 'result must have YAML frontmatter');
    assert.ok(!fm.includes('allowed-tools'), 'frontmatter must NOT contain allowed-tools');
    assert.ok(!fm.includes('argument-hint'), 'frontmatter must NOT contain argument-hint');
  ...
  test('name == forge-validate-phase for RICH_COMMAND', () => {
    const result = convertClaudeCommandToClineSkill(RICH_COMMAND, 'forge-validate-phase');
    const nameMatch = result.match(/^name:\s*(.+)$/m);
    ...
    assert.strictEqual(nameVal, 'forge-validate-phase', `name must be forge-validate-phase, got: ${nameVal}`);
  });
  ...
  test('description truncated to <=1024 chars when source description is very long', () => {
    ...
    const result = convertClaudeCommandToClineSkill(longDescCommand, 'forge-test');
    const descMatch = result.match(/^description:\s*'?(.*?)'?$/m);
    assert.ok(descMatch, 'must have description field');
  ...
  test('returns content unchanged when source has no frontmatter', () => {
    ...
    const result = convertClaudeCommandToClineSkill(noFm, 'forge-test');
...
describe('installRuntimeArtifacts — cline skills emission', () => {
  test('cline global: writes forge-prefixed skill dirs under skills/', (t) => {
    const configDir = createTempDir('forge-cline-skills-');
    ...
    assert.ok(
      ...
      `forge-help/SKILL.md must exist under ${skillsKind.destSubpath}/`
    );
  });

  test('cline global: SKILL.md has valid cline frontmatter (name + description)', (t) => {
    const configDir = createTempDir('forge-cline-fm-');
    t.after(() => cleanup(configDir));

    installRuntimeArtifacts('cline', configDir, 'global', RESOLVED_CORE);

    const skillsDir = path.join(configDir, 'skills');
    const helpSkill = path.join(skillsDir, 'forge-help', 'SKILL.md');
    assert.ok(fs.existsSync(helpSkill), 'forge-help/SKILL.md must exist');
    ...
    // name must be forge-help
    const nameMatch = content.match(/^name:\s*(.+)$/m);
    assert.ok(nameMatch, 'must have name field');
    assert.ok(nameMatch[1].includes('forge-help'), `name must include forge-help, got: ${nameMatch[1]}`);
  });

  test('cline global: SKILL.md uses .cline/ paths not .claude/', (t) => {
    const configDir = createTempDir('forge-cline-paths-');
    t.after(() => cleanup(configDir));
    ...
    const skills = fs.readdirSync(skillsDir).filter(n => n.startsWith('forge-'));
    assert.ok(skills.length > 0, 'at least one forge- skill must be installed');
  ...
  test('cline global: skill count matches resolved profile', (t) => {
    const configDir = createTempDir('forge-cline-count-');
    t.after(() => cleanup(configDir));
    ...
    const count = fs.readdirSync(skillsDir)
      .filter(n => n.startsWith('forge-') && fs.statSync(path.join(skillsDir, n)).isDirectory())
      .length;
...
describe('installRuntimeArtifacts — cline idempotency', () => {
  test('cline: running install twice leaves skills intact (idempotency)', (t) => {
    const configDir = createTempDir('forge-cline-idempotent-');
    t.after(() => cleanup(configDir));
...
describe('install() global cline — coexistence: skills AND .clinerules', () => {
  ...
  beforeEach(() => {
    ...
    tmpGlobalDir = createTempDir('forge-cline-global-');
  ...
  test('global cline install writes at least one forge-* SKILL.md under skills/', () => {
    ...
    // full profile: forge-help is nested under forge-ns-manage/skills/help/SKILL.md
    const helpSkillFile = nestedSkillPath(skillsDir, 'forge-', 'help');
    assert.ok(
  ...
  test('global cline .clinerules/gsd.md contains GSD instructions', () => {
    ...
    assert.ok(
      content.includes('GSD') || content.includes('forge'),
...
describe('convertClaudeToCliineMarkdown — bare ~/.claude and CLAUDE_CONFIG_DIR (Fix 3)', () => {
  const surfacePath = path.join(__dirname, '..', 'commands', 'forge', 'surface.md');
  ...
  test('installRuntimeArtifacts cline global: forge-surface SKILL.md has no bare ~/.claude or CLAUDE_CONFIG_DIR', (t) => {
    const configDir = createTempDir('forge-cline-surface-fix3-');
    t.after(() => cleanup(configDir));

    const MANIFEST_FULL = require('../forge-core/bin/lib/install-profiles.cjs').loadSkillsManifest(
      path.join(__dirname, '..', 'commands', 'forge')
    );
    const RESOLVED_FULL = require('../forge-core/bin/lib/install-profiles.cjs').resolveProfile({
      modes: ['full'], manifest: MANIFEST_FULL,
    });

    installRuntimeArtifacts('cline', configDir, 'global', RESOLVED_FULL);

    // full profile: surface is nested under forge-ns-manage/skills/surface/SKILL.md
    const surfaceSkill = nestedSkillPath(path.join(configDir, 'skills'), 'forge-', 'surface');
    ...
    assert.ok(
      ...
      'forge-surface SKILL.md must not contain bare ~/.claude (Fix 3)'
    );
    assert.ok(
      !content.includes('CLAUDE_CONFIG_DIR'),
      'forge-surface SKILL.md must not contain CLAUDE_CONFIG_DIR (Fix 3)'
    );
  });
});
...
describe('_applyRuntimeRewrites — cline custom-dir embedded path (Fix 1)', () => {
  test('default pathPrefix ($HOME/.cline/) leaves ~/.cline refs as $HOME/.cline', () => {
    const content = 'See ~/.cline/skills/forge-help/SKILL.md for reference.\nBare: ~/.cline\n';
  ...
  test('installRuntimeArtifacts cline with CLINE_CONFIG_DIR custom: SKILL.md embeds custom path', (t) => {
    const configDir = createTempDir('forge-cline-custom-dir-');
    t.after(() => cleanup(configDir));
    ...
    // forge-surface SKILL.md references config paths; with a custom configDir
    ...
    assert.ok(
      ...
      `forge-surface SKILL.md must not contain ~/.cline/ when configDir=${configDir} (Fix 1)`
    );
    ...
    assert.ok(
      ...
      `forge-surface SKILL.md must embed custom configDir path ${expectedPath} (Fix 1)`
    );
  });
});
...
describe('convertClaudeCommandToClineSkill — code-point-aware truncation (Fix 4)', () => {
  ...
  test('emitted description is <= 1024 code points when source overflows', () => {
    ...
    const result = convertClaudeCommandToClineSkill(cmd, 'forge-emoji-test');
...
describe('resolveRuntimeArtifactLayout — cline scope-aware (Fix 2)', () => {
  ...
  test('installRuntimeArtifacts cline local: no skills/ dir created', (t) => {
    const configDir = createTempDir('forge-cline-local-noskills-');
    t.after(() => cleanup(configDir));
