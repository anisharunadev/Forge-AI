  ...
  const sanitizeFlags = (rawVal: unknown): string => {
    ...
    if (!safe) {
      process.stderr.write(
        `forge-tools: warning: manager.flags contains invalid tokens, ignoring: ${val}\n`,
      );
      return '';
    }
    return val;
...
function cmdInitNewWorkspace(cwd: string, raw: boolean): void {
  ...
  const defaultBase = path.join(homedir, 'forge-workspaces');
...
function buildSkillManifest(cwd: string, skillsDir: string | null = null): SkillManifest {
  ...
  const canonicalRoots: CanonicalRoot[] = skillsDir
    ...
    : [
        ...
        },
        {
          ...
          path: path.join(os.homedir(), '.claude', 'forge-core', 'skills'),
          scope: 'import-only',
          kind: 'skills',
          deprecated: true,
        },
        {
          ...
          path: path.join(os.homedir(), '.claude', 'commands', 'forge'),
  ...
  for (const rootInfo of canonicalRoots) {
    ...
    for (const entry of entries) {
      ...
      if (!entry.name.startsWith('forge-ns-')) continue;
