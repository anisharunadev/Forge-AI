 ...
 *
 * Invocable as: forge_run check ui-plan-gate <phase>
 *
 ...
 * Invocable as: forge_run check ui-safety-gate <phase>
 *             or forge_run check ui.safety-gate <phase> (dots normalized to hyphens)
 *
 * Uses checkUiPresence from ui-safety-gate.cjs — does NOT reimplement frontend detection.
...
function computeUiSafetyGate(projectDir: string, phase: string): {
  ...
  if (block) {
    result.message = `UI files changed in this wave but no UI-SPEC.md exists for Phase ${phase}. ` +
      `Run /forge:ui-phase ${phase} to generate the design contract before continuing.`;
  }
  if (phaseLookupFailed) result.phaseLookupFailed = true;
  return result;
}
 ...
 *
 * Logic from forge-core/references/tdd.md <end_of_phase_review> and
 ...
 * Invocable as: forge_run check gap-analysis.plan-post <phase-dir> [phase-req-ids]
 */
...
function routeCheckCommand({ args, cwd, raw }: RouteCheckCommandOptions): void {
  ...
  // directly runnable as `forge_run check ui.plan-gate` — the dot is normalized to
  // `ui-plan-gate` before routing. The generic gate-dispatch in §5.6 reads
  // `check.query` from the active gate hook and runs `forge_run check ${hook.check.query}`,
