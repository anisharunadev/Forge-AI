...
function buildCodexHookBlock(targetDir: string, opts?: BuildCodexHookBlockOpts): string | null {
  ...
  const platform = (opts && opts.platform) || process.platform;
  const updateCheckScript = path.resolve(targetDir, 'hooks', 'forge-check-update.js');
...
function ensureCodexHooksJsonSessionStart(targetDir: string, opts: EnsureCodexSessionStartOpts = {}): ReconcileResult {
  ...
  const scriptPath = path.resolve(targetDir, 'hooks', 'forge-check-update.js').replace(/\\/g, '/');
...
function ensureCodexHooksJsonEvent(targetDir: string, eventName: string, opts: EnsureCodexEventOpts = {}): ReconcileResult {
  ...
  const scriptPath = path.resolve(targetDir, 'hooks', 'forge-context-monitor.js').replace(/\\/g, '/');
...
function applySettingsJsonHooks(settings: any, opts: ApplySettingsJsonHooksOpts): void {
  ...
  if (hooksSurface !== 'none') {
    ...
    const hasGsdUpdateHook = settings.hooks.SessionStart.some((entry: HookGroup) =>
      entry.hooks && entry.hooks.some((h: HookEntry) => referencesHook(h as Record<string, unknown>, 'forge-check-update'))
    );
    ...
    const hasContextMonitorHook = settings.hooks[postToolEvent].some((entry: HookGroup) =>
      entry.hooks && entry.hooks.some((h: HookEntry) => referencesHook(h as Record<string, unknown>, 'forge-context-monitor'))
    );
    ...
    if (!hasContextMonitorHook && fs.existsSync(contextMonitorFile) && contextMonitorCommand) {
      ...
      for (const entry of settings.hooks[postToolEvent]) {
        if (entry.hooks && entry.hooks.some((h: HookEntry) => referencesHook(h as Record<string, unknown>, 'forge-context-monitor'))) {
          let migrated = false;
          if (!entry.matcher) {
          ...
          for (const h of entry.hooks) {
            if (referencesHook(h as Record<string, unknown>, 'forge-context-monitor') && !h.timeout) {
              h.timeout = 10;
              migrated = true;
            }
          }
    ...
    const hasPromptGuardHook = settings.hooks[preToolEvent].some((entry: HookGroup) =>
      entry.hooks && entry.hooks.some((h: HookEntry) => referencesHook(h as Record<string, unknown>, 'forge-prompt-guard'))
    );

    const promptGuardFile = path.join(targetDir, 'hooks', 'forge-prompt-guard.js');
    ...
    const hasReadGuardHook = settings.hooks[preToolEvent].some((entry: HookGroup) =>
      entry.hooks && entry.hooks.some((h: HookEntry) => referencesHook(h as Record<string, unknown>, 'forge-read-guard'))
    );

    const readGuardFile = path.join(targetDir, 'hooks', 'forge-read-guard.js');
    ...
    const hasReadInjectionScannerHook = settings.hooks[postToolEvent].some((entry: HookGroup) =>
      entry.hooks && entry.hooks.some((h: HookEntry) => referencesHook(h as Record<string, unknown>, 'forge-read-injection-scanner'))
    );

    const readInjectionScannerFile = path.join(targetDir, 'hooks', 'forge-read-injection-scanner.js');
    ...
    const workflowGuardHookEntry = settings.hooks[preToolEvent].find((entry: HookGroup) =>
      entry.hooks && entry.hooks.some((h: HookEntry) => referencesHook(h as Record<string, unknown>, 'forge-workflow-guard'))
    );
    const hasWorkflowGuardHook = Boolean(workflowGuardHookEntry);

    const workflowGuardFile = path.join(targetDir, 'hooks', 'forge-workflow-guard.js');
    ...
    const hasWorktreePathGuardHook = settings.hooks[preToolEvent].some((entry: HookGroup) =>
      entry.hooks && entry.hooks.some((h: HookEntry) => referencesHook(h as Record<string, unknown>, 'forge-worktree-path-guard'))
    );
    const worktreePathGuardFile = path.join(targetDir, 'hooks', 'forge-worktree-path-guard.js');
    ...
    const hasValidateCommitHook = settings.hooks[preToolEvent].some((entry: HookGroup) =>
      entry.hooks && entry.hooks.some((h: HookEntry) => referencesHook(h as Record<string, unknown>, 'forge-validate-commit'))
    );
    ...
    const validateCommitFile = path.join(targetDir, 'hooks', 'forge-validate-commit.sh');
    ...
    const hasGraphifyUpdateHook = settings.hooks[postToolEvent].some((entry: HookGroup) =>
      entry.hooks && entry.hooks.some((h: HookEntry) => referencesHook(h as Record<string, unknown>, 'forge-graphify-update'))
    );
    const graphifyUpdateFile = path.join(targetDir, 'hooks', 'forge-graphify-update.sh');
    ...
    const hasSessionStateHook = settings.hooks.SessionStart.some((entry: HookGroup) =>
      entry.hooks && entry.hooks.some((h: HookEntry) => referencesHook(h as Record<string, unknown>, 'forge-session-state'))
    );
    const sessionStateFile = path.join(targetDir, 'hooks', 'forge-session-state.sh');
    ...
    const hasPhaseBoundaryHook = settings.hooks[postToolEvent].some((entry: HookGroup) =>
      entry.hooks && entry.hooks.some((h: HookEntry) => referencesHook(h as Record<string, unknown>, 'forge-phase-boundary'))
    );
    const phaseBoundaryFile = path.join(targetDir, 'hooks', 'forge-phase-boundary.sh');
    ...
    if (extendedEvents.includes('FileChanged')) {
      ...
      const configReloadFile = path.join(targetDir, 'hooks', 'forge-config-reload.js');
      const alreadyHasConfigReload = settings.hooks.FileChanged.some((entry: HookGroup) =>
        entry.hooks && entry.hooks.some((h: HookEntry) => referencesHook(h as Record<string, unknown>, 'forge-config-reload'))
      );
