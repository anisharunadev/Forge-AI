 ...
 *
 * Manages the runtime enable/disable surface state (the `.forge-surface.json` marker in
 * each runtime's config dir root (e.g., ~/.claude)) independently of the install-time profile marker
 * (`.forge-profile`). Runtime config locations are resolved by callers.
 *
...
const SURFACE_FILE_NAME = '.forge-surface.json';
 ...
 *  - parsed forge-file-manifest.json object ({ files: { ... } }) by rebuilding
...
function normalizeSkillManifest(runtimeConfigDir: string, manifest: Map<string, string[]> | object | null | undefined): Map<string, string[]> {
  ...
  // Parsed forge-file-manifest.json shape: rebuild the dependency map from source.
...
function resolveSurface(runtimeConfigDir: string, manifest: Map<string, string[]> | object, clusterMap?: ClusterMap | Record<string, string[]>, registry?: { capabilityClusters?: Record<string, string[]>; profileMembership?: Record<string, { tier: string; profiles: string[] }> }): { name: string; skills: Set<string>; agents: Set<string> } {
  ...
  // Determine base profile name: from surface state or from .forge-profile marker
...
function applySurface(runtimeConfigDir: string, layout: Layout, manifest: Map<string, string[]> | object, clusterMap?: ClusterMap | Record<string, string[]>, registry?: { capabilityClusters?: Record<string, string[]>; profileMembership?: Record<string, { tier: string; profiles: string[] }> }): { name: string; skills: Set<string>; agents: Set<string> } {
  ...
  // /forge-surface profile change. Same gap affected any runtime with commands
 ...
 *
 * Ownership criteria:
 *   - Non-empty prefix (e.g. 'forge-'): dir name starts with that prefix AND
 ...
 *     preserved — this prevents data loss for user-created forge-* directories.
 ...
 *
 * @param skillsDir        directory that contains the forge-STEM sub-dirs
 * @param retainedNames    set of directory names to keep (e.g. 'forge-help')
 * @param prefix           GSD dir prefix, e.g. 'forge-' (or '' for Hermes)
...
function pruneSkillDirs(skillsDir: string, retainedNames: Set<string>, prefix: string, manifest?: Map<string, string[]>): void {
  ...
  for (const entry of fs.readdirSync(skillsDir)) {
    ...
    if (prefix !== '') {
      ...
      // A user-created forge-* dir that isn't in the manifest is preserved with a warning.
 ...
 * For 'agents' kind: same, but only remove files starting with 'forge-' prefix.
...
function _syncGsdDir(stagedDir: string, destDir: string, kind: ArtifactKind | string, manifest?: Map<string, string[]>): void {
  ...
  const kindPrefix = (typeof kind === 'object' && kind !== null) ? kind.prefix : 'forge-';
  ...
  if (kindName === 'skills') {
    ...
    // the forge- prefix on copy; namespaced command dirs (commands/gsd) and agents
    // keep their staged names. Copying staged names verbatim previously diverged
    // from install and orphaned the installed forge-*.md files, and the unscoped
    ...
    //   - agents: only forge-* are GSD-owned
    ...
    for (const file of fs.readdirSync(destDir).filter(f => f.endsWith('.md'))) {
      if (kindName === 'agents' && !file.startsWith('forge-')) continue;
