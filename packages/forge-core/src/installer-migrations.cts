...
function applyInstallerMigrationPlan({
  ...
  const journalRelPath = path.posix.join('forge-migration-journal', `${runId}.json`);
  const journalPath = path.join(configDir, journalRelPath);
  const rollbackRootRelPath = path.posix.join('forge-migration-journal', `${runId}-rollback`);
  const rollbackRoot = path.join(configDir, rollbackRootRelPath);
  const backupRootRelPath = path.posix.join('forge-migration-journal', `${runId}-backups`);
