...
/**
 * #488 — Re-sync effort: frontmatter in all installed forge-*.md agent files to
 ...
 * The runtime resolver (loadConfig) does not merge ~/.forge/defaults.json when a project
...
function cmdEffortSync(cwd: string, raw: boolean, opts?: { dryRun?: boolean; configDir?: string; runtime?: string }): void {
  ...
  // Use install-time resolvers: they merge ~/.forge/defaults.json with project config,
  ...
  const files = fs.readdirSync(agentsDir).filter(f => {
    if (!f.startsWith('forge-') || !f.endsWith('.md')) return false;
