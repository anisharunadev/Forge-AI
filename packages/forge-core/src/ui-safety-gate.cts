 ...
 *   echo "$PHASE_SECTION" | node forge-core/bin/lib/ui-safety-gate.cjs
 *   echo $?   → 0 if UI tokens found, 1 if not, 2 on usage error
 *
 * Exit codes mirror grep: 0 = match found, 1 = no match, 2 = usage error.
 *
 * Canonical location: forge-core/bin/lib/ui-safety-gate.cjs (#448)
 * This path is deployed by the GSD installer to $RUNTIME_DIR/forge-core/bin/lib/.
