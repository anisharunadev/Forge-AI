'use strict';
/**
 * Audit command routers — CLI dispatchers for `forge-tools audit-uat` and
 * `forge-tools audit-open`.
 *
 * ADR-959 (phase 4d-impl-3): audit command family cutover.
 * Extracted from the hardcoded `case 'audit-uat':` and `case 'audit-open':`
 * arms in forge-tools.cjs.  Behaviour is preserved byte-for-behaviour from the
