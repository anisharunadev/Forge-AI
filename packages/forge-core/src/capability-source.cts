...
interface ResolveOptions {
  ...
  hostVersion?: string;
  /** Override the GSD home directory (where .forge/capabilities/ lives). */
  gsdHome?: string;
...
function realHttpsGet(url: string): Promise<HttpResponse> {
  return new Promise((resolve, reject) => {
    const req = _httpsGetImpl(
      url,
      { headers: { 'User-Agent': 'forge-core-capability-source/1.0' } },
      (res) => {
...
function stageValidated(opts: {
  ...
  try {
    ...
    if (!opts.skipEnginesGate && engines && typeof engines === 'object' && !Array.isArray(engines)) {
      const gsdRange = (engines as Record<string, unknown>)['forge'];
...
function resolveGit(
  ...
  const cloneDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-cap-git-'));
  try {
...
function resolveNpm(
  ...
  const tmpPackDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-cap-npm-pack-'));
  const extractDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-cap-npm-ext-'));
...
async function resolveTarball(
  ...
  const extractDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-cap-tar-'));
