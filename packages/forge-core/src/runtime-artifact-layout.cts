...
function findInstallSourceRoot(runtimeConfigDir?: string): string {
  ...
  let dir = __dirname;
  for (let i = 0; i < 6; i++) {
    const candidate = path.join(dir, 'commands', 'forge');
...
function kimiAgentsKind(destSubpath: string, prefix: string, configDir: string): ArtifactKind {
  return {
    ...
    stage: (resolved) => {
      ...
      const stageDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-kimi-agents-'));
...
//                   at the top level so Skill(skill="forge-plan-phase") succeeds)
//
