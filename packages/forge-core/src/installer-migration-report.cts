 ...
 * runs without a TTY (typical /forge:update path via Claude Code or any
...
// set — a shape regex like `^hooks/forge-[^/]+\.(?:js|sh|cjs|mjs)$` also
...
export const BUNDLED_GSD_HOOK_FILES: ReadonlySet<string> = Object.freeze(new Set([
  ...
  'hooks/forge-config-reload.js',
  'hooks/forge-context-monitor.js',
  'hooks/forge-cursor-post-tool.js',
  'hooks/forge-cursor-session-start.js',
  'hooks/forge-ensure-canonical-path.js',
  'hooks/forge-graphify-update.sh',
  ...
  'hooks/forge-worktree-path-guard.js',
]));
...
//
// Stale SDK build artifacts live under forge-core/sdk/{dist,src}/
...
export function classifyPromptUserAction(action: MigrationAction): ClassifyResult | null {
  ...
  if (/^forge-core\/sdk\/(dist|src)\//.test(relPath)) {
    return { category: 'stale-sdk-build-artifact', choice: 'remove' };
  }
  if (/^skills\/forge-[^/]+\/SKILL\.md$/.test(relPath)) {
...
// migration journal under forge-migration-journal/<runId>-backups/).
