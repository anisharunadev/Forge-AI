 ...
 */
const DEFAULT_COMMANDS_DIR = path.resolve(__dirname, '..', '..', '..', 'commands', 'forge');
...
function stageSkillsForProfile(srcDir: string, resolvedProfile: ResolvedProfile): string {
  ...
  const stageDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-profile-skills-'));
  try {
...
function stageAgentsForProfile(srcAgentsDir: string, resolvedProfile: ResolvedProfile): string {
  ...
  const stageDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-profile-agents-'));
  try {
...
function stageSkillsForRuntimeAsSkills(
  ...
  const stageDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-profile-runtime-skills-'));
  try {
...
function stageAgentsForRuntimeWithConverter(
  ...
  const stageDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-profile-runtime-agents-'));
  try {
    const entries = fs.readdirSync(srcAgentsDir, { withFileTypes: true });
    for (const entry of entries) {
      if (!entry.isFile()) continue;
...
function stageCommandsForRuntimeFlat(
  ...
  const stageDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-profile-runtime-commands-'));
  try {
    const entries = fs.readdirSync(srcCommandsDir, { withFileTypes: true });
    for (const entry of entries) {
      if (!entry.isFile()) continue;
...
function stageSkillsForMode(srcDir: string, mode: string): string {
  ...
  const stageDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-minimal-skills-'));
  try {
    const entries = fs.readdirSync(srcDir, { withFileTypes: true });
    for (const entry of entries) {
      if (!entry.isFile()) continue;
