...
function cmdWriteProfile(cwd: string, options: CmdWriteProfileOptions, raw: boolean): void {
  ...
  if (!outputPath) {
    ...
    outputPath = path.join(getGlobalConfigDir(effectiveRuntime), 'forge-core', 'USER-PROFILE.md');
...
function cmdGenerateDevPreferences(cwd: string, options: CmdGenerateDevPreferencesOptions, raw: boolean): void {
  ...
  if (!outputPath) {
    ...
    const skillDir = getGlobalSkillDir(effectiveRuntime, 'forge-dev-preferences');
    if (!skillDir) {
