...
interface LifecycleOptions {
  /** Scope root: holds .forge/capabilities/<id>, the ledger, and shared config files. */
  runtimeDir: string;
  hostVersion: string;
  /**
...
/**
 * Acquire the capability-mutation lock (the single `.forge/capabilities/.lock` under runtimeDir),
 ...
 *
 * Lexical containment alone is insufficient: a tampered ledger could record `.forge/link/victim`
 * where `.forge/link` is a symlink to `/`, and a lexical check would pass while the delete escapes
...
async function installCapability(spec: string, opts: LifecycleOptions): Promise<InstallResult> {
  ...
  // reason. The previous order called _resolve first (creating .forge/capabilities/.staging) and
  ...
  try {
    ...
    // A .forge/capabilities/constructor (or __proto__, prototype) bundle must never be promoted —
...
function removeCapability(id: string, opts: LifecycleOptions): RemoveResult {
  ...
  // .forge/capabilities and a .lock file; doing it before detecting corruption pollutes the scope
    ...
    if (shouldBindConsent(opts)) {
      try {
        ...
        consentRevokeWarning =
          ...
          `Clear it manually: forge capability trust revoke ${id}`;
...
export = {
  ...
  // re-export it (acquireLock here still takes a runtimeDir and computes the `.forge/capabilities/.lock`
