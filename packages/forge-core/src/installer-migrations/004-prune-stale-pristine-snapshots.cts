...
const migration: InstallerMigration = {
  ...
  description:
    'Migration 003 removed runtime files from get-shit-done/ but left the matching pristine snapshot ' + // forge-allow-legacy-name
  ...
  destructive: true,
  plan(ctx: MigrationPlanContext): MigrationAction[] {
    const pristineGsdRoot = path.join(ctx.configDir, 'forge-pristine', 'get-shit-done'); // forge-allow-legacy-name
    ...
    const pristineParent = path.join(ctx.configDir, 'forge-pristine');
    try {
    ...
    try {
      if (fs.lstatSync(pristineGsdRoot).isSymbolicLink()) return []; // forge-allow-legacy-name
    } catch {
      return [];
    }
    ...
    walkPristineFiles(ctx.configDir, path.posix.join('forge-pristine', 'get-shit-done'), baseResolved, relPaths); // forge-allow-legacy-name
    ...
    for (const relPath of relPaths) {
      ...
      actions.push({
        ...
        reason: REASON,
        ownershipEvidence:
          'GSD-written pristine snapshot under forge-pristine/get-shit-done/; ' + // forge-allow-legacy-name
