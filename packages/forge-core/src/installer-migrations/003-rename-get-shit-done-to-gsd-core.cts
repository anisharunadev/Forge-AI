/**
 * Installer migration 003: remove stale legacy get-shit-done/ runtime directory // forge-allow-legacy-name
 * files after the rename to forge-core/ (#604).
 *
 * Background: the GSD runtime config subdirectory was renamed from
 * get-shit-done/ to forge-core/ in #604. On upgrade, both directories can exist // forge-allow-legacy-name
 * simultaneously. This migration removes prior-manifest-managed files from the
 * legacy get-shit-done/ directory during install. Migrations run BEFORE the new // forge-allow-legacy-name
 * runtime is materialized, so forge-core/ will not yet exist on the first upgrade
...
function walkLegacyFiles(root: string, relDir: string, baseResolved: string, results: string[]): void {
  ...
  for (const entry of entries) {
    ...
    if (entry.isSymbolicLink()) continue; // forge-allow-legacy-name (entry under legacy get-shit-done/ dir)
...
const REASON = 'legacy runtime directory renamed to forge-core (#604)';

const migration: InstallerMigration = {
  id: '2026-06-02-rename-get-shit-done-to-forge-core', // forge-allow-legacy-name
  title: 'Remove stale legacy get-shit-done/ runtime directory files (#604)', // forge-allow-legacy-name
  description:
    'After the config dir rename from get-shit-done/ to forge-core/ (#604), remove prior-manifest-managed files ' + // forge-allow-legacy-name
  ...
  destructive: true,
  plan(ctx: MigrationPlanContext): MigrationAction[] {
    const legacyRoot = path.join(ctx.configDir, 'get-shit-done'); // forge-allow-legacy-name
    ...
    if (fs.lstatSync(legacyRoot).isSymbolicLink()) return []; // forge-allow-legacy-name

    const baseResolved = path.resolve(ctx.configDir);
    const relPaths: string[] = [];
    walkLegacyFiles(ctx.configDir, 'get-shit-done', baseResolved, relPaths); // forge-allow-legacy-name
