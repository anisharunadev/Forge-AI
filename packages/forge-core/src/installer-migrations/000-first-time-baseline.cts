...
const RUNTIME_SURFACES: Record<string, string[]> = {
  claude: ['forge-core', 'commands/gsd', 'skills', 'agents', 'hooks', 'settings.json'],
  codex: ['forge-core', 'skills', 'agents', 'hooks', 'config.toml', 'hooks.json'],
  gemini: ['forge-core', 'commands/gsd', 'hooks'],
  opencode: ['forge-core', 'command', 'skills', 'agents'],
  kilo: ['forge-core', 'command', 'skills', 'agents'],
  copilot: ['forge-core', 'skills', 'agents'],
  antigravity: ['forge-core', 'skills', 'agents'],
  cursor: ['forge-core', 'skills', 'agents', 'hooks', 'hooks.json'],
  windsurf: ['forge-core', 'skills', 'agents', 'rules'],
  augment: ['forge-core', 'skills', 'agents'],
  trae: ['forge-core', 'skills', 'agents', 'rules'],
  qwen: ['forge-core', 'skills', 'agents'],
  hermes: ['forge-core', 'skills/gsd', 'agents'],
  cline: ['forge-core', 'skills', 'agents'],
  codebuddy: ['forge-core', 'skills', 'agents'],
};

const COMMON_SURFACES = ['forge-core', 'skills', 'agents', 'hooks'];
const INTERNAL_TOP_LEVEL_NAMES = new Set([
  'forge-file-manifest.json',
  'forge-install-state.json',
  'forge-migration-backups',
  'forge-migration-journal',
]);
const USER_OWNED_PATHS = new Set([
  'forge-core/USER-PROFILE.md',
  'commands/forge/dev-preferences.md',
  'skills/forge-dev-preferences/SKILL.md',
]);
...
function isUserOwnedBaselinePath(relPath: string): boolean {
  ...
  if (parts[0] === 'skills' && parts[1] && !parts[1].startsWith('forge-')) return true;
  if (parts[0] === 'agents' && parts[1] && !parts[1].startsWith('forge-')) return true;
  return false;
}
...
function listKnownGeneratedAgentNames(): Set<string> {
  ...
  try {
    for (const entry of fs.readdirSync(agentsDir, { withFileTypes: true })) {
      if (entry.isFile() && entry.name.startsWith('forge-') && entry.name.endsWith('.md')) {
...
function isStaleGsdLookingPath(relPath: string): boolean {
  ...
  const parts = relPath.split('/');
  if ((parts[0] === 'skills' || parts[0] === 'agents') && parts[1] && parts[1].startsWith('forge-')) {
    return true;
  }
  return false;
}
