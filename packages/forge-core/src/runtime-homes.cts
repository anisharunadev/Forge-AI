 ...
 *   hermes  — GSD skills nest under skills/forge/<skillName>/ (not the flat
...
interface DotHomeNestedDescriptor {
   ...
   * (e.g. `forge-core/VERSION`). The same field name and check used by the
  ...
  return resolveConfigHomeFromDescriptor(
    {
      ...
      // Prefer the candidate GSD installed into (carries forge-core/VERSION) over
      ...
      probeExists: 'forge-core/VERSION',
    },
...
export interface AntigravityAmbiguity {
  ...
  presentDirs: string[];
  /**
   * Candidate dirs that carry the GSD marker (forge-core/VERSION). When this has
 ...
 * /forge-update to surface operator guidance (set ANTIGRAVITY_CONFIG_DIR or move
 * forge-core/ into the intended dir). The migration framework cannot relocate an
...
export function detectAntigravityDirAmbiguity(
  ...
  const home = opts.home ?? os.homedir();
  const existsSyncFn = opts.existsSync ?? fs.existsSync;
  const marker = path.join('forge-core', 'VERSION');
