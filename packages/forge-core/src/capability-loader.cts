 ...
 *   - global:  $GSD_HOME/.forge/capabilities/<id>/capability.json  (GSD_HOME defaults to ~)
 *   - project: <projectRoot>/.forge/capabilities/<id>/capability.json
 *
 ...
 *     federated config key collides with first-party (or uses a reserved `forge-` /
 *     `forge-core-` / `anthropic-` id prefix) is rejected.
...
export interface OverlaySkip {
  ...
  reason: string;
  /**
   * #1459 IC-02: a STRUCTURAL discriminant for the skip so consumers (forge-tools `list`) classify a
...
export interface OverlayMeta {
   ...
   * declares `commands` — `capId → <scope>/.forge/capabilities/<capId>`. First-party capabilities
...
const RESERVED_ID_PREFIX = /^(forge-|forge-core-|anthropic-)/;
...
function readHostVersion(): string {
  try {
    // forge-core/bin/lib/ -> repo/package root is three levels up.
 ...
 * 'project' scan and the in-repo `.forge/capabilities` bundle bypasses the CB-3 consent gate via aliasing.
...
/**
 * Read the per-scope ledger co-located with an overlay root (the root is `<scope>/.forge/capabilities`,
...
export function loadRegistry(options: LoadRegistryOptions = {}): Registry {
  ...
  for (const root of overlayRoots(cwd, options.gsdHome)) {
    ...
    for (const ent of entries) {
      ...
      // (loop-resolver, config-loader, surface, capability-state, forge-tools). ADR-1244 D2 mandates a
      ...
      if (RESERVED_ID_PREFIX.test(id)) {
        skip('id uses a reserved first-party prefix (forge-/forge-core-/anthropic-)');
        continue;
      }
      ...
      if (root.scope === 'project') {
        ...
        if (!consented) {
          ...
          // structural `kind: 'unconsented'` so forge-tools `list` marks it INACTIVE by discriminant, not
      ...
      // capability-state, forge-tools). ADR-1244 D2: a malformed overlay is SKIPPED with a warning,
