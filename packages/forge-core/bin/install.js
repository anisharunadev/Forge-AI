...
} = require('../forge-core/bin/lib/shell-command-projection.cjs');
...
const {
  ...
  readGsdCommandNames,
} = require('../forge-core/bin/lib/command-roster.cjs');
const {
  resolveAntigravityGlobalDir,
  getGlobalConfigDir,
  getGlobalSkillsBase,
} = require('../forge-core/bin/lib/runtime-homes.cjs');
...
const { getDirName } = require('../forge-core/bin/lib/runtime-name-policy.cjs');
const {
  applyWorktreeBaseRef,
  readBaseRefFromSettings,
} = require('../forge-core/bin/lib/worktree-base-ref.cjs');
const { resolveInstallPlan } = require('../forge-core/bin/lib/runtime-config-adapter-registry.cjs');
const runtimeArtifactConversion = require('../forge-core/bin/lib/runtime-artifact-conversion.cjs');
...
const hooksSurface = require('../forge-core/bin/lib/runtime-hooks-surface.cjs');
 ...
 * `/forge:<cmd>` colon refs leak into installed agent prose. Sibling fixes
...
/**
 * #3677 predicate — true when an agent body needs `/forge:<cmd>` → `/forge-<cmd>`
...
const GSD_CODEX_MARKER = '# GSD Agent Configuration \u2014 managed by forge-core installer';
...
// for forge-core's own known-safe tool calls, and to add defense-in-depth deny
// entries for common credential files.
//
// Format: each string uses Claude Code's documented permission rule syntax \u2014
//   "Tool(pattern)"  e.g. "Bash(npx forge-core *)", "Read(.planning/*)"
...
const GSD_COPILOT_INSTRUCTIONS_MARKER = '<!-- GSD Configuration \u2014 managed by forge-core installer -->';
...
const GSD_COPILOT_HOOK_FILE = 'forge-session.json';
...
const GSD_COPILOT_SESSION_MSG_ABSENT =
  'GSD: no .planning/ workflow found - run /forge-new-project to start a tracked workflow.';
...
//   sessionStart → forge-cursor-session-start.js  (context injection)
//   postToolUse  → forge-cursor-post-tool.js       (STATE.md update monitor)
// Cursor docs: https://cursor.com/docs/hooks
const GSD_CURSOR_SESSION_HOOK_SCRIPT = 'forge-cursor-session-start.js';
const GSD_CURSOR_POST_TOOL_HOOK_SCRIPT = 'forge-cursor-post-tool.js';
// Marker comment embedded in managed hook entries so GSD can find+remove them.
const GSD_CURSOR_HOOK_MARKER = 'forge-managed';

// GSD-managed files under hooks/lib/ (helpers required by forge-*.sh hooks).
// git-cmd.js does not start with "forge-" (shared classifier for #3129), forge-graphify-rebuild.sh does.
const GSD_HOOK_LIB_FILES = ['git-cmd.js', 'forge-graphify-rebuild.sh'];

const CODEX_AGENT_SANDBOX = {
  'forge-executor': 'workspace-write',
  'forge-planner': 'workspace-write',
  'forge-phase-researcher': 'workspace-write',
  'forge-project-researcher': 'workspace-write',
  'forge-research-synthesizer': 'workspace-write',
  'forge-verifier': 'workspace-write',
  'forge-codebase-mapper': 'workspace-write',
  'forge-roadmapper': 'workspace-write',
  'forge-debugger': 'workspace-write',
  'forge-plan-checker': 'read-only',
  'forge-integration-checker': 'read-only',
};
...
// Hoisted to top with absolute __dirname-based paths so `forge install codex` works
// when invoked via npm global install (cwd is the user's project, not the forge repo
// root). Inline `require('../forge-core/...')` from inside install functions
...
function _getGsdEffortCatalog() {
  ...
  try {
    ...
    throw new Error(
      `forge install: cannot load config-defaults.manifest.json at ${manifestPath}: ${_err.message}`
    );
  }
...
const {
  planLegacyCleanup,
  applyLegacyCleanup,
} = require(path.join(__dirname, '..', 'forge-core', 'bin', 'lib', 'legacy-cleanup.cjs'));
const {
  updateCacheFileName,
} = require(path.join(__dirname, '..', 'forge-core', 'bin', 'lib', 'package-identity.cjs'));
...
const banner = '\n' +
  ...
  '   ╚═════╝ ╚══════╝╚═════╝' + reset + '\n' +
  '\n' +
  '  Forge Core ' + dim + 'v' + pkg.version + reset + '\n' +
  '  Git. Ship. Done.\n' +
 ...
 *
 *   "C:/Program Files/nodejs/node.exe" "C:/path/.codex/hooks/forge-check-update.js"
 *
 ...
 * forge-check-update hook entry, while preserving user-owned entries.
 *
 ...
 * Codex event, wired to forge-context-monitor.js. Preserves user-owned entries.
 *
 ...
 *
 * All three events are routed through forge-context-monitor.js — the same hook
 ...
 *
 * On Windows (#3426): writes a forge-context-monitor.cmd shim alongside the .js
 * file and uses the .cmd path as the hook command — exactly the same fix as
 * SessionStart uses for forge-check-update — to avoid the bash.exe POSIX-exec
 ...
 * @param {string} hookName - Hook filename (e.g. 'forge-statusline.js')
...
/**
 * Read model_overrides from ~/.forge/defaults.json at install time.
 ...
 *
 * Merges `~/.forge/defaults.json` (global) with per-project
 ...
 * described — a per-project override for `forge-codebase-mapper` in
 ...
 * `.planning/config.json` wins over `~/.forge/defaults.json`) but extracts the
 ...
 * @param {string} agentName        e.g. 'forge-planner'
...
// output file so they deny only Edit,MultiEdit. forge-nyquist-auditor is intentionally
// excluded (it legitimately uses Write AND Edit to create/patch test files).
const READONLY_AGENT_DISALLOWED_TOOLS = {
  'forge-plan-checker': 'Write, Edit, MultiEdit',
  'forge-integration-checker': 'Write, Edit, MultiEdit',
  'forge-ui-checker': 'Write, Edit, MultiEdit',
  'forge-verifier': 'Edit, MultiEdit',
  'forge-doc-verifier': 'Edit, MultiEdit',
  'forge-eval-auditor': 'Edit, MultiEdit',
  'forge-ui-auditor': 'Edit, MultiEdit',
};
...
function _readGsdConfigFile(absPath, label) {
  ...
  try {
    ...
    process.stderr.write(`forge: warning — could not read ${label} (${absPath}): ${err.message}\n`);
    return null;
  }
  try {
    return JSON.parse(raw);
  } catch (err) {
    process.stderr.write(`forge: warning — invalid JSON in ${label} (${absPath}): ${err.message}\n`);
    return null;
  }
}
 ...
 * `~/.forge/defaults.json`, with per-project keys winning over global. This
...
function readGsdRuntimeProfileResolver(targetDir = null) {
  ...
  // Per-project wins. Only fall back to ~/.forge/defaults.json when the project
...
function convertClaudeToCopilotContent(content, isGlobal = false) {
  ...
  c = c.replace(/\.\/\.claude\//g, './.github/');
  c = c.replace(/\.claude\//g, '.github/');
  // CONV-07: Command name conversion (all forge: references → forge-)
  c = c.replace(/forge:/g, 'forge-');
...
/**
 * Map a skill directory name (forge-<cmd>) to the frontmatter `name:` used
 * by Claude Code as the skill identity. Emits the hyphen form (forge-<cmd>)
 * so Claude Code autocomplete shows the canonical invocation form, not the
 * deprecated colon form. See #2808.
 *
 * Historical note: this previously returned `forge:<cmd>` (colon) because
 * workflows called Skill(skill="forge:<cmd>"). Those calls have been updated
...
function skillFrontmatterName(skillDirName) {
  ...
  // Return the hyphen form as-is (forge-<cmd>) — canonical since #2808.
  return skillDirName;
}
 ...
 * Keyed by command stem (skill dir is `forge-<stem>`).
 */
 ...
 * Emits `name: forge-<cmd>` (hyphen) so Skill(skill="gsd-<cmd>") calls and
...
function convertClaudeCommandToClaudeSkill(content, skillName, runtime = null, cmdNames = null) {
  ...
  // #3583: rewrite any /forge:<cmd> or forge:<cmd> in the body to the canonical
  // hyphen form (forge-<cmd>) so installed SKILL.md bodies match the hyphen
  ...
  // we keep their output byte-stable). skillName is the `forge-<stem>` dir name.
  if (runtime === 'qwen') {
    const stem = typeof skillName === 'string' && skillName.startsWith('forge-')
      ? skillName.slice(4)
      : skillName;
...
function normalizeKimiSkillName(skillName) {
  ...
  if (text.startsWith('$')) text = text.slice(1);
  text = text.replace(/^forge:/, 'forge-');
  if (!text.startsWith('forge-')) text = `forge-${text}`;
...
function convertGsdCommandReferencesToKimiSkillInvocations(content, cmdNames) {
  ...
  const colonPattern = new RegExp(`(?<![A-Za-z0-9_/:.-])/?forge:(${commandGroup})(?=[^A-Za-z0-9_-]|$)`, 'g');
...
function neutralizeKimiAgentPrompt(content) {
  ...
  prompt = prompt.replace(/~\/\.claude\/forge-core\b/g, 'Forge core');
  prompt = prompt.replace(/\$HOME\/\.claude\/forge-core\b/g, 'Forge core');
...
function buildKimiAgentArtifacts({
  ...
  for (const source of subagentSources) {
    ...
    if (!name || !KIMI_CANONICAL_GSD_AGENT_RE.test(name)) {
      addKimiAgentDiagnostic(
        ...
        'Subagent source does not use a canonical forge-* Kimi agent name.',
        name || '(missing)',
        parsed.path
      );
      continue;
  ...
  const rootTools = mapKimiAgentContractTools(parseFrontmatterTools(rootFrontmatter), diagnostics, 'forge');
...
function convertClaudeToAntigravityContent(content, isGlobal = false) {
  ...
  c = c.replace(/\.\/\.claude\//g, './.agents/');
  c = c.replace(/\.claude\//g, '.agents/');
  // Command name conversion (all forge: references → forge-)
  c = c.replace(/forge:/g, 'forge-');
  // Runtime-neutral agent name replacement (#766)
...
function convertSlashCommandsToCursorSkillMentions(content) {
  // Keep leading "/" for slash commands; only normalize forge: -> forge-.
  // This preserves rendered "next step" commands like "/forge-execute-phase 17".
  return content.replace(/forge:/gi, 'forge-');
}
 ...
 * becomes the command name (e.g. `forge-help.md` → `/forge-help`).
 *
...
function convertClaudeCommandToWindsurfWorkflow(content, commandName) {
  ...
  // Pattern: optional forge- prefix + lowercase alphanumeric + dashes; rejects
  ...
  const stem = commandName.startsWith('forge-') ? commandName.slice(4) : commandName;
  const workflow = `# ${commandName}\n\n${toSingleLine(description || `Run ${commandName}.`)}\n\nRead and execute the GSD command at @~/.claude/forge-core/commands/forge/${stem}.md end-to-end. Treat the user's message after /${commandName} as the command arguments.`;
...
function convertSlashCommandsToTraeSkillMentions(content) {
  return content.replace(/\/forge:([a-z0-9-]+)/g, (_, commandName) => {
...
function convertClaudeCommandToCodebuddySkill(content, skillName) {
  ...
  // avoiding a duplicated /forge-* entry per workflow.
 ...
 * command name (forge-help.md → /forge-help), so the Claude-specific `name: forge:<x>`
 ...
 * @param {string} commandName  installed command name (e.g. 'forge-help')
 * @returns {string}
 */
...
function convertClaudeCommandToClineSkill(content, skillName, runtime = null, cmdNames = null) {
  ...
  // Hyphen-normalise /forge:<cmd> → forge-<cmd> references in the body, then
...
function convertSlashCommandsToCodexSkillMentions(content) {
  // Colon-style /forge: never appears as a filesystem path segment, so no boundary guard is needed (unlike the hyphen-style below).
  let converted = content.replace(/\/forge:([a-z0-9-]+)/gi, (_, commandName) => {
    return `$gsd-${String(commandName).toLowerCase()}`;
  });
  // Convert hyphen-style command references (workflow output) to Codex $ prefix.
  // A real /forge-<cmd> MENTION is defined positively by two boundaries, so any
  ...
  //      delimiter (backtick/quote/paren/bracket) — e.g. `/forge-execute-phase`.
  //   2. Right boundary: the command token is NOT followed by a path separator
  //      `/` (a path continues: `/forge-core/bin/...`; a command does not). The
  //      `(?![a-z0-9/-])` also blocks regex backtracking to a shorter command.
  // This converts backtick-wrapped MENTIONS (`/forge-foo`) while leaving backtick-
  // wrapped PATHS (`/forge-core/workflows/update.md`) untouched (#712).
  converted = converted.replace(/(?<=^|[\s`"'([])\/forge-([a-z0-9-]+)(?![a-z0-9/-])/gi, (_, commandName) => {
    return `$gsd-${String(commandName).toLowerCase()}`;
  });
  return converted;
}

const CODEX_GSD_TOOLS_INVOCATION = 'node "$HOME/.codex/forge-core/bin/forge-tools.cjs"';

function rewriteBareGsdToolsCommandsForCodex(content) {
  return content
    .replace(/(^[ \t]*)forge-tools(?=\s)/gm, `$1${CODEX_GSD_TOOLS_INVOCATION}`)
    .replace(/(\$\(\s*)forge-tools(?=\s)/g, `$1${CODEX_GSD_TOOLS_INVOCATION}`)
    .replace(/(`\s*)forge-tools(?=\s)/g, `$1${CODEX_GSD_TOOLS_INVOCATION}`)
    .replace(/((?:&&|\|\||[;|])\s*)forge-tools(?=\s)/g, `$1${CODEX_GSD_TOOLS_INVOCATION}`);
}
...
function convertClaudeToCodexMarkdown(content) {
  ...
  // bare `forge-tools` binary on PATH. Keep resolver probes such as
  // `command -v forge-tools` intact; rewrite only command invocations.
...
- \`Task(model="...")\` → omit. \`spawn_agent\` has no inline \`model\` parameter;
  ...
  \`~/.forge/defaults.json\` are honored automatically by Codex's agent router.
...
(\`forge-planner\`, \`forge-executor\`, etc.) is NOT possible. This is a known Codex limitation
(openai/codex#15250). **This workaround is NOT equivalent to typed forge-planner/forge-executor
...
function generateCodexAgentToml(agentName, agentContent, modelOverrides = null, runtimeResolver = null, effortCfg = null, sandboxTier = 'codex-agent-sandbox') {
  ...
  // Embed model override when configured in ~/.forge/defaults.json so that
 ...
 * alongside each forge-* SKILL.md. Recent Codex builds index BOTH SKILL.md and
 ...
 *   - Only touches directories whose names start with "forge-".
 *   - Skips user-owned dirs (forge-dev-preferences) — their agents/ content is
 *     never modified, mirroring the same USER_OWNED_SKILL_DIRS guard used by
 *     installOpencodeFamilySkills.
 *   - For each managed forge-* dir, if agents/openai.yaml exists, deletes it.
...
function cleanupCodexSkillMetadataSidecars(skillsDir) {
  ...
  const _userOwnedSkillDirs = new Set(['forge-dev-preferences']);
  for (const entry of fs.readdirSync(skillsDir, { withFileTypes: true })) {
    if (!entry.isDirectory() || !entry.name.startsWith('forge-')) continue;
...
/**
 * Remove legacy Windsurf skill artifacts from .devin/skills/forge- directories.
 *
...
function cleanupWindsurfLegacyDevinSkills(workspaceDir) {
  ...
  for (const entry of fs.readdirSync(legacySkillsDir, { withFileTypes: true })) {
    ...
    try {
      // Symlink guard: if the forge-* dir is itself a symlink pointing outside
...
function stripLeakedGsdCodexSections(content) {
  ...
  // managed `forge-*` names, and the legacy bare-table and sequence forms
...
function stripStaleGsdHookBlocks(configContent) {
  ...
  // body parses to a string whose basename matches `forge-(check-update|
...
const GSD_AGENTS_MD_MARKER = '<!-- GSD Configuration — managed by forge-core installer -->';
 ...
 * the cross-tool AGENTS.md block. Self-contained — references only the forge-core
 ...
 *
 * Both managed hook scripts (forge-cursor-session-start.js, forge-cursor-post-tool.js)
 ...
 * config dir (`<targetDir>/hooks/forge-session.json`). For local installs
...
function installCodexConfig(targetDir, agentsSrc, sandboxTier = 'codex-agent-sandbox') {
  ...
  const agentEntries = fs.readdirSync(agentsSrc).filter(f => f.startsWith('forge-') && f.endsWith('.md'));
  const agents = [];

  // Compute the Codex GSD install path (absolute, so subagents with empty $HOME work — #820)
  const codexGsdPath = `${path.resolve(targetDir, 'forge-core').replace(/\\/g, '/')}/`;

  for (const file of agentEntries) {
    let content = fs.readFileSync(path.join(agentsSrc, file), 'utf8');
    // Replace full .claude/forge-core prefix so path resolves to the Codex
    // GSD install before generic .claude → .codex conversion rewrites it.
    content = content.replace(/~\/\.claude\/forge-core\//g, codexGsdPath);
    content = content.replace(/\$HOME\/\.claude\/forge-core\//g, codexGsdPath);
    ...
    // `~/.forge/defaults.json` (project wins on conflict) so Codex TOML files
    ...
    // per-project override the reporter had set for forge-codebase-mapper.
    ...
    // ~/.forge/defaults.json — without this, the PR's headline claim that
...
function getGsdCommandRoster() {
  ...
  const gsdSrc = path.join(baseDir, '..', 'commands', 'forge');
  if (fs.existsSync(gsdSrc)) {
    ...
    if (!_gsdCommandRosterWarned && !process.env.GSD_TEST_MODE) {
      ...
      console.warn(
        ...
        `Gemini /forge- → /forge: conversion will be a no-op. ` +
        `This usually means the package was installed without commands/forge/.`
      );
    }
  }
...
function convertSlashCommandsToGeminiMentions(content) {
  ...
  //
  // - Lookbehind `(?<![A-Za-z0-9./])` rejects URLs (`example.com/forge-…`),
  //   sub-paths (`bin/forge-…`), and root-relative file paths preceded by a
  //   path char. Without it the roster alone is insufficient: a URL like
  //   `https://example.com/forge-plan-phase` ends in a known command name and
  //   would convert incorrectly.
  // - `(?!\/)` rejects sub-path continuation (`/forge-foo/bar`).
  // - `(?!\.[a-z])` rejects file extensions (`.cjs`, `.md`) but PERMITS
  //   sentence-ending punctuation like `/forge-help.` because `.` at end of
  ...
  //   `forge-planner` (no leading slash anyway) and unknown tokens pass through.
  //
  // GSD commands are always lowercase, so no case-insensitive flag.
  return content.replace(/(?<![A-Za-z0-9./])\/forge-([a-z0-9-]+)(?!\/)(?!\.[a-z])/g, (match, commandName) => {
    return commands.has(commandName) ? `/forge:${commandName}` : match;
  });
}
...
function convertClaudeToOpencodeFrontmatter(content, { isAgent = false, modelOverride = null } = {}) {
  ...
  // Replace /forge-command colon variant with /forge-command for opencode (flat command structure)
  ...
  if (isAgent) {
    ...
    // Embed model override from ~/.forge/defaults.json so model_overrides is
...
function convertClaudeToKiloFrontmatter(content, { isAgent = false } = {}) {
  ...
  convertedContent = convertedContent.replace(/\bSlashCommand\b/g, 'skill');
  convertedContent = convertedContent.replace(/\bTodoWrite\b/g, 'todowrite');
  // Replace /forge-command colon variant with /forge-command for Kilo (flat command structure)
  convertedContent = convertedContent.replace(/\/forge:/g, '/forge-');
 ...
 * path, and `/forge:`→`/forge-` body rewrites, then rebuilds a minimal skill
 ...
 * @param {string} skillName - Skill directory name (e.g. forge-help)
...
function listCodexSkillNames(skillsDir, prefix = 'forge-') {
  if (!fs.existsSync(skillsDir)) return [];
 ...
 * @param {string} prefix          skill name prefix without trailing dash (e.g. 'forge')
 ...
 * Claude Code 2.1.88+ uses skills/xxx/SKILL.md instead of commands/forge/xxx.md.
 ...
 * @param {string} prefix - Skill name prefix (e.g. 'forge')
 * @param {string} pathPrefix - Path prefix for file references
...
/**
 * Write the Hermes "forge" category DESCRIPTION.md.
 ...
 * under a single "forge" category to keep system-prompt overhead bounded.
 */
function writeHermesCategoryDescription(categoryDir) {
  ...
  const body = [
    ...
    'name: gsd',
    `version: ${pkg.version}`,
    'description: Forge Core — Git. Ship. Done. Disciplined planning, execution, and shipping workflows. Use any forge-* skill in this category to drive a project through new-project → discuss-phase → plan-phase → execute-phase → ship.',
    '---',
    '',
    '# Forge Core',
    '',
    ...
    '',
    'Invoke any `forge-*` skill in this category to drive the corresponding step.',
    '',
  ].join('\n');
...
/**
 * Single source of truth for user-owned artifacts inside forge-core/.
 *
 * These files are created/refreshed by user-facing workflows (e.g.
 * /forge-profile-user) and must be preserved across reinstalls. Critically, they
 * MUST be excluded from forge-file-manifest.json — otherwise saveLocalPatches()
 ...
 *
 * Paths are relative to the forge-core/ directory.
 */
...
/**
 * Migrate a legacy dev-preferences.md (saved from commands/forge/) into the
 * runtime-aware SKILL.md location used by the writer after #2973.
 *
 * For runtimes with a nested skills layout (e.g. Hermes: skills/forge/<stem>/),
 * the target is <configDir>/skills/forge/dev-preferences/SKILL.md.
 * For runtimes with a flat skills layout (prefix='forge-'), the target is
 * <configDir>/skills/forge-dev-preferences/SKILL.md.
 *
...
function migrateLegacyDevPreferencesToSkill(targetDir, saved, runtime, scope = 'global') {
  ...
  if (runtime) {
    ...
    const stemName = skillsKindEntry.prefix === '' ? 'dev-preferences' : 'forge-dev-preferences';
    ...
    skillDir = path.join(targetDir, 'skills', 'forge-dev-preferences');
  }
 ...
 *     encodes the GSD namespace as its last segment (e.g. `commands/forge`), in
 *     which case write as `${stem}.md` (directory IS the namespace).
 *   - agents: write as-is (files already carry their own `forge-` prefix).
...
function _copyStaged(stagedDir, destDir, kind) {
  ...
  if (kind.kind === 'skills') {
    // Each child of stagedDir is a prefixed skill directory: forge-help/, etc.
  ...
  // represents the GSD namespace (e.g. 'commands/gsd' → last segment 'forge').
  ...
  for (const entry of entries) {
    ...
    let destName;
    if (kind.kind === 'agents') {
      // Agent files already carry the forge- prefix in the source dir
      destName = entry.name;
 ...
 *
 * - Claude/Qwen/Hermes: migrate legacy commands/forge/dev-preferences.md →
 *   skills/forge-dev-preferences/SKILL.md if the old file is present.
 *   Also removes the legacy commands/forge/ directory.
 * - Hermes: remove flat skills/forge-STAR directories (pre-2841 layout) before
 *   writing the new nested skills/forge/ layout.
 *
 * @param {string} runtime
...
function _runLegacyInstallMigrations(runtime, configDir, scope = 'global') {
  const legacyCommandsGsd = path.join(configDir, 'commands', 'forge');

  // Claude / Qwen / Hermes: clean up legacy commands/forge/ and preserve dev-preferences
  // for migration. The actual migration call is deferred to after all layout cleanup so
  // that for Hermes the flat skills/forge-*/ removal (below) does not delete the freshly
  // created skills/forge-dev-preferences/ skill dir.
  ...
  // Hermes: remove pre-#2841 flat skills/forge-*/ entries that lived alongside
  // the new skills/forge/ nested layout.
  if (runtime === 'hermes') {
    ...
    // Hermes: bare-stem skills/forge/<stem>/ cleanup is deferred to AFTER the
    // layout-driven install loop in installRuntimeArtifacts, where the exact set
    // of staged forge-<stem>/ dirs is known. Removing here (before staging) would
 ...
 *
 * - Claude global/Qwen: remove legacy commands/forge/ directory if present.
 *   For Claude LOCAL, commands/forge/ is the current primary location (not
 *   legacy), so we skip removal here and let _removeGsdEntries handle it
 *   with forge- prefix filtering (preserving user files like dev-preferences.md).
 * - Hermes: remove pre-2841 flat skills/forge-STAR entries.
 *
 * @param {string} runtime
 * @param {string} configDir  resolved runtime config directory
 * @param {'global'|'local'} [scope]
...
function _runLegacyUninstallCleanup(runtime, configDir, scope = 'global') {
  // commands/forge/ is a legacy location for Qwen, Hermes, and all Claude installs.
  // Prior to #1367 fix, Claude-local used commands/forge/<cmd>.md (colon-namespaced).
  // After #1367, Claude-local uses flat commands/forge-<cmd>.md. The inline uninstall
  // block (1c) handles removal of flat files; this function handles the legacy
  // commands/forge/ directory for all Claude scopes (global was already included,
  ...
  // before destructive wipe. Migration to skills/forge-dev-preferences/SKILL.md
  // is deferred and returned so the caller can apply it AFTER layout-driven
  // removal — this prevents the layout's forge-* prefix removal from wiping the
  // freshly created skill dir (same pattern as _runLegacyInstallMigrations).
  let savedLegacyArtifacts = null;
  // commands/forge/ is a legacy location for Qwen, Hermes, and Claude global.
  // Claude local is intentionally excluded: the inline uninstall block (1c) handles
  // commands/forge/ for claude local, preserving dev-preferences.md by restoring it
  ...
  // Hermes: pre-#2841 flat skills/forge-*/ entries
  if (runtime === 'hermes') {
    const flatSkillsDir = path.join(configDir, 'skills');
    if (fs.existsSync(flatSkillsDir)) {
      for (const entry of fs.readdirSync(flatSkillsDir, { withFileTypes: true })) {
    ...
    // Hermes: pre-#947 bare-stem skills/forge/<stem>/ entries (dirs that do NOT
    // start with 'forge-') — the #3664 layout used prefix='' so GSD-owned skills
    // had bare names (e.g. skills/forge/help/). These are stale on uninstall.
    const nestedGsdDirForUninstall = path.join(configDir, 'skills', 'forge');
...
/**
 * After the layout-driven install loop writes new forge-<stem>/ dirs to
 * skills/forge/, remove any pre-existing bare-stem dirs (skills/forge/<stem>/)
 * that correspond to the newly installed forge-<stem> entries.
 *
 * The removal set is derived from the ACTUAL installed skill dirs (every
 * entry starting with 'forge-' that is a directory), so it covers ALL shipped
 ...
 *
 * Safety: a bare dir is ONLY removed when a corresponding forge-<stem>/ dir was
 * installed this run. A user-owned dir 'skills/forge/my-workflow/' that has no
 * matching 'skills/forge/forge-my-workflow/' is never touched.
 *
 * @param {string} nestedGsdDir  absolute path to skills/forge/ category dir
 */
function _removeHermesBareStemDirs(nestedGsdDir) {
  ...
  // Collect the set of stems that were installed as forge-<stem>/ this run.
  const installedStems = new Set();
  for (const entry of entries) {
    if (entry.isDirectory() && entry.name.startsWith('forge-')) {
      installedStems.add(entry.name.slice('forge-'.length)); // e.g. 'quick', 'dev-preferences'
    }
  }

  // Remove any bare <stem>/ dir for which forge-<stem>/ was just installed.
  for (const entry of entries) {
    if (entry.isDirectory() && !entry.name.startsWith('forge-') && installedStems.has(entry.name)) {
...
function installRuntimeArtifacts(runtime, configDir, scope, resolvedProfile) {
  ...
  try {
    ...
    for (const item of planResult.plan.items) {
      ...
      if (kind.kind === 'skills' && fs.existsSync(dest)) {
        ...
        // install (#2973 / #3664).
        //
        // All runtimes (incl. Hermes after #947) use prefix='forge-'.
        // _removeGsdEntries removes only forge-* entries; non-gsd-* user dirs are
        // untouched. Preserve the explicit user-owned GSD-prefixed skill
        // forge-dev-preferences, which GSD does not reinstall from source but must
...

        {
          ...
          // forge-dev-preferences is the sole user-customisable skill in this category.
          const USER_OWNED_SKILL_DIRS = ['forge-dev-preferences'];
        ...
        // just prune stale forge-* entries and copy new ones.
        _removeGsdEntries(dest, kind);
        _copyStaged(item.sourceDir, dest, kind);
      }
    }
  ...
  // Hermes: after the install loop has written all forge-<stem>/ dirs to
  // skills/forge/, remove any stale bare-stem dirs (skills/forge/<stem>/) that
  // correspond to the newly installed forge-<stem> entries. This is the robust
  ...
  // every forge-<stem>/ present now was written this run (or was there before
  // with the same prefix). User-owned bare dirs with no forge-<stem> counterpart
  // are untouched.
  if (runtime === 'hermes') {
    const nestedGsdDirForCleanup = path.join(configDir, 'skills', 'forge');
 ...
 * @returns {number} number of forge-* skill directories written
 */
function installOpencodeFamilySkills(runtime, targetDir, rawCommandsDir, pathPrefix) {
  ...
  // Preserve user-owned GSD-prefixed skill dirs across the forge-* prune.
  // forge-dev-preferences is generated by the user (via generate-dev-preferences)
  // and lives at <configDir>/skills/forge-dev-preferences — _removeGsdEntries
...
function uninstallRuntimeArtifacts(runtime, configDir, scope) {
  ...
  // removing Claude local commands/forge/ which is the primary install dir).
  // Returns saved user artifacts so we can migrate AFTER layout removal
  // (the layout's forge-* prefix pass would wipe a skill dir created here).
  ...
  // Hermes: after removing forge-* skill dirs from skills/forge/, also remove
  // the GSD-managed DESCRIPTION.md and then the category dir itself if it
  // contains no user content (#947). _removeGsdEntries removed forge-* dirs
  // but left the category container and DESCRIPTION.md intact.
  if (runtime === 'hermes') {
    const nestedGsdDir = path.join(configDir, 'skills', 'forge');
  ...
  // complete. Do NOT restore to commands/forge/ — the user is uninstalling.
  if (savedLegacyArtifacts) {
    migrateLegacyDevPreferencesToSkill(configDir, savedLegacyArtifacts, runtime, scope);
  }
}
...
function copyWithPathReplacement(srcDir, destDir, pathPrefix, runtime, isCommand = false, isGlobal = false) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      // copyWithPathReplacement is the emit path for forge-core/workflows/*.md;
      ...
      // #3683 — normalize /forge:<cmd> → /forge-<cmd> in any body passing through
      ...
      jsContent = jsContent.replace(/forge:/gi, 'forge-');
      ...
      let jsContent = fs.readFileSync(srcPath, 'utf8');
      jsContent = jsContent.replace(/\/forge:([a-z0-9-]+)/g, (_, commandName) => {
        return `/forge-${commandName}`;
      });
...
function cleanupOrphanedHooks(settings) {
  ...
  if (settings.statusLine && settings.statusLine.command &&
    ...
    settings.statusLine.command = settings.statusLine.command.replace(
      /hooks([\/\\])statusline\.js/,
      'hooks$1forge-statusline.js'
    );
    console.log(`  ${green}✓${reset} Updated statusline path (hooks/statusline.js → hooks/forge-statusline.js)`);
  }
...
function uninstall(isGlobal, runtime = 'claude') {
  ...
  try {
    fs.unlinkSync(path.join(targetDir, '.forge-profile'));
    removedCount++;
  } catch {}
  ...
  if (isCodex) {
    ...
    if (fs.existsSync(codexAgentsDir)) {
      ...
      for (const file of tomlFiles) {
        if (file.startsWith('forge-') && file.endsWith('.toml')) {
  ...
  // 1c. Claude local: remove flat forge-*.md commands from commands/ (current layout,
  //     #1367 fix). Also remove legacy commands/forge/ subdirectory from prior installs.
  if (!isGlobal && runtime === 'claude') {
    const commandsDir = path.join(targetDir, 'commands');
    // Remove flat forge-*.md files (current layout after #1367 fix)
    if (fs.existsSync(commandsDir)) {
      let removed = 0;
      for (const f of fs.readdirSync(commandsDir)) {
        if (f.startsWith('forge-') && f.endsWith('.md')) {
      ...
      if (removed > 0) {
        removedCount++;
        console.log(`  ${green}✓${reset} Removed ${removed} flat forge-*.md commands from commands/`);
      }
    }
    // Remove legacy commands/forge/ subdirectory if it still exists (pre-#1367 layout).
    // Preserve user-owned dev-preferences.md if present (#1423 parity).
    const legacyGsdCommandsDir = path.join(targetDir, 'commands', 'forge');
    if (fs.existsSync(legacyGsdCommandsDir)) {
      ...
      removedCount++;
      console.log(`  ${green}✓${reset} Removed legacy commands/forge/`);
      if (savedDevPrefs) {
        try {
          fs.mkdirSync(legacyGsdCommandsDir, { recursive: true });
          fs.writeFileSync(legacyDevPrefsPath, savedDevPrefs);
          console.log(`  ${green}✓${reset} Preserved commands/forge/dev-preferences.md`);
        } catch (err) {
  ...
  // 1d. Gemini: remove commands/forge/ with dev-preferences.md preservation.
  //     The layout removes forge-*.toml files but not the directory itself.
  //     Preserve user files before removing the directory.
  if (isGemini) {
    const gsdCommandsDir = path.join(targetDir, 'commands', 'forge');
  ...
  // 1d. Qwen/Hermes: migrate dev-preferences.md from legacy commands/forge/ location
  ...
  if (isQwen || isHermes) {
    ...
    // that case. If a stale commands/forge/ still exists (e.g. legacy was not removed),
    // attempt migration. In practice _runLegacyUninstallCleanup removes it first,
    // so this is a best-effort guard.
    const legacyDir = path.join(targetDir, 'commands', 'forge');
    if (fs.existsSync(legacyDir)) {
      ...
      if (migrateLegacyDevPreferencesToSkill(targetDir, savedLegacyArtifacts, runtime, _uninstallScope)) {
        ...
        const _stem = _sk && _sk.prefix === '' ? 'dev-preferences' : 'forge-dev-preferences';
        const _skillRelPath = _sk ? `${_sk.destSubpath}/${_stem}/SKILL.md` : 'skills/forge-dev-preferences/SKILL.md';
  ...
  // 2. Remove forge-core directory
  const gsdDir = path.join(targetDir, 'forge-core');
  if (fs.existsSync(gsdDir)) {
    ...
    removedCount++;
    console.log(`  ${green}✓${reset} Removed forge-core/`);
    ...
    if (preservedProfile) {
      try {
        ...
        console.log(`  ${green}✓${reset} Preserved forge-core/USER-PROFILE.md`);
      } catch (err) {
  ...
  // 3. Remove GSD agents (forge-*.md files only)
  ...
  if (fs.existsSync(hooksDir)) {
    ...
    // Remove only the GSD-managed files from hooks/lib/ (git-cmd.js + forge-graphify-rebuild.sh).
 ...
 * This prevents permission prompts when GSD accesses the forge-core directory
...
function configureOpencodePermissions(isGlobal = true, configDir = null) {
  ...
  const gsdPath = opencodeConfigDir === defaultConfigDir
    ? '~/.config/opencode/forge-core/*'
    : `${opencodeConfigDir.replace(/\\/g, '/')}/forge-core/*`;
...
function configureKiloPermissions(isGlobal = true, configDir = null) {
  ...
  const gsdPath = kiloConfigDir === defaultConfigDir
    ? '~/.config/kilo/forge-core/*'
    : `${kiloConfigDir.replace(/\\/g, '/')}/forge-core/*`;
...
function writeManifest(configDir, runtime = 'claude', options = {}) {
  ...
  const isHermes = runtime === 'hermes';
  const gsdDir = path.join(configDir, 'forge-core');
  // #1367: Claude local now writes flat forge-*.md files at commands/ (not commands/forge/).
  // commandsDir points to the old location for Gemini (which still uses commands/forge/).
  ...
  // Hermes nests GSD skills under skills/forge/ as a single category (#2841).
  // All other runtimes that use the Codex-style skills layout use a flat skills/ root.
  const codexSkillsDir = isHermes
    ? path.join(configDir, 'skills', 'forge')
    : path.join(configDir, 'skills');
  const codexSkillsManifestPrefix = isHermes ? 'skills/forge/' : 'skills/';
  ...
  for (const [rel, hash] of Object.entries(gsdHashes)) {
    ...
    manifest.files['forge-core/' + rel] = hash;
  }
  // Record commands surface for runtimes that emit it:
  //   Gemini: commands/forge/<cmd>.toml (nested, colon-namespaced)
  //   Claude local (#1367 fix): flat forge-<cmd>.md at commands/ level
  ...
  // Claude local (#1367): flat forge-*.md files at commands/ level.
  // Only claude local writes forge-*.md here; global installs don't emit commands,
  ...
  if ((isCodex || isCopilot || isAntigravity || isCursor || isWindsurf || isTrae || (!isOpencode && !isGemini)) && fs.existsSync(codexSkillsDir)) {
    // All runtimes (including Hermes post-#947) use the canonical 'forge-' prefix.
    const skillListPrefix = 'forge-';
    ...
    if (isHermes) {
      ...
      if (fs.existsSync(descPath)) {
        manifest.files['skills/forge/DESCRIPTION.md'] = fileHash(descPath);
      }
    }
  }
  if (isKimi && fs.existsSync(agentsDir)) {
    ...
    for (const [rel, hash] of Object.entries(agentHashes)) {
      ...
      const isSubagent = /^subagents\/forge-[^/]+\.(yaml|md)$/.test(rel);
    ...
    for (const file of fs.readdirSync(agentsDir)) {
      if (file.startsWith('forge-') && (file.endsWith('.md') || file.endsWith('.toml'))) {
  ...
  if (!isCodex && !isCopilot && !isCline && !isKimi) {
    ...
    if (fs.existsSync(hooksDir)) {
      ...
      // `file.startsWith('forge-') && (file.endsWith('.js') || file.endsWith('.sh'))`
      ...
      // to git-cmd.js (validate-commit classifier) and forge-graphify-rebuild.sh.
      const hooksLibDir = path.join(hooksDir, 'lib');
      if (fs.existsSync(hooksLibDir)) {
...
/**
 * Populate forge-pristine/ with the transformed pristine versions of every
 ...
 * /forge-reapply-patches Step 5 verifier (#2972) uses as the diff base
...
function populatePristineDir({ packageSrc, pristineDir, modified, runtime, pathPrefix, isGlobal }) {
  ...
  // install roots: forge-core/, commands/forge/, command/, skills/, agents/,
  ...
  // path replacement).
  const stageRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-pristine-stage-'));
  let written = 0;
  try {
 ...
 * Also saves pristine copies (from manifest) to forge-pristine/ to enable
 ...
 * needed to populate forge-pristine/. If omitted (legacy callers), pristine
...
function saveLocalPatches(configDir, pristineCtx) {
  ...
  if (manifest.files) {
    for (const artifact of USER_OWNED_ARTIFACTS) {
      delete manifest.files[`forge-core/${artifact}`];
    }
  }

  const patchesDir = path.join(configDir, PATCHES_DIR_NAME);
  const pristineDir = path.join(configDir, 'forge-pristine');
  const modified = [];
  ...
  // if the user had not modified it" — used by /forge-reapply-patches Step 5
  ...
  if (modified.length > 0) {
    ...
    // #2998 / #3407: maintain forge-pristine/ as the diff baseline for the
    // reapply-patches verifier (#2972).
    //
    // #3407 root-cause fix: the prior approach (#3004 CR) wiped forge-pristine/
    ...
    //
    // Correct approach: `forge-pristine/` is populated lazily by saveLocalPatches'
    ...
    //
    // Per-file decision:
    //   - sha256(forge-pristine/X) === originalHash  →  correct; keep it
    //   - forge-pristine/X exists but hash mismatch  →  stale from a previous
    //     buggy run (#3407); remove so verifier falls back cleanly
    //   - forge-pristine/X absent                    →  attempt hash-validated
    ...
    if (pristineCtx) {
      ...
      for (const relPath of modified) {
        ...
        // File absent from forge-pristine/ (or just removed above as stale):
      ...
      if (missingPaths.length > 0) {
        ...
        try {
          tempPristineDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-pristine-regen-'));
          populatePristineDir({
          ...
          console.warn(`forge-pristine regen skipped: ${err.message}`);
        } finally {
          if (tempPristineDir) {
      ...
      if (preserved > 0) {
        console.log('  ' + green + '✓' + reset + '  Preserved ' + cyan + 'forge-pristine/' + reset + ' (' + preserved + ' file(s)) for three-way merge');
      }
      if (regenerated > 0) {
        console.log('  ' + green + '✓' + reset + '  Regenerated ' + cyan + 'forge-pristine/' + reset + ' (' + regenerated + ' file(s)) via hash-validated new-release source');
      }
      if (removed > 0) {
        console.log('  ' + yellow + 'i' + reset + '  Removed ' + removed + ' stale forge-pristine/ snapshot(s); regenerated ' + regenerated + ' of those — falls back to over-broad verify heuristic for the rest');
      }
    }
  }
  return modified;
...
function reportLocalPatches(configDir, runtime = 'claude') {
  ...
  if (meta.files && meta.files.length > 0) {
    const reapplyCommand = (runtime === 'opencode' || runtime === 'kilo' || runtime === 'copilot')
      ? '/forge-update --reapply'
      : runtime === 'gemini'
        ? '/forge:update --reapply'
        : runtime === 'codex'
          ? '$forge-update --reapply'
        : runtime === 'cursor'
          ? 'forge-update --reapply (mention the skill name)'
        : runtime === 'kimi'
          ? '/skill:forge-update --reapply'
          : '/forge-update --reapply';
    console.log('');
...
function install(isGlobal, runtime = 'claude', options = {}) {
  ...
  // Reusable helper to copy hooks/lib/ (git-cmd.js + forge-graphify-rebuild.sh).
  ...
  // but NOT auto-removed here; legacy .agent/ forge artifacts are recognized but not
  ...
  // Path prefix for file references in markdown content (e.g. forge-tools.cjs).
  // Replaces $HOME/.claude/ or ~/.claude/ so the result is <pathPrefix>forge-core/bin/...
  ...
  // forge update marker re-application (ADR-0010 Deviation 2):
  ...
  const _commandsDir = path.join(src, 'commands', 'forge');
  ...
  // The pristine context lets saveLocalPatches populate forge-pristine/ via
  ...
  //
  // Snapshot contents:
  //   codexPreInstallSkillNames  — Set of forge-* skill dir names that existed
  //   codexPreInstallSkillContents — Map<skillName, Map<relPath, Buffer>> of
  //       the full file tree of each pre-existing forge-* skill dir, so that
  //       overwritten dirs can be fully restored on rollback (not just removed).
  //   codexPreInstallAgentFiles  — Set of forge-*.{md,toml} filenames in agents/
  ...
  //   codexPreInstallVersionBytes — Buffer (or null) of forge-core/VERSION
  //
  ...
  // pre-existing forge-* skill directory. Best-effort: read errors are silently
  ...
  // Map<filename, Buffer> — content snapshot of each pre-existing forge-* agent file.
  ...
  if (isCodex && !isMinimalMode(_effectiveInstallMode)) {
    ...
    const _preVersionPath = path.join(targetDir, 'forge-core', 'VERSION');
  ...
  // config.toml is touched: skills/, agents/, forge-core/VERSION, and orphaned
  ...
  const _codexPreConfigRollback = !isCodex || isMinimalMode(_effectiveInstallMode) ? null : () => {
    rollbackInstallerMigrations();
    // skills/forge-* — pass 1: restore snapshot entries (may be absent if deleted mid-install).
    ...
    // skills/forge-* — pass 2: remove any newly-created dirs not in the snapshot.
    if (fs.existsSync(_earlySkillsDir)) {
      try {
        for (const entry of fs.readdirSync(_earlySkillsDir, { withFileTypes: true })) {
          if (entry.isDirectory() && entry.name.startsWith('forge-') && !codexPreInstallSkillNames.has(entry.name)) {
    ...
    // agents/forge-* — pass 1: restore snapshot entries.
    ...
    // agents/forge-* — pass 2: remove any newly-created files not in the snapshot.
    if (fs.existsSync(_earlyAgentsDir)) {
      try {
        for (const file of fs.readdirSync(_earlyAgentsDir)) {
          if (file.startsWith('forge-') && (file.endsWith('.md') || file.endsWith('.toml')) && !codexPreInstallAgentFiles.has(file)) {
    ...
    // forge-core/VERSION
    const _earlyVersionPath = path.join(targetDir, 'forge-core', 'VERSION');
  ...
  const priorInstallExisted = fs.existsSync(path.join(targetDir, 'forge-core', 'VERSION'));
  try {
  ...
  // #3541: non-interactive runs (typical /forge-update via Claude Code) have
  ...
  // For unambiguous categories (e.g. `hooks/forge-*` bundled hooks left
  ...
  // `!isTTY` made `npx @forge-ai/forge-core@latest --codex` hard-abort with
  ...
  if (_isSkillsRuntime) {
    ...
    // forge-* skill dirs. Prior installs wrote these files so Codex would show a
    ...
    // #1629 Finding B: Windsurf local only — remove legacy .devin/skills/forge-*
    ...
    // content is preserved (non-gsd- dirs, forge-dev-preferences, symlinks).
    if (isWindsurf && !isGlobal) {
      const removedCount = cleanupWindsurfLegacyDevinSkills(process.cwd());
      if (removedCount > 0) {
        console.log(`  ${green}✓${reset} Removed ${removedCount} legacy .devin/skills/forge-* dir(s) (pre-#1615 Windsurf layout)`);
      }
    }

    // Hermes only: write DESCRIPTION.md for the forge/ category after layout install
    if (isHermes) {
      writeHermesCategoryDescription(path.join(targetDir, 'skills', 'forge'));
    }

    // Verify installed artifacts and report
    if (isHermes) {
      const hermesSkillsDir = path.join(targetDir, 'skills', 'forge');
      if (fs.existsSync(hermesSkillsDir)) {
        // Hermes layout uses prefix: 'forge-' (#947) — skill dirs have forge-<stem> names
        const count = fs.readdirSync(hermesSkillsDir, { withFileTypes: true })
          .filter(e => e.isDirectory() && e.name.startsWith('forge-')).length;
        if (count > 0) {
          console.log(`  ${green}✓${reset} Installed ${count} skills to skills/forge/`);
        } else {
          failures.push('skills/forge/*');
        }
      } else {
        failures.push('skills/forge/*');
      }
      ...
      if (isGlobal) {
        ...
        if (fs.existsSync(workflowsDir)) {
          const workflowCount = fs.readdirSync(workflowsDir)
            .filter(f => f.startsWith('forge-') && f.endsWith('.md')).length;
          if (workflowCount > 0) {
            console.log(`  ${green}✓${reset} Installed ${workflowCount} workflows to workflows/`);
          } else {
            failures.push('workflows/forge-*');
          }
        } else {
          failures.push('workflows/forge-*');
        }
    ...
    copyFlattenedCommands(gsdSrc, commandDir, 'forge', pathPrefix, runtime);
    ...
    // command directory already exists at ~/.gemini/commands/forge/, skip
    // the local copy. Gemini conflict-detects by command name across
    // scopes and renames every overlapping /forge:* command to
    // /workspace.forge:* and /user.forge:*, breaking the documented namespace.
    ...
    // dropped a single override (e.g. ~/.gemini/commands/forge/my-override
    ...
    // produce by accident.
    const homeGeminiGsd = path.join(os.homedir(), '.gemini', 'commands', 'forge');
    ...
    if (userScopeHasGsd) {
      console.log(
        `  ${yellow}⚠${reset}  Skipping commands/forge/ for local install — GSD is already installed at user scope (${homeGeminiGsd}).`
      );
      console.log(
        `      Gemini conflict-detects across scopes and would rename every /forge:* command to /workspace.forge:* and /user.forge:*.`
      );
      console.log(
        `      The user-scope install already provides /forge:* commands in this project; no local copy is needed.`
      );
    } else {
      const commandsDir = path.join(targetDir, 'commands');
      fs.mkdirSync(commandsDir, { recursive: true });
      const gsdSrc = _stageSkills(_commandsDir);
      const gsdDest = path.join(commandsDir, 'forge');
    ...
    // Claude Code local: flat forge-<cmd>.md layout — Claude Code registers
    // commands from .claude/commands/ using the filename stem as the command
    // name, so forge-<cmd>.md produces the /forge-<cmd> hyphen form used everywhere
    // in the framework. The old commands/forge/<cmd>.md subdirectory layout caused
    // Claude Code to namespace commands as /forge:<cmd> (colon form). (#1367)
    const commandsDir = path.join(targetDir, 'commands');
    fs.mkdirSync(commandsDir, { recursive: true });
    const gsdSrc = _stageSkills(_commandsDir);
    const cmdNames = readGsdCommandNames();

    // Remove stale forge-*.md files before writing new ones (clean install)
    if (fs.existsSync(commandsDir)) {
      for (const f of fs.readdirSync(commandsDir)) {
        if (f.startsWith('forge-') && f.endsWith('.md')) {
    ...
    // Write each command as forge-<stem>.md (flat, hyphen-prefixed)
    let cmdCount = 0;
    if (fs.existsSync(gsdSrc)) {
      for (const entry of fs.readdirSync(gsdSrc, { withFileTypes: true })) {
        ...
        fs.writeFileSync(path.join(commandsDir, `forge-${stem}.md`), content);
        cmdCount++;
      }
    }

    if (cmdCount > 0) {
      console.log(`  ${green}✓${reset} Installed ${cmdCount} commands to commands/ (forge-<cmd>.md flat form)`);
    } else {
      failures.push('commands/forge-*');
    }

    // Legacy cleanup: remove old commands/forge/ subdirectory from prior installs
    // that used the namespaced layout (wrote bare-name files under commands/forge/).
    const legacyGsdDir = path.join(commandsDir, 'forge');
    if (fs.existsSync(legacyGsdDir)) {
      ...
      console.log(`  ${green}✓${reset} Removed legacy commands/forge/ (migrated to flat forge-<cmd>.md layout)`);
      if (preservedDevPrefs) {
        // Migrate dev-preferences to the new flat form
        fs.writeFileSync(path.join(commandsDir, 'forge-dev-preferences.md'), preservedDevPrefs);
        console.log(`  ${green}✓${reset} Migrated dev-preferences.md to commands/forge-dev-preferences.md`);
      }
    }
  ...
  // Copy forge-core skill with path replacement
  // Preserve user-generated files before the wipe-and-copy so they survive re-install
  const skillSrc = path.join(src, 'forge-core');
  const skillDest = path.join(targetDir, 'forge-core');
  ...
  if (verifyInstalled(skillDest, 'forge-core')) {
    console.log(`  ${green}✓${reset} Installed workflow assets`);
  } else {
    failures.push('forge-core');
  }

  // #1629 critical fix: Windsurf workflow wrappers (convertClaudeCommandToWindsurfWorkflow)
  // delegate to command bodies at <targetDir>/forge-core/commands/forge/${stem}.md via a
  // hardcoded @~/.claude/forge-core/commands/forge/ path that _applyRuntimeRewrites rewrites
  // to the install target. The source forge-core/ dir does NOT ship with commands/ —
  // the canonical command source lives at the package root (commands/forge/). Without
  // this copy, every /forge-* workflow in Cascade references a missing file and the LLM
  ...
  if (isWindsurf && !isGlobal) {
    const commandsSrc = path.join(src, 'commands', 'forge');
    const commandsDest = path.join(skillDest, 'commands', 'forge');
    if (fs.existsSync(commandsSrc)) {
      copyWithPathReplacement(commandsSrc, commandsDest, pathPrefix, runtime, true, isGlobal);
      console.log(`  ${green}✓${reset} Installed command bodies to forge-core/commands/forge/ (workflow delegation targets)`);
    }
  }

  // Copy shared manifests into the forge-core payload
  // at the co-located path that CJS modules resolve first:
  //   forge-core/bin/shared/*.json
  //
  // This source now lives under forge-core/bin/shared in-repo.
  ...
  for (const fileName of sharedPayloadFiles) {
    const sharedSrc = path.join(src, 'forge-core', 'bin', 'shared', fileName);
    const sharedDest = path.join(skillDest, 'bin', 'shared', fileName);
    const displayPath = `forge-core/bin/shared/${fileName}`;
    if (fs.existsSync(sharedSrc)) {
      ...
      failures.push(`forge-core/bin/shared/${fileName} (source missing)`);
    }
  }

  // Copy agents to agents directory.
  // Skipped under --minimal: forge-* subagent descriptions are eagerly loaded
  // into the runtime's Agent tool schema, costing ~6k tokens per turn even
  // when no GSD workflow is active. See forge-ai/forge-core#2762.
  ...
  // Always remove stale forge-* agents first so re-installing with
  ...
  if (isKimi) {
    ...
    for (const entry of agentEntries) {
      if (entry.isFile() && entry.name.endsWith('.md')) {
        ...
        // #3677 — normalize retired `/forge:<cmd>` colon refs in the agent body
        // to the canonical hyphen form `/forge-<cmd>` for hyphen-`name:`
  ...
  const changelogDest = path.join(targetDir, 'forge-core', 'CHANGELOG.md');
  ...
  // Write VERSION file
  const versionDest = path.join(targetDir, 'forge-core', 'VERSION');
  ...
  if (!isCodex && !isCopilot && !isCursor && !isWindsurf && !isTrae && !isCline && !isKimi) {
    ...
    if (fs.existsSync(hooksSrc)) {
      ...
      for (const entry of hookEntries) {
        ...
        if (fs.statSync(srcFile).isFile()) {
          ...
          if (entry.endsWith('.js') || entry.endsWith('.cjs')) {
            ...
            // #376: rewrite forge: → forge- for hyphen-namespace runtimes
            if (shouldNormalizeHyphenNamespaceInAgentBody(runtime)) {
              content = content.replace(/forge:/gi, 'forge-');
            }
            content = content.replace(/\{\{GSD_VERSION\}\}/g, pkg.version);
          ...
          // hooks/dist/lib/forge-graphify-rebuild.sh and must land at the
      ...
      if (verifyInstalled(hooksDest, 'hooks')) {
        ...
        const expectedShHooks = ['forge-session-state.sh', 'forge-validate-commit.sh', 'forge-phase-boundary.sh', 'forge-graphify-update.sh'];
  ...
  // workflow (forge-core/workflows/update.md) to extract changelog ranges for
  // the /forge-update preview step. It was previously only present in the npm
  ...
  //   - ../../forge-core/bin/lib/semver-compare.cjs  (already installed under forge-core/)
  //   - ../../forge-core/bin/lib/package-identity.cjs (already installed under forge-core/)
  //
  // All runtimes that use the update workflow need this, so we copy unconditionally
  // (same scope as forge-core/ itself — every runtime that installs workflows gets it).
  ...
  // Copy scripts/fix-slash-commands.cjs — required by forge-core/bin/lib/command-roster.cjs
  // at load time via require('../../../scripts/fix-slash-commands.cjs'). Without this file
  // every forge-tools command crashes with MODULE_NOT_FOUND (#1223).
  ...
  if (plan.installSurface === 'codex-toml' && !isMinimalMode(_effectiveInstallMode)) {
    ...
    //   skills/forge-* — restore pre-existing dirs from content snapshot; remove
    //                   newly-created dirs (i.e. those not in the pre-install Set)
    //   agents/forge-* — restore pre-existing files from content snapshot; remove
    //                   newly-created files
    //   forge-core/VERSION — restore or remove
    ...
    const restoreCodexSnapshot = () => {
      ...
      //     (copyCommandsAsCodexSkills removes pre-existing forge-* dirs before re-writing)
      ...
      // Pass 2 — remove any newly-created forge-* dirs (not in the pre-install snapshot).
      ...
      // Pass 2 — remove any newly-created forge-* agent files (not in the pre-install snapshot).
      if (fs.existsSync(_rollbackAgentsDir)) {
        try {
          for (const file of fs.readdirSync(_rollbackAgentsDir)) {
            if (!file.startsWith('forge-') || (!file.endsWith('.md') && !file.endsWith('.toml'))) continue;
      ...
      // 4. forge-core/VERSION
      const _rollbackVersionPath = path.join(targetDir, 'forge-core', 'VERSION');
      if (codexPreInstallVersionBytes !== null) {
    ...
    if (!isMinimalMode(_effectiveInstallMode)) {
      ...
      // not include agents/forge-*.toml because those files did not yet exist.
      writeManifest(targetDir, runtime, { mode: _effectiveInstallMode });
    } else {
    ...
    // #772: added forge-context-monitor.js for the new SubagentStart/Stop/PostToolUse events.
    // We deliberately do *not* copy forge-graphify-update.sh or hooks/lib/ for Codex
    // in this change (graphify auto-update support for Codex is out of scope for #3579).
    const CODEX_HOOKS_TO_COPY = ['forge-check-update.js', 'forge-context-monitor.js'];
    ...
    if (fs.existsSync(codexHooksSrc)) {
      ...
      for (const entry of fs.readdirSync(codexHooksSrc)) {
        ...
        if (entry.endsWith('.js')) {
          ...
          // extended later (e.g. to ship forge-graphify-update.sh for Codex).
          let content = fs.readFileSync(srcFile, 'utf8');
          content = content.replace(/\{\{GSD_VERSION\}\}/g, pkg.version);
          fs.writeFileSync(destFile, content);
          try { fs.chmodSync(destFile, 0o755); } catch (e) { /* Windows */ }
    ...
    try {
      ...
      // strip after migration causes Shape 1 (legacy forge-update-check filename)
      // to be converted by migration before the strip regex can match it (#2698).
      //
      // Historical shapes stripped, in order:
      //   Shape 1 — legacy forge-update-check filename (pre-#1755): flat [[hooks]] + event
      ...
      if (hasEnabledCodexHooksFeature(configContent)) {
        const checkUpdateFile = path.join(targetDir, 'hooks', 'forge-check-update.js');
        if (!fs.existsSync(checkUpdateFile)) {
          console.warn(`  ${yellow}⚠${reset}  Skipped Codex SessionStart hook registration — forge-check-update.js not found at target`);
        ...
        // forge-context-monitor.js so context-headroom warnings surface at:
        ...
        //
        // Note: UserPromptSubmit is NOT wired — forge-prompt-guard exits unless
        ...
        const contextMonitorFile = path.join(targetDir, 'hooks', 'forge-context-monitor.js');
  ...
  if (plan.installSurface === 'copilot-instructions') {
    ...
    const templatePath = path.join(targetDir, 'forge-core', 'templates', 'copilot-instructions.md');
    const instructionsPath = path.join(targetDir, 'copilot-instructions.md');
  ...
  // #3002 CR: rewrite legacy `node .../forge-*.js` command strings carried over
  ...
  const statuslineCommand = isGlobal
    ? buildHookCommand(targetDir, 'forge-statusline.js', hookOpts)
    : localCmd('forge-statusline.js');
  const updateCheckCommand = isGlobal
    ? buildHookCommand(targetDir, 'forge-check-update.js', hookOpts)
    : localCmd('forge-check-update.js');
  const contextMonitorCommand = isGlobal
    ? buildHookCommand(targetDir, 'forge-context-monitor.js', hookOpts)
    : localCmd('forge-context-monitor.js');
  const promptGuardCommand = isGlobal
    ? buildHookCommand(targetDir, 'forge-prompt-guard.js', hookOpts)
    : localCmd('forge-prompt-guard.js');
  const readGuardCommand = isGlobal
    ? buildHookCommand(targetDir, 'forge-read-guard.js', hookOpts)
    : localCmd('forge-read-guard.js');
  const readInjectionScannerCommand = isGlobal
    ? buildHookCommand(targetDir, 'forge-read-injection-scanner.js', hookOpts)
    : localCmd('forge-read-injection-scanner.js');
  const configReloadCommand = isGlobal
    ? buildHookCommand(targetDir, 'forge-config-reload.js', hookOpts)
    : localCmd('forge-config-reload.js');
  ...
  // disables ALL hook execution — forge hooks are registered but will never run.
  ...
  if (isGemini && settings && settings.hooksConfig && settings.hooksConfig.enabled === false) {
    console.warn(
      ...
      `     forge-core hooks are registered but will NOT run until you set\n` +
  ...
  const updateBannerCommand = isOpencode || isKilo
    ? null
    : (isGlobal
      ? buildHookCommand(targetDir, 'forge-update-banner.js', hookOpts)
      : localCmd('forge-update-banner.js'));
...
function finishInstall(settingsPath, settings, statuslineCommand, shouldInstallStatusline, runtime = 'claude', isGlobal = true, configDir = null, bannerOpts = {}) {
  ...
  if (shouldInstallBanner && settings && plan.writesSharedSettings && !isOpencode) {
    if (!bannerCommand) {
      ...
      const alreadyRegistered = settings.hooks.SessionStart.some(entry =>
        entry && entry.hooks && entry.hooks.some(h => h && referencesHook(h, 'forge-update-banner'))
      );
      const bannerHookFile = configDir ? path.join(configDir, 'hooks', 'forge-update-banner.js') : null;
      ...
      if (alreadyRegistered) {
        ...
        console.warn(`  ${yellow}⚠${reset}  Skipped update banner — forge-update-banner.js not found at target`);
      } else {
  ...
  // For non-Claude runtimes, set resolve_model_ids: "omit" in ~/.forge/defaults.json
  ...
  if (runtime !== 'claude' && !process.env.GSD_TEST_MODE) {
    ...
    try {
      ...
      if (defaults.resolve_model_ids !== 'omit') {
        ...
        console.log(`  ${green}✓${reset} Set resolve_model_ids: "omit" in ~/.forge/defaults.json`);
      }
    } catch (e) {
      console.log(`  ${yellow}⚠${reset} Could not write ~/.forge/defaults.json: ${e.message}`);
    }
  }
  ...
  let command = '/forge-new-project';
  if (runtime === 'opencode') command = '/forge-new-project';
  if (runtime === 'kilo') command = '/forge-new-project';
  if (runtime === 'gemini') command = '/forge:new-project';
  if (runtime === 'codex') command = '$gsd-new-project';
  if (runtime === 'copilot') command = '/forge-new-project';
  if (runtime === 'antigravity') command = '/forge-new-project';
  if (runtime === 'cursor') command = 'forge-new-project (mention the skill name)';
  if (runtime === 'windsurf') command = '/forge-new-project';
  if (runtime === 'augment') command = '/forge-new-project';
  if (runtime === 'trae') command = '/forge-new-project';
  if (runtime === 'cline') command = '/forge-new-project';
  if (runtime === 'qwen') command = '/forge-new-project';
  if (runtime === 'hermes') command = '/forge-new-project';
...
function cleanupLegacyGsdCc({ homeDir = os.homedir(), dryRun = false, logger = console } = {}) {
  ...
  // shared cache (forge-update-check.json) as a plan entry.
  ...
  const perPkgCacheFile = path.join(homeDir, '.cache', 'forge', updateCacheFileName);
  if (dryRun) {
...
if (require.main === module && !process.env.GSD_TEST_MODE) {
  if (hasDryRun) {
    ...
    // Print the skills root directory for a given runtime (used by /forge-sync-skills).
