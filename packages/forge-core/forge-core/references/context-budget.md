...
| POOR | 70%+ | Emergency mode. Checkpoint progress immediately. No new reads unless critical. | `warn`: emit warning + recommend `/forge:pause-work`. `auto`: invoke pause-work before next wave. `off`: proceed anyway. |
...
Before starting a long phase (especially `/forge:execute-phase`, `/forge:plan-phase`, or anything that fans out across many subagents), run this audit:
