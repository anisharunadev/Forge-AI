...
| `manager.flags.discuss` | `""` | Flags passed to `/forge:discuss-phase` when dispatched from manager (e.g. `"--auto --analyze"`) |
...
**Using `forge-tools query` (preferred):**

```bash
# Commit with automatic commit_docs + gitignore checks:
forge-tools query commit "docs: update state" --files .planning/STATE.md

# Load config via state load (returns JSON):
INIT=$(forge-tools query state.load)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
# commit_docs is available in the JSON output

# Or use init commands which include commit_docs:
INIT=$(forge-tools query init.execute-phase "1")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
...
| `review.models.<cli>` | string\|null | `null` | Any model ID string | Per-CLI model override for /forge:review (e.g., `review.models.gemini`). Falls back to CLI default when null. |
...
| `workflow.ai_integration_phase` | boolean | `true` | `true`, `false` | Run /forge:ai-integration-phase before planning AI system phases |
...
| `workflow.context_guard_mode` | string | `"warn"` | `"auto"`, `"warn"`, `"off"` | Context exhaustion guard mode for `execute-phase`. Before each wave, the orchestrator self-assesses context pressure using degradation signals from `context-budget.md`. `"warn"` (default): emit a warning and recommend `/forge:pause-work` when POOR tier is detected. `"auto"`: automatically invoke `/forge:pause-work` before the next wave when POOR tier is detected. `"off"`: disable the guard. The guard is heuristic — no programmatic context-% API exists. |
...
| `workflow.code_review_command` | string\|null | `null` | Any shell command | External code-review command integrated into `/forge:ship`. The diff is piped to the command via stdin; the command must output JSON with a `verdict` field (`"APPROVED"` or `"REVISE"`). Non-zero exit or `"REVISE"` verdict blocks the ship workflow. When unset, the built-in review flow runs. Example: `my-review-tool --review`. |
...
Set via `ship.*` namespace in config.json. These fields affect `/forge:ship` PRD-style pull request body composition only.
...
| `git.phase_branch_template` | string | `"forge/phase-{phase}-{slug}"` | Template with `{phase}`, `{slug}` | Branch naming template for `phase` strategy |
| `git.milestone_branch_template` | string | `"forge/{milestone}-{slug}"` | Template with `{milestone}`, `{slug}` | Branch naming template for `milestone` strategy |
...
| `features.global_learnings` | boolean | `false` | `true`, `false` | Enable injection of global learnings from `~/.forge/learnings/` into agent prompts |
...
Set via `intel.*` namespace (e.g., `"intel": { "enabled": true }`). Controls the queryable codebase intelligence system consumed by `/forge:map-codebase --query`.

| Key | Type | Default | Allowed Values | Description |
|-----|------|---------|----------------|-------------|
| `intel.enabled` | boolean | `false` | `true`, `false` | Enable queryable codebase intelligence system. When `true`, `/forge:map-codebase --query` builds and queries a JSON index in `.planning/intel/`. |
...
| Key | Type | Default | Allowed Values | Description |
|-----|------|---------|----------------|-------------|
| `manager.flags.discuss` | string | `""` | Any CLI flags string | Flags passed to `/forge:discuss-phase` from manager (e.g., `"--auto --analyze"`) |
...
5. **Search API keys and flags** -- `brave_search`, `firecrawl`, and `exa_search` are auto-set to `true` during project creation if the corresponding API key is detected (environment variable or `~/.forge/<name>_api_key` file). Setting them to `true` without the API key has no effect.
