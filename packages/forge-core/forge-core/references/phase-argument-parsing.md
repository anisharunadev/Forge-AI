...
```bash
PHASE_INFO=$(forge-tools query find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(forge-tools query roadmap.get-phase "${PHASE}" --pick found)
...
```bash
PHASE_DIR=$(forge-tools query find-phase "${PHASE}" --raw)
```
