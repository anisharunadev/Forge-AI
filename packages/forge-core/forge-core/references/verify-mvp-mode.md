...
> Loaded by `verify-work` workflow and `forge-verifier` agent only when the phase under verification has `mode: mvp` in ROADMAP.md. Reframes UAT generation from technical checks to user-flow walk-throughs.
...
- The phase under verification has `**Mode:** mvp` in ROADMAP.md (parsed via `forge-tools query roadmap.get-phase --pick mode`).
- AND the phase has a user-story-formatted goal (set by `/forge mvp-phase` per Phase 2): "As a [user role], I want to [capability], so that [outcome]."

If the phase has `mode: mvp` but the goal is NOT in user-story format, the verifier surfaces this as a discrepancy and asks the user to run `/forge mvp-phase` to reformat the goal — same pattern as the planner agent under MVP_MODE (per `references/planner-mvp-mode.md`).
...
Coverage is a goal-backward check: "did this phase deliver what its user story promised?" — sourced from the existing `forge-verifier` agent's goal-backward methodology, narrowed to the user story.
...
The `forge-verifier` agent produces `VERIFICATION.md`. Under MVP mode, the report adds a top-level "User Flow Coverage" section that maps each step of the user story to evidence in the codebase:
