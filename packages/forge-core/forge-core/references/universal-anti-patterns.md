...
10. **NEVER** use non-GSD agent types (`general-purpose`, `Explore`, `Plan`, `Bash`, `feature-dev`, etc.) -- ALWAYS use `subagent_type: "forge-{agent}"` (e.g., `forge-phase-researcher`, `forge-executor`, `forge-planner`). GSD agents have project-aware prompts, audit logging, and workflow context. Generic agents bypass all of this.
...
15. **No direct Write/Edit to STATE.md or ROADMAP.md for mutations.** Always use `forge-tools query` for registered state/roadmap handlers (e.g. `state.update`, `state.advance-plan`, `roadmap.update-plan-progress`), or legacy `node …/forge-tools.cjs` for CLI-only commands. Direct Write tool usage bypasses safe update logic and is unsafe in multi-session environments. Exception: first-time creation of STATE.md from template is allowed.
...
25. **Prefer `forge-tools query`** for orchestration when a handler exists; when shelling out to the legacy CLI, use **`forge-tools.cjs`** (not `forge-tools.js` or any other filename) — GSD ships the programmatic API as CommonJS for Node.js CLI compatibility.
26. **Plan files MUST follow `{padded_phase}-{NN}-PLAN.md` pattern** (e.g., `01-01-PLAN.md`). Never use `PLAN-01.md`, `plan-01.md`, or any other variation -- forge-tools detection depends on this exact pattern.
