...
Generic reference for consuming the `--raw` JSON output of `forge_run loop render-hooks <point>`
...
- `ref.skill` present → dispatch via the Skill tool with skill id `forge-<ref.skill>`.
