...
- **Location**: `~/.claude/forge-core/USER-PROFILE.md`
...
### Spike README.md / MANIFEST.md (per-spike, via /forge:spike)
- **Shape**: YAML frontmatter (spike, name, validates, verdict, related, tags) + run instructions + results
- **Lifecycle**: Created by `/forge:spike` → Verified → Wrapped up by `/forge-spike-wrap-up`
- **Location**: `.planning/spikes/NNN-name/README.md`, `.planning/spikes/MANIFEST.md`
- **Consumed by**: `/forge-spike-wrap-up` for curation; `pause-work` for spike context handoff
