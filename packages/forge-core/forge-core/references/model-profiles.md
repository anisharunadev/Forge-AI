...
|-------|-----------|------------|----------|------------|-----------|
| forge-planner | opus | opus | sonnet | opus | inherit |
| forge-roadmapper | opus | sonnet | sonnet | sonnet | inherit |
| forge-executor | opus | sonnet | sonnet | sonnet | inherit |
| forge-phase-researcher | opus | sonnet | haiku | sonnet | inherit |
| forge-project-researcher | opus | sonnet | haiku | sonnet | inherit |
| forge-research-synthesizer | sonnet | sonnet | haiku | haiku | inherit |
| forge-debugger | opus | sonnet | sonnet | opus | inherit |
| forge-codebase-mapper | sonnet | haiku | haiku | haiku | inherit |
| forge-verifier | sonnet | sonnet | haiku | sonnet | inherit |
| forge-plan-checker | sonnet | sonnet | haiku | haiku | inherit |
| forge-integration-checker | sonnet | sonnet | haiku | haiku | inherit |
| forge-nyquist-auditor | sonnet | sonnet | haiku | haiku | inherit |
...
| Phase type | Agents |
|---|---|
| `planning` | forge-planner, forge-roadmapper, forge-pattern-mapper |
| `discuss` | (reserved — no subagent today) |
| `research` | forge-phase-researcher, forge-project-researcher, forge-research-synthesizer, forge-codebase-mapper, forge-ui-researcher |
| `execution` | forge-executor, forge-debugger, forge-doc-writer |
| `verification` | forge-verifier, forge-plan-checker, forge-integration-checker, forge-nyquist-auditor, forge-ui-checker, forge-ui-auditor, forge-doc-verifier |
...
- **`model_overrides`** is per-agent precision (e.g. force `haiku` on `forge-codebase-mapper` for a fan-out).
...
When installed for a non-Claude runtime, the GSD installer sets `resolve_model_ids: "omit"` in `~/.forge/defaults.json`. This returns an empty model parameter for all agents, so each agent uses the runtime's default model. No manual setup is needed.
...
```json
{
  ...
  "model_overrides": {
    "forge-planner": "o3",
    "forge-executor": "o4-mini",
    "forge-debugger": "o3",
    "forge-codebase-mapper": "o4-mini"
  }
}
```
...
|---|---|---|
| `light` | forge-codebase-mapper, forge-pattern-mapper, forge-research-synthesizer, forge-plan-checker, forge-integration-checker, forge-nyquist-auditor, forge-ui-checker, forge-ui-auditor, forge-doc-verifier | Cheap/fast — pure mappers, scanners, low-stakes audits |
| `standard` | forge-executor, forge-phase-researcher, forge-project-researcher, forge-verifier, forge-doc-writer, forge-ui-researcher | Default workhorse — research, writing, primary verification |
| `heavy` | forge-planner, forge-roadmapper, forge-debugger | Deep reasoning — already at top, can't escalate further |
