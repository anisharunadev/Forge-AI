...
`@~/.claude/forge-core/references/edge-probe.md`: `edge-probe` reaches the
...
- OWASP / prototype-pollution / path-traversal / injection → breadcrumb to `/forge:secure-phase`
  and `eslint` (security plugins), not a minted prohibition.
- GDPR / data-retention / consent → breadcrumb to `/forge:secure-phase`.
- Generic fairness/bias canon → breadcrumb to `/forge:secure-phase`.

The breadcrumb reads like: *"prototype-pollution is canon — covered by /forge:secure-phase +
...
fixture under `forge-core/references/prohibition-probe-fixtures/` by
