...
- forge-executor — executes a specific, scoped code change
</available_agent_types>
...
INIT=$(forge_run query audit-uat 2>/dev/null || echo "{}")
...
**b. Run tests:**
```bash
AUDIT_TEST_CMD=$(forge_run query config-get workflow.test_command --default "" 2>/dev/null || true)
