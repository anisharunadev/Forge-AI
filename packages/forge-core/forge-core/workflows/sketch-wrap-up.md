...
Run `/forge:sketch` first to create design explorations.
```
Exit.
...
```bash
forge_run query commit "docs(sketch-wrap-up): package [N] sketch findings into project skill" --files .planning/sketches/WRAP-UP-SUMMARY.md
```
</step>
...
`/forge:sketch` (run with no argument — its frontier mode analyzes the sketch landscape and proposes consistency and frontier sketches)

───────────────────────────────────────────────────────────────

**Also available:**
- `/forge:plan-phase` — start building the real UI
- `/forge:ui-phase` — generate a UI design contract for a frontend phase
- `/forge:sketch [idea]` — sketch a specific new design area
- `/forge:explore` — continue exploring
...
- [ ] Summary presented
- [ ] Next-step options presented (including frontier sketch exploration via `/forge:sketch`)
</success_criteria>
