...
<required_reading>
@~/.claude/forge-core/references/domain-probes.md
@~/.claude/forge-core/references/gate-prompts.md
@~/.claude/forge-core/references/universal-anti-patterns.md
</required_reading>
...
**Express path available:** If you already have a PRD or acceptance criteria document, use `/forge:plan-phase {phase} --prd path/to/prd.md` to skip this discussion and go straight to planning.
...
INIT=$(forge_run query init.phase-op "${PHASE}"); [[ "$INIT" == @file:* ]] && INIT=$(cat "${INIT#@file:}")
AGENT_SKILLS_ADVISOR=$(forge_run query agent-skills forge-advisor-researcher)
```
...
Use /forge:progress ${GSD_WS} to see available phases.
```
Exit workflow.
...
if [ -f "$HOME/.claude/forge-core/USER-PROFILE.md" ]; then
  ADVISOR_MODE=true
else
  ADVISOR_MODE=false
fi
...
Check if a SPEC.md (from `/forge:spec-phase`) exists for this phase. SPEC.md locks requirements before implementation decisions.
...
**Note:** SPEC.md files named `AI-SPEC.md` (from `/forge:ai-integration-phase`) are excluded — different purpose.
</step>
...
If raw spikes/sketches exist but no findings skill, note: `⚠ Unpackaged spikes/sketches detected — run /forge:spike --wrap-up or /forge:sketch --wrap-up to make findings available.`
...
```bash
TODO_MATCHES=$(forge_run query todo.match-phase "${PHASE_NUMBER}")
```
...
Read `@~/.claude/forge-core/references/scout-codebase.md` — it contains the phase-type→map selection table, single-read rule, no-maps fallback, and `<codebase_context>` output schema. Then execute:
...
```bash
DISCUSS_PRE_HOOKS_JSON=$(forge_run loop render-hooks discuss:pre --raw)
```
Apply each entry in `activeHooks` per @~/.claude/forge-core/references/loop-hook-dispatch.md. Empty list → continue to `analyze_phase`.
</step>
...
```bash
DISCUSS_POST_HOOKS_JSON=$(forge_run loop render-hooks discuss:post --raw)
```
Apply each entry in `activeHooks` per @~/.claude/forge-core/references/loop-hook-dispatch.md. Empty list → continue to `confirm_creation`.
</step>
...
`/forge:plan-phase ${PHASE} ${GSD_WS}`

---

**Also available:** `--chain` for auto plan+execute after; `/forge:plan-phase ${PHASE} --skip-research ${GSD_WS}` to plan without research; `/forge:ui-phase ${PHASE} ${GSD_WS}` for UI design contracts; review/edit CONTEXT.md before continuing.
```
</step>
...
```bash
forge_run query commit "docs(${padded_phase}): capture phase context" --files "${phase_dir}/${padded_phase}-CONTEXT.md" "${phase_dir}/${padded_phase}-DISCUSSION-LOG.md"
```
...
forge_run query commit "docs(state): record phase ${PHASE} context session" --files .planning/STATE.md
```
</step>
