...
@~/.claude/forge-core/references/questioning.md
@~/.claude/forge-core/references/domain-probes.md
</required_reading>

<available_agent_types>
Valid GSD subagent types (use exact names — do not fall back to 'general-purpose'):
- forge-phase-researcher — Researches specific questions and returns concise findings
</available_agent_types>
...
| Spike | `/forge:spike` (invoke) | Feasibility uncertainty surfaced — "will this API work?", "can we do X?" |
| Sketch | `/forge:sketch` (invoke) | Design direction unclear — "what should this look like?", "how should this feel?" |
...
- **Phases:** Use existing `/forge-add-phase` command via SlashCommand
...
forge_run query commit "docs: capture exploration — {topic_slug}" --files {file_list}
```
...
Continue exploring with `/forge:explore` or start working with `/forge:progress --next`.
```
