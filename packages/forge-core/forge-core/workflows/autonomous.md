...
When `PLAN_STRATEGY=converge`, the planning step MUST invoke the plan-review convergence workflow instead of `forge-plan-phase`. `--cross-ai` is an alias for `--converge`. Forward `CONVERGENCE_ARGS` exactly as parsed so reviewer flags and `--max-cycles N` retain the same meaning as they have on `/forge:plan-review-convergence`.
...
INIT=$(forge_run query init.milestone-op)
...
if [ "$PLAN_STRATEGY" = "converge" ]; then
  CONVERGENCE_ENABLED=$(forge_run query config-get workflow.plan_review_convergence 2>/dev/null || echo "false")
  if [ "$CONVERGENCE_ENABLED" != "true" ]; then
    printf '%s\n' \
      'forge-autonomous --converge is disabled (workflow.plan_review_convergence=false).' \
      '' \
      'Enable plan convergence with:' \
      '' \
      '  forge config-set workflow.plan_review_convergence true' \
      '' \
...
**If `roadmap_exists` is false:** Error — "No ROADMAP.md found. Run `/forge:new-milestone` first."
**If `state_exists` is false:** Error — "No STATE.md found. Run `/forge:new-milestone` first."
...
```bash
ROADMAP=$(forge_run query roadmap.analyze)
```
...
```bash
DETAIL=$(forge_run query roadmap.get-phase ${PHASE_NUM})
```
...
```bash
PHASE_STATE=$(forge_run query init.phase-op ${PHASE_NUM})
```
...
```bash
SKIP_DISCUSS=$(forge_run query config-get workflow.skip_discuss 2>/dev/null || echo "false")
```
...
```bash
forge_run query commit "docs(${PADDED_PHASE}): auto-generated context (discuss skipped)" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```
Skill(skill="forge-discuss-phase", args="${PHASE_NUM}")
```
...
HOOKS_JSON=$(forge_run loop render-hooks plan:pre --raw)
```
...
```bash
GATE=$(forge_run check ui-plan-gate "${PHASE_NUM}" --raw)
```
...
(Prepend `forge-` to `ref.skill` — so `ui-phase` → `forge-ui-phase`. Bare `${PHASE_NUM}` args — autonomous style, same pattern as the verify:post dispatch.) Entries where `kind == "gate"` are silently ignored — autonomous is always pipeline mode, there is no blocking gate here.
...
- **If `RUNTIME` is `codex`:** Dispatch plan as a background agent to keep the main context lean. While plan runs, the workflow can immediately start discussing the next phase (see step 4).
  ...
  Agent(
    ...
    run_in_background=true,
    prompt="Run plan-phase for phase ${PHASE_NUM}: Skill(skill=\"forge-plan-phase\", args=\"${PHASE_NUM}\")"
  )
  ```
...
- **Otherwise (Claude Code or any other non-Codex runtime):** Run plan **inline** (do NOT background) so the plan-checker runs. The next phase's discuss does not overlap planning here — correctness over overlap.
  ...
  ```
  Skill(skill="forge-plan-phase", args="${PHASE_NUM}")
  ```
...
Agent(
  ...
  run_in_background=true,
  prompt="Run execute-phase for phase ${PHASE_NUM}: Skill(skill=\"forge-execute-phase\", args=\"${PHASE_NUM} --no-transition\")"
)
```
...
```
Skill(skill="forge-execute-phase", args="${PHASE_NUM} --no-transition")
```
...
```
Skill(skill="forge-plan-phase", args="${PHASE_NUM} --gaps")
```
...
```bash
UI_SPEC_FILE=$(ls "${PHASE_DIR}"/*-UI-SPEC.md 2>/dev/null | head -1)
HOOKS_JSON=$(forge_run loop render-hooks verify:post --raw)
```
...
(i.e. prepend `forge-` to `ref.skill` — so `ui-review` → `forge-ui-review`.)
...
> Full instructions are in `forge-core/references/autonomous-smart-discuss.md`. Read that file now and follow it exactly.

Smart discuss is an autonomous-optimized variant of `forge-discuss-phase`. It proposes grey area answers in batch tables — the user accepts or overrides per area — and writes an identical CONTEXT.md to what discuss-phase produces.

**Inputs:** `PHASE_NUM` from execute_phase.

Read and execute: `$HOME/.claude/forge-core/references/autonomous-smart-discuss.md`
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 ...
 Resume with: /forge:autonomous --from ${next_incomplete_phase}
```
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 ...
 Lifecycle skipped — run /forge:autonomous without --only
...
On **"Stop"**: Go to handle_blocker with "User stopped — audit gaps remain. Run /forge:audit-milestone to review, then /forge:complete-milestone when ready."
...
On **"Stop"**: Go to handle_blocker with "User stopped — tech debt to address. Run /forge:audit-milestone to review details."
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 ...
 Resume with: /forge:autonomous ${ONLY_PHASE ? "--only " + ONLY_PHASE : "--from " + next_phase}${TO_PHASE ? " --to " + TO_PHASE : ""}
```
...
- [ ] `--interactive` runs discuss inline via forge-discuss-phase (asks questions, waits for user)
...
- [ ] `--converge` routes planning through `forge-plan-review-convergence`
...
- [ ] Default autonomous planning remains `forge-plan-phase` when convergence is not requested
</success_criteria>
