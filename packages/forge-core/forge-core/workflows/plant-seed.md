...
Seeds auto-surface during /forge:new-milestone when trigger conditions match the
...
_To be filled in. Run `/forge:capture --seed --enrich SEED-{PADDED}` to add context._

## When to Surface

**Trigger:** when relevant

This seed will surface during `/forge:new-milestone` when the milestone scope matches.

## Scope Estimate

**Unknown** — run `/forge:capture --seed --enrich SEED-{PADDED}` to estimate effort.
...
forge_run query commit "docs: plant seed — {$IDEA}" --files .planning/seeds/SEED-{PADDED}-{slug}.md
```
</step>
...
Trigger and scope are set to defaults. Run `/forge:capture --seed --enrich SEED-{PADDED}`
to add trigger conditions, rationale, and scope estimate at your convenience.

This seed will surface automatically when you run /forge:new-milestone.
```
</step>
...
Commit the update:
```bash
forge_run query commit "docs: enrich seed ${SEED_ID} — trigger + why + scope" --files "$SEED_FILE"
```
