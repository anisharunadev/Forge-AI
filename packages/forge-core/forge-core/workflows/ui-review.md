...
AGENT_SKILLS_UI_REVIEWER=$(forge_run query agent-skills forge-ui-auditor)
```

Parse: `phase_dir`, `phase_number`, `phase_name`, `phase_slug`, `padded_phase`, `commit_docs`.

```bash
UI_AUDITOR_MODEL=$(forge_run query resolve-model forge-ui-auditor --raw)
```
...
**If `SUMMARY_FILES` empty:** Exit — "Phase {N} not executed. Run /forge:execute-phase {N} first."
...
```markdown
Read ~/.claude/agents/forge-ui-auditor.md for instructions.
...
Agent(
  ...
  subagent_type="forge-ui-auditor",
...
- `/forge:verify-work {N}` — UAT testing
- `/forge:plan-phase {N+1}` — plan next phase
...
```bash
forge_run query commit "docs(${padded_phase}): UI audit review" --files "${PHASE_DIR}/${PADDED_PHASE}-UI-REVIEW.md"
```
...
- [ ] forge-ui-auditor spawned with correct context
