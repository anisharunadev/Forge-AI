...
INIT=$(forge_run query init.todos)
...
- `/forge:capture --list` → show all
- `/forge:capture --list api` → filter to area:api only
</step>
...
- `/forge:capture --list [area]` to filter by area
- `q` to exit
```
...
forge_run query commit "docs: start work on todo - [title]" --files .planning/todos/completed/[filename] .planning/STATE.md
```
