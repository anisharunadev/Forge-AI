...
<required_reading>
@~/.claude/forge-core/references/continuation-format.md
</required_reading>
...
INIT=$(forge_run query init.resume)
...
- This is the primary resumption source — structured data from `/forge:pause-work`
...
📋 [N] pending todos — /forge:capture --list to review
...
1. Resume interrupted agent [if interrupted agent found]
   OR
1. Execute phase (/forge:execute-phase {phase} ${GSD_WS})
   OR
1. Discuss Phase 3 context (/forge:discuss-phase 3 ${GSD_WS}) [if CONTEXT.md missing]
   OR
1. Plan Phase 3 (/forge:plan-phase 3 ${GSD_WS}) [if CONTEXT.md exists or discuss option declined]
...
- **Plan phase** → Show direct next command:
  ...
  **Also available:**
  - `/forge:discuss-phase [N] ${GSD_WS}` — gather context first
  - `/forge:plan-phase --research-phase [N] ${GSD_WS}` — investigate unknowns
