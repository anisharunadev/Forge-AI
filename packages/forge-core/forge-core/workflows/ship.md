...
- forge-mempalace-curator — Ship-time MemPalace curation (diary, KG mirror, cross-project tunnels, wing-scoped prune); dispatched at ship:post when the mempalace capability is enabled.
</available_agent_types>
...
```bash
CONFIG=$(forge_run query state.load)
```
...
```bash
BASE_BRANCH=$(forge_run query git.base-branch)
```
</step>
...
6. **Security ship gate (capability-driven).**
   ...
   ```bash
   SHIP_PRE_HOOKS_JSON=$(forge_run loop render-hooks ship:pre --raw)
   ...
   - **`SECURITY_FILE` is empty** → block with `SECURITY_SHIP_GATE_NO_REVIEW`:
     ...
     Run /forge:secure-phase {phase} and resolve findings before shipping.
     ```
   - **`SECURITY_FILE` exists** → read its frontmatter `threats_open`. The gate passes **only** when `threats_open` is exactly `0`. For any other value — `threats_open` > 0, or a missing / non-numeric / unparsable field — **fail closed and block** with `SECURITY_SHIP_GATE_OPEN_THREATS` (the predicate is strict equality to `0`; never ship on an ambiguous value):
     ...
     Resolve open threats (or re-run /forge:secure-phase {phase}) before shipping.
     ```
...
```bash
CUSTOM_PR_SECTIONS=$(forge_run query config-get ship.pr_body_sections --default '[]' 2>/dev/null || echo '[]')
```
...
Use these sections for lean/agile PRD material that should travel with the PR without making the core `/forge:ship` body configurable:
...
```bash
PR_BODY_FILE=$(mktemp "${TMPDIR:-/tmp}/forge-pr-body.XXXXXX.md")
...
```bash
REVIEW_CMD=$(forge_run query config-get workflow.code_review_command 2>/dev/null | jq -r '.' 2>/dev/null || echo "")
```
...
2. **Load phase context from STATE.md:**
   ```bash
   STATE_STATUS=$(forge_run query state.load 2>/dev/null | head -20)
   ```
...
3. **Build review prompt and pipe to command via stdin:**
   ...
   REVIEW_OUTPUT=$(echo "${REVIEW_PROMPT}" | timeout 120 ${REVIEW_CMD} 2>/tmp/forge-review-stderr.log)
   REVIEW_EXIT=$?
   ```
...
4. **Handle timeout (120s) and failure:**
   ...
   if [ $REVIEW_EXIT -ne 0 ]; then
     REVIEW_STDERR=$(cat /tmp/forge-review-stderr.log 2>/dev/null)
...
```bash
forge_run query state.update "Last Activity" "$(date +%Y-%m-%d)"
forge_run query state.update "Status" "Phase ${PHASE_NUMBER} shipped — PR #${PR_NUMBER}"
```

If `commit_docs` is true:
```bash
forge_run query commit "docs(${padded_phase}): ship phase ${PHASE_NUMBER} — PR #${PR_NUMBER}" --files .planning/STATE.md
```
</step>
...
```bash
SHIP_POST_HOOKS_JSON=$(forge_run loop render-hooks ship:post --raw)
```
...
- If `ref.agent` is set, first show the spawn banner, then dispatch the agent named by `ref.agent` (use the exact `ref.agent` value as the subagent type — e.g. `forge-mempalace-curator` — never `general-purpose`):
...
- If `ref.skill` is set, dispatch with `Skill(skill="gsd-${ref.skill}", args="${PHASE_NUMBER} --auto ${GSD_WS}")` (prepend `forge-` to `ref.skill`).
...
- /forge:complete-milestone (if last phase in milestone)
- /forge:progress (to see what's next)
...
- /forge:complete-milestone — if all phases in milestone are done
- /forge:progress — see overall project state
- /forge:execute-phase {next} — continue to next phase
</offer_next>
