...
Use after /forge:execute-phase to verify that the evaluation strategy from AI-SPEC.md was actually implemented. Mirrors the pattern of /forge:ui-review and /forge:validate-phase.
</purpose>
...
```bash
AUDITOR_MODEL=$(forge_run query resolve-model forge-eval-auditor 2>/dev/null | jq -r '.model' 2>/dev/null || true)
AGENT_SKILLS_AUDITOR=$(forge_run query agent-skills forge-eval-auditor)
```
...
**State C** — No SUMMARY.md: Exit — "Phase {N} not executed. Run /forge:execute-phase {N} first."
...
Consider running /forge:ai-integration-phase {N} before implementation next time.
```
...
```markdown
Read ~/.claude/agents/forge-eval-auditor.md for instructions.
...
{If PRODUCTION READY:}
  Next step: /forge:plan-phase (next phase) or deploy

{If NEEDS WORK:}
  Address critical gaps in EVAL-REVIEW.md, then re-run /forge:eval-review {N}
...
- [ ] forge-eval-auditor spawned with correct context
