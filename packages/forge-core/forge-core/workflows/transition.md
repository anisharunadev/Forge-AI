...
There is no `/forge-transition` command. This workflow is invoked automatically by
`execute-phase` during auto-advance, or inline by the orchestrator after phase
verification. Users should never be told to run `/forge-transition`.

**Valid user commands for phase progression:**
- `/forge:discuss-phase {N}` — discuss a phase before planning
- `/forge:plan-phase {N}` — plan a phase
- `/forge:execute-phase {N}` — execute a phase
- `/forge:progress` — see roadmap progress
...
These will carry forward as debt. Review: `/forge:audit-uat`
```
...
**Delegate ROADMAP.md and STATE.md updates to `forge-tools.cjs query phase.complete`:**
...
TRANSITION=$(forge_run query phase.complete "${current_phase}")
```
...
```text
@~/.claude/forge-core/workflows/graduation.md
```
...
**Note:** Basic position updates (Current Phase, Status, Current Plan, Last Activity) were already handled by `forge-tools.cjs query phase.complete` in the update_roadmap_and_state step.

Verify the updates are correct by reading STATE.md. If the progress bar needs updating, use:

```bash
PROGRESS=$(forge_run query progress.bar --raw)
```
...
**Use the transition result from `forge-tools.cjs query phase.complete`:**
...
if [ -n "$GSD_WORKSTREAM" ]; then
  WS_LIST=$(forge_run query workstream.list --raw)
fi
```
...
Exit skill and invoke SlashCommand("/forge:plan-phase [X+1] --auto ${GSD_WS}")
...
Exit skill and invoke SlashCommand("/forge:discuss-phase [X+1] --auto ${GSD_WS}")
...
`/forge:discuss-phase [X+1] ${GSD_WS}` — gather context and clarify approach

---

**Also available:**
- `/forge:plan-phase [X+1] ${GSD_WS}` — skip discussion, plan directly
- `/forge:plan-phase --research-phase [X+1] ${GSD_WS}` — investigate unknowns
...
`/forge:plan-phase [X+1] ${GSD_WS}`

---

**Also available:**
- `/forge:discuss-phase [X+1] ${GSD_WS}` — revisit context
- `/forge:plan-phase --research-phase [X+1] ${GSD_WS}` — investigate unknowns
...
```bash
forge_run query config-set workflow._auto_chain_active false
```
...
`/forge:workstreams complete {current_ws_name} ${GSD_WS}`

See overall milestone progress:

`/forge:workstreams progress ${GSD_WS}`
...
Do NOT suggest `/forge:complete-milestone` or `/forge:new-milestone`.
...
Exit skill and invoke SlashCommand("/forge:complete-milestone {version} ${GSD_WS}")
...
`/forge:complete-milestone {version} ${GSD_WS}`
