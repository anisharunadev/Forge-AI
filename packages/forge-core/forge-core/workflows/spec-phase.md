...
INIT=$(forge_run init phase-op "${PHASE}")
...
If "Skip": Exit with message: "Existing SPEC.md unchanged. Run /forge:discuss-phase [X] to continue."
...
ones). Reference: @~/.claude/forge-core/references/edge-probe.md.
...
EDGE_PROBE_JS=$(for _c in \
  "$_GSD_RT/forge-core/bin/lib/edge-probe.cjs" \
  "$_GSD_RT/bin/lib/edge-probe.cjs" \
  "$_GSD_RT/.claude/bin/lib/edge-probe.cjs" \
  "$HOME/.claude/forge-core/bin/lib/edge-probe.cjs" \
...
requirement list). Reference: @~/.claude/forge-core/references/prohibition-probe.md — the
...
3. **Canon-referral (ADR-550 D6, PROB-13).** A kept candidate that is canon security/compliance
   ...
   /forge:secure-phase + eslint; not minted here"*) and DROP it. Minting canon items duplicates
   /forge:secure-phase and drowns the bespoke signal.
...
Use the SPEC.md template from @~/.claude/forge-core/templates/spec.md.
...
- User directed to /forge:discuss-phase as next step
