...
Companion to `/forge:sketch --wrap-up`.
...
@~/.claude/forge-core/references/sketch-theme-system.md
@~/.claude/forge-core/references/sketch-variant-patterns.md
@~/.claude/forge-core/references/sketch-interactivity.md
@~/.claude/forge-core/references/sketch-tooling.md
</required_reading>
...
```bash
forge_run query commit "docs(sketch-NNN): [winning direction] — [key visual insight]" --files .planning/sketches/NNN-descriptive-name/ .planning/sketches/MANIFEST.md
```
...
**Also available:**
- `/forge:sketch` — sketch more (or run with no argument for frontier mode)
- `/forge:plan-phase` — start building the real UI
- `/forge:spike` — spike technical feasibility of a design pattern
