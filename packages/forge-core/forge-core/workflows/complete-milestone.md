...
[R] Resolve — stop and fix items, then re-run /forge:complete-milestone
[A] Acknowledge all — document as deferred and proceed with close
[C] Cancel — exit without closing
```

If user chooses [A] (Acknowledge):
1. Re-run `forge-tools.cjs query audit-open --json` to get structured data
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  [ -e "$summary" ] || continue
  forge_run query summary-extract "$summary" --fields one_liner --pick one_liner
done
```
...
**Note:** MILESTONES.md entry is now created automatically by `forge-tools.cjs query milestone.complete` in the archive_milestone step. The entry includes version, date, phase/plan/task counts, and accomplishments extracted from SUMMARY.md files.
...
**Delegate archival to `forge-tools.cjs query milestone.complete`:**

```bash
ARCHIVE=$(forge_run query milestone.complete "v[X.Y]" --name "[Milestone Name]")
```
...
```bash
forge_run query commit "chore: archive v[X.Y] milestone files" --files .planning/milestones/v[X.Y]-ROADMAP.md .planning/milestones/v[X.Y]-REQUIREMENTS.md .planning/milestones/v[X.Y]-MILESTONE-AUDIT.md .planning/MILESTONES.md .planning/PROJECT.md .planning/STATE.md .planning/ROADMAP.md
```
...
**If doesn't exist:** Create from template at `~/.claude/forge-core/templates/retrospective.md`.
...
**Commit:**
```bash
forge_run query commit "docs: update retrospective for v${VERSION}" --files .planning/RETROSPECTIVE.md
```
...
```bash
INIT=$(forge_run query init.execute-phase "1")
...
<config-check>
Read `git.create_tag` via `forge-tools.cjs query config-get git.create_tag 2>/dev/null || echo "true"`.
