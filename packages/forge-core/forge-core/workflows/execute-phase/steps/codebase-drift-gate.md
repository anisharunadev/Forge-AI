...
# Resolve forge-tools through the runtime shim launcher, NOT the bare PATH binary. On a
# shim-only install (forge-tools.cjs present, `forge-tools` not on PATH) the bare call exits
...
Run /forge:map-codebase --paths {affected_paths} to refresh planning context.
```
...
from step `init_context` is for `forge-executor`, not the mapper):

```bash
# forge_run is defined by the canonical preamble in the drift-check block above and reused
...
Then spawn `forge-codebase-mapper` agents with the `--paths` hint (runs in a subagent — no output until it returns, ~1–5 min; expected, not a freeze):
