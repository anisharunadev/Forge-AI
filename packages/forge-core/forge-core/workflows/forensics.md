...
3. {Recovery command if applicable — e.g., `/forge:resume-work`, `/forge:execute-phase N`}

---

*Report generated by `/forge:forensics`. All paths redacted for portability.*
```
...
BUG_LABEL=$(gh label list --repo forge-ai/forge-core --search "bug" --json name -q '.[0].name' 2>/dev/null)
LABEL_FLAG=""
if [ -n "$BUG_LABEL" ]; then
