...
post-execute codebase-drift gate in `/forge:execute-phase` or by a user running
`/forge:map-codebase --paths apps/accounting,packages/ui`), the workflow
operates in **incremental-remap mode**:

- Pass `--paths <p1>,<p2>,...` through to each spawned `forge-codebase-mapper`
...
INIT=$(forge_run query init.map-codebase)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_MAPPER=$(forge_run query agent-skills forge-codebase-mapper)
```
...
Use Agent tool with `subagent_type="forge-codebase-mapper"`, `model="{mapper_model}"`, and `run_in_background=true` for parallel execution.

**CRITICAL:** Use the dedicated `forge-codebase-mapper` agent, NOT `Explore` or `browser_subagent`. The mapper agent writes documents directly.
...
Use the same document templates as the `forge-codebase-mapper` agent. Include actual file paths formatted with backticks.
...
```bash
forge_run query commit "docs: map existing codebase" --files .planning/codebase/*.md
```
...
- If Agent tool available: 4 parallel forge-codebase-mapper agents spawned with run_in_background=true
