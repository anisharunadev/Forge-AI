<purpose>
Execute small, ad-hoc tasks with GSD guarantees (atomic commits, STATE.md tracking). Quick mode spawns forge-planner (quick mode) + forge-executor(s), tracks tasks in `.planning/quick/`, and updates STATE.md's "Quick Tasks Completed" table.
...
- forge-code-reviewer — Reviews source files for bugs, security issues, and code quality
</available_agent_types>
...
INIT=$(forge_run query init.quick "$DESCRIPTION")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_PLANNER=$(forge_run query agent-skills forge-planner)
AGENT_SKILLS_EXECUTOR=$(forge_run query agent-skills forge-executor)
AGENT_SKILLS_CHECKER=$(forge_run query agent-skills forge-plan-checker)
AGENT_SKILLS_VERIFIER=$(forge_run query agent-skills forge-verifier)
```
...
if [ "$USE_WORKTREES" != "false" ]; then
  forge_run query worktree.reap-orphans 2>/dev/null || true
fi
```
...
if [ "${USE_WORKTREES}" != "false" ]; then
  ...
  COMMIT_DOCS=$(forge_run query config-get commit_docs 2>/dev/null || echo "true")
...
if [ "${USE_WORKTREES:-true}" != "false" ]; then
  QUICK_WORKTREE_MANIFEST=$(mktemp "${TMPDIR:-/tmp}/forge-quick-worktree-XXXXXX.json")
...
ORCHESTRATOR build-time embed (NOT a sub-agent runtime step): before this dispatch, read \`forge-core/references/worktree-branch-check.md\`, substitute \`{EXPECTED_BASE}\` with the base SHA captured above (${EXPECTED_BASE}), substitute \`{EXPECTED_BASE_ALTERNATE}\` with \`${QUICK_PLAN_PARENT}\` when it differs from \`${EXPECTED_BASE}\` (otherwise empty), and replace this note with that fragment's \`<worktree_branch_check>\` block so the dispatched prompt carries the runnable guard verbatim — do not pass this instruction through in its place.
</worktree_branch_check>
...
1. **Worktree cleanup:** If the executor ran with `isolation="worktree"`, merge the worktree branch back and clean up:
   ...
   forge_run query worktree.cleanup-wave --manifest "$QUICK_WORKTREE_MANIFEST" || exit 1
   ```
...
Stage and commit quick task artifacts. This step MUST always run — even if the executor already committed some files (e.g. when running without worktree isolation). The `forge-tools.cjs query commit` command (or legacy `forge-tools.cjs` commit) handles already-committed files gracefully.
...
forge_run query commit "docs(quick-${quick_id}): ${DESCRIPTION}" --files ${file_list}
```
...
Ready for next task: /forge:quick ${GSD_WS}
```
