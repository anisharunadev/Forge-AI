...
- **Claude Code:** Uses `Agent(subagent_type="forge-executor", ...)` — blocks until complete, returns result
...
- **Other runtimes:** If `Agent`/`agent` tool is genuinely unavailable (e.g. a backgrounded
  ...
  Code), you MUST spawn forge-executor agents — inline execution is not authorized. Check for
...
@~/.claude/forge-core/references/agent-contracts.md
@~/.claude/forge-core/references/context-budget.md
@~/.claude/forge-core/references/gates.md
</required_reading>
...
- forge-executor — Executes plan tasks, commits, creates SUMMARY.md
- forge-verifier — Verifies phase completion, checks quality gates
- forge-planner — Creates detailed plans from phase scope
- forge-phase-researcher — Researches technical approaches for a phase
- forge-plan-checker — Reviews plan quality before execution
- forge-debugger — Diagnoses and fixes issues
- forge-codebase-mapper — Maps project structure and dependencies
- forge-integration-checker — Checks cross-phase integration
- forge-nyquist-auditor — Validates verification coverage
- forge-ui-researcher — Researches UI/UX approaches
- forge-ui-checker — Reviews UI implementation quality
- forge-ui-auditor — Audits UI against design requirements
</available_agent_types>
...
INIT=$(forge_run query init.execute-phase "${PHASE_ARG}")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS=$(forge_run query agent-skills forge-executor)
```
...
```bash
RUNTIME=$(forge_run query config-get runtime --default claude --raw 2>/dev/null || echo "claude")
USE_WORKTREES=$(forge_run query config-get workflow.use_worktrees --raw 2>/dev/null || echo "true")
EXECUTOR_STALL_INTERVAL_MINUTES=$(forge_run query config-get executor.stall_detect_interval_minutes 2>/dev/null || echo "5")
EXECUTOR_STALL_THRESHOLD_MINUTES=$(forge_run query config-get executor.stall_threshold_minutes 2>/dev/null || echo "10")
...
[ "$USE_WORKTREES" != "false" ] && forge_run query worktree.reap-orphans 2>/dev/null || true
...
if [ "$RUNTIME" = "claude" ] && [ "$USE_WORKTREES" != "false" ]; then
  _SHOULD_DEGRADE=$(forge_run query worktree.base-check --pick shouldDegrade 2>/dev/null || true)
  if [ "$_SHOULD_DEGRADE" = "true" ]; then
    _DEGRADE_MSG=$(forge_run query worktree.base-check --pick message 2>/dev/null || true)
...
- Executor agents omit extended deviation rule examples and checkpoint examples from inline prompt — load on-demand via @~/.claude/forge-core/references/executor-examples.md
- Planner agents omit extended anti-pattern lists and specificity examples from inline prompt — load on-demand via @~/.claude/forge-core/references/planner-antipatterns.md
...
Check if the current runtime is Copilot by testing for the `@forge-executor` agent pattern
...
if [[ ! "$ARGUMENTS" =~ --auto ]]; then
  forge_run query config-set workflow._auto_chain_active false || true
fi
```
...
MVP_MODE=$(forge_run query phase.mvp-mode "${PHASE_NUMBER}" $MVP_FLAG_ARG --pick active)
EXECUTE_POST_HOOKS_JSON=$(forge_run loop render-hooks execute:post --raw)
TDD_MODE=$(forge_run loop render-hooks execute:post --active-cap tdd)
```
...
if [ "$MVP_MODE" = "true" ] && [ "$TDD_MODE" = "true" ]; then
  IS_BEHAVIOR_ADDING=$(forge_run query task.is-behavior-adding "$TASK_FILE" --pick is_behavior_adding)
  if [ "$IS_BEHAVIOR_ADDING" = "true" ]; then
    ...
    if [ -z "$RED_COMMIT" ]; then
      forge_run query state.update last_gate_trip "${PLAN_ID}/${TASK_ID}" || true
...
```
Pure doc-only / config-only / test-only tasks return `is_behavior_adding=false` and are exempt. When the gate trips, Read `~/.claude/forge-core/references/execute-mvp-tdd.md` for the exact halt report format.
</step>
...
2. For each plan (sequentially, ignoring wave grouping):
   ...
   c. **If "Execute":** Read and follow `~/.claude/forge-core/workflows/execute-plan.md` **inline**
...
```bash
DEFAULT_BRANCH=$(forge_run query git.base-branch 2>/dev/null \
...
```bash
forge_run query state.begin-phase --phase "${PHASE_NUMBER}" --name "${PHASE_NAME}" --plans "${PLAN_COUNT}"
```
...
```bash
PLAN_INDEX=$(forge_run query phase-plan-index "${PHASE_NUMBER}")
```
...
```bash
CROSS_AI_ENABLED=$(forge_run query config-get workflow.cross_ai_execution 2>/dev/null || echo "false")
CROSS_AI_CMD=$(forge_run query config-get workflow.cross_ai_command 2>/dev/null || echo "")
CROSS_AI_TIMEOUT=$(forge_run query config-get workflow.cross_ai_timeout 2>/dev/null || echo "300")
```
...
`workflow.cross_ai_command` via `forge-tools.cjs query config-set workflow.cross_ai_command "<command>"`.
...
tooling and `/forge:manager`'s background-completion handler can grep partial
...
@~/.claude/forge-core/references/execute-phase-wave-guard.md

@~/.claude/forge-core/references/execute-phase-context-guard.md
...
3. **Spawn executor agents:**
   ...
   **Worktree mode** (`USE_WORKTREES_FOR_PLAN` is not `false` — evaluated per-plan in step 2.5):
   ...
   if [ "${USE_WORKTREES_FOR_PLAN:-true}" != "false" ] && [ -z "${WAVE_WORKTREE_MANIFEST:-}" ]; then
     WAVE_WORKTREE_MANIFEST=$(mktemp "${TMPDIR:-/tmp}/forge-worktree-wave-XXXXXX.json")
   ...
   **Sequential dispatch for parallel execution (waves with 2+ agents):**
   ...
   Agent(
     ...
     prompt="
       ...
       ORCHESTRATOR build-time embed (NOT a sub-agent runtime step): before this dispatch, read `forge-core/references/worktree-branch-check.md`, substitute `{EXPECTED_BASE}` with the base SHA captured above ({EXPECTED_BASE}), and replace this note with that fragment's `<worktree_branch_check>` block so the dispatched prompt carries the runnable guard verbatim — do not pass this instruction through in its place.
       Per-commit HEAD/cwd-drift/path-guard: `agents/forge-executor.md` steps 0/0a/0b + `references/worktree-path-safety.md` (in <execution_context>).
       </worktree_branch_check>
       ...
       <execution_context>
       @~/.claude/forge-core/workflows/execute-plan.md
       @~/.claude/forge-core/templates/summary.md
       @~/.claude/forge-core/references/checkpoints.md
       @~/.claude/forge-core/references/tdd.md
       @~/.claude/forge-core/references/worktree-path-safety.md
       ${CONTEXT_WINDOW < 200000 ? '' : '@~/.claude/forge-core/references/executor-examples.md'}
       </execution_context>
...
5. **Post-wave hook validation (parallel mode only):** Hooks run on every executor commit by default (#2924); this post-wave run only fires when `workflow.worktree_skip_hooks=true` opted out of per-commit hooks:
   ```bash
   SKIP_HOOKS=$(forge_run query config-get workflow.worktree_skip_hooks 2>/dev/null || echo "false")
   if [ "$SKIP_HOOKS" = "true" ]; then
     # Stash uncommitted changes under a named ref so we always pop (bare `git stash` strands them on hook/script failure). #3542: `refs/stash` is shared across worktrees, so this helper runs ONLY in the orchestrator's main checkout after all wave worktrees have been merged + removed; executors are forbidden from running any `git stash` subcommand (see `<destructive_git_prohibition>` in `agents/forge-executor.md`).
     STASHED=false
     ...
     [ "$STASHED" = "true" ] && (git stash pop >/dev/null 2>&1 || echo "⚠ Could not pop forge-post-wave-hook stash — recover manually")
   fi
   ```
...
5.5. **Worktree cleanup (when `isolation="worktree"` was used):**
   ...
   **Manifest source of truth (#3384):** Cleanup consumes the `WAVE_WORKTREE_MANIFEST` created and populated during executor dispatch in step 3. Do not recreate or truncate it here.
   ...
   forge_run query worktree.cleanup-wave --manifest "$WAVE_WORKTREE_MANIFEST" || exit 1
   ```
   ...
   **Cleanup-tail snippet (use after any wave whose merges did not flow through the templated path above):**
   ...
   WT_PATHS_FILE=$(mktemp "${TMPDIR:-/tmp}/forge-worktree-paths-XXXXXX")
...
5.6. **Post-merge build & test gate:**
   ...
   Read and execute `forge-core/workflows/execute-phase/steps/post-merge-gate.md`.
   ...
   **Only update tracking when tests passed (TEST_EXIT=0).**
   ...
   if [ "${TEST_EXIT}" -eq 0 ]; then
     ...
     for plan_id in {completed_plan_ids}; do
       forge_run query roadmap.update-plan-progress "${PHASE_NUMBER}" "${plan_id}" "complete"
     done
     ...
     if ! git diff --quiet .planning/ROADMAP.md .planning/STATE.md 2>/dev/null; then
       forge_run query commit "docs(phase-${PHASE_NUMBER}): update tracking after wave ${N}" --files .planning/ROADMAP.md .planning/STATE.md
     fi
...
5.75. **Execute:wave:post capability dispatch:**
   ...
   ```bash
   WAVE_POST_HOOKS_JSON=$(forge_run loop render-hooks execute:wave:post --raw)
   ```
   ...
   **For each active entry where `kind == "gate"`** (process in array order), run the gate check:
   ...
   ```bash
   GATE_RESULT=$(forge_run check ${hook.check.query} "${PHASE_NUMBER}" --raw)
   CHECK_EXIT=$?
   ```
   ...
   **Step 2 — read `GATE_RESULT.block` (boolean).** This step is only reached when the command succeeded.
   ...
   - **Non-blocking gate (`hook.blocking == false`):** never halts. If `GATE_RESULT.block` is `true` (or non-empty `message`), print `⚠ {hook.capId} advisory (wave {N}): {GATE_RESULT.message}`, then:
     - If `GATE_RESULT.spawn_mapper == true` OR `GATE_RESULT.directive == "auto-remap"`: spawn `forge-codebase-mapper` per `execute-phase/steps/codebase-drift-gate.md`; pass `--paths {GATE_RESULT.affected_paths}`. Continue regardless (wave NOT failed by remap failure).
...
7. **Handle failures:**
   **Step 7.0 — classify before branching (#3095):**
   ```bash
   CLASS_JSON=$(forge_run query agent.classify-failure -- "$AGENT_RETURN_BODY")
   ...
   **Step 7.1 — `class == "quota-exceeded"`:**
   ...
   ```
   Re-run `/forge:execute-phase` after quota reset for Option 1.
...
@~/.claude/forge-core/references/execute-phase-between-wave-reset.md
...
```bash
AUTO_MODE=$(forge_run query check auto-mode --pick active 2>/dev/null || echo "false")
```
...
**Security gate check:**
```bash
VERIFY_POST_HOOKS_JSON=$(forge_run loop render-hooks verify:post --raw)
...
```bash
POST_PLAN_INDEX=$(forge_run query phase-plan-index "${PHASE_NUMBER}")
```
...
/forge:execute-phase {phase} ${GSD_WS}                # Continue remaining waves
/forge:execute-phase {phase} --wave {next} ${GSD_WS}  # Run the next wave explicitly
```
...
**Capability gate:**
```bash
EXECUTE_POST_HOOKS_JSON=${EXECUTE_POST_HOOKS_JSON:-$(forge_run loop render-hooks execute:post --raw)}
```
...
/forge:code-review ${PHASE_NUMBER} --fix
```
...
**TDD review escalation (overrides the advisory default for the `tdd.review-checkpoint` gate only).** The tdd `execute:post` gate is declared `blocking: false`, so by the generic contract above it displays its `message`/table and continues. There is ONE documented exception (see `~/.claude/forge-core/references/execute-mvp-tdd.md`): when `MVP_MODE=true` AND `TDD_MODE=true` AND `GATE_RESULT.block == true` (one or more TDD plans miss a RED or GREEN gate commit), the end-of-phase TDD review escalates from advisory to **blocking under MVP+TDD** — refuse to mark the phase complete and present:

```
Phase blocked: {N} TDD plan(s) violate the RED→GREEN gate sequence under MVP+TDD.
Resolve and re-run /forge execute-phase, or override with /forge execute-phase {phase} --force-mvp-gate to ship anyway.
```
...
```bash
PARENT_INFO=$(forge_run query find-phase "${PARENT_PHASE}" --raw)
...
```bash
forge_run query commit "docs(phase-${PARENT_PHASE}): resolve UAT gaps and debug sessions after ${PHASE_NUMBER} gap closure" --files .planning/phases/*${PARENT_PHASE}*/*-UAT.md .planning/debug/resolved/*.md
```
</step>
...
REG_TEST_CMD=$(forge_run query config-get workflow.test_command --default "" 2>/dev/null || true)
...
```bash
VERIFIER_SKILLS=$(forge_run query agent-skills forge-verifier)
```
...
```bash
VERIFICATION=$(forge_run query verification.status "$PHASE_DIR" 2>/dev/null)
...
Commit the file:
```bash
forge_run query commit "test({phase_num}): persist human verification items as UAT" --files "{phase_dir}/{phase_num}-UAT.md"
```
...
**If user acknowledges without reporting issues (including "ok", "noted", "ack", "got it", "approved", "done", "yes", "pass", or similar):** Stop. The phase remains pending. No further orchestrator action — wait for the user to run `/forge:verify-work`.
...
`/forge:plan-phase {X} --gaps ${GSD_WS}`

Also: `cat {phase_dir}/{phase_num}-VERIFICATION.md` — full report
Also: `/forge:verify-work {X} ${GSD_WS}` — manual testing first
```

Gap closure cycle: `/forge:plan-phase {X} --gaps ${GSD_WS}` reads VERIFICATION.md → creates gap plans with `gap_closure: true` → user runs `/forge:execute-phase {X} --gaps-only ${GSD_WS}` → verifier re-runs.
</step>
...
```bash
COMPLETION=$(forge_run query phase.complete "${PHASE_NUMBER}")
```
...
These items are tracked and will appear in `/forge:progress` and `/forge:audit-uat`.
```

```bash
forge_run query commit "docs(phase-{X}): complete phase execution" --files .planning/ROADMAP.md .planning/STATE.md .planning/REQUIREMENTS.md {phase_dir}/*-VERIFICATION.md
```
</step>
...
entries from the completed phase to the global learnings store at `~/.forge/knowledge/`.

**Check config gate:**
```bash
GL_ENABLED=$(forge_run query config-get features.global_learnings --raw 2>/dev/null || echo "false")
```
...
```bash
forge_run query learnings.copy 2>/dev/null || echo "⚠ Learnings copy failed — continuing"
```
...
if [ ${#CLOSED[@]} -gt 0 ]; then
  forge_run query commit "docs(phase-${PHASE_NUMBER}): auto-close ${#CLOSED[@]} todo(s) resolved by this phase" --files .planning/todos/completed/ .planning/STATE.md|| true
...
```bash
forge_run query commit "docs(phase-{X}): evolve PROJECT.md after phase completion" --files .planning/PROJECT.md
```
...
Read and follow `~/.claude/forge-core/workflows/transition.md`, passing through the `--auto` flag so it propagates to the next phase invocation.

**If neither `--auto` nor `AUTO_MODE` is true:**

**STOP. Do not auto-advance. Do not execute transition. Do not plan next phase. Present options to the user and wait.**

**IMPORTANT: There is NO `/forge-transition` command. Never suggest it. The transition workflow is internal only.**
...
/forge:progress ${GSD_WS} — see updated roadmap
/forge:discuss-phase {next} ${GSD_WS} — start here: discuss next phase before planning  ← recommended
/forge:plan-phase {next} ${GSD_WS} — plan next phase (skip discuss)
/forge:execute-phase {next} ${GSD_WS} — execute next phase (skip discuss and plan)
```
...
/forge:progress ${GSD_WS} — see updated roadmap
/forge:plan-phase {next} ${GSD_WS} — start here: plan next phase (CONTEXT.md already present)  ← recommended
/forge:discuss-phase {next} ${GSD_WS} — re-discuss next phase
/forge:execute-phase {next} ${GSD_WS} — execute next phase (skip planning)
```
...
<failure_handling>
- **Quota / rate-limit (any runtime — #3095):** Agent return body contains a sentinel like `usage limit`, `rate limit`, `429`, `too many requests`, `RESOURCE_EXHAUSTED`, `usage_limit_reached`. Route via `forge-tools.cjs query agent.classify-failure` → `class: "quota-exceeded"`. Do not offer retry-now; the right action is wait-for-reset and resume.
