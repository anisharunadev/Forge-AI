...
Invoked by `/forge:update --reapply` (`commands/forge/update.md`).
...
**Critical invariant:** Every file in `forge-local-patches/` was backed up because the installer's hash comparison detected it was modified. The workflow must NEVER conclude "no custom content" for any backed-up file — that is a logical contradiction. When in doubt, classify as CONFLICT requiring user review, not SKIP.
...
if [ -n "$KILO_CONFIG_DIR" ]; then
  ...
  candidate="$(dirname "$(expand_home "$KILO_CONFIG")")/forge-local-patches"
  if [ -d "$candidate" ]; then
    PATCHES_DIR="$candidate"
  fi
elif [ -n "$XDG_CONFIG_HOME" ]; then
  candidate="$(expand_home "$XDG_CONFIG_HOME")/kilo/forge-local-patches"
  if [ -d "$candidate" ]; then
    PATCHES_DIR="$candidate"
  fi
fi
...
if [ -z "$PATCHES_DIR" ] && [ -n "$OPENCODE_CONFIG_DIR" ]; then
  candidate="$(expand_home "$OPENCODE_CONFIG_DIR")/forge-local-patches"
  if [ -d "$candidate" ]; then
    PATCHES_DIR="$candidate"
  fi
elif [ -z "$PATCHES_DIR" ] && [ -n "$OPENCODE_CONFIG" ]; then
  candidate="$(dirname "$(expand_home "$OPENCODE_CONFIG")")/forge-local-patches"
  if [ -d "$candidate" ]; then
    PATCHES_DIR="$candidate"
  fi
elif [ -z "$PATCHES_DIR" ] && [ -n "$XDG_CONFIG_HOME" ]; then
  candidate="$(expand_home "$XDG_CONFIG_HOME")/opencode/forge-local-patches"
  if [ -d "$candidate" ]; then
    PATCHES_DIR="$candidate"
  fi
fi
...
if [ -z "$PATCHES_DIR" ] && [ -n "$GEMINI_CONFIG_DIR" ]; then
  candidate="$(expand_home "$GEMINI_CONFIG_DIR")/forge-local-patches"
  if [ -d "$candidate" ]; then
    PATCHES_DIR="$candidate"
  fi
fi
...
if [ -z "$PATCHES_DIR" ] && [ -n "$CODEX_HOME" ]; then
  candidate="$(expand_home "$CODEX_HOME")/forge-local-patches"
  if [ -d "$candidate" ]; then
    PATCHES_DIR="$candidate"
  fi
fi
...
if [ -z "$PATCHES_DIR" ] && [ -n "$CLAUDE_CONFIG_DIR" ]; then
  candidate="$(expand_home "$CLAUDE_CONFIG_DIR")/forge-local-patches"
  if [ -d "$candidate" ]; then
    PATCHES_DIR="$candidate"
  fi
fi
...
if [ -z "$PATCHES_DIR" ]; then
  if [ -d "$HOME/.config/kilo/forge-local-patches" ]; then
    PATCHES_DIR="$HOME/.config/kilo/forge-local-patches"
  elif [ -d "$HOME/.config/opencode/forge-local-patches" ]; then
    PATCHES_DIR="$HOME/.config/opencode/forge-local-patches"
  elif [ -d "$HOME/.opencode/forge-local-patches" ]; then
    PATCHES_DIR="$HOME/.opencode/forge-local-patches"
  elif [ -d "$HOME/.gemini/forge-local-patches" ]; then
    PATCHES_DIR="$HOME/.gemini/forge-local-patches"
  elif [ -d "$HOME/.codex/forge-local-patches" ]; then
    PATCHES_DIR="$HOME/.codex/forge-local-patches"
  else
    PATCHES_DIR="$HOME/.claude/forge-local-patches"
  fi
fi
# Local install fallback — check all runtime directories
if [ ! -d "$PATCHES_DIR" ]; then
  for dir in .config/kilo .kilo .config/opencode .opencode .gemini .codex .claude; do
    if [ -d "./$dir/forge-local-patches" ]; then
      PATCHES_DIR="./$dir/forge-local-patches"
      break
    fi
  done
fi
...
Local patches are automatically saved when you run /forge:update
...
- **User's version** (backed up in `forge-local-patches/`)
...
Check if a `forge-pristine/` directory exists alongside `forge-local-patches/`:
```bash
PRISTINE_DIR="$CONFIG_DIR/forge-pristine"
```
...
1. **Read the backed-up version** (user's modified copy from `forge-local-patches/`)
2. **Read the newly installed version** (current file after update)
3. **If available, read the pristine baseline** (from git history or `forge-pristine/`)
...
git -C "$CONFIG_DIR" log --oneline --no-merges -- "{file_path}" | grep -v "forge:update\|forge-update\|GSD update\|forge-install"
```
...
Run the verifier as a child process (the forge-tools binary directory is not required — the script ships under `forge-core/bin/` in the source repo and is installed to `${GSD_HOME}/forge-core/bin/`; it is also exposed via the SDK at `sdk/dist/cli.js verify-reapply` when present):
...
VERIFY_OUTPUT="$(node "${GSD_HOME}/forge-core/bin/verify-reapply-patches.cjs" "${VERIFY_ARGS[@]}")"
VERIFY_STATUS=$?
```
...
Resolve before re-running:
  ...
  (c) If the upstream changes are acceptable, update the backup-meta.json
      ...
      re-run /forge:update --reapply to re-verify with the refreshed baseline.

Then re-run /forge:update --reapply to re-verify.
```
...
/forge:update --reapply to retry with full verification.
```
...
Ask user:
- "Keep patch backups for reference?" → preserve `forge-local-patches/`
- "Clean up patch backups?" → remove `forge-local-patches/` directory
...
- [ ] Three-way merge used when pristine baseline available (git history or forge-pristine/)
