...
**Command:** `/forge-sync-skills`

Sync managed `forge-*` skill directories from one canonical runtime's skills root to one or more destination runtime skills roots. Keeps multi-runtime installs aligned after a `forge-update` on one runtime.
...
```bash
INSTALL_JS="$(dirname "$0")/../forge-core/bin/install.js"
# If running from a global install, resolve relative to the GSD package
INSTALL_JS_GLOBAL="$HOME/.claude/forge-core/bin/install.js"
...
error: source skills root not found: <path>
       ...
       Run: node ~/.claude/forge-core/bin/install.js --global --<runtime>
```
Then exit.
...
```bash
# List forge-* subdirectories in source
SRC_SKILLS=$(ls -1 "$SRC_SKILLS_ROOT" 2>/dev/null | grep '^gsd-')

# List forge-* subdirectories in destination (may not exist yet)
...
**Non-GSD preservation:** Only `forge-*` entries are ever created, updated, or removed. Entries in the destination that do not start with `forge-` are never touched.
...
CREATE: forge-help
UPDATE: forge-update
REMOVE: forge-old-command
SKIP:   forge-plan-phase (up to date)
(N changes)
...
1. **Only `forge-*` directories** are created, updated, or removed. Any directory not starting with `forge-` in a destination root is untouched.
2. **Dry-run is the default.** `--apply` must be passed explicitly to write anything.
3. **Source root must exist.** Never create the source root; it must have been created by a prior `forge-update` or installer run.
