...
TARGET=${1:-$(forge_run query git.base-branch)}
```
...
```bash
SUB_REPOS_JSON=$(forge_run query config-get planning.sub_repos 2>/dev/null)
...
RESULT=$(forge_run query pr-subrepo "$COMMIT_MSG" \
  --repo "$REPO_REL" \
  --branch "$SUB_BRANCH")
SUBREPO_EXIT=$?
```
...
Or use /forge:ship to create the PR automatically.
```
</step>
