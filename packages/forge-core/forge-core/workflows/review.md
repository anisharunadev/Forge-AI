...
OLLAMA_HOST=$(forge_run query config-get review.ollama_host 2>/dev/null | jq -r '.' 2>/dev/null || echo "")
...
LM_STUDIO_HOST=$(forge_run query config-get review.lm_studio_host 2>/dev/null | jq -r '.' 2>/dev/null || echo "")
...
LLAMA_CPP_HOST=$(forge_run query config-get review.llama_cpp_host 2>/dev/null | jq -r '.' 2>/dev/null || echo "")
...
- Value must be a non-empty array of slug strings (configured via `forge config-set review.default_reviewers '["gemini","codex"]'`)
...
```bash
INIT=$(forge_run query init.phase-op "${PHASE_ARG}")
...
Write to a temp file: `/tmp/forge-review-prompt-{phase}.md`
...
cp "$INSTRUCTIONS_BLOCK_FILE" "/tmp/forge-review-${PHASE}-instructions.md"
cp "$ROADMAP_SECTION_FILE" "/tmp/forge-review-${PHASE}-roadmap.md"

# Plan files: copy each PLAN.md to a predictable numbered path
PLAN_INDEX=0
for PLAN_FILE in "${PHASE_DIR}"/*-PLAN.md; do
  PADDED_IDX=$(printf '%02d' "$PLAN_INDEX")
  cp "$PLAN_FILE" "/tmp/forge-review-${PHASE}-plan-${PADDED_IDX}.md"
  PLAN_INDEX=$((PLAN_INDEX + 1))
done

# Optional section files (only if content was included in the combined prompt)
if [ -f ".planning/PROJECT.md" ]; then
  cp .planning/PROJECT.md "/tmp/forge-review-${PHASE}-project.md"
fi
if ls "${PHASE_DIR}/"*"-CONTEXT.md" >/dev/null 2>&1; then
  cat "${PHASE_DIR}/"*"-CONTEXT.md" > "/tmp/forge-review-${PHASE}-context.md"
fi
if ls "${PHASE_DIR}/"*"-RESEARCH.md" >/dev/null 2>&1; then
  cat "${PHASE_DIR}/"*"-RESEARCH.md" > "/tmp/forge-review-${PHASE}-research.md"
fi
if [ -f ".planning/REQUIREMENTS.md" ]; then
  cp .planning/REQUIREMENTS.md "/tmp/forge-review-${PHASE}-requirements.md"
fi
```
...
```bash
# JSON scalars from forge-tools.cjs query; use jq -r to strip JSON string quotes (install jq if missing)
GEMINI_MODEL=$(forge_run query config-get review.models.gemini 2>/dev/null | jq -r '.' 2>/dev/null || true)
CLAUDE_MODEL=$(forge_run query config-get review.models.claude 2>/dev/null | jq -r '.' 2>/dev/null || true)
CODEX_MODEL=$(forge_run query config-get review.models.codex 2>/dev/null | jq -r '.' 2>/dev/null || true)
OPENCODE_MODEL=$(forge_run query config-get review.models.opencode 2>/dev/null | jq -r '.' 2>/dev/null || true)
# review.models.agy is reserved for future model-pinning support; agy selects its model internally
AGY_MODEL=$(forge_run query config-get review.models.agy 2>/dev/null | jq -r '.' 2>/dev/null || true)
...
if [ -n "$GEMINI_MODEL" ] && [ "$GEMINI_MODEL" != "null" ]; then
  cat /tmp/forge-review-prompt-{phase}.md | gemini -m "$GEMINI_MODEL" -p - 2>/dev/null > /tmp/forge-review-gemini-{phase}.md
else
  cat /tmp/forge-review-prompt-{phase}.md | gemini -p - 2>/dev/null > /tmp/forge-review-gemini-{phase}.md
fi
```
...
if [ -n "$CLAUDE_MODEL" ] && [ "$CLAUDE_MODEL" != "null" ]; then
  cat /tmp/forge-review-prompt-{phase}.md | claude --model "$CLAUDE_MODEL" -p - 2>/dev/null > /tmp/forge-review-claude-{phase}.md
else
  cat /tmp/forge-review-prompt-{phase}.md | claude -p - 2>/dev/null > /tmp/forge-review-claude-{phase}.md
fi
```
...
if [ -n "$CODEX_MODEL" ] && [ "$CODEX_MODEL" != "null" ]; then
  cat /tmp/forge-review-prompt-{phase}.md | codex exec --ephemeral $CODEX_BYPASS_FLAG --model "$CODEX_MODEL" --skip-git-repo-check - 2>/tmp/forge-review-codex-{phase}.err > /tmp/forge-review-codex-{phase}.md
else
  cat /tmp/forge-review-prompt-{phase}.md | codex exec --ephemeral $CODEX_BYPASS_FLAG --skip-git-repo-check - 2>/tmp/forge-review-codex-{phase}.err > /tmp/forge-review-codex-{phase}.md
fi
if [ ! -s /tmp/forge-review-codex-{phase}.md ]; then
  echo "Codex review failed or returned empty output. stderr:" > /tmp/forge-review-codex-{phase}.md
  cat /tmp/forge-review-codex-{phase}.err >> /tmp/forge-review-codex-{phase}.md
fi
```
...
```bash
coderabbit review --prompt-only 2>/dev/null > /tmp/forge-review-coderabbit-{phase}.md
```

**OpenCode (via GitHub Copilot):**
```bash
if [ -n "$OPENCODE_MODEL" ] && [ "$OPENCODE_MODEL" != "null" ]; then
  cat /tmp/forge-review-prompt-{phase}.md | opencode run --model "$OPENCODE_MODEL" - 2>/dev/null > /tmp/forge-review-opencode-{phase}.md
else
  cat /tmp/forge-review-prompt-{phase}.md | opencode run - 2>/dev/null > /tmp/forge-review-opencode-{phase}.md
fi
if [ ! -s /tmp/forge-review-opencode-{phase}.md ]; then
  echo "OpenCode review failed or returned empty output." > /tmp/forge-review-opencode-{phase}.md
fi
```

**Qwen Code:**
```bash
cat /tmp/forge-review-prompt-{phase}.md | qwen - 2>/dev/null > /tmp/forge-review-qwen-{phase}.md
if [ ! -s /tmp/forge-review-qwen-{phase}.md ]; then
  echo "Qwen review failed or returned empty output." > /tmp/forge-review-qwen-{phase}.md
fi
```
...
CURSOR_PROMPT_ARG="Read the file at /tmp/forge-review-prompt-{phase}.md in full and carry out the review request it contains. Output only the resulting markdown review. Do not edit any files."
cursor-agent -p --mode ask --trust --output-format text "$CURSOR_PROMPT_ARG" 2>/tmp/forge-review-cursor-{phase}.err > /tmp/forge-review-cursor-{phase}.md
if [ ! -s /tmp/forge-review-cursor-{phase}.md ]; then
  echo "Cursor review failed or returned empty output. stderr:" > /tmp/forge-review-cursor-{phase}.md
  cat /tmp/forge-review-cursor-{phase}.err >> /tmp/forge-review-cursor-{phase}.md
fi
```
...
agy --print-timeout 300s -p "$(cat /tmp/forge-review-prompt-{phase}.md)" 2>/dev/null > /tmp/forge-review-antigravity-{phase}.md
_AGY_RC=$?
if [ "$_AGY_RC" -ne 0 ]; then
  : > /tmp/forge-review-antigravity-{phase}.md
fi
...
if [ ! -s /tmp/forge-review-antigravity-{phase}.md ]; then
  if [ -f "$_AGY_CACHE" ]; then
    ...
    if [ -n "$_AGY_CONV" ] && [ "$_AGY_CONV" != "null" ]; then
      ...
      if [ -f "$_AGY_TX" ]; then
        ...
        _AGY_RESULT=$(tail -n +"$((_AGY_SKIP + 1))" "$_AGY_TX" 2>/dev/null | \
          ...
          2>/dev/null | tail -1)
        [ -n "$_AGY_RESULT" ] && echo "$_AGY_RESULT" > /tmp/forge-review-antigravity-{phase}.md
      fi
    fi
  fi
fi
...
if [ ! -s /tmp/forge-review-antigravity-{phase}.md ]; then
  echo "Antigravity review failed or returned empty output." > /tmp/forge-review-antigravity-{phase}.md
fi
```
...
prepare_trimmed_prompt_for_reviewer() {
  ...
  PLAN_FILE_ARGS=""
  for p in /tmp/forge-review-{phase}-plan-*.md; do
    [ -f "$p" ] && PLAN_FILE_ARGS="$PLAN_FILE_ARGS --plan-file $p"
  done
  PROJECT_ARG=""
  [ -f "/tmp/forge-review-{phase}-project.md" ] && PROJECT_ARG="--project-file /tmp/forge-review-{phase}-project.md"
  CONTEXT_ARG=""
  [ -f "/tmp/forge-review-{phase}-context.md" ] && CONTEXT_ARG="--context-file /tmp/forge-review-{phase}-context.md"
  RESEARCH_ARG=""
  [ -f "/tmp/forge-review-{phase}-research.md" ] && RESEARCH_ARG="--research-file /tmp/forge-review-{phase}-research.md"
  REQUIREMENTS_ARG=""
  [ -f "/tmp/forge-review-{phase}-requirements.md" ] && REQUIREMENTS_ARG="--requirements-file /tmp/forge-review-{phase}-requirements.md"

  forge_run query prompt-budget \
    --budget "$REVIEWER_BUDGET" \
    --instructions-file "/tmp/forge-review-{phase}-instructions.md" \
    --roadmap-file "/tmp/forge-review-{phase}-roadmap.md" \
...
OLLAMA_REVIEWER_BUDGET=$(forge_run query config-get review.max_prompt_tokens_per_reviewer.ollama 2>/dev/null | jq -r '.' 2>/dev/null || echo "null")
if [ -z "$OLLAMA_REVIEWER_BUDGET" ] || [ "$OLLAMA_REVIEWER_BUDGET" = "null" ]; then
  OLLAMA_REVIEWER_BUDGET=$(forge_run query config-get review.max_prompt_tokens 2>/dev/null | jq -r '.' 2>/dev/null || echo "null")
fi

# Apply budget trim for Ollama if a budget is configured
OLLAMA_PROMPT_FILE="/tmp/forge-review-prompt-{phase}.md"
OLLAMA_SKIP=0
if [ -n "$OLLAMA_REVIEWER_BUDGET" ] && [ "$OLLAMA_REVIEWER_BUDGET" != "null" ] && [ "$OLLAMA_REVIEWER_BUDGET" != "0" ]; then
  OLLAMA_TRIMMED_PROMPT="/tmp/forge-review-prompt-{phase}-ollama.md"
  OLLAMA_TRIM_META="/tmp/forge-review-prompt-{phase}-ollama.metadata.json"
...
OLLAMA_HOST=$(forge_run query config-get review.ollama_host 2>/dev/null | jq -r '.' 2>/dev/null || echo "")
if [ -z "$OLLAMA_HOST" ] || [ "$OLLAMA_HOST" = "null" ]; then OLLAMA_HOST="http://localhost:11434"; fi
OLLAMA_MODEL=$(forge_run query config-get review.models.ollama 2>/dev/null | jq -r '.' 2>/dev/null || echo "")
...
jq -n --rawfile content "$OLLAMA_PROMPT_FILE" \
  ...
  > /tmp/forge-review-ollama-{phase}.md
if [ ! -s /tmp/forge-review-ollama-{phase}.md ]; then
  echo "Ollama review failed or returned empty output." > /tmp/forge-review-ollama-{phase}.md
fi
fi
```
...
LM_STUDIO_REVIEWER_BUDGET=$(forge_run query config-get review.max_prompt_tokens_per_reviewer.lm_studio 2>/dev/null | jq -r '.' 2>/dev/null || echo "null")
if [ -z "$LM_STUDIO_REVIEWER_BUDGET" ] || [ "$LM_STUDIO_REVIEWER_BUDGET" = "null" ]; then
  LM_STUDIO_REVIEWER_BUDGET=$(forge_run query config-get review.max_prompt_tokens 2>/dev/null | jq -r '.' 2>/dev/null || echo "null")
fi

# Apply budget trim for LM Studio if a budget is configured
LM_STUDIO_PROMPT_FILE="/tmp/forge-review-prompt-{phase}.md"
LM_STUDIO_SKIP=0
if [ -n "$LM_STUDIO_REVIEWER_BUDGET" ] && [ "$LM_STUDIO_REVIEWER_BUDGET" != "null" ] && [ "$LM_STUDIO_REVIEWER_BUDGET" != "0" ]; then
  LM_STUDIO_TRIMMED_PROMPT="/tmp/forge-review-prompt-{phase}-lm_studio.md"
  LM_STUDIO_TRIM_META="/tmp/forge-review-prompt-{phase}-lm_studio.metadata.json"
...
LM_STUDIO_HOST=$(forge_run query config-get review.lm_studio_host 2>/dev/null | jq -r '.' 2>/dev/null || echo "")
if [ -z "$LM_STUDIO_HOST" ] || [ "$LM_STUDIO_HOST" = "null" ]; then LM_STUDIO_HOST="http://localhost:1234"; fi
LM_STUDIO_MODEL=$(forge_run query config-get review.models.lm_studio 2>/dev/null | jq -r '.' 2>/dev/null || echo "")
...
if [ -n "$LM_STUDIO_CONTENT" ]; then
  echo "$LM_STUDIO_CONTENT" > /tmp/forge-review-lm_studio-{phase}.md
else
...
LLAMA_CPP_REVIEWER_BUDGET=$(forge_run query config-get review.max_prompt_tokens_per_reviewer.llama_cpp 2>/dev/null | jq -r '.' 2>/dev/null || echo "null")
if [ -z "$LLAMA_CPP_REVIEWER_BUDGET" ] || [ "$LLAMA_CPP_REVIEWER_BUDGET" = "null" ]; then
  LLAMA_CPP_REVIEWER_BUDGET=$(forge_run query config-get review.max_prompt_tokens 2>/dev/null | jq -r '.' 2>/dev/null || echo "null")
fi

# Apply budget trim for llama.cpp if a budget is configured
LLAMA_CPP_PROMPT_FILE="/tmp/forge-review-prompt-{phase}.md"
LLAMA_CPP_SKIP=0
if [ -n "$LLAMA_CPP_REVIEWER_BUDGET" ] && [ "$LLAMA_CPP_REVIEWER_BUDGET" != "null" ] && [ "$LLAMA_CPP_REVIEWER_BUDGET" != "0" ]; then
  LLAMA_CPP_TRIMMED_PROMPT="/tmp/forge-review-prompt-{phase}-llama_cpp.md"
  LLAMA_CPP_TRIM_META="/tmp/forge-review-prompt-{phase}-llama_cpp.metadata.json"
...
LLAMA_CPP_HOST=$(forge_run query config-get review.llama_cpp_host 2>/dev/null | jq -r '.' 2>/dev/null || echo "")
if [ -z "$LLAMA_CPP_HOST" ] || [ "$LLAMA_CPP_HOST" = "null" ]; then LLAMA_CPP_HOST="http://localhost:8080"; fi
LLAMA_CPP_MODEL=$(forge_run query config-get review.models.llama_cpp 2>/dev/null | jq -r '.' 2>/dev/null || echo "")
...
if [ -n "$LLAMA_CPP_CONTENT" ]; then
  echo "$LLAMA_CPP_CONTENT" > /tmp/forge-review-llama_cpp-{phase}.md
else
...
Commit:
```bash
forge_run query commit "docs: cross-AI review for phase {N}" --files {phase_dir}/{padded_phase}-REVIEWS.md
```
</step>
...
- [ ] User knows how to use feedback (/forge:plan-phase --reviews)
</success_criteria>
