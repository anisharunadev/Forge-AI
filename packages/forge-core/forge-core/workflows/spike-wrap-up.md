...
`.planning/spikes/WRAP-UP-SUMMARY.md`. Companion to `/forge:spike`.
</purpose>
...
Run `/forge:spike` first to create experiments.
```
Exit.
...
```bash
forge_run query commit "docs(spike-wrap-up): package [N] spike findings into project skill" --files .planning/spikes/WRAP-UP-SUMMARY.md .planning/spikes/CONVENTIONS.md
```
</step>
...
`/forge:spike` (run with no argument — its frontier mode analyzes the spike landscape and proposes integration and frontier spikes)

───────────────────────────────────────────────────────────────

**Also available:**
- `/forge:plan-phase` — start planning the real implementation
- `/forge:spike [idea]` — spike a specific new idea
...
- [ ] Summary presented
- [ ] Next-step options presented (including frontier spike exploration via `/forge:spike`)
</success_criteria>
