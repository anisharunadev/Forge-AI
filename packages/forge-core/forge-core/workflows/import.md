...
- **--from**: Import external plan → conflict detection → write PLAN.md → validate via forge-plan-checker
...
INIT=$(forge_run query init.phase-op "{NN}")
...
Delegate validation to forge-plan-checker:

Print: "Delegating to forge-plan-checker (runs in a subagent — no output until it returns, ~1–5 min; expected, not a freeze)"
...
```bash
forge_run query commit "docs({phase}): import plan from {basename FILEPATH}" --files .planning/phases/{phase}/{plan}-PLAN.md .planning/ROADMAP.md
```
...
- Use `pbr:plan-checker` or `pbr:planner` — use `forge-plan-checker` and `forge-planner`
