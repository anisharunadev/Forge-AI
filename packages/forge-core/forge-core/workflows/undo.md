...
```
Usage: /forge:undo --last N | --phase NN | --plan NN-MM
...
```
Working tree has uncommitted changes. Commit or stash them before running /forge:undo.
```
...
**Also available:**
- `/forge:execute-phase ${PHASE}` — re-execute if needed
- `/forge:undo --last 1` — undo the revert itself if something went wrong
