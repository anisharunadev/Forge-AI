...
| `map`, `map-codebase` | The `/forge:map-codebase` block under `### Project Initialization` |
| `discuss`, `discuss-phase` | The `/forge:discuss-phase` block under `### Phase Planning` |
...
| `fast` | The `/forge:fast` block under `### Quick Mode` |
...
| `spike` | The `/forge:spike` and `/forge:spike --wrap-up` blocks under `### Spiking & Sketching` |
| `sketch` | The `/forge:sketch` and `/forge:sketch --wrap-up` blocks under `### Spiking & Sketching` |
| `spike-sketch`, `experiments` | `### Spiking & Sketching` |
| `capture`, `notes`, `todos` | `### Capturing Ideas, Notes, and Todos` |
| `verify`, `verify-work`, `uat` | `### User Acceptance Testing` plus the `/forge:audit-uat` block |
| `ship`, `pr` | `### Ship Work` plus the `/forge:pr-branch` block |
| `review`, `peer-review` | The `/forge:review` block under `### Ship Work` |
| `audit`, `auditing`, `audit-milestone` | `### Milestone Auditing` |
| `config`, `settings`, `configuration` | `### Configuration` |
| `cleanup` | The `/forge:cleanup` block under `### Utility Commands` |
| `update` | The `/forge:update` block under `### Utility Commands` |
...
3. If no match: emit a one-line error followed by a comma-separated list of the canonical topic names from the leftmost column (one per row, deduplicated). Suggest `/forge:help --full` for the complete reference. Stop.
...
5. Read `workflows/help/modes/full.md`. Strip `<reference>` / `</reference>` wrapper tags — never emit them. Apply the extraction rule for the matched table cell, modulated by scope:
   ...
   - *Compact scope:* emit the heading, then the first `` **`/forge:...`** `` bold line within the section (the signature) and the single non-blank line immediately after it (the one-line summary). If the section has no `` **`/forge:...`** `` bold line, emit the heading and the first paragraph.
   ...
   5c. **Sub-block** (cell says `the /forge:X block under ### Heading` or `the /forge:X ... blocks under ### Heading`): within the named heading's section, start at each `` **`/forge:X ...`** `` bold line.
   - *Full scope:* stop immediately before the next `` **`/forge:...`** `` bold line or the next heading, whichever comes first.
...
6. After the section content, emit a single closing line:
   ...
   ```text
   More: /forge:help --full · /forge:help <topic> · /forge:help --brief <topic>
   ```
