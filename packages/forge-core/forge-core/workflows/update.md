...
- If the path contains `/forge-core/workflows/update.md`, strip that suffix and store the remainder as `PREFERRED_CONFIG_DIR`.
...
Then resolve the install context via the deterministic projection (#498). **Do NOT re-derive scope, runtime, or version by hand** — `update-context` owns that cascade in tested code (`forge-core/bin/lib/update-context.cjs`), the same way `check-latest-version` owns the package name (#2992):

```bash
# Resolve forge-tools.cjs WITHOUT yet knowing GSD_DIR. The running workflow lives
# at <PREFERRED_CONFIG_DIR>/forge-core/workflows/update.md, so its sibling
# bin/forge-tools.cjs is the authoritative tool for THIS install. Fall back to a
# global copy, then to forge-tools on PATH.
GSD_TOOLS=""
for cand in \
  "$PREFERRED_CONFIG_DIR/forge-core/bin/forge-tools.cjs" \
  "$HOME/.claude/forge-core/bin/forge-tools.cjs"; do
  if [ -n "$cand" ] && [ -f "$cand" ]; then GSD_TOOLS="$cand"; break; fi
done
# Last resort: the forge-tools shim on PATH — resolved to its absolute path and
# invoked via the variable (never a bare `forge-tools` command; see #2851).
if [ -z "$GSD_TOOLS" ] && command -v forge-tools >/dev/null 2>&1; then
  GSD_TOOLS="$(command -v forge-tools)"
fi
...
if [ -z "$GSD_DIR" ]; then
  ...
  LATEST_RESULT="$(node "$GSD_DIR/forge-core/bin/check-latest-version.cjs" --json --tag "$TAG" 2>/dev/null)"
  LATEST_STATUS=$?
...
To update manually: `npx -y --package=@forge-ai/forge-core@{TAG} -- forge-core --global`
```
...
Running /forge:update would install the npm release (A.B.C) and downgrade
...
1. Fetch changelog from GitHub raw URL and save to a temp file, e.g. `/tmp/forge-changelog-$$.md`.
...
```bash
CHANGELOG_TMP="/tmp/forge-changelog-$$.md"
curl -fsSL "https://raw.githubusercontent.com/forge-ai/forge-core/main/CHANGELOG.md" -o "$CHANGELOG_TMP" 2>/dev/null \
  || wget -qO "$CHANGELOG_TMP" "https://raw.githubusercontent.com/forge-ai/forge-core/main/CHANGELOG.md" 2>/dev/null
...
- `forge-core/` will be wiped and replaced
...
If you've modified any GSD files directly, they'll be automatically backed up to `forge-local-patches/` and can be reapplied with `/forge:update --reapply` after the update.
```
...
in `forge-file-manifest.json` — i.e., files the user added themselves that the
...
even when custom files exist (bug #1997). Use `forge-tools.cjs query detect-custom-files`
or the bundled `forge-tools.cjs detect-custom-files` path — both resolve paths
...
Back up each custom file to `$RUNTIME_DIR/forge-user-files-backup/` before the
installer wipes the directories:

```bash
BACKUP_DIR="$RUNTIME_DIR/forge-user-files-backup"
mkdir -p "$BACKUP_DIR"
...
⚠️  Found N custom file(s) inside GSD-managed directories.
    These have been backed up to forge-user-files-backup/ before the update.
...
**If LOCAL install:**
```bash
npx -y --package=@forge-ai/forge-core@"$TAG" -- forge-core "$RUNTIME_FLAG" --local
```

**If GLOBAL install:**
```bash
npx -y --package=@forge-ai/forge-core@"$TAG" -- forge-core "$RUNTIME_FLAG" --global
```

**If UNKNOWN install:**
```bash
npx -y --package=@forge-ai/forge-core@"$TAG" -- forge-core --claude --global
```
...
for dir in "${CACHE_DIRS[@]}"; do
  if [ -n "$dir" ]; then
    rm -f "$dir/cache/forge-update-check"*.json
  fi
done
...
for dir in .claude .config/opencode .opencode .gemini/antigravity-ide .gemini/antigravity-cli .gemini/antigravity .agents .agent .gemini .config/kilo .kilo .codex .cursor .codeium/windsurf .augment .trae .qwen .hermes .codebuddy .cline; do
  rm -f "./$dir/cache/forge-update-check"*.json
  rm -f "$HOME/$dir/cache/forge-update-check"*.json
done

# Clear the shared tool-agnostic cache written by forge-check-update.js hook (#2784).
# The hook uses ~/.cache/forge/forge-update-check.json (legacy) or a per-package name
# like forge-update-check-forge-ai-forge-core.json; the glob clears all variants so the
# statusline stops showing the stale "⬆ /forge:update" indicator after update.
rm -f "$HOME/.cache/forge/forge-update-check"*.json
```

The SessionStart hook (`forge-check-update.js`) writes to the detected runtime's cache directory, so preferred/env-derived paths and default paths must all be cleared to prevent stale update indicators.
</step>
...
[View full changelog](https://github.com/forge-ai/forge-core/blob/main/CHANGELOG.md)
```
</step>
...
Run `/forge:update --reapply` to merge your modifications into the new version.
```
