...
@~/.claude/forge-core/references/revision-loop.md
...
- forge-pattern-mapper — Analyzes codebase for existing patterns, produces PATTERNS.md
...
forge-phase-researcher, forge-planner, and forge-plan-checker as separate Agent() calls.
...
Agent() call for forge-phase-researcher, forge-planner, and forge-plan-checker. Only
...
INIT=$(forge_run query init.plan-phase "$PHASE" $GRAN_PARAM)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_RESEARCHER=$(forge_run query agent-skills forge-phase-researcher)
AGENT_SKILLS_PLANNER=$(forge_run query agent-skills forge-planner)
AGENT_SKILLS_CHECKER=$(forge_run query agent-skills forge-plan-checker)
CONTEXT_WINDOW=$(forge_run query config-get context_window 2>/dev/null || echo "200000")
MVP_MODE_CFG=$(forge_run query config-get workflow.mvp_mode 2>/dev/null || echo "false")
```
...
/forge:plan-phase --reviews cannot replan a closed phase. If the review surfaced
...
with: /forge:plan-phase ${phase_number} --force
...
if [[ "$ARGUMENTS" =~ (^|[[:space:]])--tdd([[:space:]]|$) ]]; then
  forge_run query config-set workflow.tdd_mode true 2>/dev/null || true
fi
```
...
if [ "$MVP_MODE" = "true" ] && [ "$padded_phase" = "01" ]; then
  PRIOR_SUMMARIES=$(forge_run query phases.list --pick summaries_total 2>/dev/null || echo "0")
  if [ "$PRIOR_SUMMARIES" = "0" ]; then WALKING_SKELETON=true; fi
fi
```

When `WALKING_SKELETON=true`:
- Planner is instructed to produce `SKELETON.md` in the phase directory alongside `PLAN.md`. The template lives at `~/.claude/forge-core/references/skeleton-template.md` — the planner reads it when producing SKELETON.md (lazy; not loaded on non-skeleton runs).
...
```bash
CHUNKED_CFG=$(forge_run query config-get workflow.plan_chunked 2>/dev/null || echo "false")
CHUNKED_MODE=false
...
/forge:review --phase {N}

Then re-run /forge:plan-phase {N} --reviews
```
Exit workflow.
...
```bash
PHASE_INFO=$(forge_run query roadmap.get-phase "${PHASE}")
```
...
```bash
MVP_MODE=$(forge_run query phase.mvp-mode "${PHASE}" $MVP_FLAG_ARG --pick active)
```
...
5. Commit:
```bash
forge_run query commit "docs(${padded_phase}): generate context from PRD" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
2. Parse each resolved ADR through `forge-core/bin/lib/adr-parser.cjs` (`--input`, `--format`) and collect normalized records.
3. Status gate: reject `superseded`/`rejected`/`deprecated`; warn on `proposed`; missing status defaults to `accepted`.
4. Empty-decisions fallback: if all parsed ADRs have zero `decisions[]`, emit `ADR ingest produced no locked decisions; fall back to discuss-phase for this phase.` and exit with `/forge:discuss-phase {N}` guidance.
...
6. Commit with `forge-tools.cjs query commit "docs(${padded_phase}): generate context from ADR ingest" --files "${phase_dir}/${padded_phase}-CONTEXT.md"` and set `context_content`; continue to step 5.
...
```bash
DISCUSS_MODE=$(forge_run query config-get workflow.discuss_mode 2>/dev/null || echo "discuss")
```
...
If "Run discuss-phase first":
  **IMPORTANT:** Do NOT invoke discuss-phase as a nested Skill/Task call — AskUserQuestion
  ...
  ```
  Run this command first, then re-run /forge:plan-phase {X} ${GSD_WS}:

  /forge:discuss-phase {X} ${GSD_WS}
  ```
...
If `AI_SPEC_FILE` is non-empty, pass `AI_SPEC_PATH` and `FRAMEWORK_LINE` to the planner in step 8 so it can reference the AI design contract. If it is empty, the active `ai-integration` capability hook in step 5.6 handles any AI-system nudge or `/forge:ai-integration-phase` dispatch.
...
2. **`--research`** (force-refresh): re-spawn researcher unconditionally — fall through to "Spawn forge-phase-researcher" below.
...
```bash
if forge_run query teams-status --active >/dev/null 2>&1; then
  echo "⚠️  CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS detected. GSD's multi-agent orchestration is not validated under claude-code agent-teams and may stall (a subagent's completion can fail to route to the orchestrator). Recommend disabling agent-teams for GSD workflows. See https://github.com/forge-ai/forge-core/issues/1355" >&2
fi
```

```bash
PHASE_DESC=$(forge_run query roadmap.get-phase "${PHASE}" --pick section)
if [ -z "${PLAN_PRE_HOOKS_JSON:-}" ]; then
  PLAN_PRE_HOOKS_JSON=$(forge_run loop render-hooks plan:pre --raw)
fi
```
...
**If `RESEARCH_ONLY=true`:** the user invoked `/forge:plan-phase --research-phase <N>` for research-only mode. Do **not** continue to Section 5.5+ (validation strategy, planner, plan-checker, verification, gaps, bounce, post-planning-gaps). Print the research-complete summary and exit cleanly:
...
Re-run /forge:plan-phase ${PHASE} to plan the phase using this research,
or /forge:plan-phase ${PHASE} --research to refresh research and plan.
```
...
**If found:**
1. Read template: `~/.claude/forge-core/templates/VALIDATION.md`
...
```bash
PLAN_PRE_HOOKS_JSON=${PLAN_PRE_HOOKS_JSON:-$(forge_run loop render-hooks plan:pre --raw)}
...
- If `ref.skill` is set, dispatch with `Skill(skill="gsd-${ref.skill}", args="${PHASE} --auto ${GSD_WS}")` when pipeline mode allows auto-chaining. Prepend `forge-` to `ref.skill` — `ui-phase` → `forge-ui-phase`.
...
- In manual mode, display the existing non-blocking `/forge:ai-integration-phase {N}` recommendation and let the user continue planning without AI-SPEC or stop to run the capability workflow first.
...
```bash
GATE=$(forge_run check ui-plan-gate "${PHASE}" --raw)
```
...
```bash
AUTO_CHAIN=$(forge_run query check auto-mode --pick auto_chain_active 2>/dev/null || echo "false")
```
...
`/forge:ui-phase {N} ${GSD_WS}` — generate UI design contract before planning
───────────────────────────────────────────────
Also available:
- `/forge:plan-phase {N} --skip-ui ${GSD_WS}` — plan without UI-SPEC (not recommended for frontend phases)
```
...
```bash
DRIFT=$(forge_run verify codebase-drift 2>/dev/null || echo '{"skipped":true}')
```
...
verbatim (it ends with a `/forge:map-codebase` pointer) and continue — planning proceeds whether or not the
...
1. Re-run: `/forge:plan-phase {PHASE} --research ${GSD_WS}`
2. Disable Nyquist with the exact command:
   `forge-tools.cjs query config-set workflow.nyquist_validation false`
3. Continue anyway (plans fail Dimension 8)

Proceed to Step 7.8 (or Step 8 if pattern mapper is disabled) only if user selects 2 or 3.

## 7.8. Spawn forge-pattern-mapper Agent (Optional)

Pattern mapper activation is owned by the `pattern-mapper` capability's `plan:pre` step hook. Read `PLAN_PRE_HOOKS_JSON` and skip if no active step hook has `capId == "pattern-mapper"` and `ref.agent == "forge-pattern-mapper"`. Also skip if no CONTEXT.md and no RESEARCH.md exist for this phase (nothing to extract file lists from).
...
- {context_path} (USER DECISIONS from /forge:discuss-phase)
...
**If Mode is reviews:** REVIEWS.md is feedback input, not a hidden execution contract. /forge:execute-phase primarily consumes PLAN.md plus the normal phase context, so every current actionable review finding must become visible in the relevant PLAN.md before planning can pass.
...
**MVP_MODE:** ${MVP_MODE} (when true, follow vertical-slice rules from `~/.claude/forge-core/references/planner-mvp-mode.md`; when false, ignore MVP guidance entirely.)
**WALKING_SKELETON:** ${WALKING_SKELETON} (when true, the first deliverable must be a Walking Skeleton — Read the template at `~/.claude/forge-core/references/skeleton-template.md` and produce SKELETON.md alongside PLAN.md.)
**Granularity:** {granularity}

${MVP_MODE === 'true' ? `
<mvp_mode_active>
**MVP Mode is ENABLED.** Read `~/.claude/forge-core/references/planner-mvp-mode.md` now and follow its vertical-slice planning rules. Each plan must deliver a complete vertical slice — thin end-to-end functionality rather than horizontal layers.
</mvp_mode_active>
` : ''}
</planning_context>
...
`/forge:plan-phase {N} --chunked` resumes from the last successfully committed plan.

**Intended for new or in-progress chunked runs.** To recover plans already written by a prior
*non-chunked* run, use step 6's "Add more plans" or proceed directly to `/forge:execute-phase`
...
5. **Commit per-plan:**
   ```bash
   forge_run query commit "docs(${PADDED_PHASE}): plan ${plan_id} (chunked)" --files "${PHASE_DIR}/${plan_id}-PLAN.md"
   ```
...
3. **Stop** — exit; user can re-run `/forge:plan-phase {N}` to resume
...
**If "Split":** Use `/forge:phase --insert` to create the sub-phases, then replan each.
...
**If "Split":** Use `/forge:phase --insert` for overflow items, then replan.
...
**Stall detection:** If `issue_count >= prev_issue_count`:
  ...
  **If `stall_reentry_count >= 2`:**
    ...
    Suggest: "Consider resolving these issues manually or running `/forge:debug` to investigate root causes."
...
**Read pass count:**
```bash
BOUNCE_PASSES=$(forge_run query config-get workflow.plan_bounce_passes 2>/dev/null || echo "2")
BOUNCE_SCRIPT=$(forge_run query config-get workflow.plan_bounce_script 2>/dev/null | jq -r '.' 2>/dev/null || true)
```
...
5. **Re-run plan checker on bounced plans:** Spawn forge-plan-checker (same as step 10) on all modified plans. If a bounced plan fails the checker, restore original from its pre-bounce.md backup:
```
...
```bash
forge_run query commit "refactor(${padded_phase}): bounce plans through external refinement" --files "${PHASE_DIR}/*-PLAN.md"
```
...
```bash
GATE_CFG=$(forge_run query config-get workflow.context_coverage_gate 2>/dev/null || echo "true")
if [ "$GATE_CFG" != "false" ]; then
  GATE_RESULT=$(forge_run query check.decision-coverage-plan "${PHASE_DIR}" "${CONTEXT_PATH}")
...
```bash
forge_run query state.planned-phase --phase "${PHASE_NUMBER}" --name "${PHASE_NAME}" --plans "${PLAN_COUNT}"
```
...
```bash
forge_run query roadmap.annotate-dependencies "${PHASE_NUMBER}"
```
...
```bash
forge_run query commit "docs(${PADDED_PHASE}): create phase plan" --files "${PHASE_DIR}"/*-PLAN.md .planning/STATE.md .planning/ROADMAP.md
```
...
```bash
PLAN_POST_HOOKS_JSON=$(forge_run loop render-hooks plan:post --raw)
PHASE_REQ_IDS=$(forge_run query init.plan-phase "$PHASE" --pick phase_req_ids 2>/dev/null || echo TBD)
```
...
```bash
GATE_RESULT=$(forge_run check ${hook.check.query} "${PHASE_DIR}" "${PHASE_REQ_IDS}" --raw)
CHECK_EXIT=$?
```
...
if ([[ "$ARGUMENTS" =~ --auto ]] || [[ "$ARGUMENTS" =~ --chain ]]) && [[ "$AUTO_CHAIN" != "true" ]]; then
  forge_run query config-set workflow._auto_chain_active true
fi
```
...
```
Skill(skill="forge-execute-phase", args="${PHASE} --auto --no-transition ${GSD_WS}")
```
...
- **PHASE COMPLETE** → Display final summary:
  ...
  Next: /forge:discuss-phase ${NEXT_PHASE} --auto ${GSD_WS}
  ```
- **GAPS FOUND / VERIFICATION FAILED** → Display result, stop chain:
  ...
  /forge:execute-phase ${PHASE} ${GSD_WS}
  ```
...
- /forge:review --phase {X} --all — peer review plans with external AIs
- /forge:plan-phase {X} --reviews — replan incorporating review feedback
...
5. **Retry:** Restart Claude Code and run `/forge:plan-phase` again

If freezes persist, try `--skip-research` to reduce the agent chain from 3 to 2 agents:
```
/forge:plan-phase N --skip-research
```
</windows_troubleshooting>
