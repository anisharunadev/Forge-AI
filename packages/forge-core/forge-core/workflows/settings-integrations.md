...
`.planning/config.json` via `forge-tools` so unrelated keys are
preserved, never clobbered.

This command is deliberately separate from `/forge:settings` (workflow toggles)
and any `/forge-settings-advanced` tuning surface. It exists because API keys and
...
```bash
BRAVE=$(forge_run query config-get brave_search --default null)
FIRECRAWL=$(forge_run query config-get firecrawl --default null)
EXA=$(forge_run query config-get exa_search --default null)
SEARCH_GITIGNORED=$(forge_run query config-get search_gitignored --default false)
```
...
```bash
forge_run query config-set brave_search "<value>"     # masked in output
forge_run query config-set firecrawl "<value>"        # masked in output
forge_run query config-set exa_search "<value>"       # masked in output
forge_run query config-set search_gitignored true|false
```

For "Clear", write `null`:

```bash
forge_run query config-set brave_search null
```
</step>
...
```bash
forge_run query config-set review.models.<cli> "<model id>"
```
...
AskUserQuestion([
  {
    ...
    multiSelect: false,
    options: [
      { label: "forge-executor", description: "Skills injected when spawning executor agents" },
      { label: "forge-planner", description: "Skills injected when spawning planner agents" },
      { label: "forge-verifier", description: "Skills injected when spawning verifier agents" },
...
```bash
forge_run query config-set agent_skills.<slug> "<skill-a,skill-b,skill-c>"
```
...
Quick commands:
- /forge:settings — workflow toggles and model profile
