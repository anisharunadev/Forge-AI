...
<template>
@~/.claude/forge-core/templates/UAT.md
</template>
...
INIT=$(forge_run query init.verify-work "${PHASE_ARG}" ${GSD_WS})
...
MVP_MODE=$(forge_run query phase.mvp-mode "${phase_number}" ${GSD_WS} --pick active)
```
</step>
...
```bash
PLAN_HOOKS_JSON=$(forge_run loop render-hooks plan:pre --raw)
...
**MVP-mode UAT framing.** When `MVP_MODE=true`, follow the rules in `@~/.claude/forge-core/references/verify-mvp-mode.md`. Briefly:
...
```bash
PHASE_GOAL=$(forge_run query roadmap.get-phase "${phase_number}" ${GSD_WS} --pick goal)
...
if [ "$USER_STORY_VALID" != "true" ]; then
  ...
  echo "Run /forge mvp-phase ${phase_number} to set a user-story goal before verifying."
  exit 1
fi
```
...
```bash
COVERAGE=$(forge_run query uat.classify-coverage --summary "$SUMMARY_FILE")
```
...
```bash
CHECKPOINT=$(forge_run query uat.render-checkpoint --file "$uat_path" --raw)
...
Commit the UAT file:
```bash
forge_run query commit "test({phase_num}): complete UAT - {passed} passed, {issues} issues" --files ".planning/phases/XX-name/{phase_num}-UAT.md"
```
...
```
⚠ Security enforcement enabled — /forge:secure-phase {phase} did not produce SECURITY.md.
...
- `/forge:secure-phase {phase}` — security review (required before advancing)
- `/forge:plan-phase {next}` — Plan next phase
- `/forge:execute-phase {next}` — Execute next phase
- `/forge:ui-review {phase}` — visual quality audit (if frontend files were modified)
```
...
Read and follow `~/.claude/forge-core/workflows/transition.md`.
...
- `/forge:plan-phase {next}` — Plan next phase
- `/forge:execute-phase {next}` — Execute next phase
- `/forge:secure-phase {phase}` — security review
- `/forge:ui-review {phase}` — visual quality audit (if frontend files were modified)
```
</step>
...
`audit-open` is CJS-only until registered on `forge-tools.cjs query`:

```bash
forge_run query audit-open --json
```
...
If user declines: stop. User resolves items and re-runs `/forge:verify-work`.
...
- Follow @~/.claude/forge-core/workflows/diagnose-issues.md
