...
**Delegate the entire removal operation to `forge-tools.cjs query phase.remove`:**

```bash
RESULT=$(forge_run query phase.remove "${target}")
```

If the phase has executed plans (SUMMARY.md files), the CLI will error. Use `--force` only if the user confirms:

```bash
RESULT=$(forge_run query phase.remove "${target}" --force)
```
...
```bash
forge_run query commit "chore: remove phase {target} ({original-phase-name})" --files .planning/
```
...
- Don't manually renumber — use `forge-tools.cjs query phase.remove` which handles all renumbering
...
- [ ] `forge-tools.cjs query phase.remove` executed successfully
