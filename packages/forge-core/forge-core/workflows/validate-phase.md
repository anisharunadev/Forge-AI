...
AGENT_SKILLS_AUDITOR=$(forge_run query agent-skills forge-nyquist-auditor)
```

Parse: `phase_dir`, `phase_number`, `phase_name`, `phase_slug`, `padded_phase`.

```bash
AUDITOR_MODEL=$(forge_run query resolve-model forge-nyquist-auditor --raw)
...
If no active validate-phase step hook exists: exit with "Nyquist validation is disabled. Enable via /forge:settings."
...
- **State C** (`SUMMARY_FILES` empty): Exit — "Phase {N} not executed. Run /forge:execute-phase {N} ${GSD_WS} first."
...
**State B (create):**
1. Read template from `~/.claude/forge-core/templates/VALIDATION.md`
...
forge_run query commit "docs(phase-${PHASE}): add/update validation strategy"
```
...
▶ Next: /forge:audit-milestone ${GSD_WS}
```
...
▶ Retry: /forge:validate-phase {N} ${GSD_WS}
```
