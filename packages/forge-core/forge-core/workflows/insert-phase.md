...
Example: `/forge:phase --insert 72 Fix critical auth bug`
-> after = 72
...
Usage: /forge:phase --insert <after> <description>
Example: /forge:phase --insert 72 Fix critical auth bug
```
...
INIT=$(forge_run query init.phase-op "${after_phase}")
...
**Delegate the phase insertion to `forge-tools.cjs query phase.insert`:**

```bash
RESULT=$(forge_run query phase.insert "${after_phase}" "${description}")
```
...
1. Update STATE.md's next-phase pointer(s) to the newly inserted phase
   ...
   ```bash
   forge_run query state.patch '{"Current Phase":"{decimal_phase}","Next recommended run":"/forge:plan-phase {decimal_phase}"}'
   ```
...
- [ ] `forge-tools.cjs query phase.insert` executed successfully
- [ ] Phase directory created
- [ ] Roadmap updated with new phase entry (includes "(INSERTED)" marker)
- [ ] `forge-tools.cjs query state.add-roadmap-evolution ...` returned `{ added: true }` or `{ added: false, reason: "duplicate" }`
- [ ] `forge-tools.cjs query state.patch` returned matched next-phase pointer field(s)
