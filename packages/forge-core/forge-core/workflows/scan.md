<purpose>
Lightweight codebase assessment. Spawns a single forge-codebase-mapper agent for one focus area,
...
INIT=$(forge_run query init.map-codebase 2>/dev/null || echo "{}")
...
Spawn a single `forge-codebase-mapper` agent with the selected focus area:
...
Use `/forge:map-codebase` for a comprehensive 4-area parallel scan.
```
