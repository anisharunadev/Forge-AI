...
This workflow wires Phase 1 (session pipeline) and Phase 2 (profiling engine) into a cohesive user-facing experience. All heavy lifting is done by existing `forge-tools.cjs query` handlers (with legacy `forge-tools.cjs` parity where needed) and the forge-user-profiler agent -- this workflow orchestrates the sequence, handles branching, and provides the UX.
</purpose>
...
Key references:
- @$HOME/.claude/forge-core/references/ui-brand.md (display patterns)
- @$HOME/.claude/agents/forge-user-profiler.md (profiler agent definition)
- @$HOME/.claude/forge-core/references/user-profiling.md (profiling reference doc)
</required_reading>
...
```bash
PROFILE_PATH="$HOME/.claude/forge-core/USER-PROFILE.md"
...
```bash
cp "$HOME/.claude/forge-core/USER-PROFILE.md" "$HOME/.claude/USER-PROFILE.backup.md"
```
...
✓ Stores profile at $HOME/.claude/forge-core/USER-PROFILE.md
...
- options:
  ...
  - "Not now" -- Display "No worries. Run /forge:profile-user when ready." and exit
...
SCAN_RESULT=$(forge_run query scan-sessions --json 2>/dev/null)
```
...
```bash
SAMPLE_RESULT=$(forge_run query profile-sample --json 2>/dev/null)
```
...
**Spawn forge-user-profiler agent using Task tool:**

Use the Task tool to spawn the `forge-user-profiler` agent. Provide it with:
- The sampled JSONL file path from profile-sample output
- The user-profiling reference doc at `$HOME/.claude/forge-core/references/user-profiling.md`
...
Reference: @$HOME/.claude/forge-core/references/user-profiling.md
...
**Get questions:**
```bash
QUESTIONS=$(forge_run query profile-questionnaire --json 2>/dev/null)
```
...
```bash
ANSWERS_PATH=$(mktemp /tmp/forge-profile-answers-XXXXXX.json)
```
...
```bash
ANALYSIS_RESULT=$(forge_run query profile-questionnaire --answers "$ANSWERS_PATH" --json 2>/dev/null)
```
...
```bash
ANALYSIS_PATH=$(mktemp /tmp/forge-profile-analysis-XXXXXX.json)
```
...
```bash
forge_run query write-profile --input "$ANALYSIS_PATH" --json
```

Display: "✓ Profile written to $HOME/.claude/forge-core/USER-PROFILE.md"
...
- options (ALL pre-selected by default):
  - "/forge-dev-preferences command file" -- "Load your preferences in any session"
...
**If no artifacts selected:** Display "No artifacts generated. Your profile is saved at $HOME/.claude/forge-core/USER-PROFILE.md" and jump to step 10.
...
**For /forge-dev-preferences (if selected):**

```bash
forge_run query generate-dev-preferences --analysis "$ANALYSIS_PATH" --json
```

Display: "✓ Generated /forge-dev-preferences at $HOME/.claude/skills/forge-dev-preferences/SKILL.md"

**For CLAUDE.md profile section (if selected):**

```bash
forge_run query generate-claude-profile --analysis "$ANALYSIS_PATH" --json
```
...
```bash
forge_run query generate-claude-profile --analysis "$ANALYSIS_PATH" --global --json
```

Display: "✓ Added profile section to $HOME/.claude/CLAUDE.md"

**Error handling:** If any `forge-tools.cjs query` or forge-tools.cjs call fails, display the error message and use AskUserQuestion to offer "Retry" or "Skip this artifact". On retry, re-run the command. On skip, continue to next artifact.
...
Your profile:    $HOME/.claude/forge-core/USER-PROFILE.md
```

Then list paths for each generated artifact:
```
Artifacts:
  ✓ /forge-dev-preferences   $HOME/.claude/skills/forge-dev-preferences/SKILL.md
...
- [ ] Artifacts generated sequentially via forge-tools.cjs query (or forge-tools.cjs) subcommands
