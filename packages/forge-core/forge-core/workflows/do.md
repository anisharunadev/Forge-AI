...
INIT=$(forge_run query state.load 2>/dev/null)
```
...
|--------------------------|----------|-----|
| Starting a new project, "set up", "initialize" | `/forge:new-project` | Needs full project initialization |
| Mapping or analyzing an existing codebase | `/forge:map-codebase` | Codebase discovery |
| A bug, error, crash, failure, or something broken | `/forge:debug` | Needs systematic investigation |
| Spiking, "test if", "will this work", "experiment", "prove this out", validate feasibility | `/forge:spike` | Throwaway experiment to validate feasibility |
| Sketching, "mockup", "what would this look like", "prototype the UI", "design this", explore visual direction | `/forge:sketch` | Throwaway HTML mockups to explore design |
| Wrapping up spikes, "package the spikes", "consolidate spike findings" | `/forge:spike --wrap-up` | Package spike findings into reusable skill |
| Wrapping up sketches, "package the designs", "consolidate sketch findings" | `/forge:sketch --wrap-up` | Package sketch findings into reusable skill |
| Exploring, researching, comparing, or "how does X work" | `/forge:explore` | Socratic ideation and idea routing |
| Discussing vision, "how should X look", brainstorming | `/forge:discuss-phase` | Needs context gathering |
| A complex task: refactoring, migration, multi-file architecture, system redesign | `/forge:phase` | Needs a full phase with plan/build cycle |
| Planning a specific phase or "plan phase N" | `/forge:plan-phase` | Direct planning request |
| Executing a phase or "build phase N", "run phase N" | `/forge:execute-phase` | Direct execution request |
| Running all remaining phases automatically | `/forge:autonomous` | Full autonomous execution |
| A review or quality concern about existing work | `/forge:verify-work` | Needs verification |
| Checking progress, status, "where am I" | `/forge:progress` | Status check |
| Resuming work, "pick up where I left off" | `/forge:resume-work` | Session restoration |
| A note, idea, or "remember to..." | `/forge:capture` | Capture for later |
| Adding tests, "write tests", "test coverage" | `/forge:add-tests` | Test generation |
| Completing a milestone, shipping, releasing | `/forge:complete-milestone` | Milestone lifecycle |
| A specific, actionable, small task (add feature, fix typo, update config) | `/forge:quick` | Self-contained, single executor |

**Requires `.planning/` directory:** All routes except `/forge:new-project`, `/forge:map-codebase`, `/forge:spike`, `/forge:sketch`, and `/forge:help`. If the project doesn't exist and the route requires it, suggest `/forge:new-project` first.

**Ambiguity handling:** If the text could reasonably match multiple routes, ask the user via AskUserQuestion with the top 2-3 options. For example:

```
"Refactor the authentication system" could be:
1. /forge:phase — Full planning cycle (recommended for multi-file refactors)
2. /forge:quick — Quick execution (if scope is small and clear)
...
Run the selected `/forge-*` command, passing `$ARGUMENTS` as args.
