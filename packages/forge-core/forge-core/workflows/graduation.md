...
GRADUATION_ENABLED=$(forge_run query config-get features.graduation 2>/dev/null || echo "true")
GRADUATION_WINDOW=$(forge_run query config-get features.graduation_window 2>/dev/null || echo "5")
GRADUATION_THRESHOLD=$(forge_run query config-get features.graduation_threshold 2>/dev/null || echo "3")
```
