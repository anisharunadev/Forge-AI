...
INIT=$(forge_run query init.new-workspace)
...
Run this command from a directory containing git repos, or specify repos explicitly:
  /forge:workspace --new --name my-workspace --repos /path/to/repo1,/path/to/repo2
```
Exit.
...
Usage:
  /forge:workspace --new --name my-workspace --repos repo1,repo2 --auto
```
Exit.
...
Next steps:
  cd "$TARGET_PATH"
  /forge:new-project    # Initialize GSD in the workspace
```
...
- options:
  - "Yes — run /forge:new-project" → tell user to `cd "$TARGET_PATH"` first, then run `/forge:new-project`
