...
SEEDS=$(forge_run list-seeds "$STATUS_FILTER")
...
Plant one with /forge:capture --seed "<forward-looking idea>".
```
...
```
- /forge:capture --seed --enrich <ID>   enrich a seed with trigger, why, and scope
- /forge:capture --list-seeds <status>  filter by status
```
</step>
