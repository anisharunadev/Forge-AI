...
- forge-security-auditor — Verifies threat mitigation coverage
</available_agent_types>
...
AGENT_SKILLS_AUDITOR=$(forge_run query agent-skills forge-security-auditor)
```

Parse: `phase_dir`, `phase_number`, `phase_name`, `phase_slug`, `padded_phase`.

```bash
AUDITOR_MODEL=$(forge_run query resolve-model forge-security-auditor --raw)
VERIFY_POST_HOOKS_JSON=$(forge_run loop render-hooks verify:post --raw)
SECURITY_ASVS=$(forge_run query config-get workflow.security_asvs_level --raw 2>/dev/null || echo "1")
SECURITY_BLOCK_ON=$(forge_run query config-get workflow.security_block_on --raw 2>/dev/null || echo "high")
```

Resolve active step hooks from `VERIFY_POST_HOOKS_JSON` where `kind == "step"` and `ref.skill == "secure-phase"`.

If no active secure-phase step hook exists: exit with "Security enforcement disabled. Enable via /forge:settings."
...
- **State C** (`SUMMARY_FILES` empty): Exit — "Phase {N} not executed. Run /forge:execute-phase {N} first."
...
## 5. Spawn forge-security-auditor
...
```
Agent(
  prompt="Read ~/.claude/agents/forge-security-auditor.md for instructions.\n\n" +
...
**State B (create):**
1. Read template from `~/.claude/forge-core/templates/SECURITY.md`
...
▶ Fix mitigations then re-run: /forge:secure-phase {N}
...
```bash
forge_run query commit "docs(phase-${PHASE}): add/update security threat verification"
```
...
▶ /forge:validate-phase {N}    validate test coverage
▶ /forge:verify-work {N}       run UAT
```
