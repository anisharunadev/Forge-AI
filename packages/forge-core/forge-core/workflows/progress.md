...
INIT=$(forge_run query init.progress)
...
<step name="load">
**Use structured extraction from `forge-tools.cjs query` (or legacy forge-tools.cjs):**

Instead of reading full files, use targeted tools to get only the data needed for the report:
- `ROADMAP=$(forge-tools.cjs query roadmap.analyze)`
- `STATE=$(forge-tools.cjs query state-snapshot)`
...
- Use `summary-extract` for efficient parsing:
  ```bash
  forge_run query summary-extract <path> --fields one_liner
  ```
...
**Generate progress bar from `forge-tools.cjs query progress` / `progress.json`, then present rich status report:**

```bash
# Get formatted progress bar
PROGRESS_BAR=$(forge_run query progress.bar --raw)
```
...
## Pending Todos
- [count] pending — /forge:capture --list to review
...
```bash
MVP_MODE=$(forge_run query phase.mvp-mode "${PHASE_NUMBER}" --pick active)
```
...
**If `INCOMPLETE_PHASE` is non-empty:** emit a one-line resume notice in the routing output and route to `/forge:execute-phase ${INCOMPLETE_PHASE}` instead of running Step 1's current-phase routing. The progress report (already displayed by the `report` step above) gives the user full project status before this routing decision is shown.
...
`/forge:execute-phase ${INCOMPLETE_PHASE} ${GSD_WS}`
...
```bash
DEBT=$(forge_run query audit-uat --raw 2>/dev/null)
```
...
Review: `/forge:audit-uat ${GSD_WS}` — full cross-phase audit
Resume testing: `/forge:verify-work {phase} ${GSD_WS}` — retest specific phase
```
...
```bash
PHASE_SECTION=$(forge_run query roadmap.get-phase "${CURRENT_PHASE}" 2>/dev/null)
...
`/forge:plan-phase {phase-number} ${GSD_WS}`
...
**Also available:**
- `/forge:ui-phase {phase}` — generate UI design contract (recommended for frontend phases)
- `/forge:plan-phase {phase}` — skip discussion, plan directly
- `/forge:discuss-phase {phase}` — include assumptions check before planning
...
`/forge:discuss-phase {phase} ${GSD_WS}` — gather context and clarify approach

---

**Also available:**
- `/forge:plan-phase {phase} ${GSD_WS}` — skip discussion, plan directly
- `/forge:discuss-phase {phase} ${GSD_WS}` — include assumptions check before planning
...
`/forge:plan-phase {phase} --gaps ${GSD_WS}`

---

**Also available:**
- `/forge:execute-phase {phase} ${GSD_WS}` — execute phase plans
- `/forge:verify-work {phase} ${GSD_WS}` — run more UAT testing
...
`/forge:verify-work {phase} ${GSD_WS}` — resume testing from where you left off

---

**Also available:**
- `/forge:audit-uat ${GSD_WS}` — full cross-phase UAT audit
- `/forge:execute-phase {phase} ${GSD_WS}` — execute phase plans
...
`/forge:verify-work {phase} ${GSD_WS}` — resume human verification
...
```bash
NEXT_PHASE_SECTION=$(forge_run query roadmap.get-phase "$((Z+1))" 2>/dev/null)
...
**Also available:**
- `/forge:ui-phase {Z+1}` — generate UI design contract (recommended for frontend phases)
...
`/forge:discuss-phase {Z+1} ${GSD_WS}` — gather context and clarify approach

---

**Also available:**
- `/forge:plan-phase {Z+1} ${GSD_WS}` — skip discussion, plan directly
- `/forge:verify-work {Z} ${GSD_WS}` — user acceptance test before continuing
...
`/forge:complete-milestone ${GSD_WS}`

---

**Also available:**
- `/forge:verify-work ${GSD_WS}` — user acceptance test before completing milestone
...
- Phase complete but next phase not planned → offer `/forge:plan-phase [next] ${GSD_WS}`
- All work complete → offer milestone completion
- Blockers present → highlight before offering to continue
- Handoff file exists → mention it, offer `/forge:resume-work ${GSD_WS}`
</step>
...
- Check 2 (orphaned handoff): `Read the handoff file(s) and resume from where work was paused: /forge:resume-work ${GSD_WS}`
...
- Check 1 (STATE inconsistency): `Run /forge:verify-work ${PHASE} ${GSD_WS} to reconcile state`
</step>
