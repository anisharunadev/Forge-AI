...
INIT=$(forge_run query init.manager)
...
- `manager_flags.discuss` — appended to `/forge:discuss-phase` args (e.g. `"--auto --analyze"`)
- `manager_flags.plan` — appended to plan agent init command
- `manager_flags.execute` — appended to execute agent init command

These are empty strings by default. Set via: `forge-tools.cjs query config-set manager.flags.discuss "--auto --analyze"`
...
All {phase_count} phases done. Ready for final steps:
  → /forge:verify-work — run acceptance testing
  → /forge:complete-milestone — archive and wrap up
```
...
Handle responses:
- "Verify work": `Skill(skill="forge-verify-work")`  then loop to dashboard.
...
When the user selects a compound option, behavior depends on the runtime — the Plan Phase N / Execute Phase N handlers below resolve it via `forge_run query config-get runtime`:
...
```
Skill(skill="forge-discuss-phase", args="{PHASE_NUM} {manager_flags.discuss}")
```
...
Skill(skill=\"forge-plan-phase\", args=\"{N} --auto {manager_flags.plan}\")
...
Skill(skill=\"forge-execute-phase\", args=\"{N} {manager_flags.execute}\")
...
- Display the error clearly, then offer to fix it:
  ...
  - "Run this phase inline instead": Dispatch the same action inline via the appropriate Skill — use `Skill(skill="forge-plan-phase", args="{N}")` if the failed action was planning, or `Skill(skill="forge-execute-phase", args="{N}")` if the failed action was execution. Loop to dashboard after.
...
- Display the error, then offer options via AskUserQuestion:
  ...
  - "Run inline instead": Dispatch the action inline via the appropriate Skill — use `Skill(skill="forge-plan-phase", args="{N}")` if the failed action was planning, or `Skill(skill="forge-execute-phase", args="{N}")` if the failed action was execution. Loop to dashboard after.
  - "Skip and continue": Loop to dashboard (phase stays in current state).
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 ...
 Resume anytime: /forge:manager
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```

**Note:** Any background agents still running will continue to completion. Their results will be visible on next `/forge:manager` or `/forge:progress` invocation.
