<purpose>
Generate an AI design contract (AI-SPEC.md) for phases that involve building AI systems. Orchestrates forge-framework-selector → forge-ai-researcher → forge-domain-researcher → forge-eval-planner with a validation gate. Inserts between discuss-phase and plan-phase in the GSD lifecycle.
...
<required_reading>
@~/.claude/forge-core/references/ai-frameworks.md
@~/.claude/forge-core/references/ai-evals.md
</required_reading>
...
```bash
SELECTOR_MODEL=$(forge_run query resolve-model forge-framework-selector 2>/dev/null | jq -r '.model' 2>/dev/null || true)
RESEARCHER_MODEL=$(forge_run query resolve-model forge-ai-researcher 2>/dev/null | jq -r '.model' 2>/dev/null || true)
DOMAIN_MODEL=$(forge_run query resolve-model forge-domain-researcher 2>/dev/null | jq -r '.model' 2>/dev/null || true)
PLANNER_MODEL=$(forge_run query resolve-model forge-eval-planner 2>/dev/null | jq -r '.model' 2>/dev/null || true)
```

Check config:
```bash
AI_PHASE_ENABLED=$(forge_run query config-get workflow.ai_integration_phase 2>/dev/null || echo "true")
```

**If `AI_PHASE_ENABLED` is `false`:**
```
AI phase is disabled in config. Enable via /forge:settings.
```
Exit workflow.
...
Recommended: run /forge:discuss-phase {N} first to capture framework preferences.
...
## 5. Spawn forge-framework-selector
...
Spawn `forge-framework-selector` with:
```markdown
Read ~/.claude/agents/forge-framework-selector.md for instructions.
...
**If selector fails or returns empty:** Exit with error — "Framework selection failed. Re-run /forge:ai-integration-phase {N} or answer the framework question in /forge:discuss-phase {N} first."

## 6. Initialize AI-SPEC.md

Copy template:
```bash
cp "$HOME/.claude/forge-core/templates/AI-SPEC.md" "${PHASE_DIR}/${PADDED_PHASE}-AI-SPEC.md"
```
...
Spawn `forge-ai-researcher` with:
```markdown
Read ~/.claude/agents/forge-ai-researcher.md for instructions.
...
## 8. Spawn forge-domain-researcher
...
Spawn `forge-domain-researcher` with:
```markdown
Read ~/.claude/agents/forge-domain-researcher.md for instructions.
...
Spawn `forge-eval-planner` with:
```markdown
Read ~/.claude/agents/forge-eval-planner.md for instructions.
...
Next step:
  /forge:plan-phase {N}   — planner will consume AI-SPEC.md
```
