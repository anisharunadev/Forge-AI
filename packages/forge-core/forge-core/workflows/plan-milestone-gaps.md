...
HIGHEST=$(forge_run query phases.list --pick directories[-1])
```
...
```bash
forge_run query commit "docs(roadmap): add gap closure phases {N}-{M}" --files .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
