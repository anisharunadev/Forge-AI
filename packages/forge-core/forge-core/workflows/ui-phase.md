<purpose>
Generate a UI design contract (UI-SPEC.md) for frontend phases. Orchestrates forge-ui-researcher and forge-ui-checker with a revision loop. Inserts between discuss-phase and plan-phase in the lifecycle.
...
AGENT_SKILLS_UI=$(forge_run query agent-skills forge-ui-researcher)
AGENT_SKILLS_UI_CHECKER=$(forge_run query agent-skills forge-ui-checker)
```
...
```bash
UI_RESEARCHER_MODEL=$(forge_run query resolve-model forge-ui-researcher --raw)
UI_CHECKER_MODEL=$(forge_run query resolve-model forge-ui-checker --raw)
```

Check config:

```bash
UI_ENABLED=$(forge_run query config-get workflow.ui_phase 2>/dev/null || echo "true")
```

**If `UI_ENABLED` is `false`:**
```
UI phase is disabled in config. Enable via /forge:settings.
```
Exit workflow.
...
Recommended: run /forge:discuss-phase {N} first to capture design preferences.
...
⚡ Sketch findings detected: {SKETCH_FINDINGS_PATH}
   Validated design decisions from /forge:sketch will be loaded into the UI researcher.
...
```markdown
Read ~/.claude/agents/forge-ui-researcher.md for instructions.
...
- {SKETCH_FINDINGS_PATH} (Sketch Findings — validated design decisions, CSS patterns, visual direction from /forge:sketch, if exists)
</files_to_read>
...
Template: ~/.claude/forge-core/templates/UI-SPEC.md
</output>
...
Agent(
  ...
  subagent_type="forge-ui-researcher",
...
```markdown
Read ~/.claude/agents/forge-ui-checker.md for instructions.
...
Agent(
  ...
  subagent_type="forge-ui-checker",
...
- Re-spawn forge-ui-researcher with revision context:
...
2. Edit manually — open UI-SPEC.md in editor, re-run /forge:ui-phase
...
`/clear` then: `/forge:plan-phase {N}`

{If CONTEXT.md does NOT exist:}
**Discuss Phase {N}** — gather implementation context before planning

`/clear` then: `/forge:discuss-phase {N}`

(or `/forge:plan-phase {N}` to skip discussion)
...
```bash
forge_run query commit "docs(${padded_phase}): UI design contract" --files "${PHASE_DIR}/${PADDED_PHASE}-UI-SPEC.md"
```
...
- [ ] forge-ui-researcher spawned with correct context and file paths
- [ ] UI-SPEC.md created in correct location
- [ ] forge-ui-checker spawned with UI-SPEC.md
