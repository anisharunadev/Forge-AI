...
Examples:
  `/forge-edit-phase 5`       → phase = 5, force = false
  `/forge-edit-phase 5 --force` → phase = 5, force = true
  `/forge-edit-phase 12.1`    → phase = 12.1, force = false

If no argument provided:

```
ERROR: Phase number required
Usage: /forge-edit-phase <phase-number> [--force]
Example: /forge-edit-phase 5
Example: /forge-edit-phase 5 --force
```
...
INIT=$(forge_run query init.phase-op "${target}")
...
```bash
PHASE_DATA=$(forge_run query roadmap get-phase "${target}")
```
...
Available phases can be seen with /forge:progress.
```
...
```bash
ALL_PHASES=$(forge_run query roadmap analyze)
```
...
```bash
forge_run query state.add-roadmap-evolution \
  --phase {target} \
  --action edited \
...
- `/forge:progress` — view updated roadmap
- `/forge:plan-phase {target}` — re-plan this phase (if needed)
- `/forge:discuss-phase {target}` — discuss implementation approach
