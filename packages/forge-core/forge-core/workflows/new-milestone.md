...
forge_run query state.milestone-switch --milestone "v[X.Y]" --name "[Name]"
```
...
```bash
forge_run query phases.clear --confirm
```

```bash
forge_run query commit "docs: start milestone v[X.Y] [Name]" --files .planning/PROJECT.md .planning/STATE.md
```

## 7. Load Context and Resolve Models

```bash
INIT=$(forge_run query init.new-milestone)
...
- Tell the user to complete/archive the previous milestone first, then rerun `/forge:new-milestone --reset-phase-numbers ${GSD_WS}`.
...
**IMPORTANT:** Do NOT persist this choice to config.json. The `workflow.research` setting is a persistent user preference that controls plan-phase behavior across the project. Changing it here would silently alter future `/forge:plan-phase` behavior. To change the default, use `/forge:settings`.
...
Use template: ~/.claude/forge-core/templates/research-project/{FILE}
</output>
...
**Commit requirements:**
```bash
forge_run query commit "docs: define milestone v[X.Y] requirements" --files .planning/REQUIREMENTS.md
```
...
```bash
forge_run query commit "docs: create milestone v[X.Y] roadmap ([N] phases)" --files .planning/ROADMAP.md .planning/STATE.md .planning/REQUIREMENTS.md
```
...
```bash
forge_run query commit "docs: tag [count] pending todos with resolves_phase after milestone v[X.Y] roadmap" --files .planning/todos/pending/*.md
```
...
`/forge:discuss-phase [N] ${GSD_WS}` — gather context and clarify approach

Also: `/forge:plan-phase [N] ${GSD_WS}` — skip discussion, plan directly
```
...
- [ ] User knows next step: `/forge:discuss-phase [N] ${GSD_WS}`
