<purpose>
Auto-fix issues from REVIEW.md. Validates phase, checks config gate, verifies REVIEW.md exists and has fixable issues, spawns forge-code-fixer agent, handles --auto iteration loop (capped at 3), commits REVIEW-FIX.md once at the end, and presents results.
</purpose>
...
- forge-code-fixer: Applies fixes to code review findings
- forge-code-reviewer: Reviews source files for bugs and issues
</available_agent_types>
...
AGENT_SKILLS_FIXER=$(forge_run query agent-skills forge-code-fixer)
...
if [ ! -f "${REVIEW_PATH}" ]; then
  echo "Error: No REVIEW.md found for Phase ${PHASE_ARG}. Run /forge:code-review ${PHASE_ARG} first."
  exit 1
fi
```
...
Spawn the forge-code-fixer agent with config (runs in a subagent — no output until it returns, ~1–5 min; expected, not a freeze):
...
```text
Agent(subagent_type="forge-code-fixer", prompt="
<files_to_read>
${REVIEW_PATH}
</files_to_read>
...
You can retry with /forge:code-review ${PHASE_ARG} --fix.
```
...
if [ "$AUTO_MODE" = "true" ]; then
  ...
  - ${f}"
    ...
    # Spawn forge-code-reviewer agent to re-review (runs in a subagent — no output until it returns, ~1–5 min; expected, not a freeze)
...
if [ ! -f "${FIX_REPORT_PATH}" ]; then
  ...
  echo ""
  echo "Retry: /forge:code-review ${PHASE_ARG} --fix"
  echo ""
  echo "═══════════════════════════════════════════════════════════════"
  exit 1
fi
...
if [ "$FIX_STATUS" = "all_fixed" ]; then
  ...
  echo ""
  echo "Next step:"
  echo "  /forge:verify-work  — Verify phase completion"
  echo ""
fi
```
...
if [ "$FIX_STATUS" = "partial" ] || [ "$FIX_STATUS" = "none_fixed" ]; then
  ...
  echo "  /forge:code-review ${PHASE_NUMBER}           — Re-review code"
  echo "  /forge:verify-work                           — Verify phase completion"
  echo ""
fi
```
