...
intentionally isolated from /forge:plan-phase so upstream changes to ultraplan cannot
...
Use /forge:plan-phase for stable local planning.
```
...
/forge:ultraplan-phase requires Claude Code.
ultraplan is not available in this runtime.

Use /forge:plan-phase for local planning instead.
```
...
unplanned phase from the roadmap (same logic as /forge:plan-phase).
...
When ◆ ultraplan ready appears in your terminal:
  ...
  7. Run: /forge:import --from <the file path>

/forge:import will run conflict detection, convert to GSD format,
