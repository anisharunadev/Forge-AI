...
This is a companion to `/forge:settings` — the common-case prompt there covers model profile,
...
- `git.phase_branch_template` (default: `forge/phase-{phase}-{slug}`)
- `git.milestone_branch_template` (default: `forge/{milestone}-{slug}`)
...
AskUserQuestion([
  ...
  {
    question: "Phase branch template? (current: <value or forge/phase-{phase}-{slug}>)",
  ...
  {
    question: "Milestone branch template? (current: <value or forge/{milestone}-{slug}>)",
...
`forge-tools.cjs query config-set runtime <value>` before proceeding to Step C.
...
```bash
forge_run query config-set model_profile_overrides.<runtime>.<tier> "<model-id>"
```

For each tier where the user chose "Clear override", remove the key by setting it to null:
```bash
forge_run query config-set model_profile_overrides.<runtime>.<tier> null
```
...
Apply each selected value via `forge-tools.cjs query config-set <key> <value>` so the central
...
forge_run query config-set workflow.plan_bounce_passes 5
forge_run query config-set workflow.subagent_timeout 300000
forge_run query config-set git.base_branch main
forge_run query config-set context_window 1000000
# Runtime model tier examples:
forge_run query config-set runtime gemini
forge_run query config-set model_profile_overrides.gemini.opus gemini-3-ultra
forge_run query config-set model_profile_overrides.gemini.haiku null
```
...
route each write through `forge-tools.cjs query config-set` so sibling preservation is handled by
the central setter.
</step>
...
forge_run query config-set model_policy.provider "<provider>"   # e.g., anthropic / anthropic-fable / openai / google / qwen
forge_run query config-set model_policy.budget   "<budget>"    # high / medium / low
forge_run query config-set model_policy.high     "<high-id>"
forge_run query config-set model_policy.medium   "<medium-id>"
forge_run query config-set model_policy.low      "<low-id>"

# Generic-provider path — write only tiers the user changed ("Keep current" skipped):
forge_run query config-set model_policy.provider "custom"
forge_run query config-set model_policy.budget   null
# Per-tier writes for each non-"Keep current" answer:
forge_run query config-set model_policy.high     "<high-id>"   # omit if user chose "Keep current"
forge_run query config-set model_policy.medium   "<medium-id>" # omit if user chose "Keep current"
forge_run query config-set model_policy.low      "<low-id>"    # omit if user chose "Keep current"
```
...
These settings apply to future /forge:plan-phase, /forge:execute-phase, /forge:discuss-phase,
and /forge:ship runs.

For common-case toggles (model profile, research/plan_check/verifier, branching strategy,
UI/AI phase gates), use /forge:settings.
```
</step>
...
- [ ] Writes routed through `forge-tools.cjs query config-set` so unrelated keys are preserved
