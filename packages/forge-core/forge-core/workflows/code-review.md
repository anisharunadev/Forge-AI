<purpose>
Review source files changed during a phase for bugs, security issues, and code quality problems. Computes file scope (--files override > SUMMARY.md > git diff fallback), checks config gate, spawns forge-code-reviewer agent, commits REVIEW.md, and presents results to user. When --fix is passed, delegates to code-review-fix.md after review to auto-apply findings via forge-code-fixer.
</purpose>
...
- forge-code-reviewer: Reviews source files for bugs and quality issues
- forge-code-fixer: Applies fixes to code review findings (used via dispatch_fix → code-review-fix.md when --fix is passed)
</available_agent_types>
...
AGENT_SKILLS_REVIEWER=$(forge_run query agent-skills forge-code-reviewer)
```
...
```
Error: Phase ${PHASE_ARG} not found. Run /forge:progress to see available phases.
```
...
FLAGS_JSON=$(node -e "
  const { parseCodeReviewFlags } = require('./forge-core/bin/lib/code-review-flags.cjs');
...
2. Config value: `forge-tools.cjs query config-get workflow.code_review_depth 2>/dev/null`
3. Default: "standard"
...
if [ -n "$DEPTH_OVERRIDE" ]; then
  ...
  CONFIG_DEPTH=$(forge_run query config-get workflow.code_review_depth 2>/dev/null || echo "")
...
if [ ${#REVIEW_FILES[@]} -eq 0 ]; then
  ...
  if [ -n "$PHASE_COMMITS" ]; then
    ...
    echo "Use --files flag to specify files explicitly: /forge:code-review ${PHASE_ARG} --files=file1,file2,..."
  fi
fi
```
...
```bash
FALLOW_ENABLED=$(forge_run query config-get code_quality.fallow.enabled 2>/dev/null || echo "false")
FALLOW_SCOPE=$(forge_run query config-get code_quality.fallow.scope 2>/dev/null || echo "phase")
FALLOW_PROFILE=$(forge_run query config-get code_quality.fallow.profile 2>/dev/null || echo "standard")
FALLOW_MCP=$(forge_run query config-get code_quality.fallow.mcp 2>/dev/null || echo "false")
...
const { resolveFallowBinary } = require('./forge-core/bin/lib/fallow-runner.cjs');
...
if [ -n "$FALLOW_JSON_PATH" ] && [ -f "$FALLOW_JSON_PATH" ]; then
  ...
  FALLOW_SRC="$FALLOW_JSON_PATH" FALLOW_OUT="$FALLOW_NORMALIZED_PATH" node -e "
    const fs = require('fs');
    const { normalizeFallowReportFile } = require('./forge-core/bin/lib/fallow-runner.cjs');
...
No REVIEW.md created. You can retry with /forge:code-review ${PHASE_ARG} or check agent logs.
```
...
if [ -f "${REVIEW_PATH}" ]; then
  ...
  echo "No REVIEW.md to commit. Please retry with /forge:code-review ${PHASE_ARG}"
fi
```
</step>
...
if [ "$FIX_FLAG" = "true" ]; then
  ...
  # forge-code-fixer agent dispatch, --auto iteration loop, REVIEW-FIX.md commit,
  # and result presentation. Do not duplicate that logic here.
  Workflow(workflow="forge-core/workflows/code-review-fix.md", args="${FIX_ARGS}")
...
Next steps:
  /forge:code-review ${PHASE_NUMBER} --fix  — Auto-fix issues
