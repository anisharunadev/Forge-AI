...
forge-plan-phase N → forge-review N --codex → forge-plan-phase N --reviews → forge-review N --codex → ...
Plan-phase runs inline (bare Skill at depth 0) so it can spawn forge-planner/forge-plan-checker at depth 1.
...
@$HOME/.claude/forge-core/references/revision-loop.md
@$HOME/.claude/forge-core/references/gates.md
@$HOME/.claude/forge-core/references/agent-contracts.md
</required_reading>
...
Then re-run: /forge:plan-review-convergence {PHASE}
```

## 2. Initialize

```bash
INIT=$(forge_run init plan-phase "$PHASE")
...
```bash
PHASE_INFO=$(forge_run roadmap get-phase "${PHASE}")
```
...
Agent(
  ...
  prompt="Run /forge:review for Phase {PHASE}.

Execute: Skill(skill='forge-review', args='--phase {PHASE} {REVIEWER_FLAGS} {GSD_WS}')
...
Definitions:
  ...
  ACTIONABLE — a non-HIGH review finding that would be invisible to /forge:execute-phase unless it is incorporated into PLAN.md or explicitly deferred/rejected in PLAN.md.
...
4a. **Resolve effective authority** (deterministic — replaces manual `intel.enabled` reasoning):
   ```bash
   EFFECTIVE_AUTHORITY=$(forge_run drift-guard authority --raw)
   ```
4. **Severity & gating** — classify each symbol's verdict using the seam (do not apply the table manually):
   ```bash
   # For each symbol, e.g.:
   RESULT=$(forge_run drift-guard severity --status <verdict> --authority "$EFFECTIVE_AUTHORITY")
...
```bash
forge_run state planned-phase --phase "${PHASE}" --name "${phase_name}" --plans "${PLAN_COUNT}"
```
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 ...
 Next: /forge:execute-phase {PHASE}
```
...
To replan manually:  /forge:plan-phase {PHASE} --reviews
To restart loop:     /forge:plan-review-convergence {PHASE} {REVIEWER_FLAGS}
```
Exit workflow.
...
- [ ] Initial planning via inline Skill("forge-plan-phase") if no plans exist — NOT wrapped in Agent() (bug #936: depth-1 Agent has no Agent tool)
- [ ] Review via Agent → Skill("forge-review") — isolated Agent is correct; forge-review is a Bash leaf with no sub-agent spawns; {GSD_WS} forwarded
- [ ] Replan via inline Skill("forge-plan-phase --reviews") — NOT wrapped in Agent(); inline lets plan-phase spawn forge-planner/forge-plan-checker at depth 1
...
- [ ] The review Agent fully completes forge-review before returning (plan-phase runs inline — no Agent wrap)
