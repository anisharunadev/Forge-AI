...
```bash
slug=$(forge_run query generate-slug "$title" --raw)
```
...
```bash
forge_run query commit "docs: capture todo - [title]" --files .planning/todos/pending/[filename] .planning/STATE.md
```
...
2. Add another todo
3. View all todos (/forge:capture --list)
```
</step>
