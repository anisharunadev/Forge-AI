...
<trigger>
This workflow executes when `--power` flag is present in ARGUMENTS to `/forge:discuss-phase`.
...
Consider running /forge:discuss-phase {N} again to refine before planning.
```
