...
> `$HOME/.claude/forge-core/USER-PROFILE.md` exists). Skip the Read
...
ADVISOR_MODEL=$(forge_run query resolve-model forge-advisor-researcher --raw)
```
...
```bash
PROFILE_CONTENT=$(cat "$HOME/.claude/forge-core/USER-PROFILE.md" 2>/dev/null || true)
```
...
2. For EACH user-selected gray area, spawn a `Agent()` in parallel:
   ...
   ```
   Agent(
     prompt="First, read @~/.claude/agents/forge-advisor-researcher.md for your role and instructions.
