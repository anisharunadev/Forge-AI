...
Saves artifacts to `.planning/spikes/`. Companion to `/forge:spike --wrap-up`.
...
```bash
forge_run query commit "docs(spike-NNN): [VERDICT] — [key finding]" --files .planning/spikes/NNN-descriptive-name/ .planning/spikes/MANIFEST.md
```
...
```bash
forge_run query commit "docs(spikes): update conventions" --files .planning/spikes/CONVENTIONS.md
```
</step>
...
**Also available:**
- `/forge:spike` — spike more ideas (or run with no argument for frontier mode)
- `/forge:plan-phase` — start planning the real implementation
- `/forge:explore` — continue exploring the idea
