...
@~/.claude/forge-core/references/git-integration.md
</required_reading>
...
INIT=$(forge_run query init.execute-phase "${PHASE}")
...
# config settings can be fetched via forge-tools.cjs query config-get if needed
```
...
INLINE_THRESHOLD=$(forge_run query config-get workflow.inline_plan_threshold 2>/dev/null || echo "2")
...
```bash
forge_run query phases.list --type summaries --raw
...
Apply deviation rules from the forge-executor agent definition (single source of truth):
...
See `~/.claude/forge-core/references/tdd.md` for structure.
</tdd_plan_execution>
...
See ~/.claude/forge-core/references/checkpoints.md for details.
</step>
...
```bash
NODE_REPAIR=$(forge_run query config-get workflow.node_repair 2>/dev/null || echo "true")
```

If `NODE_REPAIR` is `true`: invoke `@./.claude/forge-core/workflows/node-repair.md` with:
...
If user_setup exists: create `{phase}-USER-SETUP.md` using template `~/.claude/forge-core/templates/user-setup.md`. Per service: env vars table, account setup checklist, dashboard config, local dev notes, verification commands. Status "Incomplete". Set `USER_SETUP_CREATED=true`. If empty/missing: skip.
</step>
...
Create `{phase}-{plan}-SUMMARY.md` at `.planning/phases/XX-name/`. Use `~/.claude/forge-core/templates/summary.md`.
...
Update STATE.md using forge-tools.cjs query (or legacy forge-tools) state mutations:
...
if [ "$IS_WORKTREE" != "true" ]; then
  ...
  forge_run query state.advance-plan

  # Recalculate progress bar from disk state
  forge_run query state.update-progress

  # Record execution metrics
  forge_run query state.record-metric \
...
forge_run query state.add-decision \
  --phase "${PHASE}" --summary-file "${DECISION_TEXT_FILE}" --rationale-file "${RATIONALE_FILE}"

# Add blockers if any found
forge_run query state.add-blocker --text-file "${BLOCKER_TEXT_FILE}"
```
</step>

<step name="update_session_continuity">
Update session info using forge-tools.cjs query (or legacy forge-tools):

```bash
forge_run query state.record-session \
...
if [ "$IS_WORKTREE" != "true" ]; then
  ...
  forge_run query roadmap.update-plan-progress "${PHASE}"
fi
```
...
```bash
forge_run query requirements.mark-complete ${REQ_IDS}
```
...
if [ "$IS_WORKTREE" = "true" ]; then
  forge_run query commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/REQUIREMENTS.md
else
  forge_run query commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/STATE.md .planning/ROADMAP.md .planning/REQUIREMENTS.md
fi
```
</step>
...
```bash
forge_run query commit "" --files .planning/codebase/*.md --amend
```
</step>
...
|-----------|-------|--------|
| summaries < plans | **A: More plans** | Find next PLAN without SUMMARY — skip any plan whose `plan_id` matches a non-terminal async-job manifest (`external_job_waiting`; see `identify_plan`). Yolo: auto-continue. Interactive: show next plan, suggest `/forge:execute-phase {phase}` + `/forge:verify-work`. STOP here. |
