<purpose>
Interactive configuration of GSD workflow agents (research, plan_check, verifier) and model profile selection via multi-question prompt. Updates .planning/config.json with user preferences. Optionally saves settings as global defaults (~/.forge/defaults.json) for future projects.
</purpose>
...
forge_run query config-ensure-section
INIT=$(forge_run query state.load)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
# `state.load` returns STATE frontmatter JSON from the SDK — it does not include `config_path`. Orchestrators may set `GSD_CONFIG_PATH` from init phase-op JSON; otherwise resolve the same path forge-tools uses for flat vs active workstream (#2282).
...
- `workflow.pattern_mapper` — run forge-pattern-mapper between research and planning (default: true if absent)
- `workflow.ui_phase` — generate UI-SPEC.md design contracts for frontend phases (default: true if absent)
- `workflow.ui_safety_gate` — prompt to run /forge:ui-phase before planning frontend phases (default: true if absent)
...
- `workflow.code_review` — enable /forge:code-review and /forge:code-review --fix commands (default: true if absent)
- `workflow.code_review_depth` — default depth for /forge:code-review: `quick`, `standard`, or `deep` (default: `"standard"` if absent; only relevant when `code_review` is on)
- `workflow.ui_review` — run visual quality audit (/forge:ui-review) in autonomous mode (default: true if absent)
- `commit_docs` — whether `.planning/` files are committed to git (default: true if absent)
- `intel.enabled` — enable queryable codebase intelligence (/forge:map-codebase --query) (default: false if absent)
- `graphify.enabled` — enable project knowledge graph (/forge:graphify) (default: false if absent)
...
- `model_policy.provider` — provider slug for model policy (default: `null`; known values: anthropic, openai, google, qwen; set via /forge:config --advanced)
- `model_policy.budget` — budget level for model policy (default: `null`; known values: high, medium, low; set via /forge:config --advanced)
- `model_policy.high` — model ID for high-cost tier (default: `null`; set via /forge:config --advanced)
- `model_policy.medium` — model ID for medium-cost tier (default: `null`; set via /forge:config --advanced)
- `model_policy.low` — model ID for low-cost tier (default: `null`; set via /forge:config --advanced)
</step>
...
AskUserQuestion([
  ...
  {
    question: "Enable Code Review? (/forge:code-review and /forge:code-review --fix commands)",
    header: "Code Review",
    multiSelect: false,
    options: [
      { label: "Yes (Recommended)", description: "Enable /forge:code-review commands for reviewing source files changed during a phase." },
  ...
  {
    question: "Code Review Depth? (default depth for /forge:code-review — override per-run with --depth=)",
    header: "Review Depth",
    multiSelect: false,
    options: [
  ...
  {
    question: "Enable UI Review? (visual quality audit via /forge:ui-review in autonomous mode)",
    header: "UI Review",
    multiSelect: false,
    options: [
  ...
  },
  {
    ...
    multiSelect: false,
    options: [
      { label: "Yes (Recommended)", description: "forge-pattern-mapper runs between research and plan steps. Surfaces conventions so new code follows house style." },
  ...
  {
    question: "Enable UI Safety Gate? (prompts to run /forge:ui-phase before planning frontend phases)",
    header: "UI Gate",
    multiSelect: false,
    options: [
      { label: "Yes (Recommended)", description: "plan-phase asks to run /forge:ui-phase first when frontend indicators detected." },
  ...
  },
  {
    ...
    multiSelect: false,
    options: [
      { label: "Yes (Recommended)", description: "Run /forge:ai-integration-phase before planning AI system phases. Surfaces the right framework, researches its docs, and designs the evaluation strategy." },
  ...
  },
  {
    ...
    options: [
      ...
      { label: "Yes", description: "Skip discuss in /forge:autonomous — chain directly to plan. Best for backend/pipeline work where phase descriptions are the spec." }
    ]
  },
  {
  ...
  {
    question: "Enable Intel? (queryable codebase intelligence via /forge:map-codebase --query — builds a JSON index in .planning/intel/)",
    header: "Intel",
    multiSelect: false,
    options: [
      { label: "No (Recommended)", description: "Skip intel indexing. Use when codebase is small or intel queries are not needed." },
      { label: "Yes", description: "Enable /forge:map-codebase --query commands. Builds and queries a JSON index of the codebase." }
    ]
  },
  {
    question: "Enable Graphify? (project knowledge graph via /forge:graphify — builds a graph in .planning/graphs/)",
    header: "Graphify",
    multiSelect: false,
    options: [
      { label: "No (Recommended)", description: "Skip knowledge graph. Use when dependency graphs are not needed." },
      { label: "Yes", description: "Enable /forge:graphify commands. Builds and queries a project knowledge graph." }
    ]
  },
  {
    ...
    multiSelect: false,
    options: [
      { label: "No (Recommended)", description: "Manual /forge:graphify build only. Conservative default — opt in if you want fresh context on every /forge:quick or /forge:plan-phase." },
...
```json
{
  ...
  "model_policy": {
    // Read-only in this flow — written only by /forge:config --advanced (Section 8).
...
- **Capability hook-gate keys** (owned by a capability in the registry — see `registry.configSchema`): write via the capability writer:
  ```bash
  forge_run capability set <owner> --gate <key>=<value> [--config-dir "$RUNTIME_CONFIG_DIR"]
  ```
...
Quick commands:
- /forge:config --integrations — configure API keys (Brave/Firecrawl/Exa), review.models CLI routing, and agent_skills injection
- /forge:config --profile <profile> — switch model profile
...
- /forge:config --advanced — power-user tuning (plan bounce, timeouts, branch templates, cross-AI, context window, model policy)
```
</step>
