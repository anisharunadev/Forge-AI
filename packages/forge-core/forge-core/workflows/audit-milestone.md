...
AGENT_SKILLS_CHECKER=$(forge_run query agent-skills forge-integration-checker)
```
...
```bash
integration_checker_model=$(forge_run query resolve-model forge-integration-checker --raw)
```
...
PHASE_INFO=$(forge_run query find-phase 01 --raw)
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  [ -e "$summary" ] || continue
  forge_run query summary-extract "$summary" --fields requirements_completed --pick requirements_completed
done
```
...
Discovery only — never auto-calls `/forge:validate-phase`.
...
|-------|---------------|-----------|--------|
| {phase} | exists/missing | true/false/partial | `/forge:validate-phase {N}` |

Phases needing validation: run `/forge:validate-phase {N}` for each flagged phase.
...
/forge:phase --insert <N> "Close gap: <REQ-ID> — <description>"
/forge:discuss-phase <N>
/forge:plan-phase <N>
/forge:execute-phase <N>

For Nyquist-coverage gaps flagged in the table above, prefer running
`/forge:validate-phase <N>` for each flagged phase (and `/forge:secure-phase
...
/forge:phase --insert <N> "Address tech debt: <area>"
/forge:discuss-phase <N>
/forge:plan-phase <N>
/forge:execute-phase <N>
