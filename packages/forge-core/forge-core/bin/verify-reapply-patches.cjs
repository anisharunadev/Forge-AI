...
/**
 * Deterministic verifier for the /forge-reapply-patches Step 5 "Hunk Verification
 ...
 *     --patches-dir <path>        \  # forge-local-patches/
 *     --config-dir <path>         \  # ~/.claude (or runtime equivalent)
 *     [--pristine-dir <path>]        # forge-pristine/; if absent, falls back to
...
function normalizeUpstreamOwnedLine(line) {
  ...
  if (GSD_HOOK_VERSION_LINE_RE.test(trimmed)) {
    ...
    return `${prefix} forge-hook-version: __GSD_VERSION_TOKEN__`;
  }
  return line;
}
...
const REASON = Object.freeze({
  ...
  // Bug #3657: the on-disk forge-pristine/ file's SHA-256 does not match the
  ...
  // forge-pristine/ file is absent from disk.  This happens on post-#604-rename
  ...
  if (pristineDir) {
    ...
    try {
      ...
      if (stat.isFile()) {
        ...
        // means the installer refreshed forge-pristine/ to a newer GSD version
        ...
        if (recordedHash) {
          if (sha256(candidate) === recordedHash) {
            ...
            // Hash mismatch: the on-disk forge-pristine/ was refreshed to a newer
    ...
    // forge-pristine/ file (the only candidate was discarded because it was from
