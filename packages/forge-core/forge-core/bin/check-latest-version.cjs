...
/**
 * Deterministic latest-version check for /forge-update (#2992).
 *
 * The /forge-update workflow's check_latest_version step was previously
 * prescribed in LLM-driven prose ("run `npm view forge-core
 * version`"). The executing model could shortcut the prescription and
 * invent npm queries against wrong-shaped names (`@forge-core/cli`,
 ...
 * `npm view @forge-ai/forge-core@next version` resolves the RC channel (#660).
 */
...
function checkLatestVersion(opts = {}) {
  ...
  // Bounded at 15s so a hung registry doesn't block /forge-update (#2993 CR).
  const defaultSpawn = () => {
