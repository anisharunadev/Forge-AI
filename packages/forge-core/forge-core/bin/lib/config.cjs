 ...
 *   2. User-level defaults from ~/.forge/defaults.json (if present)
 *   3. userChoices — the settings the user explicitly selected during /forge:new-project
 *
 ...
 * configured during /forge:new-project). All remaining keys are filled from
 * hardcoded defaults and optional ~/.forge/defaults.json.
 *
 ...
 * users can run `forge-tools migrate-config` to apply all four legacy-key
