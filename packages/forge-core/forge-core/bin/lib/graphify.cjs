...
function disabledResponse() {
    return { disabled: true, message: 'graphify is not enabled. Enable with: forge-tools config-set graphify.enabled true' };
}
// ─── Subprocess Helper ───────────────────────────────────────────────────────
/**
...
function graphifyStatus(cwd) {
    ...
    // hooks/forge-graphify-update.sh PostToolUse hook (opt-in via graphify.auto_update,
    // default false). When the most recent auto-build is "failed" or still "running",
    // fold that into the existing `stale: true` signal so consumers (forge-planner,
    // forge-phase-researcher) surface the standard "treat semantic relationships as
