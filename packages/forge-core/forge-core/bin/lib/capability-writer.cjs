 ...
 *   - enabled axis: mutates .forge-surface.json via readSurface/writeSurface
...
// ExitError (NOT process.exit) is how every forge-tools command signals a non-zero exit: runMain
    ...
    for (const entry of desired) {
        if (entry.enabled === true) {
            ...
            if (afterCap && afterCap.enabled !== true) {
                errors.push(`cannot enable "${entry.id}": not in the active surface/profile (widen the profile or use /forge:surface enable)`);
            }
        }
...
/**
 * CLI command entry point for `forge-tools capability set`.
 *
