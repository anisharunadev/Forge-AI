 ...
 *   2. For claude runtime: __dirname-relative path (agents/ sibling of forge-core/)
 *      This is correct for both repo runs and real installs (the runtime config dir's
 *      agents/ folder) because forge-tools.cjs lives inside forge-core/bin/ in both cases.
...
function checkAgentsInstalled(runtime) {
    ...
    // If a forge-file-manifest.json exists alongside the agents dir (parent dir),
    ...
    const incomplete = [];
    const manifestPath = node_path_1.default.join(node_path_1.default.dirname(agentsDir), 'forge-file-manifest.json');
    let manifestFiles = {};
    try {
