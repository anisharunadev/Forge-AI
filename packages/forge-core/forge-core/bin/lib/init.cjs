...
function cmdInitManager(cwd, raw) {
    ...
    const sanitizeFlags = (rawVal) => {
        ...
        if (!safe) {
            process.stderr.write(`forge-tools: warning: manager.flags contains invalid tokens, ignoring: ${val}\n`);
            return '';
        }
        return val;
    };
...
function cmdInitNewWorkspace(cwd, raw) {
    ...
    const defaultBase = node_path_1.default.join(homedir, 'forge-workspaces');
...
function buildSkillManifest(cwd, skillsDir = null) {
    const canonicalRoots = skillsDir
        ...
        : [
            ...
            },
            {
                ...
                path: node_path_1.default.join(node_os_1.default.homedir(), '.claude', 'forge-core', 'skills'),
                scope: 'import-only',
                kind: 'skills',
                deprecated: true,
            },
            {
                ...
                path: node_path_1.default.join(node_os_1.default.homedir(), '.claude', 'commands', 'forge'),
    ...
    for (const rootInfo of canonicalRoots) {
        ...
        // (e.g. spec-phase nested under both forge-ns-workflow and forge-ns-manage).
        ...
        for (const entry of entries) {
            ...
            // Scope to forge-ns-* routers only: never vacuum up an unrelated user skill
            // that happens to have its own `skills/` subdirectory.
            if (!entry.name.startsWith('forge-ns-'))
                continue;
    ...
    const gsdSkillsInstalled = skills.some((skill) => skill.name.startsWith('forge-'));
    return {
        skills,
        roots,
        installation: {
