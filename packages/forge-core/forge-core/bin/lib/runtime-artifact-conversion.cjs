...
// forge-tools invocation (this module sits in the forge-tools loader chain) and
...
//   1. Installed trees carry <root>/forge-core/VERSION (written by the installer);
//      this module lives at <root>/forge-core/bin/lib, so VERSION is two dirs up.
//   2. The source / npm-package tree has no forge-core/VERSION but carries a real
...
function resolveVersionFrom(libDir) {
    ...
    catch { /* not an installed tree (no forge-core/VERSION) */ }
    try {
...
function buildKimiAgentArtifacts({ rootAgent = '', subagents = [], requestedSubagents = null, } = {}) {
    ...
    for (const source of subagentSources) {
        ...
        if (!name || !KIMI_CANONICAL_GSD_AGENT_RE.test(name)) {
            addKimiAgentDiagnostic(diagnostics, 'kimi_invalid_subagent_name', 'Subagent source does not use a canonical forge-* Kimi agent name.', name || '(missing)', parsed.path);
            continue;
        }
        const description = frontmatter
...
/**
 * Canonical list of every non-Claude runtime that forge-core emits artifacts for.
...
function applyRuntimeContentRewritesForCommandsInPlace(stagedDir, runtime, pathPrefix, isGlobal = false, attribution = undefined) {
    if (!node_fs_1.default.existsSync(stagedDir))
        return stagedDir;
    const tempDir = node_fs_1.default.mkdtempSync(node_path_1.default.join(node_os_1.default.tmpdir(), 'forge-cmd-rewrites-'));
    try {
