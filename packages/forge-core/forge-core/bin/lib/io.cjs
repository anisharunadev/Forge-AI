 ...
 */
function reapStaleTempFiles(prefix = 'forge-', { maxAgeMs = 5 * 60 * 1000, dirsOnly = false } = {}) {
    try {
        ensureGsdTempDir();
        const now = Date.now();
...
function output(result, raw, rawValue) {
    ...
    if (raw && rawValue !== undefined) {
        ...
        if (json.length > 50000) {
            reapStaleTempFiles();
            ensureGsdTempDir();
            const tmpPath = node_path_1.default.join(GSD_TEMP_DIR, `forge-${Date.now()}.json`);
 ...
 * instead of plain "Error: <message>" text. Set by forge-tools.cjs when the
