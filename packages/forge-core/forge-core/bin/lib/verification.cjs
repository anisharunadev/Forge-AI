 ...
 * forge-core/bin/lib/verification.cjs (gitignored).
 *
...
// ─── Constants ────────────────────────────────────────────────────────────────
/** The set of status values that the forge-verifier agent emits. */
 ...
 * internally — the verifier (forge-verifier.md) never emits them. The verifier only
...
const VERIFICATION_ROUTING_TABLE = {
    ...
    missing: {
        ...
        next_command: '/forge:execute-phase',
    },
...
function readVerificationStatus(phaseDir, opts = {}) {
    ...
    if (rawStatus in VERIFICATION_ROUTING_TABLE && rawStatus !== 'missing' && rawStatus !== 'unknown') {
        ...
        const next_command = rawStatus === 'gaps_found'
            ? `/forge:plan-phase ${phaseNumber} --gaps`
            : entry.next_command;
        return {
            status: entry.status,
