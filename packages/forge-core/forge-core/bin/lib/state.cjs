...
// hasn't changed within the same forge-tools invocation.
...
function stateReplaceFieldWithFallback(content, primary, fallback, value) {
    ...
    process.stderr.write(`[forge-tools] WARNING: STATE.md field "${primary}"${fallback ? ` (fallback: "${fallback}")` : ''} not found — update skipped. ` +
 ...
 * File is written to .planning/WAITING.json (or .forge/WAITING.json if .gsd exists).
 * Fixes #1034.
 */
