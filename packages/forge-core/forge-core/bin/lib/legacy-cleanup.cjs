 ...
 * Deliberately excludes 'forge-core' — the current package's own infra and
 ...
 * directory — e.g. `@$HOME/.codex/get-shit-done/workflows/plan.md`. After // forge-allow-legacy-name
 * the rename to `forge-core/` (#604), those embedded paths are stale and the
 * skill file must be removed so the runtime does not pick up the wrong copy.
 *
 * Issue: #1453
 */
const LEGACY_SKILL_PATH_SIGNAL = 'get-shit-done'; // forge-allow-legacy-name

/**
 * Prefix that identifies a skill directory as GSD-managed.
 * Only `forge-*` subdirectories under the `skills/` subtree are scanned; user
 * skill directories with other prefixes are never touched.
 */
const GSD_SKILL_DIR_PREFIX = 'forge-';
 ...
 *
 * WHY: The current @forge-ai/forge-core package ships ZERO references to the old
 ...
 * installer to delete the freshly-installed forge-core/CHANGELOG.md,
 ...
 * (`get-shit-done` as a path component inside an @-import or similar reference). // forge-allow-legacy-name
 * Skips unreadable files (returns false on any error).
 *
 * @param {string} absPath
 * @param {object} fsMod
...
function fileContainsLegacySkillPathSignal(absPath, fsMod) {
  try {
    const content = fsMod.readFileSync(absPath, 'utf8');
    return content.includes('/' + LEGACY_SKILL_PATH_SIGNAL + '/'); // forge-allow-legacy-name
  } catch {
    return false;
  }
}
 ...
 *     a path reference to the pre-rename `get-shit-done/` runtime directory // forge-allow-legacy-name
 *     (skills/ subtree, forge-* directories only). Issue #1453.
...
function planLegacyCleanup(configDirs, opts = {}) {
  ...
  for (const configDir of configDirs) {
    ...
    // #1453: Scan skills/forge-* directories for stale get-shit-done path references. // forge-allow-legacy-name
    //
    // Background: older GSD installs wrote SKILL.md files that embedded a path to
    // the GSD runtime config directory, e.g.:
    //   @$HOME/.codex/get-shit-done/workflows/docs-update.md // forge-allow-legacy-name
    //
    // After the rename to forge-core/ (#604), the correct path is:
    //   @$HOME/.codex/forge-core/workflows/docs-update.md
    //
    // When Codex upgrades to forge-core 1.5.0 it writes fresh skill files to
    ...
    //
    // This scan removes GSD-managed skill files (under forge-* subdirs) that still
    ...
    if (skillDirEntries) {
      for (const entry of skillDirEntries) {
        // Only process forge-* subdirectories (GSD-managed skill dirs).
        ...
        for (const absPath of files) {
          ...
          if (SKILL_MD_EXTENSIONS.has(ext) && fileContainsLegacySkillPathSignal(absPath, fsMod)) {
            addCandidate(absPath, 'stale-get-shit-done-path'); // forge-allow-legacy-name
          }
        }
      }
    }
  ...
  const legacyCachePath = path.join(homeDir, '.cache', 'forge', 'forge-update-check.json');
  try {
