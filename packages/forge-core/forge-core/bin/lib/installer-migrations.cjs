...
const INSTALL_MIGRATION_LOCK_NAME = 'forge-install-migration.lock';
...
function applyInstallerMigrationPlan({ configDir, plan, now = () => new Date().toISOString(), }) {
    ...
    const journalRelPath = node_path_1.default.posix.join('forge-migration-journal', `${runId}.json`);
    const journalPath = node_path_1.default.join(configDir, journalRelPath);
    const rollbackRootRelPath = node_path_1.default.posix.join('forge-migration-journal', `${runId}-rollback`);
    const rollbackRoot = node_path_1.default.join(configDir, rollbackRootRelPath);
    const backupRootRelPath = node_path_1.default.posix.join('forge-migration-journal', `${runId}-backups`);
