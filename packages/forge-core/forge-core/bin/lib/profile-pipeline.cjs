...
async function cmdExtractMessages(projectArg, options, raw, overridePath) {
    ...
    reapStaleTempFiles('forge-pipeline-', { dirsOnly: true });
    ensureGsdTempDir();
    const tmpDir = node_fs_1.default.mkdtempSync(node_path_1.default.join(GSD_TEMP_DIR, 'forge-pipeline-'));
...
async function cmdProfileSample(overridePath, options, raw) {
    ...
    reapStaleTempFiles('forge-profile-', { dirsOnly: true });
    ensureGsdTempDir();
    const tmpDir = node_fs_1.default.mkdtempSync(node_path_1.default.join(GSD_TEMP_DIR, 'forge-profile-'));
