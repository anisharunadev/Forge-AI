...
function cmdWriteProfile(cwd, options, raw) {
    ...
    if (!outputPath) {
        ...
        outputPath = node_path_1.default.join((0, runtime_homes_cjs_1.getGlobalConfigDir)(effectiveRuntime), 'forge-core', 'USER-PROFILE.md');
    }
...
function cmdGenerateDevPreferences(cwd, options, raw) {
    ...
    // commands/forge subdirectory in favor of skills/<skill>/SKILL.md under
    ...
    // location so /forge:profile-user --refresh stops re-creating the legacy
    ...
    if (!outputPath) {
        ...
        const skillDir = (0, runtime_homes_cjs_1.getGlobalSkillDir)(effectiveRuntime, 'forge-dev-preferences');
        if (!skillDir) {
...
function cmdGenerateClaudeMd(cwd, options, raw) {
    ...
    try {
        ...
        // new-project.md bash workflow via `forge-tools query
