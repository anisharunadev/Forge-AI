 ...
 * form (`name: forge-<cmd>`). The legacy colon form `/forge:<cmd>` is no longer
 ...
 *   - claude, cursor, opencode, kilo, etc.:    /forge-<cmd>
 *
...
function formatGsdSlash(commandName, runtime) {
    ...
    // both legacy `/forge:<name>` and canonical hyphen-form input (plus the bare
    // `forge:<name>` shorthand and codex `$gsd-<name>` shell-var input).
    ...
    // Defensive: a degenerate input like `/forge:`, `forge-`, or whitespace-only
