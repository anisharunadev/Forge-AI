...
// ---------------------------------------------------------------------------
/**
 * Locate the GSD commands/forge source directory.
 *
 * Resolution order:
...
function findInstallSourceRoot(runtimeConfigDir) {
    ...
    let dir = __dirname;
    for (let i = 0; i < 6; i++) {
        const candidate = node_path_1.default.join(dir, 'commands', 'forge');
    ...
    throw new Error(`findInstallSourceRoot: could not locate commands/forge from ${__dirname}`);
}
/**
 ...
 * agent stem — e.g. `forge-planner.md`).
 *
 ...
 * `/forge:surface` / `--materialize` (`src/surface.cts`) — does not yet mirror the
...
function kimiAgentsKind(destSubpath, prefix, configDir) {
    return {
        ...
        stage: (resolved) => {
            ...
            const stageDir = node_fs_1.default.mkdtempSync(node_path_1.default.join(node_os_1.default.tmpdir(), 'forge-kimi-agents-'));
 ...
 * @param prefix        filename prefix, e.g. 'forge-'
 * @param converterName name of converter function in Runtime Artifact Conversion exports
 * @param configDir     runtime config dir (for .gsd-source marker resolution)
 */
