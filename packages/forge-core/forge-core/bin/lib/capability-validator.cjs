...
function validateVersionEnvelope(cap) {
  ...
  if (cap.engines !== undefined) {
    if (typeof cap.engines !== 'object' || cap.engines === null || Array.isArray(cap.engines)) {
      errors.push('engines must be an object (e.g. { "forge": ">=1.6.0" })');
  ...
  if (cap.compatVersions !== undefined) {
    if (typeof cap.compatVersions !== 'object' || cap.compatVersions === null || Array.isArray(cap.compatVersions)) {
      errors.push('compatVersions must be an object mapping capability versions to forge version ranges');
    } else {
...
function validateStep(step, prefix, declaredSkills, declaredAgents) {
  ...
  if (typeof step.ref !== 'object' || step.ref === null) {
    ...
    } else if (hasSkill && typeof step.ref.skill === 'string' && step.ref.skill.startsWith('forge-')) {
      // Double-prefix guard: ref.skill is an unprefixed stem (e.g. "ui-review").
      // Workflow dispatch prepends "forge-" at runtime → "gsd-ui-review".
      // A stem that already starts with "forge-" would produce "gsd-gsd-..." at dispatch.
      errors.push(
        prefix + '.ref.skill "' + step.ref.skill + '" must not start with "forge-" ' +
        '(it is an unprefixed stem; the workflow prepends "forge-" at dispatch — ' +
        'starting with "forge-" would produce "gsd-' + step.ref.skill + '")',
      );
