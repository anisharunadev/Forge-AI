 ...
 * Scans the full content for `forge-<stem>` patterns that correspond to
 * real agent files. Returns all unique `forge-*` stems found in the body.
 *
 * The caller is responsible for filtering by which agents actually exist —
 * this function returns all syntactically valid `forge-*` matches.
 */
function parseCallsAgents(content) {
    // Match word-boundary forge-<stem> patterns; stems are lowercase letters and hyphens.
    // We use a regex that matches `forge-` followed by one or more lowercase-alpha-or-hyphen chars.
    // This catches `forge-planner`, `forge-plan-checker`, etc. in prose and code.
...
/**
 * Load the requires: dependency graph from a commands/forge directory.
 * Also derives calls_agents for each skill by scanning the body text for
 * `forge-*` agent name references. Agent stems are stored under the special
 * key `_calls_agents_<stem>` so they don't conflict with skill stems.
 */
const DEFAULT_COMMANDS_DIR = node_path_1.default.resolve(__dirname, '..', '..', '..', 'commands', 'forge');
...
/**
 * Stage a filtered copy of commands/forge for a resolved profile.
...
function stageSkillsForProfile(srcDir, resolvedProfile) {
    ...
    if (!node_fs_1.default.existsSync(srcDir))
        return srcDir;
    const stageDir = node_fs_1.default.mkdtempSync(node_path_1.default.join(node_os_1.default.tmpdir(), 'forge-profile-skills-'));
    try {
 ...
 * For tiered profiles, copies only agents whose full stem (e.g. 'forge-planner')
...
function stageAgentsForProfile(srcAgentsDir, resolvedProfile) {
    ...
    if (!node_fs_1.default.existsSync(srcAgentsDir))
        return srcAgentsDir;
    const stageDir = node_fs_1.default.mkdtempSync(node_path_1.default.join(node_os_1.default.tmpdir(), 'forge-profile-agents-'));
    try {
        if (resolvedProfile.agents instanceof Set && resolvedProfile.agents.size > 0) {
            ...
            for (const entry of entries) {
                ...
                if (!entry.name.endsWith('.md'))
                    continue;
                // Agent stem is the full filename without extension, e.g. "forge-planner"
                const stem = entry.name.slice(0, -3);
 ...
 * whose final cell carries a `forge-<stem>` token (optionally with `--flag`
...
function transformRouterBodyToNested(converted) {
    ...
    const out = lines.map((line) => {
        ...
        const m = cell.match(/forge-([a-z0-9-]+)((?:\s+--[a-z0-9-]+)*)/i);
        if (!m)
            return line;
        const stem = m[1];
...
function stageSkillsForRuntimeAsSkills(srcCommandsDir, resolvedProfile, converter, prefix, nested = false) {
    ...
    const stageDir = node_fs_1.default.mkdtempSync(node_path_1.default.join(node_os_1.default.tmpdir(), 'forge-profile-runtime-skills-'));
    try {
 ...
 * added here — the prefix is already embedded in agent stems, e.g. `forge-planner.md`).
 *
...
function stageAgentsForRuntimeWithConverter(srcAgentsDir, resolvedProfile, converter, isGlobal = false) {
    if (!node_fs_1.default.existsSync(srcAgentsDir))
        return srcAgentsDir;
    const stageDir = node_fs_1.default.mkdtempSync(node_path_1.default.join(node_os_1.default.tmpdir(), 'forge-profile-runtime-agents-'));
    try {
        const entries = node_fs_1.default.readdirSync(srcAgentsDir, { withFileTypes: true });
        for (const entry of entries) {
            if (!entry.isFile())
...
// ---------------------------------------------------------------------------
const PROFILE_MARKER_NAME = '.forge-profile';
/**
...
/**
 * Stage a filtered copy of the source commands/forge directory.
...
function stageSkillsForMode(srcDir, mode) {
    ...
    if (!node_fs_1.default.existsSync(srcDir))
        return srcDir;
    const stageDir = node_fs_1.default.mkdtempSync(node_path_1.default.join(node_os_1.default.tmpdir(), 'forge-minimal-skills-'));
    try {
        const entries = node_fs_1.default.readdirSync(srcDir, { withFileTypes: true });
        for (const entry of entries) {
            if (!entry.isFile())
