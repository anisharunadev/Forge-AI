...
const GSD_COPILOT_SESSION_MSG_ABSENT = 'GSD: no .planning/ workflow found - run /forge-new-project to start a tracked workflow.';
...
function buildCodexHookBlock(targetDir, opts) {
    ...
    const platform = (opts && opts.platform) || process.platform;
    const updateCheckScript = node_path_1.default.resolve(targetDir, 'hooks', 'forge-check-update.js');
...
function ensureCodexHooksJsonSessionStart(targetDir, opts = {}) {
    ...
    const scriptPath = node_path_1.default.resolve(targetDir, 'hooks', 'forge-check-update.js').replace(/\\/g, '/');
...
function ensureCodexHooksJsonEvent(targetDir, eventName, opts = {}) {
    ...
    if (!absoluteRunner)
        return { changed: false, wrote: false, path: hooksJsonPath };
    const scriptPath = node_path_1.default.resolve(targetDir, 'hooks', 'forge-context-monitor.js').replace(/\\/g, '/');
    let managedCommand;
    if (platform === 'win32') {
        const shimIR = buildCodexHookWindowsShimIR(scriptPath, absoluteRunner);
        if (!shimIR)
...
function buildClineRulesBody() {
    return [
        ...
        '',
        '- GSD workflows live in `forge-core/workflows/`. Load the relevant workflow when',
        '  the user runs a `/forge-*` command.',
        '- GSD agents live in `agents/`. Use the matching agent when spawning subagents.',
        '- GSD tools are at `forge-core/bin/forge-tools.cjs`. Run with `node`.',
...
'use strict';
/* GSD-managed Cline PreToolUse hook — forge-core issue #787.
  ...
  try {
    ...
    if (isWrite && paths.some(isPlanningPath)) {
      return process.stdout.write(JSON.stringify({
        cancel: true,
        errorMessage:
          'GSD: .planning/ artifacts are managed by GSD workflows. Edit them only through a /forge-* command, not directly.',
      }));
    }
...
function writeCursorHooksJson(targetDir, src, opts) {
    ...
    const sessionStartCmd = installedScripts.has('forge-cursor-session-start.js')
        ? buildHookCommand(targetDir, 'forge-cursor-session-start.js', hookOpts)
        : null;
    const postToolCmd = installedScripts.has('forge-cursor-post-tool.js')
        ? buildHookCommand(targetDir, 'forge-cursor-post-tool.js', hookOpts)
        : null;
...
function applySettingsJsonHooks(settings, opts) {
    ...
    if (hooksSurface !== 'none') {
        ...
        const hasGsdUpdateHook = settings.hooks.SessionStart.some((entry) => entry.hooks && entry.hooks.some((h) => referencesHook(h, 'forge-check-update')));
        ...
        const checkUpdateFile = node_path_1.default.join(targetDir, 'hooks', 'forge-check-update.js');
        if (!hasGsdUpdateHook && node_fs_1.default.existsSync(checkUpdateFile) && updateCheckCommand) {
            ...
            console.warn(`  ${yellow}⚠${reset}  Skipped update check hook — forge-check-update.js not found at target`);
        }
        // Configure post-tool hook for context window monitoring
        if (!settings.hooks[postToolEvent]) {
            settings.hooks[postToolEvent] = [];
        }
        const hasContextMonitorHook = settings.hooks[postToolEvent].some((entry) => entry.hooks && entry.hooks.some((h) => referencesHook(h, 'forge-context-monitor')));
        const contextMonitorFile = node_path_1.default.join(targetDir, 'hooks', 'forge-context-monitor.js');
        if (!hasContextMonitorHook && node_fs_1.default.existsSync(contextMonitorFile) && contextMonitorCommand) {
            ...
            console.warn(`  ${yellow}⚠${reset}  Skipped context monitor hook — forge-context-monitor.js not found at target`);
        }
        else {
            // Migrate existing context monitor hooks: add matcher and timeout if missing
            for (const entry of settings.hooks[postToolEvent]) {
                if (entry.hooks && entry.hooks.some((h) => referencesHook(h, 'forge-context-monitor'))) {
                    let migrated = false;
                    if (!entry.matcher) {
                    ...
                    for (const h of entry.hooks) {
                        if (referencesHook(h, 'forge-context-monitor') && !h.timeout) {
                            h.timeout = 10;
                            migrated = true;
                        }
                    }
        ...
        const hasPromptGuardHook = settings.hooks[preToolEvent].some((entry) => entry.hooks && entry.hooks.some((h) => referencesHook(h, 'forge-prompt-guard')));
        const promptGuardFile = node_path_1.default.join(targetDir, 'hooks', 'forge-prompt-guard.js');
        if (!hasPromptGuardHook && node_fs_1.default.existsSync(promptGuardFile) && promptGuardCommand) {
            ...
            console.warn(`  ${yellow}⚠${reset}  Skipped prompt guard hook — forge-prompt-guard.js not found at target`);
        }
        ...
        const hasReadGuardHook = settings.hooks[preToolEvent].some((entry) => entry.hooks && entry.hooks.some((h) => referencesHook(h, 'forge-read-guard')));
        const readGuardFile = node_path_1.default.join(targetDir, 'hooks', 'forge-read-guard.js');
        if (!hasReadGuardHook && node_fs_1.default.existsSync(readGuardFile) && readGuardCommand) {
            ...
            console.warn(`  ${yellow}⚠${reset}  Skipped read guard hook — forge-read-guard.js not found at target`);
        }
        ...
        const hasReadInjectionScannerHook = settings.hooks[postToolEvent].some((entry) => entry.hooks && entry.hooks.some((h) => referencesHook(h, 'forge-read-injection-scanner')));
        const readInjectionScannerFile = node_path_1.default.join(targetDir, 'hooks', 'forge-read-injection-scanner.js');
        if (!hasReadInjectionScannerHook && node_fs_1.default.existsSync(readInjectionScannerFile) && readInjectionScannerCommand) {
            ...
            console.warn(`  ${yellow}⚠${reset}  Skipped read injection scanner hook — forge-read-injection-scanner.js not found at target`);
        }
        ...
        // /forge-quick or /forge-fast for state-tracked changes. Also hard-blocks
        // unsafe Bash commands that violate worktree-agent isolation.
        const workflowGuardCommand = isGlobal
            ? buildHookCommand(targetDir, 'forge-workflow-guard.js', hookOpts)
            : localCmd('forge-workflow-guard.js');
        const workflowGuardMatcher = 'Bash|Edit|Write|MultiEdit';
        const workflowGuardHookEntry = settings.hooks[preToolEvent].find((entry) => entry.hooks && entry.hooks.some((h) => referencesHook(h, 'forge-workflow-guard')));
        const hasWorkflowGuardHook = Boolean(workflowGuardHookEntry);
        const workflowGuardFile = node_path_1.default.join(targetDir, 'hooks', 'forge-workflow-guard.js');
        if (hasWorkflowGuardHook && workflowGuardHookEntry.matcher !== workflowGuardMatcher) {
            ...
            console.warn(`  ${yellow}⚠${reset}  Skipped workflow guard hook — forge-workflow-guard.js not found at target`);
        }
        ...
        const worktreePathGuardCommand = isGlobal
            ? buildHookCommand(targetDir, 'forge-worktree-path-guard.js', hookOpts)
            : localCmd('forge-worktree-path-guard.js');
        const hasWorktreePathGuardHook = settings.hooks[preToolEvent].some((entry) => entry.hooks && entry.hooks.some((h) => referencesHook(h, 'forge-worktree-path-guard')));
        const worktreePathGuardFile = node_path_1.default.join(targetDir, 'hooks', 'forge-worktree-path-guard.js');
        if (!hasWorktreePathGuardHook && node_fs_1.default.existsSync(worktreePathGuardFile) && worktreePathGuardCommand) {
            ...
            console.warn(`  ${yellow}⚠${reset}  Skipped worktree path guard hook — forge-worktree-path-guard.js not found at target`);
        }
        // Configure commit validation hook (Conventional Commits enforcement, opt-in)
        const validateCommitCommand = isGlobal
            ? buildHookCommand(targetDir, 'forge-validate-commit.sh', hookOpts)
            : localShellCmd('forge-validate-commit.sh');
        const hasValidateCommitHook = settings.hooks[preToolEvent].some((entry) => entry.hooks && entry.hooks.some((h) => referencesHook(h, 'forge-validate-commit')));
        ...
        const validateCommitFile = node_path_1.default.join(targetDir, 'hooks', 'forge-validate-commit.sh');
        if (!hasValidateCommitHook && node_fs_1.default.existsSync(validateCommitFile) && validateCommitCommand) {
            ...
            console.warn(`  ${yellow}⚠${reset}  Skipped commit validation hook — forge-validate-commit.sh not found at target`);
        }
        ...
        const graphifyUpdateCommand = isGlobal
            ? buildHookCommand(targetDir, 'forge-graphify-update.sh', hookOpts)
            : localShellCmd('forge-graphify-update.sh');
        const hasGraphifyUpdateHook = settings.hooks[postToolEvent].some((entry) => entry.hooks && entry.hooks.some((h) => referencesHook(h, 'forge-graphify-update')));
        const graphifyUpdateFile = node_path_1.default.join(targetDir, 'hooks', 'forge-graphify-update.sh');
        if (!hasGraphifyUpdateHook && node_fs_1.default.existsSync(graphifyUpdateFile) && graphifyUpdateCommand) {
            ...
            console.warn(`  ${yellow}⚠${reset}  Skipped graphify auto-update hook — forge-graphify-update.sh not found at target`);
        }
        ...
        const sessionStateCommand = isGlobal
            ? buildHookCommand(targetDir, 'forge-session-state.sh', hookOpts)
            : localShellCmd('forge-session-state.sh');
        const hasSessionStateHook = settings.hooks.SessionStart.some((entry) => entry.hooks && entry.hooks.some((h) => referencesHook(h, 'forge-session-state')));
        const sessionStateFile = node_path_1.default.join(targetDir, 'hooks', 'forge-session-state.sh');
        if (!hasSessionStateHook && node_fs_1.default.existsSync(sessionStateFile) && sessionStateCommand) {
            ...
            console.warn(`  ${yellow}⚠${reset}  Skipped session state hook — forge-session-state.sh not found at target`);
        }
        ...
        const phaseBoundaryCommand = isGlobal
            ? buildHookCommand(targetDir, 'forge-phase-boundary.sh', hookOpts)
            : localShellCmd('forge-phase-boundary.sh');
        const hasPhaseBoundaryHook = settings.hooks[postToolEvent].some((entry) => entry.hooks && entry.hooks.some((h) => referencesHook(h, 'forge-phase-boundary')));
        const phaseBoundaryFile = node_path_1.default.join(targetDir, 'hooks', 'forge-phase-boundary.sh');
        if (!hasPhaseBoundaryHook && node_fs_1.default.existsSync(phaseBoundaryFile) && phaseBoundaryCommand) {
            ...
            console.warn(`  ${yellow}⚠${reset}  Skipped phase boundary hook — forge-phase-boundary.sh not found at target`);
        }
        ...
        // three lifecycle events.  Wire forge-context-monitor so agents get context-
        ...
        // user prompt text, not a tool invocation, so forge-prompt-guard (which
        ...
        // Guard is now descriptor-driven: only events present in extendedEvents are wired.
        {
            ...
            for (const event of ['SubagentStop', 'Stop', 'PreCompact']) {
                ...
                const alreadyHasContextMonitor = settings.hooks[event].some((entry) => entry.hooks && entry.hooks.some((h) => referencesHook(h, 'forge-context-monitor')));
                if (!alreadyHasContextMonitor && node_fs_1.default.existsSync(contextMonitorFile) && contextMonitorCommand) {
                    ...
                    console.warn(`  ${yellow}⚠${reset}  Skipped ${event} hook — forge-context-monitor.js not found at target`);
                }
            }
        }
        ...
        // forge previously did not register.  Three high-value events are added here:
        //
        //   BeforeAgent  — fires after user submits a prompt, before the agent
        //                  plans.  Wire forge-context-monitor for context headroom
        //                  awareness at prompt time.
        //   AfterAgent   — fires once per turn after the model generates its final
        //                  response.  Wire forge-context-monitor to track headroom
        //                  after each agent turn completes.
        //   BeforeModel  — fires before each LLM call (per-turn, not per-session).
        //                  Wire forge-context-monitor for per-turn context injection
        //                  — more precise than session-start-only injection.
        //
        // All three reuse forge-context-monitor.js — no new hook files needed.
        // The `decision:"deny"` retry capability of AfterAgent is intentionally
        // left to the hook script to implement when triggered (forge-context-monitor
        ...
        // forge hook use case at this time; deferred to a follow-on issue.
        //
        // Guard is now descriptor-driven: only events present in extendedEvents are wired.
        for (const geminiEvent of ['BeforeAgent', 'AfterAgent', 'BeforeModel']) {
            ...
            const alreadyHasContextMonitor = settings.hooks[geminiEvent].some((entry) => entry.hooks && entry.hooks.some((h) => referencesHook(h, 'forge-context-monitor')));
            if (!alreadyHasContextMonitor && node_fs_1.default.existsSync(contextMonitorFile) && contextMonitorCommand) {
                ...
                console.warn(`  ${yellow}⚠${reset}  Skipped ${geminiEvent} hook — forge-context-monitor.js not found at target`);
            }
        }
        // ── end Gemini-only extended hook events ──────────────────────────────────
        // ── FileChanged hook: hot-reload forge config on .planning/config.json edits ─
        // Claude Code fires FileChanged when a watched file changes on disk.  Wire
        // forge-config-reload.js to reload the forge config context whenever the user
        ...
        // when the changed file is not the forge config.
        //
        ...
        if (extendedEvents.includes('FileChanged')) {
            ...
            const configReloadFile = node_path_1.default.join(targetDir, 'hooks', 'forge-config-reload.js');
            const alreadyHasConfigReload = settings.hooks.FileChanged.some((entry) => entry.hooks && entry.hooks.some((h) => referencesHook(h, 'forge-config-reload')));
            if (!alreadyHasConfigReload && node_fs_1.default.existsSync(configReloadFile) && configReloadCommand) {
                ...
                console.warn(`  ${yellow}⚠${reset}  Skipped FileChanged hook — forge-config-reload.js not found at target`);
            }
