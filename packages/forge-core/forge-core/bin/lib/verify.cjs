        ...
        if (phaseConvention === 'milestone-prefixed') {
            if (node_fs_1.default.existsSync(roadmapPath)) {
                ...
                for (const mm of mismatches) {
                    addIssue('warning', 'W021', `Phase ${mm.phaseId}: integer prefix implies ${mm.expectedMilestone} but listed under ${mm.foundInMilestone}`, 'Run `forge-tools roadmap upgrade --convention milestone-prefixed` to migrate (dry-run by default)');
                }
            }
        }
    }
