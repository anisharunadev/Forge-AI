'use strict';
/**
 * Profile-pipeline command router — CLI dispatcher for forge-tools profiling commands.
 *
 * ADR-857 phase 6 / ADR-959: profile-pipeline capability command cutover.
 * Extracted from hardcoded case arms in forge-tools.cjs (lines 1324-1410).
 ...
 * dispatchCapabilityCommand (forge-tools.cjs:366-371) explicitly errors if a
...
function routeExtractMessages({ args, cwd, raw, error, _pipeline }) {
  ...
  if (!projectArg || projectArg.startsWith('--')) {
    error('Usage: forge-tools extract-messages <project> [--session <id>] [--limit N] [--path <dir>]\nRun scan-sessions first to see available projects.', ERROR_REASON.USAGE);
    return;
  }
