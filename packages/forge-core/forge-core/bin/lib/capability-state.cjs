 ...
 * `forge-tools capability state` diagnostic share the same enablement answer.
 *
...
// ─── I/O command handler ───────────────────────────────────────────────────────
/**
 * Derive the commands/forge path from __dirname (which resolves to
 * forge-core/bin/lib/ at runtime). The source tree is:
 *   <repo>/forge-core/bin/lib/capability-state.cjs
 *   <repo>/commands/forge/*.md
 * So we walk up three levels: lib/ → bin/ → forge-core/ → <repo>/, then
 * into commands/forge/.
 */
function _resolveCommandsGsdDir() {
    // __dirname = forge-core/bin/lib/
    const repoRoot = node_path_1.default.resolve(__dirname, '..', '..', '..');
    return node_path_1.default.join(repoRoot, 'commands', 'forge');
}
/**
 * Build a skill dependency manifest from an INSTALLED runtime's skills directory.
 *
 * In an installed runtime (e.g. Codex at ~/.codex), forge skills live as
 * configDir/skills/forge-STEM/SKILL.md. There is no commands/forge source tree.
 * This function scans that installed layout and builds the same
 * Map shape that loadSkillsManifest produces from sources.
 *
 * Stem extraction: a directory named forge-secure-phase maps to stem secure-phase.
 * Only directories whose names start with forge- are included so user-created
 * skills (without the forge- prefix) are not accidentally pulled in.
 *
...
function _loadInstalledSkillsManifest(configDir) {
    ...
    for (const entry of entries) {
        ...
        if (!entry.name.startsWith('forge-'))
            continue;
        // Strip the 'forge-' prefix to get the skill stem
        const stem = entry.name.slice(4); // 'forge-'.length === 4
        if (!stem)
            continue;
        ...
        // no file exists. Mirror that here: a stale forge-<stem>/ directory with no
 ...
 *   2. Otherwise, fall back to installed skills at configDir/skills/forge-[stem]/SKILL.md.
 *
 * In an installed runtime the commands/forge source tree is absent; only the
...
function resolveCapabilityRuntimeState(cwd, runtimeConfigDir, configOverride) {
    ...
    try {
        ...
        // commands/forge is absent) fall back to <configDir>/skills/forge-*/SKILL.md.
