...
function walkLegacyFiles(root, relDir, baseResolved, results) {
    ...
    for (const entry of entries) {
        ...
        if (entry.isSymbolicLink())
            continue; // forge-allow-legacy-name (entry under legacy get-shit-done/ dir)
...
const migration = {
    ...
    description: 'After the config dir rename from get-shit-done/ to forge-core/ (#604), remove prior-manifest-managed files ' + // forge-allow-legacy-name
    ...
    destructive: true,
    plan(ctx) {
        const legacyRoot = node_path_1.default.join(ctx.configDir, 'get-shit-done'); // forge-allow-legacy-name
