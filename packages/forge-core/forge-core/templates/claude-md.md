...
Template for project-root `CLAUDE.md` — auto-generated by `forge-tools generate-claude-md`.
...
**Fallback text:**
```
Project not yet initialized. Run /forge:new-project to set up.
```
...
- Skips GSD's own installed skills (directories starting with `forge-`)
...
- `/forge:quick` for small fixes, doc updates, and ad-hoc tasks
- `/forge:debug` for investigation and bug fixing
- `/forge:execute-phase` for planned phase work
...
> Profile not yet configured. Run `/forge:profile-user` to generate your developer profile.
