...
> AI design contract generated by `/forge:ai-integration-phase`. Consumed by `forge-planner` and `forge-eval-auditor`.
...
> Researched by `forge-domain-researcher`. Grounds the evaluation strategy in domain expert knowledge.
...
> Fetched from official docs by `forge-ai-researcher`. Distilled for this specific use case.
...
> Written by `forge-ai-researcher`. Cross-cutting patterns every developer building AI systems needs — independent of framework choice.
