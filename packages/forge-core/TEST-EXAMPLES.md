...
for (const scenario of cases) {
  test(`forge-tools rejects ${scenario.name}`, (t) => {
    const projectDir = createTempProject('cli-negative-');
    t.after(() => cleanup(projectDir));

    const result = spawnSync(
      process.execPath,
      [path.join(__dirname, '..', 'forge-core', 'bin', 'forge-tools.cjs'), ...scenario.args, '--json'],
...
test('installer refuses symlink escape outside target root', (t) => {
  ...
  assert.equal(fs.existsSync(path.join(outside, 'forge-prompt-guard.js')), false);
});
```
