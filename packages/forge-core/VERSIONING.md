...
|------|-----------|---------------|---------|--------|---------|
| **Patch** | Bug fixes only | `1.27.1` | `latest` | `hotfix/1.27.1` | `npx @forge-ai/forge-core@latest` |
| **Minor** | Fixes + enhancements | `1.28.0` | `latest` (after RC) | `release/1.28.0` | `npx @forge-ai/forge-core@next` (RC) |
| **Major** | Fixes + enhancements + features | `2.0.0` | `latest` (after beta) | `release/2.0.0` | `npx @forge-ai/forge-core@next` (beta) |
...
|-----|---------|-------------|
| `latest` | Stable production release | `npm install @forge-ai/forge-core` (default) |
| `next` | Pre-release (RC or beta) | `npm install @forge-ai/forge-core@next` (opt-in) |
...
3. Trigger `release.yml` with `action=finalize`. The workflow:
   ...
   - Builds SDK, bundles `sdk-bundle/forge-sdk.tgz` inside the CC tarball.
...
4. Test the RC: `npx @forge-ai/forge-core@next`
...
npm dist-tag ls @forge-ai/forge-core
```
