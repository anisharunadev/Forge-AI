...
**Context**: Follow-up to `/forge-inbox` triage on 2026-05-16
...
|---|-------|-------------|------|------------------|-------------|--------|-------|
| 1 | [#3583](https://github.com/forge-ai/forge-core/issues/3583) | Claude skill install leaves `/forge:<cmd>` in `SKILL.md` body | Installer / Command namespace | PR 3629 (our branch) + competing 3586 | Small (1 file + test) | PR opened / Review | **Leading PR: 3629** (cristianuibar) — reviewed + hardened with CodeRabbit feedback (left-boundary regex + body-scoped guard). Competing PR 3586 has "needs changes" + "ci: failing". Issue still carries `confirmed-bug`. |
| 2 | [#3579](https://github.com/forge-ai/forge-core/issues/3579) | `build-hooks.js` + npm publish omit graphify auto-update hook | Packaging / Build | `/forge-quick` | Small | Not started | Classic "new feature missed in release artifact". Easy local verification. |
| 3 | [#3496](https://github.com/forge-ai/forge-core/issues/3496) | `/forge:update` changelog extraction skips intermediate versions | Workflow / Update logic | `/forge-quick` or lightweight plan | Medium-small | Not started | Needs deterministic version-range helper. |
| 4 | [#3588](https://github.com/forge-ai/forge-core/issues/3588) | Production `npm audit` has 1 high + 5 moderate advisories | Security / Dependencies | Direct + careful review | Medium | Not started | Transitive via `@anthropic-ai/claude-agent-sdk`. May need overrides. |
| 5 | [#3584](https://github.com/forge-ai/forge-core/issues/3584) | Runtime `bin/lib/*.cjs` still emit `/forge:<cmd>` (larger piece deferred from #3583) | Runtime output / Slash formatter | Short plan first, then execute | Medium-Large | Not started | 16+ files. Design a centralized runtime-aware formatter. Do after #3583. |
| 6 | [#3340](https://github.com/forge-ai/forge-core/issues/3340) | SDK publish lag — agent dir fix never shipped in `@openforge/forge-sdk@0.1.0` | Release / SDK publishing | Plan + coordination | Medium (release-focused) | Not started | Oldest. Mostly a publishing/versioning task. |
...
- **GSD self-use**: For the small ones (#3583, #3579, #3496), using `/forge-quick` (or `/forge-fast`) inside the fix session is encouraged and appropriate. For #3584, a short planning step is recommended.
...
- These issues were identified during a `/forge-inbox` run on 2026-05-16.
...
- Inbox triage report: `/tmp/GSD-INBOX-TRIAGE-2026-05-16.md` (from the `/forge-inbox` run)
- Full issue list with `confirmed-bug` label: `gh issue list --state open --label confirmed-bug`

---

**Next action**: #3583 now has active PR(s) under review. Next clean quick win (0 PRs, small packaging effort, high value for recently-landed graphify feature): **#3579**. Validated via GitHub search: no PRs mention 3579. Ready for `/forge-quick` or direct fix (update `scripts/build-hooks.js` HOOKS_TO_COPY + ensure `hooks/lib/` copy in installer + fix any publish filter).
