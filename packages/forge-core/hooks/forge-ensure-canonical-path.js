#!/usr/bin/env node
// forge-hook-version: {{GSD_VERSION}}
//
// forge-ensure-canonical-path — SessionStart hook (#997)
//
// PROBLEM: GSD agents/commands/templates use markdown `@`-file-includes that
// hardcode the canonical path `@~/.claude/forge-core/...` (references, workflows,
...
// `~/.claude/forge-core/` is never created and every @-include resolves to
...
// set and a bundled `forge-core/` tree found beneath it), ensure
// `~/.claude/forge-core/` exists and its immutable subdirs (bin, contexts,
...
// are the directories `@~/.claude/forge-core/<subdir>/...` includes point into.
 ...
 * forge-check-update.js detectConfigDir), else falls back to ~/.claude. The
 * canonical GSD tree always lives at `<configDir>/forge-core`.
 */
...
/**
 * Locate the bundled `forge-core/` tree beneath a plugin root.
 *
 * Claude Code unpacks the package so the bundled tree sits at
 * `<pluginRoot>/forge-core/`. Returns the absolute, realpath-normalised path to
...
function resolveBundledTree(pluginRoot) {
  ...
  const bundled = path.join(root, 'forge-core');
  let bundledReal;
  try {
    ...
    // ENOTDIR if <pluginRoot>/forge-core is absent, so no separate existence
  ...
  // root. A crafted symlink at <pluginRoot>/forge-core pointing outside the root
...
/**
 * Ensure the canonical `~/.claude/forge-core/` path exposes the bundled subdirs.
 *
...
function ensureCanonicalPath(opts = {}) {
  ...
  const canonicalDir = path.join(configDir, 'forge-core');
