...
// forge-check-update-worker.js writes to ~/.cache/forge/<updateCacheFileName> (per-package).
//
...
function buildBannerOutput(state) {
  ...
  return {
    systemMessage: `GSD update available: ${installed} → ${latest}. Run /forge:update.`,
  };
}
...
function main() {
  const cacheDir = path.join(os.homedir(), '.cache', 'forge');
  ...
  if (parseError && !suppressFailureWarning) {
    ...
    // where ~/.cache/forge was created by check-update but the parent dir got
    // wiped between runs.
    try {
