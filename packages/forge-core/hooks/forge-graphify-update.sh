...
# forge-graphify-update.sh — PostToolUse hook (Bash matcher) that auto-rebuilds
...
#      the exact `forge-tools query commit` command shape; the SDK command invokes
...
#   - Detaches hooks/lib/forge-graphify-rebuild.sh which copies graphify-out/* to
...
# Gate 2 — HEAD-advancing git op (shell-direct or exact `forge-tools query commit`)
case "$COMMAND" in
  *"git commit"*|*"git merge"*|*"git pull"*|*"git rebase --continue"*|*"git cherry-pick"*) ;;
  *"forge-tools query commit"|*"forge-tools query commit "*) ;;
  *) exit 0 ;;
esac
...
REBUILD_SCRIPT="$HOOK_DIR/lib/forge-graphify-rebuild.sh"
