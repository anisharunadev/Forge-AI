...
// Background worker spawned by forge-check-update.js (SessionStart hook).
...
const { isSemverNewer } = require('../forge-core/bin/lib/semver-compare.cjs');
...
const { checkLatestVersion } = require('../forge-core/bin/check-latest-version.cjs');
...
// (e.g., forge-intel-*.js) must be ignored to avoid permanent stale warnings (#1750)
    ...
    if (fs.existsSync(hooksDir)) {
      ...
      for (const hookFile of hookFiles) {
        try {
          ...
          const versionMatch = content.match(/(?:\/\/|#) forge-hook-version:\s*(.+)/);
          if (versionMatch) {
