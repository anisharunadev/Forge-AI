...
// (no active /forge- skill or Task subagent) and injects an advisory warning.
//
// This is a SOFT guard — it advises, not blocks. The edit still proceeds.
// The warning nudges Claude to use /forge:quick or /forge:fast instead of
...
process.stdin.on('end', () => {
  ...
  try {
    ...
    // Check if we're inside a GSD workflow (Task subagent or /forge- skill)
    ...
    const output = {
      hookSpecificOutput: {
        ...
        additionalContext: `⚠️ WORKFLOW ADVISORY: You're editing ${path.basename(filePath)} directly without a GSD command. ` +
          ...
          'Consider using /forge:fast for trivial fixes or /forge:quick for larger changes ' +
