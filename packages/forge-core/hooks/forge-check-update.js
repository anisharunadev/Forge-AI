...
function detectConfigDir(baseDir) {
  ...
  if (envDir && fs.existsSync(path.join(envDir, 'forge-core', 'VERSION'))) {
    return envDir;
  }
  for (const dir of ['.claude', '.gemini', '.config/kilo', '.kilo', '.config/opencode', '.opencode']) {
    if (fs.existsSync(path.join(baseDir, dir, 'forge-core', 'VERSION'))) {
...
const cacheDir = path.join(homeDir, '.cache', 'forge');
const cacheFile = path.join(cacheDir, updateCacheFileName);

// VERSION file locations (check project first, then global)
const projectVersionFile = path.join(projectConfigDir, 'forge-core', 'VERSION');
const globalVersionFile = path.join(globalConfigDir, 'forge-core', 'VERSION');
...
const workerPath = path.join(__dirname, 'forge-check-update-worker.js');
