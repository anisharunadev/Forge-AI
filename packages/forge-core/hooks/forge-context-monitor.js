...
process.stdin.on('end', () => {
  ...
  try {
    ...
    // breadcrumb for /forge:resume-work (#1974). Fire-and-forget subprocess —
    ...
    if (isCritical && isGsdActive && !warnData.criticalRecorded) {
      try {
        ...
        // and forge-tools.cjs lives at <runtime-config>/forge-core/bin/.
        // Using __dirname makes this work on Claude Code, OpenCode, Gemini,
        // Kilo, etc. without hardcoding ~/.claude/.
        const gsdTools = path.join(__dirname, '..', 'forge-core', 'bin', 'forge-tools.cjs');
