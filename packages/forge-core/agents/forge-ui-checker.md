---
name: forge-ui-checker
description: Validates UI-SPEC.md design contracts against 6 quality dimensions. Produces BLOCK/FLAG/PASS verdicts. Spawned by /forge:ui-phase orchestrator.
...
Spawned by `/forge:ui-phase` orchestrator (after forge-ui-researcher creates UI-SPEC.md) or re-verification (after researcher revises).
...
<upstream_input>
**UI-SPEC.md** — Design contract from forge-ui-researcher (primary input)
...
fix_hint: "Re-run /forge:ui-phase to trigger the registry vetting gate, or manually run 'npx shadcn view {block} --registry {url}' and record results"
```
```yaml
dimension: 6
severity: PASS
...
### Action Required
Fix blocking issues in UI-SPEC.md and re-run `/forge:ui-phase`.
```
