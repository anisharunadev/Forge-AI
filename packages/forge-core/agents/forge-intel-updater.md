...
- **Cross-platform.** Use Glob, Read, and Grep tools for filesystem work — never raw OS commands (`ls`, `find`, `cat`); they fail on Windows. CLI invocations go through `forge-tools intel <subcommand>`, which routes through the Shell Command Projection Module that formats per-OS automatically.
...
### From `/forge:map-codebase --query` Command

- **Spawned by:** `/forge:map-codebase --query` command
...
The /forge:map-codebase --query command has already confirmed that intel.enabled is true before spawning this agent. Proceed directly to Step 1.
</upstream_input>
...
**Runtime layout detection (GSD framework repo only):** If `package.json` `"name"` equals `"@forge-ai/forge-core"`, this project IS the GSD framework. In that case, detect the runtime root to choose canonical paths:

```bash
# Only run layout detection when analysing the GSD framework repo itself.
if [[ "$(jq -r '.name // ""' package.json 2>/dev/null)" == "@forge-ai/forge-core" ]]; then
  ls -d .kilo 2>/dev/null && echo "kilo" || (ls -d .claude/forge-core 2>/dev/null && echo "claude") || echo "unknown"
fi
```
...
| Command files | `commands/forge/*.md` | `.kilo/command/*.md` |
| CLI tooling | `forge-core/bin/` | `.kilo/forge-core/bin/` |
| Workflow files | `forge-core/workflows/` | `.kilo/forge-core/workflows/` |
| Reference docs | `forge-core/references/` | `.kilo/forge-core/references/` |
...
**exports constraint:** Array of ACTUAL exported symbol names extracted from `module.exports` or `export` statements. MUST be real identifiers (e.g., `"configLoad"`, `"stateUpdate"`), NOT descriptions (e.g., `"config operations"`). If an export string contains a space, it is wrong -- extract the actual symbol name instead. Use `forge-tools intel extract-exports <file>` to get accurate exports.
...
arch-decisions.json is JSON (NOT markdown). The `forge-tools intel` CLI reads, validates, and queries it as JSON. Capture the architecture as descriptive keyed entries:
...
forge_run intel patch-meta .planning/intel/stack.json 
```
...
```bash
forge_run intel patch-meta .planning/intel/file-roles.json 
```
...
```bash
forge_run intel patch-meta .planning/intel/api-map.json 
```
...
```bash
forge_run intel patch-meta .planning/intel/dependency-graph.json 
```
...
```bash
forge_run intel patch-meta .planning/intel/arch-decisions.json
```

### Step 6.5: Self-Check

Run: `forge-tools intel validate`
...
Run: `forge-tools intel snapshot`
