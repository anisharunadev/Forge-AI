...
- Created via `/forge:phase --insert`
...
This annotation is consumed by downstream workflows (`new-project`, `progress`) to suggest `/forge:ui-phase` at the right time. Phases without UI indicators omit the annotation entirely.
...
Reference full template: `~/.claude/forge-core/templates/roadmap.md`

## STATE.md Structure

Use template from `~/.claude/forge-core/templates/state.md`.
...
User can review actual files in the editor or via SDK queries (e.g. `forge-tools query roadmap.analyze` and `forge-tools query state.load`) instead of ad-hoc shell `cat`.
