...
<required_reading>
@~/.claude/forge-core/references/verification-overrides.md
...
@~/.claude/forge-core/references/thinking-models-verification.md

At verification decision points, reference calibration examples:
@~/.claude/forge-core/references/few-shot-examples/verifier.md
...
forge_run query roadmap.get-phase "$PHASE_NUM"
...
```bash
PHASE_DATA=$(forge_run query roadmap.get-phase "$PHASE_NUM" --raw)
```
...
Use `forge-tools query` for artifact verification against must_haves in PLAN frontmatter:

```bash
ARTIFACT_RESULT=$(forge_run query verify.artifacts "$PLAN_PATH")
```
...
Use `forge-tools query` for key link verification against must_haves in PLAN frontmatter:

```bash
LINKS_RESULT=$(forge_run query verify.key-links "$PLAN_PATH")
```
...
SUMMARY_FILES=$(forge_run query summary-extract "$PHASE_DIR"/*-SUMMARY.md --fields key-files)

# Option 2: Verify commits exist (if commit hashes documented)
COMMIT_HASHES=$(grep -oE "[a-f0-9]{7,40}" "$PHASE_DIR"/*-SUMMARY.md | head -10)
if [ -n "$COMMIT_HASHES" ]; then
  COMMITS_VALID=$(forge_run query verify.commits $COMMIT_HASHES)
fi
...
> **Shared status seam**: the status vocabulary (`passed`, `gaps_found`, `human_needed`) and the per-status routing (next action and next command for each value) are owned by `src/verification.cts` via `forge_run query verification.status`. This agent is the single emitter of the frontmatter status field; consumers (ship.md, execute-phase.md) read routing from that query instead of re-deriving it.
...
```bash
ROADMAP_DATA=$(forge_run query roadmap.analyze --raw)
```
...
**When the phase under verification has `mode: mvp` in ROADMAP.md (resolved by the verify-work workflow):** Apply the goal-backward methodology, narrowed to the phase's user-story goal. Required reading: `@~/.claude/forge-core/references/verify-mvp-mode.md`.
...
```bash
USER_STORY_VALID=$(forge_run query user-story.validate --story "$PHASE_GOAL" --pick valid)
```

If `valid != true`, refuse to verify. Surface the discrepancy and ask the user to run `/forge mvp-phase ${PHASE}` to set a proper User Story goal. The verb owns the canonical regex `/^As a .+, I want to .+, so that .+\.$/` and surfaces per-error guidance in `errors[]` plus slot extractions in `slots`. Do NOT attempt to verify against a non-User Story goal under MVP mode — the User Flow Coverage section would be low-quality.
