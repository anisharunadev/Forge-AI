...
@~/.claude/forge-core/references/thinking-models-execution.md

**iOS app scaffolding:** If this plan creates an iOS app target, follow ios-scaffold guidance:
@~/.claude/forge-core/references/ios-scaffold.md
...
<task type="checkpoint:human-verify" gate="blocking-human">
  ...
  <how-to-verify>
    ...
    3. If the package does not exist, re-run /forge:plan-phase --research-phase <N> to find the correct package
  </how-to-verify>
...
@~/.claude/forge-core/references/executor-examples.md
</deviation_rules>
...
```bash
AUTO_CHAIN=$(forge_run query config-get workflow._auto_chain_active 2>/dev/null || echo "false")
AUTO_CFG=$(forge_run query config-get workflow.auto_advance 2>/dev/null || echo "false")
```
...
**See @~/.claude/forge-core/references/checkpoints.md**
...
**When the orchestrator passes both `MVP_MODE=true` and `TDD_MODE=true`:** Before running the implementation step of any task with `tdd="true"`, run the runtime gate from `~/.claude/forge-core/references/execute-mvp-tdd.md` (Read it). If the gate trips, halt and report — do NOT proceed to the implementation step.
...
case "$WT_GIT_DIR" in
  *.git/worktrees/*)
      SENTINEL="$WT_GIT_DIR/forge-spawn-toplevel"
...
```bash
forge_run query commit-to-subrepo "{type}({phase}-{plan}): {concise task description}" --files file1 file2 ...
```
...
**Use template:** @~/.claude/forge-core/templates/summary.md
...
<state_updates>
After SUMMARY.md, update STATE.md using `forge-tools query` state handlers (positional args; see `sdk/src/query/QUERY-HANDLERS.md`):
...
for decision in "${DECISIONS[@]}"; do
  forge_run query state.add-decision "${decision}"
done
...
forge_run query roadmap.update-plan-progress "${PHASE_NUMBER}"
...
```bash
forge_run query state.add-blocker "Blocker description"
```
</state_updates>

<final_commit>
```bash
forge_run query commit "docs({phase}-{plan}): complete [plan-name] plan" --files \
  .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/STATE.md .planning/ROADMAP.md .planning/REQUIREMENTS.md
```

Separate from per-task commits — captures execution results only.

**Handling the SDK return envelope (#3678):** `forge-tools query commit` returns
one of three shapes:
