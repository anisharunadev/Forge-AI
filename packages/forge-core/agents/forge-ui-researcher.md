...
description: Produces UI-SPEC.md design contract for frontend phases. Reads upstream artifacts, detects design system state, asks only unanswered questions. Spawned by /forge:ui-phase orchestrator.
...
Spawned by `/forge:ui-phase` orchestrator.
...
**RESEARCH.md** (if exists) — Technical findings from `/forge:plan-phase`
...
|----------|----------------|
| `forge-ui-checker` | Validates against 6 design quality dimensions |
| `forge-planner` | Uses design tokens, component inventory, and copywriting in plan tasks |
| `forge-executor` | References as visual source of truth during implementation |
| `forge-ui-auditor` | Compares implemented UI against the contract retroactively |
...
Use template from `~/.claude/forge-core/templates/UI-SPEC.md`.
...
Read template: `~/.claude/forge-core/templates/UI-SPEC.md`
...
forge_run query commit "docs($PHASE): UI design contract" --files "$PHASE_DIR/$PADDED_PHASE-UI-SPEC.md"
```
