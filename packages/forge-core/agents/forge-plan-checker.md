...
**REVIEWS.md** (if included by reviews mode) — Cross-AI review feedback from `/forge:review`

REVIEWS.md is audit trail and feedback input, not a hidden execution contract. /forge:execute-phase primarily consumes PLAN.md plus normal phase context. Add verification dimension: **Review Incorporation**.
...
- If a current actionable finding remains only in REVIEWS.md and would be invisible to /forge:execute-phase, return `## ISSUES FOUND`. Use WARNING by default; use BLOCKER when the missing incorporation can prevent the phase goal, create unsafe execution, or invalidate verification.
</upstream_input>
...
@~/.claude/forge-core/references/few-shot-examples/plan-checker.md
...
**If missing:** **BLOCKING FAIL** — "VALIDATION.md not found for phase {N}. Re-run `/forge:plan-phase {N} --research` to regenerate."
...
```bash
forge_run query phase.list-plans "$phase_number"
# Research / brief artifacts (deterministic listing)
forge_run query phase.list-artifacts "$phase_number" --type research
forge_run query roadmap.get-phase "$phase_number"
forge_run query phase.list-artifacts "$phase_number" --type summary
```
...
Use `forge-tools query` to validate plan structure:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  echo "=== $plan ==="
  PLAN_STRUCTURE=$(forge_run query verify.plan-structure "$plan")
  echo "$PLAN_STRUCTURE"
done
```
...
Extract must_haves from each plan using `forge-tools query`:

```bash
MUST_HAVES=$(forge_run query frontmatter.get "$PLAN_PATH" must_haves)
```
...
```bash
PLAN_STRUCTURE=$(forge_run query verify.plan-structure "$PLAN_PATH")
```
...
```bash
forge_run query plan.task-structure "$PLAN_PATH"
```
...
```bash
forge_run query plan.task-structure "$PHASE_DIR/$PHASE-01-PLAN.md"
forge_run query frontmatter.get "$PHASE_DIR/$PHASE-01-PLAN.md" files_modified
```
