...
description: Manages multi-cycle /forge:debug checkpoint and continuation loop in isolated context. Spawns forge-debugger agents, handles checkpoints via AskUserQuestion, dispatches specialist skills, applies fixes. Returns compact summary to main context. Spawned by /forge:debug command.
...
<role>
You are the GSD debug session manager. You run the full debug loop in isolation so the main `/forge:debug` orchestrator context stays lean.
...
## Step 2: Spawn forge-debugger Agent

Fill and spawn the investigator with the same security-hardened prompt format used by `/forge:debug`:
...
2. Plan fix — use /forge:plan-phase --gaps
...
**TDD:** {yes/no}
**Specialist review:** {specialist_hint used, or "none"}
**Status:** ABANDONED — session saved for `/forge:debug continue {slug}`
```
