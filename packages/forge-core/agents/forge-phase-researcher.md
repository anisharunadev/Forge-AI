...
Spawned by `/forge:plan-phase` (integrated) or `/forge:plan-phase --research-phase <N>` (standalone).
...
forge_run query research-plan --input /tmp/research-plan-input.json
```
...
| `brave` | `forge-tools query websearch "<fetch.query>"` (Brave-backed) or built-in `WebSearch` |
...
```bash
forge_run query research-store put <key> \
...
```bash
forge_run query classify-confidence --provider <provider-id>
# for cross-checked findings, add --verified:
forge_run query classify-confidence --provider <provider-id> --verified
```
...
```bash
forge_run query package-legitimacy check --ecosystem <npm|pypi|crates> <pkg1> <pkg2> ...
```
...
- **Phase gate:** Full suite green before `/forge:verify-work`
...
@~/.claude/forge-core/references/thinking-models-research.md
...
```bash
INIT=$(forge_run query init.phase-op "${PHASE}")
...
```bash
forge_run graphify query "<capability-keyword>" --budget 1500
```
...
For each domain, use the `<tool_strategy>` seam (Steps A–D): build questions JSON, call `forge-tools query research-plan`, run the indicated provider per item, then cache each digest. Document findings with confidence levels as you go (use `forge-tools query classify-confidence --provider <id>` to obtain the tier).
...
```bash
forge_run query commit "docs($PHASE): research phase domain" --files "$PHASE_DIR/$PADDED_PHASE-RESEARCH.md"
```
