...
description: Verifies threat mitigations from PLAN.md threat model exist in implemented code. Produces SECURITY.md. Spawned by /forge:secure-phase.
tools:
  - Read
  - Write
  - Edit
...
**Verification depth scales with `asvs_level`** (see @~/.claude/forge-core/references/security-asvs-levels.md for full definitions):
...
Next: Implement mitigations or document as accepted in SECURITY.md accepted risks log, then re-run /forge:secure-phase.
