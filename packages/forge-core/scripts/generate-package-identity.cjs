 ...
 * `forge-core/bin/lib/package-identity.cjs` bakes those values at build
...
function deriveIdentity(pkg = {}) {
  ...
  const updateCacheFileName = cacheSlug ? `forge-update-check-${cacheSlug}.json` : 'forge-update-check.json';
...
function main() {
  ...
  const out = path.join(__dirname, '..', 'forge-core', 'bin', 'lib', 'package-identity.cjs');
