 ...
 * registry (forge-core/bin/lib/capability-registry.cjs), so the matrix can never
...
| Column | Description |
|---|---|
| **id** | Canonical capability identifier; unique across first- and third-party capabilities. Reserved prefixes: \`forge-\`, \`forge-core-\`, \`anthropic-\`. |
...
| **source** | \`first-party\` — ships with Forge Core; \`third-party\` — installed from an external source via \`gsd capability install\`. |
...
registry and therefore lists only the capabilities that ship with Forge Core.
...
[\`gsd capability\` command reference](forge-capability-command.md)), which reports
...
| Column | Value |
|---|---|
| **id** | As declared in \`capability.json\`. Must not use reserved prefixes (\`forge-\`, \`forge-core-\`, \`anthropic-\`). |
...
| \`compatVersions\` | No | object: cap-version → forge-range | Graceful-downgrade table for sources that enumerate versions (git tags, registry, npm). |
