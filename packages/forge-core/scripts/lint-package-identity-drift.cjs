 ...
 *
 * The seam (`get-shit-done/bin/lib/package-identity.cjs`, derived from // forge-allow-legacy-name
 ...
 *
 * Scope: the runtime/code surface (bin/, hooks/, scripts/, get-shit-done/). // forge-allow-legacy-name
...
// "get-shit-done" (so @openforge/forge-sdk and unrelated scopes never match). // forge-allow-legacy-name
const PACKAGE_RE = /@[A-Za-z0-9._-]+\/[A-Za-z0-9._-]*get-shit-done[A-Za-z0-9._-]*/g; // forge-allow-legacy-name
...
const SLUG_RE = /(?:github\.com[/:]|raw\.githubusercontent\.com\/)([A-Za-z0-9._-]+\/[A-Za-z0-9._-]*get-shit-done[A-Za-z0-9._-]*)/g; // forge-allow-legacy-name
...
const SCAN_DIRS = ['bin', 'hooks', 'scripts', 'forge-core'];
...
const EXEMPT = new Set([
  path.join('forge-core', 'bin', 'lib', 'package-identity.cjs'),
...
function scanRepo(root) {
  const seam = require(path.join(root, 'forge-core', 'bin', 'lib', 'package-identity.cjs'));
