...
const RULES = [
  ...
  },
  {
    ...
    // src/*.cts compiles into forge-core/bin/lib/*.cjs; a source-only edit must
  ...
  },
  {
    ...
    match: path => path.startsWith('bin/') ||
      path.startsWith('forge-core/bin/') ||
  ...
  },
  {
    ...
    match: path => path.startsWith('forge-core/workflows/'),
    tests: [
...
function classify(files) {
  ...
  for (const file of files) {
    ...
    if (
      ['bin/', 'src/', 'forge-core/', 'agents/', 'commands/', 'hooks/', 'tests/', 'scripts/'].some(p => file.startsWith(p)) ||
