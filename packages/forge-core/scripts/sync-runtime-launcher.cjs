 ...
 *
 * Idempotent transform: for every forge-core/workflows/*.md (and subdirs)
 ...
 *      previously-inserted forge_run preamble).
 *   2. Replace $GSD_SDK tokens with forge_run (idempotent).
 *   3. Insert the canonical preamble at the TOP of ONLY the FIRST bash block
 *      (document order) that contains a forge_run call. All other bash blocks
 *      keep their forge_run calls with NO preamble. (Define once per file,
...
function stripAndReplace(lines, preamble) {
  ...
  // Step 2: Replace $GSD_SDK tokens with forge_run
 ...
 *      GSD_TOOLS="...forge-tools.cjs"   (or ...${_GSD_SHIM_NAME})
 *      if [ -f "$GSD_TOOLS" ]; then
 *        ...
 *      fi
 *
 ...
 *
 * F) The forge_run() function definitions that are part of the preamble
...
function removeResolverLines(lines, preamble) {
  ...
  while (i < lines.length) {
    ...
    if (
      /^GSD_TOOLS=.*forge-tools\.cjs.*;\s*if\s+\[/.test(trimmed) ||
    ...
    // A) Multi-line form: starts with GSD_TOOLS=...forge-tools.cjs or GSD_TOOLS=...${_GSD_SHIM_NAME}
    if (
      /^GSD_TOOLS=.*forge-tools\.cjs"$/.test(trimmed) ||
    ...
    if (/^GSD_SDK=/.test(trimmed) && !/forge_run/.test(trimmed)) {
      i++;
      continue;
    }
    ...
    // and forge_run() function definition lines.
...
function isSdkComment(line) {
  ...
  return /^\s*#\s*SDK resolution/.test(line) || /^\s*#.*prefer local.*forge-tools/.test(line);
}
...
function isCanonicalPreambleComment(line) {
  return /^\s*#\s*Runtime launcher:.*prefer local forge-tools\.cjs.*installed forge-tools on PATH/.test(line);
}

/**
 * Replace all $GSD_SDK tokens with forge_run in a line.
 * Handles: $GSD_SDK, ${GSD_SDK} forms.
 */
function replaceGsdSdk(line) {
  return line.replace(/\$\{?GSD_SDK\}?(?=\s|$|;|"|'|\))/g, 'forge_run');
}
 ...
 * 2. Find the FIRST block (document order) that has a forge_run call.
...
function transformFile(content, preamble) {
  ...
  let changed = false;
  let firstGsdRunBlockIdx = -1; // index into shellBlockRanges of the first forge_run block (after strip)

  // First: strip + replace in all blocks, find first forge_run block
  ...
  // Find the first block that has a forge_run call (after stripping)
  for (let bi = 0; bi < strippedBlocks.length; bi++) {
    if (strippedBlocks[bi].some((l) => /\bforge_run\b/.test(l))) {
      firstGsdRunBlockIdx = bi;
      break;
    }
  }

  // Insert preamble into the first forge_run block only
