 ...
 *    For each commands/forge/*.md, parse requires: and walk the body for
 *    references to other GSD skills (pattern: /forge:<stem> or forge:<stem>).
 ...
 *
 * Usage:
 *   node scripts/lint-skill-deps.cjs               # scans commands/forge/
...
const PROFILES_MODULE = path.join(__dirname, '..', 'forge-core', 'bin', 'lib', 'install-profiles.cjs');
...
let commandsDir = path.join(__dirname, '..', 'commands', 'forge');
    ...
    for (const ref of referenced) {
      ...
      if (!allStems.has(ref)) {
        violations.push({
          ...
          filePath,
          undeclared: ref,
          message: 'body references unknown skill forge:' + ref,
        });
        continue;
      }
      if (!declaredSet.has(ref)) {
        violations.push({
          ...
          filePath,
          undeclared: ref,
          message: 'body references forge:' + ref + ' but requires: does not list it',
        });
      }
    }
  }
