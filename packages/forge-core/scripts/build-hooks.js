...
const HOOKS_TO_COPY = [
  ...
  // plugin install, ~/.claude/forge-core is never created, so every
  // `@~/.claude/forge-core/...` include in agents/commands/templates resolves to
  // nothing. This hook symlinks the canonical path's immutable subdirs to the
  // plugin's bundled forge-core/ tree; no-op in classic installs. Must ship to
  // dist so the installer copies it into the target hooks/ dir.
  'forge-ensure-canonical-path.js',
  // Required by forge-check-update-worker.js at runtime — must ship alongside it
  ...
  // Claude Code FileChanged hook (#770) — hot-reloads forge config when
...
// helpers (e.g. hooks/lib/forge-graphify-rebuild.sh) resolve from the hook's
...
function build() {
  ...
  for (const hook of HOOKS_TO_COPY) {
    ...
    // hooks lacked their "# forge-hook-version:" header. POSIX rename(2)
