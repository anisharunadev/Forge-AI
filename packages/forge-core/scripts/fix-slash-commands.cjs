 ...
 *
 * - Default direction (transformContent): retired /forge-<cmd> → /forge:<cmd>
 *   (keeps monorepo sources, docs, and workflows in the active colon form).
 * - Reverse direction (transformContentToHyphen): /forge:<cmd> / forge:<cmd> → forge-<cmd>
 *   (used during skill installation for runtimes that register skills under the
 *   canonical hyphen form established in #2808).
 *
 * Both directions only rewrite known commands from `commands/forge/*.md` (longest-first
 * matching + word-boundary safety). Non-commands (forge-sdk, forge-tools, etc.) are
...
const SEARCH_DIRS = [
  path.join(__dirname, '..', 'forge-core', 'bin', 'lib'),
  path.join(__dirname, '..', 'forge-core', 'workflows'),
  path.join(__dirname, '..', 'forge-core', 'references'),
  path.join(__dirname, '..', 'forge-core', 'templates'),
  path.join(__dirname, '..', 'forge-core', 'contexts'),
...
function buildPattern(cmdNames) {
  // Empty input would compile `/forge-()(?=[^a-zA-Z0-9_-]|$)/g`, which the regex
  // engine still matches at any `/forge-` token followed by a non-word boundary
  // (e.g. EOL, whitespace, punctuation) — rewriting it to a stray `/forge:`.
  ...
  return new RegExp(`/forge-(${sorted.join('|')})(?=[^a-zA-Z0-9_-]|$)`, 'g');
}

/**
 * Pure transform: rewrite retired `/forge-<cmd>` to `/forge:<cmd>` for the given command names.
 * Returns the rewritten string. Identifiers not in `cmdNames` (e.g. `/forge-sdk`,
 * `/forge-tools`) are left untouched.
 */
function transformContent(src, cmdNames) {
  const pattern = buildPattern(cmdNames);
  if (!pattern) return src;
  return src.replace(pattern, (_, cmd) => `/forge:${cmd}`);
}

/**
 * Build regex for the reverse direction (colon form → hyphen form).
 * Matches both "forge:cmd" and "/forge:cmd" (the leading / is preserved automatically
...
function buildColonPattern(cmdNames) {
  ...
  return new RegExp(`(?<![a-zA-Z0-9_-])forge:(${sorted.join('|')})(?=[^a-zA-Z0-9_-]|$)`, 'g');
}

/**
 * Pure transform (reverse): rewrite `/forge:<cmd>` / `forge:<cmd>` to hyphen form
 * for known GSD commands.
 *
 * Non-command identifiers (e.g. forge-sdk, forge-tools) are left untouched, matching
...
function readCmdNames() {
  try {
    ...
    // global Claude installs (no local commands/forge/ directory). Return [] so
