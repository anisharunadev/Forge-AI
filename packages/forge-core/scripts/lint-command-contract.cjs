 ...
 *
 * Enforces the commands/forge/*.md contract across all 65 command files:
 *
 *   1. name:        present, non-empty, matches forge: or forge- prefix
...
function check(filePath) {
  ...
  // 1. name: present + forge: / forge- prefix
  if (!fm.name || !fm.name.trim()) {
    violations.push('name: field missing or empty');
  } else if (!/^gsd[:-]/.test(fm.name.trim())) {
    violations.push(`name: must start with "forge:" or "forge-", got "${fm.name.trim()}"`);
  }
