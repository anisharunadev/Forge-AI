...
// ADR-457 build-at-publish: forge-core/bin/lib/*.cjs is generated from
...
//   2. Verify every src/*.cts (non-.d.cts) maps to a non-empty forge-core/bin/lib/*.cjs.
...
function ensureBuiltArtifacts(overrides = {}) {
  ...
  const outDir = overrides.outDir || join(root, 'forge-core', 'bin', 'lib');
  ...
  // Handles subdirectories (e.g. src/installer-migrations/*.cts → forge-core/bin/lib/installer-migrations/*.cjs).
  ...
  // #996 placed the tsbuildinfo inside forge-core/bin/ (a copied/shipped tree), which
  ...
  const legacyTsBuildInfo = join(root, 'forge-core', 'bin', 'tsconfig.build.tsbuildinfo');
...
function main() {
  ...
  // Sandbox the overlay home so the loader's global scan ($GSD_HOME/.forge/capabilities)
  ...
  // invocation (that churned ~20+ temp dirs per harness run and amplified Docker load).
  {
    ...
    if (!_gh || !_basename(_gh).startsWith('forge-test-home-')) {
      process.env.GSD_HOME = mkdtempSync(_join(tmpdir(), 'forge-test-home-'));
    }
  }
