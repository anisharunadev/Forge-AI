 ...
 *     - Assert commands/forge/<cmd>.md exists in the installed package
 *     - Parse the .md for a workflow @-import or inline reference
 *     - Assert the referenced workflow .md exists in the installed package
 *   If 'init' is in lifecycleCommands, runs `forge-core --local --claude`
 ...
 *   - Scans all installed forge-core/workflows/*.md for /forge:<known-cmd>
...
const { PACKAGE_NAME } = require('../forge-core/bin/lib/package-identity.cjs');
...
/**
 * Locate the lib/node_modules/@forge-ai/forge-core package root inside
...
/**
 * Locate the installed forge-tools binary (symlink in <prefix>/bin/).
 */
function findGsdToolsBin(installPrefix) {
  for (const c of binCandidates(installPrefix, 'forge-tools')) {
...
/**
 * Locate the forge-core installer binary (the symlink in <prefix>/bin/).
 */
function findInstallerBin(installPrefix) {
  for (const c of binCandidates(installPrefix, 'forge-core')) {
    if (fs.existsSync(c)) return c;
  }
  return null;
}
 ...
 *
 *   1. @-import line:  `@~/.claude/forge-core/workflows/<name>.md`
 *   2. Inline mention: any line containing `~/.claude/forge-core/workflows/<name>.md`
...
function parseWorkflowRef(mdContent) {
  const WORKFLOW_PREFIX = 'forge-core/workflows/';
...
function readInstalledCmdNames(pkg) {
  const commandsDir = path.join(pkg, 'commands', 'forge');
...
/**
 * Scan a single workflow .md file for /forge:<cmd> colon-namespace leaks.
 *
 * Uses the word-boundary-safe regex shape from scripts/fix-slash-commands.cjs:
 *   /forge-(<cmd1>|<cmd2>|...)(?=[^a-zA-Z0-9_-]|$)/g  — forward
 * We check the colon form: /forge:<cmd> leaking in installed workflow bodies.
 *
...
function scanWorkflowColonLeak(filePath, cmdNames) {
  ...
  const pattern = new RegExp(`/forge:(${sorted.join('|')})(?=[^a-zA-Z0-9_-]|$)`, 'g');
...
function runSmoke({
  ...
  // --- Locate the installed forge-tools binary --------------------------------
  const actualBin = findGsdToolsBin(installPrefix);

  if (!actualBin) {
    const searched = binCandidates(installPrefix, 'forge-tools');
    return {
  ...
  // --- Invoke `forge-tools --help` to assert the shipped binary is callable ---
  ...
  if (shouldRunInit && fixtureDir) {
    ...
    if (!installerBin) {
      return {
        ...
        details: {
          ...details,
          reason: 'forge-core binary not found in installPrefix',
          installPrefix,
        },
      };
    }
    ...
    const expectedDirs = [
      ...
      path.join(fixtureDir, '.claude', 'forge-core'),
    ];
  ...
  for (const cmd of commandsToCheck) {
    const cmdFilePath = path.join(pkg, 'commands', 'forge', `${cmd}.md`);
    ...
    if (workflowName) {
      // Workflow files live at forge-core/workflows/<name> in the package.
      // Some live in subdirectories; try flat first then scan once.
      const flat = path.join(pkg, 'forge-core', 'workflows', workflowName);
  ...
  const workflowsDir = path.join(pkg, 'forge-core', 'workflows');
...
function cliMain() {
  ...
  const packDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-smoke-pack-'));
  const installPrefix = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-smoke-prefix-'));
  const fixtureDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-smoke-fixture-'));
