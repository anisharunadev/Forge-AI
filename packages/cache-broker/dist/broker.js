...
export class CacheBroker {
     ...
     */
    async set(ctx, parts, value, opts = {}) {
        if (parts.tenant_id !== ctx.tenant_id) {
            ...
            throw new TenantMismatchError(`cache-broker: set tenant_id '${parts.tenant_id}' does not match context tenant_id '${ctx.tenant_id}'`, parts.tenant_id, ctx.tenant_id);
        }
        const key = deriveKey(parts);
        const payload = { __tnt: ctx.tenant_id, v: value, ts: Date.now() };
    ...
    async del(ctx, parts) {
        if (parts.tenant_id !== ctx.tenant_id) {
            throw new TenantMismatchError(`cache-broker: del tenant_id '${parts.tenant_id}' does not match context tenant_id '${ctx.tenant_id}'`, parts.tenant_id, ctx.tenant_id);
        }
     ...
     */
    async emitTenancyDenied(ctx, details) {
        const event = {
