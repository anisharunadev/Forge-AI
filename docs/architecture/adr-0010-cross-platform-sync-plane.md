...
| Field             | Value                                                                                                                                          |
|-------------------|------------------------------------------------------------------------------------------------------------------------------------------------|
| **Status**        | **Accepted** — Board answered both open questions on 2026-06-17T20:56:59Z (FORA-199 interaction `02d96f8c-…`): Q-`clipup` = **ClickUp**, Q-`sync-direction` = **every event**. |
| **Date**          | 2026-06-17                                                                                                                                     |
...
| **Issue**         | [FORA-199](/FORA/issues/FORA-199) (this ADR)                                                                                                  |
| **Parent**        | [FORA-62](/FORA/issues/FORA-62) Sub-goal 1.4 (Jira sync, `in_review`) — temporarily anchored here until the new Epic is created (see §10)       |
| **Source**        | Board response to FORA-62 `ask_user_questions` interaction `4c94c158-…`, 2026-06-17T16:25Z, resolved by `local-board`                            |
| **Parent ADR**    | [ADR-0001](./adr-0001-master-orchestrator-sdlc-architecture.md), [ADR-0006](./adr-0006-event-bus-nats-jetstream.md), [ADR-0008](./adr-0008-paperclip-approvals.md), [FORA-125 IAM shipped](/FORA/docs) |
| **Supersedes**    | none                                                                                                                                           |
| **Superseded by** | none                                                                                                                                           |
...
A Paperclip `agent` has no first-class human identity in the customer's IdP. The decision is **synthetic service accounts, one per agent, scoped per platform per tenant**, plus a per-agent **persona display name** so humans see "DocAgent (FORA-118)" instead of an opaque UUID.
...
| Paperclip agent acting **on behalf of a human** (e.g. PM triages a customer report) | The human's IdP identity is used; the agent is recorded as the *editor* in audit but the comment author is the human. Implemented via OAuth delegation: the agent has `delegate_for` scope to act as the human, gated by the FORA-125 IAM policy and the customer's policy DSL |
...
```json
{
  ...
  "author": {
    ...
    "display_name": "DocAgent (FORA-118)"
  },
...
4. **Audit divergence**: Audit log missing a `event.*` row that the sync log says should be there → **P0 alert**, page on-call. This is the canary that the audit forwarder (FORA-36) is healthy.

## 8. Audit + risk register

The S7 / Epic 1 audit wedge (FORA-36, audit-system-design ADR) covers Paperclip-side events only. Cross-platform sync adds three new audit surfaces. Decision: **all sync events flow through the existing FORA-36 audit forwarder**; we do not invent a second audit pipeline.

### 8.1 New event types (extending FORA-36)
...
| R-SYNC-02 | Service-account token sprawl (the synthetic accounts accumulate broad scopes if not least-privilege'd per platform) | P0 | Per-platform install-time OAuth with explicit scope list; deny scopes outside the Sync Plane allow-list (FORA-125 IAM policy DSL); quarterly scope audit |
...
| R-SYNC-04 | Cross-tenant data leak (the Sync Plane confuses two tenants on a shared remote like a shared Jira Cloud) | P0 | Tenant ID is part of every adapter key, every JetStream subject, and every remote write; deny-by-default cross-tenant references; per-tenant webhook secrets (FORA-161 pattern) |
...
| R-SYNC-07 | "clipup" being a custom internal tool introduces an unknown threat model | P1 | Treated as a black-box adapter; no source-side trust; all writes pass through the FORA-125 broker check |
...
8. **Sync Plane audit forwarding to FORA-36** — wire the eight `sync.*` event types into the existing audit forwarder. (Engineer + Security)
...
Both questions were answered by `local-board` on **2026-06-17T20:56:59Z** on FORA-199 interaction `02d96f8c-…`:
...
- **Re-implementing Epic 1's approval-gate** (ADR-0008) in remote systems — remote state changes that need Paperclip approval go through the existing approval interaction (FORA-123, FORA-168), not a parallel gate on the remote.

## 12. Decision status

**Accepted** (2026-06-17). Sub-task #1–#8 can start in parallel with the Architect hire; sub-tasks #9–#10 are Security-owned follow-ons. The CEO adjudication on whether the new "Forge Integration Layer" Epic exists and how it is named is the remaining gating step (per FORA-17 Board scope expansion).
...
- [FORA-36](/FORA/issues/FORA-36) — Audit 0.5 (FORA-36 ADR D1 — audit-system-one-way-doors)
- [FORA-62](/FORA/issues/FORA-62) — Sub-goal 1.4 Jira sync (parent of this ADR; `in_review`)
- [FORA-117 DocIndex](/FORA/docs/adr/0002-knowledge-layer-storage-contract.md) — Knowledge Layer storage contract (relevant for the canonical comment envelope shape)
- [FORA-125 IAM](/FORA/docs/adr/0008-paperclip-approvals.md) — agent IAM shipped (policy DSL for §5 actor mapping)
- [FORA-126 Customer Cloud Broker](/FORA/docs/adr/0006-event-bus-nats-jetstream.md) — deny-list + audit patterns (relevant for §8 R-SYNC-02)
- [FORA-161 IdP Revoke Webhook](/FORA/issues/FORA-161) — per-tenant webhook secrets pattern (relevant for §8 R-SYNC-04)
