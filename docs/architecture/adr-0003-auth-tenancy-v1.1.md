...
| **Issue**       | [FORA-526](/FORA/issues/FORA-526) — 0.1.8 In-process JWT validation |
| **Parent ADR**  | [ADR-0003 §4.2](/FORA/docs/architecture/adr-0003-auth-tenancy.md) |
...
ADR-0003 v0.1 (FORA-38, accepted 2026-06-19) defined the brokered identity model:
a single `identity-broker` service terminates OIDC, mints FORA-issued access tokens,
and the Orchestrator (along with the other FORA services) consumes those tokens.
...
Orchestrator and the rest of FORA's services then trusted that header as the
...
2. **An untrusted LB / sidecar cannot be in front of the Orchestrator.**
   ...
   Orchestrator will trust. v0.1 implicitly required a FORA-owned
   gateway in front of every Orchestrator deployment.
3. **The broker's own audit (FORA-36) is bypassed for service-to-service
...
FORA-526 is the v1.1 amendment that closes these gaps. The decision: the
...
- The broker is still the only service that mints FORA-issued tokens.
...
- The broker still owns revocation (FORA-36 audit events; per-tenant
  `jti` deny-list).
...
3. **`request.tenantContext`** carries `tenantId` / `actorId` /
   ...
   rest of the request handlers (FORA-50 §4.1 routes, the
...
4. **The legacy `x-fora-tenant-id` header is REMOVED from the trust
   ...
   `FORA_REQUIRE_JWT=false` (local dev). Production MUST run with
   `FORA_REQUIRE_JWT=true` (the default). A process-startup warning
...
|---|---|---|
| `FORA_JWT_VERIFIER_URL` | `{IDENTITY_BROKER_PUBLIC_URL}/.well-known/jwks.json` | The broker's JWKS document |
| `FORA_JWT_ISSUER` | `identity-broker.fora.local` | Expected `iss` claim |
| `FORA_JWT_AUDIENCE` | `forge-runtime` | Expected `aud` claim |
| `FORA_REQUIRE_JWT` | `true` | When `false`, fall back to `x-fora-tenant-id` (LOCAL DEV ONLY) |
| `FORA_JWT_CLOCK_TOLERANCE_SEC` | `0` | Allow N seconds of clock skew on `exp` / `nbf` |

## 3. Why now (and not later)

Three concrete pressure points make this the right time:

1. **FORA-38 (Epic 0.7 auth foundation) shipped 2026-06-19.** The
   broker is live and minting tokens. The Orchestrator is the
   first FORA service that consumes those tokens; verifying in
   process is the smallest change that gets the broker → service
   contract into a deployable state.
2. **FORA-110 (Epic 0.1 Master Orchestrator) shipped 2026-06-20.**
   The v0.1 ship was a v0.1 deployment, with the gateway-stamp
   model. FORA-526 is the first v0.2 follow-up that removes a
...
1. **Phase 1 — v1.1 ships behind a feature flag.** The new
   `FORA_REQUIRE_JWT` defaults to `true`, so a fresh deployment
   requires the JWT. The existing gateway-stamp path still works
   when `FORA_REQUIRE_JWT=false` is set.
2. **Phase 2 — Production cutover.** Operators flip
   `FORA_REQUIRE_JWT=true` in the production env. The gateway
...
3. **Phase 3 — Gateway retirement.** The FORA-owned gateway is
...
- **FORA-50 §4.1 routes** — unchanged. The `requireTenant` /
  ...
  header.
- **FORA-137 approval router** — unchanged. The router receives
  the request after the hook has run, so `tenantContext` is
  already stamped.
- **Audit (FORA-36 §8.1)** — additive. The hook logs the JWT
...
- **Per-tenant rate-limit (FORA-437)** — unchanged. The
...
|---|---|---|
| `jose` `createRemoteJWKSet` flapping when the broker rotates keys | `jose` caches the JWKS internally (default 10 min) and refreshes on `kid` miss; the `JwtValidator` constructor accepts a `clockToleranceSec` to absorb small skew | Bypass the hook via `FORA_REQUIRE_JWT=false` to fall back to the v0.1 gateway-stamp model. The flag is the rollback switch. |
| A clock-skew bug between broker and orchestrator locks out all clients | `FORA_JWT_CLOCK_TOLERANCE_SEC=30` opens a 30-second window. Default is 0; bump on incident. | Bump the env var; no rebuild required. |
...
| The local-dev opt-out (`FORA_REQUIRE_JWT=false`) leaks into production | A startup `app.log.warn` fires whenever the flag is off; CI runs with the flag on; a smoke test asserts the flag is `true` in the production build profile | Re-deploy with `FORA_REQUIRE_JWT=true`. |
...
The rollback switch is `FORA_REQUIRE_JWT=false`. The flag restores the
v0.1 deployment model in one config flip, with no rebuild.

## 7. Acceptance criteria (FORA-526 §Acceptance)

| FORA-526 AC | Where it lands |
|---|---|
...
| 4. New env vars: `FORA_JWT_VERIFIER_URL` / `FORA_JWT_ISSUER` / `FORA_JWT_AUDIENCE` / `FORA_REQUIRE_JWT` / `FORA_JWT_CLOCK_TOLERANCE_SEC` | `src/config.ts` |
...
- **Token revocation propagation** — the broker still owns
  revocation (FORA-36). v1.1 does NOT add a per-`jti` deny-list
...
sub-decision. The wire format is unchanged (FORA-issued JWTs
