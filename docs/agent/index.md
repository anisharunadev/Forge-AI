...
## 6. The 8 Immutable Rules (Quick Reference)

These rules **MUST NOT be violated**. Every AI agent working on Forge MUST honor them:

1. **R1 — Provider-agnostic**: All LLM traffic goes through LiteLLM Proxy. NO direct SDK imports.
2. **R2 — Multi-tenancy by default**: Every record carries `tenant_id` + `project_id`. RLS enforced at DB level.
3. **R3 — Human approval gates**: No autonomous crossing of Architecture / Security / Deployment boundaries.
4. **R4 — Typed artifacts only**: ADRs, contracts, risks, task breakdowns, deployment plans.
5. **R5 — Layer isolation**: Org Knowledge shared; Project Intelligence isolated.
6. **R6 — Mandatory auditability**: Every action — agent, model, prompt, tool, cost, artifact, timestamp, result.
7. **R7 — Mandatory observability**: OpenTelemetry tracing, metrics, logs from day one.
8. **R8 — Configurable everything**: No hardcoded GitHub / Claude / OpenAI / AWS / Jira assumptions.

Full rules: [`reference/8-rules.md`](./reference/8-rules.md) + `CLAUDE.md`

---

## 7. Naming Conventions (so AI agents don't get confused)

| Term | Meaning |
|---|---|
| **Tenant** | An organization in Forge (e.g., `acme-corp`) |
| **Project** | A workspace under a tenant (e.g., `acme-platform`) |
| **Workspace** | The user-facing synonym for "tenant" |
...
| **Runtime** | A sandbox where agents execute (e.g., `local-docker`, `production-k8s`) |
...
| **Steward** | The persona responsible for org knowledge + standards |
| **LiteLLM** | The AI gateway that Forge proxies all LLM traffic through |
