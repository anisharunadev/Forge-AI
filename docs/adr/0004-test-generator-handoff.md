...
| **Issue**         | [FORA-41](/FORA/issues/FORA-41)                                                                |
...
as a fixture by `agents/qa/smoke_test.py` (see FORA-46).
...
5. **Defer the contract to FORA-43 / FORA-46.** Rejected: per `qa.md` §3,
   the contract is the source of truth; without it, FORA-43 (the QA
   skeleton) and FORA-46 (the smoke test) cannot be reviewed against
...
- The **Self-Healing Test** agent's `RepairProposal` contract
  (`agents/selfhealing/schemas.py`, FORA-91). Same pattern; not this ADR.
...
- [FORA-46](/FORA/issues/FORA-46) — Smoke test for QA Agent (blocked by FORA-43)
- [FORA-49](/FORA/issues/FORA-49) — Wire QA Agent to @fora/mcp-github (blocked by FORA-43 and FORA-46)
- [FORA-65](/FORA/issues/FORA-65) — Document QA playbook in `workspace/memory/qa.md` (already drafted in this PR)
- [FORA-91](/FORA/issues/FORA-91) — Self-Healing v1 scaffold (Phase 4, separate contract)
