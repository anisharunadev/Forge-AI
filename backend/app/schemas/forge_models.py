...
class ModelDescriptor(BaseModel):
    ...
    tier: str | None = Field(
        default=None, description="Coarse tier label, e.g. 'pro' | 'flagship' | 'small'"
    )
    context_window: int | None = Field(
        default=None, description="Max context tokens reported by LiteLLM"
    )
    ...
    allowed_for_caller: bool = Field(
        description="True if the caller's virtual key permits this model"
    )
    ...
    input_cost_per_token: float | None = Field(
        default=None, description="USD per input token (from cost map)"
    )
    output_cost_per_token: float | None = Field(
        default=None, description="USD per output token (from cost map)"
    )
