...
class TechRadarEntry(Base, UUIDPrimaryKeyMixin, TenantScopedMixin, TimestampMixin):
    """Tech Radar blip (Day 2 mock-removal track G).

    Quadrants: languages | tools | platforms | techniques.
    Rings: adopt | trial | assess | hold.
    ``prev_ring`` is the previous ring if the blip moved this cycle.
    """

    __tablename__ = "architecture_tech_radar"

    name: Mapped[str] = mapped_column(String(120), nullable=False)
    quadrant: Mapped[str] = mapped_column(String(32), nullable=False)
    ...
    prev_ring: Mapped[str | None] = mapped_column(String(16), nullable=True)
