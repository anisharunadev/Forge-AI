...
async def get_idea(
    ...
    try:
        idea = await idea_intake_service.get_idea(idea_id, tenant_id=principal.tenant_id)
...
async def get_analysis(
    ...
    try:
        analysis = await idea_analysis_service.get_analysis(idea_id, tenant_id=principal.tenant_id)
    except LookupError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
    if analysis is None:
        return None
