...
class ArchitectureVersion:
    def __init__(self, version_id, artifact_type, artifact_id, version_number, content_hash, snapshot_reason, actor_id, created_at):
...
class ArchitectureVersioningService:
    ...
    async def create_version(self, artifact_type: str, artifact_id: UUID, snapshot_reason: str, actor_id: UUID) -> ArchitectureVersion:
        ...
        if self.event_bus:
            await self.event_bus.publish("ArchitectureVersionCreated", {"version_id": str(version.version_id)})
        return version
    ...
    async def rollback_to_version(self, artifact_type: str, artifact_id: UUID, version_id: UUID, actor_id: UUID) -> ArchitectureVersion:
        return await self.create_version(artifact_type, artifact_id, f"rollback to {version_id}", actor_id)
