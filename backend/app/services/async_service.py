...
class AsyncService:
    ...
    async def upload_file(self, *, tenant_id: UUID, payload: FileCreate) -> FileRead:
    ...
    async def get_file(self, *, tenant_id: UUID, file_id: str) -> FileRead:
    ...
    async def delete_file(self, *, tenant_id: UUID, file_id: str) -> dict[str, Any]:
    ...
    async def download_file(self, *, tenant_id: UUID, file_id: str) -> bytes:
    ...
    async def create_batch(self, *, tenant_id: UUID, payload: BatchCreate) -> BatchRead:
    ...
    async def get_batch(self, *, tenant_id: UUID, batch_id: str) -> BatchRead:
    ...
    async def cancel_batch(self, *, tenant_id: UUID, batch_id: str) -> BatchRead:
    ...
    async def batch_results(self, *, tenant_id: UUID, batch_id: str) -> BatchResultsResponse:
        batch = await self.get_batch(tenant_id=tenant_id, batch_id=batch_id)
        raw_bytes = b""
        if batch.output_file_id:
            raw_bytes = await self.download_file(tenant_id=tenant_id, file_id=batch.output_file_id)
    ...
    async def get_fine_tune_job(self, *, tenant_id: UUID, job_id: str) -> FineTuneJobRead:
        upstream = await self._client().ft_jobs_get(job_id)
        if "_status" in upstream:
            raise AsyncError("job_not_found", f"job {job_id} not found", job_id=job_id)
        return FineTuneJobRead.model_validate(upstream)

    async def cancel_fine_tune_job(self, *, tenant_id: UUID, job_id: str) -> FineTuneJobRead:
    ...
    async def start_response(self, *, tenant_id: UUID, payload: ResponseCreate) -> ResponseRead:
    ...
    async def get_response(self, *, tenant_id: UUID, response_id: str) -> ResponseRead:
    ...
    async def cancel_response(self, *, tenant_id: UUID, response_id: str) -> ResponseRead:
    ...
    async def append_response_inputs(
        ...
        *,
        ...
        upstream = await self._client().responses_input_items(response_id, payload.model_dump())
        if "_status" in upstream:
            raise AsyncError(
                "response_not_found", f"response {response_id} not found", response_id=response_id
            )
    ...
    async def compact_response(self, *, tenant_id: UUID, payload: CompactRequest) -> dict[str, Any]:
    ...
    async def stream_response(self, *, tenant_id: UUID, response_id: str) -> Any:
        ...
        async def _gen():
            ...
            while True:
                snap = await self.get_response(tenant_id=tenant_id, response_id=response_id)
                chunk = f"event: response.snapshot\ndata: {snap.model_dump_json()}\n\n"
    ...
    def _shape_file(upstream: dict[str, Any]) -> FileRead:
        ...
        if (
            ...
            and "data" in upstream
            and isinstance(upstream["data"], list)
            and upstream["data"]
        ):
