...
async def test_under_budget_passes(two_tenants) -> None:
    ...
    with patch.object(
        cost_ledger, "get_total_for_tenant", AsyncMock(return_value=10.0)
    ):
        out = await tenant_budget_guard.check_pre_call(
            tenant_id=ta.id, est_cost_usd=0.0
        )
...
async def test_at_budget_warns_passes(two_tenants) -> None:
    ...
    ta, _tb, _pa = two_tenants
    with patch.object(
        cost_ledger, "get_total_for_tenant", AsyncMock(return_value=4950.0)
    ):
        out = await tenant_budget_guard.check_pre_call(
...
async def test_over_budget_raises(two_tenants) -> None:
    ...
    ta, _tb, _pa = two_tenants
    with patch.object(
        cost_ledger, "get_total_for_tenant", AsyncMock(return_value=5100.0)
    ):
...
async def test_enforcement_flag_disables_guard(two_tenants) -> None:
    ...
    with patch.object(
        cost_ledger, "get_total_for_tenant", AsyncMock(return_value=99999.0)
    ):
        out = await tenant_budget_guard.check_pre_call(
            tenant_id=ta.id, est_cost_usd=0.0
        )
...
async def test_tenant_ceiling_override(two_tenants) -> None:
    ...
    with patch.object(
        cost_ledger, "get_total_for_tenant", AsyncMock(return_value=101.0)
    ):
        with pytest.raises(TenantBudgetExceeded) as exc_info:
            await tenant_budget_guard.check_pre_call(
                tenant_id=ta.id, est_cost_usd=0.0
