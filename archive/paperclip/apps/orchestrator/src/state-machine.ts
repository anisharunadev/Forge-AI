 ...
 */
const VERB_NEXT_STATUS: Readonly<
  Record<LifecycleVerb, Readonly<Partial<Record<RunStatus, RunStatus>>>>
> = {
...
export function nextStatus(
  ...
  if (next === undefined) {
    ...
    throw new Error(
      `nextStatus: invalid transition verb=${verb} from current=${current}`,
    );
  }
  return next;
}
 ...
 */
export function currentStageOnVerb(
  _verb: LifecycleVerb,
  currentStage: string,
): string {
  return currentStage;
