...
import { STAGES_IN_ORDER, type RunRecord, type StageRecord, type TenantId } from './types.js';
...
const noopLogger: RehydrateLogger = () => undefined;
 ...
 */
export async function buildRecoveryTickets(
  pool: Pool,
  tenantId: TenantId,
  logger: RehydrateLogger = noopLogger,
): Promise<ReadonlyArray<RecoveryTicket>> {
  const runs = await listActiveRunsForRecovery(pool, tenantId);
  const tickets: RecoveryTicket[] = [];
