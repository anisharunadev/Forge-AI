 ...
 * a second query.
 */
export function gateOf(record: ApprovalRecord): GateKind {
  return record.gate_kind;
}

/** Helper: true iff the interaction primitive for `kind` matches `p`. */
export function primitiveMatches(
  kind: GateKind,
  p: PaperclipPrimitive,
): boolean {
  ...
  if (kind === 'launch') return p === 'request_board_approval';
