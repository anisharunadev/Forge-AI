...
export async function openNatsConnection(
  url: string,
  ...
  let js: import('nats').JetStreamClient | undefined;
  try {
    js = nc.jetstream();
  } catch {
    js = undefined;
 ...
 */
export function natsProducerFactoryFor(
  bundle: NatsConnectionBundle,
): (tenantId: string) => EventProducer {
  return (tenantId: string) => {
    const cfg: NatsProducerConfig = {
      nc: bundle.nc,
...
}): Promise<{ bus: NatsApprovalEventBus; bundle: NatsConnectionBundle }> {
 ...
 */
export class NatsApprovalEventBus implements EventBus {
  private readonly producers = new Map<string, EventProducer>();
  private closed = false;
  private readonly log: NonNullable<NatsApprovalEventBusOptions['log']>;

  constructor(private readonly opts: NatsApprovalEventBusOptions) {
    this.log = opts.log ?? (() => {});
  }
  ...
  private producerFor(tenantId: string): EventProducer {
 ...
 */
function eventTenantId(event: ApprovalEvent): string {
  // ApprovalEvent is a discriminated union; every variant carries tenantId.
  return event.tenantId as unknown as string;
}
 ...
 */
function project(event: ApprovalEvent): {
  eventType: EventType;
  payload: unknown;
} {
  switch (event.type) {
    ...
    case 'approval_decided': {
      const decision =
        event.decision === 'accept' ? 'approved' : 'rejected';
      return {
