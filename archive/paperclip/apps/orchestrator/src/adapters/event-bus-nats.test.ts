...
class MultiTenantFakeProducer {
  public readonly perTenant = new Map<string, InMemoryEventProducer>();
  ...
  all(): ReadonlyArray<{ tenantId: string; subject: string; envelope: TypedEvent<EventType> }> {
    return this.publishes;
  }

  /** Envelopes for a single tenant, in publish order. */
  forTenant(tenantId: string): ReadonlyArray<{ subject: string; envelope: TypedEvent<EventType> }> {
    return this.publishes
...
function buildBus(opts: Partial<NatsApprovalEventBusOptions> = {}): {
  bus: NatsApprovalEventBus;
  fake: MultiTenantFakeProducer;
} {
...
const samples = {
  ...
  } as const satisfies ApprovalEvent,
...
describe('NatsApprovalEventBus — projection + subject scheme (FORA-170 acceptance #1)', () => {
  let bus: NatsApprovalEventBus;
  let fake: MultiTenantFakeProducer;
  ...
  it('every (state-change → event) pair in the FORA-170 acceptance bar is covered', async () => {
    ...
    const cases: Array<{ event: ApprovalEvent; expectType: EventType; expectSubject: string }> = [
      {
...
describe('NatsApprovalEventBus — lifecycle', () => {
  ...
  it('disconnect flushes every per-tenant producer and is idempotent', async () => {
    ...
    await expect(bus.emit({ ...samples.approvalRequested, tenantId: A })).rejects.toThrow(
      /after close/i,
    );
  });
...
describe('natsProducerFactoryFor — substrate wiring', () => {
  it('returns a function that mints a producer scoped to a tenant', () => {
    ...
    const factory = natsProducerFactoryFor({
      nc: fakeNc as unknown as import('nats').NatsConnection,
      close: async () => {},
    });
