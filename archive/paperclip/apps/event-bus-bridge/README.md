...
The SQS+SNS bridge — subscribes to NATS JetStream on the per-tenant subject glob and republishes every event verbatim to an SNS topic so the audit account (a separate AWS account per [Forge AI-36 D3](/Forge AI/docs/adr/0001-audit-system-one-way-doors.md)) can ingest via SQS.

**Implements:** [Forge AI-136](/Forge AI/issues/Forge AI-136) · **Substrate:** [ADR-0006 §5](/Forge AI/docs/architecture/adr-0006-event-bus-nats-jetstream.md)

## Run

```sh
export Forge AI_NATS_URL=nats://nats.platform.svc:4222
export Forge AI_SNS_TOPIC_ARN=arn:aws:sns:us-east-1:111111111111:fora-audit-events
export Forge AI_AWS_REGION=us-east-1
export Forge AI_TENANT_ID=tnt_acme        # one bridge per tenant (recommended)
export Forge AI_DURABLE_NAME=audit-bridge # default: audit-bridge-<tenant>
export Forge AI_MAX_MAJOR_VERSION=1
export Forge AI_RATE_RPS=200
...
1. Tenant ACL — refuses events whose tenant segment ≠ `Forge AI_TENANT_ID`.
2. Schema-version guard — drops v2 envelopes when `Forge AI_MAX_MAJOR_VERSION=1`; the v1 subject continues to be served for the 30-day window.
