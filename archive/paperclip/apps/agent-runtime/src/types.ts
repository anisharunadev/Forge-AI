...
export interface StagePolicy {
  plan: { allowedTools: ReadonlySet<ToolName> };
  act: { allowedTools: ReadonlySet<ToolName> };
  observe: { allowedTools: ReadonlySet<ToolName> };
  reflect: { allowedTools: ReadonlySet<ToolName> };
}
...
export interface Observation {
   ...
   */
  costHint?: { tokens?: number; usd?: number };
  /**
   ...
   */
  idempotencyHit?: { key: IdempotencyKey; storedAt: string };
}
...
export function asToolResult<T>(v: unknown): ToolResult<T> | null {
  if (v && typeof v === 'object' && 'output' in (v as Record<string, unknown>)) {
    return v as ToolResult<T>;
  }
  return null;
}
...
export interface RunRecord {
  ...
  budget?: {
    ...
    spent: { tokens: number; usd: number; wallClockMs: number };
  };
}
...
export type TypedError =
  | { code: 'NotAllowed'; message: string; tool: ToolName; stage: Stage; runId: RunId }
  | { code: 'IdempotencyMissing'; message: string; handlerId: string; runId: RunId }
  | { code: 'HandlerThrew'; message: string; handlerId: string; runId: RunId; cause: string }
  | { code: 'AbortRequested'; message: string; runId: RunId }
  | { code: 'ReplanBudgetExhausted'; message: string; runId: RunId; maxReplans: number }
  | { code: 'UnknownAgent'; message: string; agentId: AgentId }
  | { code: 'AgentAlreadyRegistered'; message: string; agentId: AgentId }
  | {
      code: 'BudgetExceeded';
      message: string;
      runId: RunId;
  ...
  | { code: 'Cancelled'; message: string; runId: RunId; reason: string };

/** Helper: build a typed error. */
export function makeError<E extends TypedError>(e: E): E {
  return e;
}
