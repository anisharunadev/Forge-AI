...
exchanges a Forge AI-issued token for a short-lived cloud-native token per
...
4. Adapter lookup. Refuse if no adapter is registered (e.g. Azure
   before Forge AI-126.2 lands).
5. Adapter `assume()` with the Forge AI-issued JWT.
...
- Default port: `7100` (`Forge AI_CCB_LISTEN_PORT`).
...
apps/customer-cloud-broker/
  src/
    ...
    adapters/
      aws.ts            # STS AssumeRoleWithWebIdentity (Forge AI-126.5 per-service dispatch)
      azure.ts          # stub (Forge AI-126.2)
      gcp.ts            # WIF + per-service dispatch (Forge AI-126.3)
      sonarqube.ts      # per-project user tokens (Forge AI-321 / Forge AI-290 Path B)
...
- **Forge AI-126.2** — full Azure Workload Identity Federation adapter.
- **Forge AI-126.3** — full GCP Workload Identity Federation adapter.
- **Forge AI-126.4** — canary-assume phase of the trust probe (real STS).
- **Forge AI-126.5** — per-service AWS SDK dispatch (replaces v1 shim).
- **Forge AI-126.6** — `customer-cloud-broker` portal runbook for the
  Azure / GCP cloud pages.
- **Forge AI-321** — SonarQube adapter (per-project user tokens). Ships
  the broker-side credential layer for the Forge AI-290 read-only MCP
  server; Forge AI-290 (Path A) is the MCP boundary, Forge AI-321 (Path B) is
