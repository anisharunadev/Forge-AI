# Azure onboarding runbook (Forge AI-126 / 0.7.4)

This runbook walks a customer's Azure admin through granting Forge AI
...
issued by the Forge AI identity-broker. The customer-cloud-broker
exchanges a Forge AI-signed OIDC token for a short-lived Microsoft
...
> [Forge AI-126.2](/Forge AI/issues/Forge AI-126.2). `assume()` exchanges the
> Forge AI-issued JWT for an ARM access token via `@azure/identity`
...
- The Forge AI support team has confirmed the **OIDC issuer URL** for the
...
- **Name**: `Forge AI customer-cloud-broker` (or your tenant convention).
...
- **Audience**: `api://AzureADTokenExchange` (Microsoft's
  ...
  do not change).
- **Name**: `Forge AI deploy-agent run-*` (descriptive only).
- **Description**: `OIDC federation trust for the Forge AI
  customer-cloud-broker (Forge AI-126).`
...
federated tokens issued by the Forge AI identity-broker.
...
- `role_ref`: the broker's internal handle to the App Registration's
  ...
  Forge AI onboarding automation generates it from the App Registration
...
phase-2 canary assume (Forge AI-126.4 follow-up — not yet shipped) is
what actually proves the customer's federated credential accepts
the broker's issuer end-to-end. Until phase-2 ships, the
[Forge AI-126.2](/Forge AI/issues/Forge AI-126.2) adapter is itself the
...
| `aad_tenant_id` | The customer's **Entra ID tenant id** (UUID) hosting the App Registration. New in Forge AI-126.2; the broker uses it to construct `ClientAssertionCredential(tenantId, clientId, getAssertion)`. The agent populates this from the customer's Entra setup (e.g. `tenants/{tenant_id}/policy.yaml`). |
...
> **The broker does not log the Forge AI JWT, the Azure access token, or
...
| Azure actions return `assume_failed` with `AADSTS70021/22` | Federated credential `subject` / `issuer` | Match the agent's `sub` claim and Forge AI's OIDC issuer URL exactly |
