# Customer-cloud-broker onboarding playbook (Forge AI-126 / 0.7.4)
...
```
Forge AI identity-broker
   │ (signs short-lived Forge AI-issued JWT, ~5 min)
   ▼
Forge AI customer-cloud-broker
   │ (exchanges the Forge AI JWT via cloud-native federation)
   ▼
...
- A working Forge AI tenant (`tenants/{tenant_id}/policy.yaml` exists).
...
- The customer's IdP is already federated to Forge (Forge AI-123).
...
After the customer's IAM trust is in place, the Forge AI tenant admin
...
1. **Phase 1 — config check.** The trust record parses, the role ARN
   ...
   no-network.
2. **Phase 2 — canary assume.** The broker signs a synthetic Forge AI JWT
...
2. **Forge-initiated**: Forge AI support marks the tenant as `cloud_disabled`
