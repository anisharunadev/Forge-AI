...
Sub-goal **11.6** of [Forge AI-249 Epic 11 — Forge Integration Layer](/Forge AI/issues/Forge AI-249).
DAY-ONE P0 (per Board answer Q-sync-direction = every_event on Forge AI-199 2026-06-17T20:56:59Z).
...
- **Pattern reuse.** `TokenBucket` and `CircuitBreaker` are the same
  shape as `@fora/customer-cloud-broker/src/adapters/aws.ts` (Forge AI-126.5).
...
- **No MCP servers added.** This sub-goal is a pure library. The
  sync-plane service (Forge AI-252 / 11.1) wires the platform adapters
  when it lands.
...
- **Audit event schema.** `sync.platform.degraded` /
  ...
  ADR-0010 §8.1. The forwarder (Forge AI-204) reads from the
...
- **Per-tenant override surface.** Today the bucket defaults are
  ...
  the existing customer-cloud-broker policy DSL (Forge AI-125).
- **Composite body renderers.** The coalescer joins bodies with
  `\n\n---\n\n`; the per-platform renderer (Jira ADF vs. GitHub
  Markdown vs. ClickUp) is owned by Forge AI-253 (canonical comment
  envelope).
