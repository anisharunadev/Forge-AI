# Outbound Burst Control — Forge AI-267 / Epic 11.6

**Status:** v0.1, smoke-test green (92 assertions, 0 failures, 474 ms)
**Reference:** ADR-0010 §7.1 (failure modes), §8.1 (audit), §9 sub-task #6
**Issue:** [Forge AI-267](/Forge AI/issues/Forge AI-267) — Day-one P0 per Board `every_event` answer
...
The Board's [Forge AI-199](/Forge AI/issues/Forge AI-199) answer of `every_event` for `Q-sync-direction` removes
...
| `audit.py` | Pure factory for the three burst audit rows; routes through the existing Forge AI-36 forwarder (no second pipeline) |
...
All three are routed through the existing Forge AI-36 forwarder via the
...
## Acceptance criteria (Forge AI-267) — evidence
...
- **Per-tenant config storage** — `PlatformConfig` defaults work for the smoke; production reads from the tenant-config service (Forge AI-200) once that lands.
