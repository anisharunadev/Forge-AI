...
**Stage:** Architecture prep (risk-register child of [Forge AI-62](/Forge AI/issues/Forge AI-62))
**Issue:** [Forge AI-204](/Forge AI/issues/Forge AI-204) — Sync-plane audit + risk register (extends S7 from Epic 1)
**Producer:** CTO (`f4d4bf77-2a6b-41e0-b3c5-4a688e2913f0`)
**Generated at:** 2026-06-17
**Companion artefacts:** [`forge/1.3/risk_register.md`](../1.3/risk_register.md) (template) · [`forge/1.4/jira_sync_report.md`](../1.4/jira_sync_report.md) §6 (scope-expansion source) · [`workspace/memory/security.md`](../../workspace/memory/security.md) §7 (audit-log shape, one-way-door) · [`agents/audit/schema.py`](../../agents/audit/schema.py) (Forge AI-36 foundation)
**Revision:** v0.1 — **provisional**, gated on [Forge AI-199](/Forge AI/issues/Forge AI-199) (ADR — sync topology decision). Topology-dependent risks are marked **PROVISIONAL**; the document is rev'd to v1.0 after Forge AI-199 lands.
...
### S0 — Architecture ADR (Forge AI-199 — the upstream gate)
...
| R0.4 | ADR lands and is treated as final without a deprecation/reversal path — one-way-door on the topology | L | H | P0 | ADR requires an explicit "exit cost" section (what does it take to swap the event bus for CDC, etc.); Forge AI-199 is gated on Architectural Review board approval per `agents/audit/README.md` pattern | Architect + CTO | ADR ships without an exit-cost section |

> **All four S0 risks are P0.** The ADR is the single point of failure for the rest of the sync plane — every downstream story inherits whatever the ADR locks in. Until Forge AI-199 lands, every risk below marked **PROVISIONAL** is a placeholder; the risk register is revved to v1.0 after Forge AI-199.
...
| R1.3 | Retry storm on a degraded downstream platform (e.g., Jira 5xx) saturates the queue and stalls other tenants' mirrors | M | M | P2 | Per-tenant retry buckets (token-bucket pattern from Forge AI-126, `agents/customer_cloud_broker/` lands post-ship); circuit breaker per (tenant, platform); shared queue has a hard ceiling | DevOps + Dev | One tenant exceeds 20 % of the queue for > 5 min |
...
> Topology of S1 (event bus vs. CDC vs. polling) is **PROVISIONAL pending Forge AI-199** — R1.1, R1.3 mitigation specifics depend on the ADR's topology pick.
...
| ID | Risk | L | I | Sev | Mitigation | Owner | Trigger to escalate |
|----|------|---|---|-----|------------|-------|---------------------|
| **R3.1** | **GitHub App installation token stored in `process.env` of a process the agent runtime does not own** — direct violation of `workspace/memory/security.md` §3. A single memory dump or error log leaks the token. | **L** | **C** | **P0** | **GitHub App private key lives in AWS Secrets Manager; the runtime fetches a short-lived (≤ 15 min) installation token via the existing `mcp-servers/secrets/` MCP (Forge AI-128); the token is injected as a sidecar env var, never persisted. Pinned unit test asserts no `process.env.GITHUB_*` reference.** | **Dev + Security (Epic 5)** | **Any `process.env.GITHUB_*` reference in `git grep` — page on-call** |
...
### S8 — Re-provisioning workflow + tenant-facing UI (FORA-589)

> Added 2026-06-20 by Security Engineer (`231cc5ae`) as a Phase-2 follow-on to S6 / R-SYNC-02. Companion artefact: [`threat-model-FORA-589.md`](./threat-model-FORA-589.md). Implementation paused behind the CTO threat-model walkthrough + FORA-256 ship + Designer hire.

| ID | Risk | L | I | Sev | Mitigation | Owner | Trigger to escalate |
|----|------|---|---|-----|------------|-------|---------------------|
| **R-SYNC-02a** | **Consent token replay** — attacker replays a captured `consent_token` against the broker (FORA-126) to skip re-consent on a removed-from-allow-list scope. Stale `consent_token` accepted → broker mints a new token on the previously-granted scope set. | **M** | **H** | **P0** | **Broker treats `consent_token` as single-use + freshness-bounded (`≤ 10 min` between OAuth flow start and broker `assume()`). Replay → 401 + audit `auth.login.failed reason:consent_token_replay`. Single-use ledger in the broker vault (per `apps/customer-cloud-broker` vault). Property test (`memory-dump-scan` pattern from FORA-126) verifies no `consent_token` byte shape in process memory after `assume()`.** | **Security + SeniorEngineer** | **Any `consent_token` accepted twice — page on-call** |
| **R-SYNC-02b** | **Tenant denies granting re-consent** (Repudiation, R-SYNC-02 row 1 carried) — tenant admin claims they did not grant re-consent on a scope the audit trail shows they granted. SOC 2 "who did what" answer fails. | L | M | P2 | Every `re_consenting → active` transition records (a) OAuth consent timestamp from the platform, (b) scope snapshot at consent time, (c) `consent_token_id` resolving to the broker's signed receipt. Tenant-facing UI displays the grant history at the consent-grant moment; consent record signed by tenant admin at install time. | Security + Dev (audit wedge) | Any consent grant missing the 3-field record |
| **R-SYNC-02c** | **Security Engineer denies scheduling re-consent** (Repudiation, R-SYNC-02 row 2 carried) — drift detector's transition decision is challenged; no audit chain. | L | M | P2 | Drift detector's transition decision recorded in `sync_scope_drift_audit` with the input digest, policy version (git hash of the allow-list YAML), and decision rule. Append-only audit wedge (FORA-36). | Security + Dev | Any state transition without the 3-field audit record |
| **R-SYNC-02d** | **Email digest leaks the granted scope list** to a forwarding tenant-internal address — scope names are themselves sensitive (which customer has `admin:org`, etc.) | M | M | P2 | Email digest body carries scope *names* only — never `client_secret`, never broker-vault ARN. Subject is opaque (`Forge re-consent needed`); body links to in-app banner. DKIM + DMARC `reject` on the sending domain. Bounce handling auto-disables the email channel for that tenant (does **not** opt them out of re-consent). | Security + DevOps | Any email leak of OAuth `client_secret` / broker-vault ARN — page on-call |
| **R-SYNC-02e** | **Broker unreachable during de-provision** — drift detector triggers `de_provisioning` while the broker cannot reach the platform (AWS regional outage, GCP IAM propagation lag); partial state (audit says de-provisioned, platform still has the token). Tenant-side token still valid → silent credential sprawl resumes. | **M** | **H** | **P0** | **`de_provisioning → de_provisioned` is broker-confirmed: state machine waits for broker `de_provision_ok`. On broker timeout: `audit.reprovision_stalled` + exponential backoff (max 5 attempts / 24 h). After 24 h: PagerDuty alert (FORA-258 AC #2 wiring) + manual runbook (`agents/runbooks/reprovision_stalled.md`). Daily sample (FORA-210) verifies no `(tenant, sa)` sits in `de_provisioning > 24 h`.** | **DevOps + Security** | **Any `(tenant, sa)` in `de_provisioning > 24 h` — page on-call** |
| **R-SYNC-02f** | **Per-tenant opt-out abused as silent suppression** — tenant admin sets `reprovision_opt_out = true` to suppress drift detection entirely; silent credential sprawl resumes. The opt-out is a defensive control that becomes an offensive channel. | **M** | **C** | **P0** | **Opt-out suppresses the re-consent UI, not the drift detector. Drift events still emit; PagerDuty still pages; audit row records `opted_out: true`. Tenant admin must explicitly ack each drift in the Forge console (read-only acknowledgement). Annual tenant-facing reminder; per-tenant opt-in revocation by Security Engineer on `> 90d` of unreviewed opt-out drift (runbook).** | **Security + Dev (console) + Tenant admin** | **Any tenant with `> 90d` of un-acked opt-out drift — escalate to tenant admin + Security review** |

> **S8 adds 3 new P0 controls to the register:** R-SYNC-02a (consent token replay), R-SYNC-02e (broker-unreachable de-provision), R-SYNC-02f (opt-out abuse). These bring the register's P0 count from 19 to **22** (rev to v0.2 — owned by KnowledgeSteward for ADR-style promotion).
...
> **P0 count is 22 (provisional; was 19, +3 added 2026-06-20 in S8 / FORA-589).** This is roughly 2× the P0 concentration of `forge/1.3/risk_register.md` (9 P0s), which is consistent with the new surface area — a sync plane introduces new actors, new credentials, new failure modes, and a new SOC 2 audit surface all at once. Once Forge AI-199 lands and the topology-dependent mitigations solidify, expect 1–3 PROVISIONAL P0s to either resolve (ADR mitigates) or stay P0 (and we add controls). Either way, this register is revved to v1.0.

**Composite risk profile:** **high**, dominated by **P0 controls on cross-platform actor identity, per-platform credential storage, and audit completeness**. The control framework that exists (`workspace/memory/security.md` §3 secrets, §4 tenancy, §7 audit; `agents/audit/` foundation from Forge AI-36) covers most of the per-control mitigations, but several controls are not yet *wired* into the sync-plane event-emit path. The verification path for most P0s is Security (Epic 5) — the security-engineer hire owns the daily sample, the cross-tenant lint, the per-tenant inventory check, and the mirror-divergence sample.
...
| P0 | **R-SYNC-02a** (consent token replay) | SeniorEngineer | Security (Epic 5) |
| P0 | **R-SYNC-02e** (broker-unreachable de-provision) | DevOps | Security (Epic 5) |
| P0 | **R-SYNC-02f** (opt-out abuse) | Security (Epic 5) | Security (Epic 5) + Tenant admin |

The P0 controls are again predominantly **Dev-built + Security-verified** — the same shape as Epic 1. The PM/BA role is to ensure the Story-level AC explicitly references the control (Forge AI-204's AC does; see R0.x → S0 ADR sections, R2.1 → S2 AC, R6.1 → S6 AC, R-X2 → S7 sample).
...
|----|----------|-------|-----|
| R-Q0 | The new "Forge Integration Layer / Cross-Platform Sync Plane" Epic is not yet created on Paperclip. Should it be a child of Forge AI-62 (the Jira sync sub-goal) or of Forge AI-15 (the parent Forge Epic)? My recommendation: child of Forge AI-15, with Forge AI-62 as the originating audit link. | CEO + CTO | Before Forge AI-199 ships |
| **R-Q1** | **What does the ADR (Forge AI-199) decide on each of R0.1–R0.4?** This entire register's v1.0 is the ADR's output. Until Forge AI-199 lands, the P0 controls are placeholders. | **Architect + CTO** | **Forge AI-199 acceptance** |
...
- [`agents/audit/README.md`](../../agents/audit/README.md) — Forge AI-36 foundation (append-only store, hash chain, tenant-scoped read API, admin override)
- [`agents/audit/schema.py`](../../agents/audit/schema.py) — `AuditEvent` contract; `AUDIT_SCHEMA_VERSION = "0.1.0"`
- [Forge AI-126 — Customer-cloud broker v1](/Forge AI/issues/Forge AI-126) — reference for per-tenant token bucket + circuit-breaker pattern (R1.3, R-X3). The broker module lands as `agents/customer_cloud_broker/` post-Forge AI-126 ship; until then the pattern is documented in the Forge AI-126 ship note.
- [`mcp-servers/secrets/`](../../mcp-servers/secrets/) — Forge AI-128 reference for per-platform credential storage (R3.1, R-X1)
- [Forge AI-199 — ADR — sync topology decision](/Forge AI/issues/Forge AI-199) — upstream gate (R0.1–R0.4)
- [Forge AI-62 — Sub-goal 1.4 — Jira sync report](/Forge AI/issues/Forge AI-62) — parent
- [VOY-3066 — S7 Audit Log Capture](https://neptunetriton.atlassian.net/browse/VOY-3066) — the wedge this register extends
- [Forge AI-168 — Approvals pg adapter](/Forge AI/issues/Forge AI-168) — reference for the integration-child unblock pattern that closed a similar recovery loop
...
|-----|------|--------|--------------|
| v0.1 | 2026-06-17 | CTO (`f4d4bf77-2a6b-41e0-b3c5-4a688e2913f0`) | Initial risk register — 20 P0s, 4 P1s, 9 P2s, 2 P3s. Provisional, gated on Forge AI-199. Documents the sync-plane audit entry shape (extends `security.md` §7.1 via `metadata.sync.*` keys — no top-level field additions until `AUDIT_SCHEMA_VERSION` is bumped per R6.1). Specifies the daily-sample design (R-X2) and the per-tenant namespace rule (R6.3 + R8.1 extension). Daily-sample production run remains blocked on sync-plane implementation (S1–S5). |
| **v0.2 (proposed)** | 2026-06-20 | Security Engineer (`231cc5ae-3235-482c-a791-d8ff3e217c8e`) | **Added S8 — Re-provisioning workflow (FORA-589). 3 new P0 controls (R-SYNC-02a consent token replay, R-SYNC-02e broker-unreachable de-provision, R-SYNC-02f opt-out abuse) and 3 P2 (R-SYNC-02b/c/d). P0 count 19 → 22. Companion addendum: [`threat-model-FORA-589.md`](./threat-model-FORA-589.md). Pending CTO walkthrough + FORA-256 ship + Designer hire before FORA-589 ships.** |
