# Jira Discovery — Sub-goal 1.4 (Forge AI-62)
...
| Forge AI-61 board approval | `accepted` at 2026-06-17T16:28:04Z by `local-board` (interaction `ecee6f96-…`) | `GET /api/issues/Forge AI-61/interactions` |
| Approved Epic package | 8 stories, S1–S8; revision `1dd556f7-b3b4-4d19-92a8-3d2d0bda3327` | `GET /api/issues/Forge AI-61/documents` (key=`epic_package`) |
...
| Q1 (trigger scope) resolution | "Self-bootstrapping dogfood — Forge AI is the product" | Forge AI-17 interaction `f79b7063-…` |
| Target Jira project | **UNRESOLVED** — see §2 | this heartbeat |
| Target sprint | **UNRESOLVED** — depends on project pick | this heartbeat |

## 2. The blocking ambiguity — no Forge AI project in the customer's Jira

The customer's Jira instance (`neptunetriton.atlassian.net`) is provisioned with **7 projects**, none of which is `Forge AI`:

| Project | Type | Has Epic + Story? | Last activity | Assessment for Forge AI-platform delivery |
|---|---|---|---|---|
...
| `VOY` (Voyager) | software | yes | **most active** (VOY-1011, -1148, -1178, -1765, -1999, -2043, -2121, -2139, -2295) | most defensible *existing* target; mixes Forge AI-platform work with Voyager-app work |
| `WR` (WCAG Readiness) | business | **no** Epic/Story | unknown | not usable for this Epic shape |
| **`Forge AI` (new)** | — | — | — | **matches convention** but requires admin permission we don't have |

### Why I won't silently pick a project

- The customer convention in `workspace/customer/conventions.md` §3 says Jira projects use `<CUSTOMER>-<NNN>`. With Q1 resolved as "Forge AI is the product" (dogfood), the customer is Forge AI — so the project should be `Forge AI`, not a Neptune app project.
...
- "Use VOY because it's the most active" mixes Forge AI-platform delivery (8 stories on the Forge Ideation Agent) with Neptune's customer-facing Voyager app backlog. That contaminates VOY-2139's existing epic list, and breaks the audit trail of which work is internal platform vs. customer app.
...
|---|---|---|---|---|
| **A** | **Create a new `Forge AI` project** in `neptunetriton.atlassian.net` (admin action outside the agent) and have me push the 8 stories there | Matches `<CUSTOMER>-<NNN>` convention; clean separation from customer-app backlogs; reusable for every internal Forge AI-platform epic going forward | Adds 1 board day (admin action); needs a Jira admin with `manage:jira-project` | A Jira admin (currently nobody inside the agent team) creates the project with Epic + Story + Sub-task issuetypes and an active sprint; agent pushes the 8 stories |
| **B** | **Use `VOY` (Voyager)** as a temporary holding pen | No new admin step; project already has Epic/Story/Sub-task; most active project so most likely to be noticed | Mixes Forge AI-platform work with Voyager-app work; contaminates VOY-2139's epic namespace; sets a bad precedent | CTO confirms; agent pushes stories under the Epic with `Epic Link = Forge Ideation Agent` and labels `forge-platform`, `dogfood`, `epic-1` so the work is grep-able |
| **C** | **Park Forge AI-62 as `blocked`**, escalate via parent `Forge AI-17` for a board call | Most explicit; no risk of putting work in the wrong place | Delays the downstream stages (Architecture handoff, Forge AI-27); Epic 1 is "done" per the Forge AI-62 acceptance criteria | Board call on the parent epic; agent re-picks the issue on resolution |
...
2. Create the Epic in that project (issue type `Epic`) with the Forge AI-59 → Forge AI-60 → Forge AI-61 lineage as the description and the Paperclip links per the comment-style rule.
...
8. Generate `jira_sync_report.md` (this file's successor) and attach it to Forge AI-62 + comment-link on Forge AI-17.

## 5. Audit

- Run: `3ae3d0d1-630e-4d38-a4ff-0f631b4b812b`
- Source issue: [Forge AI-62](/Forge AI/issues/Forge AI-62) — Sub-goal 1.4 — Jira sync
- Upstream approved Epic: [Forge AI-61](/Forge AI/issues/Forge AI-61) — Sub-goal 1.3 — Epic generator
- Parent Epic: [Forge AI-17](/Forge AI/issues/Forge AI-17) — Epic 1 — Forge Ideation Agent (currently `blocked`, assigned to CEO)
