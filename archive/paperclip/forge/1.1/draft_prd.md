...
**Stage:** Ideation (sub-goal 1.1 of [Forge AI-17](/Forge AI/issues/Forge AI-17))
**Owner:** `ba-agent` (Hire #3, BMAD PM + BA, reports to CTO)
**Source one-line prompt:** "Turn a one-line product idea into a board-approved PRD + Jira Epic." (parent Epic [Forge AI-17](/Forge AI/issues/Forge AI-17) description)
**Related:**

- Platform PRD template — [workspace/project/PRD.md](../../../workspace/project/PRD.md)
- Parent Epic — [Forge AI-17](/Forge AI/issues/Forge AI-17) "Forge Ideation Agent"
- Sibling sub-goals — [Forge AI-60](/Forge AI/issues/Forge AI-60) (1.2), [Forge AI-61](/Forge AI/issues/Forge AI-61) (1.3), [Forge AI-62](/Forge AI/issues/Forge AI-62) (1.4)
- Intermediate artifact — [requirement_brief.json](./requirement_brief.json)
- CEO scope delegation — [comment `3426cc84`](/Forge AI/issues/Forge AI-17#comment-3426cc84-2599-4f7c-adc3-78a85998e03f) on [Forge AI-17](/Forge AI/issues/Forge AI-17)
- Handoff to ba-agent — [comment `74bade6e`](/Forge AI/issues/Forge AI-59#comment-74bade6e-8e21-4a9b-8ea2-e3e577e1da45) on [Forge AI-59](/Forge AI/issues/Forge AI-59)
...
The Forge Ideation Agent's **Requirement Ingestion** sub-capability is the front door of Forge AI. A user supplies a one-line product idea; the agent returns a structured `requirement_brief.json` and a Draft PRD aligned to the Forge AI PRD template, grounded in the customer's Knowledge Layer and the MCP-fed context the customer's tenant allows. The Draft PRD is honest about what it knows, what it inferred, and what it is still missing.

This sub-capability is the first trust-building interaction the customer has with Forge AI. If the Draft PRD is vague, the customer assumes the rest of the platform is vague. If the Draft PRD is precise, structured, and clearly scoped, the customer trusts the downstream stages (Architect, Dev, QA, Security, DevOps, Docs) to do their job.
...
- **Today, without Forge AI** — the idea lives in a Slack message, a Notion page, a half-finished Google Doc, or a Jira ticket that was created before anyone knew what the ticket was for.
...
## 5. Product surface (what changes in Forge AI as a result of 1.1)
...
- `draft_prd.md` — the Draft PRD, aligned to the Forge AI PRD template. Attached to the issue. Reviewed by the board, then by the PM, then by the Architect.
...
3. Sees every Knowledge Layer file read (with `content_hash`), every Jira MCP call, every prompt/response pair (when `Forge AI_LOG_LLM=1`).
...
- **Generating the Jira Epic or stories.** That is sub-goal 1.3 ([Forge AI-61](/Forge AI/issues/Forge AI-61)). The output of 1.1 is an `epic_stub` placeholder, not a real Epic.
- **Pushing anything to Jira.** That is sub-goal 1.4 ([Forge AI-62](/Forge AI/issues/Forge AI-62)). Requirement Ingestion reads from Jira for context; it does not write.
...
| **Audit-log completeness** | 100 % (per the platform PRD §8) | 100 % | < 100 % is a P0 | Every Knowledge Layer file read, every MCP call, every prompt/response pair (when `Forge AI_LOG_LLM=1`) is captured. Daily sample audit. |
...
| The "first design partner" scenario is too narrow to validate the agent | Medium | Medium | Q3 2026 milestone #7: "First design partner runs a real feature end-to-end." The schema is designed for that scenario, not for Forge AI-internal dogfooding. |

## 10. Open questions (must answer before sub-goal 1.1 can be `done`)

These are surfaced in the `ask_user_questions` interaction on [Forge AI-59](/Forge AI/issues/Forge AI-59). The Master Orchestrator will not advance to sub-goal 1.2 ([Forge AI-60](/Forge AI/issues/Forge AI-60)) until the **blocking** ones resolve.

1. **Q1 (blocking) — Trigger semantics for the first iteration.** Is the trigger a real customer one-line idea, or a self-bootstrapping dogfood where Forge AI is the product being specced? The Draft PRD scope flips on the answer. **Owner:** CEO. **Blocks:** [Forge AI-60](/Forge AI/issues/Forge AI-60) (1.2) and all of 1.3, 1.4. **Due by:** 2026-06-20.
2. **Q2 (blocking) — MCP read-set for sub-goal 1.1.** PRD says Jira/Zendesk/Slack/Confluence; tech-stack priority-1 is Jira only for Q3 2026. Does Requirement Ingestion proceed with Jira-only and stub the others, or does it wait for the priority-1 set to be wired? **Owner:** CTO. **Blocks:** [Forge AI-60](/Forge AI/issues/Forge AI-60) (1.2) — the market + tech-debt analysis depends on whether Zendesk support themes are part of the input set. **Due by:** 2026-06-18.
...
- The next sub-goal — [Forge AI-60](/Forge AI/issues/Forge AI-60) "Sub-goal 1.2 — Market & tech-debt analysis" (currently `blocked` on this issue)
- The sibling sub-goals — [Forge AI-61](/Forge AI/issues/Forge AI-61) (1.3 Epic + story generator), [Forge AI-62](/Forge AI/issues/Forge AI-62) (1.4 Jira sync)
