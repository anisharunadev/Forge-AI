 ...
 */
export function checkSql(filePath, sql) {
    if (!sql.trim())
        return [];
        ...
        for (let i = 0; i < lines.length; i += 1) {
            ...
            if (createTableRe.test(line)) {
                findings.push({
                    ...
                    file: filePath,
                    line: i + 1,
                    message: 'CREATE TABLE is only allowed in `migrations/`. Use the FORA migration runner (FORA-162 0.7.2a) to add or alter tables.',
                });
    ...
    while ((m = createTableRe2.exec(cleaned)) !== null) {
        ...
        if (!/\btenant_id\b/i.test(block))
            continue; // not a multi-tenant table
        ...
        if (!/\bENABLE\s+ROW\s+LEVEL\s+SECURITY\b/i.test(tail)) {
            findings.push({
                ...
                file: filePath,
                line: blockStartLine,
                message: 'Multi-tenant table (has `tenant_id`) is missing `ENABLE ROW LEVEL SECURITY` after the CREATE TABLE block.',
            });
        }
        if (!/\bCREATE\s+POLICY\b[\s\S]*?\btenant_isolation\b/i.test(tail)) {
            findings.push({
                ...
                file: filePath,
                line: blockStartLine,
                message: 'Multi-tenant table is missing a `tenant_isolation` policy. Use `USING (tenant_id = current_setting(\'app.tenant_id\')::uuid)`.',
            });
        }
    }
        ...
        for (let i = 0; i < lines.length; i += 1) {
            ...
            if (bypassRe.test(line)) {
                findings.push({
                    ...
                    file: filePath,
                    line: i + 1,
                    message: '`BYPASSRLS` is only allowed in `migrations/` (migration runner) and `audit/` (audit-log writer). Application roles MUST be `NOBYPASSRLS`.',
                });
