 ...
 */
function stripTsComments(line: string): string {
  // Single-line comment.
  ...
  if (slc !== -1) return line.slice(0, slc);
  return line;
}

export function checkTs(filePath: string, source: string): Finding[] {
  if (!source.trim()) return [];
  if (isUnderMigrations(filePath) || isUnderAudit(filePath)) return [];
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    if (BYPASS_TOKEN_RE.test(line)) {
      findings.push({
        ...
        line: i + 1,
        message:
          '`BYPASSRLS` is only allowed in `migrations/` (migration runner) and `audit/` (audit-log writer). Application code MUST NOT mint a `BYPASSRLS` role.',
      });
    }
  }
  return findings;
