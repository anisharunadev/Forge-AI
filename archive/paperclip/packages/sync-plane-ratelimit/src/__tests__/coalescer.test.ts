...
import { Coalescer, type OutboundEdit, type CompositeEdit } from '../coalescer.js';
...
describe('Coalescer', () => {
  it('merges N edits within the window into a single composite', async () => {
    vi.useFakeTimers();
    let now = 1_000_000;
    const flush = vi.fn(async (c: CompositeEdit) => ({ ok: true, status: 200, per_event: c.source_event_ids.map((e) => ({ event_id: e, ok: true })) }));
    const c = new Coalescer({ window_ms: 30_000, now: () => now, flush });
    for (let i = 0; i < 5; i++) {
      const r = c.enqueue(edit(i));
      if (i === 0) expect(r.coalesced).toBe(false);
      else expect(r.coalesced).toBe(true);
    }
    ...
    const merged = flush.mock.calls[0]![0] as CompositeEdit;
  ...
  it('debounces — last edit within W resets the deadline', async () => {
    ...
    now = 20; c.enqueue(edit(1, { enqueued_at_ms: 20 }));
    await new Promise((r) => setTimeout(r, 20));
    now = 40; c.enqueue(edit(2, { enqueued_at_ms: 40 }));
