...
Implements **[Forge AI-48](/Forge AI/issues/Forge AI-48) §3.3** (Epic 0.3.3 router sub-goal) per **ADR-0013**.
...
// Router (Forge AI-460) calls this around every MCP-server invocation:
...
- **Typed error**: `CircuitOpenError` with `kind: 'circuit_open'`, `tenant_id`, `server_name`, `state`, `retry_after_ms`. Maps to `mcp_unavailable` at the router seam in Forge AI-48 §3.4.
...
export function toEventBusSink(producer: NatsEventProducer): BreakerEventSink {
  return {
    async emit(event: BreakerEvent) {
      ...
      // events live on the audit-out-of-band channel (Forge AI-36 forwarder).
...
This is the same `AuditSink` pattern used by `@fora/sync-plane-ratelimit` (Forge AI-256, ADR-0010 §8.1). The breaker never opens a NATS socket itself; the router (which already owns the connection) injects the sink.

## Performance

The state machine is synchronous and pure; the only IO is one cache-broker `get` + one `set` per call, both O(1) hash lookups. AC #2 from Forge AI-48: **5 errors trip the breaker, and the next invoke returns `circuit_open` in ≤50ms** (the trip path itself is sub-millisecond; the budget covers cache RTT + warm-up).
...
- **The MCP router** (per-tenant scope, schema registry, retry/idempotency). Lives in `packages/mcp-router/` (Forge AI-460, parallel to this one).
- **The `mcp_unavailable` rename**. The router seam in Forge AI-48 §3.4 wraps `CircuitOpenError` into the canonical MCP error.
