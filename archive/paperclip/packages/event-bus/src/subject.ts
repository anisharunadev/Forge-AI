...
export function parseSubject(
  subject: string,
): { tenantId: string; eventType: string; major: number } | null {
  const match = /^fora\.events\.([a-zA-Z0-9_-]{1,64})\.([a-z][a-z0-9_]{0,63})\.v([0-9]+)$/.exec(
    subject,
  );
 ...
 */
export function assertSubjectTenant(
  subject: string,
...
export function tenantSubjectPrefix(tenantId: string): string {
  if (!isValidTenantId(tenantId)) {
    throw new InvalidInputError(`invalid tenant_id "${tenantId}"`);
  }
