...
export const EVENT_SCHEMAS: {
  readonly [K in EventType]: { readonly major: number; readonly payload: z.ZodTypeAny };
} = {
...
export function buildEnvelope<T extends EventType>(params: {
  eventType: T;
  runId: string;
  tenantId: string;
  stage: Stage | null;
  ...
  const parsed = entry.payload.parse(params.payload) as EventPayload<T>;
  return {
    v: CURRENT_VERSION,
...
export function parseEnvelope(raw: unknown): z.infer<typeof EventEnvelopeSchema> {
