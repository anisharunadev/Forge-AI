...
[Forge AI-124 acceptance bar #2](https://Forge AI/Forge AI/issues/Forge AI-124), every
...
The wrapper is gated by `Forge AI_TENANT_POOL`. The default is `disabled`
(pass-through). To turn the gate on:

```bash
export Forge AI_TENANT_POOL=enforced
```
...
- Ship the wrapper. Existing DB code that bypasses the wrapper still
  works (pass-through).
- Set `Forge AI_TENANT_POOL=enforced` in staging. The mismatch test
...
const pg = new Pool({ connectionString: process.env.Forge AI_DATABASE_URL });
const pool = new TenantAwarePool({
  underlying_pool: pg,
  audit: productionAuditSink, // wire to Forge AI-36 in prod
  enforcement: parseEnforcement(process.env.Forge AI_TENANT_POOL),
});
...
pnpm --filter @fora/db-pool test:integration  # needs Forge AI_DATABASE_URL
```
