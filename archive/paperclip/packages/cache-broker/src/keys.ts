...
export function deriveKey(parts: Pick<KeyParts, 'resource' | 'id'>): CacheKey {
  if (!parts.resource) throw new Error('deriveKey: resource is required');
  if (!parts.id) throw new Error('deriveKey: id is required');
  const material = `${parts.resource}:${parts.id}`;
  const hex = createHash('sha256').update(material, 'utf-8').digest('hex');
  return hex as CacheKey;
}
 ...
 * resources are.
 */
export function deriveTag(parts: Pick<KeyParts, 'tenant_id' | 'resource'>): string {
  return `tenants:${parts.tenant_id}:${parts.resource}`;
}

/** Test-only: the canonical input string for a key, for diagnostic dumps. */
export function keyMaterial(parts: Pick<KeyParts, 'resource' | 'id'>): string {
  return `${parts.resource}:${parts.id}`;
}
