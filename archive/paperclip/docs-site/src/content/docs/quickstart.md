...
The audit log is the JSONL file at `$Forge AI_AUDIT_LOG` (default `.fora/audit/tenancy-denied.jsonl`) and the broker audit at `$Forge AI_CCB_AUDIT_LOG_PATH` (default `.fora/audit/customer-cloud-broker.jsonl`). Read with `jq`:
