...
8. `pnpm --filter @fora/forge dev` (port 3000) — the Forge AI console ([Forge AI-374](/Forge AI/issues/Forge AI-374))
...
- **Tenant:** `acme-corp` (`Forge AI_SEED_TENANT_ID`) — policy `public-preview`
- **S3 bucket:** `fora-dev-bucket` (`OBJECT_STORE_BUCKET`) — created by `scripts/localstack-init.sh` on LocalStack boot
- **Secrets Manager:** `fora/probe-signing-key` — placeholder for Forge AI-126.4 canary probe

The Forge console does **not** seed a demo run; the persona dashboards fall back to the seed run id `demo-run-001` (override with `Forge AI_SEED_RUN_ID`). If that run is not present yet, the dashboards render the empty state. Create one with:
...
## Forge console (apps/forge, Forge AI-374)

The Forge console is a Next.js 15 (App Router) UI shell over the orchestrator REST API. Default port is **3000** (`Forge AI_FORGE_PORT`); the orchestrator URL defaults to `http://localhost:4000` (`Forge AI_FORGE_API_URL` and `NEXT_PUBLIC_FORGE_API_URL`). The single-tenant auth stub uses a `forge.persona` cookie — pick a persona via the avatar menu in the top-right (PM / Engineering Lead / CTO).
