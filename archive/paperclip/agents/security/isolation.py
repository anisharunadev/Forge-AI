"""
Isolation guarantees for the Security Agent (FORA-74).

The Security Agent is the AppSec gate. It MUST NOT share context
with the Coding Agent — neither the developer's prompt, nor the
scratch space, nor the conversation log. The agent runs in a
separate process with a separate JWT, and the runtime enforces
a tool allow-list.
...
in the FORA-74 deliverable and nothing more.
...
# The four tools the Security Agent is allowed to call. The order
# is the order they appear in the FORA-74 deliverable; the
...
# reviewer can see at a glance what the Security Agent must never
...
class IsolationError(RuntimeError):
    """Raised when the Security Agent is asked to call a tool that
...
class ToolAllowList:
    """Runtime check + audit seam for the Security Agent's allow-list.
    ...
    def check(self, tool: str) -> None:
        ...
        if tool not in self._allowed:
            ...
            raise IsolationError(
                f"Security Agent is not allowed to call {tool!r}; "
...
# agent process. The Security Agent also asserts FORA_AGENT_ROLE=appsec
...
class AgentRoleError(IsolationError):
    """Raised when the process env doesn't match the Security Agent's
    expected identity (FORA_AGENT_ROLE != 'appsec')."""


def assert_process_identity() -> None:
    """Assert the process is wired up as the Security Agent.

    Called from `SecretScanner.__init__`. Refuses to start if any of
    ...
    if missing:
        raise AgentRoleError(
            f"Security Agent process missing identity env vars: {missing!r}"
        )
    role = os.environ.get("FORA_AGENT_ROLE", "")
    if role != "appsec":
        raise AgentRoleError(
            f"FORA_AGENT_ROLE={role!r}; Security Agent requires 'appsec'"
        )
