"""
I/O seams for the Security Agent (FORA-74).
...
The scanner writes three artefacts during a scan:
  ...
  2. Handoff artefact — the v1.0.0 envelope, persisted so the
     orchestrator can hand it to DevOps without re-running.
...
on `SecretScanner.__init__` or `ScannerInputs`.
...
class HandoffArtifactWriter(Protocol):
    ...
    def write_artifact(
        self, handoff_output: HandoffOutput,
    ) -> str:
    ...
    def write_evidence(
        ...
        *,
        ...
        self.rows.append({
            ...
            "scanners": [r.scanner for r in scan_results],
...
class InMemoryHandoffArtifactWriter:
    ...
    def write_artifact(self, handoff_output: HandoffOutput) -> str:
        key = f"memory://security/{handoff_output.handoff_id}.json"
...
def default_writers() -> Dict[str, Any]:
    ...
    FORA-74's S3 + GitHub MCP adapters land.
    """
    return {
