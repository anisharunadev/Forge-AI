...
| Feature flag | `feature_flag.py` | `Forge AI_AUDIT_ENABLED`; default on |
...
[Forge AI-75 (0.6 Cost tracking)](../../../../issues/Forge AI-75) — 0.6
reads from this store rather than maintaining a parallel ledger.

## Feature flag

`Forge AI_AUDIT_ENABLED=1` (default) — events are appended.
`Forge AI_AUDIT_ENABLED=0` — events are dropped (no-op store).  The
...
- Board UI views — consumed by the Forge console team
  (Forge AI-110 / Phase 1 of the roadmap).
