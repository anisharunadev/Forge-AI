# agents/sync_plane_service — Forge AI-252 / Epic 11.1
...
> (Forge AI-254). Keeping 11.1 in a parallel package preserves the
...
audit events, [Forge AI-117 storage-contract pattern](../docs/adr/0002-knowledge-layer-storage-contract.md),
...
| Day-one `AuditForwarder` (11.8) | `audit_forwarder.InMemoryAuditForwarder` | Wires the 8 `sync.*` events to Forge AI-36 |
| Production Postgres schema | `migrations/0005_sync_plane.sql` | 4 `sync.*` tables, RLS per tenant, `(tenant_id, …)` PRIMARY KEY |
| Smoke test (Forge AI-117 pattern) | `smoke_test.py` | All 7 ACs; evidence to `evidence/smoke_<utc>.json` |
...
AC6 — sync.event.received and sync.event.applied wire to
      Forge AI-36; the audit row carries stage=sync_plane,
...
sub-task 11.6 (Forge AI-???) replaces it with a Redis-backed
...
| `ConflictResolver` | 11.4 (Forge AI-254) | No — sub-task 11.4 |
...
```
========================================================================
Sync Plane service skeleton — smoke test (Forge AI-252 / 11.1)
========================================================================
...
[AC6 sync.event.received/applied wire to Forge AI-36]
  OK  (0 ms)
...
(Forge AI-254). Recorded as a finding in the smoke test
evidence JSON.
