# agents/cost — Forge AI-75 (0.6 Cost tracking)

Per-run, per-stage, per-tenant cost ledger derived from the Audit
system 0.5 (Forge AI-36), with per-tenant monthly ceiling enforcement and
a `budget.remaining(runId)` hint the Agent runtime 0.2 (Forge AI-30) reads
...
| Tenant gate | `gate.py` | `TenantGate`; the orchestrator (Forge AI-110) calls `check(tenant_id)` before launching a run |
| Board read API | `board.py` | `BoardReader`; `monthly_burndown`, `top_spending`, `current_month`, `alert_log` |
| Runtime budget hint | `integration.py` | `RuntimeBudgetHint`; the `budget.remaining(runId)` seam for Forge AI-30 |
| Feature flag | `feature_flag.py` | `Forge AI_COST_ENABLED`; default on |
...
```
   ...
   runtime ---->   RuntimeBudgetHint.remaining(tenantId, runId)
                  ...
                  +--------------------+                |
                            |                           v
                            +--->  TenantGate.check(tenantId) --> orchestrator (Forge AI-110)
                            |                           |
...
inherited from Forge AI-36.  A bug in the ledger that misreports is
...
`Forge AI_COST_ENABLED=1` (default) -- alerts fire and the gate enforces.
`Forge AI_COST_ENABLED=0` -- the ledger still works (it is a read), but
