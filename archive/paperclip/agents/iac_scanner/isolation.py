"""
Isolation guarantees for the IaC Scanner (FORA-77).

The IaC scanner is the AppSec gate for Infrastructure-as-Code
misconfigurations. It MUST NOT share context with the Coding
Agent — neither the developer's prompt, nor the scratch space,
nor the conversation log. The agent runs in a separate process
with a separate JWT, and the runtime enforces a tool allow-list.
...
the smoke test asserts the allow-list is exactly the four tools
in the FORA-77 deliverable and nothing more.

The four tools (per FORA-77 deliverable):

  1. read_pr_diff            — pull the immutable handoff artefact
  2. write_pr_comment        — post the verdict as a sanitised PR comment
  3. write_scan_evidence     — write the audit-log row
  4. write_handoff_artifact  — persist the v1.0.0 envelope

The IaC scanner has FEWER tools than the dep scanner because the
scanner's input is the PR diff only (no lockfile, no SBOM, no
harness for a separate read tool) — the dev cost of an extra
"read" tool is also a cross-context-leak risk (FORA-77 design
§3 — smallest possible tool surface).

Public surface:

    ALLOWED_TOOLS              — tuple of the four tool names
...
# The four tools the IaC scanner is allowed to call. The order
# is the order they appear in the FORA-77 deliverable; the
# determinism matters for the audit replay test (AC #5).
...
# reviewer can see at a glance what the IaC scanner must never
...
FORBIDDEN_TOOLS: FrozenSet[str] = frozenset({
    ...
    "read_lockfile_body",   # not used by IaC scanner at all
    "read_iac_file_body",   # only the line pointer is allowed; see FileRef
    "write_source_file",
    "commit_changes",
    "merge_pr",
    "post_jira_comment",
...
class IsolationError(RuntimeError):
    """Raised when the IaC scanner is asked to call a tool that
...
class ToolAllowList:
    """Runtime check + audit seam for the IaC scanner's allow-list.
    ...
    def check(self, tool: str) -> None:
        ...
        if tool not in self._allowed:
            ...
            raise IsolationError(
                f"iac scanner is not allowed to call {tool!r}; "
...
# agent process. The IaC scanner also asserts FORA_AGENT_ROLE=appsec
...
class AgentRoleError(IsolationError):
    """Raised when the process env doesn't match the IaC scanner's
    expected identity (FORA_AGENT_ROLE != 'appsec')."""


def assert_process_identity() -> None:
    """Assert the process is wired up as the IaC scanner.

    Called from `IacScanner.__init__`. Refuses to start if any of
    ...
    if missing:
        raise AgentRoleError(
            f"iac scanner process missing identity env vars: {missing!r}"
        )
    role = os.environ.get("FORA_AGENT_ROLE", "")
    if role != "appsec":
        raise AgentRoleError(
            f"FORA_AGENT_ROLE={role!r}; iac scanner requires 'appsec'"
        )
