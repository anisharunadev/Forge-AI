...
The runtime seam that loads this contract, enforces the gate between stages, and invokes the stage handler is the `StageEngine` port (see [Forge AI-173](/Forge AI/issues/Forge AI-173) and the [glossary entry](../customer/glossary.md#stageengine)). The handoff contract is the schema; the `StageEngine` is the runtime. A change to either side without updating the other is a contract drift bug.
