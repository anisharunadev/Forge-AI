...
**Issue:** [Forge AI-40](/Forge AI/issues/Forge AI-40)
**Stage:** DevOps (Epic 6)
...
2. **No IAM keys in code or in the agent's runtime env.** The agent's only
   ...
   credentials are blocked by `iam-boundary` (`Forge AI-125`).
...
5. **No secrets in code.** Every secret reference is `aws_secretsmanager_secret`
   ...
   `secrets:` block in the agent prompt is forbidden (`Forge AI-128.d`).
...
| `aws` (and `azure` / `gcp` per `customer.cloud`) | `pricing:get-products`, `ec2:describe-vpcs`, `eks:describe-cluster`, `secretsmanager:list-secrets` | **read-only**; OIDC-brokered via `customer-cloud-broker` (`Forge AI-126`) |
...
| `audit` | `audit.append` | append-only sink (`Forge AI-36`) |
...
(`Forge AI-126`) and is not an artifact-generator concern except for the contract
...
(`Forge AI-36`, JSONL append-only). The event shape is fixed; the broker
redacts `secrets`, `tokens`, and `bearer` fields at append time
(`Forge AI-128.f`, `Forge AI-189`).
...
enforced by the cost-agent (`Forge AI-75`); a run that would push the tenant over
...
`plan` output, and a new child issue is opened (`Forge AI-44X`) to triage the
delta.
...
| 2. Reference-service PR (no direct commit) with CI: lint + `terraform plan` against dev | `forge/6.1/reference-service/...` PR, dispatched as child of Forge AI-40 | child issue `Forge AI-212` |
