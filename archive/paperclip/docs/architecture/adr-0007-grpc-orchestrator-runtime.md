...
| **Sub-task**      | [Forge AI-135](/Forge AI/issues/Forge AI-135) (0.1.2 — Stage transition engine)                           |
...
[ADR-0001](./adr-0001-master-orchestrator-sdlc-architecture.md) §2.3 and §7 make the Master Orchestrator the **only component that talks to the Agent Runtime, Memory, Cost, and Audit directly**. Sub-agents do not call each other; the Orchestrator is the seam. [Forge AI-50 spec §4.2](/Forge AI/issues/Forge AI-50#document-spec) names this seam as a gRPC contract with five RPCs: `CreateRun`, `AdvanceStage`, `GetRunContext`, `ReportCost`, `HealthCheck`.
...
- **Tooling.** gRPC has first-class generators for TypeScript and Python (the two languages the platform uses per [standards.md §1.1](/Forge AI/docs/engineering/standards.md)). Avro requires a separate schema-registry deployment; Thrift is fine but its generators are second-class for the languages we ship.
...
- **Industry.** Every MCP server in [mcp-servers/](/Forge AI/mcp-servers) is JSON-RPC; the Agent Runtime is Python. gRPC is the natural upgrade path when JSON over HTTP stops scaling.

## 4. Authn

- Every gRPC call carries the **Forge AI-issued JWT** from the identity broker (ADR-0003 §3.2).
...
2. Orchestrator receives. Validates JWT (aud=forge-runtime, tenant_id matches run.tenant_id).
   ...
   (Forge AI-50 spec §2.3). Validates artefact.sha256 is 64 lowercase hex.
...
Per [Forge AI-50 spec §2.3 acceptance](/Forge AI/issues/Forge AI-135) and architecture.md §8:
...
| Auth broker down                 | The Orchestrator's gRPC server returns `UNAUTHENTICATED`; runs cannot start. This is the one failure mode that halts (per Forge AI-50 spec §7). |
...
- [Forge AI-135](/Forge AI/issues/Forge AI-135) — Implement the state machine and `AdvanceStage` per this ADR
- [Forge AI-30](/Forge AI/issues/Forge AI-30) — Agent runtime uses this IDL as the seam
