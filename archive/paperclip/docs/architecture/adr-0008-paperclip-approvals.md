...
| **Sub-task**      | [Forge AI-137](/Forge AI/issues/Forge AI-137) (0.1.4 — Human-approval router)                             |
...
Per [Forge AI-50 spec §6.1](/Forge AI/issues/Forge AI-50#document-spec):
...
8. On wake:
     ...
     b. UPDATE agent_run_approvals SET status=decision, decided_at=now(),
        decided_by={actor}.
     c. If accept: advance the run per the gate rule (Forge AI-50 spec §2.3);
...
The `return` primitive (Forge AI-50 spec §2.3) reuses the rejection flow. A CTO who says "send Dev back to Architect" issues a typed action; the router:
...
Per [Forge AI-50 spec §6.3](/Forge AI/issues/Forge AI-50#document-spec):
...
| Operator cancels a pending approval       | `UPDATE agent_run_approvals SET status='rejected', decided_by=operator`; the run transitions to `aborted` per Forge AI-50 spec §2.2. |
...
- **Customer-facing approval queue** in the Forge console. Per the Forge AI-50 spec §9 deferred question, the default is console view only; a future ADR may expose the queue to customers.
...
- [Forge AI-137](/Forge AI/issues/Forge AI-137) — Implement the router per this ADR
