...
| **Issue**         | [Forge AI-50](/Forge AI/issues/Forge AI-50) Sub-goal 0.1 (Master Orchestrator)                            |
| **Sub-task**      | [Forge AI-136](/Forge AI/issues/Forge AI-136) (0.1.3 — Event bus)                                         |
...
[ADR-0001](./adr-0001-master-orchestrator-sdlc-architecture.md) §3 makes the Master Orchestrator the only writer of run state and the event bus the only reader of state changes outside the Orchestrator. The [Forge AI-50 spec §5](/Forge AI/issues/Forge AI-50#document-spec) defines 19 typed events (`run_created`, `stage_started`, `cost_reported`, `invalid_transition`, etc.) that the rest of the platform subscribes to — Audit 0.5, Cost 0.6, Memory 0.4, the Forge console, and future customer webhooks.
...
- The cross-account boundary mechanism (the audit account is a separate AWS account per [Forge AI-36 ADR](/Forge AI/docs/adr/0001-audit-system-one-way-doors.md) D3; the bus must cross that boundary without violating the IAM isolation).
...
- `<event_type>` — one of the 19 typed events from Forge AI-50 spec §5.1 (`run_created`, `stage_started`, `cost_reported`, `invalid_transition`, etc.).
- `<major>` — major version. The Orchestrator publishes `v1` for all current events. A breaking change emits `v2` on a new subject and keeps `v1` for 30 days (per Forge AI-50 spec §5.2).
...
`event_id` is generated by the Orchestrator as `uuidv7()`; consumers dedupe on it (Forge AI-50 spec §5.2).
...
Per [Forge AI-36 ADR D3](/Forge AI/docs/adr/0001-audit-system-one-way-doors.md) the audit store lives in a separate AWS account. The platform account has no read or admin access to it. The bridge is the **only** cross-account writer.
...
- A DLQ captures SQS delivery failures; an alert pages on `> 0` after a 5-minute grace period (the bridge normally drains in < 60 s p99 per Forge AI-50 spec §5.1 acceptance).
...
- The Orchestrator must not have AWS credentials to the audit account (Forge AI-36 D3 — the audit account is the trust boundary).
...
- **Two systems to operate** (NATS + the bridge). We accept this in exchange for the cross-account boundary; a single system would force us to either weaken the boundary or move the bus into the audit account (which the Forge AI-36 ADR explicitly forbids).
...
- **End-to-end encryption of event payloads** beyond TLS-in-transit. The events carry `tenant_id`; payload-level encryption is per-customer as part of the secrets ADR (Forge AI-38).
- **Replay UI** for the Forge console. Part of [Forge AI-50 spec §11](/Forge AI/issues/Forge AI-50) (the Orchestrator console), not this ADR.
...
- [Forge AI-136](/Forge AI/issues/Forge AI-136) — Implement event bus per this ADR (blocked on spec acceptance)
- [Forge AI-32](/Forge AI/issues/Forge AI-32) — Memory agent subscribes per [Forge AI-50 spec §5.3](/Forge AI/issues/Forge AI-50#document-spec)
- [Forge AI-36](/Forge AI/issues/Forge AI-36) — Audit account subscribes via the SQS+SNS bridge
- [Forge AI-75](/Forge AI/issues/Forge AI-75) — Cost agent subscribes to `cost_reported` + `budget_exceeded`
