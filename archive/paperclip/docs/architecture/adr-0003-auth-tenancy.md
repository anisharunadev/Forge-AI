...
| **Issue**         | [Forge AI-38](/Forge AI/issues/Forge AI-38) — 0.7 Authentication & tenancy |
| **Parent**        | [Forge AI-16](/Forge AI/issues/Forge AI-16) — Epic 0; [ADR-0001](/Forge AI/issues/Forge AI-2) |
| **Supersedes**    | none |
| **Superseded by** | none |
...
| **Sub-agents**    | Forge AI-owned agents (BA, Dev, Security, …)  | Operate inside the tenant they were bound to | Short-lived Forge AI-issued agent tokens |
...
[ADR-0001 §2](/Forge AI/issues/Forge AI-2) calls for "a typed handoff contract" between sub-agents. The hand-off cannot be type-safe until **identity and tenancy are type-safe**: every tool call, every memory read, every MCP request must carry an unforgeable `(agent_id, tenant_id, role, scopes, trace_id)` envelope. ADR-0001 §5 stages forbid the Security agent from sharing context with the Developer agent — that constraint is meaningless without an IAM boundary that *enforces* separation at the tool layer.
...
1. **One identity broker, three principals.** A single `identity-broker` service terminates OIDC for board users, mints agent tokens for Forge AI sub-agents, and brokers short-lived cloud tokens for the customer cloud. There is exactly one place to audit, rate-limit, and revoke.
...
6. **Audit is non-negotiable.** Every identity event, scope grant, tenant denial, and secret resolution lands in the append-only audit log (Forge AI-36, 0.5) with `(actor, tenant, action, scopes_used, outcome, trace_id)`.
...
- **Token shape is Forge AI's, not the IdP's.** The IdP token is exchanged for a Forge AI-issued access token whose claims we control (`tenant_id`, `roles`, `scopes`, `aud`, `trace_id`).
...
3. IdP returns the OIDC `id_token` + `code`. Broker verifies, looks up or provisions the `board_user` row, evaluates tenant policy (does this user have a seat? what role?), and issues a Forge AI-issued session cookie + access token.
4. UI presents the user's tenant. Every API call carries the Forge AI access token, never the IdP token.
...
| **LLM context**         | The Knowledge Layer is *materialised* per tenant at load time (see [Forge AI-103 0.8](/Forge AI/issues/Forge AI-103)); a board user in tenant A cannot pull from tenant B's `customer/` files | The model is the softest layer — see §4.4 |
...
The customer never grants Forge a long-lived key. Instead, the customer grants Forge an **OIDC federation trust** (AWS IAM Role with `WebIdentity` from the broker; Azure Workload Identity; GCP Workload Identity Federation). The broker then exchanges a Forge AI-issued token for a cloud-native short-lived token per action.
...
- **No secret in code, ever.** Lint rule, pre-commit `gitleaks`, CI secret-scan — all from [engineering standards §5](/Forge AI/docs/engineering/standards.md).
...
### 8.1 Audit (handoff to [Forge AI-36 0.5](/Forge AI/issues/Forge AI-36))
...
### 8.2 Cost (handoff to [Forge AI-75 0.6](/Forge AI/issues/Forge AI-75))
...
## 9. Acceptance criteria (mapped to Forge AI-38 §Acceptance)

| Forge AI-38 criterion | How this ADR satisfies it |
|---|---|
