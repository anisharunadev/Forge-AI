...
| **Reviewer**     | CEO                                                                                       |
| **Issue**        | [Forge AI-12](/Forge AI/issues/Forge AI-12)                                                          |
...
The acceptance criteria in [Forge AI-12](/Forge AI/issues/Forge AI-12) are: (a) merged in `/docs/architecture/adr-0002-memory-store.md` with CTO + CEO sign-off, (b) a one-page diagram that fits the company goal document, and (c) a worked example showing a fact flowing `write → curate → inject per stage → expire`.
...
This is the question that came up in the Forge AI-12 scoping. The rule:
...
- The Cost agent's pricing model and per-project budget algorithm (Forge AI roadmap).
- The Approval engine UX and routing (Forge AI roadmap).
- The Security agent's threat model framework (Forge AI roadmap).
- The Evaluation agent's measurement framework for recall precision (Forge AI roadmap).
- The Self-Healing Test agent's mutation strategy (Forge AI roadmap).
