...
**Sub-goal:** [FORA-60](/FORA/issues/FORA-60) — Sub-goal 1.2 of [FORA-17](/FORA/issues/FORA-17) (Epic 1)
**Subject of analysis:** Forge Ideation Agent · Requirement Ingestion (Sub-goal 1.1)
**Inputs:** [Draft PRD](/FORA/issues/FORA-59) (FORA-59), [requirement_brief.json](/FORA/issues/FORA-59) (FORA-59), `workspace/memory/*.md`, `workspace/project/*.md`, `workspace/customer/*.md`, FORA monorepo (`apps/`, `packages/`, `mcp-servers/`, `docs/architecture/`, `.github/workflows/`)
**Recommendation:** **GO** — ship Sub-goal 1.1 on the v1 timeline, conditional on Q1, Q2, Q5 resolution before architecture handoff. Pivot signal: escalate if Atlassian ships a comparable idea → PRD flow with audit inside Jira Product Discovery before FORA's first design partner is signed.
...
Does the Forge Ideation Agent — Sub-goal 1.1, Requirement Ingestion — have a defensible market position AND a buildable technical foundation to ship on the Q3 2026 milestone, given the current FORA platform state? This memo says **yes**, with two sequencing conditions and one watch-list item.
...
- **Tech debt:** Can the existing FORA monorepo host a customer-facing AI agent without dragging v1 into a refactor of the orchestrator, MCPs, or audit surface?
- **Risk:** What blocks v1, and what is a v1.1 / v2 problem?

---

## 3. Target customer (per [FORA-59 §3](/FORA/issues/FORA-59))
...
- **GitHub Copilot Workspace** — sits at the Dev stage (Issue → PR), not the Ideation stage. They are a FORA *downstream* candidate, not a competitor.
...
FORA's Forge Ideation Agent wins on **three** dimensions and loses on **one**. (Per the 2–3 differentiators bar — we surface four and pick the strongest three for the executive read.)

| Differentiator | FORA wins on | Why competitors don't match |
|----------------|---------------|------------------------------|
...
| **Auditable AI actions + tenant isolation** — every Knowledge Layer read, MCP call, prompt/response (when `FORA_LOG_LLM=1`) is logged under `run_id`, queryable by `tenant_id` | Yes | The CISO wedge. None of the competitors carry a SOC 2-grade agent audit log per design-partner tenant. |
| **Knowledge Layer grounding** — the Draft PRD is grounded in the customer's `workspace/memory/`, `workspace/customer/`, `workspace/project/`, not in an LLM's priors | Yes | ChatPRD and Atlassian Intelligence generate from generic templates. FORA reads the customer's actual standards, conventions, glossary, and ADRs. |

**Where FORA loses:** brand recognition and install-base density vs Atlassian. Atlassian has 300k+ Jira customers; FORA has zero design-partner contracts at this writing. This is a **go-to-market** risk, not a product risk.

### 4.2 Risks

1. **Atlassian bundling** — if Atlassian ships "Idea → PRD with audit" inside Jira Product Discovery + Atlassian Intelligence before FORA's first design partner is signed, the Jira wedge narrows. **Mitigation:** Knowledge Layer grounding + end-to-end SDLC closure is hard to replicate inside Atlassian; the DevOps stage in particular (container build → deploy → runbook) is not a Jira-shaped problem. **Owner:** CEO + CTO. **Watch:** Atlassian Summit announcements, Jira Product Discovery release notes.
...
## 5. Product surface — what changes in FORA if 1.1 ships

(Per FORA-59 §5; this memo does not re-litigate the PRD.)
...
(Per FORA-59 §6; ownership is unchanged.)
...
## 8. Success metrics — inherited from FORA-59 §8
...
Guardrails (numbers retained from FORA-59 §8): time-to-Draft-PRD p50 ≤ 5 min, p95 ≤ 10 min; cost p50 ≤ $0.50, p95 ≤ $3.00, hard ceiling $5.00; ambiguity-surfacing rate ≥ 95 %; Knowledge Layer read-set compliance 100 %; tenant isolation 100 %; audit-log completeness 100 %; open-question close rate ≥ 90 % within 7 days.
...
| LLM cost / latency creeps above $3 p95 | Medium | Medium | Orchestrator-enforced per-run budget; prompt cache; per-tenant token cap | CTO | §4.2, FORA-59 §8 |
...
| `requirement_brief.json` schema drifts | Low | Medium | Schema-versioned; ADR-gated breaking changes | CTO | FORA-59 §9 |
| Board never responds to `ask_user_questions` | Medium | Medium | 7-day auto-park + CTO page | CTO | FORA-59 §9 |
...
These are inherited from [FORA-59](/FORA/issues/FORA-59) §10 and the CTO resolution captured in [FORA-17 comment `74a3de28`](/FORA/issues/FORA-17#comment-74a3de28).

| ID | Question | Owner | Blocks | Due |
|----|----------|-------|--------|-----|
| **Q1** | Trigger semantics — real customer one-line vs FORA-self-dogfood? | CEO | FORA-60, FORA-61, FORA-62 | 2026-06-20 |
| **Q2** | MCP read-set for 1.1 — Jira only (stubbed others) vs wait for priority-1 set? | CTO | FORA-60 | 2026-06-18 — **CTO-resolved**: Jira + Confluence for v1; Zendesk + Slack stubbed `not_wired` until Epic 9 ships them. No brief rewrite. |
...
1. **Q1 + Q5 must close before 1.2 → 1.3 hand-off** — the trigger-semantics answer shapes whether the Epic in 1.3 targets the FORA-self-dogfood scenario or the customer scenario, and the reassignment question shapes whether 1.3 auto-resumes. Both are CEO-owned and due 2026-06-20.
...
3. **The composite tech-debt score is 43 / 100**, below the 70 escalation threshold. **No Security agent escalation is required** for FORA-60. The Security hand-off items (Q6, Q7, the secrets-client gap, the prompt-injection eval set) are tracked above and will be re-scored when Epic 5 (Security) is unblocked.
...
**Next sub-goal on success:** [FORA-61](/FORA/issues/FORA-61) (Sub-goal 1.3 — Epic generator) is currently `blocked` on FORA-60. On this memo's sign-off, FORA-61 auto-resumes (per the `blockedBy` linkage).

---

## 13. Related

- Draft PRD — [FORA-59](/FORA/issues/FORA-59) (Sub-goal 1.1)
- Parent epic — [FORA-17](/FORA/issues/FORA-17) (Epic 1 — Forge Ideation Agent)
- BMad workflow — [FORA-15](/FORA/issues/FORA-15) (done)
- Plan of record — [FORA-15 plan](/FORA/issues/FORA-15#document-plan)
- Next sub-goal — [FORA-61](/FORA/issues/FORA-61) (Sub-goal 1.3)
