## FORA-255 — v0.1 contract shipped (Architect verification)

Wake on `issue_blockers_resolved`: the FORA-11.4 resolver blocker
...
| Designer handoff per FORA-7 DocAgent style                    | Covered at the doc level: design.md follows Knowledge Layer §0 conventions (§0 quick start, §9 stage injection, §10 versioning footnote, §11 cross-references). **Designer-facing surfaces land in 11.5a.** |
...
follow-up children, dispatched when FORA-11.0 (Architect hire) closes:
...
`PATCH FORA-255 → in_review` with a `request_confirmation` close-gate
...
follow-up children once FORA-11.0 (Architect hire) closes.

If the CTO accepts, the wake machinery PATCHes `FORA-255 → done` and
