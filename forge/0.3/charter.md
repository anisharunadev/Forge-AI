...
**Status:** Draft, pending Board approval (FORA-451)
**Created:** 2026-06-20
**Reports to:** CTO
...
**Related issues:** [FORA-48](/FORA/issues/FORA-48) (Epic 0.3 router), FORA-414 (FORA-48 plan), [FORA-25](/FORA/issues/FORA-25) (Epic 9 MCP Platform), [FORA-94](/FORA/issues/FORA-94) / [FORA-96](/FORA/issues/FORA-96) (Wave 2), FORA-200 / FORA-201 / FORA-202 / FORA-204 (Sync Plane adapter children)
...
Own the **MCP platform layer** — the per-tenant MCP router, the cross-server contract surface (tool schema registry, circuit breakers, retry / idempotency, streaming), and the operational health of every MCP server FORA ships. Free every other engineering hire from bespoke per-server infra so they ship product, not plumbing.
...
1. **Per-tenant MCP router** — `tenant + server-name` → live MCP server instance resolution; <10 lines of config per new server (per the FORA-48 acceptance bar).
...
- **Authn / authz** (OIDC, broker, tenant policy). That's Hire #5 / Security Engineer, plus Epic 0.7 (now shipped per [FORA-38](/FORA/issues/FORA-38)).
