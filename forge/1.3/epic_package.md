...
**Stage:** Ideation (sub-goal 1.3 of [FORA-15](/FORA/issues/FORA-15) → [FORA-17](/FORA/issues/FORA-17))
**Issue:** [FORA-61](/FORA/issues/FORA-61) — Sub-goal 1.3 — Epic generator (Epic + stories + acceptance criteria)
**Producer:** `ba-agent` (Hire #3, BMAD PM + BA, reports to CTO) — `99b34c5d-87d4-42a0-a66a-c65a916aeeec`
**Generated at:** 2026-06-17
**Inputs consumed:**
- [Draft PRD — FORA-59](/FORA/issues/FORA-59) (`forge/1.1/draft_prd.md`, attached as `ad8b9a31-54b9-4612-ac74-2da832085f4b`)
- [Requirement brief — FORA-59](/FORA/issues/FORA-59) (`forge/1.1/requirement_brief.json`, attached as `791378bc-129a-4f81-b151-9a2d74f5f07e`)
- [Feasibility memo — FORA-60](/FORA/issues/FORA-60) (`forge/1.2/feasibility_memo.md`, attached as `e0a79ebb-5d0c-4dba-82ab-ebbbc4ab3bfa`)
- [Tech-debt score — FORA-60](/FORA/issues/FORA-60) (`forge/1.2/tech_debt_score.json`, attached as `aa4b9204-0130-4e9c-85a2-3b1df2537491`)
...
**Board gate required:** yes — sub-goal 1.3 exit is gated on board approval per FORA-15 §3.3 and FORA-17 description.
...
| **Consumer** | Architect (Sub-goal 2.1, [FORA-27](/FORA/issues/FORA-27)) takes the approved Epic and writes the Architecture design. |
| **Pushed to Jira by** | Sub-goal 1.4 ([FORA-62](/FORA/issues/FORA-62)) once the board approves this Epic package. |
| **Success metric (inherited)** | North star: ≥ 80 % of Draft PRDs pass structural lint on first try and advance to Architect without rework (per [FORA-59 §8](/FORA/issues/FORA-59)). |
...
**Out of scope (v1 of this Epic, FORA-59 §7):**
...
- Generating the Jira Epic or pushing to Jira (handled by FORA-61 itself + FORA-62).
...
- **And** `links.parent_epic == "FORA-17"` and `links.draft_prd == "draft_prd.md"`
- **And** the brief is attached to FORA-59 (in 1.1) and is referenced by `parentId` on this Epic's stories.

**Dependencies:** S2, S3
**Effort:** M
**Risk:** medium — schema drift is a known failure mode (FORA-59 §9 risk #7); every schema change requires an ADR, downstream stages pin to a specific version.

---

### S5 — `draft_prd.md` Generator

**Description:** Produce a Draft PRD aligned to the 11 sections of `workspace/project/PRD.md`: Vision, Problem, Target customer, Core value propositions, Product surface, Key user journeys, Non-goals, Success metrics, Risks and mitigations, Open questions, Related. Lint step: every section non-empty; every `partial` section paired with an `open_question_id`; every success metric has `number + baseline + target`; every internal reference is a clickable `/FORA/issues/<id>` link per the Paperclip comment-style rule.
...
- **And** every ticket reference is a Markdown link with the `FORA-` prefix
...
- **And** when `FORA_LOG_LLM=1`, every prompt/response pair is captured with the same `run_id`
...
**Effort:** M
**Risk:** high — audit completeness is a P0; missing data is unrecoverable. The audit log is the CISO wedge (FORA-60 §4.1 differentiator #2).

---

### S8 — Per-Tenant Isolation + Per-Run Budget Enforcement

**Description:** Enforce per-tenant namespace on every read and MCP call — a run for tenant A must never read tenant B's Knowledge Layer or MCP data. Enforce the per-run budget: **p50 ≤ $0.50, p95 ≤ $3.00, hard ceiling $5.00** (per FORA-59 §8). Above the ceiling, the run halts and the CTO is paged. Both controls are enforced at the orchestrator layer, not at the agent prompt.
...
|----|----------|-------|--------|---------------------|
| Q1 | Trigger semantics — real customer vs FORA-self-dogfood? | CEO | FORA-60, FORA-61, FORA-62 | Open, due 2026-06-20. This Epic is trigger-agnostic; the *first* Epic shipped will be one or the other. |
| Q2 | MCP read-set for 1.1 | CTO | FORA-60 | **Resolved** (per [FORA-17 comment `74a3de28`](/FORA/issues/FORA-17#comment-74a3de28)): Jira + Confluence in v1; Zendesk + Slack stubbed `not_wired`. **S3 updated to reflect Confluence.** |
...
| Q5 | Owner reassignment 1.2–1.4 | CEO | none (process) | Open, due 2026-06-20 — does not affect this Epic, but answers who closes FORA-62. |
...
## 7. Acceptance criteria for this Epic package (per FORA-61 description)
...
- [ ] `request_confirmation` interaction posted; status of FORA-61 → `in_review`; comment names what the board must decide.
- [ ] On board approval → issue moves to `done`; FORA-62 unblocks automatically.
...
- Parent Epic — [FORA-17](/FORA/issues/FORA-17) "Epic 1 — Forge Ideation Agent"
- Draft PRD (input) — [FORA-59](/FORA/issues/FORA-59) (Sub-goal 1.1)
- Feasibility memo (input) — [FORA-60](/FORA/issues/FORA-60) (Sub-goal 1.2)
- BMad workflow — [FORA-15](/FORA/issues/FORA-15) (done)
- Next sub-goal — [FORA-62](/FORA/issues/FORA-62) (Sub-goal 1.4 — Jira sync, currently blocked on this issue)
...
|-----|------|--------|--------------|
| v1.0 | 2026-06-17 | BA (`99b34c5d-87d4-42a0-a66a-c65a916aeeec`) | Initial Epic package — 8 stories decomposed from the Draft PRD's product surface, aligned to FORA-17's sub-goal scope. |