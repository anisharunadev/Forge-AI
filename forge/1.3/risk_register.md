...
| R4.2 | Open questions without `owner` or `blocks` field (per FORA-59 §10 bar) | M | M | P2 | Lint step rejects briefs with `null` fields in `open_questions[]` | Dev | Lint fails > 5 % of runs |
...
| R5.4 | Internal ticket references left as bare ids (e.g., `FORA-61`) instead of clickable links | L | L | P3 | Lint step: regex match for `FORA-\d+` patterns not wrapped in `[...](/FORA/issues/...)` | Dev | Bare id detected |
...
| ID | Risk | L | I | Sev | Mitigation | Owner | Trigger to escalate |
|----|------|---|---|-----|------------|-------|---------------------|
| **R7.1** | **Audit-log completeness < 100 % — a required entry is missing.** This is the CISO wedge (FORA-60 §4.1 differentiator #2); missing data is unrecoverable. | **L** | **C** | **P0** | **Daily audit sample (n=10 random runs); missing entry is a P0 and pages the Security agent immediately. Audit-sink write path is fail-closed (write fails → run halts, not "swallow the failure").** | **Security (Epic 5)** | **Any missing entry in the daily sample — page on-call** |
...
| R7.3 | Prompt/response capture (`FORA_LOG_LLM=1`) accidentally captures secrets | L | H | P0 | Secret-detection layer runs before write; if a prompt/response matches a secret pattern, the entry is redacted and a `REDACTION_APPLIED` audit entry is written | Security (Epic 5) | Secret detected in prompt/response — page on-call |
...
| R-X2 | Atlassian ships "Idea → PRD + audit" inside Jira Product Discovery before FORA's first design partner is signed | M | H | P1 | Epic-level | Knowledge Layer grounding + end-to-end SDLC closure is hard to replicate inside Atlassian; watch Atlassian Summit + Jira Product Discovery release notes | CEO + CTO |
...
| R-X6 | The "first design partner" scenario is too narrow to validate the agent end-to-end | M | M | P2 | Epic-level | Q3 2026 milestone #7: "First design partner runs a real feature end-to-end." Schema designed for that scenario, not for FORA-internal dogfood. | PM + CEO |
...
- [Draft PRD §9 Risks](/FORA/issues/FORA-59) — Epic-level risks (inherited)
- [Feasibility memo §10](/FORA/issues/FORA-60) — consolidated risks (inherited)
