...
- Per-tenant+service `TokenBucket` + `CircuitBreaker` (FORA-126.5).
...
- Per-tenant IdP config in `tenant_idp_configs` (out of scope for FORA-35; documented in ADR-0003 + identity ADR).
- Per-tenant webhook secret (HMAC-SHA256, `X-IdP-Signature: sha256=<hex>`, `tenant_webhook_secrets` map — see FORA-161).
...
- Subscription map: `(tenant_id, event_type) → topic_arn` in `bridge_subscriptions` (out of scope for FORA-35).
