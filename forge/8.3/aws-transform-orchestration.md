# FORA-84 / Sub-goal 8.3 — AWS Transform Orchestration

> **Status:** v0.1 — `wave-planner/0.1.0` (schema v1)
> **Source:** `agents.refactor.plan_waves` (FORA-84 / 8.3)
> **Upstream:** [FORA-82](/FORA/issues/FORA-82) (8.1 `MigrationScope`) + [FORA-83](/FORA/issues/FORA-83) (8.2 `DependencyGraph`)
> **Downstream:** [FORA-85](/FORA/issues/FORA-85) (8.4 migration planner + Jira)
...
- A per-wave **command list** that names the existing FORA seams (CCB v1 dispatch, secrets MCP, canary probe, MCP servers). v0.2 will execute that command list through those seams; v0.1 only plans it.

The orchestrator does **not**:

- Rebuild AWS MGN, DMS, Migration Hub, or Refactor Spaces logic.
- Reimplement the trust / OIDC federation already in `apps/customer-cloud-broker/` (FORA-126).
- Reimplement the per-service AWS dispatch already in `apps/customer-cloud-broker/` `dist/` (FORA-126.5).
- Reimplement the canary probe already in `apps/customer-cloud-broker/probe-signer.ts` (FORA-194).
- Reimplement the secrets-fetch already in `mcp-servers/secrets/` (FORA-128).
...
- FORA-82 (8.1) — `analyze_scope` ships `MigrationScope` with categorisations, transform mappings (unit + tier), and risk assessments. v0.1 of that sub-goal is `in_review` per memory; we treat it as the contract the orchestrator consumes.
- FORA-83 (8.2) — `build_graph` ships `DependencyGraph` with fan-in / fan-out / blast-radius / Tarjan SCC cycles / service-level graph / tightly-coupled clusters. The wake for FORA-84 (`issue_blockers_resolved`) cleared when FORA-83 reached the gate. Orchestration without the graph would force us to guess at cycles and clusters — the same anti-pattern that 8.1's deterministic mapper was designed to eliminate.
...
`kind` is closed: `canary_probe` (FORA-194), `compile` (run the project's build), `unit_test` (run the smoke test), `lint`, `dep_check`, `secret_rotate_check` (FORA-128), `audit_completeness_check` (FORA-36). v0.1 emits the gate list; v0.2 wires the executor.
...
`service` is the AWS service the command targets. `action` is the AWS API call. `params` reference upstream values via `${repo.*}` or `${tenant.*}` (the broker resolves at execution). `via` names the FORA seam that executes it. v0.1 emits; v0.2 executes through the seam.
...
1. Run the canary probe (`apps/customer-cloud-broker/probe-signer.ts`, FORA-194) before the wave fires.
2. Run a `secret_rotate_check` (FORA-128) before any wave that touches RDS / Aurora credentials.
3. Run a `audit_completeness_check` (FORA-36) after the wave completes.
...
| Wave kind | Target AWS service(s) | FORA seam |
| --- | --- | --- |
...
This is the seam-by-seam proof that "orchestrate, do not rebuild" is honoured: every wave's execution path is a FORA seam that already exists.
...
- A `WaveGate.secret_rotate_check` referencing `mcp-servers/secrets/` (FORA-128). The orchestrator emits the gate; v0.2 runs it.
- A `WaveCommand.audit_action` referencing the canonical `agents/audit/schema.py` event name (FORA-36). The CCB audit emitter (`apps/customer-cloud-broker/audit.ts`) writes the event.
...
Per-tenant isolation is preserved by the existing CCB trust model (FORA-126 §"OIDC federation broker" + the per-tenant deny-list). The orchestrator emits `WaveCommand.params.${tenant.*}` placeholders; the CCB resolves them against the tenant's broker context.
...
## 10. Hand-off to FORA-85 (8.4 migration planner + Jira)
...
The migration planner does **not** call AWS. It calls the Jira MCP (shipped via FORA-25) and writes stories whose AC text references the `audit_action` from each wave. The FORA-85 sub-goal inherits the same "orchestrate, do not rebuild" posture — it orchestrates Jira through the MCP, never rebuilds Jira fields locally.
...
- **Per-tenant cost ceilings** — FORA-204 sync-plane R-X7 owns the cost-attribution work; 8.3 emits the right `audit_action` so cost events can be attributed per wave, but the per-tenant budget gate is owned by the sync-plane risk register.
...
- [x] Every wave's `target_aws_services` maps to a CCB dispatch seam that already exists (FORA-126, FORA-126.5, FORA-194, FORA-128).
