# FORA-128 v0 — CEO review checklist

This is the one-pager the CEO sees when responding to the pending
`request_confirmation` (id `9c1235a6`) on FORA-128. It is also the
...
**If you (the CEO / Board) confirm**, the CTO will PATCH FORA-128 to
`done` and FORA-211 (the auth-engineer follow-up child) will unblock
...
> **FORA-128 v0 — 271 checks green. 7 of 7 children `done`. 1
> post-v0 follow-up filed. 1 confirmation pending (this one).**

## Acceptance criteria (from the FORA-128 epic)
...
| 3 | `gitleaks` pre-commit + CI secret-scan, regression-tested by a known-bad fixture | `.gitleaks.toml`; `.github/workflows/ci.yml` (gitleaks-action + regression step); `tools/gitleaks/gitleaks-fixture.test.mjs` (4 cases) | ✅ (CI-only; per-developer pre-commit hook is a follow-up — see FORA-211 §5) |
...
| 5 | Memory dump shows no raw value (property test) | `test/property/memory-dump.mjs` (200 fuzz runs + 100 grammar round-trips) | ✅ (in-memory variant; AWS SDK buffer second pass is FORA-211 §4) |

## Deliverables (from the FORA-128 epic)
...
- [x] Per-tenant backing-store configuration — `src/config.ts`; v1 `aws-secrets-manager` shipped (FORA-185)
- [x] Pre-commit `gitleaks` + CI secret-scan; lint rule for `secrets:` blocks — `.gitleaks.toml` + `tools/lint-secrets-blocks.mjs`
- [x] Audit events: `secret.resolved`, `secret.rotated`, `secret.access_denied`, `secret.used_for_<intent>` — `src/broker.ts::SecretAuditEvent` + `src/audit-fora.ts::ForaAuditSink`

## Children — all `done`

| Issue | Title | Tests |
|-------|-------|-------|
| [FORA-185](https://paperclip/FORA/issues/FORA-185) | AWS Secrets Manager adapter | 6 unit |
| [FORA-186](https://paperclip/FORA/issues/FORA-186) | FORA-36 audit-sink forwarder | 8 unit |
| [FORA-187](https://paperclip/FORA/issues/FORA-187) | `secrets:` block lint rule | 7 lint self-tests |
| [FORA-188](https://paperclip/FORA/issues/FORA-188) | gitleaks pre-commit + CI | 4 regression |
| [FORA-189](https://paperclip/FORA/issues/FORA-189) | Broker-side raw-use MCP wrapper | 7 unit |
| [FORA-190](https://paperclip/FORA/issues/FORA-190) | Memory-dump property test | 200 fuzz + 100 grammar |
| [FORA-191](https://paperclip/FORA/issues/FORA-191) | Operator runbook + docs handoff | docs-only |
...
| `tools/lint-secrets-blocks.test.mjs` | 7 checks green | self-test for FORA-187 |
| `tools/gitleaks/gitleaks-fixture.test.mjs` | 4 checks green | self-test for FORA-188 |
...
- ✅ `lint-unbound-mcps` (existing; FORA-125)
- ✅ `lint-secrets-blocks` (FORA-187) + self-test
- ✅ `gitleaks` via `gitleaks/gitleaks-action@v2` (existing) + `gitleaks-fixture.test.mjs` (FORA-188 regression)
- ✅ TypeScript typecheck, Python lint/format/mypy (existing)
- ✅ Tenancy lint, agent-IAM lint (existing)

## Post-v0 follow-ups (filed in FORA-211)
...
See [FORA-211](https://paperclip/FORA/issues/FORA-211) for the full scope.

## Risk

Every change is additive behind env-var defaults:
- `FORA_BACKING_STORE=memory` (default for tests) is unchanged.
- `FORA_AUDIT_SINK=memory` (default for tests) is unchanged.
...
4. The 4 follow-up stubs in `src/brokered.ts` are deterministic; the real wiring lives in FORA-211.
