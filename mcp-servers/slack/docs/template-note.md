# Template note — FORA MCP servers and what `@fora/mcp-slack` copies from

This package is the **first chat/IM MCP server** in the FORA priority-1 set. It is built by copy-from-template of `@fora/mcp-github` ([FORA-4](/FORA/issues/FORA-4), [template note](../github/docs/template-note.md)). The seven contract points in the GitHub template-note are mandatory and non-negotiable; this server preserves all of them.
...
| `@fora/mcp-slack` | P1 | shipped (this issue) | **Workspace-pinned** (Slack `team_id`): `SLACK_TEAM_ID` + `SLACK_BOT_TOKEN`; chat surface (channels, threads, messages, reactions) instead of repo/PR surface. No Slack SDK — direct `fetch` calls to `slack.com/api` mirror the Jira server's SDK-free stance. `post_message` and `update_message` require `confirm: true` as a human-in-the-loop gate (FORA-5 §5.2). |
